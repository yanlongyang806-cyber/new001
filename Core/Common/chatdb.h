...
typedef struct ChatUser
{
	...
	PlayerInfoStruct ** ppPlayerInfo; AST_NOT(PERSIST) AST(NO_NETSEND NO_TEXT_SAVE NO_INDEX) // List of characters this account has online
	...
	U32 uGatewaySessionRefCount; AST_NOT(PERSIST) AST(NO_NETSEND NO_TEXT_SAVE)
	...
	AST_COMMAND("Set Access Level", "ForceSetChatAccessLevel $FIELD(id) $INT(ALvl)")
} ChatUser;
AST_PREFIX();
