...
int channelIsNameValid(const char *channel_name, bool adminAccess, int flags)
{
	...
	else if (strStartsWith(channel_name, TRADE_CHANNEL_NAME) && !(flags & CHANNEL_SPECIAL_TRADE))
		retVal = CHATRETURN_CHANNEL_RESERVEDPREFIX;
	else if (strStartsWith(channel_name, LFG_CHANNEL_NAME) && !(flags & CHANNEL_SPECIAL_LFG))
		retVal = CHATRETURN_CHANNEL_RESERVEDPREFIX;
...
Watching *channelFindWatchingReservedByType(ChatUser *user, int reserved_flag)
{
	...
	else if (reserved_flag == CHANNEL_SPECIAL_TRADE)
		channel_prefix = TRADE_CHANNEL_PREFIX;
	else if (reserved_flag == CHANNEL_SPECIAL_LFG)
		channel_prefix = LFG_CHANNEL_PREFIX;
...
const char *convertExactChannelName (const char *channelName, U32 uReservedFlags)
{
	...
	} else if (uReservedFlags & CHANNEL_SPECIAL_TRADE)
	{
		return TranslateMessageKeyDefault("ChatServer_Special_" CHAT_TRADE_SHORTCUT, CHAT_TRADE_SHORTCUT);
	} else if (uReservedFlags & CHANNEL_SPECIAL_LFG)
	{
		return TranslateMessageKeyDefault("ChatServer_Special_" CHAT_LFG_SHORTCUT, CHAT_LFG_SHORTCUT);
	...
	{
		if (stricmp(channelName, UGCEDIT_CHANNEL_NAME) == 0)
		{
			return TranslateMessageKeyDefault("ChatServer_Special_" UGCEDIT_CHANNEL_SHORTCUT, UGCEDIT_CHANNEL_SHORTCUT);
		}
		else
		{
