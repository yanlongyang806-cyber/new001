...
// used for all friend roster changes (request, auth, remove)
AUTO_STRUCT;
typedef struct XMPP_NotifyFriendResponse
{
	const char *fromHandle;
	const XMPP_ClientStateList *stateList;
} XMPP_NotifyFriendResponse;

// XMPP Escaping and Unescaping
// http://xmpp.org/extensions/xep-0106.html#escaping for supporting legacy XMPP JID authentication
// To handle clients that do not allow separate input of username and domain for SASL authentication
