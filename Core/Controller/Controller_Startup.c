...
static StatusReporting_SelfReportedState sStartupState = STATUS_UNSPECIFIED;
...
void ControllerStartup_AddMachine(char *pMachineIdentifier, bool bMachineIsRequired, char *pWhyRequired)
{
	...
	//if localhost is also specified by name, ignore that
	if (stricmp(pMachineIdentifier, getHostName()) == 0)
	{
		return;
	}
...
bool ControllerStartup_ConnectionStuffWithSentryServer(char **ppErrorString)
{
	...
	sStartupState = STATUS_CONTROLLER_STARTUP;
	...
	estrPrintf(&pFullCommandLine, "%s\\patchclient -sync -project CrypticTools -root C:/Night/tools -cleanup", pPatchingDir);
	loadstart_printf("Updating C:/Night/tools (%s)", pFullCommandLine);
	...
	for (i=0; i < eaSize(&shardSetupList.ppMachines); i++)
	{
		...
		for (j=0; j < eaSize(&shardSetupList.ppMachines[i]->ppSettings); j++)
		{
			if (shardSetupList.ppMachines[i]->ppSettings[j]->eSetting == CAN_LAUNCH_SPECIFIED)
			{
				...
				if (!gServerTypeInfo[shardSetupList.ppMachines[i]->ppSettings[j]->eServerType].pOneMachineSpecifiedInShardSetupFile)
				{
					gServerTypeInfo[shardSetupList.ppMachines[i]->ppSettings[j]->eServerType].pOneMachineSpecifiedInShardSetupFile = strdup(shardSetupList.ppMachines[i]->pMachineName);
				}
			}
	...
	sStartupState = STATUS_CONTROLLER_PATCHING;
	...
	sStartupState = STATUS_CONTROLLER_POST_PATCH_STARTUP;
	...
	sStartupState = STATUS_UNSPECIFIED;
...
StatusReporting_SelfReportedState ControllerStartup_GetState(void)
{
