...
AutoSetting_SingleSetting *CreateAndAddFutureSetting(char *pSettingName, char *pSettingValue, char **ppOutError);
...
typedef struct ControllerAutoSetting_LoadingWidget
{
	...
	//special case... future settings can be assigned container types, which you do when you've added an auto_setting in a frankenbuilt
	//exe type.
	int *pTemporaryServerTypes; AST(SUBTABLE(GlobalTypeEnum))
...
void ControllerAutoSetting_DumpAllToString(char **ppOutString)
{
	...
	for (i=0; i < eaSize(&sppSettings); i++)
	{
		...
		if (pSetting->eOrigin == ASORIGIN_FUTURE && ea32Size(&pSetting->pServerTypes))
		{
			int j;
			estrConcatf(ppOutString, "\n{\n");

			for (j = 0; j < ea32Size(&pSetting->pServerTypes); j++)
			{
				estrConcatf(ppOutString, "\tTemporaryServerTypes %s\n", GlobalTypeToName(pSetting->pServerTypes[j]));
			}
			estrConcatf(ppOutString, "}\n\n");
		}
		else
...
//						sMainControllerAutoSettingFileName, pSetting->pCmdName, pSetting->pCurValueString);
	...
	FOR_EACH_IN_EARRAY(sppDeferredLoadingWidgets, ControllerAutoSetting_LoadingWidget, pWidget)
	{
		if (pWidget->bFuture)
		{
			...
			AutoSetting_SingleSetting *pNewSetting = CreateAndAddFutureSetting(pWidget->pName, pWidget->pValue, &pErrorString);
			...
			{
				if (pWidget->pTemporaryServerTypes)
				{
					ea32Copy(&pNewSetting->pServerTypes, &pWidget->pTemporaryServerTypes);
				}
			}
...
AUTO_COMMAND;
char *SetTemporaryServerTypesForFutureSetting(char *pSettingName, ACMD_SENTENCE pServerTypes)
{
	AutoSetting_SingleSetting *pSetting;
	char **ppServerTypeNames = NULL;
	static char *spErrorString = NULL;

	if (!stashFindPointer(sSettingsByName, pSettingName, &pSetting))
	{
		return "No setting found by that name";
	}

	if (pSetting->eOrigin != ASORIGIN_FUTURE)
	{
		return "That's not a FUTURE auto setting, can't do this";
	}

	ea32Destroy(&pSetting->pServerTypes);

	estrClear(&spErrorString);

	DivideString(pServerTypes, " ,", &ppServerTypeNames, DIVIDESTRING_STANDARD);

	FOR_EACH_IN_EARRAY(ppServerTypeNames, char, pServerTypeName)
	{
		GlobalType eType = NameToGlobalType(pServerTypeName);
		if (!eType)
		{
			estrConcatf(&spErrorString, "Unknown server type: %s\n", pServerTypeName);
		}
		else
		{
			ea32Push(&pSetting->pServerTypes, eType);
		}
	}
	FOR_EACH_END;
	...
	if (estrLength(&spErrorString))
	{
		estrInsertf(&spErrorString, 0, "One or more errors encountered:");
		return spErrorString;
	}

	return "Server types added!";
