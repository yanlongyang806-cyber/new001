...
ShardInfo_Full *GetShardInfo(void)
{
	...
	locGetImplementedLocaleNames(&pShardInfo->basicInfo.ppLanguages);
...
void SendPerfInfoToControllerTracker(bool bAlsoLog)
{
	...
	gShardPerf.iNumNotResponding = Alerts_GetCountByKey(ALERTKEY_GAMESERVERNOTRESPONDING) + Alerts_GetCountByKey(ALERTKEY_UGCEDIT_GAMESERVERNOTRESPONDING);
	...
	gShardPerf.iRunningSlow = Alerts_GetCountByKey(ALERTKEY_GAMESERVERRUNNINGSLOW) + Alerts_GetCountByKey(ALERTKEY_UGCEDIT_GAMESERVERRUNNINGSLOW);
	...
	//always do lightweight log
	servLog(LOG_CONTROLLER, "MiniPerfLog", "Players %d LoggingIn %d", gShardPerf.iPlayers, gShardPerf.iLoggingIn);
