...
bool gbGameServerInactivityTimeoutMinutesSet = true;
int giGameServerInactivityTimeoutMinutes = 3;
...
//if set, then dump a list of the implemented languages that have actual data to the specified filename and then quit
char *spDumpLocaleNamesAndExitFileName = NULL;
AUTO_CMD_ESTRING(spDumpLocaleNamesAndExitFileName, DumpLocaleNamesAndExitFileName);
...
void PutExtraStuffInLauncherCommandLine(char **ppFullCommandLine, U32 iContainerID)
{
	...
	{
		estrConcatf(ppFullCommandLine, " -SetOverlord %s ", UtilitiesLib_GetOverlordName());
	}
...
static StashTable sMachinesByIP = NULL;
...
TrackedMachineState *GetMachineFromNetLink(NetLink *pLink)
{
	...
	if (pLink)
	{
		IP = linkGetSAddr(pLink);
		bIsLocal = (IP == getHostLocalIp() || IP == LOCALHOST_ADDR);
		iPublicIP = IP;
	}
	else
	{
		iPublicIP = getHostPublicIp();
		IP = getHostLocalIp();
		bIsLocal = true;
	}


	if (bIsLocal && spLocalMachine)
	{
		return spLocalMachine;
	}

	if (stashIntFindPointer(sMachinesByIP, IP, &pMachine))
	{
		return pMachine;
	}
	...
	for (i=0; i < giNumMachines; i++)
	{
		if (bIsLocal && gTrackedMachines[i].bIsLocalHost || IP == gTrackedMachines[i].IP || iPublicIP == gTrackedMachines[i].iPublicIP)
		{
			if (!sMachinesByIP)
			{
				sMachinesByIP = stashTableCreateInt(16);
			}
	
			stashIntAddPointer(sMachinesByIP, IP, &gTrackedMachines[i], false);
...
void StopTrackingServer(TrackedServerState *pServer, char *pMessage, bool bUnnaturalDeath, bool bAlreadyAlerted)
{
	...
	SendStringToCB(CBSTRING_COMMENT, "Server %s on machine %s went away due to: %s", 
		GlobalTypeAndIDToString(pServer->eContainerType, pServer->iContainerID), pServer->pMachine ? pServer->pMachine->machineName : "UNKNOWN",
		pMessage);
...
//	UpdateAlerts();
	...
	if (iCounter % 20 == 0 )
	{
		...
		//don't do our periodic printouts while someone is typing a command into the debug console as it's a massive pain
		if (!ConsoleDebugSomeoneIsTyping())
		{
...
void GrabMachineForShardAndMaybePatch_Internal(char *pMachineName, int iStepNum, bool bKillEverything, bool bPatch, bool bFirstTimeConnectingToThisMachine, char *pPatchJobName)
{
	...
	//if we're not spawning the .exe, then we have to just hope that patchclient.exe lives in the path on that
	//machine
	if (!gbSendPatchClientThroughSentryServer)
	{
		strcpy(patchExecutableFullName, "patchclient.exe");
	}
	else
	{
...
//	char *pSuperEscFailExecuteCmdLine = NULL;
	...
	if (iStepNum == 1)
	{
		...
		if (bPatch)
		{
			SentryServerComm_ExecuteCommandWith64BitFixup_1Machine(pMachineName, "patchclient", pFullCommandLine, pPatchingDir, "MainShardGrab");
		}
		else
		{
	...
	if (iStepNum == 0 && bPatch)
	{
		...
		estrPrintf(&pFullCommandLine, "-forkPrintfsToFile c:\\temp\\patch_output_cryptictools_%u.txt -sync -project CrypticTools -root C:/Night/tools -cleanup",
//			pPatchingDir,
//			patchExecutableFullName,
		...
		SentryServerComm_ExecuteCommandWith64BitFixup_1Machine(pMachineName, "patchclient", pFullCommandLine, pPatchingDir, "CrypticToolsBin");
	}
...
void MaybeRestartIn64BitMode(void)
{
	...
	if (ParseCommandOutOfCommandLine("Launch64BitWhenPossible", launch64BitLocal) && atoi(launch64BitLocal))
	{
		//continue
	}
	else
	...
	{
		TriggerAlertDeferred("CONTROLLER_X64_STARTUP_FAIL", ALERTLEVEL_CRITICAL, ALERTCATEGORY_NETOPS,
			"While starting up, couldn't find %s, which should have existed, so we could restart in 64 bit mode",
			pMyName64);
...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	if (spDumpLocaleNamesAndExitFileName)
	{
		FILE *pOutFile = fopen(spDumpLocaleNamesAndExitFileName, "wt");
		char **ppNames = NULL;
		char tempFileName[CRYPTIC_MAX_PATH];

		assertmsgf(pOutFile, "Couldn't open %s", spDumpLocaleNamesAndExitFileName);
		locGetImplementedLocaleNames(&ppNames);
		FOR_EACH_IN_EARRAY(ppNames, char, pLocaleName)
		{
			bool bExists = false;

			if (stricmp(pLocaleName, "English") == 0)
			{
//				bExists = true;
			}
			else
			{
				sprintf(tempFileName, "%s/defs/ServerMessages.%s.translation", fileDataDir(), pLocaleName);
				if (fileExists(tempFileName))
				{
					bExists = true;
				}
...

	{
		...
		HRSRC rsrc = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(CONTROLLER_SERVER_SETUP_TXT), L"TXT");
		if (rsrc)
		{
...
//		ParserReadTextFile("server/ControllerServerSetup.txt", parse_ServerTypeInfoList, pList);
		...
		for (i=0; i < eaSize(&pList->ppServerInfo); i++)
		{
			...
			{
				//StructCopyFields because some fields such as pSharedCommandLine_FromScript may have already been set
				StructCopyFields(parse_ServerTypeInfo, pList->ppServerInfo[i], &gServerTypeInfo[pList->ppServerInfo[i]->gServerType], 0, 0);
...
//	ParserLoadFiles("server", "_ControllerAlertEmailRecipients.txt", NULL, 0, parse_AlertEmailRecipientList, &gAlertEmailRecipients);
	...
	listenInitReceiveStats(net_links, "Main controller listen", LauncherAnswerEnum);
