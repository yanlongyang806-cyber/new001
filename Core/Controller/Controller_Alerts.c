...
void Controller_FixupNewAlert(Alert *pAlert)
{
	...
	else if (pAlert->pKey == ALERTKEY_GAMESERVERNOTRESPONDING || pAlert->pKey == ALERTKEY_UGCEDIT_GAMESERVERNOTRESPONDING)
	{
		estrPrintf(&pAlert->pCommand1, "AcknowledgeAndKill %u %u %u $NORETURN $COMMANDNAME(AcknowledgeAndKill)",
			pAlert->iAlertUID, pAlert->eContainerTypeOfServer, pAlert->iIDOfServer);
	}
...
void CheckGameServerInfoForAlerts(TrackedServerState *pServer)
{
	...
	{
		if (pServer->pGameServerSpecificInfo && pServer->pGameServerSpecificInfo->bReceivedGSLGlobalInfo)
		{
			U32 iLastContact = pServer->pGameServerSpecificInfo->iLastContact;
			if (isProductionMode() && iLastContact < iCurTimeToUse - gDelayBeforeKillingGraceful - pServer->pMachine->iLastLauncherLag_ToUse)
			{
			...
			else if (isProductionMode() && iLastContact < iCurTimeToUse - gDelayBeforeAlertingGraceful - pServer->pMachine->iLastLauncherLag_ToUse && !pServer->bAlreadyAlertedSlowGracefulDeath)
			{
	...
	if (pServer->pGameServerSpecificInfo && pServer->pGameServerSpecificInfo->bReceivedGSLGlobalInfo)
	{
		...
		if (isProductionMode() && iLastContact < iCurTimeToUse - gDelayBeforeKillingStalledGameserver - pServer->pMachine->iLastLauncherLag_ToUse)
		{
			const char* astrAlertKey;
			if( pServer->pGameServerSpecificInfo->bIsEditingServer ) {
				astrAlertKey = ALERTKEY_UGCEDIT_KILLINGNONRESPONDINGGAMESERVER;
			} else {
				astrAlertKey = ALERTKEY_KILLINGNONRESPONDINGGAMESERVER;
			}
			if (ConditionChecker_CheckCondition(pServer->pAlertsConditionChecker,astrAlertKey))
			{			
				...
				if (!giHoursToLeaveStalledGameServerRunning || pServer->pMachine->iPIDOfIgnoredServer)
				{
					...
					Controller_DoXperfDumpOnMachine(pServer->pMachine, "KillingNonResponsiveGS");
						TriggerAlert(astrAlertKey, pErrorString,
		...
		else if (iLastContact < iCurTimeToUse - gDelayBeforeNonRespondingAlert - pServer->pMachine->iLastLauncherLag_ToUse)
		{
			const char* astrAlertKey;
			if( pServer->pGameServerSpecificInfo->bIsEditingServer ) {
				astrAlertKey = ALERTKEY_UGCEDIT_GAMESERVERNOTRESPONDING;
			} else {
				astrAlertKey = ALERTKEY_GAMESERVERNOTRESPONDING;
			}
			if (ConditionChecker_CheckCondition(pServer->pAlertsConditionChecker,astrAlertKey))
			{	
				...
				if (!RetriggerAlertIfActive(astrAlertKey, GLOBALTYPE_GAMESERVER, pServer->iContainerID, GLOBALTYPE_GAMESERVER, pServer->iContainerID))
				{
					...
					TriggerAlert(astrAlertKey, pAlertString,
