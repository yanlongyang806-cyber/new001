...
void ToggleMonitorLaunchersAndMultiplexers(CmdContext *pContext)
{
	...
	if (!sMonitorLaunchersAndMultiplexers)
	{
		sMonitorLaunchersAndMultiplexers = stashTableCreateWithStringKeys(16, StashDeepCopyKeys_NeverRelease);
	}
...
void GetShardLockageString(char **ppOutString)
{
	...
	if (Controller_DoingTransServerStressTest())
	{
		estrPrintf(ppOutString, "Doing Trans server stress test... %s\n", Controller_GetTransServerStressTestDescriptionString());
	}
...
ControllerOverview *GetControllerOverview_Internal(bool bForceFullList, bool bMonitorLaunchersAndMultiplexers)
{
	...
	if (bForceFullList)
	{
		...
		if (objLocalManager())
		{
			...
			ShardVariableContainer *pContainer = Controller_GetShardVariableContainer(&bAlreadyRequested);
			if (pContainer)
			{
				pControllerOverview->pShardVariableContainer = StructClone(parse_ShardVariableContainer, pContainer);
			}
		}
...
ControllerOverview *GetControllerOverview(bool bForceFullList, const char *pAuthNameAndIP)
{
	...
	if (bForceFullList)
	{
		static ControllerOverview *spFull = NULL;

		StructDestroySafe(parse_ControllerOverview, &spFull);
		

		spFull = GetControllerOverview_Internal(true, false);
...
//intended to transparently handle superescaped or non-superescaped command lines
AUTO_COMMAND;
void AppendCommandLine(char *pTypeName, ACMD_SENTENCE cmdLine)
{
	GlobalType eType = NameToGlobalType(pTypeName);
	...
	if (estrSuperUnescapeString(&pUnEscaped, cmdLine))
	{
	    estrConcatf(&gServerTypeInfo[eType].pSharedCommandLine_FromScript, " %s", pUnEscaped);
	}
	else
	{
	    estrConcatf(&gServerTypeInfo[eType].pSharedCommandLine_FromScript, " %s", cmdLine);
	}
