...
bool ControllerScripting_ImportVariables(char *pVarNames_In, bool bFailureIsNonFatal)
{
	...
	for (i=0; i < eaSize(&ppVarNames); i++)
	{
		...
		else
		{
			...
			{
				char *pTemp = NULL;
				estrPrintf(&pTemp, "Unable to import variable %s... this is nonfatal\n", 
					ppVarNames[i]);

				ControllerScripting_LogString(pTemp, 0, 0);
				printf("%s", pTemp);
				estrDestroy(&pTemp);
			}
			else
...
bool ControllerScripting_IsExpressionStringTrue(char *pString)
{
	...
	if (!pContext)
	{
		...
		pFuncTable = exprContextCreateFunctionTable("ControllerScripting");
...
void LaunchServerFromCommand(ControllerScriptingCommand *pCommand)
{
	...
	else
	{
		...
		for (i=0; i < pCommand->iCount; i++)
		{
			...
			if (gServerTypeInfo[pCommand->eServerType].bMachineSpecifiedInShardSetupFile  && pMachine->canLaunchServerTypes[pCommand->eServerType].eCanLaunch == CAN_LAUNCH_DEFAULT)
			{
				if (gServerTypeInfo[pCommand->eServerType].bNonRelocatable)
				{
				...
				else
				{
					if (isProductionMode())
					{
						CRITICAL_NETOPS_ALERT("MACHINE_LAUNCH_ON_UNSPECIFIED_MACHINE", 
							"Launching a %s. It had one or more machines specified in the shardSetupFile, including %s, but none of them currently seem to exist, so launching on %s by default. This may be a problem, but the server type is not NonRelocatable, so it is being allowed",
							GlobalTypeToName(pCommand->eServerType),
							gServerTypeInfo[pCommand->eServerType].pOneMachineSpecifiedInShardSetupFile,
...
char *ControllerScripting_GetCurStepName()
{
	...
	case CONTROLLERCOMMAND_IMPORT_VARIABLES_IF_EXIST:
...
void NextCommandStep(void)
{
	...
	if (gCommandStep < eaSize(&gCommandList.ppCommands))
	{
		...
		ControllerScripting_LogString(STACK_SPRINTF("NEW STEP: %s (%s:%d)\n", ControllerScripting_GetCurStepName(),
			gCommandList.ppCommands[gCommandStep]->pSourceFileName,
			gCommandList.ppCommands[gCommandStep]->iLineNum), 0, 0);
...
void ControllerScripting_Fail(char *pFailureString)
{
	if (isProductionMode())
	{
		CRITICAL_NETOPS_ALERT("CONTROLLER_SCRIPT_FAILED", "Controller script failed due to: %s. This will likely prevent the shard from starting",
			pFailureString);
	}
	...
	if (gCommandStep < eaSize(&gCommandList.ppCommands))
	{
		...
		if (iCurTime - gTimeCurStepStarted > gCommandList.ppCommands[gCommandStep]->iFailureTime)
		{
			...
			if (!bDontActuallyFail)
			{
				...
				//non-CB production shards NEVER actually fail, they just trigger an alert
				if (isProductionMode() && !g_isContinuousBuilder)
				{
					CRITICAL_NETOPS_ALERT("SCRIPT_STEP_TIME_OVERFLOW", "Controller scripting timed out: %s. Because this is production mode, this is being forced nonfatal",
						pErrorString);
					gCommandList.ppCommands[gCommandStep]->iFailureTime = 1000000000;
				}
				else
				{
...
void ControllerScripting_Update(void)
{
	...
	if (!bLoaded)
	{
		...
		if (gbShutdownWatcher)
		{
			if (Controller_GetClusterControllerName() && Controller_GetClusterControllerName()[0])
			{
				...
				estrPrintf(&pCmdLine, "c:\\night\\tools\\bin\\ClusterAwareShutdownWatcher%s.exe -ShardName %s -MainExecutable %s controller%s %d", 
			...
			else
			{
				...
				estrPrintf(&pCmdLine, "c:\\night\\tools\\bin\\ShutdownWatcher%s.exe -MainExecutable controller%s.exe %d", (iPointerSize == 8 ? "X64" : ""), (iPointerSize == 8 ? "X64" : ""), _getpid());
	...
	do
	{
		...
		case CONTROLLERCOMMAND_LAUNCHXBOXCLIENT:
			{
				...
				estrPrintf(&pSystemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbcp\" -Y -T c:\\tempcmdline.txt %s\\cmdline.txt", pProgramFilesFolder, pWorkingDir);
				system(pSystemString);

				estrPrintf(&pSystemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbreboot\" ",pProgramFilesFolder ); 
		...
		case CONTROLLERCOMMAND_KILLXBOXCLIENT:
			{
				...
				sprintf(systemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbreboot\"", pProgramFilesFolder);
				system(systemString);
				NextCommandStep();
				gXBOXClientState[0] = 0;
		...
		case CONTROLLERCOMMAND_IMPORT_VARIABLES:
			if (ControllerScripting_ImportVariables(pCurCommand->pScriptString_Use, false))
			{
				NextCommandStep();
			}
			return;		
		...
		case CONTROLLERCOMMAND_IMPORT_VARIABLES_IF_EXIST:
			if (ControllerScripting_ImportVariables(pCurCommand->pScriptString_Use, true))
			{
				NextCommandStep();
			}
			return;
