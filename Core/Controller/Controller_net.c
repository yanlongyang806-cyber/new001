...
void HandleProcessListUpdate(Packet *pak, NetLink *pLink)
{
	...
	for (i=0; i < eaSize(&globalInfo.ppProcesses); i++)
	{
		...
		else
		{
			...
			pServer->bWasSeenByLauncher = true;
	...
	for (i=0; i < GLOBALTYPE_MAXTYPES; i++)
	{
		if (!gServerTypeInfo[i].bLaunchersDontKnowAboutMe)
		{
			...
			while (pServer && pServer->pMachine == pMachine)
			{
				...
				//the server is NOT on the launcher
				if (pServer->bWaitingForUpdate)
				{
					if (pServer->bWasSeenByLauncher)
					{
						char *pReason = NULL;
						Controller_DoXperfDumpOnMachine(pServer->pMachine, 
							"ServerVanished_%s", GlobalTypeToName(pServer->eContainerType));
						estrPrintf(&pReason, "Server %s[%u] was previously seen by the launcher, but now vanished, with no process closed message.",
							GlobalTypeToName(pServer->eContainerType), pServer->iContainerID);
						StopTrackingServer(pServer, pReason, true, false);
						estrDestroy(&pReason);
					}
					else if (!pServer->bWaitingToLaunch)
					{
...
void SendMonitoringCommandResultToMCP(ContainerID iMCPID, int iRequestID, int iClientID, CommandServingFlags eFlags, char *pMessageString, void *pUserData)
{
...
void HandleMCPMonitoringCommand(Packet *pak, NetLink *link)
{
	...
	CommandServingFlags eFlags = pktGetBits(pak, 32);
	...
	if (eType == GLOBALTYPE_CONTROLLER)
	{
		if (eFlags & CMDSRV_NORETURN)
		{
			...
			{
				SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, "Execution failed... talk to a programmer. Most likely the AST_COMMAND was set up wrong", NULL);			
			}
			else
			{
				SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, SERVERMON_CMD_RESULT_HIDDEN, NULL);
			}
		}
		else
		{
...
ever there.*/
				...
				if (!(bSlowReturn = slowReturnInfo.bDoingSlowReturn))
				{
					...
					SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, pRetString, NULL);
				}
...
#endif
			{
				cmdParseForServerMonitor(eFlags, pCommandString, iAccessLevel, &pRetString, iClientID, iCommandRequestID, iMCPID, SendMonitoringCommandResultToMCP, NULL, pAuthNameAndIP, &bSlowReturn);
				if (!bSlowReturn)
				{
					SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, pRetString, NULL);
				}
	...
	else
	{
		...
		if (!pServer || !ServerReadyForMonitoring(pServer))
		{
			...
			if (strStartsWith(pCommandString, "<?xml"))
			{
				...
				StructDestroy(parse_XMLMethodResponse, response);
				SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, out, NULL);
				estrDestroy(&out);
			}
			else
#endif
			{
				...
				{
					SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, 
						GetJsonRPCErrorString(pCommandString, "Server does not exist, or not yet ready"), 
						NULL);
				}
				else
				{
					SendMonitoringCommandResultToMCP(iMCPID, iCommandRequestID, iClientID, eFlags, "Server not yet ready", NULL);
				}
			}
			return;
		}
		...
		else if (objLocalManager())
		{
			RemoteCommand_CallLocalCommandRemotelyAndReturnVerboseHtmlString(eType, iContainerID, 
				iClientID, iCommandRequestID, iMCPID, eFlags, pCommandString, iAccessLevel, pAuthNameAndIP);
		}
	}
}
...
static void HandleClientREquestsOpenMonitoringUrl(Packet *pPak)
{
	char *pUrl = pktGetStringTemp(pPak);
	TrackedServerState *pMCP = gpServersByType[GLOBALTYPE_MASTERCONTROLPROGRAM];

	while (pMCP)
	{
		if (pMCP->pMachine->bIsLocalHost && pMCP->pLink)
		{
			Packet *pOutPack = pktCreate(pMCP->pLink, FROM_CONTROLLER_OPEN_URL_FOR_MONITORING);
...
//when an MCP connects, immediately send it a server list
	...
	PERF_CASE( LAUNCHER_ANSWER_CLIENT_REQUESTS_OPEN_MONITORING_URL )
		HandleClientREquestsOpenMonitoringUrl(pak);
