...
void InformControllerOfServerState_Internal(TrackedServerState *pServer, char *pStateString)
{
		...
		strstr(pStateString, gServerTypeInfo[pServer->eContainerType].beginMemLeakTrackingState))
	{
		...
		{
			estrPrintf(&pCmdString, "BeginMemLeakTracking %d %d %d %d %s",
				gServerTypeInfo[pServer->eContainerType].iMemLeakTracking_BeginDelay,
				gServerTypeInfo[pServer->eContainerType].iMemLeakTracking_Frequency,
				gServerTypeInfo[pServer->eContainerType].iMemLeakTracking_IncreaseAmountThatIsALeak_Megs,
				gServerTypeInfo[pServer->eContainerType].iMemLeakTracking_FirstIncreaseAllowance_Megs,
				NULL_TO_EMPTY(gServerTypeInfo[pServer->eContainerType].pMemLeakTracking_AlertKeyPrefix));
...
AUTO_COMMAND_REMOTE;
void ReturnHtmlStringFromRemoteCommand(int iClientID, int iCommandRequestID, U32 iMCPID, CommandServingFlags eFlags, char *pRetString)
{
...
void HereIsGSLGlobalInfo(ContainerID iServerID, int iLowLevelIndex, GameServerGlobalInfo *pInfo)
{
	...
	if (pServer)
	{
		...
		pServer->pGameServerSpecificInfo->bReceivedGSLGlobalInfo = true;
	}
...
void RequestAutoSettings(GlobalType eType, ContainerID iID)
{
	...
	//because every server that connects to the trans server always asks for remote settings, we
	//throw in lock status here as well
	if (gServerTypeInfo[eType].bInformAboutShardLockStatus)
	{
		RemoteCommand_ShardIsLocked(eType, iID, gbShardIsLocked);
	}

	if (gServerTypeInfo[eType].bInformAboutGatewayLockStatus)
	{
		RemoteCommand_GatewayIsLocked(eType, iID, gbGatewayIsLocked);
	}
