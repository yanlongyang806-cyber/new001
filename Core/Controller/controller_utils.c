...
static FloatMaxTracker *spMainQueueMaxTracker = NULL;
static FloatMaxTracker *spVIPQueueMaxTracker = NULL;
	...
	else
	{
		...
		while (pServer)
		{
			...
			if (pServer->pGameServerSpecificInfo)
			{
				...
				pStuff->iNumPlayersOnGameServers += pServer->pGameServerSpecificInfo->iNumPlayers;
			}
		...
		while (pServer)
		{
			if (pServer->pLoginServerSpecificInfo)
			{
				...
				pStuff->iMainQueueSize +=  pServer->pLoginServerSpecificInfo->iNumQueued_Main;
				pStuff->iVIPQueueSize +=  pServer->pLoginServerSpecificInfo->iNumQueued_VIP;
			}
...
/*	pStuff->iMainQueueSize = PointerFIFO_Count(spMainLoginQueue);
	...
	if (spMainQueueMaxTracker)
	{
		pStuff->iMaxMainQueueSize = FloatMaxTracker_GetMax(spMainQueueMaxTracker, 0);
		pStuff->iMaxMainQueueSize_LastHour = FloatMaxTracker_GetMax(spMainQueueMaxTracker,  timeSecondsSince2000() - 60 * 60);
		pStuff->iMaxMainQueueSize_LastDay = FloatMaxTracker_GetMax(spMainQueueMaxTracker, timeSecondsSince2000() - 24 * 60 * 60);

		pStuff->iMaxVIPQueueSize = FloatMaxTracker_GetMax(spVIPQueueMaxTracker, 0);
		pStuff->iMaxVIPQueueSize_LastHour = FloatMaxTracker_GetMax(spVIPQueueMaxTracker,  timeSecondsSince2000() - 60 * 60);
		pStuff->iMaxVIPQueueSize_LastDay = FloatMaxTracker_GetMax(spVIPQueueMaxTracker, timeSecondsSince2000() - 24 * 60 * 60);
	}
...
void MaxQueueTrackerCB(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	...
	if (!spMainQueueMaxTracker)
	{
		spMainQueueMaxTracker = FloatMaxTracker_Create();
		spVIPQueueMaxTracker = FloatMaxTracker_Create();
	}

	FloatMaxTracker_AddDataPoint(spMainQueueMaxTracker, pStuff->iMainQueueSize, iCurTime);
	FloatMaxTracker_AddDataPoint(spVIPQueueMaxTracker, pStuff->iVIPQueueSize, iCurTime);
}
...
void SetupInterestingStuffList(void)
{
	...
	TimedCallback_Add(MaxQueueTrackerCB, NULL, 5.0f);
}
...
void BeginFrankenBuildCompressedFileCB(CompressedFileCache *pCache, FrankenBuild *pBuild)
{
	...
	if (pCache->iCompressedSize)
	{
		MemLeakTracking_IgnoreLargeAllocation(STACK_SPRINTF("FrankenBuild of %s",  pCache->pFileName), pCache->iCompressedSize);
	}
...
void RestartWithNewControllerFiles(char *pExeName, char *pPdbName)
{
	...
	{
		if (!DeleteFile_UTF8(pBackupFullName))
		{
		...
		|| gServerTypeInfo[GLOBALTYPE_CONTROLLER].iMemLeakTracking_FirstIncreaseAllowance_Megs)
	{
		...
		else
		{
			BeginMemLeakTracking(gServerTypeInfo[GLOBALTYPE_CONTROLLER].iMemLeakTracking_BeginDelay,
				...
				gServerTypeInfo[GLOBALTYPE_CONTROLLER].pMemLeakTracking_AlertKeyPrefix);
		}
	}
}
...
void Controller_SetStartupStatusString(char *pCategoryName, FORMAT_STR const char* format, ...)
{
	...
	StatusReporting_Log("%s", Controller_GetStartupStatusString());
}
...
static int siTransPerServerPerSecond = 0;
static int siAverageTransactionBytes = 0;

char *Controller_GetTransServerStressTestCommandArgString(void)
{
	static char retVal[64];
	sprintf(retVal, "%d %d", siTransPerServerPerSecond, siAverageTransactionBytes);

	return retVal;
}

AUTO_COMMAND;
void BeginTransServerStressTest(int iNumServersToStart, int iTransPerServerPerSecond, int iBytesPerTransaction)
{
	TrackedServerState *pServer;
	int i;

	sbDoingTransServerStressTest = true;
	siTransPerServerPerSecond = iTransPerServerPerSecond;
	siAverageTransactionBytes = iBytesPerTransaction;
	...
	while (pServer)
	{
		if (strstri(pServer->stateString, "gslrunning"))
		{
			RemoteCommand_BeginTransServerStressTestMode(GLOBALTYPE_GAMESERVER, pServer->iContainerID, Controller_GetTransServerStressTestCommandArgString());
		}

		pServer = pServer->pNext;
	}

	for (i = 0; i < iNumServersToStart; i++)
	{
		RemoteCommand_CallLocalCommandRemotely(GLOBALTYPE_MAPMANAGER, 0, "NewMapTransfer_LaunchNewServerCmd EmptyMap 1 -");
	}
}

bool Controller_DoingTransServerStressTest(void)
{
	return sbDoingTransServerStressTest;
}

char *Controller_GetTransServerStressTestDescriptionString(void)
{
	static char *spRetVal = NULL;
	estrPrintf(&spRetVal, "%d transactions per server per second, %d bytes per transaction",
		siTransPerServerPerSecond, siAverageTransactionBytes);
...
{
	int iRetVal = ControllerStartup_GetState();
	if (iRetVal)
	{
		return iRetVal;
	}
	...
	{
		return STATUS_CONTROLLER_SERVERS_STARTING;
	}

	if (gbShardIsLocked)
	{
		return STATUS_CONTROLLER_LOCKED;
	}

	return STATUS_CONTROLLER_RUNNING;
}
...
NameValuePairList *OVERRIDE_LATELINK_StatusReporting_GetSelfReportedNamedValuePairs(void)
{
	...
	static NameValuePair *spNumPlayers = NULL;
	if (!spRetVal)
	{
		spRetVal = StructCreate(parse_NameValuePairList);
		spNumPlayers = StructCreate(parse_NameValuePair);
		spNumPlayers->pName = strdup("NumPlayers");
		eaPush(&spRetVal->ppPairs, spNumPlayers);
	}

	SAFE_FREE(spNumPlayers->pValue);
	spNumPlayers->pValue = strdupf("%d", gShardPerf.iPlayers);

	return spRetVal;
}

void OVERRIDE_LATELINK_OverlordWasJustSet(void)
{
	char *pOverlord = UtilitiesLib_GetOverlordName();
	if (pOverlord && pOverlord[0])
	{
		DoExtraStatusReporting(STACK_SPRINTF("%s:%d", pOverlord, OVERLORD_SIMPLE_STATUS_PORT));
	}
}

AUTO_COMMAND;
char *AbandonServersOnDeadMachine(char *pMachineName, char *pConfirm, CmdContext *pContext)
{
	TrackedMachineState *pMachine = FindMachineByName(pMachineName);
	int iServerType;
	TrackedServerState **ppServersToKill = NULL;
	int iKilledCount = 0;
	static char *spRetVal = NULL;
	TrackedServerState *pServer;
	char *pReason = NULL;
	...
	{
		return "That machine is running again! Lawdy lawdy it's a miracle!";
	}

	for (iServerType = 0; iServerType < GLOBALTYPE_MAXTYPES; iServerType++)
	{
		pServer = pMachine->pServersByType[iServerType];

		while (pServer && pServer->pMachine == pMachine)
		{
			eaPush(&ppServersToKill, pServer);
			pServer = pServer->pNext;
		}
	}

	estrPrintf(&pReason, "AbandonServersOnDeadMachine on %s via %s(%s)",
		pMachine->machineName, GetContextHowString(pContext), pContext->pAuthNameAndIP);

	while ((pServer = eaPop(&ppServersToKill)))
	{
		iKilledCount++;
	...
	eaDestroy(&ppServersToKill);
	estrDestroy(&pReason);

	estrPrintf(&spRetVal, "Abandoned %d servers", iKilledCount);
