...
typedef struct Controller_DeadMachineOverview
{
	...
	AST_COMMAND("Abandon servers", "AbandonServersOnDeadMachine $FIELD(MachineName_Internal) $STRING(Type yes to forget/kill all servers on $FIELD(MachineName_Internal))")
...
{
	VersionHistoryEntry **ppEntries; AST(FORMATSTRING(HTML_COLLAPSED_ARRAY=3)) //most recent == 0
} VersionHistory; 
...
//	Controller_ClientControllerOverview **ppClientControllers;
	...
	ShardVariableContainer *pShardVariableContainer; AST(LATEBIND)
...
typedef struct TrackedServerState
{
	...
	//at least once, this server did at least exist for a moment on the launcher
	bool bWasSeenByLauncher;
