...
// Handle is escaped with spaces because of limited clients with full XEP-0106 support
...
static void xmppchat_getroster(SA_PARAM_NN_VALID ChatUser *forUser, SA_PARAM_NN_VALID EARRAY_OF(ChatUser) *eaRoster, bool xmppExtra)
{
	...
	if (xmppExtra && forUser->xmppSettings)
	{
		EARRAY_INT_CONST_FOREACH_BEGIN(forUser->xmppSettings->roster, i, n);
		{
			if (friendUser = userFindByContainerId(forUser->xmppSettings->roster[i]))
				eaPushUnique(eaRoster, friendUser);
		}
		EARRAY_FOREACH_END;
	}
	...
	EARRAY_CONST_FOREACH_BEGIN(guilds, i, n);
	{
		EARRAY_INT_CONST_FOREACH_BEGIN(guilds[i]->pGuildMembers, j, m);
		{
			...
			{
				friendUser = userFindByContainerId(id);
				if (friendUser)
					eaPushUnique(eaRoster, friendUser);
				else
...
{
	ChatUser *user;
	EARRAY_OF(ChatUser) roster = NULL;
...
{
	EARRAY_OF(ChatUser) friends = NULL;
	...
	EARRAY_CONST_FOREACH_BEGIN(friends, i, n);
	{
		...
		if (node)
		{
			...
			if (!playerName)
				XMPP_WRITE_STRUCT(&structString, parse_XMPP_PresenceData, &fromPData);
			EARRAY_CONST_FOREACH_BEGIN(node->resources, j, m);
			{
				...
				{
					// Send presence from this player to this XMPP resource.
					sendCommandToLinkEx(XMPP_GetLink(), "XMPP_SendPresence", "%d \"\" %s", friendState->uStateID, structString);
				}
			}
			EARRAY_FOREACH_END;
			if (!playerName)
...
static enum XMPP_RoomDomain XMPPChat_DomainFromChannelName(const char *pName)
{
	...
	else if (strStartsWith(pName, TRADE_CHANNEL_PREFIX))
		domain = XMPP_RoomDomain_Unknown;
	else if (strStartsWith(pName, LFG_CHANNEL_PREFIX))
		domain = XMPP_RoomDomain_Unknown;
...
void XMPPChat_NotifyChatMessage(ChatChannel *channel, const ChatMessage *message)
{
	...
	XMPPChat_GetXMPPChannelInfoEx(&chatMsg.room, NULL, channel, XMPP_RoomDomain_Channel, true);
	...
	EARRAY_INT_CONST_FOREACH_BEGIN(channel->members, i, n);
	{
		...
		{
			int found;
			found = eaIndexedFindUsingString(&node->resources[j]->rooms, message->pchChannel);
			if (found != -1)
			...
			if (giXmppOnlyListWatching)
			{
				EARRAY_CONST_FOREACH_BEGIN(user->watching, i, n);
				{
					...
					if (domain == XMPP_RoomDomain_Channel)
					{
						...
						XMPPChat_GetXMPPChannelInfoEx(&channelInfo->node, &channelInfo->name, channel, XMPP_RoomDomain_Channel, true);
		...
		xcase XMPP_RoomDomain_Guild:
			...
			EARRAY_CONST_FOREACH_BEGIN(guilds, i, n);
			{
				...
				if (shardData)
				{
					XmppChannelInfo *channelInfo = StructCreate(parse_XmppChannelInfo);
					ChatUser **members = NULL;
					XMPPChat_GetXMPPChannelInfoEx(&channelInfo->node, &channelInfo->name, guild, XMPP_RoomDomain_Guild, true);
		...
		xcase XMPP_RoomDomain_Officer:
			...
			EARRAY_CONST_FOREACH_BEGIN(officerGuilds, i, n);
			{
				...
				if (shardData)
				{
					XmppChannelInfo *channelInfo = StructCreate(parse_XmppChannelInfo);
					ChatUser **members = NULL;
					XMPPChat_GetXMPPChannelInfoEx(&channelInfo->node, &channelInfo->name, guild, XMPP_RoomDomain_Officer, true);
					XMPPChat_GuildOfficersOnline(&members, guild);
					channelInfo->members = eaSize(&members);
					eaDestroy(&members);
					eaPush(eaChannels, channelInfo);
