...
int userGetNaughtyDecayPoints(ChatUser *user, U32 uCurrentTime)
{
		...
		uTimeDiff >= gGlobalChatServerConfig.iNaughtyQuarterDecay)
	{
		...
		return (user->naughty*3 + 3) / 4; // +3 so it rounds up;
	}
	return user->naughty;
}
...
static const char* blacklist_ReduceString(const char* str)
{
	static char* estrNormalized = NULL;
	static char* estrNormalizedAndReduced = NULL;
	UTF8NormalizeString( str, &estrNormalized );
	UTF8ReduceString( estrNormalized, &estrNormalizedAndReduced );
	return estrNormalizedAndReduced;
}
...
{
	ChatBlacklistString *blString = blacklist_AddString_Internal(blacklist_ReduceString(string), reason);
...
{
	ChatBlacklistString *blString = blacklist_RemoveString_Internal(blacklist_ReduceString(string), false);
	BlacklistXMLRPCResponse *response = StructCreate(parse_BlacklistXMLRPCResponse);
	if (blString)
	{
		ChatBlacklist bl = {0};
