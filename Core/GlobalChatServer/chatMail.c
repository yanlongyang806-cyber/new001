...
int ChatServerSendEmailWrappers(int accountID, EmailWrappers *pEmailWrappers)
{
	...
	estrStackCreate(&command);
	estrPrintf(&command, "ChatServerForwardEmailWrappersFromGlobal %d %s", accountID, wrapperString);
	sendCommandToLink(chatLink, command);
	estrDestroy(&command);
...
AUTO_COMMAND_REMOTE;
void chatCommandRemote_TransferAndDeleteShardMail(int accountID, const char *shardName)
{
	ChatUser *user = userFindByContainerId(accountID);
	const char *shardPooled = allocAddString(shardName);
	EmailWrappers wrapper = {0};
	int i, iSize;

	if (!user || nullStr(shardName))
		return;

	// We're going to construct a fairly large struct and send it via remote command to the gameserver. I hate this, 
	// and it's probably going to burn us real bad when this goes live for the first time, but I don't see any other
	// way of possibly doing this
	EARRAY_CONST_FOREACH_BEGIN(user->email, j, size);
	{
		if (user->email[j]->shardName && user->email[j]->shardName == shardPooled && !user->email[j]->iNPCEMailID)
		{
			EmailWrapper *pEmailWrapper = StructCreate(parse_EmailWrapper);
			Email *pEmail = user->email[j];

			pEmailWrapper->uSent = pEmail->sent;
			pEmailWrapper->uExpireTime = pEmail->expireTime;
			pEmailWrapper->pchSubject = StructAllocString(pEmail->subject);
			pEmailWrapper->pchBody = StructAllocString(pEmail->body);
			pEmailWrapper->pchSenderName = StructAllocString(pEmail->senderName);
			pEmailWrapper->eTypeOfEmail = pEmail->eTypeOfEmail;
			pEmailWrapper->bRead = pEmail->bRead;
			pEmailWrapper->uLotID = pEmail->uLotID;

			eaPush(&wrapper.eaEmailWrappers, pEmailWrapper);
		}
	}
	EARRAY_FOREACH_END;

	ChatServerSendEmailWrappers(accountID, &wrapper);

	iSize = eaSize(&wrapper.eaEmailWrappers);
	if (!iSize)
	{
		return;
	}
	...
	for (i = iSize - 1; i >= 0; i--)
	{
		EmailWrapper *pEmailWrapper = eaRemove(&wrapper.eaEmailWrappers, i);
		if (pEmailWrapper)
