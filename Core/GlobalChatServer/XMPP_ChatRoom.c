...
void XMPPChat_InitChannelInfoData(XMPP_ChannelInfoData *data, int eDomain, const char *room, const char *name)
{
	data->eDomain = eDomain;
	xmpp_escape_jid(&data->room, room);
	if (name)
		estrCopy2(&data->name, name);
}

void XMPPChat_GetXMPPChannelInfoEx (char **estrNode, char **estrDescription, void *data, int eDomain, bool bEscape)
{
	...
	case XMPP_RoomDomain_Channel:
		{
			...
			{
				if (bEscape)
					xmpp_escape_jid(estrNode, channel->name);
				else
	...
	case XMPP_RoomDomain_Officer: // intentional fall-through
		{
			...
			if (shardData) // this is set for Guild/Officer
			{
				...
				if (estrNode) // default to the internal shard name if the display one isn't set
				{
					...
					{
						char *temp = NULL;
						estrPrintf(&temp, "%s!%s", shardName ? shardName : shardData->pShardName, guild->pchName);
						xmpp_escape_jid(estrNode, temp);
						estrDestroy(&temp);
					}
					else
...
void XMPPChatRoom_NotifyJoin (XmppClientState *state, ChatUser *user, int eDomain, const char *room)
{
	...
	if (state)
	{
		...
		if (!chatRoom)
		{
			...
			if (!gXmppState.stChatRooms[eDomain])
				gXmppState.stChatRooms[eDomain] = stashTableCreateWithStringKeys(256, StashDeepCopyKeys_NeverRelease);
...
void XMPPChatRoom_NotifyLeaveAll (ChatUser *user, int eDomain, const char *room, bool bKicked)
{
	...
	if (node)
	{
		...
		XMPPChat_InitChannelInfoData(&occupant, eDomain, room, user->handle);
...
void XMPPChatRoom_NotifyLeaveSingle (XmppClientState *state, ChatUser *user, int eDomain, const char *room, void *data)
{
	...
	if (!chatRoom)
	{
		if (!assertIsDevelopmentMode())
			ErrorDetailsf("%s, %d", room, user->id);
		devassertmsg(0, "User attempting to leave unknown XMPP MUC.");
		return;
	}
	...
	if (state)
	{
		...
		{
			if (!assertIsDevelopmentMode())
				ErrorDetailsf("%s, %d", room, node->uAccountID);
			devassertmsg(0, "Unsubscribed user attempting to leave XMPP MUC.");
			bSendUpdate = false;
		}
	}
...
void *XMPPChat_GetChatRoomData(enum XMPP_RoomDomain eDomain, const char *pName)
{
	...
	case XMPP_RoomDomain_Channel:
		{
			ChatChannel *channel = channelFindByName(pName);
			return channel;
		}
...
// Get the Chat Server channel name from an XMPP channel name.
...
void XMPPChat_RecvGuildChatMessage (const ChatMessage *msg)
{
	...
	XMPPChat_GetXMPPChannelInfoEx(&chatMsg.room, NULL, guild, chatMsg.eDomain, true);
	XMPPChat_GetXMPPChannelInfoEx(&unescapedRoom, NULL, guild, chatMsg.eDomain, false);
	
	{
		XmppClientState **ppMembers = NULL;
		XMPP_ClientStateList list = {0};

		XMPPChatRoom_GetClientStates(&ppMembers, chatMsg.eDomain, unescapedRoom);
