...
/* @(#) $Id: deflate.c 114229 2011-04-07 19:02:15Z alaframboise $ */
...
#	define RSYNC_DEFAULT_CHECKSUM_TYPE Z_RSYNCABLE_OFF  /* Z_RSYNCABLE_RSSUM */
#endif
...
#define RSYNC_MINIMUM_FLUSH_DISTANCE 32
...
{
    deflate_state *s;
    uLong complen, wraplen, addend = 0;
    Bytef *str;
	...
	/* if using rsync, add worst case flush point creation */
	if (zlib_rsync && s->rsync_rollfunction != rsync_roll_noop)
		addend = sourceLen/RSYNC_MINIMUM_FLUSH_DISTANCE*4;

    /* if not default parameters, return conservative bound */
    if (s->w_bits != 15 || s->hash_bits != 8 + 7)
        return complen + wraplen + addend;

    /* default settings: return tight bound for that case */
    return sourceLen + (sourceLen >> 12) + (sourceLen >> 14) +
           (sourceLen >> 25) + 13 - 6 + wraplen + addend;
}
...
local void rsync_roll2(deflate_state *s, unsigned start, unsigned num)
{
    ...
    for (i = start; i < start+num; i++) {
		...
		if (s->rsync_chunk_end == 0xFFFFFFFFUL
			&& RSYNC_SUM_MATCH(s)
			&& s->rsync_roll_count >= RSYNC_MINIMUM_FLUSH_DISTANCE){
			s->rsync_chunk_end = i;
