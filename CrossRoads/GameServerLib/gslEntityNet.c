...
// Track if we sent out a full version of an entity this frame at a given info level. Reset every frame.
static U8					entWasSentFullAtInfoLevel[MAX_ENTITIES_PRIVATE][ENT_SEND_INFO_COUNT];
// Track the validity info level of the backup struct. Reset when a new backup struct is created
static U8					entBackupIsValidInfoLevel[MAX_ENTITIES_PRIVATE];
...
enum {
	...
	//for debug logging. Incremented every frame in gslEntityUpdateEnd
...
// Used for tracking what detail level we did full sends at.
// This is the clear that we do every frame.
static void gslEntityClearSentFullLevels(void)
{
	int i,j;

	for (i=0;i<=gHighestActiveEntityIndex;i++)
	{
		for (j=0;j<ENT_SEND_INFO_COUNT;j++)
		{
			entWasSentFullAtInfoLevel[i][j]=0;
		}
	}
}

// Store the WasSentFullAtInfoLEvel struct. Use this to clear off the flags if needed.
static void gslEntitySetSentFullLevel(int entIndex, const EntitySendInfo entSendInfo)
{
	int iSendInfo = entSendInfo & ENT_SEND_INFO_MASK_NO_FLAGS;
	// Now we have an array index
	entWasSentFullAtInfoLevel[entIndex][iSendInfo]=1;
}
...
void gslEntityUpdateEnd(void){
	...
	FOR_BEGIN(entityIndex, gHighestActiveEntityIndex + 1);
	{
		...
		// Okay. We may have sent some number of diffs and full sends to various clients. Each of the sends
		//   could have been at one of the 3 send levels (Normal/Special/Local). If we didn't send anything for
		//   this enity, we will delete the sendInfo and backup data for the entity.

		if (e->ServerClientSentThisFrame)
		{
			int usedDiffPacket = false;

 			// First check if we FORCED a full send. If we did, just do a full backup regardles of diff packets or backup validity level.
			if (!(e->mySendFlags & ENT_SEND_FLAG_FULL_NEEDED))
			{
				// If we were NOT FORCED to send full, find the highest quality diffs and fulls that we sent.
				// See if we did a full send at a level higher than the validity level of the backup, or if we don't have a sufficiently high enough diff
				// to send this frame. In either case, we sent a full packet at a more detailed level than the diffs we've been sending or sent.
				// Don't bother updating by diff. We need to do a full copy.
				// (The underlying assumption is that if we had been diff updating the backup HAS to be accurate at that level. Otherwise we would detect it
				// in gslQueueEntitySend and force a full send, though we do need to worry about the case where we WERE sending high enough, but did not this frame)

				S32 iSendLevel;

				int iHighestFullSent = ENT_SEND_INFO_NO_SEND;
				int iHighestDiffSent = ENT_SEND_INFO_NO_SEND;
				
				bool bNeedToUpgradeBackupLevel=false;

				// Find the highest detail level that we sent for fulls and diffs
				
				// Find best full
				for (iSendLevel=ENT_SEND_INFO_COUNT-1; iSendLevel > ENT_SEND_INFO_NO_SEND; iSendLevel--)
				{
					// The threaded sends can set these. They are stored separately so we don't have to worry about thread contention.
					if (entWasSentFullAtInfoLevel[entityIndex][iSendLevel] != 0)
					{
						iHighestFullSent = iSendLevel;
						break;
					}
				}
				// Find best diff
				for (iSendLevel=ENT_SEND_INFO_COUNT-1; iSendLevel > ENT_SEND_INFO_NO_SEND; iSendLevel--)
				{
					Packet* pak = entDiffPackets[entityIndex][iSendLevel-1];	// We don't store a packet for the "NO_SEND" case so the storage is off by one.
					if(pak)
					{
						if (pktGetWriteIndex(pak))
						{
							iHighestDiffSent = iSendLevel;
							break;
						}
					}
				}

				// If we sent a full packet that was strictly more detailed than the best diff we sent this frame,
				//   or if we sent a full packet that was strictly more detailed than what we've backed up,
				//   we need to upgrade the backup level.
				if (iHighestFullSent > iHighestDiffSent || iHighestFullSent > entBackupIsValidInfoLevel[entityIndex])
				{
					bNeedToUpgradeBackupLevel=true;
				}
				else
				{
					// If we aren't going to back up fully, it should be the case that the backup is at LEAST the same
					//  quality level as the best diff. Otherwise we will be backing up over possibly stale data.
					devassert(iHighestDiffSent <= entBackupIsValidInfoLevel[entityIndex]);
				}

				if (!bNeedToUpgradeBackupLevel)
				{
					// Okay. We know we can update via diff if there was one sent out.
					// Apply highest quality diff packet.
					
					Packet* pak = entDiffPackets[entityIndex][iHighestDiffSent-1];	// We don't store a packet for the "NO_SEND" case so the storage is off by one.
					if(pak)
					{
						if (pktGetWriteIndex(pak)){
							switch(iHighestDiffSent){
								xcase ENT_SEND_INFO_NORMAL:
							...
							if(!ePrevSent)
							{
								// This is a problematic case. We are applying a diff to a newly created ent, so the result could be incomplete.
								// Is it even possible to have this happen? Should possibly be an assert though that is dangerous in this code.
								ePrevSent = EntSystem_CreateLastFrameEntityFromIndex(entityIndex);
								entBackupIsValidInfoLevel[entityIndex]=ENT_SEND_INFO_NO_SEND;

								devassertmsg(0, "Creating a backup copy of an entity for net sends, but applying a diff instead of a full copy.");

								// When this was originally written, this was treated as an okay case. In prod we will continue with out-of-synch data.
								// as before. 
							}
							...
							entBackupIsValidInfoLevel[entityIndex]=iHighestDiffSent;
						}
					}
				}
			}
			...
			if(!usedDiffPacket)
			{
				// No diff packet was found, or we sent a full entity, or we need to upgrade the quality of the backup.
				// Copy directly to last-frame Entity.
				// (A theoretical efficiency gain would be to only copy at the infolevel required rather than the full)
				...
				// This is a FULL backup. We are now at highest info validity
				entBackupIsValidInfoLevel[entityIndex]=ENT_SEND_INFO_COUNT-1;
			}
		...
		// Clear our Sent flags.
		e->ServerClientSentThisFrame=0;
	}
	FOR_END;
								...
								SendEntityUpdateThreadData*const td)
{
	...
	// This is stored in the thread data, and is copied out to the link specific data in gslSaveSentState
	// (which also happens in the thread).
		...
		!entSentLastFrameInfo[entIndex])
	{
		...
		// Set the global sentFull level so we know at the end of the threaded stuff what we sent.
		// This should be safe even though threaded since all we do is set a value here.
		// This is eventually checked in gslEntityUpdateEnd which is run
		//  after all the threads that pack up the data are finished (which is the code
		//  where we currently are) It is cleared in gslEntityUpdateEndInThread on entityIndex 0 (like deleteflags)
		// If we do not do this we could get INDENTICAL problems on the client due to out-of-synch data.
		gslEntitySetSentFullLevel(entIndex, entSendInfo);
	...
	else
	{
		...
		if (entSentLastFrameInfo[entIndex] != entSendInfo ||
			(e->mySendFlags & ENT_SEND_FLAG_FULL_NEEDED))
		{
			// ...but either used different send info OR we were asked to force a full update.

			// SEND_FLAG_FULL_NEEDED is mostly a debuggy type thing that means we are FORCING a full send.

			// Perhaps we do not need to full send if we do a downgrade? That would be an efficiency thing.

			// This is the 'upgrade' code. We have changed what level of information we are sending.
			// We need to do a full send.
