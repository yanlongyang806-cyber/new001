...
enumTransactionOutcome trCharacter_SetStatsPresetFromSaved(ATR_ARGS,NOCONST(Entity)* ent, AttribStatsPresetDef* pDef, S32 bReset)
{
	...
	{
		AssignedStats **ppAttribStats = pDef->ppAttribStats;
		int n;

		for(n = eaSize(&pDef->ppOverrides) -1; n >= 0; n--)
		{
			StatPointPoolDef *pStatPointPoolDef;
			int iPointsToSpend = 0;

			if(eaSize(&pDef->ppOverrides[n]->ppAttribStats) <= 0)
				continue;

			pStatPointPoolDef = StatPointPool_DefFromAttrib(pDef->ppOverrides[n]->ppAttribStats[0]->eType);

			iPointsToSpend = entity_GetAssignedStatAllowed(ent, pStatPointPoolDef->pchName);

			if(pDef->ppOverrides[n]->iTotalPoints == iPointsToSpend)
			{
				ppAttribStats = pDef->ppOverrides[n]->ppAttribStats;
				break;
			}
		}

		for ( j = eaSize(&ppAttribStats) - 1; j >= 0; j-- )
		{
			StatPointPoolDef *pStatPointPoolDef = StatPointPool_DefFromAttrib(ppAttribStats[j]->eType);
			if (pStatPointPoolDef == NULL || !trhCharacter_SetStatPoints(ent, pStatPointPoolDef->pchName, ppAttribStats[j]->eType, ppAttribStats[j]->iPoints))
				break;
		}
	}
...
bool gslExpLoveDispatch(ExpLoveBaton *baton)
{
	...
	if (!baton->list)
	{
		...
		{
			TransactionReturnVal *returnVal = LoggedTransactions_CreateManagedReturnValEnt("ExpLove", container->containerData, NULL, NULL);
			inv_FillItemChangeReason(&reason, NULL, "Internal", "gslExpLove");

			AutoTrans_inv_tr_ApplyNumeric(returnVal,
...
void ExpLove(Entity *pClientEntity, int iValue)
{
	...
	baton->BagID = GetBestBagForItemDef(pClientEntity, pItemDef, NULL, -1, false, NULL);
	baton->Exp = iValue;
...
void ExpLove2(Entity *pClientEntity, int iValue)
{
	...
	pItemDef = item_DefFromName(gConf.pcLevelingNumericItem);
	baton->BagID = GetBestBagForItemDef(pClientEntity, pItemDef, NULL, -1, false, pExtract);
	baton->Exp = iValue;
...
AUTO_COMMAND ACMD_ACCESSLEVEL(5);
void GiveNumeric(Entity *pClientEntity, ACMD_NAMELIST("ItemDef", REFDICTIONARY) const char* numericType, int iValue)
{
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_PRODUCTS(StarTrek, FightClub);
