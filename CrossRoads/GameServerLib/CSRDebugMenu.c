...
AUTO_COMMAND ACMD_ACCESSLEVEL(4) ACMD_SERVERCMD ACMD_NAME(ugc_TempBanProject);
...
AUTO_COMMAND ACMD_ACCESSLEVEL(4) ACMD_SERVERCMD ACMD_NAME(ugc_TempBanProjectByID);
...
AUTO_COMMAND ACMD_NAME(ugcFeatured_SetAverageDurationInMinutesOverride) ACMD_ACCESSLEVEL(4);
void gslUGC_FeaturedSetAverageDurationInMinutesOverride( SA_PARAM_NN_VALID Entity* ent, ContainerID projectId, float averageDurationInMinutes )
{
	RemoteCommand_Intershard_aslUGCDataManager_FeaturedSetAverageDurationInMinutesOverride(
			ugc_ShardName(), GLOBALTYPE_UGCDATAMANAGER, 0,
			GetShardNameFromShardInfoString(),
			entGetContainerID( ent ),
			projectId, SAFE_MEMBER2(ent, pPlayer, publicAccountName), averageDurationInMinutes );
}
