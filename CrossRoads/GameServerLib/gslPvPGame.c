...
static PVPLogging s_PVPLogging = {0};
...
void gslPVPGame_awardPoints(Entity *pEntity, int iPoints, const char *pchMessage)
{
	...
	if(pValue && iPoints)
	{
		...
		eventsend_RecordPvPPlayerPointsAwarded(entGetPartitionIdx(pEntity), pEntity, iPoints);
...
void gslPVPGame_PlayerKilled(Entity *pKilled, Entity *pKiller)
{
	...
	if (!pGameDetails || !entIsPlayer(pKilled))
		return; // was not a player that was killed
	...
	if (pGameDetails->pQueueMatch)
	{
		...
		{
			FOR_EACH_IN_EARRAY(pGroup->eaMatchMembers, QueueMatchMember, pMember)
			{
	...
	for(i=0;i<eaSize(eaTeams);i++)
	{
		for(m=0;m<eaSize(&(*eaTeams)[i]->eaMembers);m++)
		{
			...
			if(pPlayer)
			{
				...
				else if (pKillCredit->fPercentCreditSelf)
				{
					// the above check duplicates one in Reward.c.  Make sure you adjust both.  Probably should factor the logic out
					// into a function they both call.
...
void pvpGame_InitGroups(PVPCurrentGameDetails *pGameDetails)
{
	...
	if(pGameDetails && pGameDetails->pRules && pGameDetails->pRules->publicRules.eGameType)
	{
...
void gslPVPGame_start(QueuePartitionInfo *pInfo,QueueDef *pDef)
{
	...
	{
		int iMember;
		for(iMember=0;iMember<eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);iMember++)
		{
			Entity *pPlayer = entFromContainerID(pInfo->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[i]->eaMatchMembers[iMember]->uEntID);

			if(!gConf.bDisableRespawnOnPvPMatchStart && pPlayer)
			{
														...
														QueueDef *pQueueDef, Entity **ppEnts, QueueRewardTable *pRewardInfo)
{
	...
	float fPvPMatchLengthScale;
	
	// Calculate a scalar based on the duration of the match so we can reduce awards for short matches (to prevent short-match abuse)
	if (g_QueueConfig.uPvPRewardScalarNormativeTime==0)
	{
		fPvPMatchLengthScale=1.0f;
	}
	else
	{
		fPvPMatchLengthScale = (float)(mapState_GetScoreboardTotalMatchTimeInSeconds(pMapState)) / (float)(g_QueueConfig.uPvPRewardScalarNormativeTime);
		if (fPvPMatchLengthScale < 0.0f)
		{
			fPvPMatchLengthScale=0.0f;
		}
		if (fPvPMatchLengthScale > 1.0f)
		{
			fPvPMatchLengthScale=1.0f;
		}
	}
	...
	if(pRewardTable)
	{
		...
		FOR_EACH_IN_EARRAY_FORWARDS(ppEnts, Entity, pEnt)
		{
			...
			if (pQueueDef->uMinimumPointsForReward)
			{
				F32 fPoints = 0.f;
				MultiVal *mFactor = mapState_GetPlayerValue(pMapState, pEnt, "player_points");
				if(mFactor)
					fPoints = MultiValGetFloat(mFactor,NULL);
				if (fPoints < (F32)pQueueDef->uMinimumPointsForReward)
				{
					continue;
				}
			}
			...
			reward_PowerExec(pEnt, pRewardTable, iLevel, fPvPMatchLengthScale, true, &reason);
		}
		FOR_EACH_END
	}
...
static void LogGroupResult(const char *logStr, QueueGroup *pQueueGroup, PVPGroupGameParams *pGroupParams, PVPCurrentGameDetails *pGameDetails, MapState *pMapState)
{
	char *estrBuffer = NULL;

	if(nullStr(logStr) || !pQueueGroup || !pGameDetails || !pMapState || !pMapState->pPlayerValueData)
		return;

	log_printf(LOG_PVP, "[partition %d][Score %d]", pGameDetails->iPartitionIdx, pGroupParams->iScore);

	estrStackCreate(&estrBuffer);

	FOR_EACH_IN_EARRAY_FORWARDS(pQueueGroup->eaMatchMembers, QueueMatchMember, pQueueMatchMember)
	{
		if(pQueueMatchMember)
		{
			Entity *pEnt = entFromContainerID(pGameDetails->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER, pQueueMatchMember->uEntID);
			CharacterAttribs *pCharacterAttribs = SAFE_MEMBER2(pEnt, pChar, pattrBasic);
			PlayerMapValues *pPlayerMapValues = eaIndexedGetUsingInt(&pMapState->pPlayerValueData->eaPlayerValues, pQueueMatchMember->uEntID);

			estrClear(&estrBuffer);

			estrConcatf(&estrBuffer, "[partition %d]", pGameDetails->iPartitionIdx);

			if(pPlayerMapValues)
			{
				FOR_EACH_IN_EARRAY_FORWARDS(pPlayerMapValues->eaValues, MapStateValue, pMapStateValue)
				{
					if(pMapStateValue)
					{
						if(!nullStr(pMapStateValue->pcName))
						{
							bool tst;
							S64 value = MultiValGetInt(&pMapStateValue->mvValue, &tst);
							if(tst)
								estrConcatf(&estrBuffer, "[%s %lld]", pMapStateValue->pcName, value);
						}
					}
				}
				FOR_EACH_END;
			...
			if(pCharacterAttribs)
			{
				FOR_EACH_IN_EARRAY_FORWARDS(s_PVPLogging.eaBasicAttribs, char, attribName)
				{
					if(!nullStr(attribName))
					{
						S32 eAttrib = StaticDefineIntGetInt(AttribTypeEnum, attribName);
						if(eAttrib >= 0 && IS_NORMAL_ATTRIB(eAttrib))
						{
							F32 attrib = *F32PTR_OF_ATTRIB(pCharacterAttribs, eAttrib);
							estrConcatf(&estrBuffer, "[%s %f]", attribName, attrib);
						}
					}
				}
				FOR_EACH_END;
			...
			entLog(LOG_PVP, pEnt, logStr, "%s", estrBuffer);
		}
	}
	FOR_EACH_END;
...
void gslPVPGame_endWithWinner(int iPartitionIdx, int iWinningGroup, int iHighScore, bool bAllowTies)
{
	...
	PVPCurrentGameDetails *pGameDetails = gslPvPGameDetailsFromIdx(iPartitionIdx);
	MatchMapState * pMatchMapState = NULL;
	QueueMatch * pQueueMatch = NULL;
	const char *pchWinningFaction = NULL;
	ItemChangeReason reason = {0};

	if (pGameDetails == NULL || pQueueDef == NULL)
		return;

	pMatchMapState = pGameDetails->pMapState;
	pQueueMatch = pGameDetails->pQueueMatch;
	
	if(	pMatchMapState == NULL || pQueueMatch == NULL)
		return;
	// if we've already gotten set to kScoreboardState_Final, do not process the winnings again
	...
	if(eaSize(&pGameDetails->pRules->ppchRankingLeaderboards))
	{
		...
		for(i=0;i<eaSize(&pQueueMatch->eaGroups);i++)
		{
			for(m=0;m<eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);m++)
			{
				Entity *pEnt = entFromContainerID(pGameDetails->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID);

				if(i==iWinningGroup)
				{
					ea32Push(&pWinners->piTeam,pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID);
				}
				else
				{
					ea32Push(&pLosers->piTeam,pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID);
				}
	...
	log_printf(LOG_PVP, "[partition %d][Queue %s][Map %s][TotalMatchTime %us]", iPartitionIdx, pQueueDef->pchName, NULL_TO_EMPTY(zmapInfoGetPublicName(NULL)), mapState_GetScoreboardTotalMatchTimeInSeconds(pMapState));

	for(i=0;i<eaSize(&pQueueMatch->eaGroups);i++)
	{
		QueueRewardTable **eaRewards = NULL;
		Entity **ppEnts = NULL;
		PVPGroupGameParams *pGroupParams = eaGet(&pMatchMapState->ppGroupGameParams, i);
		int j;
		...
		if(i == iWinningGroup)
		{
			...
			LogGroupResult("WIN", pQueueMatch->eaGroups[i], pGroupParams, pGameDetails, pMapState);
		}
		else if(bAllowTies && (pGroupParams ? pGroupParams->iScore : 0) == iHighScore)
		{
			...
			LogGroupResult("DRAW", pQueueMatch->eaGroups[i], pGroupParams, pGameDetails, pMapState);
		}
		else
		{
			gslPVPGame_GetQueueRewards(pQueueDef, kQueueRewardTableCondition_Loss, &eaRewards);

			LogGroupResult("LOSS", pQueueMatch->eaGroups[i], pGroupParams, pGameDetails, pMapState);
		}
		
		for (m=0;m<eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);m++)
		{
			Entity *pPlayer = entFromContainerID(pInfo->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID);
	...
	FOR_EACH_IN_EARRAY(pQueueDef->eaRewardTables, QueueRewardTable, pRewardTable)
	{
			...
			pRewardTable->pExprRewardCondition)
		{
			...
			{
				FOR_EACH_IN_EARRAY_FORWARDS(pGroup->eaMatchMembers, QueueMatchMember, pMember)
				{
...
void gsl_PVPGame_DOM_GameStart(PVPCurrentGameDetails *pGameDetails, PVPGameRules *pGameRules)
{
	...
	do
	{
		...
		if(pEncounter)
		{
			...
			for (j = 0; j < encounter_GetNumActors(pEncounter); ++j)
			{
				if (pEncounter && pEncounter->pWorldEncounter
							   && pEncounter->pWorldEncounter->properties
							   && pEncounter->pWorldEncounter->properties->eaActors)
				{
					pMessage = encounter_GetActorDisplayMessage(pGameDetails->iPartitionIdx, pEncounter->pWorldEncounter->properties, pEncounter->pWorldEncounter->properties->eaActors[j]);
				}

				if (pMessage)
				{
					pCurrentPoint->pMessage = StructCreate(parse_Message);
					StructCopy(parse_Message, pMessage, pCurrentPoint->pMessage, 0, 0, 0);
					break;
				}
			}
...
void gslPVPGame_DOM_ProcessPoints(PVPCurrentGameDetails * pGameDetails, MapState *pMapState, F32 fTime)
{
	...
	for(i=0;i<eaSize(&pMatchMapState->ppGameSpecific);i++)
	{
		...
		{
			for(m=0;m<eaSize(&pQueueMatch->eaGroups[g]->eaMatchMembers);m++)
			{
				Entity *pEnt = entFromContainerID(pGameDetails->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[g]->eaMatchMembers[m]->uEntID);
				...
				if(pEnt)
				{
					...
					if(distance3Squared(vPlayerLocation,pPoint->vLocation) <= pGameDetails->pRules->fCaptureDistance * pGameDetails->pRules->fCaptureDistance)
					{
						...
						if(g!=pPoint->iOwningGroup)
							ea32Push(&pPoint->iAttackingEnts,pQueueMatch->eaGroups[g]->eaMatchMembers[m]->uEntID);
					}
				}
			}
		}
		...
		switch (pGroupParams->eFlagStatus)
		{
			...
			case kCTFFlagStatus_Dropped:
				...
				for(iGroup=0;iGroup<eaSize(&pQueueMatch->eaGroups);iGroup++)
				{
					...
					for(iMember=0;iMember<eaSize(&pQueueMatch->eaGroups[iGroup]->eaMatchMembers);iMember++)
					{
						Entity *pPlayer = entFromContainerID(pGameDetails->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[iGroup]->eaMatchMembers[iMember]->uEntID);
...
void gslPVPGame_DM_PlayerKilled(PVPCurrentGameDetails *pGameDetails, Entity *pKilled, Entity *pKiller)
{
	...
	for(i=0;i<eaSize(&pQueueMatch->eaGroups);i++)
	{
		...
		for(m=0;m<eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);m++)
		{
			if(entGetContainerID(pMember) == pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID)
			{
...
void gslPVPGame_LMS_RoundStart(PVPCurrentGameDetails *pGameDetails)
{
	...
	for(i=0;i<eaSize(&pQueueMatch->eaGroups);i++)
	{
		...
		for(m=0;m<eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);m++)
		{
			Entity *pPlayer = entFromContainerID(pGameDetails->iPartitionIdx,GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID);
...
void gslPVPGame_LMS_PlayerKilled(PVPCurrentGameDetails *pGameDetails, Entity *pKilled, Entity *pKiller)
{
	...
	for(i=0;i<eaSize(&pQueueMatch->eaGroups);i++)
	{
		...
		for(m=0;m<eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);m++)
		{
			if(pQueueMatch->eaGroups[i]->eaMatchMembers[m]->uEntID != entGetContainerID(pKilled))
				continue;
...
void gslPVPGame_update(PVPCurrentGameDetails *pGameDetails, F32 fTime)
{
	...
	if (pQueueMatch)
	{
		...
		{
			pGameDetails->pMapState->ppGroupGameParams[i]->iNumMembers = eaSize(&pQueueMatch->eaGroups[i]->eaMatchMembers);
		}
	}
	...
	if(pGameDetails->bTimerCountDown)
	{
		...
		else if(eCurrentState == kScoreboardState_Init && pGameDetails->bTimerCountDown && pGameDetails->fTimer > g_QueueConfig.uMinGameInitTime)
		{
			...
			for(iGroup=0;iGroup<eaSize(&pQueueMatch->eaGroups);iGroup++)
			{
				for(iMember=0;iMember<eaSize(&pQueueMatch->eaGroups[iGroup]->eaMatchMembers);iMember++)
				{
					Entity *pPlayerEntity = entFromContainerID(pGameDetails->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pQueueMatch->eaGroups[iGroup]->eaMatchMembers[iMember]->uEntID);
...
static void PVPGameReload(const char *pchRelPath, int UNUSED_when)
{
	...
	StructInit(parse_PVPLogging, &s_PVPLogging);
	ParserLoadFiles(NULL,"defs/config/PVPLogging.def","PVPLogging.bin",PARSER_OPTIONALFLAG,parse_PVPLogging,&s_PVPLogging);
	loadend_printf(" done.");
}
...
void PVPGameLoad(void)
{
	...
	if(isDevelopmentMode())
	{
		...
		FolderCacheSetCallback(FOLDER_CACHE_CALLBACK_UPDATE, "defs/config/PVPLogging.def", PVPGameReload);
	}
