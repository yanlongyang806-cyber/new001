...
static int gslTeam_CountTeamPets(SavedPetTeamList *pTeamList)
{
	S32 i, j, n, iPets = 0;
	U32 *piPlayers = NULL;
	S32 *piCounts = NULL;
	...
	n = pTeamList ? ea32Size(&pTeamList->uiPetIDs) : 0;
	for (i = 0; i < n; i++)
	{
		U32 uiPetID = pTeamList->uiPetIDs[i];
		U32 uiOwnerID = pTeamList->uiOwnerIDs[i];
		Entity *pOwner = entFromContainerID(pTeamList->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER, uiOwnerID);

		j = ea32Find(&piPlayers, uiOwnerID);
		if (j < 0)
		{
			if (pOwner)
			{
				entGetPos(pOwner, vEntPos);
				pRules = RegionRulesFromVec3(vEntPos);
			}

			j = ea32Size(&piPlayers);
			ea32Push(&piPlayers, uiOwnerID);
			eaiPush(&piCounts, !pRules || pRules->iUnteamedPetsPerPlayer <= 0 ? 0 : -pRules->iUnteamedPetsPerPlayer);
		}
		piCounts[j]++;
	}

	for (i = 0; i < ea32Size(&piCounts); i++)
	{
		if (piCounts[i] > 0)
			iPets += piCounts[i];
	}

	ea32Destroy(&piPlayers);
	ea32Destroy(&piCounts);

	return iPets;
}

static int gslTeam_CountEntTeamPets(SavedPetTeamList *pTeamList, Entity *pEnt)
{
	S32 i, n, iPets = 0;
	EntityRef erEnt;
	RegionRules *pRules;
	...
	pRules = RegionRulesFromVec3(vEntPos);
	if (pRules && pRules->iUnteamedPetsPerPlayer > 0)
		iPets = -pRules->iUnteamedPetsPerPlayer;

	erEnt = entGetRef(pEnt);

	n = pTeamList ? ea32Size(&pTeamList->uiOwnerIDs) : 0;
	for (i = 0; i < n; i++)
	{
		if (pTeamList->uiOwnerIDs[i] == entGetContainerID(pEnt))
			iPets++;
	}
						  ...
						  PropPowerSaveList* pSavePowerList)
{
	...
	NOCONST(AlwaysPropSlot) *pNewSlot = NULL;
	NOCONST(AlwaysPropSlot) *pOldSlot = NULL;
	int i;
	bool bSuccess = true;
	...
	if ( bSuccess )
	{
		pNewSlot = AlwaysPropSlot_trh_FindByPetID(pOwner, pNewPetEnt->pSaved->iPetID, uiPropEntID, ePropCategory);
	}
	...
	if ( bSuccess && NONNULL(pOldPetEnt) && NONNULL(pOldPetEnt->pSaved) )
	{
		...
		if ( bSuccess )
		{
			pOldSlot = AlwaysPropSlot_trh_FindByPetID(pOwner, pOldPetEnt->pSaved->iPetID, uiPropEntID, ePropCategory);
	...
	{
		int iOldPetID = -1;
		pOldSlot = AlwaysPropSlot_trh_FindBySlotID(pOwner, iSlotID);

		if ( pOldSlot )
			iOldPetID = pOldSlot->iPetID;
		else
			bSuccess = false;
	}
	else
	...
	if ( bSuccess )
	{
		...
		if ( NONNULL(pOldPetRel) )
		{
			...
			if ( pNewSlot )
			{
				int iNewSlotID = pNewSlot->iSlotID;
		...
		if ( NONNULL(pNewPetRel) && bSuccess )
		{
			...
			if ( pOldSlot )
			{
				int iOldSlotID = pOldSlot->iSlotID;
		...
		if (bSuccess && NONNULL(pOldPetEnt) && !pNewSlot && pSavePowerList && eaSize(&pSavePowerList->eaData))
		{
...
static void gslTeam_AddPetToTeamListEx(int iPartitionIdx, U32 uOwnerID, GlobalType eTeamType, U32 uiTeamID, U32 uiPetID)
{
	...
	i = ea32Find(&sTeamList->uiPetIDs,uiPetID);
	ea32Set(&sTeamList->uiOwnerIDs,uOwnerID,i);
}

static void gslTeam_AddPetToTeamList(Entity *pOwner, GlobalType eTeamType, U32 uiTeamID, U32 uiPetID)
{
	gslTeam_AddPetToTeamListEx(entGetPartitionIdx(pOwner), entGetContainerID(pOwner), eTeamType, uiTeamID, uiPetID);
}
...
static void gslTeam_RemovePetTeamList(SavedPetTeamList *pTeamList)
{
	...
	ea32Destroy(&pTeamList->uiOwnerIDs);
	free(pTeamList);
}
...
static int gslTeam_GetOpenSlots(int iPartitionIdx, SavedPetTeamList *pTeamList, Team *pTeam)
{
	...
	if(pTeamList && pTeamList->eGlobalType == GLOBALTYPE_TEAM)
	{
		...
		iOpenSpots = TEAM_MAX_SIZE - gslTeam_CountTeamPets(pTeamList) - iTeamMatesActive;
	}
	else
	{
		//-1 for counting yourself
		iOpenSpots = TEAM_MAX_SIZE - gslTeam_CountTeamPets(pTeamList) - 1;
	}
...
{
	int iFilledSlots=0, iOpenSpots=0, iPetsPerPlayer;
	...
	if (pRegionRules==NULL || (pRegionRules->iAllowedPetsPerPlayer < 0 && pRegionRules->iUnteamedPetsPerPlayer <= 0))
		return gslTeam_GetOpenSlots(iPartitionIdx, pTeamList, GET_REF(pEnt->pTeam->hTeam));

	if (pRegionRules->iAllowedPetsPerPlayer == 0 && pRegionRules->iUnteamedPetsPerPlayer <= 0)
		return 0;

	// Count how many spots on the team are open for filling
	iOpenSpots = gslTeam_GetOpenSlots(iPartitionIdx, pTeamList,pEnt->pTeam ? GET_REF(pEnt->pTeam->hTeam) : NULL);

	// Count how many pets the player has out, if it's negative then the player
	// doesn't have enough pets out to take up team slots. That is the player has
	// some free unteamed pet slots available.
	iFilledSlots = gslTeam_CountEntTeamPets(pTeamList, pEnt);

	// Determine how many pets the player may use to fill out the team.
	// If iFilledSlots > 0, then some of the player's pets are being used
	// to pad the team.
	iPetsPerPlayer = MAX(0, pRegionRules->iAllowedPetsPerPlayer);
	iPetsPerPlayer -= MAX(0, iFilledSlots);

	// Clamp the number of open team slots available for the player's pets
	// by how many more pets the player is allowed to take out.
	iOpenSpots = MIN(iOpenSpots, iPetsPerPlayer);

	// If the player has free unteamed pet slots, add them to the total
	// number of open spots.
	iOpenSpots += MAX(0, -iFilledSlots);
...
static void gslTeam_RemovePetFromTeamList(int iPartitionIdx, GlobalType eTeamType, U32 uiTeamID, U32 uiPetID)
{
	...
	if(sTeamList)
	{
		int idx = ea32FindAndRemove(&sTeamList->uiPetIDs,uiPetID);
		ea32Remove(&sTeamList->uiOwnerIDs,uiPetID);
...
static bool gslTeam_PetSpawnIfAvailableSlots(Entity *pOwner, U32 uiPetID)
{
	...
	if (gslTeam_IsRoomAvailableForPet(pOwner, uiPetID, &bAlreadySummoned))
	{
		...
		{
			if(team_IsMember(pOwner)) {
				gslTeam_AddPetToTeamList(pOwner, GLOBALTYPE_TEAM, pOwner->pTeam->iTeamID, uiPetID);
			} else {
				gslTeam_AddPetToTeamList(pOwner, GLOBALTYPE_ENTITYPLAYER, entGetContainerID(pOwner), uiPetID);
			}
		}
		return true;
	}
...
static bool gslTeam_PetSpawnRequest(Entity *pOwner, U32 uiPetID, bool bCareAboutAwayTeamSize)
{
	...
	if(bOpenSlots)
	{
		...
		if(pTeam!=NULL && pOwner->pSaved)
		{
			...
			gslTeam_AddPetToTeamList(pOwner,GLOBALTYPE_ENTITYPLAYER,pOwner->myContainerID,uiPetID);
			return true;
		}
	}
...
static void gslTeam_AutoFillPetTeamList(int iPartitionIdx, SavedPetTeamList *pTeamList, Team *pTeam, RegionRules *pRules)
{
	...
	if (pTeam)
	{
		for(i=0;i<eaSize(&pTeam->eaMembers);i++)
		{
			...
			if (pMember && pMember->pSaved)
			{
				int iMinSlots = gslTeam_CountTeamPets(pTeamList);
				if (iMinSlots < 0)
					iOpenSlots += -iMinSlots;
			}
		}
	}
	...
	if(iOpenSlots > 0)
	{
		...
		if(pTeamList->eGlobalType == GLOBALTYPE_TEAM)
		{
			if(pTeam)
			{
				for(i=0;i<eaSize(&pTeam->eaMembers);i++)
				{
					...
					if(pTeamMember && pTeamMember->pSaved)
		...
		for(i=0;i<eaSize(&ppTeamMembers);i++)
		{
			Entity *pTeamMember = ppTeamMembers[i];
			int j;
			ea32Set(&piCount, 0, i);
		...
		while(eaSize(&ppPets) > 0 && iOpenSlots > 0)
		{
			...
			if(!pPet || !pOwner || iOwnerSlot<0 || piCount[iOwnerSlot]>=(pRules->iAllowedPetsPerPlayer + MAX(0,pRules->iUnteamedPetsPerPlayer)))
			{
				if (!pPet)
				{
			...
			//Spawn pet
			gslTeam_AddPetToTeamList(pOwner,pTeamList->eGlobalType,pTeamList->iUniqueID,uiPetID);
		...
		{
			int iTeamSize = eaSize(&ppTeamMembers);
			int iSelected = iTeamSize > 1 ? randomIntRange(0, iTeamSize - 1) : 0;
			Entity* pTeamMember = ppTeamMembers[iSelected];
			int iCritterPetSize = eaSize(&pTeamMember->pSaved->ppAllowedCritterPets);
			int iCritterSelected = iCritterPetSize > 1 ? randomIntRange(0, iCritterPetSize - 1) : 0;
			PetDefRefCont* pPetDefRefCont = iCritterPetSize > iCritterSelected ? pTeamMember->pSaved->ppAllowedCritterPets[iCritterSelected] : NULL;
			PetDef* pPetDef = SAFE_GET_REF(pPetDefRefCont, hPet);
			CritterPetRelationship *pNewRelationship = NULL;

			if (!pPetDef || piCount[iSelected]>=(pRules->iAllowedPetsPerPlayer + MAX(0,pRules->iUnteamedPetsPerPlayer)))
			{
				if (!pPetDef && pPetDefRefCont)
				{
			...
			gslTeam_AddPetToTeamList(pTeamMember,pTeamList->eGlobalType,pTeamList->iUniqueID,pNewRelationship->uiPetID);
								   ...
								   AlwaysPropSlotCategory ePropCategory)
{
	...
	{
		AlwaysPropSlot* pPropSlot = AlwaysPropSlot_FindByPetID(pOwner, pOldPet->pSaved->iPetID, uiPropEntID, ePropCategory);
	...
	if(peaOldPropEntIDs)
	{
		for (i = 0; i < ea32Size(peaOldPropEntIDs); i++)
		{
			...
			if (ea32Find(peaPropEntIDs, uiOldPropEntID) < 0)
			{
				...
				{
					AlwaysPropSlot* pPropSlot = AlwaysPropSlot_FindByPetID(pOwner, pPetEnt->pSaved->iPetID, uiOldPropEntID, ePropCategory);
					AlwaysPropSlotDef* pPropSlotDef = pPropSlot ? GET_REF(pPropSlot->hDef) : NULL;
					if (pPropSlotDef)
					{
...
static bool gslEntity_RemoveLeastPreferredPet(Entity* pEnt, SavedPetTeamList* pPetTeamList)
{
	...
	if (iRemoveIndex >= 0 && gslUnSummonSavedPet(pLeastPreferredPetEnt))
	{
		...
		ea32RemoveFast(&pPetTeamList->uiOwnerIDs,iRemoveIndex);
									...
									int* piFailedCount)
{
	...
	{
		S32 i, iTeamPets = gslTeam_CountEntTeamPets(pPetTeamList, pEnt);
		if (iTeamMatesActive + iTeamPetsActive > TEAM_MAX_SIZE && iTeamPets > 0)
		{
			...
			for (i = ea32Size(&pPetTeamList->uiPetIDs) - 1; i >= 0; i--)
			{
				...
				if (uiPetID & CRITTERPETIDFLAG)
				{
					...
					if (Entity_RemoveCritterPet(pEnt, uiPetID, &bFailed))
					{
						...
						ea32RemoveFast(&pPetTeamList->uiOwnerIDs,i);
						ea32FindAndRemove(&pEnt->pSaved->ppRequestedCritterIDs, uiPetID);
						iTeamPetsActive = gslTeam_CountTeamPets(pPetTeamList);
						--iTeamPets;
		...
		while (iTeamMatesActive + iTeamPetsActive > TEAM_MAX_SIZE && iTeamPets > 0 &&
...
void gslTeam_UnSummonPetsForEnt(int iPartitionIdx, Entity* pEnt)
{
	...
	for (i = ea32Size(&pTeamList->uiPetIDs) - 1; i >= 0; i--)
	{
		...
		if (uiPetID & CRITTERPETIDFLAG && Entity_RemoveCritterPet(pEnt, uiPetID, NULL))
		{
			...
			ea32RemoveFast(&pTeamList->uiOwnerIDs,i);
			ea32Push(&pEnt->pSaved->uiRemovedAwayTeamPetIDs, uiPetID);
		}
	}
}
...
void gslTeam_ReSummonPetsForEnt(Entity *pOwner)
{
	...
	while (ea32Size(&pOwner->pSaved->uiRemovedAwayTeamPetIDs) && iOpenSlots > 0 && iPetCount < (MAX(0, pRules->iAllowedPetsPerPlayer) + MAX(0, pRules->iUnteamedPetsPerPlayer)))
	{
		...
		if (uiPetID & CRITTERPETIDFLAG) // Critter Pet
		{
			...
			gslTeam_AddPetToTeamList(pOwner, pTeamList->eGlobalType, pTeamList->iUniqueID, pNewRelationship->uiPetID);
		}
		else // Saved Pet
		{
...
static void gslPetTeamList_convert(int iPartitionIdx, GlobalType eOldTeam, ContainerID iOldTeamID, GlobalType eNewTeam, ContainerID iNewTeamID )
{
	...
	if(pOldTeam)
	{
		...
		{
			gslTeam_AddPetToTeamListEx(iPartitionIdx,pOldTeam->uiOwnerIDs[i],eNewTeam,iNewTeamID,pOldTeam->uiPetIDs[i]);
		}
...
void gslPetTeamList_LeaveTeam(Entity *pLeaveEnt, Team *pTeam)
{
	...
	for(i=0;i<eaSize(&pLeaveEnt->pSaved->ppOwnedContainers);i++)
	{
		if(pLeaveEnt->pSaved->ppOwnedContainers[i]->curEntity)
			gslTeam_AddPetToTeamList(pLeaveEnt,GLOBALTYPE_ENTITYPLAYER,pLeaveEnt->myContainerID,pLeaveEnt->pSaved->ppOwnedContainers[i]->conID);
	}

	for(i=0;i<eaSize(&pLeaveEnt->pSaved->ppCritterPets);i++)
	{
		if(pLeaveEnt->pSaved->ppCritterPets[i]->pEntity)
			gslTeam_AddPetToTeamList(pLeaveEnt,GLOBALTYPE_ENTITYPLAYER,pLeaveEnt->myContainerID,pLeaveEnt->pSaved->ppCritterPets[i]->uiPetID);
	}
...
void FillPetTeamList(Entity *pOwner)
{
	...
	Vec3 vOwnerPos;
	RegionRules *pRules = NULL;
	static U32 petsDisabledType = 0;

	if(!pOwner)
		return;

	if (!petsDisabledType)
		petsDisabledType = wlVolumeTypeNameToBitMask("PetsDisabled");

	// If we are in a Pets Disabled volume, don't fill the pet team list
	if(volume_IsEntityInVolumeByType(pOwner, petsDisabledType))
		return;
...
AUTO_COMMAND ACMD_SERVERCMD;
void DismissAllPets(Entity *pOwner)
{
	SavedPetTeamList *pList = NULL;
	int iPartitionIdx = entGetPartitionIdx(pOwner);
	int i;
	...
	if(team_IsMember(pOwner))
		pList = gslTeam_FindPetTeamList(iPartitionIdx, GLOBALTYPE_TEAM, pOwner->pTeam->iTeamID);
	else
		pList = gslTeam_FindPetTeamList(iPartitionIdx, GLOBALTYPE_ENTITYPLAYER, pOwner->myContainerID);

	if (pList)
	{
		for (i = ea32Size(&pList->uiPetIDs) - 1; i >= 0; i--)
		{
			U32 uiPetID = pList->uiPetIDs[i];

			if (uiPetID & CRITTERPETIDFLAG)
			{
				Entity_RemoveCritterPet(pOwner, uiPetID, NULL);
				ea32RemoveFast(&pList->uiPetIDs, i);
				ea32RemoveFast(&pList->uiOwnerIDs, i);
			}
			else
			{
				Entity *pSavedPetEnt = entity_GetSubEntity(iPartitionIdx, pOwner, GLOBALTYPE_ENTITYSAVEDPET, uiPetID);
				if (pSavedPetEnt)
				{
					gslUnSummonSavedPet(pSavedPetEnt);
				}
		...
		if (pPetEnt && pPetEnt->myContainerID == (ContainerID)ID && (conRelation->eRelationship == CONRELATION_PET || conRelation->eRelationship == CONRELATION_PRIMARY_PET))
		{
 			...
 			if(!pRegionRules || (pClass && ea32Find(&pRegionRules->peCharClassTypes, pClass->eType)>=0) && pPet != pOwner)
			{
				if (gslTeam_PetSpawnRequest(pOwner, pPet->myContainerID, false))
				{
					...
					{
						gslTeam_AddPetToTeamList(pOwner,GLOBALTYPE_TEAM,pOwner->pTeam->iTeamID,ID);
					}
					else if(pOwner)
					{
						gslTeam_AddPetToTeamList(pOwner,GLOBALTYPE_ENTITYPLAYER,pOwner->myContainerID,ID);
					}
				}
			}
		}
...
static AlwaysPropSlotDefRef *AlwaysPropSlotDef_FindDefIndex(AlwaysPropSlotDef* pDef, U32 uiPuppetID, AlwaysPropSlotDefRef*** peaList, S32 iCount)
{
	S32 i;
...
static void AlwaysPropSlotDef_PushUniqueToRefList(AlwaysPropSlotDef* pDef, U32 uiPuppetID, AlwaysPropSlotDefRef*** peaList, S32* piCount)
{
	AlwaysPropSlotDefRef *pDefRef = AlwaysPropSlotDef_FindDefIndex(pDef, uiPuppetID, peaList, *piCount);
	if (pDefRef)
	{
		pDefRef->iCount++;
	}
...
bool Entity_ValidateAlwaysPropSlots_HasTempPuppetWithType(Entity *pEntity, CharClassTypes eClassType)
{
	int i;
	for(i=eaSize(&pEntity->pSaved->pPuppetMaster->ppTempPuppets)-1; i>=0; i--)
	{
		PetDef *pTempPetDef = GET_REF(pEntity->pSaved->pPuppetMaster->ppTempPuppets[i]->hPetDef);
		if(petdef_GetCharacterClassType(pTempPetDef) == eClassType)
			return true; 
	}
	return false;
}
...
static bool Entity_ValidateAlwaysPropSlots_FillSlotListFromPuppets(Entity *pEntity, AlwaysPropSlotDefRef ***peaFixupSlotRefs, int *piListSize, int *piSlotCount) 
{
	int i, j;
	int iPartitionIdx = entGetPartitionIdx(pEntity);
	for(i=0;i<eaSize(&pEntity->pSaved->ppOwnedContainers);i++)
	{
		PetRelationship *pRel = pEntity->pSaved->ppOwnedContainers[i];
		Entity *pPetEntity = SavedPet_GetEntity(iPartitionIdx, pRel);
		PetDef *pPetDef = SAFE_GET_REF2(pPetEntity, pCritter, petDef);
		
		if (pPetEntity == pEntity)
		{
			pPetEntity = GET_REF(pEntity->pSaved->ppOwnedContainers[i]->hPetRef);
			pPetDef = SAFE_GET_REF2(pPetEntity, pCritter, petDef);
		}

		if (pPetEntity && !pPetDef) // This is a captain
		{
			continue; 
		}

		if (pPetEntity && SavedPet_IsPetAPuppet(pEntity, pRel)) // This is a puppet
		{
			PuppetEntity *pPuppetEntity = SavedPet_GetPuppetFromContainerID(pEntity, pPetEntity->myContainerID);
			if (SAFE_MEMBER(pPuppetEntity, eState) != PUPPETSTATE_ACTIVE
				//|| Entity_ValidateAlwaysPropSlots_HasTempPuppetWithType(pEntity, pPuppetEntity->eType)
				)
			{
				continue;
			}
		...
		if (!pPetEntity && pRel->eState != OWNEDSTATE_ACTIVE)	// This is a pet
		{
			continue;
		}
		
		// check for ent after we know its puppet. Non-active puppets won't be subscribed in most cases 
		if (!pPetEntity)
		{
			pEntity->pSaved->bCheckPets = true;
			return false; // This means your pet isn't loaded, so we need to set the check pet flag
		}
		...
		for (j=0;j<eaSize(&pPetDef->ppAlwaysPropSlot);j++)
		{
			...
			if (pSlotDef)
			{
				AlwaysPropSlotDef_PushUniqueToRefList(pSlotDef, pPetEntity->myContainerID, peaFixupSlotRefs, piListSize);
				(*piSlotCount)++;
			}
		}
	}
...
static void Entity_ValidateAlwaysPropSlots_FillSlotListFromTempPuppets(Entity *pEntity, AlwaysPropSlotDefRef ***peaFixupSlotRefs, int *piListSize, int *piSlotCount ) 
{
	int i, j;
	if(pEntity->pSaved->pPuppetMaster)
	{
		for(i=0;i<eaSize(&pEntity->pSaved->pPuppetMaster->ppTempPuppets);i++)
		{
			...
			for(j=0;j<eaSize(&pPetDef->ppAlwaysPropSlot);j++)
			{
				...
				if (pSlotDef)
				{
					AlwaysPropSlotDef_PushUniqueToRefList(pSlotDef, pEntity->pSaved->pPuppetMaster->ppTempPuppets[i]->uiID, peaFixupSlotRefs, piListSize);
					(*piSlotCount)++;
				}
			}
		}
...
static bool Entity_ValidateAlwaysPropSlots_CheckSlotCounts(Entity *pEntity, AlwaysPropSlotDefRef*** peaFixupSlotRefs, int iSlotCount) 
{
	int i, j;
	...
	{
		for (i = eaSize(peaFixupSlotRefs)-1; i >= 0; i--)
		{
			S32 iCount = 0;
			for (j = eaSize(&pEntity->pSaved->ppAlwaysPropSlots)-1; j >= 0; j--)
			{
				...
				if (pSlotDef == GET_REF((*peaFixupSlotRefs)[i]->hDef))
				{
					iCount++;
				}
			}
			if (iCount != (*peaFixupSlotRefs)[i]->iCount)
			{
				return true;
			}
		}
...
static bool Entity_ValidateAlwaysPropSlots_CheckSlotIDs(Entity *pEntity) 
{
	int i;
	for (i = eaSize(&pEntity->pSaved->ppAlwaysPropSlots)-1; i >= 0; i--)
	{
		...
		{
			int j;
			// Checking for duplicate slot IDs
			for (j = i-1; j >= 0; j--)
			{
...
static bool Entity_ValidateAlwaysPropSlots_CheckSlotPuppetIDs(Entity *pEntity) 
{
	int i, j, k;
	static U32 eSpace = 0;
	...
	if (entCheckFlag(pEntity, ENTITYFLAG_PLAYER_LOGGING_IN) && pEntity->pSaved->pPuppetMaster)
	{
		for (i = 0; i < eaSize(&pEntity->pSaved->pPuppetMaster->ppPuppets); i++)
		{
			PuppetEntity *pActivePuppet = pEntity->pSaved->pPuppetMaster->ppPuppets[i];
			if (pActivePuppet->eState == PUPPETSTATE_ACTIVE && pActivePuppet->eType == eSpace)
			{
				S32 iPuppetSlotCount = eaSize(&pActivePuppet->ppSavedPropSlots);
				for (j = 0; j < iPuppetSlotCount; j++)
				{
					AlwaysPropSlot* pSlot = pActivePuppet->ppSavedPropSlots[j];
					bool bFound = (pSlot->iPuppetID == pActivePuppet->curID);

					// Check to see if this prop slot is pointing to an actual puppet
					if (!bFound && pSlot->iPuppetID)
					{
						PuppetEntity* pPuppet = NULL;
						for (k = 0; k < eaSize(&pEntity->pSaved->pPuppetMaster->ppPuppets); k++)
						{
							pPuppet = pEntity->pSaved->pPuppetMaster->ppPuppets[k];
...
static void Entity_ValidateAlwaysPropSlots(Entity *pEntity)
{
	...
	if(!Entity_ValidateAlwaysPropSlots_FillSlotListFromPuppets(pEntity, &s_ppFixupSlotRefs, &iListSize, &iSlotCount))
		return;
	Entity_ValidateAlwaysPropSlots_FillSlotListFromTempPuppets(pEntity, &s_ppFixupSlotRefs, &iListSize, &iSlotCount);

	bFixup = 
		Entity_ValidateAlwaysPropSlots_CheckSlotCounts(pEntity, &s_ppFixupSlotRefs, iSlotCount)
		|| Entity_ValidateAlwaysPropSlots_CheckSlotIDs(pEntity)
		|| Entity_ValidateAlwaysPropSlots_CheckSlotPuppetIDs(pEntity);
...
static void TransformtoPuppet_TransformComplete(SA_PARAM_NN_VALID Entity *pEnt)
{
	...
	//Send all the AttribModNets of this character, since he puppet swapped and
	//we need all new attrib mods on the client
	pEnt->pChar->bSendAllAttribModNet = true;
...
static void gslSetPuppetOffline_CB(TransactionReturnVal *pVal, void* userData)
{
...
AUTO_COMMAND ACMD_NAME(SetPuppetOffline) ACMD_ACCESSLEVEL(7) ACMD_SERVERCMD;
void gslSetPuppetOffline(Entity *pPlayerEnt, U32 uEntID, U32 uPuppetID)
{
	EntityRef uEntRef = entGetRef(pPlayerEnt);
	TransactionReturnVal* pReturn = LoggedTransactions_CreateManagedReturnVal("SetPuppetOffline", gslSetPuppetOffline_CB, (void *)(intptr_t)uEntRef);
	AutoTrans_trEntity_SetPuppetOffline(pReturn, GLOBALTYPE_GAMESERVER, GLOBALTYPE_ENTITYPLAYER, uEntID, uPuppetID);
}
...
void entity_PuppetSubscribe(Entity *pEnt)
{
	if(pEnt && pEnt->pSaved && pEnt->pSaved->pPuppetMaster && pEnt->pSaved->pPuppetMaster->ppPuppets)
	{
		...
		for(i=eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets)-1; i>=0; i--)
		{
			...
			{
				if(pEnt->pSaved->pPuppetMaster->bPuppetSubscribeAll || pEnt->pSaved->pPuppetMaster->ppPuppets[i]->eState == PUPPETSTATE_ACTIVE)
				{
					char idBuf[128];
...
{
	if(pEnt && pEnt->pSaved && pEnt->pSaved->ppOwnedContainers && pEnt->pSaved->pPuppetMaster)
	{
		int i;
		...
		for(i=eaSize(&pEnt->pSaved->ppOwnedContainers)-1; i>=0; i--)
		{
				...
				|| StringToContainerID(REF_STRING_FROM_HANDLE(pPet->hPetRef))!=pPet->conID)
			{
				if
				(
					SavedPet_DoesPetNeedToBeSubscribed(pEnt, pPet) || (pEnt->pSaved->pPuppetMaster->bPuppetSubscribeAll && SavedPet_IsPetAPuppet(pEnt, pPet)) ||
					(pEnt->pSaved->bSubscribeAllPets && !SavedPet_IsPetAPuppet(pEnt, pPet) )
				)
				{
					char idBuf[128];
					...
					pEnt->pSaved->bFixupPets = true;
					pPet->bNeedsFixup = true;
				}
			}
...
// Set the name and class of pets in the PetRelationship so it is available even when not subscribed.
// Now includes flag to always subscribe
void SavePet_SetPetNames(Entity *pEnt)
{
	if(pEnt && pEnt->pSaved && pEnt->pSaved->ppOwnedContainers && pEnt->pSaved->pPuppetMaster)
	{
		S32 i;
		...
		for(i=eaSize(&pEnt->pSaved->ppOwnedContainers)-1; i>=0; --i)
		{
			...
			if(pPet)
			{
				...
				if(pPetEnt && pPetEnt->pSaved)
				{
					...
					if(!pPet->pcName || stricmp(pPet->pcName, pPetEnt->pSaved->savedName) != 0)
					{
						if(pPet->pcName)
						{
							StructFreeString(pPet->pcName);
						}

						pPet->pcName = StructAllocString(pPetEnt->pSaved->savedName);
						bChangeMade = true;
					}

					if(pPetEnt->pChar && (!IS_HANDLE_ACTIVE(pPet->hCharacterClassRef)) ||
						(pPetEnt->pChar && !REF_COMPARE_HANDLES(pPet->hCharacterClassRef, pPetEnt->pChar->hClass)))
					{
						COPY_HANDLE(pPet->hCharacterClassRef, pPetEnt->pChar->hClass);
						bChangeMade = true;
					}

					if(pPetDef && !IS_HANDLE_ACTIVE(pPet->hPetDefRef) ||
						(pPetDef && !REF_COMPARE_HANDLES(pPet->hPetDefRef, pPetEnt->pCritter->petDef)))
					{
						COPY_HANDLE(pPet->hPetDefRef, pPetEnt->pCritter->petDef);
						bChangeMade = true;
					}
				}
			}
		...
		if(bChangeMade)
		{
			...
			// If a change was made, chances are other pets are broken too, so subscribe to them all
			// and run this fixup again. If we're already subscribed, then we don't need to do this
			if(!pEnt->pSaved->bSubscribeAllPets || !pEnt->pSaved->pPuppetMaster->bPuppetSubscribeAll)
			{
				PetCommands_SubscribeAllPuppets(pEnt);
				PetCommands_SubscribeAllPets(pEnt);
			}
		}
	}
}
...
void Entity_PetCheck(Entity *pEnt)
{
	...
	if(pEnt->pSaved->bCheckPets == false)
	{
		...
		// Fixup the pet names, classes and pet def, in case they are broken
		if(pEnt->pSaved->bSubscribeAllPets || (pEnt->pSaved->pPuppetMaster && pEnt->pSaved->pPuppetMaster->bPuppetSubscribeAll))
		{
			SavePet_SetPetNames(pEnt);
		}
	}
												   ...
												   GameAccountDataExtract *pExtract)
{
	...
	for(i=0; i<eaiSize(&pEnt->pSaved->piPetIDsRemovedFixup); i++)
	{
		...
		entity_ResetPowerIDsHelper(ATR_PASS_ARGS, pEnt, uiSourceMask, pExtract,false,NULL);
		for(j=eaSize(&ppOwned)-1; j>=0; j--)
		{
			entity_ResetPowerIDsHelper(ATR_PASS_ARGS, ppOwned[j], uiSourceMask, pExtract,false,NULL);
		}
	}
...
bool findRemovedPetIDInPowers(Entity *pEnt, Entity *pPet)
{
	...
	for(i=0;i<eaiSize(&pEnt->pSaved->piPetIDsRemovedFixup);i++)
	{
		...
		if(entity_ResetPowerIDsHelper(ATR_EMPTY_ARGS,(NOCONST(Entity)*)pPet,uiSourceMask,NULL,true,NULL))
		{
			return true;
		}
	}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pOwner, ".Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Hallegiance, .Hsuballegiance, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containertype, .Psaved.Conowner.Containerid, .Pplayer.Playertype, .Psaved.Ugamespecificfixupversion, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pinventoryv2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
ATR_LOCKS(pSavedPet, ".Psaved.Costumedata.Iactivecostume, .Psaved.Pptrayelems_Obsolete, .Psaved.Ppautoattackelems_Obsolete, .Psaved.Pppreferredpetids, .Pchar.Hclass, .Pchar.Ilevelexp, .Pinventoryv2, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pplayer.Playertype, .Pplayer.Eaplayernumericthresholds, .Hallegiance, .Hsuballegiance, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Pscpdata, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pchar.Pppowertrees, .Pchar.Ppassignedstats, .Pchar.Pppowersclass, .Pchar.Pppowerspersonal, .Pchar.Hspecies, .Psaved.Ugamespecificfixupversion, .Psaved.Costumedata.Pcslotset, .Psaved.Costumedata.Islotsetversion, .Psaved.Costumedata.Eacostumeslots, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
static S32 SavedPetFixupTest(SA_PARAM_NN_VALID Entity *pOwner, SA_PARAM_NN_VALID PetRelationship *pPetRelationship, bool bIsPuppet, GameAccountDataExtract* pExtract)
{
	...
	if(!pPetEnt && pPetRelationship->eState != OWNEDSTATE_ACTIVE)
	{
		//pets that are not offline will not subscribe, so it's ok to not fix them up here
		bReturn = false;
	}
...
static void SavedPetFixup_CB(TransactionReturnVal* returnVal, void* userData)
{
	ContainerID conID = (uintptr_t)(userData);
	Entity *pPlayerEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, conID);
	if(pPlayerEnt && pPlayerEnt->pSaved)
	{
		pPlayerEnt->pSaved->bSavedPetFixupInProgress = false;
	}
}

static bool Entity_SavedPetFixup(Entity *pOwner)
{
	int i;
	bool bResult = true;
	...
	for(i=0;i<eaSize(&pOwner->pSaved->ppOwnedContainers);i++)
	{
		...
		if(!pPet->bNeedsFixup || pOwner->pSaved->bSavedPetFixupInProgress)
		{
			continue;
		}

		if(!pEntity && IS_HANDLE_ACTIVE(pPet->hPetRef))
		{
			bResult = false;
			continue;
		}

		pPet->bNeedsFixup = false;
		...
		{
			TransactionReturnVal* pReturn = LoggedTransactions_CreateManagedReturnValEnt("SavedPetFixup", pEntity, SavedPetFixup_CB, (void*)((uintptr_t)(pOwner->myContainerID)) );
			ItemChangeReason reason = {0};
			inv_FillItemChangeReason(&reason, pOwner, "Pets:SavedPetFixup", pEntity ? pEntity->debugName : NULL);
			pOwner->pSaved->bSavedPetFixupInProgress = true;
		...
		// Puppets should not have power trees, so make sure that they don't get them
		if (!bIsPuppet)
		{
			// If we're in development mode, remove any power trees from pets that they are not eligible for
			// Reduces bug reports from in-development power trees being applied to pets
			if (isDevelopmentMode())
			{
				for (i = eaSize(&pEntity->pChar->ppPowerTrees) - 1; i >= 0; i--)
				{
					PowerTreeDef* pDef = GET_REF(pEntity->pChar->ppPowerTrees[i]->hDef);
					if (pDef && !entity_CanBuyPowerTreeHelper(entGetPartitionIdx(pOwner), CONTAINER_NOCONST(Entity, pEntity), pDef, false))
					{
						TransactionReturnVal* pReturn = LoggedTransactions_CreateManagedReturnValEnt("SavedPet_RemoveInvalidPowerTree", pEntity, NULL, NULL);

						Errorf("Invalid PowerTree (%s) found on a SavedPet. Removing PowerTree. Fixup may be required for Production servers, please verify with a programmer!", pDef->pchName);

						AutoTrans_SavedPet_RemovePowerTree(pReturn, GLOBALTYPE_GAMESERVER, GLOBALTYPE_ENTITYSAVEDPET, pPet->conID, pDef);
					}
				}
			}
		...
		// Puppets were never supposed to have PowerTrees, but until recently, they could, so we need to clean them out if we find them
		else if (pEntity && pEntity->pChar && eaSize(&pEntity->pChar->ppPowerTrees) > 0)
		{
			TransactionReturnVal* pReturn = LoggedTransactions_CreateManagedReturnValEnt("SavedPet_RemovePuppetPowerTrees", pEntity, NULL, NULL);
			AutoTrans_SavedPet_RemovePuppetPowerTrees(pReturn, GLOBALTYPE_GAMESERVER, GLOBALTYPE_ENTITYSAVEDPET, pPet->conID);
		}
...
void Entity_PuppetMasterAndPetTick(Entity *pent)
{
	...
	if(pent->pSaved->bFixupPets)
	{
		pent->pSaved->bFixupPets = !Entity_SavedPetFixup(pent);
	}

	if(pent->pSaved->bCheckPets && !pent->pSaved->bFixupPets)
	{
	...
	if(pent->pSaved->bPowerPropFail || !pent->pSaved->bValidatedOwnedContainers)
	{
		...
		for(i=eaSize(&pent->pSaved->ppOwnedContainers)-1;i>=0;i--)
		{
			PetRelationship *pRel = pent->pSaved->ppOwnedContainers[i];
			if(pRel->eState == OWNEDSTATE_ACTIVE && !SavedPet_GetEntity(iPartitionIdx, pRel))
				break;
		}
...
void gslHandlePetsAtLogin(Entity *pOwner)
{
	...
	//Entity_SavedPetFixup is now called in Entity_PuppetMasterAndPetTick
...
void ent_PropagatePowers(int iPartitionIdx, Entity *pEntity, Power** ppOldPropPowers, GameAccountDataExtract *pExtract)
{
	...
	int i;
	int iCount = 0;
	U32 uiCurID = SAFE_MEMBER3(pEntity, pSaved, pPuppetMaster, curID);
	if (!uiCurID)
		uiCurID = SAFE_MEMBER3(pEntity, pSaved, pPuppetMaster, curTempID);
	...
	if (eaSize(&pEntity->pSaved->ppOwnedContainers))
	{
		for(i=0;i<eaSize(&pEntity->pSaved->ppOwnedContainers);i++)
		{
			...
			if(pEntity->myEntityType == GLOBALTYPE_ENTITYPLAYER && !curEnt && SavedPet_IsPetAPuppet(pEntity, pPetRelationship))
			{
				// don't event try to add powers from inactive ships (puppets)
				continue;
			}

			if(pPetRelationship->eState == OWNEDSTATE_ACTIVE && (!curEnt || !pPetRelationship->uiPetID))
			{
			...
			for(j=eaSize(&pEntity->pSaved->ppAlwaysPropSlots)-1;j>=0;j--)
			{
				if (pEntity->pSaved->ppAlwaysPropSlots[j]->iPetID == pPetRelationship->uiPetID
					&& pEntity->pSaved->ppAlwaysPropSlots[j]->iPuppetID == uiCurID)
					break;
			}

			// fail case, pet should have been active but is not
			// This fixes broken puppets which could have occurred in the past
			if(!curEnt && j > -1)
			{
				// subscribe all pets as this character has bad data and we need to fix it
				PetCommands_SubscribeAllPets(pEntity);
				SavedPet_StatusFlagRequestAlwaysPropSlot(pEntity, pPetRelationship->uiPetID, uiCurID, j, 0, true);
				pEntity->pSaved->bPowerPropFail = true;
				return;
			}

			// Propagate all powers if the entity is currently online, or is set to active with the always propagate flag turned on
			if (curEnt && (pPetRelationship->eState == OWNEDSTATE_AUTO_SUMMON || j > -1))
			{
			...
			else if(curEnt && pEntity->pSaved->pPuppetMaster && SavedPet_IsPetAPuppet(pEntity,pEntity->pSaved->ppOwnedContainers[i]) && uiCurID != pPetRelationship->conID)
			{
...
void SavedPet_StatusFlagRequestAlwaysPropSlot(Entity *e, U32 uiPetID, U32 uiPuppetID, int iSlotID, int ePropCategory, bool bAdd)
{
	...
	for(i=0;i<eaSize(&e->pSaved->ppOwnedContainers);i++)
	{
		...
		if(pRel->uiPetID == uiPetID)
		{
			...
			int j;
			OwnedContainerState eState = (pRel->bTeamRequest || bAdd) ? OWNEDSTATE_ACTIVE : OWNEDSTATE_OFFLINE;
