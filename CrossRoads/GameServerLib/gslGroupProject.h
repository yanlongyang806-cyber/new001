...
AUTO_STRUCT;
typedef struct GroupProjectSetTaskAndDonateAllState
{
    ...
    int taskSlotNum;
    U32 timeStarted;
} GroupProjectSetTaskAndDonateAllState;

AUTO_STRUCT;
typedef struct GroupProjectDependentMissions
{
    ContainerID playerID;           AST(KEY)
    STRING_EARRAY missionRefs;     AST(POOL_STRING)
    U32 time;
} GroupProjectDependentMissions;
...
void gslGroupProject_HandleDependentMission(Entity *playerEnt, const char *refString);
void gslGuildProject_ClaimRewardTable(Entity *pEnt, RewardTable *rewardTable, const char *projectName, DonationTaskDef *taskDef, int taskSlotNum);