...
static U32 s_AuctionExpireOverride = 0;
AUTO_CMD_INT(s_AuctionExpireOverride, AuctionExpireOverride) ACMD_AUTO_SETTING(Auction, GAMESERVER);
...
static bool CanAccessAuctions(Entity *pEnt)
{
	...
	return (pContactDef && contact_IsMarket(pContactDef)) || GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER || gAuctionConfig.bAllowContactlessAuctionHouse;
}
...
static void gslAuction_GetLotsBidByPlayer_CB(TransactionReturnVal *pReturn, void *pData)
{
	...
	{		
		...
		if (ea32Size(&pList->eaiNonExistingAuctionLotContainerIDs) > 0)
		{
			...
			TransactionReturnVal* pRet = NULL;

			ea32Copy(&param.eaiAuctionLotContainerIDsToRemove, &pList->eaiNonExistingAuctionLotContainerIDs);

			pRet = LoggedTransactions_CreateManagedReturnValEnt("Auction", pEnt, NULL, NULL);

			AutoTrans_auction_tr_RemoveExpiredAuctionsFromPlayerBidList(pRet, GetAppGlobalType(), GLOBALTYPE_ENTITYPLAYER, (ContainerID)((intptr_t)pData), &param);
...
ATR_LOCKS(pAuctionLot, ".Price, .bWasPostedRemotely, .Ppitemsv2, .Upostingfee, .Uexpiretime, .Creationtime, .Pbiddinginfo.Icurrentbid");
void auction_trh_LotSoldMailCallback(ATR_ARGS, GlobalType iEntityType, ContainerID iContainerID, ATH_ARG NOCONST(Entity)* pSeller, ATH_ARG NOCONST(AuctionLot) *pAuctionLot)
{
	...
	if(NONNULL(pSeller) && NONNULL(pSeller->pPlayer))
	{
		...
		pData->uSoldFee = auction_trh_GetFinalSalesFee(ATR_PASS_ARGS, pSeller, pAuctionLot, true);
		...
		estrPrintf(&estrBody, "Auction sold by P[%d@%d %s@%s],  price %d: PFee %d: SFee %d: %s", pSeller->myContainerID, pData->iAccountSellerID,
			pSeller->pSaved->savedName, pData->eSellerAccountName, pAuctionLot->price, pAuctionLot->uPostingFee, pData->uSoldFee, estrItemNames);
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pBuyer, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Accountid, .Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Mail, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Ppuppetmaster.Curid, .Hallegiance, .Hsuballegiance, .Psaved.Savedname, .Pplayer.Publicaccountname, .Pplayer.Langid, .Psaved.Ppuppetmaster.Curtype, .Pchar.Ilevelexp, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Pppuppets, .Psaved.Ppallowedcritterpets, .Psaved.Ppalwayspropslots, .Psaved.Ppuppetmaster.Pppuppetrequests, .Pinventoryv2.Ppinventorybags, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pinventoryv2.Pplitebags, .Pplayer.Playertype, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Pptemppuppets, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(pSeller, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Mail, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Hallegiance, .Hsuballegiance, .Pplayer.Accountid, .Psaved.Savedname, .Pplayer.Publicaccountname, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Langid, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pAuctionLot, ".Pbiddinginfo.Ibiddingplayeraccountid, .Pbiddinginfo.Iminimumbid, .Owneraccountid, .Icontainerid, .Pbiddinginfo.Ibiddingplayerlangid, .State, .Ownerid, .Ppitemsv2, .Price, .bWasPostedRemotely, .Pbiddinginfo.Icurrentbid, .Pbiddinginfo.Ibiddingplayercontainerid, .Upostingfee, .Ilangid, .Uexpiretime, .Creationtime")
	ATR_LOCKS(eaContainerItemPets, ".Psaved.Ppuppetmaster.Curtempid, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Psaved.Ipetid, .Psaved.Ppallowedcritterpets, .Pcritter.Petdef, .Pplayer.Playertype, .Pchar, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Peaowneduniqueitems, .Itemidmax, .Psaved.Savedname, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
bool auction_trh_PurchaseAuction(ATR_ARGS, ATH_ARG NOCONST(Entity) *pBuyer, ATH_ARG CONST_EARRAY_OF(NOCONST(Entity)) eaPets, ATH_ARG NOCONST(Entity) *pSeller, ATH_ARG NOCONST(Entity) *pLastBidder, ATH_ARG NOCONST(AuctionLot) *pAuctionLot, ATH_ARG CONST_EARRAY_OF(NOCONST(Entity)) eaContainerItemPets, const ItemChangeReason *pBuyerReason, const ItemChangeReason *pSellerReason, const ItemChangeReason *pOutbidReason, GameAccountDataExtract *pBuyerExtract, GameAccountDataExtract *pSellerExtract)
{
	int i;
	...
	U32 uiCurrentBid = SAFE_MEMBER2(pAuctionLot, pBiddingInfo, iCurrentBid);
	char* estrDetails = NULL;

	if (pAuctionLot->price > INT_MAX)
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, auction lot has a price greater than INT_MAX.");
	...
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, auction cannot be purchased.");
		QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_FAIL, pBuyer->myEntityType, pBuyer->myContainerID, "AuctionServer_Error_BuyoutClosed", kNotifyType_AuctionFailed);
		return false;
	}

	if (NONNULL(pLastBidder) && !auction_trh_HandleOutbid(ATR_PASS_ARGS, pLastBidder, pBuyer, pAuctionLot, uiCurrentBid, pOutbidReason))
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, old bidder's escrow money could not be refunded.");
		QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_FAIL, pBuyer->myEntityType, pBuyer->myContainerID, "AuctionServer_Error_OldBidderFailed", kNotifyType_AuctionFailed);
		return false;
	}
	...
	if (gAuctionConfig.bMailAllItemsAndCurrency)
	{
		...
		{
			QueueRemoteCommand_Auction_RecordSalePriceForItem(ATR_RESULT_SUCCESS, GLOBALTYPE_AUCTIONSERVER, 0, pchItemName, ceilf(fPricePerUnit), NONNULL(pSeller) ? pSeller->myContainerID : 0, NONNULL(pBuyer) ? pBuyer->myContainerID : 0);
			return true;
		}
	}
	...
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, item already consumed.");
	...
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, Can't be purchase auction from self.");
	...
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, Sell price too high.");
	...
	for (i = 0; i < eaSize(&pAuctionLot->ppItemsV2); i++)
	{
		...
		if(itemDef && itemDef->eType == kItemType_Container)
		{
			...
			if(ISNULL(pBuyerAllegiance) || ISNULL(pSellerAllegiance) || pBuyerAllegiance != pSellerAllegiance)
			{
				...
				{
					TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed: Auction has container item(s) and buyer and seller belong to different allegiances");
			...
			{
				TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, saved pet from item failed");// %s", pBuyer->debugName );
		...
		{
			TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, item add failed");// %s", pBuyer->debugName );
	...
	iSoldPriceAfterFees = pAuctionLot->price - auction_trh_GetFinalSalesFee(ATR_PASS_ARGS, pSeller, pAuctionLot, true);
	...
	AuctionLot_trh_GetBuyoutSaleLogDetailString(pAuctionLot, pBuyer, iSoldPriceAfterFees, &estrDetails);
	...
	if (pAuctionLot->price)
	{
		...
		{
			TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, Removing resources failed");
		...
		if(NONNULL(pSeller))
		{
			...
			{
				TRANSACTION_APPEND_LOG_FAILURE("auction_trh_PurchaseAuction failed, could not pay the owner[%d].", pSeller->myContainerID);
				QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_FAIL, pBuyer->myEntityType, pBuyer->myContainerID, "AuctionServer_Error_PurchaseAuction_Unknown", kNotifyType_AuctionFailed);
				estrDestroy(&estrDetails);
				return false;
			}	
...
AUTO_TRANSACTION
	ATR_LOCKS(pBuyer, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Accountid, .Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Mail, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Ppuppetmaster.Curtype, .Hallegiance, .Hsuballegiance, .Psaved.Savedname, .Pplayer.Publicaccountname, .Pplayer.Langid, .Psaved.Ppuppetmaster.Curtempid, .Pchar.Ilevelexp, .pInventoryV2.Pplitebags, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Pppuppets, .Psaved.Ppallowedcritterpets, .Psaved.Ppalwayspropslots, .Psaved.Ppuppetmaster.Pppuppetrequests, .pInventoryV2.Ppinventorybags, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Playertype, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Pptemppuppets, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems")
	ATR_LOCKS(pSeller, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Mail, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Hallegiance, .Hsuballegiance, .Pplayer.Accountid, .Psaved.Savedname, .Pplayer.Publicaccountname, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Langid, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pLastBidder, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Mail, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pAuctionLot, ".Pbiddinginfo.Ibiddingplayeraccountid, .Pbiddinginfo.Iminimumbid, .Owneraccountid, .Icontainerid, .Pbiddinginfo.Ibiddingplayerlangid, .State, .Ownerid, .Ppitemsv2, .Price, .bWasPostedRemotely, .Pbiddinginfo.Icurrentbid, .Pbiddinginfo.Ibiddingplayercontainerid, .Upostingfee, .Ilangid, .Uexpiretime, .Creationtime")
	ATR_LOCKS(eaContainerItemPets, ".Psaved.Ppuppetmaster.Curtempid, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Psaved.Ipetid, .Psaved.Ppallowedcritterpets, .Pcritter.Petdef, .Pplayer.Playertype, .Pchar, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Peaowneduniqueitems, .Itemidmax, .Psaved.Savedname, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
enumTransactionOutcome auction_tr_PurchaseAuction(ATR_ARGS, NOCONST(Entity) *pBuyer, CONST_EARRAY_OF(NOCONST(Entity)) eaPets, 
    NOCONST(Entity) *pSeller, NOCONST(Entity) *pLastBidder,
	NOCONST(AuctionLot) *pAuctionLot, CONST_EARRAY_OF(NOCONST(Entity)) eaContainerItemPets, 
	const ItemChangeReason *pBuyerReason, const ItemChangeReason *pSellerReason, const ItemChangeReason *pOutbidReason,
	GameAccountDataExtract *pBuyerExtract, GameAccountDataExtract *pSellerExtract, int buyerIsOldBidder)
{
    if ( buyerIsOldBidder )
    {
	    if(!auction_trh_PurchaseAuction(ATR_PASS_ARGS, pBuyer, eaPets, pSeller, pBuyer, pAuctionLot, eaContainerItemPets, pBuyerReason, pSellerReason, pOutbidReason, pBuyerExtract, pSellerExtract))
	    {
    ...
    {
        if(!auction_trh_PurchaseAuction(ATR_PASS_ARGS, pBuyer, eaPets, pSeller, pLastBidder, pAuctionLot, eaContainerItemPets, pBuyerReason, pSellerReason, pOutbidReason, pBuyerExtract, pSellerExtract))
        {
            TRANSACTION_RETURN_LOG_FAILURE("Items not purchased.");
        }
    }
			...
			if(pData->bRetryWithNoSeller)
			{
				...
				for(i = 0; i < pReturn->iNumBaseTransactions; ++i)
				{
						...
						strstri(pReturn->pBaseReturnVals[i].returnString, OBJ_TRANS_CONTAINER_DOES_NOT_EXIST) && strstri(pReturn->pBaseReturnVals[i].returnString, "GlobalType=EntityPlayer"))
					{
						...
						ItemChangeReason reasonOutbid = {0};
						ContainerID uiOldBidder = 0;
                        ContainerID uiBuyerID = 0;
						...
						if (pData->pLot && pData->pLot->pBiddingInfo)
							uiOldBidder = pData->pLot->pBiddingInfo->iBiddingPlayerContainerID;

						AuctionLot_GetGameEconomyDetailString(pData->pLot, &estrDetail);

						inv_FillItemChangeReason(&reason, pEnt, "Auction:Purchase", estrDetail);
						inv_FillItemChangeReason(&reasonOutbid, pEnt, "Auction:Outbid", estrDetail);
				
                        uiBuyerID = entGetContainerID(pEnt);

    					AutoTrans_auction_tr_PurchaseAuction(LoggedTransactions_CreateManagedReturnValEnt("AuctionNoSeller", pEnt, LotPurchase_CB, pStruct), GLOBALTYPE_GAMESERVER, 
								entGetType(pEnt), uiBuyerID, 
								GLOBALTYPE_ENTITYSAVEDPET, &eaPets,
                                GLOBALTYPE_ENTITYPLAYER, 0,
								GLOBALTYPE_ENTITYPLAYER, uiOldBidder != uiBuyerID ? uiOldBidder : 0,
								GLOBALTYPE_AUCTIONLOT, pData->pLot->iContainerID, 
								GLOBALTYPE_ENTITYSAVEDPET, &eaiContainerItemPets, 
								&reason, NULL, uiOldBidder ? &reasonOutbid : NULL , pExtract, NULL, uiOldBidder == uiBuyerID);
...
ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems")
ATR_LOCKS(pAuctionLot, ".Pbiddinginfo.Iminimumbid, .Owneraccountid, .Price, .Icontainerid, .State, .Ownerid, .Ppitemsv2, .Pbiddinginfo.Inumbids")
ATR_LOCKS(eaContainerItemPets, ".Psaved.Ppuppetmaster.Curtempid, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Psaved.Ipetid, .Psaved.Ppallowedcritterpets, .Pcritter.Petdef, .Pplayer.Playertype, .Pchar, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Peaowneduniqueitems, .Itemidmax, .Psaved.Savedname, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
											...
											const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	...
	AuctionLot_trh_GetGenericLogDetailString(pAuctionLot, pAuctionLot->iContainerID, &estrDetails);

	TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(LOG_AUCTION, "Auction_Cancelled", "%s", estrDetails);
...
void gslAuction_PurchaseAuction(SA_PARAM_NN_VALID Entity* pEnt, AuctionLot *pCopiedLot, bool bPurchasing)
{
	...
	if (gslAuction_ValidatePermissions(pEnt, pCopiedLot, &eaiContainerItemPets, "Auction.AuctionPurchasedFail", "Auction.NoTrialAuctionPurchases"))
	{
		...
		if (pCopiedLot->pBiddingInfo)
			uiOldBidderID = pCopiedLot->pBiddingInfo->iBiddingPlayerContainerID;
		...
		if (pCopiedLot->ownerID != entGetContainerID(pEnt))
		{
			...
			ItemChangeReason reasonBuyer = {0}, reasonSeller = {0}, reasonOutbid = {0};
            static char *reasonDetail = NULL;
			...
			estrClear(&reasonDetail);

			AuctionLot_GetGameEconomyDetailString(pCopiedLot, &reasonDetail);

			pStruct->bRetryWithNoSeller = !gAuctionConfig.bMailAllItemsAndCurrency; // When items are mailed we need a seller

			inv_FillItemChangeReason(&reasonBuyer, pEnt, "Auction:Purchase:Buyer", reasonDetail);
			inv_FillItemChangeReason(&reasonSeller, NULL, "Auction:Purchase:Seller", reasonDetail);
			inv_FillItemChangeReason(&reasonOutbid, NULL, "Auction:Purchase:Outbid", reasonDetail);
			...
			AutoTrans_auction_tr_PurchaseAuction(LoggedTransactions_CreateManagedReturnValEnt("Auction",pEnt,LotPurchase_CB,pStruct), GetAppGlobalType(), 
                ...
                GLOBALTYPE_ENTITYPLAYER, uiOldBidderID != uiBuyerID ? uiOldBidderID : 0,
				GLOBALTYPE_AUCTIONLOT, pCopiedLot->iContainerID,
				GLOBALTYPE_ENTITYSAVEDPET, &eaiContainerItemPets,
				&reasonBuyer, &reasonSeller, uiOldBidderID ? &reasonOutbid : NULL,
                pExtract, pSellerExtract, uiOldBidderID == uiBuyerID);
		}
		else
		{
			bool bDoCancel = true;
...
bool gslAuction_ValidateBidOnAuction(Entity *pEnt, AuctionLot *pLot, U32 iBidValue)
{
	...
	// Make sure the bid amount won't blow out a S32. This would also fail the next
	// if clause (since the numeric can't be bigger than an S32) but let's be specific
	// about it.
	if(iBidValue > INT_MAX)
	{
	...
	if ((S64)inv_GetNumericItemValue(pEnt, Auction_GetCurrencyNumeric()) < (S64)iBidValue)
	{
		ClientCmd_gclAuctionPostComplete(pEnt, entTranslateMessageKey(pEnt, "Auction.NotEnoughResourcesForBid"), false);
		return false;
	}
...
void gslAuction_BidOnAuction(SA_PARAM_NN_VALID Entity* pEnt, AuctionLot *pCopiedLot, U32 iBidValue)
{
	...
	{
		AuctionCallbackStructure *pStruct;
		ItemChangeReason bidReason = {0};
		ItemChangeReason raiseBidReason = {0};
		ItemChangeReason outbidReason = {0};
		char* estrDetail = NULL;
		...
		AuctionLot_GetGameEconomyDetailString(pCopiedLot, &estrDetail);
		inv_FillItemChangeReason(&bidReason, pEnt, "Auction:Bid", estrDetail);
		inv_FillItemChangeReason(&outbidReason, NULL, "Auction:OutBid", estrDetail);

		if (pCopiedLot->pBiddingInfo->iBiddingPlayerContainerID != entGetContainerID(pEnt))
		{
			AutoTrans_auction_tr_BidOnAuction(LoggedTransactions_CreateManagedReturnValEnt("Auction", pEnt, gslAuction_BidCB, pStruct), GetAppGlobalType(),
				...
				GLOBALTYPE_AUCTIONLOT, pCopiedLot->iContainerID,
				iBidValue, &bidReason, &outbidReason);
		}
		else
		{
			inv_FillItemChangeReason(&raiseBidReason, pEnt, "Auction:RaiseBid", estrDetail);
			//Somebody is outbidding themselves, don't pass the same entity twice.
			AutoTrans_auction_tr_RaiseBid(LoggedTransactions_CreateManagedReturnValEnt("Auction", pEnt, gslAuction_BidCB, pStruct), GetAppGlobalType(),
				GLOBALTYPE_ENTITYPLAYER, pCopiedLot->pBiddingInfo->iBiddingPlayerContainerID,
				GLOBALTYPE_AUCTIONLOT, pCopiedLot->iContainerID,
				iBidValue, &raiseBidReason);
		}
...
AUTO_TRANSACTION
ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Psaved.Ppuppetmaster, .Pplayer.Publicaccountname, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Psaved.Ppownedcontainers, .Psaved.Ppalwayspropslots, .Psaved.Pipetidsremovedfixup, .Psaved.Pppreferredpetids, .Hallegiance, .Hsuballegiance, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Peaowneduniqueitems, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pinventoryv2.Pplitebags, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pAuctionLot, ".State, .Ppitemsv2, .Upostingfee, .Auctionpetcontainerids")
	ATR_LOCKS(eaPets, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Psaved.Conowner.Containerid, .Pinventoryv2.Ppinventorybags, .Psaved.Conowner.Containertype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pinventoryv2.Peaowneduniqueitems, .Pinventoryv2.Pplitebags");
	...
	if (pEnt || pData->eContainerID)
	{
		...
		else
		{
			...
			if(gslAuction_GetContainerItemPetsFromLot(pData->pLot, NULL) < eaSize(&pData->pLot->ppItemsV2))
			{
				...
				AuctionLot_GetGameEconomyDetailString(pData->pLot, &estrDetail);
				inv_FillItemChangeReason(&reason, pEnt, "Auction:Add", estrDetail);
...
static void CreateLotFromDescription_CB(TransactionReturnVal *pReturn, AuctionCallbackStructure *pData)
{
	...
	if (pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS) 
	{
		...
		if (pData && pData->pLot && eaSize(&pData->pLot->ppItemsV2))
		{
			...
			ItemChangeReason reason = {0};
			char* estrDetail = NULL;
			char* estrLogDetail = NULL;

			pData->lotID = newLotID;


			AuctionLot_GetGameEconomyDetailString(pData->pLot, &estrDetail);
			AuctionLot_trh_GetNewLotLogDetailString(CONTAINER_NOCONST(AuctionLot, pData->pLot), newLotID, &estrLogDetail);
			
			inv_FillItemChangeReason(&reason, pEnt, "Auction:CreateLot", estrDetail);

			entLog(LOG_AUCTION, pEnt, "Auction_Posted", "%s", estrLogDetail);
...
void AuctionSlot_CacheSearchableAttribMagnitudes(Entity* pEnt, NOCONST(AuctionSlot)* pSlot)
{
	...
	if (pItem && pEnt && pEnt->pChar)
	{
		combateval_ContextSetupSimple(pEnt->pChar,
			item_GetLevel(pConstItem) ? item_GetLevel(pConstItem) : pEnt->pChar ? pEnt->pChar->iLevelCombat : 1,
			pConstItem);
...
void gslAuction_CreateLotFromDescription(SA_PARAM_NN_VALID Entity* pEnt, AuctionLot *pCopiedLot, U32 iAuctionDuration)
{
	...
	ContactDialog* pDialog = SAFE_MEMBER3(pEnt, pPlayer, pInteractInfo, pContactDialog);
	bool bPlayerIsAtAuctionHouse = (GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER) || (pDialog && pDialog->screenType == ContactScreenType_Market);
	int i;
	...
	//If we are the gateway server, pEnt->pPlayer->uLastLotsPostedByPlayer cannot be trusted
	// find the auction search result for owned containers and use that count
	if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
	{
		GatewaySession *psess = wgsFindSessionForAccountId(pEnt->pPlayer->accountID);
		ContainerTracker *pTracker = psess ? session_FindContainerTracker(psess,GW_GLOBALTYPE_AUCTION_SEARCH,GATEWAYAUCTION_OWNED) : NULL;

		if(pTracker && pTracker->pAuctionLotList)
			uNumPostedAuctions = eaSize(&pTracker->pAuctionLotList->ppAuctionLots);
	}
	...
	if (iStartingBid > INT_MAX || pCopiedLot->price > INT_MAX)
	{
		ClientCmd_gclAuctionPostComplete(pEnt, entTranslateMessageKey(pEnt, "Auction.AuctionPostedFail"), false);
		return;
	}
...

	{
        ...
        if ( s_AuctionExpireOverride )
        {
            // If auction expire override is set, then just use it to compute the expire time.
            pNewLot->uExpireTime = pNewLot->creationTime + s_AuctionExpireOverride;
        }
        else
        {
		...
		pNewLot->bWasPostedRemotely = !bPlayerIsAtAuctionHouse;
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(9) ACMD_CATEGORY(Auction, Inventory);
...
bool auction_trh_BidOnAuction(ATR_ARGS, 
	...
	ATH_ARG NOCONST(AuctionLot) *pAuctionLot, 
	U32 iBidValue,
	const ItemChangeReason *pBidReason,
	const ItemChangeReason *pOutBidReason)
{
	char* estrDetails = NULL;
	...
	if (iBidValue > INT_MAX)
	{
		TRANSACTION_APPEND_LOG_FAILURE("auction_tr_BidOnAuction failed, bid price is greater than INT_MAX.");
	...
	// Make sure the bidder has enough money
	if ((S64)inv_trh_GetNumericValue(ATR_PASS_ARGS, pNewBidder, Auction_GetCurrencyNumeric()) < (S64)iBidValue)
	{
	...
	if (!inv_ent_trh_AddNumeric(ATR_PASS_ARGS, pNewBidder, false, Auction_GetCurrencyNumeric(), -((S32)iBidValue), pBidReason))
	{
...

	{
		...
		if (NONNULL(pOldBidder))
		{
			if (!auction_trh_HandleOutbid(ATR_PASS_ARGS, pOldBidder, pNewBidder, pAuctionLot, iOldBid, pOutBidReason))
			{
	...
	AuctionLot_trh_GetNewBidLogDetailString(pAuctionLot, &estrDetails);

	TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(LOG_AUCTION, "Auction_BidReceived", "%s", estrDetails);
...
AUTO_TRANSACTION
	ATR_LOCKS(pOldBidder, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Mail, .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pNewBidder, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Pplayer.Pplayerauctiondata, .Pplayer.Accountid, .Pplayer.Langid, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Psaved.Savedname, .Pplayer.Publicaccountname, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pAuctionLot, ".Owneraccountid, .Price, .Pbiddinginfo.Ibiddingplayeraccountid, .Ownerid, .Pbiddinginfo.Iowneraccountid, .State, .Pbiddinginfo.Ibiddingplayercontainerid, .Pbiddinginfo.Icurrentbid, .Pbiddinginfo.Inumbids, .Pbiddinginfo.Ibiddingplayerlangid, .Pbiddinginfo.Iminimumbid, .Uexpiretime, .Ppitemsv2, .Icontainerid");
enumTransactionOutcome auction_tr_BidOnAuction(ATR_ARGS, 
	...
	const ItemChangeReason *pOutBidReason)
{
	if(!auction_trh_BidOnAuction(ATR_PASS_ARGS, pOldBidder, pNewBidder, pAuctionLot, iBidValue, pBidReason, pOutBidReason))
	{
...
AUTO_TRANSACTION
	ATR_LOCKS(pBidder, ".Pplayer.Pemailv2.Ilastusedid, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Pemailv2.Mail, .Pplayer.Pemailv2.Unextfuturedelivery, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Pplayer.Pplayerauctiondata, .Pplayer.Accountid, .Pplayer.Langid, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Psaved.Savedname, .Pplayer.Publicaccountname, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pAuctionLot, ".Owneraccountid, .Price, .Pbiddinginfo.Ibiddingplayeraccountid, .Ownerid, .Pbiddinginfo.Iowneraccountid, .State, .Pbiddinginfo.Ibiddingplayercontainerid, .Pbiddinginfo.Icurrentbid, .Pbiddinginfo.Inumbids, .Pbiddinginfo.Ibiddingplayerlangid, .Pbiddinginfo.Iminimumbid, .Uexpiretime, .Ppitemsv2, .Icontainerid");
enumTransactionOutcome auction_tr_RaiseBid(ATR_ARGS, 
	NOCONST(Entity) *pBidder, // The doofus who's outbidding themselves
	NOCONST(AuctionLot) *pAuctionLot,
	U32 iBidValue,
	const ItemChangeReason *pBidReason)
{
	if(!auction_trh_BidOnAuction(ATR_PASS_ARGS, pBidder, pBidder, pAuctionLot, iBidValue, pBidReason, pBidReason))
	{
		TRANSACTION_RETURN_LOG_FAILURE("Auction bidding has failed.");
	}
	TRANSACTION_RETURN_LOG_SUCCESS("Auction bidding was successful.");
...
void AuctionTestRecordPrice(const char* pchName, S32 price)
{
	if(!isProductionMode())
	{
		RemoteCommand_Auction_RecordSalePriceForItem(GLOBALTYPE_AUCTIONSERVER, 0, pchName, price, 0, 0);
	}
}
...
void gslAuction_MigrateMailAuctionLotsToEmailV3(Entity* pEnt)
{
	if (pEnt && pEnt->pPlayer)
	{
		RemoteCommand_Auction_Migrate_MailAuctionLotsToEmailV3(GLOBALTYPE_AUCTIONSERVER, 0, pEnt->myContainerID, pEnt->pPlayer->accountID);
	}
}

typedef struct MailBombState
{
	EntityRef erSender;
	U32 iSenderID;
	U32 iAccountID;
...
void gslAuction_MailBomb_CreateLot_CB(TransactionReturnVal *pReturn, MailBombState *pState);
void gslAuction_MailBomb_AddItems_CB(TransactionReturnVal *pReturn, MailBombState *pState);
void gslAuction_MailBomb_Cancel_CB(TransactionReturnVal *pReturn, MailBombState *pState);

void gslAuction_MailBomb_SubmitNextMail(MailBombState *pState)
{
	Entity* pEnt = entFromEntityRefAnyPartition(pState->erSender);
	NOCONST(AuctionLot) *pNewLot = StructCreateNoConst(parse_AuctionLot);
	NOCONST(AuctionSlot) *pNewItem;
	InventoryBag *pBag;

	if (!pEnt || !pEnt->pPlayer || !pEnt->pInventoryV2)
		goto complete;

	pBag = eaIndexedGetUsingInt(&pEnt->pInventoryV2->ppInventoryBags, InvBagIDs_Inventory);
	if (!pBag)
		goto complete;
	...
	pNewLot->recipientID = pState->iAccountID;

	while (pState->iNextSlot < eaSize(&pBag->ppIndexedInventorySlots) && eaSize(&pNewLot->ppItemsV2) < pState->iItemCount)
	{
		if (pBag->ppIndexedInventorySlots[pState->iNextSlot] && pBag->ppIndexedInventorySlots[pState->iNextSlot]->pItem
			&& (pBag->ppIndexedInventorySlots[pState->iNextSlot]->pItem->flags & (kItemFlag_Bound | kItemFlag_BoundToAccount)) == 0)
		{
			S32 iRemain = pBag->ppIndexedInventorySlots[pState->iNextSlot]->pItem->count;
			while (iRemain > 0 && eaSize(&pNewLot->ppItemsV2) < pState->iItemCount)
			{
				pNewItem = StructCreateNoConst(parse_AuctionSlot);
				pNewItem->slot.pItem = StructCloneDeConst(parse_Item, pBag->ppIndexedInventorySlots[pState->iNextSlot]->pItem);

				if (pState->iMaxStack > 0)
					pNewItem->slot.pItem->count = MIN(iRemain, pState->iMaxStack);
				else
					pNewItem->slot.pItem->count = iRemain;

				iRemain -= pNewItem->slot.pItem->count;
				eaPush(&pNewLot->ppItemsV2, pNewItem);
			}
			if (iRemain <= 0)
				pState->iNextSlot++;
		}
	...
	{
		objRequestContainerCreate(objCreateManagedReturnVal(gslAuction_MailBomb_CreateLot_CB, pState), 
...
void gslAuction_MailBomb_CreateLot_CB(TransactionReturnVal *pReturn, MailBombState *pState)
{
	...
	if (pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		inv_FillItemChangeReason(&reason, pEnt, "Mail:Debug:MailBomb", NULL);

		pState->uLastLotID = atoi(pReturn->pBaseReturnVals->returnString);
		AutoTrans_gslAuction_MailBomb_tr_AddItems(
			objCreateManagedReturnVal(gslAuction_MailBomb_AddItems_CB, pState),
			GLOBALTYPE_GAMESERVER,
			GLOBALTYPE_ENTITYPLAYER, pState->iSenderID,
			GLOBALTYPE_AUCTIONLOT, pState->uLastLotID,
			&reason,
			pExtract
		);
	}
	...
	{
		gslAuction_MailBomb_SubmitNextMail(pState);
	}
}

AUTO_TRANSACTION
ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Ppinventorybags, .Pchar.Ilevelexp, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags")
ATR_LOCKS(pLot, ".state, .Ppitemsv2");
enumTransactionOutcome gslAuction_MailBomb_tr_AddItems(ATR_ARGS, NOCONST(Entity) *pEnt, NOCONST(AuctionLot) *pLot, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	int i, n;
	BagIterator *iter = NULL;

	if (pLot->state != ALS_New)
	{
		TRANSACTION_RETURN_LOG_FAILURE("AuctionLot isn't in New state");
	}

	if (eaSize(&pLot->ppItemsV2) <= 0)
	{
		TRANSACTION_RETURN_LOG_FAILURE("AuctionLot has no items");
	}

	n = eaSize(&pLot->ppItemsV2);
	for (i = 0; i < n; i++)
	{
		iter = bagiterator_Create();

		if (ISNULL(pLot->ppItemsV2[i]->slot.pItem))
		{
			bagiterator_Destroy(iter);
			TRANSACTION_RETURN_LOG_FAILURE("AuctionLot slot missing item");
		}
		
		if (inv_trh_FindItemByID(ATR_PASS_ARGS, pEnt, pLot->ppItemsV2[i]->slot.pItem->id, iter))
		{
			NOCONST(Item) *pItem = bagiterator_GetItem(iter);
			Item *pRemoved;
			bool bRemoved = false;

			if ((pItem->flags & (kItemFlag_Bound | kItemFlag_BoundToAccount)) == 0)
			{
				pRemoved = bagiterator_RemoveItem(ATR_PASS_ARGS, iter, pEnt, pLot->ppItemsV2[i]->slot.pItem->count, pReason, pExtract);
				bRemoved = pRemoved != NULL;
				bagiterator_Destroy(iter);
				StructDestroy(parse_Item, pRemoved);
			}

			if (!bRemoved)
			{
				bagiterator_Destroy(iter);
				TRANSACTION_RETURN_LOG_FAILURE("Failed to remove item %I64u from inventory", pLot->ppItemsV2[i]->slot.pItem->id);
			}
		}
		else
		{
			bagiterator_Destroy(iter);
			TRANSACTION_RETURN_LOG_FAILURE("Failed to find item %I64u in inventory", pLot->ppItemsV2[i]->slot.pItem->id);
		}
	}

	pLot->state = ALS_Mailed;

	TRANSACTION_RETURN_LOG_SUCCESS("Removed items from lot");
}
...
void gslAuction_MailBomb_AddItems_CB(TransactionReturnVal *pReturn, MailBombState *pState)
{
	...
	{
		AutoTrans_gslAuction_MailBomb_tr_Cancel(
			objCreateManagedReturnVal(gslAuction_MailBomb_Cancel_CB, pState),
			GLOBALTYPE_GAMESERVER,
			GLOBALTYPE_AUCTIONLOT, pState->uLastLotID
		);
	}
...
AUTO_TRANSACTION
ATR_LOCKS(pLot, ".state");
enumTransactionOutcome gslAuction_MailBomb_tr_Cancel(ATR_ARGS, NOCONST(AuctionLot) *pLot)
{
	if (pLot->state != ALS_New)
	{
		TRANSACTION_RETURN_LOG_FAILURE("AuctionLot isn't in New state");
	...
	pLot->state = ALS_Closed;

	TRANSACTION_RETURN_LOG_SUCCESS("AuctionLot canceled");
}
...
AUTO_COMMAND ACMD_ACCESSLEVEL(7) ACMD_CATEGORY(Debug);
void gslAuction_MailBombMyInventoryToAccount(Entity* pEnt, U32 iAccountID, U32 iMaxStack, U32 iItemsPerMail)
{
	if (pEnt && pEnt->pPlayer && pEnt->pInventoryV2 && iAccountID)
	{
		MailBombState *pState = callocStruct(MailBombState);
		pState->erSender = entGetRef(pEnt);
		pState->iSenderID = entGetContainerID(pEnt);
		pState->iAccountID = iAccountID;
		pState->iNextSlot = 0;
		pState->iMaxStack = MIN(MAX(1, iMaxStack), INT_MAX);
		pState->iItemCount = MIN(MAX(iItemsPerMail, 1), INT_MAX);
		gslAuction_MailBomb_SubmitNextMail(pState);
	}
}
