	...
	else if ( pDef->ppCostume )
	{
		...
		for(i=0;i<eaSize(&pDef->ppCostume);i++)
		{
			if ( ( (pDef->ppCostume[i]->iMinLevel == -1) || (iLevel >= pDef->ppCostume[i]->iMinLevel) ) &&
				 ...
				 ( (pDef->ppCostume[i]->iAssetSetIndex == -1) || (pDef->ppCostume[i]->iAssetSetIndex == gslGetAssetSetIndex()) ))
			{
		...
		if (fWeightTotal == 0)
		{
			...
			Errorf("No Costume for Level %d, TeamSize %d, AssetSet %d, on Critter %s, using Corey",
				   iLevel, iTeamSize, gslGetAssetSetIndex(), pDef->pchName );
			...
			for(i=0; i<eaSize(&pDef->ppCostume); i++)
			{
				if(iCostumeIndex<0)
				{
					if ( ( (pDef->ppCostume[i]->iMinLevel != -1) && (iLevel < pDef->ppCostume[i]->iMinLevel) ) ||
						 ...
						 ( (pDef->ppCostume[i]->iMinTeamSize != -1) && (iTeamSize < pDef->ppCostume[i]->iMinTeamSize) ) ||
						 ( (pDef->ppCostume[i]->iMaxTeamSize != -1) && (iTeamSize > pDef->ppCostume[i]->iMaxTeamSize) ) ||
						 ( (pDef->ppCostume[i]->iAssetSetIndex != -1) && (gslGetAssetSetIndex() != pDef->ppCostume[i]->iAssetSetIndex) ))
					{
						continue;
					}
...
Entity* critter_CreateByDef( CritterDef * def, CritterCreateParams *pCreateParams, const char* blameFile, bool bImmediateCombatUpdate)
{
	...
	critter->uiEntCreatePowerID = pCreateParams->uiPowerID;
	critter->uiEntCreateAttribModDefIdx = pCreateParams->uiAttribModDefIdx;
...
int critter_CritterDefPostTextRead(CritterDef *def)
{
	...
	if (!exprContext)
	{
		...
		exprContextSetFuncTable(exprContext, exprContextCreateFunctionTable("CritterDefPostTextRead"));
	}
...
ExprContext *critter_GetEncounterExprContext()
{
	...
	if(!s_pContext)
	{
		...
		s_FuncTable = exprContextCreateFunctionTable("Critter_Encounter");
...
#define ROLLOVER_REATTEMPT_TIMEOUT			4.f
#define ROLLOVER_REATTEMPT_TIMEOUT_REPEAT	1.5f
// ~20s 
#define ROLLOVER_MAX_ATTEMPTS				5
		


// --------------------------------------------------------------------------------------------------------------------
static RolloverTracker* gslCritter_FindRolloverTracker(SA_PARAM_NN_VALID Entity *pLootEnt, SA_PARAM_NN_VALID Entity *pEnt)
{
	FOR_EACH_IN_EARRAY_FORWARDS(pLootEnt->pCritter->eaRolloverTrackers,  RolloverTracker, pTracker)
	{
		if (pTracker->erRef == entGetRef(pEnt))
			return pTracker;
	}
	FOR_EACH_END
...
// --------------------------------------------------------------------------------------------------------------------
// returns true if the pEnt can attempt to pickup the rollover
static bool gslCritter_ProcessProximityPickupRollover(SA_PARAM_NN_VALID Entity *pLootEnt, SA_PARAM_NN_VALID Entity *pEnt, U32 uiCurTime)
{
	RolloverTracker *pTracker = gslCritter_FindRolloverTracker(pLootEnt, pEnt);

	// check the timeout this is so we don't spam transactions on rollovers we can't collect for whatever reason
	if (pTracker)
	{
		pTracker->bWasInProximity = true;
		return !pTracker->bLockedAttempts && (uiCurTime >= pTracker->uiNextPickupAttemptTime);
	}
...
// --------------------------------------------------------------------------------------------------------------------
// returns true if the given entity was previously in proximity
static bool gslCritter_WasInProximity(SA_PARAM_NN_VALID Entity *pLootEnt, SA_PARAM_NN_VALID Entity *pEnt)
{
	RolloverTracker *pTracker = gslCritter_FindRolloverTracker(pLootEnt, pEnt);

	if (pTracker)
		return pTracker->bWasInProximity;

	return false;
}

// --------------------------------------------------------------------------------------------------------------------
// creates or updates the RolloverTracker for the given entity
static void gslCritter_AttemptedPickupRollover(SA_PARAM_NN_VALID Entity *pLootEnt, SA_PARAM_NN_VALID Entity *pEnt)
{
	RolloverTracker *pTracker = gslCritter_FindRolloverTracker(pLootEnt, pEnt);

	if (!pTracker)
	{
		pTracker = calloc(1, sizeof(RolloverTracker));
		pTracker->bWasInProximity = true;
		pTracker->erRef = entGetRef(pEnt);
		eaPush(&pLootEnt->pCritter->eaRolloverTrackers, pTracker);
	}
	
	if (pTracker->attemptCount > ROLLOVER_MAX_ATTEMPTS)
	{
		// they've been sitting on the rollover for too long and failing to pick it up. 
		// lock them out until they move off
		pTracker->bLockedAttempts = true;
	}
	else
	{
		pTracker->attemptCount++;
		pTracker->uiNextPickupAttemptTime = pmTimestamp(ROLLOVER_REATTEMPT_TIMEOUT + ROLLOVER_REATTEMPT_TIMEOUT_REPEAT * pTracker->attemptCount);
	}
}

// --------------------------------------------------------------------------------------------------------------------
// to be called before the checking for proximity entities. 
// Flags all trackers as not in proximity, so that after processing proximity ents, 
// we can see which ents were no longer in proximity
static void gslCritter_PreProcessRolloverTrackers(SA_PARAM_NN_VALID Entity *pLootEnt)
{
	FOR_EACH_IN_EARRAY_FORWARDS(pLootEnt->pCritter->eaRolloverTrackers, RolloverTracker, pTracker)
	{
		pTracker->bWasInProximity = false;
	}
	FOR_EACH_END
}

// --------------------------------------------------------------------------------------------------------------------
// after all the processing of proximity ents has taken place, 
// for the RolloverTracker's that aren't touching reset the lockouts 
static void gslCritter_PostProcessRolloverTrackers(SA_PARAM_NN_VALID Entity *pLootEnt)
{
	FOR_EACH_IN_EARRAY_FORWARDS(pLootEnt->pCritter->eaRolloverTrackers, RolloverTracker, pTracker)
	{
		if (pTracker->bWasInProximity == false)
		{
			pTracker->bLockedAttempts = false;
			pTracker->attemptCount = 0;
			pTracker->uiNextPickupAttemptTime = 0;
		}
	}
	FOR_EACH_END
}
...
// --------------------------------------------------------------------------------------------------------------------
// handles players running over this critter and picking up any rewardBags that are of type kRewardPickupType_Rollover 
void gslCritter_UpdateRollovers(Entity* pEnt)
{
	...
	entGridProximityLookupExEArray( entGetPartitionIdx(pEnt), vSourcePos, &s_proxEnts, gConf.rollover_distance, 
									(gConf.bRolloverLootIsForPlayersOnly ? ENTITYFLAG_IS_PLAYER : 0), 
									(ENTITYFLAG_IGNORE|ENTITYFLAG_DEAD), pEnt);

	uiCurrentTime = pmTimestamp(0);

	gslCritter_PreProcessRolloverTrackers(pEnt);

	// Iterate through the ent list and find the closest ent within the rollover distance that is able to pick it up
	fMinDist = gConf.rollover_distance;
	for(ii = eaSize(&s_proxEnts)-1; ii >= 0; ii--)
	{
		Entity* currEnt = s_proxEnts[ii];

		// see if this entity can attempt to pickup the rollover
		if(currEnt != pEnt && reward_MyDrop(pEnt, currEnt))
		{
			F32 f = entGetDistance(pEnt, NULL, currEnt, NULL, NULL);
			if (f <= gConf.rollover_distance)
			{
				if(gslCritter_ProcessProximityPickupRollover(pEnt, currEnt, uiCurrentTime) && 
					f <= fMinDist)
				{
					fMinDist = f;
					pentClosest = currEnt;
	...
	if(pentClosest)
	{
		FOR_EACH_IN_EARRAY(pEnt->pCritter->eaLootBags, InventoryBag, pBag)
		{
			if (reward_BagIsMyDrop(pentClosest, pBag))
			{
				...
				LootBag_TakeAll(pentClosest, InvBagIDs_None, 
								&pEnt->pCritter->eaLootBags, FOR_EACH_IDX(-, pBag), 
								entGetRef(pEnt), &reason);
			}
		}
		FOR_EACH_END

		gslCritter_AttemptedPickupRollover(pEnt, pentClosest);
	}

	gslCritter_PostProcessRolloverTrackers(pEnt);
