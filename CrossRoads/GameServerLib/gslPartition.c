...
ContainerID partition_VirtualShardIDFromID(U32 uPartitionID)
{
	S32 i = partition_IdxFromID(uPartitionID);
	if (i>=0) {
		return s_ppPartitions[i]->summary.iVirtualShardID;
	} else {
		assertmsgf(0, "Partition ID %d does not exist", uPartitionID);
	}
}

ContainerID partition_VirtualShardIDFromIdx(int iPartitionIdx)
{
	if (partition_ExistsByIdx(iPartitionIdx)) {
		return s_ppPartitions[iPartitionIdx]->summary.iVirtualShardID;
	} else {
		assertmsgf(0, "Partition %d does not exist", iPartitionIdx);
	}
}
...
void partition_DestroyLate(TimedCallback *pCallback, F32 fTimeSinceLastCallback, MapPartition *pPart)
{
    ...
    coarseTimerAddInstance(NULL, "partition_DestroyLate:Iterate Entities");
	...
	eaDestroy(&ppEnts);
    coarseTimerStopInstance(NULL, "partition_DestroyLate:Iterate Entities");
	...
	coarseTimerAddInstance(NULL, "worldDestroyPartition");
	worldDestroyPartition(iPartitionIdx);
	coarseTimerStopInstance(NULL, "worldDestroyPartition");
