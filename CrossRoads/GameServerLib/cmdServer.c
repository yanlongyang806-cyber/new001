...
void power_requestinfo(Entity *e, U32 uiPowerID)
{
	if(e && e->pChar)
	{
		...
		if(ppow)
		{
			ClientCmd_PowerSetRechargeClient(e, uiPowerID, -1, ppow->fTimeRecharge);

			// if we have Combat power states, and the powers should maintain their own recharge times
			// send information for each power
			if (!CombatPowerStateSwitching_ShouldShareRecharge(e->pChar))
			{ 
				FOR_EACH_IN_EARRAY_FORWARDS(ppow->ppSubCombatStatePowers, Power, pStatePower)
				{
					if (pStatePower)
					{
						U32 uiID = 0;
						S32 iIdxSub = 0;
						S16 iLinkedSub = 0;
						power_GetIDAndSubIdx(pStatePower, &uiID, &iIdxSub, &iLinkedSub);
						
						ClientCmd_PowerSetRechargeClient(e, uiPowerID, iLinkedSub, pStatePower->fTimeRecharge);
					}
				}
				FOR_EACH_END
			}
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_PRIVATE ACMD_ACCESSLEVEL(0);
void gslKeyBindSetVersion(Entity* pEnt, S32 iKeybindsVersion)
{
	PlayerUI *pUI = SAFE_MEMBER2(pEnt, pPlayer, pUI);

	pUI->iKeybindsVersion = iKeybindsVersion;

	entity_SetDirtyBit(pEnt, parse_PlayerUI, pUI, false);
}
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void gslSetUICompactMode(Entity *pEnt, int iCompactMode)
{
	if (iCompactMode < 0 || 3 <= iCompactMode)
		return;

	if (pEnt && pEnt->pPlayer && pEnt->pPlayer->pUI && pEnt->pPlayer->pUI->uiCompactMode != iCompactMode)
	{
		pEnt->pPlayer->pUI->uiCompactMode = iCompactMode;
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void gslSetUIDisableDamageOverlays(Entity *pEnt, bool bDisableDamageOverlays)
{
	bDisableDamageOverlays = bDisableDamageOverlays ? true : false;

	if (pEnt && pEnt->pPlayer && pEnt->pPlayer->pUI && pEnt->pPlayer->pUI->uiDisableDamageOverlays != bDisableDamageOverlays)
	{
		pEnt->pPlayer->pUI->uiDisableDamageOverlays = bDisableDamageOverlays;
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void gslSetColorPaletteStateDisables(Entity *pEnt, U32 ubfColorPaletteStateDisables)
{
	if (pEnt && pEnt->pPlayer && pEnt->pPlayer->pUI && pEnt->pPlayer->pUI->ubfColorPaletteStateDisables != ubfColorPaletteStateDisables)
	{
		pEnt->pPlayer->pUI->ubfColorPaletteStateDisables = ubfColorPaletteStateDisables;
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void gslSetUserColorPalette(Entity *pEnt, const char *pchUserColorPalette)
{
	if (pEnt && pEnt->pPlayer && pEnt->pPlayer->pUI && stricmp_safe(pEnt->pPlayer->pUI->pchUserColorPalette, pchUserColorPalette) != 0)
	{
		StructCopyString(&pEnt->pPlayer->pUI->pchUserColorPalette, pchUserColorPalette);
