	...
	////////////////////////////////////////////////////////////////////////
										...
										QueueInstanceParams* pParams)
		{
			...
			for(i = 0; i < eaSize(&pTeam->eaMembers); i++)
			{
				...
				pData = calloc(1, sizeof(QueueTeamValidationCBStruct));	// changed to a calloc as pParams needs to be NULL as its an optional parameter
...
static PlayerQueueInstance* gslQueue_EntFindValidPublicInstance(Entity* pEnt, PlayerQueue* pQueue, QueueDef* pDef)
{
	S32 i;
	if (pEnt && pQueue && !pQueue->eCannotUseReason && pDef)
	{
		for (i = eaSize(&pQueue->eaInstances)-1; i >= 0; i--)
		{
			...
			// We believe this used to work in STO by always grabbing the always created public instance,
			// but now that we don't create those instances anymore, this was choosing private queues, when
			// we only wanted public queues. So now we no longer choose any queues with an owner ID in this
			// function
			if (pInstance->pParams && pInstance->pParams->uiOwnerID)
			{
				continue;
			}
...
//			gslQueue_ResultMessage(pEnt->myContainerID, 0, pchQueueName, QUEUE_RESTRICTIONS_MESG);
		...
		// If our instance ID is 0, we are looking for a public instance to join
		if (uiInstanceID == 0)
		{
			pInstance = gslQueue_EntFindValidPublicInstance(pEnt, pQueue, pDef);
			if (pInstance)
			{
...
void gslQueue_MapTransferMember(U32 iEntID, QueueInstantiationInfo* pInstantiationInfo)
{
	...
	if (pEnt && pInstantiationInfo)
	{
		...
		// If we haven't specified a spawn point at this stage, use SPAWN_AT_FURTHEST. If there is no 'furthest' it will
		//   default back to the start spawn, which is the same as leaving this blank. So we have basically unchanged functionality.
		if (pchSpawnName==NULL || pchSpawnName[0]==0)
		{
			pchSpawnName = SPAWN_AT_FURTHEST;
		}
...
//  Pass through remote command for gslQueue_EntAbandonThisMapAndQueue when the ent is on a different game server than the queue map or we
// are calling directly from the TeamServer in the instance of doing a VoteKick
//  In the non-votekick case, the actual abandon has to be done on the queue map so we get the correct penalty data, etc. The Entity Server calls this
// server (Which is the map server). The actual Entity may not be valid at this stage even though we have the ID.
//  In the votekick case, we are calling from the TeamServer to do this. The TeamServer determines if the penalty will need applying.
// Again, the actual Entity may not be valid at this stage even though we have the ID. (It won't be if the entity is already on a different server)

AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER) ACMD_IFDEF(APPSERVER);
void gslQueue_DoEntAbandonThisMapAndQueue(ContainerID uEntID, ContainerID uMapID, U32 uPartitionID, bool bApplyPenaltyIfNeeded)
{
	bool bDoMapLeaveIfOnMap=true; // Always do this
	gslQueue_EntAbandonThisMapAndQueue(uEntID, uMapID, uPartitionID, bDoMapLeaveIfOnMap, bApplyPenaltyIfNeeded);
}
...
void gslQueue_UpdateQueueMatch(U32 uPartitionID, QueueMatch *pNewMatchMembers)
{
	...
	if (!pInfo || !pInfo->bQueueServerInstanceInfoReceived)
	{
		gslQueue_AddPendingMatchUpdate(uPartitionID, pNewMatchMembers,0);
	}
	else
	{
		gslQueue_DoUpdateQueueMatch(uPartitionID, pNewMatchMembers);
	}
}

// This is sort of a problematic command. We mostly get information about who's in a queue from
//  gslQueue_UpdateQueueMatch which is called whenever the QueueServer things the membership of
//  a match is changed. Unfortunately, this code does not trigger when a member is removed and
//  this function is called instead. Also unfortunately, this means that we can possibly get
//  out of synch with the information coming in from the QueueServer. If a member is removed
//  at the same time another member is added to a match. The QueueServer will generate a remove
//  command and an update command. Depending on timing, the GameServer could receive the update
//  packet that says the member being removed is still in the list after the remove command
//  is processed. This will result in a bad team formation. QueueServer should be rearchitected
//  so it actually keeps track of matches rather than creating them on the fly when it needs them
//  and then the LeaveQueue stuff can explicitly ping the matches to update. This function can go
//  away at that point.
//   In the mean time, we will hope this ordering thing is not common. Certainly it will rectify
//  itself if someone else joins the match.
//   There's also an issue that we need to worry about where a remove is received before the
//  server is fully set up. We keep track of a list of pending updates and removes and process
//  them all at once when the server is ready. All updates will happen before removes. This
//  is not quite accurate, but should be mostly correct.
...
void gslQueue_RemoveMemberFromMatch(U32 uPartitionID, ContainerID iEntID)
{
	...
	if (!pInfo || !pInfo->bQueueServerInstanceInfoReceived || !pInfo->pMatch)
	{
		gslQueue_AddPendingMatchUpdate(uPartitionID, NULL, iEntID);
		return;
	}

	// We are set up. We can go ahead and do the remove.
	{
		S32 iGroupIndex;
		for (iGroupIndex = eaSize(&pInfo->pMatch->eaGroups)-1; iGroupIndex >= 0; iGroupIndex--)
		{
			...
			if (pGroup)
			{
				...
				if (iMemberIdx >= 0)
				{
					StructDestroy(parse_QueueMatchMember, eaRemove(&pGroup->eaMatchMembers, iMemberIdx));
				}
				pGroup->iGroupSize = eaSize(&pGroup->eaMatchMembers);
...
void gslQueue_AttemptToPenalizePlayer(U32 uPartitionID, U32 uPlayerID, const char* pchQueueName)
{
	...
	{
		RemoteCommand_aslQueue_PenalizePlayer(GLOBALTYPE_QUEUESERVER, 0, uPlayerID, pchQueueName, false);
	}
}
...
void gslQueuePartition_PrintMatchList(QueuePartitionInfo *pInfo)
{
	...
	for(iGroup=0;iGroup<eaSize(&pInfo->pMatch->eaGroups);iGroup++)
	{
		...
		for(iMemeber=0;iMemeber<eaSize(&pGroup->eaMatchMembers);iMemeber++)
		{
			printf("\t\tMember %d: Entity ID %d Team ID %d\n",iMemeber,pGroup->eaMatchMembers[iMemeber]->uEntID,pGroup->eaMatchMembers[iMemeber]->uTeamID);
		}
	}
