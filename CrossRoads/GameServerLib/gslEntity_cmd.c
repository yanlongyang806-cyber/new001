...
void gslEntity_TrackVolumeStatus(Entity *pEnt, const char *pchVolumeName)
{
 	if (pEnt && pEnt->pPlayer)
	{
		InteractVolumeStatus *pVolumeStatus = eaIndexedGetUsingString(&pEnt->pPlayer->InteractStatus.ppInteractVolumeStatus, pchVolumeName);
		if (!pVolumeStatus)
		{
			pVolumeStatus = StructCreate(parse_InteractVolumeStatus);
			pVolumeStatus->pchVolumeName = StructAllocString(pchVolumeName);
			pVolumeStatus->bInVolume = eaFindString(&pEnt->pPlayer->InteractStatus.eaInVolumes, pchVolumeName) >= 0;
			eaIndexedAdd(&pEnt->pPlayer->InteractStatus.ppInteractVolumeStatus, pVolumeStatus);
...
void gslPlayer_NumericThresholdExceededAlert(const char *debugName,
	const char *strItemDefName, const char *strThresholdName, S32 iThreshold, U32 uDuration, S32 iOldValue, S32 iNewValue, ItemChangeReason *pReason)
{
	TriggerAutoGroupingAlert("NUMERIC_THRESHOLD_EXCEEDED_ALERT", ALERTLEVEL_WARNING, ALERTCATEGORY_GAMEPLAY, 60*60,
		"EntityPlayer[%s] triggered the %s (%d) numeric threshold alert for %s in %u seconds on map %s (%.02f,%.02f,%.02f) with reason %s. Prev value: %d. New value: %d",
		debugName, strThresholdName, iThreshold, strItemDefName, uDuration, pReason->pcMapName,
		pReason->vPos ? pReason->vPos[0] : 0, pReason->vPos ? pReason->vPos[1] : 0, pReason->vPos ? pReason->vPos[2] : 0, pReason->pcReason, iOldValue, iNewValue);

	log_printf(LOG_NUMERIC_THRESHOLD,
		"EntityPlayer[%s] triggered the %s (%d) numeric threshold alert for %s in %u seconds on map %s (%.02f,%.02f,%.02f) with reason %s. Prev value: %d. New value: %d",
		debugName, strThresholdName, iThreshold, strItemDefName, uDuration, pReason->pcMapName,
		pReason->vPos ? pReason->vPos[0] : 0, pReason->vPos ? pReason->vPos[1] : 0, pReason->vPos ? pReason->vPos[2] : 0, pReason->pcReason, iOldValue, iNewValue);
}
