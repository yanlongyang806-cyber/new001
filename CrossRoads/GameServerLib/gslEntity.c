...
void gslEntityMovementManagerMsgHandler(const MovementManagerMsg* msg){
	...
	switch(msg->msgType){
		...
		xcase MM_MSG_FG_MOVE_ME_SOMEWHERE_SAFE:{
			if(e->pPlayer){
				spawnpoint_MovePlayerToNearestSpawn(e, NULL, true, false);
			}
		}
...
static void gslInitPlayerLoginPosition(Entity* e, Vec3 vEstimateOut); // req header to init. entity
...
void gslInitializeEntity(Entity* e, bool bIsReloading)
{
	...
	if (gGSLState.gbGatewayServer || gGSLState.gbWebRequestServer || gGSLState.gbReplayServer)
	{
	...
	PERFINFO_AUTO_START("Estimate Spawn Pos", 1);
	if (!bIsReloading &&
		entGetType(e) == GLOBALTYPE_ENTITYPLAYER)
	{
		// goto an estimate of the player's spawn position
		// the actual spawn may be different after we complete logging the player in
		// if we don't setup the spawn point here, we're inheriting the entity's last position from the network
		// it's very likely that last position may be from a different map leading to loads of bugs & possible crashes if we don't do this
		Vec3 vEstimatedSpawnPos;
		gslInitPlayerLoginPosition(e, vEstimatedSpawnPos);
		copyVec3(vEstimatedSpawnPos, e->pos_use_accessor); // bootstrapping for mmCreate
	}
	PERFINFO_AUTO_STOP(); // Estimate Spawn Pos
...
bool LeaveMapEx(Entity* e, DoorTransitionSequenceDef* pTransOverride, const char* strReason)
{
	if ( e )
	{
		...
		} else if ( isProductionEditMode() && ugcGetDefaultMapName( gGSLState.pLastPlayData )) {
			// If in production edit mode, it's possible to get here
			// because the project is incomplete.  Just go to the
			// default map then.
			const char* defaultUGCMap = ugcGetDefaultMapName( gGSLState.pLastPlayData );
			RegionRules* pNextRules = MapTransferGetRegionRulesFromMapName(defaultUGCMap);
			MapDescription* pDestination = StructCreate( parse_MapDescription );
			MapMoveFillMapDescription( pDestination, defaultUGCMap, ZMTYPE_UNSPECIFIED, NULL, 0, 0, 0, 0, NULL );
			gslEntityPlayTransitionSequenceThenMapMove(e, pDestination, pCurrRules, pNextRules, pTransOverride, 0);
			StructDestroy( parse_MapDescription, pDestination );
		} else {
			// If we ever get here, the player is stuck.  Log it along with the player name and name of the current map.
			ErrorDetailsf("Reason=%s, Ent=%s, Map=%s", strReason, e->debugName, zmapInfoGetPublicName(NULL));
...
{
	LeaveMapEx(e, NULL, __FUNCTION__);
}
...
void ReturnFromMissionMap(Entity* e)
{
	...
	if (LeaveMapEx(e, NULL, __FUNCTION__))
	{
...
{
	// This function appears to get called in a lot of places that it probably shouldn't (where the region isn't being changed).
	// I'm wondering if people are calling it to abuse the side effects?
	WorldRegion *pRegion, *pPrevRegion;
	Vec3 vPlayerPos;
	...
	pPrevRegion = e->astrRegion ? zmapGetWorldRegionByName(NULL, e->astrRegion) : NULL;

	ePrevRegionType = worldRegionGetType(pPrevRegion);
...
static void gslInitPlayerLoginPosition(Entity* e, Vec3 vEstimateOut)
{
	// when you pass vEstimateOut into this function, it shouldn't modify
	...
	if (!vEstimateOut)
	{
		...
		// we can probably delete this if-block, Rob indicated it was for Creatures of the Night (unreleased title)
	...
	{
		spawnpoint_MovePlayerToStartSpawn(e, vEstimateOut, false);
	}
	else if (stricmp(spawnPointToUse, SPAWN_AT_NEAR_RESPAWN) == 0)
	{
		spawnpoint_MovePlayerToNearestSpawn(e, vEstimateOut, true, false);
	}
	else if (stricmp(spawnPointToUse, SPAWN_AT_FURTHEST) == 0)
	{
		spawnpoint_MovePlayerToFurthestActivatedSpawnPoint(e, vEstimateOut, false);
	}
	else if (stricmp(spawnPointToUse, POSITION_SET) == 0)
	{
		...
		// make sure the player isn't going to try spawning somewhere invalid
		// this can happen if they escape the world collision geo then explore until they crash their client on a static map
		if (CHECK_DYNPOS_NONFATAL_WITH_TOLERANCE(spawnPos, 50.f))
		{
			if (!vEstimateOut)
			{
				if (eMount = entGetMount(e))
			...
			{
				copyVec3(spawnPos, vEstimateOut);
			}
		}
		else
		{
	...
	{
		spawnpoint_MovePlayerToNamedSpawn(e, spawnPointToUse, NULL, vEstimateOut, true);
	}
...
void gslPlayerEnteredMap(Entity* e, bool bInitPosition)
{
	...
	if (bInitPosition) {
		gslInitPlayerLoginPosition(e, NULL);
	}
	...
	gslGameSpecific_Entity_Login(e);
...
int RegisterEntityContainers(void)
{
	...
	// Register gateway game schema.
	gslGatewayGame_SchemaInit();
...
AUTO_TRANSACTION
	ATR_LOCKS(pOwner, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Psaved.Costumedata.Eacostumeslots, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(e, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Psaved.Costumedata.Eacostumeslots, .Psaved.Savedname, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Savedsubname, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
...
AUTO_TRANSACTION
ATR_LOCKS(pOwner, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Psaved.Costumedata.Eacostumeslots, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
ATR_LOCKS(e, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Psaved.Savedsubname, .Psaved.Costumedata.Eacostumeslots, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Psaved.Savedname, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
ATR_LOCKS(pData, ".Eakeys");
...
void AllegianceSetValidate(Entity *e, ACMD_NAMELIST("Allegiance", REFDICTIONARY) const char *allegianceName)
{
	...
	if(e && pNewAllegiance)
	{
		...
		if(e->pPlayer && e->pPlayer->accessLevel < ACCESS_DEBUG && e->pPlayer->missionInfo &&
			pMissionDef && !mission_FindMissionFromDef(e->pPlayer->missionInfo, pMissionDef))
		{
			return;
		}
...
AUTO_TRANSACTION
ATR_LOCKS(e, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Psaved.Bbadname, .Psaved.Esoldname, .Psaved.Savedname, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Savedsubname, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
...
void DEFAULT_LATELINK_gslGameSpecific_Entity_Login(Entity* e)
{
...
// --------------------------------------------------------------------------------------------------------------------
// fills the peaPowersOut list with powers on the character
static void gslEntity_GetListOfPowers(Entity *pEnt, Power ***peaPowersOut)
{
	if (!pEnt || !pEnt->pChar)
		return;

	eaClear(peaPowersOut);

	// Put all the powers the entity has in a list, sort it and then check for any duplicate IDs
	eaPushEArray(peaPowersOut, &pEnt->pChar->ppPowersPersonal);
	eaPushEArray(peaPowersOut, &pEnt->pChar->ppPowersClass);
	eaPushEArray(peaPowersOut, &pEnt->pChar->ppPowersSpecies);

	FOR_EACH_IN_EARRAY_FORWARDS(pEnt->pChar->ppPowerTrees, PowerTree, pPowerTree)
	{
		FOR_EACH_IN_EARRAY_FORWARDS(pPowerTree->ppNodes, PTNode, pNode)
		{
			eaPushEArray(peaPowersOut, &pNode->ppPowers);
		}
		FOR_EACH_END
	}
	FOR_EACH_END
	...
	if(NONNULL(pEnt->pInventoryV2))
	{
		...
		for(i=0; i<eaSize(&pEnt->pInventoryV2->ppInventoryBags); i++)
		{
			...
			bagiterator_SetBagByID(pEnt, pEnt->pInventoryV2->ppInventoryBags[i]->BagID, iter,pExtract);
			for(; !bagiterator_Stopped(iter); bagiterator_Next(iter))
			{
				Item *pItem = (Item*)bagiterator_GetItem(iter);
				if(pItem)
				{
					eaPushEArray(peaPowersOut, &pItem->ppPowers);
					if (pItem->pSpecialProps)
					{
						FOR_EACH_IN_EARRAY_FORWARDS(pItem->pSpecialProps->ppItemGemSlots, ItemGemSlot, pGemSlot)
						{
							if (pGemSlot)
								eaPushEArray(peaPowersOut, &pGemSlot->ppPowers);
						}
						FOR_EACH_END
					}
				}
...
// --------------------------------------------------------------------------------------------------------------------
static S32 SortPowersByID(const Power **left, const Power **right)
{
	if ((*left)->uiID < (*right)->uiID)
		return -1;
	else if ((*left)->uiID > (*right)->uiID)
		return 1;
	return 0;
}

// --------------------------------------------------------------------------------------------------------------------
// returns true if there are any powers on the entity have the the same ID
static bool gslEntity_FindDuplicatePowerIDs(Entity *pEnt, Power ***peaPowersListOut)
{
	S32 i, size;
	static Power** s_eaPowers = NULL;

	if (!pEnt || !pEnt->pChar)
		return false;

	gslEntity_GetListOfPowers(pEnt, &s_eaPowers);
	
	eaQSort(s_eaPowers, SortPowersByID);

	size = eaSize(&s_eaPowers) - 1;
	for(i = 0; i < size; ++i)
	{
		if (s_eaPowers[i]->uiID == s_eaPowers[i+1]->uiID)
		{
			eaPush(peaPowersListOut, s_eaPowers[i]);
			eaPush(peaPowersListOut, s_eaPowers[i+1]);
			++i;
		}
	}

	return eaSize(peaPowersListOut);
}



typedef struct ResetPowerIDsOldPowersData
{
	// list of the duped powerIDs
	U32 *eaiDupePowerIDs;

	// the original list of ordered powerIDs
	U32 *eaiPowerIDs;

	// the original list of PowerDefs, safety measure to make sure the powerIDs are proper
	ResetPowerIDsRefs **eaPowerDefRefs;

} ResetPowerIDsOldPowersData;


typedef struct ResetPowerIDsCBData
{
	ResetPowerIDsOldPowersData	powersData;
	
	// the containerID for the entity that is being fixed
	ContainerID containerID;

	// the callback we'll call after we're done
	TransactionReturnCallback callerCB;

	// the data for the caller's CB
	void *callerUserData;
...
// --------------------------------------------------------------------------------------------------------------------
// helper- returns true if it should ignore trying to fixup the given power ID. 
// Checks if the powerID is on the dupelist, if we have one
static S32 _PostResetPowerIDsFixup_ShouldFixup(U32 uiPowerID, ResetPowerIDsOldPowersData *pOldPowerData)
{
	return	(uiPowerID == 0) || 
			(eaiSize(&pOldPowerData->eaiDupePowerIDs) == 0) || 
			(eaiFind(&pOldPowerData->eaiDupePowerIDs, uiPowerID) == -1);
}

// --------------------------------------------------------------------------------------------------------------------
static U32 _PostResetPowerIDsFixup_GetNewPowerID(	U32 uiPowerID, 
													ResetPowerIDsOldPowersData *pOldPowerData, 
													Power** eaNewPowers,
													PowerDef *pDefHint)
{
	U32 uiNewID = 0;
	if (uiPowerID != 0)
	{
		...
		// find the index of the powerID in the old powerID list
		idx = eaiFind(&pOldPowerData->eaiPowerIDs, uiPowerID);
		if (idx != -1)
		{
			ResetPowerIDsRefs *pRef = eaGet(&pOldPowerData->eaPowerDefRefs, idx);
			Power *pPower = eaGet(&eaNewPowers, idx);
			if (pPower && pRef)
			{
				// check to make sure this is the right power
				if ((pDefHint && GET_REF(pRef->hDef)) || 
					(!pDefHint && REF_COMPARE_HANDLES(pPower->hDef, pRef->hDef)))
				{
					uiNewID = pPower->uiID;
				}
				else
				{
					if (!pDefHint)
						pDefHint = GET_REF(pPower->hDef);
					// this isn't the right ref. This shouldn't really happen, but if it does we're going to make a
					// best guess as to the new ID
					// go through all the new powers and find one that matches the ref
					FOR_EACH_IN_EARRAY(eaNewPowers, Power, pNewPower)
					{
						if (pNewPower && pDefHint == GET_REF(pNewPower->hDef))
						{
							uiNewID = pNewPower->uiID;
							break;
						}
					}
					FOR_EACH_END
...
// --------------------------------------------------------------------------------------------------------------------
static void _PostResetPowerIDsFixup_FixupTrayElems( TrayElem **ppTrayElems, 
													ResetPowerIDsOldPowersData *pOldPowerData,
													Power** eaNewPowerIDs)
{
	FOR_EACH_IN_EARRAY_FORWARDS(ppTrayElems, TrayElem, pTrayElem)
	{
		switch (pTrayElem->eType)
		{
		case kTrayElemType_Power:
		case kTrayElemType_PowerSlot:
			{
				if (_PostResetPowerIDsFixup_ShouldFixup(pTrayElem->lIdentifier, pOldPowerData))
				{
					U32 uiPowerID = (U32)pTrayElem->lIdentifier;
					
					uiPowerID = _PostResetPowerIDsFixup_GetNewPowerID(uiPowerID, pOldPowerData, eaNewPowerIDs, NULL);
					pTrayElem->lIdentifier = (U32)uiPowerID;
				}
			} break;
		}
	}
...
// --------------------------------------------------------------------------------------------------------------------
// attempts to fixup any old powerIDs. 
//		currently handles: powerSlots, attribMods
static void gslEntity_PostResetPowerIDsFixup(Entity *pEnt, ResetPowerIDsOldPowersData *pOldPowerData)
{
	static Power** s_eaPowers = NULL;
		
	// get the new list of powerIDs, 
	// using the old powerIDs, find old references to those powerIDs and replace them with the new powerID
	// we are doing this by assuming that the powerID is the same position in the array, which should be the case!
	gslEntity_GetListOfPowers(pEnt, &s_eaPowers);

	if (pEnt->pChar->pSlots)
	{
		FOR_EACH_IN_EARRAY_FORWARDS(pEnt->pChar->pSlots->ppSets, PowerSlotSet, pSlotSet)
		{
			S32 i, size = eaiSize(&pSlotSet->puiPowerIDs);
			for (i = 0; i < size; ++i)
			{
				U32 uiPowerID = pSlotSet->puiPowerIDs[i];
				if (_PostResetPowerIDsFixup_ShouldFixup(uiPowerID, pOldPowerData))
				{
					uiPowerID = _PostResetPowerIDsFixup_GetNewPowerID(pSlotSet->puiPowerIDs[i], pOldPowerData, s_eaPowers, NULL);
					pSlotSet->puiPowerIDs[i] = uiPowerID;
				}
			}
		}
		FOR_EACH_END

		// if it's a player waiting to login, 
		// don't run the power slots validate as it will be validated later in the login process
		if (pEnt->pPlayer && !pEnt->pPlayer->eLoginWaiting)
		{
			character_PowerSlotsValidate(pEnt->pChar);
		}
	}

	// fixup all the saved AttribMods
	FOR_EACH_IN_EARRAY_FORWARDS(pEnt->pChar->modArray.ppModsSaved, AttribMod, pMod)
	{
		if (_PostResetPowerIDsFixup_ShouldFixup(pMod->uiPowerID, pOldPowerData))
		{
			U32 uiPowerID = _PostResetPowerIDsFixup_GetNewPowerID(pMod->uiPowerID, pOldPowerData, 
																	s_eaPowers, GET_REF(pMod->hPowerDef));
			pMod->uiPowerID = uiPowerID;
		}
	}
	FOR_EACH_END

	// fixup the SavedEntityData tray
	if (pEnt->pSaved && pEnt->pSaved->pTray)
	{
		SavedTray *pTray = pEnt->pSaved->pTray;
		S32 i, size = eaiSize(&pTray->puiNoSlotPowerIDs);
		for (i = size - 1; i >= 0; --i)
		{
			if (_PostResetPowerIDsFixup_ShouldFixup(pTray->puiNoSlotPowerIDs[i], pOldPowerData))
			{
				U32 uiPowerID = _PostResetPowerIDsFixup_GetNewPowerID(pTray->puiNoSlotPowerIDs[i], pOldPowerData, s_eaPowers, NULL);
				if (uiPowerID != 0)
				{
					pTray->puiNoSlotPowerIDs[i] = uiPowerID;
				}
				else
				{	// we couldn't find a new powerID. this shouldn't happen, but incase it does- remove the index
					eaiRemove(&pTray->puiNoSlotPowerIDs, i);
				}
			}
		}
		
		// fix up the trays
		_PostResetPowerIDsFixup_FixupTrayElems(pTray->ppTrayElems, pOldPowerData, s_eaPowers);
		_PostResetPowerIDsFixup_FixupTrayElems(pTray->ppAutoAttackElems, pOldPowerData, s_eaPowers);
	}

	// fixup the saved PowerActivation states
	if (pEnt->pChar->ppActivationState)
	{
		FOR_EACH_IN_EARRAY(pEnt->pChar->ppActivationState, PowerActivationState, pPowerActState)
		{
			if (_PostResetPowerIDsFixup_ShouldFixup(pPowerActState->ref.uiID, pOldPowerData))
			{
				U32 uiPowerID = _PostResetPowerIDsFixup_GetNewPowerID(pPowerActState->ref.uiID, pOldPowerData, 
																		s_eaPowers, GET_REF(pPowerActState->ref.hdef));

				pPowerActState->ref.uiID = uiPowerID;
			}
		}
		FOR_EACH_END
	}
...
// --------------------------------------------------------------------------------------------------------------------
static void ResetPowerIDs_CB(TransactionReturnVal *pVal, void *userData)
{
	ResetPowerIDsCBData *pCBData = (ResetPowerIDsCBData*)userData;

	if (pCBData)
	{
		TransactionReturnCallback callerCB = NULL;
		void *callerUserData = NULL;

		Entity *pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, pCBData->containerID);
		if (pEnt && pEnt->pChar)
		{
			gslEntity_PostResetPowerIDsFixup(pEnt, &pCBData->powersData);
		}

		callerCB = pCBData->callerCB;
		callerUserData = pCBData->callerUserData;
		eaiDestroy(&pCBData->powersData.eaiPowerIDs);
		eaiDestroy(&pCBData->powersData.eaiDupePowerIDs);
		eaDestroyStruct(&pCBData->powersData.eaPowerDefRefs, parse_ResetPowerIDsRefs);
		free(pCBData);
		pCBData = NULL;

		if (callerCB)
		{
			callerCB(pVal, callerUserData);
		}
	}
}

// --------------------------------------------------------------------------------------------------------------------
static ResetPowerIDsCBData* gslEntity_PowersResetIDs_CreateCBData(Entity *pEnt, U32 *eaiDupePowerIDs, TransactionReturnCallback pCB, void *userData)
{	
	static Power** s_eaPowers = NULL;

	ResetPowerIDsCBData *pCBData = calloc(1, sizeof(ResetPowerIDsCBData));
	...
	if (pCBData)
	{
		...
		eaiCopy(&pCBData->powersData.eaiDupePowerIDs, &eaiDupePowerIDs);
		
		FOR_EACH_IN_EARRAY_FORWARDS(s_eaPowers, Power, pPower)
		{
			if (pPower)
			{
				ResetPowerIDsRefs *pPowerRef = NULL;

				eaiPush(&pCBData->powersData.eaiPowerIDs, pPower->uiID);

				pPowerRef = eaGetStruct(&pCBData->powersData.eaPowerDefRefs, parse_ResetPowerIDsRefs, iNumPowers);
				if (pPowerRef)
				{
					COPY_HANDLE(pPowerRef->hDef, pPower->hDef);
					iNumPowers++;
				}
			}
		}
		...
		pCBData->callerCB = pCB;
		pCBData->callerUserData = userData;
		pCBData->containerID = entGetContainerID(pEnt);
	}
...
// --------------------------------------------------------------------------------------------------------------------
static void gslEntity_PowersResetIDs_Log(Entity *pEnt, Power **eaDupedPowers)
{
	char* pchLog = NULL;
	char* pchLine = NULL;
	
	estrStackCreate(&pchLog);
	estrStackCreate(&pchLine);
	
	FOR_EACH_IN_EARRAY(eaDupedPowers, Power, pPower)
	{
		PowerDef *pDef = GET_REF(pPower->hDef);

		if (pDef)
		{
			estrPrintf(&pchLine, "Power: Name %s ID %d ", pDef->pchName, pPower->uiID);
			estrAppend2(&pchLog, pchLine);
		}
	}
	FOR_EACH_END

	
	entLog(LOG_POWERSID_DUPE_RESET, pEnt, "PowersResetDupeIDs", "PowerIDMax %d %s", pEnt->pChar->uiPowerIDMax, pchLog);
...
{
	Power **eaDupedPowers = NULL;
	
	if (pEnt && pEnt->pChar && gslEntity_FindDuplicatePowerIDs(pEnt, &eaDupedPowers))
	{
		GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
		ResetPowerIDsCBData *pCBData = NULL;
		U32 *eaiDupePowerIDs = NULL;
		
		gslEntity_PowersResetIDs_Log(pEnt, eaDupedPowers);

		FOR_EACH_IN_EARRAY_FORWARDS(eaDupedPowers, Power, pPower)
		{
			eaiPushUnique(&eaiDupePowerIDs, pPower->uiID);
		}
		FOR_EACH_END

		pCBData = gslEntity_PowersResetIDs_CreateCBData(pEnt, eaiDupePowerIDs, pCB, userData);

		if (pCBData)
		{
			TransactionReturnVal *pReturn = NULL;
			PowerIDList *pPowerIDList = NULL;
			
			if (pEnt->pPlayer)
				pEnt->pPlayer->eLoginWaiting |= kLoginSuccess_PowerIDFixup;

			pReturn = LoggedTransactions_CreateManagedReturnValEnt("PowersResetDupeIDs", pEnt, ResetPowerIDs_CB, pCBData);

			pPowerIDList = StructCreate(parse_PowerIDList);
			pPowerIDList->eaiPowerIDs = eaiDupePowerIDs;
			eaiDupePowerIDs = NULL;
			
			AutoTrans_trEntity_PowersListResetIDs( pReturn, GLOBALTYPE_GAMESERVER, 
													pEnt->myEntityType, pEnt->myContainerID,
													pPowerIDList, pExtract);

			StructDestroy(parse_PowerIDList, pPowerIDList);
			return true;
		}

		ea32Destroy(&eaiDupePowerIDs);
	}
