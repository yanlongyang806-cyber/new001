...
AUTO_TRANSACTION
ATR_LOCKS(playerEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Savedname, .Pplayer.Publicaccountname, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pplayer.Eaplayernumericthresholds")
ATR_LOCKS(projectContainer, ".Containertype, .Containerid, projectList[], .Discountlist");
...
GroupProject_tr_DonateSimpleItems(ATR_ARGS, NOCONST(Entity) *playerEnt, NOCONST(GroupProjectContainer) *projectContainer, const char *projectName, int slotNum, const char *bucketName, const char *itemName, S32 count, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
    ...
    // Get the required amount after applying discounts
    donationRequired = GroupProject_trh_GetDiscountedBucketRequirement(ATR_PASS_ARGS, projectContainer, projectState, taskSlot, bucketName);
    if ( donationRequired == GROUP_PROJECT_INVALID_DISCOUNTED_BUCKET_REQUIREMENT )
    {
        return TRANSACTION_OUTCOME_FAILURE;
    }
    ...
    remaining = donationRequired - bucketData->donationCount;
    if ( remaining < 0 )
    {
        return TRANSACTION_OUTCOME_FAILURE;
    }
    ...
    if ( ( countRemoved > 0 )  && ( bucketData->donationCount == donationRequired ) )
    {
...
GroupProject_trh_DonateSingleItemStack(ATR_ARGS, ATH_ARG NOCONST(Entity) *playerEnt, int remaining, const char *itemName, int bagID, int slotIdx, int count, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
    ...
    if ( remaining > 0 )
    {
        ...
        removedItem = invbag_RemoveItem(ATR_PASS_ARGS, playerEnt, false, bagID, slotIdx, count, false, pReason, pExtract);
...
AUTO_TRANSACTION
ATR_LOCKS(playerEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Savedname, .Pplayer.Publicaccountname, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaplayernumericthresholds")
ATR_LOCKS(projectContainer, ".Containertype, .Containerid, projectList[], .Discountlist");
enumTransactionOutcome
GroupProject_tr_DonateExpressionItem(ATR_ARGS, NOCONST(Entity) *playerEnt, NOCONST(GroupProjectContainer) *projectContainer, const char *projectName, int taskSlotNum, const char *taskName, const char *bucketName, const char *itemName, int bagID, int slotIdx, int count, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
    ...
    {
        // Check for bucket completion.
        if ( bucketData->donationCount == donationRequired )
        {
            taskSlot->completedBuckets++;
            bucketFilled = true;
        }
...
AUTO_TRANSACTION
ATR_LOCKS(playerEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Savedname, .Pplayer.Publicaccountname, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, pInventoryV2.ppInventoryBags, pInventoryV2.ppLiteBags, .Pplayer.Eaplayernumericthresholds")
ATR_LOCKS(projectContainer, ".Containertype, .Containerid, projectList[], .Discountlist");
enumTransactionOutcome
...
AUTO_TRANSACTION
ATR_LOCKS(projectContainer, "projectList[], .Discountlist");
enumTransactionOutcome
GroupProject_tr_ForceFillBucket(ATR_ARGS, NOCONST(GroupProjectContainer) *projectContainer, const char *projectName, int slotNum, const char *bucketName)
{
    ...
    // Fill the bucket
    bucketData->donationCount = donationRequired;
    ...
    for ( i = eaSize(&projectContainer->projectList) - 1; i >= 0; i-- )
    {
        ...
        for ( j = eaSize(&projectState->taskSlots) - 1; j >= 0; j-- )
        {
            ...
            for ( k = eaSize(&taskDef->buckets) - 1; k >= 0; k-- )
            {
                ...
                // Get the required amount after applying discounts
                donationRequired = GroupProject_trh_GetDiscountedBucketRequirement(ATR_PASS_ARGS, projectContainer, projectState, taskSlot, donationRequirement->name);
                if ( donationRequired == GROUP_PROJECT_INVALID_DISCOUNTED_BUCKET_REQUIREMENT )
                {
                    return TRANSACTION_OUTCOME_FAILURE;
                }
...
GroupProject_tr_ForceSetBucketFill(ATR_ARGS, NOCONST(GroupProjectContainer) *projectContainer, const char *projectName, int slotNum, const char *bucketName, int count)
{
    ...
    if (count > (int)donationRequired)
    {
        count = donationRequired;
    }
    ...
    const char *projectName, int slotNum)
{
    ...
    // and set the state indicating that the reward has been claimed.
    if ( taskSlot->state == DonationTaskState_FinalizedRewardPending )
    {
        taskSlot->state = DonationTaskState_Finalized;
    }
    else if (taskSlot->state == DonationTaskState_RewardPending)
    {
...
ATR_LOCKS(projectContainer, "projectList[]");
enumTransactionOutcome
GroupProject_tr_CancelNextProject(ATR_ARGS, NOCONST(GroupProjectContainer) *projectContainer, const char *projectName, int slotNum)
{
    NOCONST(GroupProjectState) *projectState;
    NOCONST(DonationTaskSlot) *taskSlot;
    GroupProjectDef *projectDef;
...
AUTO_TRANSACTION
ATR_LOCKS(playerEnt, ".Pinventoryv2.Peaowneduniqueitems, .Psaved.Savedname, .Pplayer.Publicaccountname, .Pchar.Ilevelexp, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Pplayer.Eaplayernumericthresholds, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype")
ATR_LOCKS(projectContainer, ".Containertype, .Containerid, .Projectlist, .Discountlist, .Bonuslist");
enumTransactionOutcome
GroupProject_tr_SetTaskAndDonateAllItems(ATR_ARGS, NOCONST(Entity) *playerEnt, NOCONST(GroupProjectContainer) *projectContainer, const char *projectName, const char *taskName, int slotNum, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
    NOCONST(GroupProjectState) *projectState;
    NOCONST(DonationTaskSlot) *taskSlot;
    NOCONST(DonationTaskBucketData) *bucketData;
    ...
    int i;
    ItemChangeReason reason = {0};
    const char *contributionNumericName = "";
    ...
    // Make sure the right task got slotted.
    if ( stricmp(taskName, taskDef->name) != 0 )
    {
        return TRANSACTION_OUTCOME_FAILURE;
    }
    ...
    // Iterate over all the requirements, if we should have them
	if( !taskDef->noCost )
	{
		for ( i = eaSize(&taskDef->buckets) - 1; i >= 0; i-- )
		{
        // Get the requirements.
        donationRequirement = eaGet(&taskDef->buckets, i);
        if ( ISNULL(donationRequirement) )
        {
            return TRANSACTION_OUTCOME_FAILURE;
        }
        ...
        // Nothing should have been donated yet.
        if ( bucketData->donationCount != 0 )
        {
            return TRANSACTION_OUTCOME_FAILURE;
        }
        ...
        if ( NONNULL(spendingNumeric) )
        {
            ...
            {
                S32 numericRemaining = donationNumericValue - spendingNumericValue;
                if ( numericRemaining >= (S32)donationRequirement->count )
                {
                    countRemoved = donationRequirement->count;
                }
                else
                {
                    countRemoved = numericRemaining;
        ...
        {
            // Remove the items from inventory
            countRemoved = inv_trh_FindItemCountByDefName(ATR_PASS_ARGS, playerEnt, itemNameFromReq, donationRequirement->count, true, pReason, pExtract);
        }

        // Make sure the full required count was removed.
        if ( countRemoved != (S32)donationRequirement->count )
        {
            return TRANSACTION_OUTCOME_FAILURE;
        }
        ...
        if ( NONNULL(contributionNumeric) )
        {
            // Get the value of the contribution constant.
            if ( !GroupProject_trh_GetProjectConstant(ATR_PASS_ARGS, projectState, donationRequirement->contributionConstant, &contributionPerItem) )
            {
            ...
            contributionNumericName = contributionNumeric->pchName;
        }
        ...
        {
            // Buckets must be filled.
            return TRANSACTION_OUTCOME_FAILURE;
        }

        SendContributionNotification(ATR_PASS_ARGS, playerEnt->myEntityType, playerEnt->myContainerID, projectContainer->containerType, projectContainer->containerID,
            itemNameFromReq, donationRequirement->count, countRemoved, initialContribution, contributionGiven, contributionNumericName, contribution, projectName, taskDef->name, donationRequirement->name, bucketFilled, true, (S32)donationRequirement->count == countRemoved, NULL, NULL);
    }

		// All donations must be completed.
		if ( taskSlot->completedBuckets != (U32)eaSize(&taskDef->buckets) )
		{
			return TRANSACTION_OUTCOME_FAILURE;
		}
    ...
    TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(LOG_GROUPPROJECT, "SetTaskAndDonateAllItems", 
        "ProjectName %s TaskSlot %d TaskName %s ContributionNumeric %s Contribution %d BucketFilled %d TaskFinalized %d", 
        projectName, taskSlot->taskSlotNum, taskDef->name, contributionNumericName, contribution, bucketFilled, true);
