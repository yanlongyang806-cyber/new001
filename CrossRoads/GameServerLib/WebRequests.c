...
DumpAllLanguagesResponse *DumpAllLanguages()
{
	...
	for (i = 0; i != languages; ++i)
		response->pLangCodes[i] = locGetCrypticSpecific2LetterIdentifier(i);
...
DumpAllTranslationsResponse *DumpAllTranslations(SA_PARAM_NN_VALID const char *pLanguageCode, U64 uBase)
{
	...
	// Look up language.
	locale = locGetIDByCrypticSpecific2LetterIdentifier(pLanguageCode);
	if (locale == DEFAULT_LOCALE_ID && stricmp(pLanguageCode, locGetCrypticSpecific2LetterIdentifier(locale)))
	{
...
static MissionDefWeb *wrCopyMissionDef(MissionDef *pDef)
{
	...
	pDefCopy->pchDetailString = StructAllocString(wrGetMessageKey(&pDef->detailStringMsg.msg));
...
MicroTransactionUserPurchaseResponse *MicrotransactionProductPurchaseStatus(MicroTransactionPurchaseStatusRequest *request)
{
	...
	PERFINFO_AUTO_START_FUNC();
	response = StructCreate(parse_MicroTransactionUserPurchaseResponse);
	eStatus = gslAP_GetPurchaseStatus(request->accountID, request->requestID, &error, NULL);
...
gslLoginWebRequest_GetCharacterChoices(CmdContext *cmdContext, ContainerID accountID)
{
    ...
    RemoteCommand_aslLogin2_GetCharacterChoicesCmd(returnVal, GLOBALTYPE_LOGINSERVER, SPECIAL_CONTAINERID_RANDOM, accountID, true);
...
void SendNPCMailWebRequest_cb(TransactionReturnVal* val, NPCMailRequest* request)
{
	switch(val->eOutcome)
	{
		case TRANSACTION_OUTCOME_SUCCESS:
			DoSlowCmdReturn(1, "Successfully sent item.",request->pSlowReturnInfo);
			break;

		case TRANSACTION_OUTCOME_FAILURE:
		case TRANSACTION_OUTCOME_NONE:
		default:
			DoSlowCmdReturn(1, "Unknown failure on item send.",request->pSlowReturnInfo);
			break;
	}

	StructDestroy(parse_NPCMailRequest, request);
}

AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
void SendNPCMailWebRequest(CmdContext* cmdContext, NPCMailRequest *request)
{
	if (request->fromName && request->subject)
	{
		if (request->itemDefName && request->itemQuantity == 0)
			request->itemQuantity = 1;

		//Build up our slow return
		request->pSlowReturnInfo = WebRequestSlow_SetupSlowReturn(cmdContext);

		//Check to see if the item exists
		if(RefSystem_ReferentFromString(g_hItemDict, request->itemDefName))
		{
			wrCSub_AddNPCMailRequest(request);
		}
		else
		{
			//Return with failure
			char returnString[128];
			snprintf_s(returnString, 128, "Failed to find item %s.", request->itemDefName);
			DoSlowCmdReturn(1,returnString,request->pSlowReturnInfo);
		}
	}
}

AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
void PlayerBadNameWebRequest(U32 uEntityID)
{
	wrCSub_AddBadNameRequest(uEntityID);
}

AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
void PlayerNameChangeWebRequest(U32 uEntityID, const char *rename)
{
	if (rename && *rename)
		wrCSub_AddNameChangeRequest(uEntityID, rename);
}

#include "WebRequests_h_ast.c"
