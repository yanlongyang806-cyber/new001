...
//This function no longer sets the dirtybit because that's handled by entObjInventoryChangePostCB().
...
static void entObjInventoryChangeNonNumericPostCB(Container *con, ObjectPathOperation **operations)
{
	...
	// 05/30 - if we're still waiting to login, early out
	// the character isn't completely done loading yet, so we shouldn't need to do anything below
	// as once the login has succeeded, it should setup everything this seeks to fixup
	if (ent && ent->pPlayer && ent->pPlayer->eLoginWaiting)
		return;
...
AUTO_TRANSACTION
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags, .Pchar.Ppsavedattributes, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .pInventoryV2.Ppinventorybags, .Psaved.Conowner.Containertype, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Pchar.Ilevelexp")
ATR_LOCKS(itemOwner, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .pInventoryV2.Ppinventorybags, .Psaved.Conowner.Containertype, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Pchar.Ilevelexp");
...
void gslLogOutEntityEx(Entity *ent, SlowRemoteCommandID iCmdID, U32 iPlayerBooterHandle, LogoffType eLogoffType)
{
	...
	RemoteCommand_crManager_UserLogout(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(ent));
...
{
	if(entGetAccessLevel(pEnt) < ACCESS_GM && !isProductionEditMode())
	{ 
...
static bool s_SkipPlayerCallbacksWhenReplaying = false;
void SetSkipPlayerCallbacksWhenReplaying(bool enable)
{
	s_SkipPlayerCallbacksWhenReplaying = enable;
}
...
enumTransactionOutcome PlayerLogoutCB(ATR_ARGS, NOCONST(Entity) *newPlayer, NOCONST(Entity) *backupPlayer, GlobalType locationType, ContainerID locationID)
{
	...
	if(s_SkipPlayerCallbacksWhenReplaying)
		return TRANSACTION_OUTCOME_SUCCESS;
...
AUTO_TRANSACTION
	ATR_LOCKS(pOwner, ".Pplayer.Skilltype, .Pchar.Ilevelexp, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Eskillspecialization, .Psaved.Ugamespecificfixupversion, pInventoryV2.ppLiteBags[], .Pplayer.Playertype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(pSavedPet, ".Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.Pscpdata, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pchar.Hspecies, .Psaved.Costumedata.Iactivecostume, .Psaved.Pptrayelems_Obsolete, .Psaved.Ppautoattackelems_Obsolete, .Psaved.Pppreferredpetids, .Pchar.Hclass, .Pchar.Ilevelexp, .Pinventoryv2, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Pppowertrees, .Pchar.Ppassignedstats, .Pchar.Pppowersclass, .Pchar.Pppowerspersonal, .Psaved.Ugamespecificfixupversion, .Psaved.Costumedata.Pcslotset, .Psaved.Costumedata.Islotsetversion, .Psaved.Costumedata.Eacostumeslots, .Pplayer.Playertype, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
AUTO_TRANSACTION
	ATR_LOCKS(pSavedPet, ".Pchar.Pppowertrees");
enumTransactionOutcome SavedPet_RemovePuppetPowerTrees(ATR_ARGS, NOCONST(Entity) *pSavedPet)
{
	int i;

	if (ISNULL(pSavedPet->pChar))
	{
		return TRANSACTION_OUTCOME_FAILURE;
	}

	for (i = eaSize(&pSavedPet->pChar->ppPowerTrees) - 1; i >= 0; i--)
	{
		PowerTreeDef* pDef = GET_REF(pSavedPet->pChar->ppPowerTrees[i]->hDef);
		if (pDef)
		{
			entity_PowerTreeRemoveHelper(pSavedPet, pDef);
		}
	}
...
AUTO_TRANSACTION
	ATR_LOCKS(pSavedPet, ".Pchar.Pppowertrees");
enumTransactionOutcome SavedPet_RemovePowerTree(ATR_ARGS, NOCONST(Entity) *pSavedPet, PowerTreeDef* pDef)
{
	if (ISNULL(pSavedPet->pChar))
	{
		return TRANSACTION_OUTCOME_FAILURE;
...
AUTO_STARTUP(Schemas) ASTRT_DEPS(UGCTags, AS_CharacterAttribs, AS_AttribSets, ItemQualities, InventoryBagIDs, ItemTags, AS_ActivityLogEntryTypes, UsageRestrictionCategories, AS_ControlSchemeRegions, RewardGatedLoad, GroupProjectTaskSlotTypes);
