...
// Updates to a particular partition that we received before the partition is fully set up.
AUTO_STRUCT;
typedef struct QueuePartitionMatchUpdate
{
	U32 uPartitionID;	// The partition that this update is for.
	
	// Either we are an full match update, or we are a request to remove an entity. One of these fields will be valid
	QueueMatch* pMatch;
	ContainerID iEntIDToRemove;
...
void gslQueue_DoUpdateQueueMatchDebug(QueueMatch *pOldMatchMembers, QueueMatch *pNewMatchMembers)
{
	int i, j;
	...
	if (g_iDebugQueueServer)
	{
		...
		if (pNewMatchMembers)
		{
			for (i = 0; i < eaSize(&pNewMatchMembers->eaGroups); i++)
			{
				...
				for (j = 0; j < eaSize(&pNewGroup->eaMatchMembers); j++)
				{
					QueueMatchMember* pMember = pNewGroup->eaMatchMembers[j];
		...
		if (pOldMatchMembers)
		{
			for (i = 0; i < eaSize(&pOldMatchMembers->eaGroups); i++)
			{
				...
				for (j = 0; j < eaSize(&pOldGroup->eaMatchMembers); j++)
				{
					QueueMatchMember* pMember = pOldGroup->eaMatchMembers[j];
...
void gslQueue_DoUpdateQueueMatch(U32 uPartitionID, QueueMatch *pNewMatchMembers)
{
	...
	{
		// Used to do Nemesis system updates for teams here. That feature is being retired.
		// SVN has the old code.
		
		// Do some Debuggy stuff for qDebug

		if (g_iDebugQueueServer)
		{
			gslQueue_DoUpdateQueueMatchDebug(pInfo->pMatch, pNewMatchMembers);
		}
...
// Either pMatch or iEntIDToRemove should be set
void gslQueue_AddPendingMatchUpdate(U32 uPartitionID, QueueMatch* pMatch, ContainerID iEntIDToRemove)
{
	...
	pMatchUpdate->iEntIDToRemove = iEntIDToRemove;
...
static void gslQueue_ProcessPendingMatchUpdates(int iPartitionIdx)
{
	...
	// First process all full match updates;
	for (i=eaSize(&s_ppMatchUpdates)-1; i>=0; i--)
	{
		QueuePartitionMatchUpdate* pMatchUpdate = s_ppMatchUpdates[i];
		if (pMatchUpdate->uPartitionID == uPartitionID)
		{
			if (pMatchUpdate->pMatch!=NULL)
			{
				gslQueue_DoUpdateQueueMatch(uPartitionID, pMatchUpdate->pMatch);
				StructDestroy(parse_QueuePartitionMatchUpdate, eaRemove(&s_ppMatchUpdates, i));
			}
		}
	}

	// Then process all removes.
	for (i=eaSize(&s_ppMatchUpdates)-1; i>=0; i--)
	{
		QueuePartitionMatchUpdate* pMatchUpdate = s_ppMatchUpdates[i];
		if (pMatchUpdate->uPartitionID == uPartitionID)
		{
			...
			if (pMatchUpdate->iEntIDToRemove!=0)
			{
				gslQueue_RemoveMemberFromMatch(uPartitionID, pMatchUpdate->iEntIDToRemove);
			}
			// Destroy the update no matter what as we do not need it anymore.
			StructDestroy(parse_QueuePartitionMatchUpdate, eaRemove(&s_ppMatchUpdates, i));
		}
	}
}
...
static void gslQueue_SetupLoggedEvents(QueuePartitionInfo *pInfo)
{
	if(pInfo && g_QueueConfig.bQueuedLogging)
	{
		S32 i;

		for(i=0;i<eaSize(&g_QueueConfig.eaQueueLoggedEvents);i++)
		{
			QueueLoggedEvents *pLoggedEvent = g_QueueConfig.eaQueueLoggedEvents[i];
			GameEvent* pEvent = NULL;
			bool result;

			if(!pLoggedEvent->pcEventValue)
			{
				Errorf("Missing value name for queue logged tracked event %d",i);
				continue;
			}

			if(pLoggedEvent->pcEventToLog)
			{
				pEvent = gameevent_EventFromString(pLoggedEvent->pcEventToLog);

				if (!pEvent)
				{
					Errorf("Couldn't create event from string %s", pLoggedEvent->pcEventToLog);
					continue;
				}
			}

			result = mapState_AddPrototypePlayerValue(pInfo->iPartitionIdx, pLoggedEvent->pcEventValue, 0, pEvent);
...
static void gslQueue_InitGameData(QueuePartitionInfo *pInfo, QueueGameInfo* pQueueGameInfo)
{
	if (!pInfo->pGameInfo)
	{
		...
		{
			// We used to process pendings as soon as the partition came up. Now we need to wait 'til we get the instance info back from the QueueServer.
			// On the other hand we need to do this before we try sending info to the PvPGame init because it depends on the proper match being set up.
			gslQueue_ProcessPendingMatchUpdates(pInfo->iPartitionIdx);
			...
			gslQueue_SetupLoggedEvents(pInfo);	// setup logging if set in queueconfig
			...
			{
				// We're a PvE map. We need to set the scoreboard state (which isn't really a scoreboard) to active and inform the map
				// For PvP maps this is handled through gslPVPGame_init which will either call gslPVPGame_start directly, or let it be called after countdown
			...
			for (iInstanceIdx = eaSize(&pQueue->eaInstances)-1; iInstanceIdx >= 0; iInstanceIdx--)
			{
				...
				if (!(*puInstanceID) || pInstance->uiID == (*puInstanceID))
				{
					...
					for (iMemberIdx = eaSize(&pInstance->eaUnorderedMembers)-1; iMemberIdx >= 0; iMemberIdx--)
					{
						...
						uMatchSize = queue_MatchMemberCountWithMap(pInstance, pDef, pMap);
...
static void gslQueue_UpdatePlayerQueueInstance(Entity* pEnt, PlayerQueue* pQueue, QueueInfo* pQueueInfo, QueueInstance* pInstance, QueueMember* pMember)
{
	...
	{
		U32 iQueuedPlayerCount = 0, iOfferedPlayerCount = 0, iAcceptedPlayerCount = 0, iInMapPlayerCount = 0;
		...
		for (iMemberIdx = eaSize(&pInstance->eaUnorderedMembers)-1; iMemberIdx >= 0; iMemberIdx--)
		{
			...
			switch(pCurrMember->eState)
			{
				case PlayerQueueState_InQueue:		++iQueuedPlayerCount; break;
				case PlayerQueueState_Offered:		++iOfferedPlayerCount; break;
				case PlayerQueueState_Accepted:		++iAcceptedPlayerCount; break;
				case PlayerQueueState_Countdown:	++iAcceptedPlayerCount; break;
				case PlayerQueueState_InMap:		++iInMapPlayerCount; break;
				case PlayerQueueState_Limbo:		++iInMapPlayerCount; break;
			}
		}

		pPlayerQInstance->iQueuedPlayerCount = iQueuedPlayerCount;
		pPlayerQInstance->iOfferedPlayerCount = iOfferedPlayerCount;
		pPlayerQInstance->iAcceptedPlayerCount = iAcceptedPlayerCount;
		pPlayerQInstance->iInMapPlayerCount = iInMapPlayerCount;
...
static bool gslQueue_IsInvalidPrivatePlayerInstance(Entity* pEnt, QueueInstance* pInstance)
{
	// WOLF[26Apr13] The comment I had added here on 22Mar was not valid. This looks like it was only intended
	//   as a check for validity of private queues. Public queues do not want to delete stuff the player is not in.
	//   STO certainly needs them for queueCount info. There is a certain amount of inefficiency due to this, but we'll
	//   possibly deal with that elsewhere at another time.
	
	if (!pInstance->pParams->uiOwnerID)
	{
		// Not private.
		return(false);
	}
	...
	{
		// The name indicates something. I do not know what.
		return false;
	}
	else
	{
...
static void gslQueue_CheckPlayerForceLeaveInstance(Entity* pEnt, QueueDef* pDef, PlayerQueue* pQueue, PlayerQueueInstance* pPlayerQInstance)
{
	...
	{
		// Don't leave if we are already on map (or about to be on map). These are the same checks as used to be in aslQueue_LeaveForced, which is what used to keep this working
				 ...
				 eQueueState == PlayerQueueState_Delaying)
		{
			...
			// WOLF[26Apr13] This instance removal is now slightly suspect. I am guessing, but it seems that instance removal is possibly only useful for private queues
			//  that the player does not qualify for. In all other cases, UI's such as STOs want at least the count info for any such queues that can appear in the UI
			//  list. To that end, we theoretically want to keep instances for queues/instances that have errors that are "ShouldShowInvalidQueue" as we still may
			//  want the queue count info for such instances/queues.
			// The original implementation (SVN 84037) did the removal based on gslEntCannotUseQueueInstance and not gslEntCannotUseQueue. Of course at the time
			//  the two functions checked the same criteria.
			// If STO needs this to be different, we'll address when it becomes clear what's up. Testing may be in order for public vs. private queues.
	...
	else
	{
		...
		for (iQIdx = 0; iQIdx < iNumQueues; iQIdx++)
		{
			...
			if (pQueueInfo && pQueueInfo->pchName == pMyQueue->pchQueueName)
			{
				...
				if (!pInstance || !pInstance->pParams || gslQueue_IsInvalidPrivatePlayerInstance(pEnt, pInstance))
				{
					// If we have bad data, or we are a private queue instance that is no longer valid, we go away.
					StructDestroy(parse_PlayerQueueInstance, eaRemove(&pMyQueue->eaInstances,iMyInstIdx));
					entity_SetDirtyBit(pEnt, parse_Player, pEnt->pPlayer, false);		// A little unsafe but we must have a pPlayer if we got the pMyQueue off of it
				}

				// Check to see if we're in an in-queue state, but are not in the queue. This will happen when a player drops a queue or is force dropped but the
				//   client poke won't happen soon. (For NW, this prevents a delay in the queue UI updating, or the Accept/Decline dialog staying present.
				//   for team members without the queue window open who have a different team member decline.
				
				// Are they in the list of unordered members
				{
					QueueMember* pMember = queue_FindPlayerInInstance(pInstance, entGetContainerID(pEnt));
					if ((pMember==NULL) && pMyInstance->eQueueState!=PlayerQueueState_None)
					{
		...
		while ((currEnt = EntityIteratorGetNext(iter)))
		{
			...
			if (pMyQueues->bDataUpdateRequested || bRandomQueuesUpdated == true)
			{
				...
				for (iQIdx = 0; iQIdx < iNumQueues; iQIdx++)
				{
					...
					//
					//  Currently everything other than private unnamed queues stick around on the player.
					//  There may be efficiency to be gained (particularly for NW) in somehow not updating
					//  the instance info if it's not really needed. That could be tricky.
					...
					for (iInstanceIdx = 0; iInstanceIdx < iInstanceCount; iInstanceIdx++)
					{
						...
						//    information. For NW, it is not completely clear when this SHOULD be necessary, but there have been issues
						...
						//  STO needs non-member-of queue instances to update so that it gets the correct player-in-queue counts and such.
						//  Private Queues evidently behave slightly different than public and delete their instances when they are not
						//    available.
						//  gslQueue_IsInvalidPrivatePlayerInstance now encapsulates the old owner/private name check that used to be here.
						if ( !gslQueue_IsInvalidPrivatePlayerInstance(currEnt, pInstance) && 
...
//  This is the end result of gslQueue_HandleLeaveMap or gslQueue_HandleAbandonMap, or a successful VoteKick from the TeamServer
...
// ApplyPossiblyPenalty will be true unless we are in a VoteKick situation. The TeamServer determines whether to apply the penalty

void gslQueue_EntAbandonThisMapAndQueue(ContainerID uEntID, ContainerID uMapID, U32 uPartitionID, bool bDoMapLeaveIfOnMap, bool ApplyPossiblyPenalty)
{
...
bool gslQueue_HandleLeaveMap(Entity* pEnt)
{
	...
	{
		bool bDoMapLeaveIfOnMap=false; // We are assumably leaving the map by some other means, so don't try leaving from within the abandon.
		bool bApplyPenaltyIfNeeded = true;
		gslQueue_EntAbandonThisMapAndQueue(entGetContainerID(pEnt),
									...
									bDoMapLeaveIfOnMap, bApplyPenaltyIfNeeded);
...
bool gslQueue_HandleAbandonMap(Entity* pInstigatorEnt, ContainerID uAbandoningEntID, bool bLeaveTeam)
{
	...
	if (!gslQueue_IsQueueMap())
	{
		...
		if (pInfo!=NULL && pInfo->pQueueInstantiationInfo!=NULL)
		{
			...
			RemoteCommand_gslQueue_DoEntAbandonThisMapAndQueue(GLOBALTYPE_GAMESERVER, uMapID, uAbandoningEntID, uMapID, uPartitionID, bApplyPenaltyIfNeeded);
	...
	else
	{
		...
		gslQueue_EntAbandonThisMapAndQueue(uAbandoningEntID, gGSLState.gameServerDescription.baseMapDescription.containerID,
										   partition_IDFromIdx(entGetPartitionIdx(pInstigatorEnt)), bDoMapLeaveIfOnMap, bApplyPenaltyIfNeeded);
...
S32 gslQueue_GetPlayerGroupIndex(Entity* pEnt)
{
	...
	if (pInfo->pMatch)
	{
		...
		for (iGroupIdx = eaSize(&pInfo->pMatch->eaGroups)-1; iGroupIdx >= 0; iGroupIdx--)
		{
			...
			for (iMemberIdx = eaSize(&pGroup->eaMatchMembers)-1; iMemberIdx >= 0; iMemberIdx--)
			{
				if (entGetContainerID(pEnt) == pGroup->eaMatchMembers[iMemberIdx]->uEntID)
				{
					return iGroupIdx;
				}
			}
...
static void gslQueue_UpdateKickOnAFK(QueuePartitionInfo *pInfo, QueueDef *pDef, QueueGroup *pGroup)
{
	int i;
	U32 uiTime = timeSecondsSince2000();

	for(i=0;i<eaSize(&pGroup->eaMatchMembers);i++)
	{
		Entity *pPlayer = entFromContainerID(pInfo->iPartitionIdx,GLOBALTYPE_ENTITYPLAYER,pGroup->eaMatchMembers[i]->uEntID);
		
		if(pPlayer && SAFE_MEMBER3(pPlayer,pPlayer,pUI,uiLastClientPoke) + pDef->Settings.uMaxAFKTime < uiTime && entGetAccessLevel(pPlayer) <= ACCESS_USER)
		{
			char *pch = NULL;
			entFormatGameMessageKey(pPlayer, &pch, "QueueServer_KickedForAFK",
				STRFMT_END);
			notify_NotifySend(pPlayer, kNotifyType_AwayKickWarning, pch, NULL, NULL);
			gslQueue_HandleAbandonMap(pPlayer,pPlayer->myContainerID,true);
		}
	}
}
...
static void gslQueue_CheckGroups(QueuePartitionInfo *pInfo, MapState *state)
{
	...
	if (pMatch && mapState_GetScoreboardState(state) == kScoreboardState_Active)
	{
		for (iGroupIdx = eaSize(&pMatch->eaGroups)-1; iGroupIdx >= 0; iGroupIdx--)
		{
			...
			// Once the queue is empty and there are no more players being added to the map,
			// allow players to leave the map without penalty if two minutes have passed and there are less
			// players than iLeaverPenaltyMinGroupMemberCount in any of the groups in this match
			if (pInfo->pGameInfo && pInfo->pGameInfo->uLeaverPenaltyDuration &&
				...
				eaSize(&pGroup->eaMatchMembers) < pQueueDef->Settings.iLeaverPenaltyMinGroupMemberCount)
			{
			...
			if(pQueueDef->Settings.uMaxAFKTime)
			{
				gslQueue_UpdateKickOnAFK(pInfo,pQueueDef,pMatch->eaGroups[iGroupIdx]);
			}
		}
	}
}
...
//  WOLF[19Apr13] I'm not sure why this was ever here. Currently we rely on thee gameServer pinging the queue server periodically
//  in order to keep the QueueMap in the right state. If we stop pinging, the QueueServer thinks the map is gone and will shut it
//  down. Sometimes this will place the players in limbo and then forcibly remove them from the map. But if the map times out
//  first, the map will detach and the player will be left in the instance. In any case, on NW. We need to fix a bug where the
//  PvP scoreboard is up, the player gets booted, the leave button disappears, and there is no way to dismiss the rest of the UI.
...
// log player queue events if they are turned on
void gslQueueLogPlayerEvents(QueuePartitionInfo *pInfo, const char *pQueueDefName)
{
	if(pInfo && g_QueueConfig.bQueuedLogging && pQueueDefName)
	{
		MapState* pState = mapState_FromPartitionIdx(pInfo->iPartitionIdx);

		if(pState)
		{
			S32 i, j, n = eaSize(&pState->pPlayerValueData->eaPlayerValues);
			bool bLowStats = false;
			S32 *eaiValues = NULL;
			S32 iLowValue = 99000000;
			U32 uTotalSeconds = 0, uTimesCounted = 0, uTm = timeSecondsSince2000();
			...
			for (i = 0; i < n; ++i)
			{
				...
				if(pPlayerValues)
				{
					...
					S32 iTotalValue = 0;
					for(j = 0; j < eaSize(&g_QueueConfig.eaQueueLoggedEvents);++j)
					{
						pValue = mapState_FindMapValueInArray(&pPlayerValues->eaValues, g_QueueConfig.eaQueueLoggedEvents[j]->pcEventValue);
						if(pValue)
						{
					...
					eaiPush(&eaiValues, iTotalValue);
					if(iTotalValue < 1000)
					{
						bLowStats = true;
					}
					if(iTotalValue < iLowValue)
					{
						iLowValue = iTotalValue;
					}

					if(pPlayerValues->uTimeInitialized > 0)
					{
						uTotalSeconds += uTm - pPlayerValues->uTimeInitialized;
						++uTimesCounted;
					}
				}
			}

			// Queue logging data on game server
			{
				F32 iAveragePoints= 0.0f;
				S32 iNumPlayers = eaiSize(&eaiValues);
				char *pchQueueData = NULL;
				estrStackCreate(&pchQueueData);
		
				if(iNumPlayers > 0)
				{
					for(i = 0; i < iNumPlayers; ++i)
					{
						iAveragePoints += eaiValues[i];
					}

					iAveragePoints /= ((F32)iNumPlayers);
				}

				// average seconds for players in the match
				if(uTimesCounted > 0)
				{
					uTotalSeconds /= uTimesCounted;
				}

				estrPrintf(&pchQueueData, "%s %s %d %d %d %d", pQueueDefName, zmapGetName(NULL), iNumPlayers, iLowValue, (S32)(iAveragePoints + 0.5f), uTotalSeconds);

				for(i = 0; i < iNumPlayers; ++i)
				{
					estrConcatf(&pchQueueData, " %d", eaiValues[i]);		
				}

				// queue name,  players, low, average, p1-p10
				log_printf(LOG_QAFK, "%s", pchQueueData);

				estrDestroy(&pchQueueData);
			}
...
void gslQueue_MapSetStateFromScoreboardStateEx(int iPartitionIdx, ScoreboardState eScoreboardState, const char* pchWinnerFaction)
{
	...
	if(pchQueueDefName && gslQueue_IsQueueMap())
	{
		...
		else if (eScoreboardState==kScoreboardState_Final)
		{
			...
			// log queue events if its on
			gslQueueLogPlayerEvents(pInfo, pchQueueDefName);
