...
static void gslActivity_Start(ActivityDef *pDef, U32 uStartTime, U32 uEndTime)
{
	...
	if(!pActivity)
	{
		...
		reward_ActivityStart(pActivity->pchActivityName);

		if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
		{
			wgsNotifyActivityStarted(pActivity);
		}
...
void gslActivity_End(const char *pchActivity)
{
	...
	if(pActivity)
	{
		...
		reward_ActivityEnd(pActivity->pchActivityName);

		if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
		{
			wgsNotifyActivityEnded(pActivity);
		}
...
AUTO_COMMAND ACMD_ACCESSLEVEL(7) ACMD_NAME(EventClockSet, ShardClockSet);
...
AUTO_COMMAND ACMD_ACCESSLEVEL(7) ACMD_NAME(EventClockUnset, ShardClockUnset);
...
AUTO_COMMAND ACMD_ACCESSLEVEL(7) ACMD_NAME(EventClockGet, ShardClockGet);
...
static void gslActivity_FillCalendarEvent(CalendarEvent* pCalendarEvent, EventDef* pEventDef)
{
	...
	pCalendarEvent->iEventRunMode = pEventDef->iEventRunMode;
}
...
static bool gslActivity_UpdateCalendarEvent(CalendarEvent* pCalendarEvent, EventDef* pEventDef, EventWarpDef* pWarpDef, EventContactDef* pEventContact, Entity* pEntity)
{
	...
	int iEventRunMode = pEventDef->iEventRunMode;
	...
	if (pCalendarEvent->bEventActiveOnServer != (U32)bEventActiveOnServer ||
		...
		pCalendarEvent->iEventRunMode != iEventRunMode ||
...
// Return false if we didn't add/change anything in peaCalendarEvents. 
static bool gslActivity_AddCalendarEventsForTimeRange(Entity* pEntity, 
													  EventDef* pEventDef, 
...
static bool gslActivity_AddManualOnEvent(EventDef* pEventDef, Entity* pEntity, U32* piTagsInclude, U32* piTagsExclude, CalendarEvent*** peaCalendarEvents)
{
	bool bUpdated = false;
	
	// This is to deal with the under-designed manually on/off stuff in conjunction with timed entries.
	// We need to add active events to a list if they are manually on, even if they are not in the
	// requested time range as defined by their schedule. So set up a
	// CalendarEvent as if AddCalendarEventsForTimeRange had done it. (There will be no TimingEntry because it would not be valid)
	// We must rely on display filters to keep this from sneaking into other display formats on the UI end.

	if (!ActivityCalendarFilterByTag(pEventDef->uDisplayTags, piTagsInclude, piTagsExclude))
	{
		...
		// Add the CalendarEvent as if we had a good time range.
		CalendarEvent *pCalendarEvent = eaIndexedGetUsingString(peaCalendarEvents, pEventDef->pchEventName);
		if (!pCalendarEvent)
		{
			pCalendarEvent = StructCreate(parse_CalendarEvent);
		...
		if (gslActivity_UpdateCalendarEvent(pCalendarEvent, pEventDef, pWarpDef, pEventContact, pEntity))
		{
			bUpdated=true;
		}
		// We won't give this a Timing entry as it is manually on without a true timing entry
...
void gslActivity_UpdateServerCalendar(Entity *pEntity)
{
	...
	while(pEventDef = (EventDef *)RefSystem_GetNextReferentFromIterator(&eventIterator))
	{
		...
		if (!gslActivity_AddCalendarEventsForTimeRange(pEntity, pEventDef, pWarpDef, pEventContact, &pendingRequest, &testCalendar.eaEvents))
		{
			// The event was not added based on its calendar data. If it is active, we still want to add it to this list, though.
			if (gslEvent_IsActive(pEventDef))
			{
				// Add it manually (Without an associated time range)
				gslActivity_AddManualOnEvent(pEventDef, pEntity, pendingRequest.piTagsInclude, pendingRequest.piTagsExclude, &testCalendar.eaEvents);
			}
		}
	}
	...
	if (pEntity->pPlayer->pEventInfo)
	{
		...
		for (i = 0; i < eaSize(&g_ActiveEvents.ppDefs); i++)
		{
			...
			else if (!gslActivity_ShouldHideEvent(pEventDef))
			{
				...
				if (!gslActivity_AddCalendarEventsForTimeRange(pEntity,pEventDef,pWarpDef,pEventContact,&Request,&pEntity->pPlayer->pEventInfo->eaActiveEvents))
				{
					// This is a sort of a sad hack to deal with the underdesigned manually on/off stuff in conjunction with timed entries.
					// We need to get active events on the player to the client if they are manually on, even if they are not in the
					// requested time range as defined by their schedule. We know this event is On because it's in g_ActiveEvents.
					
					if (gslActivity_AddManualOnEvent(pEventDef, pEntity, Request.piTagsInclude, Request.piTagsExclude, &pEntity->pPlayer->pEventInfo->eaActiveEvents))
					{
						// We added or changed something
						bDirty=true;
					}
				}
				else
				{
					bDirty = true; // This is probably not the best. But we need to do it. Any newly active events need to set this
								   //   so that we update the client. Somehow we should actually determine if something updated?
				}
			}
		}
