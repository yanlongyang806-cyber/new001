...
bool GetCommandSimple(char *pName)
{
	char temp[16];
	if (ParseCommandOutOfCommandLine(pName, temp))
	{
		return !!atoi(temp);
	}
...
{
	if (GetCommandSimple("WebRequestServer"))
	{
		gGSLState.gbWebRequestServer = true;
		SetAppGlobalType(GLOBALTYPE_WEBREQUESTSERVER);
	}
	else if (GetCommandSimple("GatewayServer"))
	{
		gGSLState.gbGatewayServer = true;
		SetAppGlobalType(GLOBALTYPE_GATEWAYSERVER);
	}
	else if (GetCommandSimple("ServerBinner"))
	{
		gGSLState.gbServerBinner = true;
		SetAppGlobalType(GLOBALTYPE_SERVERBINNER);
	}
	else if(GetCommandSimple("BeaconMasterServer"))
	{
		SetAppGlobalType(GLOBALTYPE_BCNMASTERSERVER);
	}
	else if(GetCommandSimple("BeaconServer") ||
			GetCommandSimple("BeaconAutoServer") ||
			GetCommandSimple("BeaconRequestServer"))
	{
		SetAppGlobalType(GLOBALTYPE_BCNSUBSERVER);
	}
	else if(GetCommandSimple("BeaconClient"))
	{
		SetAppGlobalType(GLOBALTYPE_BCNCLIENTSENTRY);
	}
	else if(GetCommandSimple("SyntheticLogData"))
	{
		gGSLState.gbReplayServer = true;
	...
	if ( e->pPlayer->bStuckRespawn )
	{
		if ( !entCheckFlag(e, ENTITYFLAG_DEAD) && !e->pPlayer->iStasis )
		{
			if ( e->pPlayer->uiRespawnTime <= timeSecondsSince2000() )
			{
				...
				spawnpoint_MovePlayerToNearestSpawn(e, NULL, true, true);
	...
	GSL_PERFINFO_SECTION_START("CheckPlayers");
	{
		...
		while(pent = EntityIteratorGetNext(iter))
		{
			...
			if (entCheckFlag(pent, ENTITYFLAG_PLAYER_DISCONNECTED) && entCheckFlag(pent, ENTITYFLAG_PLAYER_LOGGING_IN))
			{
				...
				if(pent->pSaved && pent->pSaved->uGameSpecificFixupVersion != (U32)gameSpecificFixup_Version() && pent->pSaved->pPuppetMaster)
				{
					// version is different, we need all pets and puppets
			...
			if(pent->pSaved->uGameSpecificFixupVersion == (U32)gameSpecificFixup_Version() && !pent->pSaved->bSavedPetFixupInProgress)
			{
