...
U32 gDebugDependentMissions = 0;
AUTO_CMD_INT(gDebugDependentMissions, DebugDependentMissions);
...
void PlayerProject_ProcessPendingDonateAllStates(void);
void gslGroupProject_DependentMissionPeriodicCheck(void);
void
gslGroupProject_PerFrame(void)
{
    ...
    PlayerProject_ProcessPendingDonateAllStates();

    gslGroupProject_DependentMissionPeriodicCheck();
}
...
static EARRAY_OF(GroupProjectDependentMissions) s_DependentMissions = NULL;
static U32 s_LastDependentMissionCheckTime = 0;

void
gslGroupProject_DependentMissionPeriodicCheck(void)
{
    int i;
    GroupProjectDependentMissions *dependentMissions;
    Entity *playerEnt;
    GroupProjectContainer *projectContainer;

    // Only run once per second.
    if ( timeSecondsSince2000() == s_LastDependentMissionCheckTime )
    {
        return;
    }

    s_LastDependentMissionCheckTime = timeSecondsSince2000();

    for ( i = eaSize(&s_DependentMissions) - 1; i >= 0; i-- )
    {
        dependentMissions = s_DependentMissions[i];
        playerEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, dependentMissions->playerID);
        if ( playerEnt == NULL )
        {
            // Player has left the map, so remove them from the array.
            StructDestroy(parse_GroupProjectDependentMissions, dependentMissions);
            eaRemove(&s_DependentMissions, i);

            if ( gDebugDependentMissions )
            {
                printf("Player with dependent missions left map: playerID=%d\n", playerEnt->myContainerID);
            }
        ...
        if ( projectContainer )
        {
            ...
            // The group project container exists now, so we can poke all the dependent missions.
            for ( j = eaSize(&dependentMissions->missionRefs) - 1; j >= 0; j-- )
            {
                MissionInfo *pInfo = mission_GetInfoFromPlayer(playerEnt);
                const char *missionRef = dependentMissions->missionRefs[j];

                if ( gDebugDependentMissions )
                {
                    printf("Waking dependent mission: playerID=%d, missionRef=%s\n", playerEnt->myContainerID, missionRef);
                }

                if (pInfo) 
                {
                    Mission *pMission = mission_FindMissionFromRefString(pInfo, missionRef);
                    if (pMission) 
                    {
            ...
            // Now that the group project container has arrived and missions have been poked, we can remove this player from the array.
            StructDestroy(parse_GroupProjectDependentMissions, dependentMissions);
            eaRemove(&s_DependentMissions, i);
            continue;
        }
...
void
gslGroupProject_HandleDependentMission(Entity *playerEnt, const char *refString)
{
    GroupProjectContainer *projectContainer;
    GroupProjectDependentMissions *dependentMissions;

    // Make sure all the pointers are non-null.
    if ( playerEnt == NULL || playerEnt->pPlayer == NULL || refString == NULL )
    {
        return;
    }
    ...
    if ( projectContainer )
    {
        // If the project container exists, then we don't need to do anything.
        return;
    }

    if ( gDebugDependentMissions )
    {
        printf("Adding dependent mission: playerID=%d, refString=%s\n", playerEnt->myContainerID, refString);
    }

    dependentMissions = eaIndexedGetUsingInt(&s_DependentMissions, playerEnt->myContainerID);
    if ( dependentMissions == NULL )
    {
        // This player doesn't have any pre-existing dependent missions.

        // Enable indexing on the array of dependent missions if it has not been done yet.
        if ( s_DependentMissions == NULL )
        {
            eaIndexedEnable(&s_DependentMissions, parse_GroupProjectDependentMissions);
        }

        // Create and initialize a new dependent missions struct.
        dependentMissions = StructCreate(parse_GroupProjectDependentMissions);
        dependentMissions->playerID = playerEnt->myContainerID;
        eaPush(&dependentMissions->missionRefs, (char *)refString);
        dependentMissions->time = timeSecondsSince2000();

        // Add it to the array.
        eaIndexedAdd(&s_DependentMissions, dependentMissions);
    }
    else
    {
        // Add the mission to the record for this player.
        eaPushUnique(&dependentMissions->missionRefs, (char *)refString);
    }
}

typedef struct ClaimRewardData
{
	ContainerID eID;
	const char* pchDonationTaskDef;
} ClaimRewardData;
...
{
    ClaimRewardData* claimData = (ClaimRewardData*)userData;
    Entity* pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, claimData->eID);
    const char* feedbackKey = NULL;
    DonationTaskDef* taskDef = RefSystem_ReferentFromString("DonationTaskDef", claimData->pchDonationTaskDef);

    if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
    {
        pEnt = entForClientCmd(claimData->eID,pEnt);
    }

    if (returnVal->eOutcome == TRANSACTION_OUTCOME_FAILURE)
    {
        feedbackKey = "PlayerProject_CollectRewardsFailed";
    }
    else if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
    {
        feedbackKey = "PlayerProject_CollectRewardsSucceeded";
        if (pEnt && pEnt->pInventoryV2)
        {
            entity_SetDirtyBit(pEnt, parse_Inventory, pEnt->pInventoryV2, true);
        }
    }

    if (pEnt && feedbackKey)
    {
        char* feedbackString = NULL;
        entFormatGameMessageKey(pEnt, &feedbackString, feedbackKey, STRFMT_DISPLAYMESSAGE("ProjectName", taskDef->displayNameMsg), STRFMT_END);
        notify_NotifySend(pEnt, kNotifyType_GroupProjectCollectionFeedback, feedbackString, claimData->pchDonationTaskDef, NULL);
        estrDestroy(&feedbackString);
    }
...
void
gslGuildProject_ClaimRewardTable(Entity * pEnt, RewardTable * rewardTable, const char * projectName, DonationTaskDef * taskDef, int taskSlotNum) 
{
    ...
    claimData = calloc(1, sizeof(ClaimRewardData));
    claimData->eID = entGetContainerID(pEnt);
    claimData->pchDonationTaskDef = taskDef->name;

    pReturn = LoggedTransactions_CreateManagedReturnValEnt("PlayerProject_ClaimReward", pEnt, ClaimReward_CB, claimData);
