	...
	if (pEntity)
	{
		...
		if (pChatPlayer && pEntity->pPlayer && pEntity->pPlayer->pUI && pEntity->pPlayer->pUI->pChatState) {
			...
			switch (eType) {
				...
				case FRIEND_UPDATED:
					ChatPlayer_AddToList(&pChatState->eaFriends, pChatPlayer, iVirtualShardID, false);
					break;

				case FRIEND_OFFLINE:
				case FRIEND_ONLINE: // These ALWAY overwrites the pPlayerInfo 
					ChatPlayer_AddToList(&pChatState->eaFriends, pChatPlayer, iVirtualShardID, true);
					break;
	...
	if (pEntity)
	{
		...
		if (pChatPlayer && pEntity->pPlayer && pEntity->pPlayer->pUI && pEntity->pPlayer->pUI->pChatState) {
			...
			switch (eType) {
				case IGNORE_ADDED:
				case IGNORE_UPDATED:
					ChatPlayer_AddToList(&pChatState->eaIgnores, pChatPlayer, iVirtualShardID, true);
					break;
