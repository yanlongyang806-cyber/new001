...
ConvertNumeric_CB(TransactionReturnVal *pReturn, ConvertNumericCBData *pData)
{
	...
	{
		pEnt = entForClientCmd(pData->entID, pEnt);
	}
...
ShouldDoAutoConvert(Entity *pEnt, NumericConversionDef *conversionDef)
{
	...
	if ( conversionState != NULL )
	{
		...
		if (!NumericConversion_TimeIsInCurrentInterval(conversionState->lastAutoConversionTime, conversionDef))
		{
...
gslConvertNumeric(Entity *pEnt, const char *conversionName)
{
    ...
    if ( NumericConversion_QuantityRemaining(pEnt, &pEnt->pPlayer->eaNumericConversionStates, conversionDef) > 0 )
    {
        ...
        AutoTrans_trConvertNumeric(pReturn, GetAppGlobalType(), pEnt->myEntityType, pEnt->myContainerID, conversionName, &reason, pExtract);
    }
    else
    {
        NotifyError(pEnt);
...
trConvertNumeric(ATR_ARGS, NOCONST(Entity) *pEnt, const char *conversionName, const ItemChangeReason *pReason, GameAccountDataExtract* pExtract)
{
	...
	newInterval = !NumericConversion_TimeIsInCurrentInterval(conversionState->lastConversionTime, conversionDef);
...
AUTO_TRANSACTION
ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Pplayer.Eanumericconversionstates, .Hallegiance, .Hsuballegiance, pInventoryV2.ppLiteBags[], .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Pplayer.Eaplayernumericthresholds");
enumTransactionOutcome
trAutoConvertNumeric(ATR_ARGS, NOCONST(Entity) *pEnt, const char *conversionName, U32 maxIntervalsToConvert, const ItemChangeReason *pReason)
{
	...
	if (NumericConversion_TimeIsInCurrentInterval(conversionState->lastAutoConversionTime, conversionDef) ||
		NumericConversion_TimeIsInCurrentInterval(conversionState->lastConversionTime, conversionDef))
	{
		return TRANSACTION_OUTCOME_SUCCESS;
	}

	// compute the interval numbers for the current interval and the last interval where a conversion happened
	curIntervalNum = NumericConversion_IntervalNumOfTime(curTime, conversionDef);
	lastConversionIntervalNum = NumericConversion_IntervalNumOfTime(conversionState->lastConversionTime, conversionDef);
	lastAutoConversionIntervalNum = NumericConversion_IntervalNumOfTime(conversionState->lastAutoConversionTime, conversionDef);
	...
	if ( convertMax == 0 )
	{
		// nothing to convert, just set the time stamps and return
		conversionState->lastConversionTime = curTime;
	...
	//  now and the quantity to zero.  This will allow full conversion in the current interval.
	conversionState->lastConversionTime = curTime;
	conversionState->lastAutoConversionTime = curTime;
	conversionState->quantityConverted = 0;
