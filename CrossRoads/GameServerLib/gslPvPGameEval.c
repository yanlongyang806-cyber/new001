...
int exprPVPGame_playersAliveInGroup(ACMD_EXPR_PARTITION partitionIdx, int iGroupNumber)
{
	...
	if(iGroupNumber < eaSize(&pInfo->pMatch->eaGroups) && iGroupNumber >= 0)
	{
		...
		for(i=0;i<eaSize(&pInfo->pMatch->eaGroups[iGroupNumber]->eaMatchMembers);i++)
		{
			Entity *pEnt = entFromContainerID(pInfo->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pInfo->pMatch->eaGroups[iGroupNumber]->eaMatchMembers[i]->uEntID);
...
void exprPVPGame_addPointsToTeam(ACMD_EXPR_PARTITION partitionIdx, int iTeamNumber, int iPoints, const char *pchMessageKey)
{
	...
	if(pInfo && pInfo->pMatch && iTeamNumber > 0 && iTeamNumber <= eaSize(&pInfo->pMatch->eaGroups))
	{
		...
		for(i=0;i<eaSize(&pInfo->pMatch->eaGroups[iTeamNumber-1]->eaMatchMembers);i++)
		{
			Entity *pMember = entFromContainerID(pInfo->iPartitionIdx, GLOBALTYPE_ENTITYPLAYER,pInfo->pMatch->eaGroups[iTeamNumber-1]->eaMatchMembers[i]->uEntID);
