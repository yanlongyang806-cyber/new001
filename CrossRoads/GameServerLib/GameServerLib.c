...
#define STRING_CACHE_NUM_STRINGS		((2048+512+256)*1024)	
...
void gslPlayerLoggedIn(ClientLink* clientLink, Entity* e, S32 noTimeout, int locale)
{
	...
	if (e->pPlayer->bIsFirstSessionLogin)
	{
		...
		// subscribe to the shared bank in order to fix it up. Only do this once per session.
		if(gConf.bSubscribeToSharedBank)
		{
			char idBuf[128];
			SET_HANDLE_FROM_STRING(GlobalTypeToCopyDictionaryName(GLOBALTYPE_ENTITYSHAREDBANK), ContainerIDToString(e->pPlayer->accountID, idBuf), e->pPlayer->hSharedBank);
		}
...
void gslEntityFrameUpdate(Entity* pEnt)
{
	...
	if(ent_mat[3][1] <= 50.f - MAX_PLAYABLE_COORDINATE){
		if (pEnt && pEnt->pPlayer) {
			spawnpoint_MovePlayerToStartSpawn(pEnt, NULL, false);
		} else {
	...
	gslCritter_UpdateRollovers(pEnt);
	PERFINFO_AUTO_STOP();
...
int gslGetPartitionInactivityTimeout(void)
{
	...
	if ((isProductionMode() || s_bSimulateProductionMapLifecycle) && !GslIsDoingLoginServerOrTransServerStressTest())
	{	
...
const U32 addictionReminderTimes[] = {0,65,125,185,305,605,905};
#define ADDICTION_REMINDER_COUNT 7

void AddictionPlaySessionNotifyPlayer(Entity *playerEnt)
{
	//Should we update?
	Player* pPlayer = playerEnt->pPlayer;
	U32 timeLeft = 0;
	S32* reminderIdx = &(pPlayer->addictionReminderTimeIdx);
	char *pch = NULL;

	if(timeSecondsSince2000() < pPlayer->addictionPlaySessionEndTime) {
		timeLeft = pPlayer->addictionPlaySessionEndTime - timeSecondsSince2000();
	}

	//If our reminder index is -1 then set it to something reasonable
	if(*reminderIdx == -1)
	{
		//Calculation is: find the next half hour interval that we need to notify someone for.
		//The five is used to do the notification shortly before the actual minute, so we send the notification at
		//120 minutes instead of 119, for example
		*reminderIdx = (timeLeft-5) / 1800 + ADDICTION_REMINDER_COUNT - 1;
	}

	//If our time left is greater than 15 mins, just notify on the half hour
	if(*reminderIdx >= ADDICTION_REMINDER_COUNT)
	{
		//This is the last notification we needed
		S32 targetIdx = (timeLeft-5) / 1800 + ADDICTION_REMINDER_COUNT - 1;

		if(targetIdx < (*reminderIdx))
		{
			(*reminderIdx) = targetIdx;
			entFormatGameMessageKey(playerEnt, &pch, "Addiction_Reminder",
				STRFMT_TIMER("PlayTimeRemaining", playerEnt->pPlayer->addictionPlaySessionEndTime - timeSecondsSince2000()),
				STRFMT_END);
			notify_NotifySend(playerEnt, kNotifyType_AddictionReminder, pch, NULL, NULL);
		}
	}
	else if(timeLeft < addictionReminderTimes[*reminderIdx])
	{
		//Go through until we have a reasonable time
		while(addictionReminderTimes[*reminderIdx] > timeLeft &&
			addictionReminderTimes[*reminderIdx])
		{
			(*reminderIdx)--;
		}

		entFormatGameMessageKey(playerEnt, &pch, "Addiction_Reminder",
			STRFMT_TIMER("PlayTimeRemaining", timeLeft),
			STRFMT_END);
		notify_NotifySend(playerEnt, kNotifyType_AddictionReminder, pch, NULL, NULL);
	}
}
...
void gslPeriodicUpdate(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	...
	PERFINFO_AUTO_START("EntCounting", 1);
	{
		...
		while(pent = EntityIteratorGetNext(iter))
		{
			...
			if (pent->pPlayer)
			{
				...
				//Give the player a heads up on when they'll be booted
				if ( pent->pPlayer->addictionPlaySessionEndTime )
				{
					AddictionPlaySessionNotifyPlayer(pent);
				}
	...
	//Also note that any discussion of this topic needs to be aware of TestPartitionForImmediateDeath()
