...
// Determine if there is a team member of pEnt on the same map as is in mapDesc
static bool NonTeamTransferHasTeamMemberOnDestinationMap(Entity *pEnt, 	MapDescription *destMap)
{
	//   Use just the map name to determine if the destination map matches. This may be too weak, but
	//   should be okay.
	
	if ( team_IsMember(pEnt) && ( destMap != NULL ) && destMap->pZoneMapInfo!=NULL )
	{
		Team *team = team_GetTeam(pEnt);
		if ( team != NULL )
		{
			...
			for ( i = 0; i < n; i++ )
			{
				if (zmapInfoGetPublicName(destMap->pZoneMapInfo)==team->eaMembers[i]->pcMapName)
				{
					return(true);
				}
			}
...
{	
	...
	case TRANSACTION_OUTCOME_SUCCESS:
		...
		if (transfer->eFlags & TRANSFERFLAG_ONLY_EXISTING_MAPS)
		{
...
static bool s_bOverSoftCapAllowedWhenNonTeamTransferHasTeamMemberOnDestinationMap = true;
AUTO_CMD_INT(s_bOverSoftCapAllowedWhenNonTeamTransferHasTeamMemberOnDestinationMap, MapTransferSoftCapFix) ACMD_AUTO_SETTING(Misc, GAMESERVER);
bool gslMapTransfer_OverSoftCapAllowedWhenNonTeamTransferHasTeamMemberOnDestinationMap()
{
	return s_bOverSoftCapAllowedWhenNonTeamTransferHasTeamMemberOnDestinationMap;
}
...
static void MapTransferSelectingMap_Enter(InstancedMachineHandleOrName pStateMachineHandle, CharacterTransfer *transfer, F32 fElapsed)
{
	...
	// My current understanding is that TeamTransfers (in this context) are used for any case where we know we are moving to a same destination
	//   as another team member who transferred there recently from the map we are on. We will leave this code intact as written.
	// I am adding a new case if we are NOT team transfering to cover for generic moving to the same map that a team member happens to already
	//   be on. In that case, do not treat it as a team transfer, but make sure we use the SoftCapBypass to determine eligible maps, because
	//   we want to move to that team member's destination if we aren't at hard cap. If we don't do this, we have issues with team mates
	//   being unable to meet up if we are over the soft cap but below the hard cap. WOLF[2May13]
	...
	{
		// Add a kill switch in case something goes wrong with this new code since there is not time for testing.
		//  Theoretically we can remove this after it's been on live for a bit. WOLF[6May2013]
		if (gslMapTransfer_OverSoftCapAllowedWhenNonTeamTransferHasTeamMemberOnDestinationMap())
		{
			// Check for team members on the destination map. Allow over soft cap for possible destination maps.
			//   Use just the map name to determine if the destination map matches. This may be too weak, but
			//   should be okay.
			if (NonTeamTransferHasTeamMemberOnDestinationMap(ent, destMap))
			{
				transfer->pMapSearchInfo->overSoftCapOK = true;
			}
		}
	...
	if(pEnt && pEnt->pPlayer && pEnt->pPlayer->pMapMoveConfirm && !pEnt->pPlayer->pMapMoveConfirm->bConfirmed)
	{
			...
			(!pConfirm->uiTimeToConfirm || pConfirm->uiTimeStart + pConfirm->uiTimeToConfirm >= timeServerSecondsSince2000()))
		{
			...
			else
			{
				...
				{
					spawnpoint_MovePlayerToNamedSpawn(pEnt, pConfirm->pWarp->pchSpawn, NULL, NULL, 0);
				}
...
void MapMoveOrSpawnWithDescription(SA_PARAM_NN_VALID Entity *ent, SA_PARAM_NN_VALID MapDescription *pMapDesc, char *pReason, TransferFlags eFlags)
{
	...
	{
		spawnpoint_MovePlayerToNamedSpawn(ent, pMapDesc->spawnPoint, NULL, NULL, true);
	}
}
