...
void gslAP_WebCStorePurchase (Entity *pEnt, GameAccountData *pAccountData, PurchaseRequestData *pCBStruct)
{
	...
	else if ((pCBStruct->eErrorType = microtrans_GetCanPurchaseError(WEBREQUEST_PARTITION_INDEX, pEnt, pAccountData, pDef, entGetLanguage(pEnt), NULL)) != kMicroPurchaseErrorType_None)
	{
		gslMTPurchase_SetStatus(pCBStruct, PURCHASE_FAILED);
		pCBStruct->pError = strdup(WEBPURCHASE_ERROR_RESTRICTED);
...
PurchaseRequestStatus gslAP_GetPurchaseStatus(U32 uAccountID, U32 uRequestID, char **pOutError, MicroPurchaseErrorType *peError)
{
	...
	if(peError)
		*peError = kMicroPurchaseErrorType_Unknown;
