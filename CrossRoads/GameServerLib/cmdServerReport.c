...
static void CostumeAssetScan(ReportCostumeInfo ***peaInfos)
{
	...
	for(i=eaSize(&pContacts->ppReferents)-1; i>=0; --i) {
		...
		for(j=eaSize(&pContact->specialDialog)-1; j>=0; --j) {
			...
			for(l=eaSize(&pBlock->extraCostumes)-1; l>=0; --l) {
				pCostume = GET_REF(pBlock->extraCostumes[l]->costumeOverride);
				for(k=eaSize(peaInfos)-1; k>=0; --k) {
					if ((*peaInfos)[k]->pCostume == pCostume) {
						eaPush(&(*peaInfos)[k]->eaContacts, pContact);
						break;
	...
	for(i=eaSize(&pSuperPets->ppReferents)-1; i>=0; --i) {
		...
		for(j=eaSize(&pDef->eaCostumes)-1; j>=0; --j) {
			PlayerCostume *pCostume = GET_REF(pDef->eaCostumes[j]->hCostume);
			if (pCostume) {
				for(k=eaSize(peaInfos)-1; k>=0; --k) {
					if ((*peaInfos)[k]->pCostume == pCostume) {
...
void report_DumpMissionOfferText(ContactDef* pContactDef, ContactMissionOffer* pOffer, FILE *file)
{
	...
	if(pMissionDef)
	{
		...
		pMessage = GET_REF(pMissionDef->detailStringMsg.msg.hMessage);
		...
		{
			pMessage = eaSize(&pOffer->offerDialog) ? GET_REF(pOffer->offerDialog[0]->displayTextMesg.msg.hMessage) : NULL;
			if(pMessage)
				fprintf(file, "    Contact Offer: %s\n", pMessage ? pMessage->pcDefaultString : "*UNKNOWN*");
			pMessage = eaSize(&pOffer->failureDialog) ? GET_REF(pOffer->failureDialog[0]->displayTextMesg.msg.hMessage) : NULL;
			if(pMessage)
				fprintf(file, "    Contact Failure: %s\n", pMessage ? pMessage->pcDefaultString : "*UNKNOWN*");
		}
		pMessage = eaSize(&pOffer->inProgressDialog) ? GET_REF(pOffer->inProgressDialog[0]->displayTextMesg.msg.hMessage) : NULL;
		...
		{
			pMessage = eaSize(&pOffer->completedDialog) ? GET_REF(pOffer->completedDialog[0]->displayTextMesg.msg.hMessage) : NULL;
...
char* ReportDumpMissionTextFiltered(const char* nameFilter, const char* scopeFilter)
{
	...
	for(i=0; i < eaSize(&pContactDefs->ppReferents); i++)
	{
		...
		pMessage = eaSize(&pDef->greetingDialog) ? GET_REF(pDef->greetingDialog[0]->displayTextMesg.msg.hMessage) : NULL;
		if(pMessage)
			fprintf(file, "Greeting: %s\n", pMessage ? pMessage->pcDefaultString : "*NONE*");

		pMessage = eaSize(&pDef->missionListDialog) ? GET_REF(pDef->missionListDialog[0]->displayTextMesg.msg.hMessage) : NULL;
		if(pMessage)
			fprintf(file, "Mission List: %s\n", pMessage ? pMessage->pcDefaultString : "*NONE*");

		pMessage = eaSize(&pDef->noMissionsDialog) ? GET_REF(pDef->noMissionsDialog[0]->displayTextMesg.msg.hMessage) : NULL;
		if(pMessage)
			fprintf(file, "No Missions: %s\n", pMessage ? pMessage->pcDefaultString : "*NONE*");

		pMessage = eaSize(&pDef->exitDialog) ? GET_REF(pDef->exitDialog[0]->displayTextMesg.msg.hMessage) : NULL;
		if(pMessage)
		...
		for(k=0; k<eaSize(&pDef->specialDialog); k++)
		{
			...
			pMessage = pDef->specialDialog[k]->dialogBlock[0] ? GET_REF(pDef->specialDialog[k]->dialogBlock[0]->displayTextMesg.msg.hMessage) : NULL;
...
void ReportAudioAssets_ProcessOnServer(EntityRef clientEntityRef)
{
	...
	pComponent = StructCreate(parse_AudioAssetComponents);
	gslVOGetAudioAssets(&pComponent->pcType, &pComponent->eaStrings, &pComponent->uiNumData, &pComponent->uiNumDataWithAudio);
	eaPush(&serverAudioAssets.eaComponents, pComponent);
