...
static StashTable sCachedAutoCompletionNamesByCmdName = NULL;
...
void RequestAutoCompletionArgNamesFromServer(Entity *pClientEntity, ACMD_SENTENCE pCommandName)
{
	...
	if (pCmd->flags & CMDF_CACHE_AUTOCOMPLETE)
	{
		bDoCaching = true;
	}
	...
	if (bDoCaching)
	{
		if (stashFindPointer(sCachedAutoCompletionNamesByCmdName, pCommandName, &pNames))
		{
	...
	for (i=0; i < CMDMAXARGS; i++)
	{
		if (pCmd->data[i].eNameListType != NAMELISTTYPE_NONE)
		{
			...
			if (pNameList)
			{
				...
				{
					eaPush(&pNamesSingleArg->ppNames, bDoCaching ? (char*)pNameFromList : strdup(pNameFromList));
				}
	...
	if (bDoCaching)
	{
		if (!sCachedAutoCompletionNamesByCmdName)
		{
			sCachedAutoCompletionNamesByCmdName = stashTableCreateWithStringKeys(16, StashDeepCopyKeys_NeverRelease);
		}

		stashAddPointer(sCachedAutoCompletionNamesByCmdName, pCommandName, pNames, false);
...
// Sets the flag that indicates that the flight parameters have been updated
AUTO_COMMAND ACMD_SERVERCMD ACMD_PRIVATE ACMD_CATEGORY(Movement) ACMD_ACCESSLEVEL(0);
void UpdateFlightParams(Entity* e)
{
	if(e && e->pChar)
	{
		e->pChar->bUpdateFlightParams = true;
...
void VerifyCommandForGclAutoComplete(char *pCommandName, int iAccessLevel)
{
	...
	//this is only called from VerifyCommand on the client, which should only be called in dev mode, and all this ever does
	//is generate errorfs, so we make it AL 0 but then only work in dev mode.
	if (!isDevelopmentMode())
	{
		return;
	}

	pCmd = cmdListFind(&gGlobalCmdList, pCommandName);
	if (pCmd)
	{
