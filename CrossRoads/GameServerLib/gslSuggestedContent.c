	...
	SA_PARAM_NN_VALID SuggestedContentInfo ***peaContentInfo)
{
	...
	if (pBestEventDef!=NULL || pBestQueueDef!=NULL)
	{
		...
		if (pBestQueueDef!=NULL)
		{
			...
			{
				pSuggestedContentInfo->strDisplayName = StructAllocString(entTranslateDisplayMessage(pEntity, pBestQueueDef->displayNameMesg));
				pSuggestedContentInfo->strSummary = StructAllocString(entTranslateDisplayMessage(pEntity, pBestQueueDef->descriptionMesg));
		...
		if (pBestEventDef!=NULL)
		{
			...
			{
				pSuggestedContentInfo->strDisplayName = StructAllocString(entTranslateDisplayMessage(pEntity, pBestEventDef->msgDisplayName));
				pSuggestedContentInfo->strSummary = StructAllocString(entTranslateDisplayMessage(pEntity, pBestEventDef->msgDisplayLongDesc));
				...
				if (pCurrentNodeDef->pMissionGroupInfo!=NULL)
				{
					for (i = 0; i < eaSize(&pCurrentNodeDef->pMissionGroupInfo->eaMissions); i++)
					{
						...
						if (pCompletedMission==NULL)
						{
							...
							if (pMissionDef!=NULL)
							{
								pSuggestedContentInfo->strDisplayName = StructAllocString(entTranslateDisplayMessage(pEntity, pMissionDef->displayNameMsg));
							}
						
							pSuggestedContentInfo->strSummary = StructAllocString(entTranslateDisplayMessage(pEntity, pProgMission->msgDescription));
					...
					// If no mission match was found, try to find a ZoneRewards
					if( pCurrentNodeDef->pMissionGroupInfo->pZoneRewardsDef )
					{
						ZoneRewardsDef *pZoneRewardsDef = ZoneRewards_GetRewardsDef(pCurrentNodeDef->pMissionGroupInfo->pZoneRewardsDef->pchZoneRewardsDef);
						// We have something
						SuggestedContentInfo *pSuggestedContentInfo = StructCreate(parse_SuggestedContentInfo);
						...
						pSuggestedContentInfo->eType = SuggestedContentType_ZoneRewards;
						pSuggestedContentInfo->strListName = allocAddString(pchListName);
						if( pZoneRewardsDef )
						{
							pSuggestedContentInfo->pchStoryName = pZoneRewardsDef->pchName;
							pSuggestedContentInfo->strDisplayName = entTranslateDisplayMessage(pEntity, pZoneRewardsDef->msgShortName);
						}
						pSuggestedContentInfo->strSummary = entTranslateDisplayMessage(pEntity, pCurrentNodeDef->pMissionGroupInfo->pZoneRewardsDef->msgDescription);
						pSuggestedContentInfo->pchArtFileName = pCurrentNodeDef->pMissionGroupInfo->pZoneRewardsDef->pchImage;
						return;
					}
