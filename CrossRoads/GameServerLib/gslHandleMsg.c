...
// callback for login fixup when resetting powerIDs
static void ResetPowerIDs_CB(TransactionReturnVal *pVal, void *userData)
{
	Entity *pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, (ContainerID)(intptr_t)userData);
	if(pEnt && pEnt->pPlayer)
	{
		HandlePlayerLogin_Success(pEnt, kLoginSuccess_PowerIDFixup);
	}
}
...
void HandlePlayerLogin_Success(Entity *pEntity, PlayerLoginWait eWaitSucceeded)
{
	...
	// STO rebuild costume as this is the first place that we have our subscribed ship which is needed for the costume
	if(pEntity->pSaved->pPuppetMaster)
	{
		costumeEntity_ResetCostumeData(pEntity);
		GameSpecific_CaptainCostumeFixup(pEntity);

		// make sure pet names are available to the client
		SavePet_SetPetNames(pEntity);
	}

	if(ent_NeedsProjectSpecificPowersFixup(pEntity))
	{
		ent_FixupProjectSpecificPowers(pEntity);
	}
	...
	if(inv_NeedsBagFixup(pEntity))
	{
		...
		inv_FixupBags(pEntity, FixupBags_CB, (void*)(intptr_t)entGetContainerID(pEntity));
		
		PERFINFO_AUTO_STOP();
		return;
	}
	
	if (gslEntity_ResetDupePowerIDs(pEntity, ResetPowerIDs_CB, (void*)(intptr_t)entGetContainerID(pEntity)))
	{
		pEntity->pPlayer->eLoginWaiting |= kLoginSuccess_PowerIDFixup;
...
void HandlePlayerLogin(Packet *pak, ClientLink *clientLink)
{
	...
	if(!isDevelopmentMode() && !g_isContinuousBuilder)
	{
		if(clientLink->accessLevel < ACCESS_GM && !langIsSupportedThisShard((Language)clientLink->clientLangID))
		{
			log_printf(LOG_LOGIN, "Client %s sent login packet with an unsupported language", gslGetAccountNameForLink(clientLink));
			gslSendLoginFailure(clientLink, langTranslateMessageKey(LANGUAGE_ENGLISH, "Login2_UnsupportedLanguage"));
			return;
		}
	}
	...
	while (pEnt = EntityIteratorGetNext(iter))
	{
		if (pEnt->pPlayer && !pEnt->pPlayer->clientLink && pEnt->pPlayer->loginCookie == cookie && entGetContainerID(pEnt) == iEntContainerID)
		{
			...
			// Verify that the player is allowed to login to this map, and break immediately if it is not
			bUGCMismatch = (entGetVirtualShardID(pEnt) != partition_VirtualShardIDFromIdx(entGetPartitionIdx(pEnt)));
			if (bUGCMismatch)
			{
				break;
			}
	...
	else if (bUGCMismatch)
	{
		// This entity is either a UGC character trying to login to a non-UGC map, or a non-UGC
		// character trying to login to a UGC map. Either way, fail login and send out an alert
		gslSendLoginFailure(clientLink,"Entity not permitted on this map!");
		if (entity_IsUGCCharacter(pEnt))
		{
			TriggerAlertf("LOGIN_UGCENTITY_MAP_MISMATCH", ALERTLEVEL_WARNING, ALERTCATEGORY_SECURITY, 0, GLOBALTYPE_ENTITYPLAYER, pEnt->myContainerID,
				GetAppGlobalType(), GetAppGlobalID(), getHostName(), 0, "UGC Entity attempted to login to non-UGC map! "
				"This is not permitted, and is a likely indication of someone attempting to exploit.");
		}
		else
		{
			TriggerAlertf("LOGIN_ENTITY_MAP_MISMATCH", ALERTLEVEL_WARNING, ALERTCATEGORY_SECURITY, 0, GLOBALTYPE_ENTITYPLAYER, pEnt->myContainerID,
				GetAppGlobalType(), GetAppGlobalID(), getHostName(), 0, "Non-UGC Entity attempted to login to a UGC map! "
				"This is not permitted, and is a likely indication of someone attempting to exploit.");
		}
	}
	else
...
void HandleDoneLoading_Entity(Entity *pEnt)
{
	...
	if (pEnt->pPlayer) 
	{
		...
		Entity_SuperCritterPetFixup(pEnt);

	}

	gslDoAutoNumericConversion(pEnt);
		...
		PERFINFO_AUTO_START("GAMECLIENTLIB_CMD_PRIVATE", 1);
			{
				...
				{			
					...
					if (bUnknownCommand)
					{
						if (ent && cmdExistsButIsAccessLevelOverridden(&gPrivateCmdList, pString, GetAccessLevelForGSLCmdParse(ent), entGetLanguage(ent)))
						{
							//special case where we used cmdAccessLevelOverride to stop end users from calling a particular GS command
							//Normally, getting an unrecognized private command means hacking or corruption, in this case it does not,
							//so we do nothing
						}
						else
						{
							...
							estrPrintf(&pTemp, "Unrecognized private server command %s", pString);
