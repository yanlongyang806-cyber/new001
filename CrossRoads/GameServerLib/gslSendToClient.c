...
debugLogStuckClient(ClientLink * clientLink, NetLink *link, Entity *pEnt, S32 assertOnSolo)
{
	...
	estrConcatf( &logString, "Client link stopped sending 3 minutes ago. clientLoggedIn=%d, primaryEntityID=%d, entityFlags=0x%x, clientDisconnected=%d, clientAddr=%s, eLoginWaiting=0x%x, sendBufferFull=%d, puppetCheckPassed=%d, skippedSuccessOnLogin=%d, missingPuppets=%s, ReadforGeneralUpdate=%d, debugStopUpdateTime=%d, uClientPatchTm=%d",
		...
		clientLink->readyForGeneralUpdates, clientLink->debugStopUpdateTime, clientLink->uClientPatchTm);

	log_printf(LOG_CLIENTSERVERCOMM, "%s", logString);
	if ( transferringOut )
	{
		ErrorDetailsf("%s", logString);
		Errorf("Player has been waiting 4 minutes to transfer out of map");
	}
...
static void gslQueueGeneralUpdateForLink(NetLink *link,ClientLink *clientLink)
{
		...
		clientLink->disconnected)
	{
		if(!clientLink->debugStopUpdateTime){
			...
			max(clientLink->uClientPatchTm, clientLink->debugStopUpdateTime) + 240 < curTime)
		{
	...
	if(linkSendBufFull(link)){
		...
		else if ( ( clientLink->debugStopUpdateTime != U32_MAX ) && ( max(clientLink->uClientPatchTm, clientLink->debugStopUpdateTime) + 240 < curTime ) )
		{
			clientLink->debugStopUpdateTime = U32_MAX;
			debugLogStuckClient(clientLink, link, primaryEntity, 1);
		}
