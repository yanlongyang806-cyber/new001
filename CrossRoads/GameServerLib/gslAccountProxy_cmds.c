...
void gslAPCmdSuspendFlagSet(SA_PARAM_NN_VALID Entity *pEntity, F32 hoursToSuspend, CmdContext *pCmdContext)
{
	...
	if(iVal < 0)
	{
		iVal = 0;	// no longer suspended as key of 0 or greater == suspension
	}
	else if(iVal > 0)
	{
...
void gslAPCmdPurchaseProductEx(SA_PARAM_NN_VALID Entity *pEntity, SA_PARAM_NN_STR const char *pCategory, U32 uProductID, SA_PARAM_OP_VALID const Money *pExpectedPrice, SA_PARAM_OP_STR const char *pPaymentMethodVID, U64 uItemId, bool bUseItemAfterPurchase)
{
	...
	if(pEntity && pEntity->pPlayer)
	{
		...
		if(pMTCategory)
		{
			if(pEntity->pPlayer->accountID && uProductID)
			{
				...
				APPurchaseProduct(pEntity, entGetAccountID(pEntity), entGetLanguage(pEntity), pcBuffer, uProductID, pExpectedPrice, uItemDiscount, pPaymentMethodVID, pcIP, uItemId, bUseItemAfterPurchase);
		...
		{
			APPurchaseProduct(pEntity, entGetAccountID(pEntity), entGetLanguage(pEntity), pCategory, uProductID, pExpectedPrice, uItemDiscount, pPaymentMethodVID, pcIP, uItemId, bUseItemAfterPurchase);
			notify_NotifySend(pEntity, kNotifyType_MicroTrans_PointBuyPending, langTranslateMessageKey(entGetLanguage(pEntity), "MicroTrans_Purchase_Pending"), NULL, NULL);
		}
	}
}
...
{
	gslAPCmdPurchaseProductEx(pEntity, pCategory, uProductID, pExpectedPrice, pPaymentMethodVID, 0, false);
}
