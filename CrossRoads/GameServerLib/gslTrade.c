...
void trade_AddSavedPet(SA_PARAM_NN_VALID Entity *pEnt, ContainerID srcPetID, S32 iTargetSlot)
{
	...
	//Verify we aren't in escrow
	if(gConf.bEnableEscrowTrades && pEnt->pPlayer->pTradeBag->escrowed)
		return;
...
void trade_Cancel(SA_PARAM_NN_VALID Entity* pPlayerEnt)
{
	...
	if (pTargetEnt && pTargetEnt->pPlayer && pTargetEnt->pPlayer->pTradeBag && pTargetEnt->pPlayer->erTradePartner == erPlayerEntRef) {
		...
		pTargetEnt->pPlayer->pTradeBag->escrowed = false;
...
void trade_Accept(SA_PARAM_NN_VALID Entity* pPlayerEnt)
{
	...
	EntityRef	erTargetEntRef = pPlayerEnt->pPlayer->erTradePartner;
	Entity*		pTargetEnt = entFromEntityRefAnyPartition(erTargetEntRef);
	TradeBag*	pPlayerTradeBag = pPlayerEnt->pPlayer->pTradeBag;
	
	if (!pPlayerEnt->pPlayer || !pPlayerTradeBag|| !pTargetEnt || !pTargetEnt->pPlayer || !pTargetEnt->pPlayer->pTradeBag) {
		return;
	}
	...
	if (pTargetEnt->pPlayer->pTradeBag->finished) {
		...
		//if we haven't entered escrow mode, do that now (if the global flag is set)
		if(gConf.bEnableEscrowTrades && !pPlayerTradeBag->escrowed)
		{
			//Set up escrow
			pPlayerTradeBag->escrowed = true;
			pTargetEnt->pPlayer->pTradeBag->escrowed = true;
			
			//Use that same finished flag to determine if the escrow is complete
			pPlayerTradeBag->finished = false;
			pTargetEnt->pPlayer->pTradeBag->finished = false;
			
			//Make sure we set the other player as dirty
			entity_SetDirtyBit(pTargetEnt, parse_Player, pTargetEnt->pPlayer, false);

			//Client callbacks
			ClientCmd_trade_ReceiveTradeEscrow(pPlayerEnt);
			ClientCmd_trade_ReceiveTradeEscrow(pTargetEnt);
		}
		//otherwise finish the trade
		else
		{
...
void trade_ClearAcceptance(SA_PARAM_NN_VALID Entity* pPlayerEnt)
{
	...
	if(gConf.bEnableEscrowTrades) {
		pPlayerEnt->pPlayer->pTradeBag->escrowed = false;
	}
