...
typedef struct RemovePowerPersonalCBData
{
	int iPartitionIdx;
	EntityRef erRef;
} RemovePowerPersonalCBData;

static void RemovePowerPersonal_CB(TransactionReturnVal *pReturnVal, RemovePowerPersonalCBData* pData)
{
	if (pData)
	{
...
{
	RemovePowerPersonalCBData *pRemovePowerCBData = calloc(1, sizeof(RemovePowerPersonalCBData));
	Power *ppow = NULL;
	bool bRequiresTransaction = false;
	int iResult;

	// We set expiration pending to true so that the power is not expired again.
	// We do not ever need to unset this, because the power is being deleted.
	ppow = powers_IndexedFindPowerByID(&pchar->ppPowersPersonal, uiID);
	if (ppow)
	{
		ppow->bExpirationPending = true;
	}

	pRemovePowerCBData->iPartitionIdx = entGetPartitionIdx(pchar->pEntParent);
	pRemovePowerCBData->erRef = entGetRef(pchar->pEntParent);

	iResult = character_RemovePowerPersonalEx(pchar, uiID, &bRequiresTransaction, RemovePowerPersonal_CB, pRemovePowerCBData);

	if (!bRequiresTransaction)
	{
		SAFE_FREE(pRemovePowerCBData);
	}
...
void character_PowerExpire(Character *pchar, U32 uiID, GameAccountDataExtract *pExtract)
{
	...
	if(ppow = powers_IndexedFindPowerByID(&pchar->ppPowersPersonal,uiID))
	{
		...
		character_RemovePowerPersonalEx(pchar,uiID,&bRequiresTransaction,RemovePowerPersonal_CB,pRemovePowerCBData);
				 ...
				 (SAFE_MEMBER(pDef, flags) & kItemDefFlag_ExpireOnAnyPower ? item_AnyPowersExpired(pItem, true) : item_AllPowersExpired(pItem, true)) )
			{
				...
				if (pBag && pDef && pBagDef && pBagDef->bUseItemsInInventoryFirst && !bLifetimeExpire)
				{
					...
					if (eInvBag != InvBagIDs_None && iSlot >= 0)
					{
						...
						{
							invbag_RemoveItem(ATR_EMPTY_ARGS,(NOCONST(Entity)*)pchar->pEntParent,true,eInvBag,iSlot,iRemoveCount,false,NULL,NULL);
				...
				if (!bFound) //call transaction to remove the item
				{
					if(entGetType(pchar->pEntParent)==GLOBALTYPE_ENTITYCRITTER)
					{
						invbag_RemoveItem(ATR_EMPTY_ARGS,(NOCONST(Entity)*)pchar->pEntParent,true,BagID,SlotIdx,iRemoveCount,false,NULL, NULL);
						character_ResetPowersArray(entGetPartitionIdx(pchar->pEntParent), pchar, NULL);
					}
					else
					{
...
// resets the given list of powerIDs as specified in PowerIDList
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pchar.Pppowerspersonal, .Pchar.Pppowersclass, .Pchar.Pppowersspecies, .Pchar.Pppowertrees, .Pinventoryv2.Ppinventorybags, .Psaved.Ppallowedcritterpets, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Curtempid, pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
enumTransactionOutcome trEntity_PowersListResetIDs(	ATR_ARGS,
													NOCONST(Entity)* pEnt,
													PowerIDList *pPowerIDs,
													GameAccountDataExtract *pExtract)
{
	if (pPowerIDs && eaiSize(&pPowerIDs->eaiPowerIDs))
	{
		entity_ResetListedPowerIDsHelper(ATR_PASS_ARGS, pEnt, pPowerIDs, pExtract);
		return TRANSACTION_OUTCOME_SUCCESS;
	}
	else
	{
