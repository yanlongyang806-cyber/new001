...
#include "cmdServerSyntheticLogs.h"
...
// The following was increased from 120 MB to 380 MB on July 10, 2013 due to NW UGC memory needs.
// This was after jfinder fixed the most gross memory leaks.
// I just this number because this it will take all current alert #1 and alert #2 and make them go away.
// This brings our editing GameServer memleak alert count go from a few per hour to a few per week.
// andrewa [NNO-16425]
#define UGC_EDIT_MEMORY_ALLOWANCE      380 * 1024 * 1024
...
void gslInit_BeginFrame(void)
{
	...
	if (  gServerLibState.writeSchemasAndExit || gbMakeBinsAndExit || gpcCalcDepsAndExit
		  || gServerLibState.fixupAllMapsAndExit || gbMakeVOTXTFilesAndExit
		  || gServerLibState.validateUGCProjects[0])
	{
...
void gslMapManagerHandshake_BeginFrame(void)
{
	...
	else if (gGSLState.gbReplayServer)
	{
		// ReplayServers always load "emptymap" and create a single partition
		MapPartitionSummary summary = {0};
		gGSLState.gameServerDescription.baseMapDescription.mapDescription = allocAddString("emptymap");

		StructInit(parse_MapPartitionSummary, &summary);
		summary.eOwnerType = GLOBALTYPE_GAMESERVER;
		summary.pMapVariables = StructAllocString("");
		partition_CreateAndInit(&summary, "Default ReplayServer partition");
		StructDeInit(parse_MapPartitionSummary, &summary);
	}
	else
	{
	...
	if (!objLocalManager() || gGSLState.gbWebRequestServer || gGSLState.gbGatewayServer)
	{
		...
		RemoteCommand_aslMapManagerUpdateActivites(GLOBALTYPE_MAPMANAGER,0,GLOBALTYPE_GATEWAYSERVER,gGSLState.gameServerDescription.baseMapDescription.containerID);
		return;
	}
	...
	else
	{
		...
		if (ea32Size(&spPortsToTry))
		{
			...
			else
			{
				...
				if(local_listen)
				{
					listenInitReceiveStats(local_listen, "Local Gameclients", ClientToServerMsgEnum);
					eaPush(&gGSLState.clients.netListens, local_listen);
				}
				if(public_listen)
				{
					listenInitReceiveStats(public_listen, "Public Gameclients", ClientToServerMsgEnum);
...
void OVERRIDE_LATELINK_GetMemSizeForMemLeakTracking(size_t *pOutSize, size_t *pOutEffectiveSize, char **ppOutPrettySizeString)
{
	...
	// Need to cast to S64 in all cases to avoid math errors due to wrap-around on 32-bit math
	iEffectiveSize64 = ((S64)iRaw) - (((S64)siMaxPlayers) * ((S64)siMemLeakCorrectionPerPlayer)) - (((S64)iMaxPartitions) * ((S64)siMemLeakCorrectionPerPartition));
...
void gslStateBeginMemLeakTracking(void)
{
	...
	if ((isProductionMode() || gbTrackMemLeaksInDevMode) && siMemLeakTrackingIncreaseAmount && siTimeBeforeStartingMemLeakTracking &&  siMemLeakTrackingInterval)
	{
		...
		if (isProductionEditMode()) 
		{
			...
			pAlertKeyPrefix = "UGCEDIT_";
		}
		else if(resNamespaceIsUGC(zmapInfoGetPublicName(NULL)))
		{
			pAlertKeyPrefix = "UGC_";
		}

		if ((eType == ZMTYPE_STATIC) || (eType == ZMTYPE_SHARED) || (eType == ZMTYPE_QUEUED_PVE) || (eType == ZMTYPE_PVP))
		{
			siFirstIncreaseAllowanceAmount += STATIC_MAP_MEMORY_ALLOWANCE;
		}
		BeginMemLeakTracking(siTimeBeforeStartingMemLeakTracking, siMemLeakTrackingInterval, siMemLeakTrackingIncreaseAmount, siFirstIncreaseAllowanceAmount, pAlertKeyPrefix);
	}
...
void gslReplaySyntheticLogs_Enter(void)
{
	frameLockedTimerCreate(&gGSLState.flt, 3000, 3000 / 60);
}

void gslReplaySyntheticLogs_BeginFrame(void)
{
	frameLockedTimerStartNewFrame(gGSLState.flt, 1);
...
void gslReplaySyntheticLogs_EndFrame(void)
{
	ReplaySyntheticLogs_Tick();

	gslSleepForRestOfFrame(gGSLState.flt, inverseMaxFPS);
}

void gslReplaySyntheticLogs_Leave(void)
{
...
void GSL_InitStates(void)
{
	...
	GSM_AddGlobalState(GSL_REPLAYSYNTHETICLOGS);
	GSM_AddGlobalStateCallbacks(GSL_REPLAYSYNTHETICLOGS, gslReplaySyntheticLogs_Enter, gslReplaySyntheticLogs_BeginFrame, gslReplaySyntheticLogs_EndFrame, gslReplaySyntheticLogs_Leave);
