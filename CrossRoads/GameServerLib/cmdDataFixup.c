...
int UpdateMessageKeyGameActions( FILE* log, WorldGameActionProperties** props, const char* layerFName, const char* groupName, const char* scopeName )
{
	...
	for( it = 0; it != eaSize( &props ); ++it ) {
		...
		if( prop->pSendNotificationProperties ) {
			numChangedAccum += UpdateMessageKey( log, &prop->pSendNotificationProperties->notifyMsg.msg, layerFName, groupName, scopeName, &messageIt );
			++messageIt;
		}
		...
		if (pDefCopy)
		{
			if(pDefCopy->defaultDialog && pDefCopy->defaultDialog[0])
			{
				...
				else
				{
					...
					strcpy(buf, pDefCopy->infoDialog[0]->displayTextMesg.msg.pEditorCopy->pcDefaultString);
					strcat(buf, "<br><br>");
					strcat(buf, pDefCopy->defaultDialog[0]->displayTextMesg.msg.pEditorCopy->pcDefaultString);

					StructFreeString(pDefCopy->infoDialog[0]->displayTextMesg.msg.pEditorCopy->pcDefaultString);
					pDefCopy->infoDialog[0]->displayTextMesg.msg.pEditorCopy->pcDefaultString = StructAllocString(buf);
				}
		...
		if (pDef && pDef->scope && strstri(pDef->scope, "lore"))
		{
			if (contact_IsSingleScreen(pDef) && eaSize(&pDef->specialDialog) == 1){
				...
				if (pDefCopy)
				{
					...
					pItemDef->descriptionMsg.pEditorCopy = StructClone(parse_Message, pDefCopy->specialDialog[0]->dialogBlock[0]->displayTextMesg.msg.pEditorCopy);
					if (pItemDef->displayNameMsg.pEditorCopy){
...
static bool gameaction_FindBadMessages(WorldGameActionProperties *pAction, const char *pcBaseMessageKey, int iIndex, FILE* pFile, const char* pchFileName, const char*** peaVisitedKeys)
{
	...
	if (pAction->eActionType == WorldGameActionType_SendFloaterMsg) {
		...
		DisplayMessage *pDispMsg = &pAction->pSendNotificationProperties->notifyMsg.msg;
		Message* pMsg = pDispMsg->bEditorCopyIsServer ? pDispMsg->pEditorCopy : GET_REF(pDispMsg->hMessage);
		if (pMsg) {
...
static bool mission_FindBadMessages(MissionDef *pRootMission, MissionDef *pMission, FILE* pFile, const char*** peaVisitedKeys)
{
	...
	pMsg = pMission->detailStringMsg.msg.bEditorCopyIsServer ? pMission->detailStringMsg.msg.pEditorCopy : GET_REF(pMission->detailStringMsg.msg.hMessage);
	if(pMsg) {
	...
	for (i=0; i<n; i++) {
		...
		pMsg = pMission->eaMissionDisplayOverride[i]->detailStringMsg.msg.bEditorCopyIsServer ? pMission->eaMissionDisplayOverride[i]->detailStringMsg.msg.pEditorCopy : GET_REF(pMission->eaMissionDisplayOverride[i]->detailStringMsg.msg.hMessage);
		if(pMsg) {
			estrPrintf(&estrNewKey, "%s", msgCreateUniqueKey(MKP_MISSIONDETAIL, buf1, pMsg->pcMessageKey));
			if(stricmp(estrNewKey, pMsg->pcMessageKey) != 0 && !strStartsWith(pMsg->pcMessageKey, "Missiondef.") && eaFindString(peaVisitedKeys, pMsg->pcMessageKey) > -1)
			{
		...
		pMsg = pMission->eaMissionDisplayOverride[i]->summaryMsg.bEditorCopyIsServer ? pMission->eaMissionDisplayOverride[i]->summaryMsg.pEditorCopy : GET_REF(pMission->eaMissionDisplayOverride[i]->summaryMsg.hMessage);
		if(pMsg) {
			estrPrintf(&estrNewKey, "%s", msgCreateUniqueKey(MKP_MISSIONDETAIL, buf1, pMsg->pcMessageKey));
			if(stricmp(estrNewKey, pMsg->pcMessageKey) != 0 && !strStartsWith(pMsg->pcMessageKey, "Missiondef.") && eaFindString(peaVisitedKeys, pMsg->pcMessageKey) > -1)
			{
...
static void MakeContactMissionOfferDialogBlockEditorCopyMessage(DialogBlock* pDialogBlock, MissionDef* pMissionDef)
{
	...
	MakeContactMissionOfferEditorCopyMessage(&pDialogBlock->displayTextMesg.msg, pMissionDef);
}
