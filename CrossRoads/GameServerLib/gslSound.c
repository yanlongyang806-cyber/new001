...
static void entGetCloseStatusEnts(Entity *e, EntityRef **refsOut)
{
	...
	if(e && refsOut)
	{
		...
		for(i=0; i<eaSize(&e->aibase->statusTable); i++)
		{
			...
			Entity *statEnt = entFromEntityRef(iPartitionIdx, entry->entRef);
			int visible = 0;

			if(entry->visible || ABS_TIME_SINCE_PARTITION(iPartitionIdx, entry->time.lastVisible) < SEC_TO_ABS_TIME(15))
			{
				visible = 1;
			}
...
void sndServerPlayAd(Entity *e)
{
	...
	ClientCmd_VoicePlayAd(e, "sip:confctl-1270@" ORGANIZATION_DOMAIN);
