...
AUTO_EXPR_FUNC(Emote,player) ACMD_NAME(AccessLevel);
...
AUTO_EXPR_FUNC(Emote,player) ACMD_NAME(AccountAccessLevel);
S32 emoteExprAccountAccessLevel(ACMD_EXPR_SELF Entity *pEnt)
{
    return ( pEnt && pEnt->pPlayer ) ? pEnt->pPlayer->accountAccessLevel : -1;
}
...
static bool EmoteCheckRequires(Entity *pEnt, EmoteDef *pDef)
{
	...
	{
		MultiVal mv;
		exprContextSetPartition(s_pEmoteContext, entGetPartitionIdx(pEnt));
		exprContextSetSelfPtr(s_pEmoteContext, pEnt);
		exprContextSetPointerVar(s_pEmoteContext, "Player", pEnt, parse_Entity, false, false);
...
{
	ExprFuncTable* stFuncs = exprContextCreateFunctionTable("Emote");
	...
	exprContextSetAllowRuntimePartition(s_pEmoteContext);
...
static void EntityEmote(Entity *pEnt, EmoteDef *pDef, bool bShowText)
{
	...
	if (!(pEnt && pDef)
		...
		(	entIsPlayer(pEnt)
			&&
			(	SAFE_MEMBER(pEnt->pChar,pNearDeath)	||	// don't allow player emotes during near death
				!entIsAlive(pEnt))))					// don't allow player emotes during death
	{
		return;
	}
