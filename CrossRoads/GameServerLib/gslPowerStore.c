...
void powerstore_TrainPet(Entity* pEnt, U32 uiContainerID, const char* pchNode, U32 uStorePowerIndex)
{
	...
	else
	{
		...
		if (pContactDialog->bIsOfficerTrainer)
		{
			...
			if (!Officer_CanTrain(iPartitionIdx, pEnt, pEnt, uiContainerID, pchNode, pNewNodeDef->pchNameFull, false, false, pPreferredAllegiance, &pTrainNodeDef))
			{
				return;
			}

			pRankDef = Officer_GetRankDefFromNode(pTrainNodeDef, pAllegiance, pSubAllegiance);
		...
		character_StartTraining(entGetPartitionIdx(pEnt),pEnt, pPetEnt,
								pchNode, pTrainNodeDef->pchNameFull, iNewNodeRank, pchCostNumeric, iCost, 
								fRefundPercent, uiStartTime, uiEndTime, 
								uiRemoveItemID, bDestroyItem, true, eTrainType, TrainEnt_CB,
								TrainEnt_CreateCBData(pEnt, pPetEnt, pTrainNodeDef->pchNameFull, eTrainerType));
	}
}