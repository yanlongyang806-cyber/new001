...
gslCurrencyExchange_OncePerFrame(void)
{
	...
	if ( curTime != sLastUpdateTime )
	{
		...
		// If there are current subscribers or this is a gateway server and the request interval has passed, than request again.
		if ( (GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER || ( CurrencyExchange_NumSubscribers() > 0 )) && ( ( sLastRequestForUIData + REQUEST_UI_DATA_INTERVAL ) < curTime ) )
		{
...
{
	if(StructCompare(parse_CurrencyExchangeGlobalUIData,uiData, &sGlobalUIData, 0, 0, 0))
	{
		StructCopy(parse_CurrencyExchangeGlobalUIData, uiData, &sGlobalUIData, 0, 0, 0);

		if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
		{
			wgsContainerModified(GLOBALTYPE_CURRENCYEXCHANGE,NULL);
		}
	}
...
gslCurrencyExchange_NotifyPlayer(ContainerID playerID, CurrencyExchangeResultType resultType, CurrencyExchangeOperationType operationType)
{
	...
	if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
	{
		pEnt = entForClientCmd(playerID,NULL);
	}
	else
	{
		pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER,playerID);
	}
...
CurrencyExchange_CreateBuyOrder(Entity *pEnt, U32 quantity, U32 price)
{
    ...
    {
        RemoteCommand_aslCurrencyExchange_CreateOrder(GLOBALTYPE_CURRENCYEXCHANGESERVER, 0, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pEnt->pSaved->savedName, OrderType_Buy, quantity, price, pEnt->debugName, entity_GetProjSpecificLogString(pEnt));
    }
}
...
CurrencyExchange_CreateSellOrder(Entity *pEnt, U32 quantity, U32 price)
{
    if ( pEnt && pEnt->pPlayer && pEnt->pSaved && ( entGetVirtualShardID(pEnt) == 0 ) )
    {
        RemoteCommand_aslCurrencyExchange_CreateOrder(GLOBALTYPE_CURRENCYEXCHANGESERVER, 0, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pEnt->pSaved->savedName, OrderType_Sell, quantity, price, pEnt->debugName, entity_GetProjSpecificLogString(pEnt));
    }
}
...
CurrencyExchange_WithdrawOrder(Entity *pEnt, U32 orderID)
{
	if ( pEnt && pEnt->pPlayer && pEnt->pSaved && ( entGetVirtualShardID(pEnt) == 0 ) )
	{
        RemoteCommand_aslCurrencyExchange_WithdrawOrder(GLOBALTYPE_CURRENCYEXCHANGESERVER, 0, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pEnt->pSaved->savedName, orderID, pEnt->debugName, entity_GetProjSpecificLogString(pEnt));
	}
}
...
CurrencyExchange_ClaimTC(Entity *pEnt, U32 quantity)
{
	if ( pEnt && pEnt->pPlayer && pEnt->pSaved && ( entGetVirtualShardID(pEnt) == 0 ) )
	{
        RemoteCommand_aslCurrencyExchange_ClaimTC(GLOBALTYPE_CURRENCYEXCHANGESERVER, 0, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pEnt->pSaved->savedName, quantity, pEnt->debugName, entity_GetProjSpecificLogString(pEnt));
	}
}
...
CurrencyExchange_ClaimMTC(Entity *pEnt, U32 quantity)
{
	if ( pEnt && pEnt->pPlayer && pEnt->pSaved && ( entGetVirtualShardID(pEnt) == 0 ) )
	{
        RemoteCommand_aslCurrencyExchange_ClaimMTC(GLOBALTYPE_CURRENCYEXCHANGESERVER, 0, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pEnt->pSaved->savedName, quantity, pEnt->debugName, entity_GetProjSpecificLogString(pEnt));
	}
}

static void CurrencyExchangeDictionaryChangeCB(enumResourceEventType eType, const char *pDictName, const char *name, CurrencyExchangeAccountData *p, void *userData)
{
	if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER && p)
	{
		gslGatewayServer_ContainerSubscriptionUpdate(eType, GLOBALTYPE_CURRENCYEXCHANGE, p->iAccountID);
	}
}
...
gslCurrencyExchange_SchemaInit(void)
{
	...
	resDictRegisterEventCallback(GlobalTypeToCopyDictionaryName(GLOBALTYPE_CURRENCYEXCHANGE), CurrencyExchangeDictionaryChangeCB,NULL);
}