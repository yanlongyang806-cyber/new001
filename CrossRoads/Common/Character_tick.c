...
static int gclCharacter_ShouldCancelQueuedPowers(Entity *e, PowerActivation *pact, S32 bBecomeCurrent)
{
	if(!bBecomeCurrent && !pact->bCommit)
	{
		...
		if( !gclAutoAttack_IsEnabled() && !CombatPowerStateSwitching_GetActivateDelay(e->pChar) &&
			(g_CombatConfig.bMovementCancelsRootingQueuedPowers && poweract_DoesRootPlayer(pact)) &&
			(mmGetLastQueuedInputValueBitFG(e->mm.movement,MIVI_BIT_FORWARD) 
				|| mmGetLastQueuedInputValueBitFG(e->mm.movement,MIVI_BIT_BACKWARD)))
		{
			if (pmTimestamp(0.15f) < pact->uiTimestampCurrented)
			{
				return true;
			}
		}
...
{
#ifdef GAMESERVER
	static bool bFiledActivationSeqResetError = false;
#endif
	...
	if(pchar->pPowActQueued)
	{
		...
		// check if we can still pay the power cost, if not we'll need to cancel it
		...
		if (bBecomeCurrent && pchar->pPowActQueued->uiTimestampCurrented)
		{
			if (pmTimestamp(0) < pchar->pPowActQueued->uiTimestampCurrented)
			{
				bBecomeCurrent = false;
			}
		}
		...
		if(bBecomeCurrent && !pact->bUnpredicted && !pact->bCommit)
		{
			...
			else
			{
				if (g_CombatConfig.bClientPredictSeq)
				{
					...
					if (!bFiledActivationSeqResetError)
					{
						Errorf("PowerActivation Seq reset due to server-stalled queued PowerActivation going current");
						bFiledActivationSeqResetError = true;
					}
					else
					{
						Alertf("PowerActivation Seq reset due to server-stalled queued PowerActivation going current");
					}
		...
		if(bBecomeCurrent)
		{
			...
			if(bOverride)
			{
				...
				if(ppowCurrent)
				{
					...
					character_AirFrictionOverrideStop(pchar, pchar->pPowActCurrent, 0);
			...
			if (pdef)
			{
				...
				// we don't need to change the fTimeActivating since the first tick it won't get a tick
			}
			...
			eventsend_RecordPowerActivation(pchar->pEntParent, pentTarget, pdef);
				...
				if(bGood)
				{
												...
												!g_CombatConfig.bPayPowerCostAndRechargePostHitframe, pExtract))
					{
						...
						else
						{
							...
							if(pPreActivateDef)
							{
								character_CancelModsFromDef(pchar, pPreActivateDef, entGetRef(pchar->pEntParent), pact->uiIDServer, false, true);
							}
...
#define CRAZY_MAINTAINED_PREDICTION 0
					...
					if(bDeactivate)
					{
						...
						{	// normal maintain deactivation, no post-maintain time
							...
							character_DeactivateMaintained(iPartitionIdx, pchar, ppow, pact, pafx, uiAnimTime, true);
...
// Non-powers updates for the character. Should these be here at all? They seem out of place compared to everything else that happens in these ticks
#ifdef GAMESERVER
		gsl_SetMailMessageCount(pchar->pEntParent);

		if(pchar->pEntParent->pPlayer && pchar->pEntParent->bNeedsChatMailFixup && !pchar->pEntParent->bChatMailFixupInProgress)
		{
			Entity *pSharedBank = GET_REF(pchar->pEntParent->pPlayer->hSharedBank);
			if (!pSharedBank)
			{
				if (!IS_HANDLE_ACTIVE(pchar->pEntParent->pPlayer->hSharedBank))
				{
					char idBuf[128];
					// subscribe the bank
					NOCONST(Entity) *pEntNoconst = CONTAINER_NOCONST(Entity, pchar->pEntParent);
					SET_HANDLE_FROM_STRING(GlobalTypeToCopyDictionaryName(GLOBALTYPE_ENTITYSHAREDBANK), ContainerIDToString(pchar->pEntParent->pPlayer->accountID, idBuf), pEntNoconst->pPlayer->hSharedBank);
				}
			}
			else
			{
				pchar->pEntParent->bChatMailFixupInProgress = true;
				gsl_TransferMail(pchar->pEntParent);
			}
		}

		if(pchar->pEntParent->pPlayer &&
			eaSize(&pchar->pEntParent->pPlayer->eaPendingAccountRewards) > 0)
		{
			reward_ProcessPendingAccountRewards(pchar->pEntParent);
		}
#endif
	}
										  ...
										  F32* pfDelayInSeconds)
{
	...
	if(!pPowToQueueDef || pPowToQueueDef->eType != kPowerType_Instant)
	{
		...
		{
			F32 fCombatPowerStateSwitchDelay = 0.f;
			fCombatPowerStateSwitchDelay = CombatPowerStateSwitching_GetActivateDelay(pchar);
			if (fCombatPowerStateSwitchDelay != 0.f)
			{
				F32 fStateSwitchTicks = 1.f + ceil(fCombatPowerStateSwitchDelay / gConf.combatUpdateTimer);
				MAX1(fTicks,fStateSwitchTicks);
			}
		}
