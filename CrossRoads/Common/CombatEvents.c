...
U8 g_ubCombatEventComplexWhitelist[] = {
	...
	kCombatEvent_NearDeathReviveIn
};
									   ...
									   const Vec3 vSourcePos)
{
	if(pchar->pCombatEventState && pchar->pCombatEventState->abCombatEventTriggerComplex[eEvent])
	{
		...
		{
			pTracker->fAngleToSource = EntityAngleToSourcePosUtil(pchar->pEntParent, vSourcePos);
		}
		else
		{
									 ...
									 CombatEventTracker *pTriggerEvent)
{
	...
	if (bRet && pmoddefTarget && pmoddefTarget->pExprIgnores)
	{
		...
		PERFINFO_AUTO_START_FUNC();
		combateval_ContextSetupAffects(pchar,pmodTarget,ppowdefAffects,pmoddefAffects,pmodAffects,pTriggerEvent);
