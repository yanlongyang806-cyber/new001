...
AUTO_STRUCT;
typedef struct UGCPlayComponentData
{
	int componentID;						AST(NAME("ComponentID"))
	Mat4 componentMat4;						AST(NAME("ComponentMat4"))
} UGCPlayComponentData;
...
typedef struct UGCPlayResult
{
	...
	UGCPlayComponentData** eaComponentData;	AST(NAME("ComponentData"))
...
// Useful function for checking if a UGCProject is in a state where it will be auto-deleted some time in the future. Returns true if this is a zombie project.
bool UGCProject_CanAutoDelete(const UGCProject *pUGCProject);

// An Uninitialized UGCProjectSeries is one that has no versions
bool UGCProjectSeries_IsInitialized(const UGCProjectSeries *pUGCProjectSeries);
// An Zombie UGCProjectSeries is one that has been uninitialized for some time since creation
...
{
	return  (UGCProjectVersion*) UGCProject_trh_GetSpecificVersion(CONTAINER_NOCONST(UGCProject, pProject), pNameSpace);
}
...
//timeouts for various publishing steps
extern int gUGCPublish_ServerBinner_Timeout;
extern int gUGCPublish_ClientBinner_Timeout;
extern int gUGCPublish_Beaconizing_Timeout;
extern int gUGCPublish_WaitForSync_Timeout;
extern int gUGCPublish_FlushCache_Timeout;
extern int gUGCPublish_ServerBinner_SoftTimeout;
extern int gUGCPublish_ClientBinner_SoftTimeout;
extern int gUGCPublish_Beaconizing_SoftTimeout;
extern int gUGCPublish_WaitForSync_SoftTimeout;
extern int gUGCPublish_FlushCache_SoftTimeout;
extern int g_bUGCDurationAndCompletionStatsResetAfterPublish;
