...
int gUGCPublish_ServerBinner_Timeout = 7200;
//hard timeout for Serverbinning part of UGC publish
AUTO_CMD_INT(gUGCPublish_ServerBinner_Timeout, ServerBinner_Timeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);


int gUGCPublish_ClientBinner_Timeout = 7200;
//hard timeout for Clientbinning part of UGC publish
AUTO_CMD_INT(gUGCPublish_ClientBinner_Timeout, ClientBinner_Timeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);


int gUGCPublish_Beaconizing_Timeout = 7200;
//hard timeout for Beaconizing part of UGC publish
AUTO_CMD_INT(gUGCPublish_Beaconizing_Timeout, Beaconizing_Timeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);

int gUGCPublish_WaitForSync_Timeout = 7200;
//hard timeout for WaitForSync part of UGC publish
AUTO_CMD_INT(gUGCPublish_WaitForSync_Timeout, WaitForSync_Timeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);

int gUGCPublish_FlushCache_Timeout = 7200;
//hard timeout for FlushCache part of UGC publish
AUTO_CMD_INT(gUGCPublish_FlushCache_Timeout, FlushCache_Timeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);


int gUGCPublish_ServerBinner_SoftTimeout = 3600;
//soft timeout (generates alerts) for Serverbinning part of UGC publish
AUTO_CMD_INT(gUGCPublish_ServerBinner_SoftTimeout, ServerBinner_SoftTimeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);


int gUGCPublish_ClientBinner_SoftTimeout = 3600;
//soft timeout (generates alerts) for Clientbinning part of UGC publish
AUTO_CMD_INT(gUGCPublish_ClientBinner_SoftTimeout, ClientBinner_SoftTimeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);


int gUGCPublish_Beaconizing_SoftTimeout = 3600;
//soft timeout (generates alerts) for Beaconizing part of UGC publish
AUTO_CMD_INT(gUGCPublish_Beaconizing_SoftTimeout, Beaconizing_SoftTimeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);

int gUGCPublish_WaitForSync_SoftTimeout = 3600;
//soft timeout (generates alerts) for WaitForSync part of UGC publish
AUTO_CMD_INT(gUGCPublish_WaitForSync_SoftTimeout, WaitForSync_SoftTimeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);

int gUGCPublish_FlushCache_SoftTimeout = 3600;
//soft timeout (generates alerts) for FlushCache part of UGC publish
AUTO_CMD_INT(gUGCPublish_FlushCache_SoftTimeout, FlushCache_SoftTimeout) ACMD_AUTO_SETTING(UGC, GAMESERVER, UGCDATAMANAGER);

int g_bUGCDurationAndCompletionStatsResetAfterPublish = 0;
AUTO_CMD_INT( g_bUGCDurationAndCompletionStatsResetAfterPublish, UGCDurationAndCompletionStatsResetAfterPublish ) ACMD_AUTO_SETTING(Ugc, UGCDATAMANAGER, UGCSEARCHMANAGER);
...
bool UGCProjectSeries_IsInitialized(const UGCProjectSeries *pUGCProjectSeries)
{
	return 0 != eaSize(&pUGCProjectSeries->eaVersions);
}

bool UGCProjectSeries_IsZombie(const UGCProjectSeries *pUGCProjectSeries, U32 uTimeSinceCreation)
{
	// [COR-17872] Note that old UGCProjectSeries never had their iCreationTime set, which
	// causes this code to treat UGCProjectSeries with no versions as to be immediately deletable. That's ok, because we are just trying
	// to prevent the race condition on newly created UGCProjectSeries between: 1) once per frame auto delete and 2) creation/initialization steps.
	return !UGCProjectSeries_IsInitialized(pUGCProjectSeries) && (timeSecondsSince2000() >= pUGCProjectSeries->iCreationTime + uTimeSinceCreation);
}

bool UGCProjectSeries_DeletedByAuthor(const UGCProjectSeries *pUGCProjectSeries, U32 uTimeSinceDeletion)
{
	return 0 != pUGCProjectSeries->iDeletionTime && (timeSecondsSince2000() >= pUGCProjectSeries->iDeletionTime + uTimeSinceDeletion);
}

bool UGCProjectSeries_IsAccessible(const UGCProjectSeries *pUGCProjectSeries)
{
	return UGCProjectSeries_IsInitialized(pUGCProjectSeries) && !UGCProjectSeries_DeletedByAuthor(pUGCProjectSeries, /*uTimeSinceDeletion=*/0);
}

bool UGCProjectSeries_CanAutoDeleteNow(const UGCProjectSeries *pUGCProjectSeries, const char **cstrReasonOut)
{
	if(UGCProjectSeries_DeletedByAuthor(pUGCProjectSeries, 30*24*60*60)) // author deleted 30 days ago or more
	{
		if(cstrReasonOut)
			(*cstrReasonOut) = "deleted by the author more than 30 days ago";
		return true;
	}

	if(UGCProjectSeries_IsZombie(pUGCProjectSeries, 1*60*60)) // orphaned container for at least an hour
	{
		if(cstrReasonOut)
			(*cstrReasonOut) = "failed to fully initialize for at least an hour";
		return true;
	}
...
bool UGCProject_ValidatePotentialName(const char *pName, bool bIsSeries, char **ppErrorString)
{
	...
	if (pName==NULL)
	{
		estrPrintf(ppErrorString, "UGC.ProjectName_TooShort");
		return false;
	}
	...
	{
		estrPrintf(ppErrorString, "UGC.ProjectName_TooLong");
		return false;
	}

	if(IsAnyProfane(pNameMutable))
	{
		estrPrintf(ppErrorString, "UGC.ProjectName_Profane");
		return false;
	}

	else if(IsAnyRestricted(pNameMutable))
	{
		estrPrintf(ppErrorString, "UGC.ProjectName_Restricted");
		return false;
	}

	// MJF Aug/10/2013 -- No longer relevant with Search 2.0
...
NOCONST(UGCProjectVersion) *UGCProject_trh_GetSpecificVersion(ATH_ARG NOCONST(UGCProject) *pProject, const char *pNameSpace)
{
	int i;
...
{
	S32 iStart, iNumPages;
	UGCSingleReview** eaReviews = NULL;

	PERFINFO_AUTO_START_FUNC();

	// Only consider reviews with comments
	FOR_EACH_IN_EARRAY(pReviews->ppReviews, UGCSingleReview, pReview)
	{
		// The following condition should match the one in the following function ugcReviews_GetPageCount
		if(!pReview->bHidden && !nullStr(pReview->pComment))
			eaPush(&eaReviews, pReview);
	}
	FOR_EACH_END;
...
S32 ugcReviews_GetPageCount( const UGCProjectReviews* pReviews )
{
	...
	result = (S32)ceil(iNumReviews/(F32)UGC_REVIEWS_PER_PAGE);
