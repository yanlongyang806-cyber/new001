...
StaticDefineInt AttribTypeEnum[] =
{
	...
	{ "BankRateFlying", kAttribType_BankRateFlying},
	{ "BankMaxAngleFlying", kAttribType_BankMaxAngleFlying},
...
int InitCharacterAttribs(void)
{
	...
	s_hInnateAccrualSetsCritter = stashTableCreateWithStringKeys(16, StashDeepCopyKeys_NeverRelease);
	...
	switch (pdef->offAttrib)
	{
		...
		xcase kAttribType_EntCreate:
		{
			...
			if(pParams)
			{
				...
				combateval_Generate(pParams->pExprFacing,kCombatEvalContext_Apply);
			}
		}
					 ...
					 EntityRef erEffectedTarget)
{
	...
	switch(pdef->offAttrib)
	{
		xcase kAttribType_EntCreate:
		{
			...
			if(pParams->pExprFacing)
			{
				fFacing = combateval_EvalNew(iPartitionIdx,pParams->pExprFacing,kCombatEvalContext_Apply,NULL);
			}
...
static bool AccrueBag(int iPartitionIdx, AttribAccrualSet *pSet, InventoryBag *pBag, Character *p, CharacterClass *pClass, int iLevel, GameAccountDataExtract *pExtract)
{
	...
	bagiterator_SetBagByID(pEnt,BagID,iter,pExtract);
		...
		for(i=s-1; i>=0; i--)
		{
				...
				&& moddef_AffectsModOrPowerChk(iPartitionIdx,pmod->pDef,pchar,pmod,pmodDamage->pDef,pmodDamage,pmodDamage->pDef->pPowerDef))
			{
				...
				if(pdef && pmod->pSourceDetails)
				{
					...
					{
						Entity *pModOwner = pmod->erOwner ? entFromEntityRef(iPartitionIdx, pmod->erOwner) : NULL;
						Entity *pModSource = pmod->erSource ? entFromEntityRef(iPartitionIdx, pmod->erSource) : NULL;
						Character *pCharSource = power_GetPowerEnhancementSource(SAFE_MEMBER(pModSource, pChar),
																				SAFE_MEMBER(pModOwner, pChar));
						if (pCharSource)
						{
							power_GetEnhancementsForAttribModApplyPower(iPartitionIdx, pCharSource, 
		...
		if(ppEnhancements)
		{
			for(i=eaSize(&ppEnhancements)-1; i>=0; i--)
			{
				...
				for(j=eaSize(&pDefEnh->ppOrderedMods)-1; j>=0; j--)
				{
					...
					if (!bDamageEstimate || pmoddefEnh->bIncludeInEstimatedDamage)
					{
						if((pmoddefEnh->offAttrib==offAttrib
							...
							&& (bDamageEstimate || !pmoddefEnh->pExprRequires
								|| (!bExcludeRequires
					   ...
					   F32 *pfStrAddOut)
{
	return CharacterGetStrengthEx(	iPartitionIdx, pchar, pdef, pdef->offAttrib, ppowdefActing,
									...
									erTarget, pattrStrOverride, bExcludeRequires, bDamageEstimate, peFlagsOut, pfStrAddOut);
}
...
F32 character_GetStrengthGeneric(int iPartitionIdx,
								 ...
								 F32 *pfStrAddOut)
{
	return CharacterGetStrengthEx(iPartitionIdx,pchar,NULL,offAttrib,NULL,0,0,0,NULL,0,NULL,false,false,NULL,pfStrAddOut);
}
								 ...
								 EntityRef erTarget)
{
	...
	for(i=eaSize(&pchar->modArray.ppMods)-1; i>=0; i--)
	{
		...
		if(pmoddefChar
			...
			&& moddef_AffectsPowerChk(iPartitionIdx,pmoddefChar,pchar,pchar->modArray.ppMods[i],pdef))
		{
			eaPush(&ppMods,pchar->modArray.ppMods[i]);
		}
		else if(pmoddefChar
			...
			if(pDefEnh) 
			{
				...
				for(j=eaSize(&pDefEnh->ppOrderedMods)-1; j>=0; j--)
				{
					if(pDefEnh->ppOrderedMods[j]->offAttrib==offAttrib
						...
						&& moddef_AffectsPowerChk(iPartitionIdx,pDefEnh->ppOrderedMods[j],pchar,NULL,pdef))
					{
	...
	if(ppow && pdef && pdef->bEnhancementExtension && pdef->eType!=kPowerType_Enhancement)
	{
		...
		for(j=eaSize(&pdef->ppOrderedMods)-1; j>=0; j--)
		{
			if(pdef->ppOrderedMods[j]->offAttrib==offAttrib
				...
				&& moddef_AffectsPowerChk(iPartitionIdx,pdef->ppOrderedMods[j],pchar,NULL,pdef))
			{
								...
								S32 *pbCountAsDamaged, bool bPeekOnly, CombatTrackerFlag *pFlagsAddedOut)
{
	...
	assert(0);
#else
	F32 fRemaining, fSensitivity, fMagResult, fMagResultNoSensitivity, fScaleNoResist;
	...
	fMagResult = fMag * fSensitivity;
	fMagResultNoSensitivity = fMag;
	fScaleNoResist = fMagNoResist/fMag;
	if(s)
	{
		int i;
		for(i=0; i<s && fMagResultNoSensitivity > 0; i++)
		{
			AttribMod *pmod = pchar->ppModsShield[i];
			AttribModDef *pdef = pmod->pDef;
			ShieldParams *pparams = (ShieldParams*)pdef->pParams;

			if(!pparams->bIgnoreShieldSensitivity && fMagResult <= 0)
				continue;
				...
				&& moddef_AffectsModOrPowerChk(iPartitionIdx,pdef,pchar,pmod,pmodDamage->pDef,pmodDamage,pmodDamage->pDef->pPowerDef))
			{
				...
				F32 fMagPossible = pparams->bIgnoreShieldSensitivity ? fMagResultNoSensitivity * (1.f - fPercentIgnored) : fMagResult * (1.f - fPercentIgnored);
				if(fMaxAbsorb)
				{
				...
				fMagResult -= pparams->bIgnoreShieldSensitivity ? MIN(fAbsorbed * fSensitivity, fMagResult) : fAbsorbed;
				fMagResultNoSensitivity -= fAbsorbed;
	...
	fRemaining = fMagResultNoSensitivity / fMag;
			...
			if(ppowdef)
			{
				...
				if(pApplySource)
				{
					...
					if(bValid)
					{
						...
						{
							Character *pCharSource = power_GetPowerEnhancementSource(pModSource, pModOwner);
			...
			case kEntCreateType_Critter:
				{
					if(pdefCritter && (pParams->eTeam==kEntCreateTeam_None || entFromEntityRef(iPartitionIdx,erOwner)))
					{
						...
						createParams.uiPowerID = pmod->uiPowerID;
						createParams.uiAttribModDefIdx = pmod->pDef->uiDefIdx;

						if (pParams->bUseOwnersFaction && eCreator)
						{
							createParams.pFaction = entGetFaction(eCreator);
						}

						if (pParams->pcBoneGroup && *pParams->pcBoneGroup && eCreator && eCreator->pSaved)
						{
							// Get the effective costume as the directly accessing a player will not be accurate as puppets no longer swap costumes with the player ent.
//							NOCONST(PlayerCostume) *pBaseCostume = CONTAINER_NOCONST(PlayerCostume, eCreator->pSaved->costumeData.eaCostumeSlots[eCreator->pSaved->costumeData.iActiveCostume]->pCostume);
							NOCONST(PlayerCostume) *pBaseCostume =  CONTAINER_NOCONST(PlayerCostume, costumeEntity_GetEffectiveCostume(eCreator));
			...
			if(e && pdefCritter)
			{
				...
				if (e->pChar)
				{
					...
					if(pParams->eStrength==kEntCreateStrength_Locked)
					{
						if (pmod->ppApplyStrengths)
						{
							eaCopyStructs(&pmod->ppApplyStrengths,&e->pChar->ppApplyStrengths,parse_PowerApplyStrength);
						}

						if (pmod->pSourceDetails && pmod->pSourceDetails->pCritical)
						{
							e->pChar->pSavedCritical = StructClone(parse_PACritical, pmod->pSourceDetails->pCritical);
						}
					}
				...
				{
					axisAngleToQuat(upvec,-RAD(pParamsMod->fFacing), qRotFinal);
				}
				...
				else 
				{
					...
					pyrFace[1] -= RAD(pParamsMod->fFacing);
				...
				if (eFaceEnt && (pParamsMod->fFacing || !pParams->bUseFacingPitch))
				{
					Vec3 vRotPYR;
					quatToPYR(qRotFinal, vRotPYR);
					if (pParamsMod->fFacing)
					{
						vRotPYR[1] -= RAD(pParamsMod->fFacing);
					}
					if (!pParams->bUseFacingPitch)
					{
						vRotPYR[0] = 0.0f;
			...
			if(pdef && pParams)
			{
				...
				if(fRecharge > 0)
				{
					...
					if(uchAct)
					{
						...
						{
							character_PowerSetRecharge(iPartitionIdx, pchar, ppow, fRecharge);
						}
				}
			}
		}
...
static void ModProcessKnockTo(int iPartitionIdx, AttribMod *pmod, Character *pchar, F32 fResistTrue, F32 fResist, F32 fAvoid)
{
#if !GAMESERVER && !GAMECLIENT
	assert(0);
#else
	PERFINFO_AUTO_START_FUNC();
	if(!pchar->bUnstoppable && !pchar->bUsingDoor && pmod->pDef->offAspect==kAttribAspect_BasicAbs)
	{
...
// helper function to ModProcessPowerRecharge to process the PowerRecharge for a given power
static void ModProcessPowerRechargeInternal(int iPartitionIdx, AttribMod *pmod, PowerRechargeParams *pParams, 
											Character *pchar, Power *pPower)
{
	PowerDef *pdef = GET_REF(pPower->hDef);
	...
	if (pdef)
	{
		...
		F32 fRechargeCurrent = pPower->fTimeRecharge;
		...
		if(fRechargeMod <= 0 && fRechargeCurrent <= 0 && 
			(!pdef->bChargesSetCooldownWhenEmpty || power_GetChargesUsed(pPower) == 0))
			return;

		// Do the check to see if this AttribMod actually affects the Power in question
		if(moddef_AffectsPowerChk(iPartitionIdx,pmod->pDef,pchar,pmod,pdef))
		{
			character_PowerSetRecharge(iPartitionIdx, pchar, pPower, fRechargeMod);
		}
	}
}
...
static void ModProcessPowerRecharge(int iPartitionIdx, AttribMod *pmod, Character *pchar)
{
	PERFINFO_AUTO_START_FUNC();
	{
		...
		if(pParams)
		{
			...
			{
				int i;
				S32 bRechargeCombateStatePowers = !CombatPowerStateSwitching_ShouldShareRecharge(pchar);
				...
				for(i=eaSize(&pchar->ppPowers)-1; i>=0; i--)
				{
					...
					if(ppow)
					{
						ModProcessPowerRechargeInternal(iPartitionIdx, pmod, pParams, pchar, ppow);

						if (bRechargeCombateStatePowers)
						{	// our power state switched powers do not share recharge, 
							// try and apply the recharge to them individually
							FOR_EACH_IN_EARRAY_FORWARDS(ppow->ppSubCombatStatePowers, Power, pSubStatePower)
							{
								if (pSubStatePower)
									ModProcessPowerRechargeInternal(iPartitionIdx, pmod, pParams, pchar, pSubStatePower);
							}
							FOR_EACH_END
						}
					}
	...
	PERFINFO_AUTO_START_FUNC();
	{
		...
		if(pProjParams && pParamsMod && !pmod->erCreated)
		{
			...
			if (pProjectileEnt)
			{
				...
				if (pmod->pSourceDetails && pmod->pSourceDetails->pCritical)
				{
					pProjectileEnt->pChar->pSavedCritical = StructClone(parse_PACritical, pmod->pSourceDetails->pCritical);
				}
...
static void ModProcessTeleport(AttribMod *pmod, Character *pchar, F32 fResistTrue, F32 fResist)
{
#if GAMESERVER
	PERFINFO_AUTO_START_FUNC();
	if (!pchar->bUnstoppable && pmod->pDef->offAspect==kAttribAspect_BasicAbs)
	{
	...
	PERFINFO_AUTO_START_FUNC();
	{
			...
			&& pmod->pSourceDetails)
		{
			...
			if(iEvents)
			{
				...
				Entity *eModSource = pmod->erSource ? entFromEntityRef(iPartitionIdx,pmod->erSource) : NULL;
				Entity *eModOwner = pmod->erOwner ? entFromEntityRef(iPartitionIdx, pmod->erOwner) : NULL;
				...
				if(pcharSource)
				{
					...
					{
						Character *pCharSource = power_GetPowerEnhancementSource(SAFE_MEMBER(eModSource, pChar), SAFE_MEMBER(eModOwner, pChar));
						if (pCharSource)
						{
							power_GetEnhancementsForAttribModApplyPower(iPartitionIdx, pCharSource,
																		pmod, EEnhancedAttribList_DEFAULT, 
								...
								GameAccountDataExtract *pExtract)
{
	...
	case kAttribType_KnockTo:
		ModProcessKnockTo(iPartitionIdx,pmod,pchar,fResistTrue,fResist,fAvoid);
		break;
	...
	case kAttribType_Teleport:
		ModProcessTeleport(pmod,pchar,fResistTrue,fResist);
		break;
	...
	PERFINFO_AUTO_START("Step",1);
	{
		...
		if(bApplyThisTick)
		{
			...
			else if(!pmod->bIgnored)
			{
							...
							case kAttribType_Power:
							{
								...
								if(eAttrib==kAttribType_HitPoints)
								{
									...
									if(fEffMagNoResist < 0)
									{
										...
										// NOTE: Currently, NearDeath reviving is done via kAttribType_HitPoints gain
										// make sure to only count the NearDeathReviveIn once, so in the unlikely event 
										// we get two HitPoints modifications in the same tick, we'll only get one event
										if(pchar->pNearDeath && 
											(0 == character_GetCombatEventCount(pchar, kCombatEvent_NearDeathReviveIn))
											&& character_CombatEventTrack(pchar, kCombatEvent_NearDeathReviveIn))
										{
											Entity *entModOwner = pmod->erOwner ? entFromEntityRef(iPartitionIdx, pmod->erOwner) : eSource;

											character_CombatEventTrackComplex(	pchar, kCombatEvent_NearDeathReviveIn, 
																				entModOwner, pmoddef->pPowerDef, 
																				pmoddef, fMag, fMagNoResist, pmod->vecSource);
										}
									}
								}
								else
				...
				{
					if(!bFoundMitigators && (eAttrib==kAttribType_KnockTo || 
												eAttrib==kAttribType_SetCostume || 
												eAttrib==kAttribType_Teleport))
					{
						character_ModGetMitigators(iPartitionIdx, pmod, pmoddef, pchar, &fResistTrue, &fResist, &fImmune, &fAvoid, &eFlagsMit);
						bFoundMitigators = true;
					}
					...
					{
						ModProcessSpecial(iPartitionIdx, pmod, pmoddef,pchar, fRate, piModes, 
											fResistTrue, fResist, fAvoid, pExtract);
					}
