...
typedef struct PTNodeDef
{
	...
	REF_TO(PTNodeDef) hNodeAltTrainer;			AST(NAME(AltTrainerNode))
		// If specified, when training an officer in this node, if the node cannot be trained, it
		// will attempt to train this alt node instead
	...
	Expression *pExprAccessCondition;			AST(NAME(AccessCondition) LATEBIND)
		// Evaluated to figure out if the character can purchase this (group/node)
		// if not specified (normal), it can be accessed
...
typedef struct PTGroupDef
{
	...
	bool bUIIgnoreGroupRequirements;
		// When set, it causes several of the UIGen expression functions that enumerate PowerListNodes to ignore the group requirements
...
typedef struct PowerTreeDef
{
	...
	U32 bIsTalentTree;
		...
		// except for talent trees where there is a concept of a stack of 
...
typedef struct PowerTreeRespecConfig
{
	...
	bool bNoContactRequiredForRespec;	AST(NAME(NoContactRequiredForRespec))
		// If no contact is required in order to allow respecs
...
void powertree_FinalizeNodes(ATH_ARG NOCONST(Entity) *e, NOCONST(PowerTree) *pConstTree);
...
// returns true if the node is a conditional node, it has an expression pExprAccessCondition
bool powertree_IsConditionalNode(SA_PARAM_NN_VALID PTNodeDef *pNodeDef);

// returns true if the entity can buy the given PTNodeDef. 
// if no pExprAccessCondition is defined, returns true
bool powertree_CanAccessConditionalNode(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_NN_VALID PTNodeDef *pNodeDef);
...
// Check to fix any missing power tree nodes, or other power tree problems on the entity
LATELINK;
bool ent_NeedsProjectSpecificPowersFixup(Entity *pEnt);

LATELINK;
void ent_FixupProjectSpecificPowers(Entity *pEnt);
