...
static int SortAttribModDefsAutoDesc_helper(const AttribModDef **ppModDefA, const AttribModDef **ppModDefB)
{
	int r = 0;
	...
	// Sort by tooltip order
	if(!r)
	{
		r = pModDefB->iAutoDescOrder - pModDefA->iAutoDescOrder;
	}
...
static int SortAttribModDefsAutoDesc(const AttribModDef **ppModDefA, const AttribModDef **ppModDefB)
{
	...
	int r = SortAttribModDefsAutoDesc_helper(ppModDefA, ppModDefB);
...
static int SortAttribModScaleAutoDesc(const AutoDescModScale **ppModScaleA, const AutoDescModScale **ppModScaleB)
{
	...
	int r = SortAttribModDefsAutoDesc_helper(&pModDefA, &pModDefB);

	//Sort by gem socketing
	if(!r) 
	{
		if((*ppModScaleA)->iRequiredGemSlot) {
			r = 1;
		} else if ((*ppModScaleB)->iRequiredGemSlot) {
			r = -1;
		}
	}
	...
	if(pcombo->pExprRequires)
	{
		...
		for(i=0; i<s; i++)
		{
 			...
 			if(pVal->type==MULTIOP_FUNCTIONCALL)
 			{
				...
				{
					langFormatGameMessageKey(lang,&pchRequirements,"AutoDesc.PowerDef.ComboRequires.Falling",STRFMT_END);
					continue;
				}
					...
					const char* pchPowerAutoDescMessageKey)
{
	...
	if(pdef)
	{
		...
		if (pchar && pchar->pEntParent && pdef->pExprHideAutoDesc)
		{
			MultiVal retVal = {0};
			ExprContext *pContext = powers_GetAutoDescExprContext(pchar, pdef);
			exprContextSetSelfPtr(pContext, pchar->pEntParent);
			exprContextSetPartition(pContext, iPartitionIdx);

			exprEvaluate(pdef->pExprHideAutoDesc, pContext, &retVal);

			// If the hide expression returns true, then we need to stop
			// right now before we generate the auto desc
			if (MultiValGetInt(&retVal, NULL))
			{
				return;
			}
		}
		...
		AutoDescPowerDefInternal(iPartitionIdx,lang,pdef,ppchDesc,pAutoDescPower,cpchLine,cpchModIndent,cpchModPrefix,eHeader,pchar,ppow,s_ppEnhancements,iLevel,true,false,iDepth,eDetail,NULL,NULL,NULL,pExtract,pchPowerAutoDescMessageKey);
...
const char* powerdef_GetAttributeType(Language lang, PowerDef* pDef)
{
	int i;
	...
	for (i = 0; i < eaSize(&pDef->ppOrderedMods); i++)
	{
		...
		{
			return StaticDefineLangGetTranslatedMessage(lang, AttribTypeEnum, pModDef->offAttrib);
		}
	}
	return NULL;
}
...
static F32 _getMinMaxExpectedDamage(int iPartitionIdx, PowerDef* pDef, Character* pChar, S32 iLevel, Item** ppWeapons, Power** ppEnhancements, bool bMax)
{
	...
	for (i = 0; i < eaSize(&pDef->ppOrderedMods); i++)
	{
			...
			&& pModDef->bIncludeInEstimatedDamage)
		{
			...
			{
				const S32 bExcludeRequires = false;
				const S32 bDamageEstimate = true; 
				fStrength = moddef_GetStrength(iPartitionIdx, pModDef, pChar, pModDef->pPowerDef, 
											iLevel, iLevel, 1.0f, ppEnhancements, 0, NULL, bExcludeRequires, bDamageEstimate, NULL, &fStrAdd);
			}
...
void powerdef_UseNNOFormatting(	int iPartitionIdx, Language lang, Power *ppow, PowerDef* pDef, char** ppchDesc, Character* pChar, 
								Item * pItem, Power** ppEnhancements, GameAccountDataExtract *pExtract, const char *pchMessageKey)
{
	...
	AutoDescPowerTimeRecharge(iPartitionIdx,max(fTimeRecharge, maxCategoryCooldown),pDef,NULL,&fCooldown,pChar,ppow,0,lang);
	...
	*/
	...
	if (pChar && pChar->pattrBasic)
	{
		...
		{
			estrPrintf(&estrWeapName, "%s d", langTranslateDisplayMessage(lang,GET_REF(pEquippedWeapon->hItem)->displayNameMsg));
		}
		else
		{
	...
	{
		if (range == 0)
			estrPrintf(&estrAoEString,"AutoDesc.PowerDef.EffectAreaSphereBurst");
		else
			estrPrintf(&estrAoEString,"AutoDesc.PowerDef.EffectArea%s",StaticDefineIntRevLookup(EffectAreaEnum,pDef->eEffectArea));
	}
	...
	langFormatMessageRef(lang, &estrHitDesc, pDef->msgDescription.hMessage,
	...
	langFormatMessageRef(lang, &estrMissDesc, pDef->msgDescriptionLong.hMessage,
		STRFMT_STRING("W", weaponDamageBuffer),
		STRFMT_INT("STR", combat_DDAbilityMod(attr_STR)),
		STRFMT_INT("CON", combat_DDAbilityMod(attr_CON)),
		STRFMT_INT("DEX", combat_DDAbilityMod(attr_DEX)),
	...
	langFormatMessageRef(lang, &estrSpecialDesc, pDef->msgDescriptionFlavor.hMessage,
		STRFMT_STRING("W", weaponDamageBuffer),
		STRFMT_INT("STR", combat_DDAbilityMod(attr_STR)),
		STRFMT_INT("CON", combat_DDAbilityMod(attr_CON)),
		STRFMT_INT("DEX", combat_DDAbilityMod(attr_DEX)),
	...
	langFormatMessageRef(lang, &estrRankChangeDesc, pDef->msgRankChange.hMessage,
		STRFMT_STRING("W", weaponDamageBuffer),
		STRFMT_INT("STR", combat_DDAbilityMod(attr_STR)),
		STRFMT_INT("CON", combat_DDAbilityMod(attr_CON)),
		STRFMT_INT("DEX", combat_DDAbilityMod(attr_DEX)),
...
/*	if (IS_HANDLE_ACTIVE(pDef->hCostRecipe) && pChar)
	{
		...
		for (i = 0; i < eaSize(&pRecipeDef->pCraft->ppPart); i++)
		{
			...
			estrClear(&estrTemp);
			estrPrintf(&estrTemp, "%s x%i (Owned: <font color=%s>%i</font>)<br>", langTranslateDisplayMessage(lang,pComponentItem->displayNameMsg), (int)pRecipeDef->pCraft->ppPart[i]->fCount, (numOwned >= pRecipeDef->pCraft->ppPart[i]->fCount) ? "limegreen" : "red", numOwned);
	...
	if (pItem == NULL && pChar)
	{
		...
		pchAttribType = powerdef_GetAttributeType(lang ,pDamagePowerDef);
	}
	...
	{//Convert cooldown time into minutes, seconds, etc.
		...
		langFormatGameMessageKey(lang, &estrTime, "CooldownTime.Formatted",
			STRFMT_INT("TIMEDAYS", days),
	...
	langFormatGameMessageKey(lang, ppchDesc, pchMessageKey && *pchMessageKey ? pchMessageKey : "AutoDesc.NNOPowerFormat",
		STRFMT_STRING("NAME", pDef ? langTranslateDisplayMessage(lang,pDef->msgDisplayName) : NULL),
		STRFMT_INT("RANGE", range),
		STRFMT_MESSAGEKEY("AOE", estrAoEString),
...
void powerdef_UseNNOFormattingForBuff(Language lang, PowerDef* pDef, char** ppchDesc)
{
	ItemDef* pItemDef = NULL;
	ItemDef* pWeaponDef = NULL;
	ItemPowerDef* pItemPowerDef = NULL;

	langFormatMessageRef(lang, ppchDesc, pDef->msgDescriptionLong.hMessage,
		STRFMT_END);
}

// This is super hacky, and deserves to be revisited.
bool g_bNNOOverrideUseOldAutodec = false;
									 ...
									 const char *pchPowerAutoDescMessageKey)
{
	...
	if (gConf.bUseNNOPowerDescs && (!ppow || !ppow->pSourceItem) && !g_bNNOOverrideUseOldAutodec)
	{
		powerdef_UseNNOFormatting(iPartitionIdx, lang, ppow, pdef, ppchDesc, pchar, ppow ? ppow->pSourceItem : NULL, ppEnhancements, pExtract, pchPowerAutoDescMessageKey);
		return;
	}
		...
		&& !(eDetail<kAutoDescDetail_Maximum && eHeader!=kAutoDescPowerHeader_Full && pdef->eEffectArea==kEffectArea_Character))
	{
		...
		if(pdef->eType!=kPowerType_Innate 
			...
			&& !bSelf)
		{
			RegionRules *pRegionRules = regionRules_GetAnyRegionRulesValidForPower(pdef);  // This is really problematic as there may be multiple regionrules 
	...
	for(i=s-1; i>=0; i--)
	{
		...
		if (pchar && pchar->pEntParent && pdefCombo && pdefCombo->pExprHideAutoDesc)
		{
			MultiVal retVal = {0};
			ExprContext *pContext = powers_GetAutoDescExprContext(pchar, pdefCombo);
			exprContextSetSelfPtr(pContext, pchar->pEntParent);
			exprContextSetPartition(pContext, iPartitionIdx);

			exprEvaluate(pdefCombo->pExprHideAutoDesc, pContext, &retVal);
		...
		if(pdefCombo)
		{
			...
			if(ppow && eaSize(&ppow->ppSubPowers) > i && ppow->ppSubPowers[i] && GET_REF(ppow->ppSubPowers[i]->hDef)==pdefCombo)
			{
				...
				AutoDescPowerDefInternal(iPartitionIdx,lang,pdefCombo,ppchDesc,pAutoDescPowerCombo,cpchLine,cpchModIndent,cpchModPrefix,kAutoDescPowerHeader_Full,pchar,ppowCombo,s_ppEnhancementsCombo,iLevel,bIncludeStrength,bFullDescription,iDepth,eDetail,pAutoDescAttribModEvent,NULL,pppchFootnoteMsgKeys,pExtract, pchPowerAutoDescMessageKey);
				eaClearFast(&s_ppEnhancementsCombo);
			}
			else
			{
				AutoDescPowerDefInternal(iPartitionIdx,lang,pdefCombo,ppchDesc,pAutoDescPowerCombo,cpchLine,cpchModIndent,cpchModPrefix,kAutoDescPowerHeader_Full,pchar,NULL,NULL,iLevel,bIncludeStrength,bFullDescription,iDepth,eDetail,pAutoDescAttribModEvent,NULL,pppchFootnoteMsgKeys,pExtract, pchPowerAutoDescMessageKey);
			}
...
void powerdefs_AutoDescInnateMods(int iPartitionIdx,
								  ...
								  AutoDescDetail eDetail)
{
	powerdefs_GetAutoDescInnateMods(iPartitionIdx, lang, pItem,ppdefs, pfScales, ppchDesc, NULL, cpchKey, pchar, eaSlotRequired,-1, iLevel, bIncludeStrength, eActiveGemSlotType, eDetail);
}
								  ...
								  AutoDescDetail eDetail)
{
	...
	// AttribMods
	{
		...
		//Sort the powers before showing
		s = eaSize(&ppMods);
		...
		for(i=0; i<s; i++)
		{
				...
				ppMods[i+1]->pmodDef->offAspect==ppMods[i]->pmodDef->offAspect)
			{
				...
				//For gem mods, use a seperate magnitude addition.
				//Gems MUST be sorted to appear immediately after the appropriate item magnitudes they are modifying.
				i++;
			...
			if (ppchDesc)
			{
				if(cpchKey)
				{
					...
					langFormatMessageKey(lang,&pchTemp,cpchKey,
						...
						STRFMT_STRING("SlotTypeName", StaticDefineLangGetTranslatedMessage(lang,ItemGemTypeEnum, ppMods[i]->iRequiredGemSlot)),
...
static void AutoDescSpecialAIAvoid(Language lang,
								   AttribModDef *pdef,
...
static void AutoDescSpecialAISoftAvoid(Language lang,
										AttribModDef *pdef,
										char **ppchDescription,
										const char *pchMessageKey,
										const char *pchSign,
...
static void AutoDescSpecialAIThreat(Language lang,
									AttribModDef *pdef,
								    char **ppchDescription,
								    const char *pchMessageKey,
								    const char *pchSign,
...
static void AutoDescSpecialApplyPower(Language lang, 
									  AttribModDef *pdef,
									  char **ppchDescription,
									  const char *pchMessageKey,
									  const char *pchSign,
									  ...
									  const char *pchMagnitude)
{
	...
	if(pParams)
	{
		...
		case kApplyPowerEntity_ModOwner:
			pchApplySource = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.AttribModOwner");
			break;
		case kApplyPowerEntity_ModSource:
			pchApplySource = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.AttribModSource");
			break;
		case kApplyPowerEntity_ModSourceCreator:
			pchApplySource = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.AttribModSourceCreator");
			break;
		...
		case kApplyPowerEntity_Random:
			pchApplySource = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.RandomTarget");
			break;
		...
		case kApplyPowerEntity_ClosestNotTarget:
			pchApplySource = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.ClosestTarget");
			break;
		case kApplyPowerEntity_HeldObject:
			pchApplySource = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.HeldObject");
			break;
		}
		...
		if(pParams->eTarget!=pParams->eSource || pParams->eTarget==kApplyPowerEntity_RandomNotSource || pParams->eTarget==kApplyPowerEntity_Random)
		{
			...
			case kApplyPowerEntity_ModSourceTargetDual:
				langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.AttribModSourceTargetDual"),STRFMT_END);
				break;
			case kApplyPowerEntity_ModSourceCreator:
				langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.AttribModSourceCreator"),STRFMT_END);
				break;
			case kApplyPowerEntity_ModTarget:
				langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.AttribModTarget"),STRFMT_END);
				break;
			case kApplyPowerEntity_RandomNotSource:
			case kApplyPowerEntity_Random:
				langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.RandomTarget"),STRFMT_END);
				break;
			case kApplyPowerEntity_ClosestNotSource:
			case kApplyPowerEntity_ClosestNotSourceOrTarget:
			case kApplyPowerEntity_ClosestNotTarget:
				langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.ClosestTarget"),STRFMT_END);
				break;
			case kApplyPowerEntity_ApplicationTarget:
				langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.ApplicationTarget"),STRFMT_END);
				break;
			}
		}
	}
...
static void AutoDescSpecialAttribModHeal(Language lang,
										 AttribModDef *pdef,
										 char **ppchDescription,
										 const char *pchMessageKey,
										 const char *pchSign,
...
static void AutoDescSpecialAttribModExpire(Language lang,
										   AttribModDef *pdef,
										   char **ppchDescription,
										   const char *pchMessageKey,
										   const char *pchSign,
										 ...
										 Language lang)
{
	...
	if(pParams)
	{
		...
		if(RefSystem_ReferentFromString(gMessageDict, "AutoDesc.AttribModDef.Trigger.Chance"))
		{
			...
			{
				langFormatMessageKey(lang, &pchChance, "AutoDesc.AttribModDef.Trigger.ChanceAlways", STRFMT_STRING("ApplySource", ""), STRFMT_FLOAT("Chance", fChance*100.f), STRFMT_END);
			}
			else
			{
				langFormatMessageKey(lang, &pchChance, "AutoDesc.AttribModDef.Trigger.Chance", STRFMT_STRING("ApplySource", ""), STRFMT_FLOAT("Chance", fChance*100.f), STRFMT_END);
			}
		}
		else if(fChance != 1.f)
		{
			estrStackCreate(&pchChance);
			langFormatMessageKey(lang, &pchChance, "AutoDesc.AttribModDef.Chance", STRFMT_FLOAT("Chance", fChance*100.f), STRFMT_END);
		}
	}
...
static void AutoDescSpecialEntCreate(Language lang,
									 AttribModDef *pdef,
									 char **ppchDescription,
									 const char *pchMessageKey,
									 const char *pchSign,
									 ...
									 const char *pchMagnitude)
{
	...
	if(pParams)
	{
		...
		if(!pchCritter)
		{
			pchCritter = langTranslateMessageKey(lang, "AutoDesc.AttribModDef.GenericPetEntity");
		}
	}
	...
	if (pchMagnitude)
	{
		iLevel = atoi(pchMagnitude);
		iLevel = MAX(iLevel, 1);
		if (s_AutoDescConfig.bEntCreateLevelAdjustDown)
		{
			--iLevel;
		}
	}
									   ...
									   const char *pchMagnitude)
{
	...
	if(pParams)
	{
		...
		case kKillTriggerEntity_Victim:
			langFormatMessageKey(lang,&pchApplyTarget,"AutoDesc.AttribModDef.ApplyTarget",STRFMT_MESSAGEKEY("Target","AutoDesc.AttribModDef.Victim"),STRFMT_END);
			break;
		}
		...
		if(RefSystem_ReferentFromString(gMessageDict, "AutoDesc.AttribModDef.Trigger.Chance"))
		{
			...
			{
				langFormatMessageKey(lang,&pchChance, "AutoDesc.AttribModDef.Trigger.ChanceAlways", STRFMT_STRING("ApplySource", ""), STRFMT_FLOAT("Chance", pParams->fChance*100.f), STRFMT_END);
			}
			else
			{
				langFormatMessageKey(lang,&pchChance, "AutoDesc.AttribModDef.Trigger.Chance", STRFMT_STRING("ApplySource", ""), STRFMT_FLOAT("Chance", pParams->fChance*100.f), STRFMT_END);
			}
		}
		else if(pParams->fChance != 1.f)
		{
			estrStackCreate(&pchChance);
			langFormatMessageKey(lang,&pchChance, "AutoDesc.AttribModDef.Chance", STRFMT_FLOAT("Chance", pParams->fChance*100.f), STRFMT_END);
		}
	}
...
static void AutoDescSpecialPowerMode(Language lang,
									 AttribModDef *pdef,
									 char **ppchDescription,
									 const char *pchMessageKey,
									 const char *pchSign,
									 ...
									 const char *pchMagnitude)
{
	...
	if(!pchMode)
	{
		pchMode = langTranslateMessageKey(lang, "AutoDesc.Unknown");
	}
...
static void AutoDescSpecialPowerRecharge(Language lang,
										 AttribModDef *pdef,
										 char **ppchDescription,
										 const char *pchMessageKey,
										 const char *pchSign,
										 ...
										 const char *pchMagnitude)
{
	...
	if(!pchApply)
	{
		pchApply = langTranslateMessageKey(lang,"AutoDesc.Unknown");
	}

	langFormatMessageKey(lang,ppchDescription,pchTemp ? pchTemp : pchMessageKey,
...
static void AutoDescSpecialPowerShield(Language lang,
									   AttribModDef *pdef,
									   char **ppchDescription,
									   const char *pchMessageKey,
									   const char *pchSign,
										  ...
										  const char *pchMagnitude)
{
	...
	if(pParams)
	{
		...
		if(RefSystem_ReferentFromString(gMessageDict, "AutoDesc.AttribModDef.Trigger.Chance"))
		{
			...
			if(pParams->pExprChance == NULL)
			{
				langFormatMessageKey(lang,&pchChance, "AutoDesc.AttribModDef.Trigger.ChanceAlways", STRFMT_STRING("ApplySource", pchApplySource), STRFMT_FLOAT("Chance", fChance*100.f), STRFMT_END);
			}
			else
			{
				langFormatMessageKey(lang,&pchChance, "AutoDesc.AttribModDef.Trigger.Chance", STRFMT_STRING("ApplySource", pchApplySource), STRFMT_FLOAT("Chance", fChance*100.f), STRFMT_END);
			}
		}
		else if(fChance != 1.f)
		{
									   ...
									   Language lang)
{
	...
	if(!*pchError)
	{
		...
		if(pdef->fAutoDescMagnitudeScale)
		{
			fMagnitude *= pdef->fAutoDescMagnitudeScale;
			fMagnitudeMax *= pdef->fAutoDescMagnitudeScale;
		}

		if(fMagnitude > 0)
		{
			*ppchSign = langTranslateMessageKey(lang, "AutoDesc.AttribModDef.MagnitudeSignPositive");
		}
									...
									Language lang)
{
	...
	else if(offAspect==kAttribAspect_BasicAbs && IS_SPECIAL_ATTRIB(offAttrib))
	{
		...
		{
			AutoDescSpecialAIAvoid(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib == kAttribType_AISoftAvoid)
		{
			AutoDescSpecialAISoftAvoid(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_AIThreat)
		{
			AutoDescSpecialAIThreat(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_ApplyPower)
		{
			AutoDescSpecialApplyPower(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		...
		{
			AutoDescSpecialAttribModHeal(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_AttribModExpire)
		{
			AutoDescSpecialAttribModExpire(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		...
		{
			AutoDescSpecialEntCreate(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		...
		{
			AutoDescSpecialKillTrigger(iPartitionIdx,lang,pchar,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_PowerMode)
		{
			AutoDescSpecialPowerMode(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_PowerRecharge)
		{
			AutoDescSpecialPowerRecharge(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_PowerShield)
		{
			AutoDescSpecialPowerShield(lang,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_TriggerComplex)
		{
			AutoDescSpecialTriggerComplex(iPartitionIdx,lang,pchar,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
		else if(pdef->offAttrib==kAttribType_TriggerSimple)
		{
			AutoDescSpecialTriggerSimple(iPartitionIdx,lang,pchar,pdef,ppchDesc,pchTemp,pchSign,pchAttrib,pchTarget,pchMagnitude);
		}
	}
	else
	{
								 ...
								 GameAccountDataExtract *pExtract)
{
	...
	// Ignores
	if(pdef->pExprIgnores && !(ATTRIB_AFFECTOR(pdef->offAttrib) && IS_BASIC_ASPECT(pdef->offAspect)) && !(eDetail!=kAutoDescDetail_Maximum && IS_HANDLE_ACTIVE(pdef->msgAutoDesc.hMessage)))
	{
		pAutoDescAttribMod->pchAffects = langTranslateMessageKey(lang,"AutoDesc.AttribModDef.Ignores");
		if(pppchFootnoteMsgKeys)
		{
			eaPushUnique(pppchFootnoteMsgKeys,"AutoDesc.AttribModDef.Ignores.Footnote");
		}
	}	
	...
	if(bIncludeStrength && ppowdef->eType!=kPowerType_Innate && pdef->eType!=kModType_None)
	{
		...
		fStrength = moddef_GetStrength(	iPartitionIdx, pdef, pchar, ppowdefActing, iLevel, iLevelInline, 
										1, ppEnhancements, 0, NULL, true, false, NULL, &fStrAdd);
	}
		...
		else if(pdef->offAttrib==kAttribType_TeleThrow)
		{
			...
			if(pParams)
			{
							...
							&& (bShowAny || !pdefInline->bMultiAttribPower))))
				{
					...
					AutoDescPowerDefInternal(iPartitionIdx,lang,pdefInline,NULL,pAutoDescPower,NULL,NULL,NULL,eHeaderInline,pchar,NULL,NULL,iLevel,bIncludeStrength,bLocalFullDescription,iDepth+3,eDetail,pAutoDescAttribMod,pppPowerDefsDescribed,pppchFootnoteMsgKeys,pExtract, NULL);
				...
				for(i=0; i<s; i++)
				{
					...
					if(pdefCritterPower)
					{
								...
								&& (!pppPowerDefsDescribed || -1==eaFind(pppPowerDefsDescribed,pdefCritterPower))))
						{
							...
							AutoDescPowerDefInternal(iPartitionIdx,lang,pdefCritterPower,NULL,pAutoDescPower,NULL,NULL,NULL,kAutoDescPowerHeader_Full,bLocked?pchar:NULL,NULL,NULL,iLevel,bLocked?bIncludeStrength:false,bLocalFullDescription,iDepth+4,eDetail,pAutoDescAttribMod,pppPowerDefsDescribed,pppchFootnoteMsgKeys,pExtract, NULL);
							eaPush(&pAutoDescAttribMod->ppPowersInline,pAutoDescPower);
						}
					}
				}
	...
	if(pdef->pExpiration)
	{
		...
		if(eDetail>kAutoDescDetail_Minimum)
		{
			if(pdefExpiration && (bLocalFullDescription || !pppPowerDefsDescribed || -1==eaFind(pppPowerDefsDescribed,pdefExpiration)))
			{
				...
				if(bLocalFullDescription)
					eHeaderInline = kAutoDescPowerHeader_Full;
				AutoDescPowerDefInternal(iPartitionIdx,lang,pdefExpiration,NULL,pAutoDescPower,NULL,NULL,NULL,eHeaderInline,pchar,NULL,NULL,iLevel,bIncludeStrength,bLocalFullDescription,iDepth+3,eDetail,pAutoDescAttribMod,pppPowerDefsDescribed,pppchFootnoteMsgKeys,pExtract, NULL);
						  ...
						  U32 bPositive)
{
	...
	else if(iAttrib==kAttribType_Shield && bPositive)
	{
		estrStackCreate(&pchKeyCompiled);
		estrCopy2(&pchKeyCompiled,cpchKey);
		estrAppend2(&pchKeyCompiled,".Shield.Positive");
	}
...
static void ModAutoDescCustom(Entity *pEnt, Language lang, char **ppchDescription, PowerDef *pPowerDef, AutoDescAttribMod *pAutoDescAttribMod, const char* pchPowerMessageKey, const char* pchAttribModsMessageKey)
{
	if (pEnt && ppchDescription && pAutoDescAttribMod && pchPowerMessageKey)
	{
		...
		if(pAutoDescAttribMod->pchCustom)
		{
			langFormatGameMessageKey(lang, ppchDescription,pchAttribModsMessageKey,
		...
		{
			AutoDescPower *pAutoDescPower = pAutoDescAttribMod->ppPowersInline[i];
			powerdef_AutoDescCustom(pEnt, lang, &pchInlinePowers, pPowerDef, pAutoDescPower, pchPowerMessageKey, pchAttribModsMessageKey);
		}

		if(pAutoDescAttribMod->pPowerExpire)
		{
			powerdef_AutoDescCustom(pEnt, lang, &pchPowerExpire, pPowerDef, pAutoDescAttribMod->pPowerExpire, pchPowerMessageKey, pchAttribModsMessageKey);
		}
...
void powerdef_AutoDescCustom(Entity *pEnt, Language lang, char **ppchDescription, PowerDef *pPowerDef, AutoDescPower *pAutoDescPower, const char* pchPowerMessageKey, const char* pchAttribModsMessageKey)
{
	if (pEnt && pPowerDef && ppchDescription && pAutoDescPower && pchPowerMessageKey)
	{
		...
		if(!pAutoDescPower->pchCustom)
		{
			for (i = 0; i < eaSize(&pAutoDescPower->ppPowersCombo); i++)
			{
				...
				powerdef_AutoDescCustom(pEnt, lang, &pchCombo, pPowerDef, pComboPower, pchPowerMessageKey, pchAttribModsMessageKey);
			}

			for(i = 0; i<eaSize(&pAutoDescPower->ppAttribMods); i++)
			{
				AutoDescAttribMod *pAutoDescAttribMod = pAutoDescPower->ppAttribMods[i];
				ModAutoDescCustom(pEnt, lang, &pchAttribMods, pPowerDef, pAutoDescAttribMod, pchPowerMessageKey, pchAttribModsMessageKey);
			}
		...
		langFormatGameMessageKey(lang, ppchDescription, pchPowerMessageKey,
			...
			STRFMT_STRING("Type", (SAFE_MEMBER2(pEnt, pPlayer, accessLevel) >= ACCESS_GM) ? pAutoDescPower->pchType : ""),
			...
			STRFMT_STRING("SlotTypeName", NULL_TO_EMPTY(StaticDefineLangGetTranslatedMessage(lang, ItemGemTypeEnum, pAutoDescPower->eRequiredGemType))),
