...
typedef struct QueueSettings
{
	...
	U32 uMaxAFKTime;				AST(NAME(MaxAFKTime))
	// The max time a player is allowed to be in AFK mode before being kicked from the map
...
AUTO_ENUM;
typedef enum QueueSmartGroupingMode
{
	kQueueSmartGroupingMode_Off,
	kQueueSmartGroupingMode_TankHealer,
	kQueueSmartGroupingMode_TwoOfClass,
} QueueSmartGroupingMode;
...
typedef struct QueueMapRules
{
	...
	QueueSmartGroupingMode eSmartGroupingMode;	AST(NAME(SmartGroupingMode))
	// The kind of smart grouping to use for this queue. The default is "Off", allowing any group composition for this queue.
	//   Note that g_QueueConfig.bRoleClassSmartGroups and gConf.bQueueSmartGroupNNO have to be on
	//   as well as this being false to get smart grouping to work. Thie first controls this feature as a whole, the second controls how the
	//   queue joining code calculates the join criteria
...
typedef struct QueueDef
{
	...
	U32 uMinimumPointsForReward;  AST(NAME(MinimumPointsForReward))
...
AUTO_STRUCT;
typedef struct QueueLoggedEvents
{
	char *pcEventValue;				AST(NAME(EventValue) STRUCTPARAM)
	char *pcEventToLog;				AST(NAME(EventToLog) STRUCTPARAM)

}QueueLoggedEvents;

AUTO_STRUCT;
typedef struct QueueThreshold
{
	CONST_REF_TO(QueueDef) hDef;				AST(REFDICT(QueueDef) STRUCTPARAM)
	S32 iThreshold;								AST(NAME(Threshold) STRUCTPARAM)
...
typedef struct QueueConfig
{
	...
	U32 uPvPRewardScalarNormativeTime;	AST(NAME(PvpRewardScalarNormalTime) DEFAULT(0))// If non-zero, this is number of seconds that a 'normal' PvP match should take.
																					// Rewards are scaled based on this value in
																					// gslPVPGame_ProcessQueueRewardTableForEnts in gslPvPGame.c
	...
	U32 bQueuedLogging : 1;
	// log results of queues at the end. 

	U32 bQueuedRewardThreshold : 1;
	// Reward thresholds are on
	...
	EARRAY_OF(QueueLoggedEvents) eaQueueLoggedEvents;		AST(NAME(QueueLoggedEvents))
	// events we need to log

	EARRAY_OF(QueueThreshold) eaQueueThresholds;			AST(NAME(QueueThresholds))				
	// The threshold of each queue we want to prevent AFK players in

	F32 QueueThresholdMultipler;
	// The percentage of the threshold required not to be an afk player

	U32 QueueAFKPenaltyTimeSeconds;
	// number of seconds that an AFK penalty will be
...
AUTO_STRUCT AST_CONTAINER AST_IGNORE_STRUCT(eaOrderedMembers);
...
QueueMatchMember* queue_Match_AddMemberToGroup(SA_PARAM_NN_VALID QueueMatch* pMatch, SA_PARAM_OP_VALID QueueGroup* pGroup, U32 uEntID, U32 uTeamID, S32 iGroupRole, S32 iGroupClass, bool bPresetMember);
...
void queue_Match_ClearMembers(SA_PARAM_NN_VALID QueueMatch *pMatch);

/////////////////////////////////////
/// GroupCache stuff
					   
//   Separate routines for Private queues which always have no map and do not have include/exclude states
void queue_trh_PrivateGroupCache(ATH_ARG NOCONST(QueueInstance)* pInstance, SA_PARAM_NN_VALID QueueDef* pQueueDef, SA_PARAM_NN_VALID QueueMatch* pMatch);
#define queue_PrivateGroupCache(pInstance, pQueueDef, pMatch) queue_trh_PrivateGroupCache(CONTAINER_NOCONST(QueueInstance,(pInstance)),pQueueDef,pMatch)

//   Just counts the members that would have been added by doing a GroupCache
int queue_MatchMemberCountWithMap(QueueInstance *pInstance, QueueDef *pQueueDef, QueueMap *pMap);
...
// Given the def and level band (or -1 if check all level bands), return the min and max level the queue requires.
void queue_GetMinMaxLevel(QueueDef* pDef, int iLevelBand, int* piMinLevel, int* piMaxLevel);
...
// Find the level band for this queue for a given Entity
int queue_GetQueueLevelBandIndex(Entity *pEnt, QueueDef *pQueueDef);
