...
static void CharacterActCancelPowerRef(SA_PARAM_NN_VALID Character *pchar, 
										...
										U32 uiActivationID,
										S32 bDisable,
										S32 bAllowLeavingIndividualMods)
{
	if(ppowerRef->uiID)
	{
		character_CancelModsFromPowerID(pchar, ppowerRef->uiID, er, uiActivationID, bDisable, bAllowLeavingIndividualMods);
	}
	else
	{
		...
		if(pDef)
		{
			character_CancelModsFromDef(pchar, pDef, er, uiActivationID, bDisable, bAllowLeavingIndividualMods);
		}
	}
}
									...
									S32 bAllowLeavingIndividualMods)
{
	...
	CharacterActCancelPowerRef(pchar, &pact->ref, er, pact->uiIDServer, bDisable, bAllowLeavingIndividualMods);
	CharacterActCancelPowerRef(pchar, &pact->preActivateRef, er, 0, bDisable, bAllowLeavingIndividualMods);
	for(j=eaSize(&pact->ppRefEnhancements)-1; j>=0; j--)
	{
		CharacterActCancelPowerRef(pchar, pact->ppRefEnhancements[j], er, pact->uiIDServer, bDisable, bAllowLeavingIndividualMods);
	}
	...
	for(i=eaiSize(&pact->perTargetsHit)-1; i>=0; i--)
	{
		if(er!=pact->perTargetsHit[i])
		{
			...
			if(pcharTarget)
			{
				CharacterActCancelPowerRef(pcharTarget, &pact->ref, er, pact->uiIDServer, bDisable, bAllowLeavingIndividualMods);
				CharacterActCancelPowerRef(pcharTarget, &pact->preActivateRef, er, 0, bDisable, bAllowLeavingIndividualMods);
				for(j=eaSize(&pact->ppRefEnhancements)-1; j>=0; j--)
				{
					CharacterActCancelPowerRef(pcharTarget, pact->ppRefEnhancements[j], er, pact->uiIDServer, bDisable, bAllowLeavingIndividualMods);
				}
			}
		}
	}
...
static void powerAct_ApplyPowerDefRotations(PowerDef *pPowerDef, Vec3 vDirInOut)
{
	Vec3 vNormal;
	copyVec3(vDirInOut, vNormal);
	normalVec3(vNormal);

	if(pPowerDef->fPitch)
	{
		Mat3 xMat;
		orientMat3(xMat, vNormal);
	...
	if(pPowerDef->fYaw)
	{
		Mat3 xMat;
		orientMat3(xMat, vNormal);
		yawMat3(RAD(pPowerDef->fYaw), xMat);
		copyVec3(xMat[2], vDirInOut);
	}
}
									...
									Vec3 vDirInOut)
{
	...
	powerAct_ApplyPowerDefRotations(pPowerDef, vDirInOut);
}
									  ...
									  SA_PARAM_NN_VALID S32 *pbPowerChangedOut)
{
	...
	*pbPowerChangedOut = false;
	...
	CharacterActCancelPowerRef(pchar, &pact->preActivateRef, entGetRef(pchar->pEntParent), 0, false, true);
	return true;
}
			...
			*peModeInOut = kChargeMode_CurrentMaintain;
			...
			if(pdefCurrent->bInstantDeactivation)
			{
				...
				if(ppow)
				{
					...
					character_DeactivateMaintained(iPartitionIdx, pchar, ppow, pact, pafx, uiTimeOfEvent, true);
				}
	...
	if(pact && (!uchIDRequired || pact->uchID==uchIDRequired))
	{
		...
		if(pdef && (eType == kAttribType_Null || !powerdef_IgnoresAttrib(pdef,eType)) )
		{
			...
			if(bNotify)
			{
				...
				ClientCmd_PowersCancelPowers(pchar->pEntParent, pact->uchID, true, false, 
											 pact->ref.uiID, pact->ref.iIdxSub, pact->ref.iLinkedSub);
#elif GAMECLIENT
			...
			CharacterActCancelPowerRef(pchar, &pact->preActivateRef, entGetRef(pchar->pEntParent), 0, false, false);
...
S32 character_ActivationHasReachedHit(SA_PARAM_NN_VALID PowerActivation *pact)
{
	...
	// we are offsetting the hit time check by a combatTick to account for a small amount of error with attribMods
	// going current on the same frame as the activation 
	// this fixes the problem of canceling a power at the right time, but the attribMods have already been processed, but then
	// powers canceling because it thinks it hasn't gotten to the actual hitframe.
	return (MAX(pact->fTimeActivating, 0.f) + fTimeActAdjust) >= (pact->fActHitTime - gConf.combatUpdateTimer);
}
									...
									AttribType eType)
{
	...
	if(pact)
	{
				...
				|| (eType != kAttribType_Interrupt && !powerdef_IgnoresAttrib(pdef,eType))))
		{
			...
			else
			{
				...
				{
					character_DeactivateMaintained(iPartitionIdx, pchar, ppow, pact, NULL, pmTimestamp(0), bRecharge);
					pchar->eChargeMode = kChargeMode_None;
				}
			...
			if(bCancel)
			{
				...
				{
					CharacterActCancelMods(iPartitionIdx, pchar, pact, false, false);
#if GAMESERVER
				...
				// Notify the client that its power has been canceled
				ClientCmd_PowersCancelPowers(pchar->pEntParent, pact->uchID, true, bSetClientRecharge,
						  					 pact->ref.uiID, pact->ref.iIdxSub, pact->ref.iLinkedSub);
#endif
	...
	{	// check to see if we *just* finished an activation and if we did, just send the recharge time to the client
		...
		Power *ppow = character_FindPowerByRefParams(pchar, pchar->pPowActFinished->ref.uiID, 0, 
														pchar->pPowActFinished->ref.iLinkedSub );
		...
		if (ppow)
		{
			ClientCmd_PowerSetRechargeClient(pchar->pEntParent, pchar->pPowActFinished->ref.uiID, 
											 pchar->pPowActFinished->ref.iLinkedSub, ppow->fTimeRecharge);
		}
	}
#endif
	// Make sure we clean up the ChargeMode regardless
...
#if GAMESERVER
				eventsend_RecordPowerActivationEnded(pchar->pEntParent, entFromEntityRef(iPartitionIdx, pact->erTarget), pdef);
#endif
			}
			else
			{
								  ...
								  bool *pbGaveFeedbackOut)
{
	...
	if(SAFE_MEMBER(pchar->pattrBasic, fDisable) > 0.0f 
		...
		&& (!bActiveToggle || pdef->bDeactivatesPowerOnDisable) )
	{
		if (!bTest)
		{
...
static bool character_IsPowerCombatPowerStateSwitchActviated(Character *pChar, Power *pTestPower, PowerActivation* pAct)
{
#if GAMESERVER || GAMECLIENT
	Power *pCurrentPower = character_ActGetPower(pChar, pAct);
	if (pCurrentPower && pCurrentPower->pParentPower)
		pCurrentPower = pCurrentPower->pParentPower;

	if (pCurrentPower && pCurrentPower != pTestPower)
	{
		Power *pTestBasePower = CombatPowerStateSwitching_GetBasePower(pTestPower);
		Power *pCurrentBasePower = CombatPowerStateSwitching_GetBasePower(pCurrentPower);
		if (pTestBasePower && pTestBasePower == pCurrentBasePower)
		{
			return true;
		}
	}
							...
							
{
	...
	if (pchar->pPowActCurrent)
	{
		Power *pTestPower = ppow;
		if (pTestPower->pParentPower)
			pTestPower = pTestPower->pParentPower;

		if ( (pchar->pPowActCurrent && character_IsPowerCombatPowerStateSwitchActviated(pchar, pTestPower, pchar->pPowActCurrent))  || 
			 (pchar->pPowActQueued && character_IsPowerCombatPowerStateSwitchActviated(pchar, pTestPower, pchar->pPowActQueued))  || 
			 (pchar->pPowActOverflow && character_IsPowerCombatPowerStateSwitchActviated(pchar, pTestPower, pchar->pPowActOverflow)))
		{
	...
	if (!CombatPowerStateSwitching_CanQueuePower(pchar))
	{
		PowersDebugPrintEnt(EPowerDebugFlags_ACTIVATE, pchar->pEntParent, "ActTestStatic: FAIL: Cannot Queue due to powerStateSwitching\n");
		SetActivationFailureReason(peFailOut,kActivationFailureReason_Other);
		return 0;
	}
#endif
	...
	if(pdef)
	{
		...
		if(pafx)
		{
			...
			if(pdef->eType!=kPowerType_Passive)
			{
				...
				character_AirFrictionOverrideStop(pchar, pact, uiTimeAnim);
			}
										...
										U32 uiTimeAnim)
{
	...
	if(pdef)
	{
		...
		{
			CharacterActCancelMods(iPartitionIdx, pchar, pact, true, true);
		}
		else if(!pdef->bDeactivationLeavesMods)
		{
			CharacterActCancelMods(iPartitionIdx, pchar, pact, false, true);
		}
	}
}
								...
								int bRecharge)
{
	...
	if(ppow && pchar->pEntParent && !entIsAlive(pchar->pEntParent))
	{
		PowerDef *pDef = GET_REF(ppow->hDef);
		if(pDef && pDef->bNoDeactivateOnDeath)
		{
			PERFINFO_AUTO_STOP();
			return;
		}
										...
										U32 uiTimeAnim)
{
	...
	{
		character_DeactivateMaintained(iPartitionIdx, pChar, pPow, pAct, pafx, uiTimeAnim, true);
	}
}
							...
							S32 *pbRangeOut)
{
	...
	if(ppowertarget->bRequireSelf ||
		(SAFE_MEMBER2(pentSource, pChar, bUseCameraTargeting) &&
		...
		!ISZEROVEC3(vecTarget))
	{
		...
		*ppentTargetOut = pentSource;
		...
		else
		{
			...
			{	// this case is usually used for critters
				...
				// if the power has the AITag to UseTargetPos, then derive the direction from the target position if it is valid
				// otherwise use our source direction.

				// if this code starts being used for players, this should stop relying on the AITags (This is somewhat kludgy)

				if (ISZEROVEC3(vecTarget) || !(pdef->eAITags & kPowerAITag_UseTargetPos))
				{	// use the source's current direction to get the vecTarget
				...
				else
				{
					Vec3 vSelfCombatPos;
					F32 fLength;
					// the VecTarget is a valid vector, get our position and get the direction to it
					entGetCombatPosAtTime(pentSource, uiTimestampClientView, vSelfCombatPos);
					subVec3(vecTarget, vSelfCombatPos, vecSourceDirFinal);
					
					fLength = normalVec3(vecSourceDirFinal);
										
					if (fLength <= FLT_EPSILON)
					{
						// zero vector, this probably targeting itself? 
						// we're going to use our source direction
						copyVec3(vecSourceDir, vecSourceDirFinal);
					}
				}
		...
		*ppentTargetOut = entFromEntityRef(iPartitionIdx,erTarget);
		...
		if(*ppentTargetOut)
		{
			...
			if (fRange && (pdef->eEffectArea == kEffectArea_Cylinder ||  pdef->eEffectArea == kEffectArea_Cone))
			{
				...
				powerAct_ApplyPowerDefRotations(pdef, vDirToTarget);
