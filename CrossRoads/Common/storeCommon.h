...
typedef struct StoreDef
{
    ...
    // Sometimes we want the same discounts on multiple stores. However, discounts are defined on storedefs, so to
    // add discount inheritance without inheriting the entire store, we inherit the discountdefs from another store
    REF_TO(StoreDef) inheritDiscountsStoreDef;
...
LATELINK;
bool store_VendorValidations(StoreDef *pStore);
