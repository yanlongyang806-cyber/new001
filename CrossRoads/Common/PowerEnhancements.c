													...
													PowerDef *pPowDef, Power ***pppAttachedOut)
{
	...
	if (pPowDef->eType != kPowerType_Enhancement && !pPowDef->bNeverAttachEnhancements)
	{
		...
		FOR_EACH_IN_EARRAY(pChar->ppPowers, Power, pEnhancementPower)
		{
			...
			if (pEnhancementDef && pEnhancementDef->eType == kPowerType_Enhancement && 
				pEnhancementDef->eEnhancementAttachUnowned != kEnhancementAttachUnownedType_Never &&
													...
													PowerDef *pPowerDef, Power ***peaAttachedOut)
{
	...
	if (pMod->uiPowerID)
	{
		...
		{	// we have the power so check if there is any attribs that have enhancements
			...
			// Get local enhancements for the apply power
			if(pPower->eSource==kPowerSource_Item || (pPower->pParentPower && pPower->pParentPower->eSource==kPowerSource_Item))
			{
				eaClearFast(&s_ppEnhancementsLocal);
				power_GetEnhancementsLocalItem(pChar->pEntParent, pPower, &s_ppEnhancementsLocal);
				power_AttachEnhancementsLocalApplyPower(iPartitionIdx, pPowerDef, pChar, s_ppEnhancementsLocal, peaAttachedOut);
			}
		}
	}
	else if (pPowerDef)
	{
		power_GetEnhancementsForUnownedPower(iPartitionIdx, pChar, pPowerDef, peaAttachedOut);
	}
...
// Filters the list of Local Enhancements to ones that can actually attach, and put them into the
//  attach list for an unowned apply power.
void power_AttachEnhancementsLocalApplyPower(int iPartitionIdx, PowerDef *pDef, Character *pchar, Power **ppEnhancementsLocal, Power ***pppAttachedOut)
{
	int i;
	for(i=eaSize(&ppEnhancementsLocal)-1; i>=0; i--)
	{
		PowerDef *pdefAttach = GET_REF(ppEnhancementsLocal[i]->hDef);
		if(pdefAttach && pDef && pdefAttach->eEnhancementAttachUnowned != kEnhancementAttachUnownedType_Never &&
			(pdefAttach->eEnhancementAttachUnowned != kEnhancementAttachUnownedType_CheckAttachExpr ||
			power_EnhancementAttachIsAllowed(iPartitionIdx,pchar,pdefAttach,pDef,false)))
		{
			eaPush(pppAttachedOut,ppEnhancementsLocal[i]);
...
// to be used in conjunction with power_GetEnhancementsForAttribModApplyPower to get the actual character 
// we should be checking for unattached enhancements
Character* power_GetPowerEnhancementSource(Character *pModSource, Character *pModOwner)
{
	if (!pModSource)
		return pModOwner;
	...
	if (pModSource->pEntParent && pModOwner->pEntParent && 
		pModSource->pEntParent->erOwner == entGetRef(pModOwner->pEntParent))
	{	// this is coming from an entity that the modOwner owns, 
		// use the pModSource entity instead
		return pModSource;
	}
