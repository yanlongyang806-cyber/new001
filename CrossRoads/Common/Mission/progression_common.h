...
// Details about a zone rewards, for when you're done with missions
AUTO_STRUCT;
typedef struct GameProgressionZoneRewards
{
	// String reference to the ZoneRewardsDef
	const char* pchZoneRewardsDef;			AST(NAME(ZoneRewardsDef) RESOURCEDICT(ZoneRewardsDef) STRUCTPARAM POOL_STRING)

	// description
	DisplayMessage msgDescription;			AST(NAME(Description) STRUCT(parse_DisplayMessage))

	// Image for this ZoneRewards
	const char* pchImage;					AST(NAME(Image) POOL_STRING)

} GameProgressionZoneRewards;
...
typedef struct GameProgressionNodeMissionGroupInfo
{
	...
	// A ZoneRewardsDef to offer if all the missions are done.
	GameProgressionZoneRewards *pZoneRewardsDef;	AST(NAME(ZoneRewardsDef))
