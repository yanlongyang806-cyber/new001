...
AUTO_STRUCT AST_IGNORE("animBits") AST_FIXUPFUNC( fixupDialogBlock );
typedef struct DialogBlock
{
	...
	DisplayMessageWithVO displayTextMesg;	AST(NAME(DisplayTextMesg) STRUCT(parse_DisplayMessageWithVO))
	...
	char* audioName_Deprecated;	AST( NAME("Audio", "Audio:") POOL_STRING)
...
AUTO_STRUCT;
typedef struct NameNamePair
{
	const char* pchName;						AST(POOL_STRING)
	const char* pchAltName;						AST(POOL_STRING)

}NameNamePair;

AUTO_STRUCT;
typedef struct PetContactListAltClassNames
{
	EARRAY_OF(NameNamePair)	eaNames;

}PetContactListAltClassNames;
...
AUTO_STRUCT;
typedef struct ContactCostumeStyle
{
	// What texture, if any, to display as a background for the headshot
	const char* pchBackground;	AST(NAME("BackgroundImage") POOL_STRING)
...
typedef struct ContactCostume 
{
		...
		// Headshot Style
		const char* pchHeadshotStyle;					AST(NAME("HeadshotStyleDef", "HeadshotStyleOverride") POOL_STRING)

		// Headshot Style Overrides
		ContactCostumeStyle* pCostumeStyleOverrides;	AST(NAME("CostumeStyleOverrides"))

		// Frame border
		const char* pchHeadshotBorderFrame;				AST(NAME("HeadshotBorderFrame") POOL_STRING)
...
typedef struct SpecialDialogBlock
{
	...
	ContactCostume** extraCostumes;			AST(NAME("ExtraCostume"))
...
{ 
	...
	// TEMP HACK FIELD: event logical name, because the UI needs this and I can't finish a clean way in time :(
	const char* eventName;				AST(NAME(EventName) POOL_STRING)
	...
	// If this ImageMenuItem is an override added by a mission, this field stores the overriding mission.
	REF_TO(MissionDef) overridingMissionDef;	AST(NO_TEXT_SAVE)
...
typedef struct ContactDef
{
	...
	// VO Comments, not used by the game
	char* strVOComments;						AST(NAME(VOComments) SERVER_ONLY)
...
//	ContactMissionSeries** seriesList;			AST(NAME("MissionSeries:"))
	...
	// Only when contact flags has ZoneRewards. This field indicates which StickerBookCollection should be opened when the Zone Rewards Option in the contact is chosen.
	REF_TO(StickerBookCollection) hStickerBookCollection;	AST(NAME("StickerBookCollection") REFDICT(StickerBook))
...
typedef struct ContactConfig
{
	...
	U32 bEnableLipSync : 1; AST(NAME(EnableLipSync))
		// Have the contact dialog provide information to the audio system required for lip syncing
} ContactConfig;
...
typedef enum ContactDialogState
{
	...
	ContactDialogState_ZoneRewards,
...
typedef enum ContactScreenType
{
	...
	ContactScreenType_ZoneRewardsInfo,
...
typedef struct ContactDialogOption
{
	...
	F32 yPos;							AST( NAME(YPos))
		//these is used by the client to add some UI frills like "you are here".
	const char* pchEventName;			AST( NAME(EventName) POOL_STRING)
...
AUTO_STRUCT;
typedef struct ContactDialogCostume
{
	...
	const char* pchHeadshotStyleDef;	AST( POOL_STRING )
	const char* pchHeadshotBorderFrame;	AST( POOL_STRING )
...
typedef struct ContactDialog
{
	...
	// Headshot
	ContactDialogCostume headshot;						AST(EMBEDDED_FLAT NAME(Headshot))

	// Alternate costumes
	ContactDialogCostume **eaExtraHeadshots;
	...
	REF_TO(StickerBookCollection) hStickerBookCollection;	 AST(NAME("StickerBookCollection"))
