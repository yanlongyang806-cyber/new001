...
bool gameaction_ValidateActions(WorldGameActionProperties ***actions, const char *pchSourceMapName, MissionDef *pRootMissionDef, MissionDef *pMissionDef, bool bAllowMapVariables, const char *pchFilename)
{
	...
	for (i = eaSize(actions) - 1; i >= 0; --i)
	{
		...
		else if (action->eActionType == WorldGameActionType_SendNotification)
		{
			...
			{
				if (IS_HANDLE_ACTIVE(action->pSendNotificationProperties->notifyMsg.msg.hMessage) && !GET_REF(action->pSendNotificationProperties->notifyMsg.msg.hMessage))
				{
					ErrorFilenamef(pchFilename, "Send Notification action references unknown message (%s)", REF_STRING_FROM_HANDLE(action->pSendNotificationProperties->notifyMsg.msg.hMessage));
					result = false;
				}
			}
		}
				...
				if (action->pWarpProperties->warpDest.eDefaultType == WVARDEF_SPECIFY_DEFAULT && action->pWarpProperties->warpDest.pSpecificValue)
				{
					...
					{ 
						...
						if (pcZoneMap)
						{
							...
							else if (action->pWarpProperties->bSinglePlayer && zmapInfoGetMapType(pZoneMap) != ZMTYPE_OWNED)
							{
								ErrorFilenamef(pchFilename, "Warp action is single player but '%s' is not an Owned map.", pcZoneMap);
							}
						}
