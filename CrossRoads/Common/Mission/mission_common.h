...
typedef struct MissionWaypoint
{
	...
	// Alternate maps -- if you are not able to reach mapName, then
	// try to go anywhere on this map instead.
	//
	// NOTE: Right now we just support a single alternate map, as that
	// is enough for Neverwinter.  Two ideas make sense for
	// generalizing this, either do pathfinding on a map graph to get
	// to this map -OR- make this an array of maps to go to.
	const char* astrAlternateMapName;		AST(POOL_STRING NAME(AlternateMapName))
...
typedef struct MissionDefParams
{
	...
	// Open Mission Success messages
	DisplayMessage RewardMsgGold;				AST(STRUCT(parse_DisplayMessage))
	DisplayMessage RewardMsgSilver;				AST(STRUCT(parse_DisplayMessage))
	DisplayMessage RewardMsgBronze;				AST(STRUCT(parse_DisplayMessage))
	DisplayMessage RewardMsgDefault;			AST(STRUCT(parse_DisplayMessage))

	// Open Mission Failure messages
	DisplayMessage RewardMsgFailureGold;		AST(STRUCT(parse_DisplayMessage))
	DisplayMessage RewardMsgFailureSilver;		AST(STRUCT(parse_DisplayMessage))
	DisplayMessage RewardMsgFailureBronze;		AST(STRUCT(parse_DisplayMessage))
	DisplayMessage RewardMsgFailureDefault;		AST(STRUCT(parse_DisplayMessage))
...
AUTO_STRUCT;
typedef struct MissionDisplayOverride
{
	// Long description of mission with flavor text
	DisplayMessageWithVO detailStringMsg;	AST(STRUCT(parse_DisplayMessageWithVO)) 
	...
	char *pchSoundOnContactOffer_Deprecated;	AST(NAME("SoundOnContactOffer"))

	// Requirements for player to see this dialog
	Expression* condition;					AST(NAME("Condition") REDUNDANT_STRUCT("Condition:", parse_Expression_StructParam) LATEBIND)
} MissionDisplayOverride;

// Static Definition of a mission;
AUTO_STRUCT AST_IGNORE("IconString:") AST_IGNORE("NextActionUniqueID") AST_FIXUPFUNC( fixupMissionDef );
typedef struct MissionDef
{
	...
	// If set, this means to act as if eaObjectiveMaps has every
	// static map listed in it.
	bool bObjectiveMapsIncludesAllStaticMaps;		AST( NAME("ObjecitveMapsIncludesAllStaticMaps") )
	...
	// Sound Event Path (optional) - to be played when the mission is offered from a contact
	const char *pchSoundOnContactOffer_Deprecated;	AST( NAME("SoundOnContactOffer") POOL_STRING)
	...
	// Contact specifically offering this mission, if there is ambiguity
	const char* astrContactForOfferVO;		AST(NAME(ContactForOfferVO) POOL_STRING)
	// name of FX for splat to display
	char *pchSplatFX;

	MissionDisplayOverride **eaMissionDisplayOverride;		AST(NAME("MissionDisplayOverride"))
	...
	////  If we are Repeatable, we may have a cooldown/daily aspect to being repeatable.
	////  To determine the cooldown behaviour, first look at the IntervalMode.
	////  If it is not "Off" use the appropriate ShardInterval timing. 

		// Shard-clock-based interval timing. Use the completion time to determine availability.
		ShardIntervalTimingMode eMissionRepeatIntervalMode;		AST(NAME(IntervalMode))
	...
	//// For any cooldown type, we have a count.		
 
		// Number of times that the mission can be run before it enter cooldown. Zero is treated the same as 1.
		// Which is to say it can be done once in a cooldown interval.
		U32 uRepeatCooldownCount;				AST( NAME("RepeatCooldownCount"))

	////  Only use cooldown/daily according to the CooldownOnlyOnSuccess flag
	...
	// don't send a notify when this mission completes
	bool bSendCompletionNotify;				AST( DEFAULT(true))
...
// This mission has cooldown restrictions and can have a cooldown on it.
bool mission_HasCooldownProperties(MissionDef *pMissionDef);

// The current time is within the current cooldown block of a mission. This does not mean it is necessarily
//  On Cooldown as we may be allowed multiple completions within a single cooldown block.
bool mission_IsInCooldownBlock(MissionDef *pMissionDef, U32 uLastStartedTime, U32 uLastCompletedTime);

// Return the number of seconds left within the current repeat cooldown block. This ignores repeat count.
...
void missiondef_GetSummaryAndDetails(MissionDef* pMissionDef, Entity* pEnt, DisplayMessage* summaryMsg, DisplayMessage* detailStringMsg);

#define missiondef_GetSummary(pMissionDef, pEnt, summaryMsg) missiondef_GetSummaryAndDetails(pMissionDef, pEnt, summaryMsg, NULL)
#define missiondef_GetDetails(pMissionDef, pEnt, detailStringMsg) missiondef_GetSummaryAndDetails(pMissionDef, pEnt, NULL, detailStringMsg)
