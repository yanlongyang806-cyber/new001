...
bool missiondef_Validate(MissionDef *pDef, MissionDef *pRootDef, bool bIsChild)
{
	...
	if (IsServer() && !GET_REF(pDef->detailStringMsg.msg.hMessage) && REF_STRING_FROM_HANDLE(pDef->detailStringMsg.msg.hMessage)) {
		ErrorFilenamef( pDef->filename, "Mission refers to non-existent message '%s'", REF_STRING_FROM_HANDLE(pDef->detailStringMsg.msg.hMessage));
		bResult = false;
	}
	...
	n = eaSize(&pDef->eaMissionDisplayOverride);
	for (i=0; i < n; i++) {
		MissionDisplayOverride *pOverride = pDef->eaMissionDisplayOverride[i];
		if (!pOverride->condition) {
			ErrorFilenamef( pDef->filename, "Mission has a Mission Display Override with no condition");
			bResult = false;
		}

		if (IsServer() && !GET_REF(pOverride->detailStringMsg.msg.hMessage) && REF_STRING_FROM_HANDLE(pOverride->detailStringMsg.msg.hMessage)) {
			ErrorFilenamef( pDef->filename, "Mission Display Ovberride refers to non-existent detail message '%s'", REF_STRING_FROM_HANDLE(pDef->detailStringMsg.msg.hMessage));
			bResult = false;
		}

		if (IsServer() && !GET_REF(pOverride->summaryMsg.hMessage) && REF_STRING_FROM_HANDLE(pOverride->summaryMsg.hMessage)) {
			ErrorFilenamef( pDef->filename, "Mission Display Ovberride refers to non-existent summary message '%s'", REF_STRING_FROM_HANDLE(pDef->summaryMsg.hMessage));
			bResult = false;
		}
	}
	...
	if (pDef->params) {
		...
		if (IsServer() && pDef->params->OnsuccessRewardTableName && (pTable = RefSystem_ReferentFromString(g_hRewardTableDict, pDef->params->OnsuccessRewardTableName))) {
			...
			// Clickable rewards are always wrong
			if (rewardTable_HasItemsWithType(pTable, kRewardPickupType_Clickable, RewardContextType_SubMissionTurnIn)) {
	...
	if (!pDef->repeatable)
	{
		if (pDef->fRepeatCooldownHours > 0.0f ||
			pDef->fRepeatCooldownHoursFromStart > 0.0f ||
			pDef->eMissionRepeatIntervalMode!=kShardIntervalTimingMode_Off)
		{
			ErrorFilenamef(pDef->filename, "Mission %s has a Repeat Cooldown or Interval but is not repeatable!", pDef->name);
			bResult = false;
		}
	}
	else
	{
		...
		if ((pDef->fRepeatCooldownHours != 0.0f ||
			...
			pDef->eMissionRepeatIntervalMode!=kShardIntervalTimingMode_Off)
		{
			ErrorFilenamef(pDef->filename, "Mission %s has a Repeat Cooldown and Shard Interval Timing!", pDef->name);
			bResult = false;
		}
	}
	...
	if (pDef->bDisableCompletionTracking) {
		...
		if (pDef->fRepeatCooldownHours != 0 || pDef->fRepeatCooldownHoursFromStart != 0 || pDef->eMissionRepeatIntervalMode!=kShardIntervalTimingMode_Off) {
			ErrorFilenamef(pDef->filename, "Mission %s's completion tracking is disabled, but it has a cooldown or interval specified.  Since completion tracking is disabled, there is no way to track how much time has passed since the mission was completed.", pDef->name);
			bResult = false;
		}
	}
	...
	//Check to make sure handoff missions are not able to be shared
	if ( pDef->bIsHandoff && pDef->eShareable != MissionShareableType_Never) {
		ErrorFilenamef(pDef->filename, "Mission is both shareable and a handoff mission, this is a potential exploit and is not allowed.");
		bResult = false;
	}

	// Check for external mission offers
	if( gConf.bOnlyOneContactPerMission ) {
		// All mission overrides must be for this mission
		FOR_EACH_IN_EARRAY( pDef->ppMissionOfferOverrides, MissionOfferOverride, pOverride ) {
			if( !pOverride || !pOverride->pMissionOffer ) {
				continue;
			}

			if( stricmp( REF_STRING_FROM_HANDLE( pOverride->pMissionOffer->missionDef ), pDef->name ) != 0 ) {
				ErrorFilenamef( pDef->filename, "Mission: %s -- Mission has offer override that is not for this mission.  Validation can not guarantee that there is only one contact that offers a mission in this case.",
								pDef->name );
			}
		} FOR_EACH_END;


		// There must be just one offering contact (with an exception
		// for tutorial and/or flashback offers)
		if( pDef->astrContactForOfferVO ) {
			bool contactForOfferVOFound = false;
			FOR_EACH_IN_EARRAY( pDef->ppMissionOfferOverrides, MissionOfferOverride, pOverride ) {
				if( !pOverride || !pOverride->pMissionOffer ) {
					continue;
				}
				...
				if(   pOverride->pMissionOffer->allowGrantOrReturn == ContactMissionAllow_GrantOnly
					...
					if( pOverride->pcContactName == pDef->astrContactForOfferVO ) {
						contactForOfferVOFound = true;
						break;
					}
				}
			} FOR_EACH_END;

			if( !contactForOfferVOFound ) {
				ErrorFilenamef( pDef->filename, "Mission: %s -- Mission claims to use VO for offer text of contact %s, but that contact does not offer this mission.",
								pDef->name, pDef->astrContactForOfferVO );
			}
		} else {
			const char* offerContact = NULL;
			FOR_EACH_IN_EARRAY( pDef->ppMissionOfferOverrides, MissionOfferOverride, pOverride ) {
				if( !pOverride || !pOverride->pMissionOffer ) {
					continue;
				}
				...
				if(   pOverride->pMissionOffer->allowGrantOrReturn == ContactMissionAllow_GrantOnly
					  || pOverride->pMissionOffer->allowGrantOrReturn == ContactMissionAllow_GrantAndReturn ) {
					if( !offerContact ) {
						offerContact = RefSystem_ReferentFromString( g_ContactDictionary, pOverride->pcContactName );
					} else {
						ErrorFilenamef( pDef->filename, "Mission: %s -- Mission has at least two contacts that offer this mission.  This is not allowed.",
										pDef->name );
					}
				}
			} FOR_EACH_END;
...
TextParserResult fixupMissionDef( MissionDef* pDef, enumTextParserFixupType eType, void* pExtraData )
{
	switch( eType ) {
		xcase FIXUPTYPE_POST_TEXT_READ:
			if( pDef->pchSoundOnContactOffer_Deprecated ) {
				pDef->detailStringMsg.bHasVO = true;
				pDef->detailStringMsg.astrLegacyAudioEvent = allocAddString( pDef->pchSoundOnContactOffer_Deprecated );
				pDef->pchSoundOnContactOffer_Deprecated = NULL;
			}

			FOR_EACH_IN_EARRAY_FORWARDS( pDef->eaMissionDisplayOverride, MissionDisplayOverride, pOverride ) {
				if( pOverride->pchSoundOnContactOffer_Deprecated ) {
					pOverride->detailStringMsg.bHasVO = true;
					pOverride->detailStringMsg.astrLegacyAudioEvent = allocAddString( pOverride->pchSoundOnContactOffer_Deprecated );
					pOverride->pchSoundOnContactOffer_Deprecated = NULL;
				}
			} FOR_EACH_END;
	}
...
int missiondef_PostProcessFixup(MissionDef *pDef, MissionDef *pParentDef)
{
	...
	// Generate all of the display override expressions. This needs to happen on
	// the client and the server, since this expression is run on both
	n = eaSize(&pDef->eaMissionDisplayOverride);
	for (i = 0; i < n; ++i) {
		if (pDef->eaMissionDisplayOverride[i]->condition) {
			exprGenerate(pDef->eaMissionDisplayOverride[i]->condition, pRequiresContext);
		}
	}
...
void missiondef_CleanSubmission(MissionDef *pDef)
{
	...
	if (IS_HANDLE_ACTIVE(pDef->detailStringMsg.msg.hMessage)) {
		REMOVE_HANDLE(pDef->detailStringMsg.msg.hMessage);
	}
	...
	n = eaSize(&pDef->eaMissionDisplayOverride);
	for (i=0; i<n; i++){
		if (IS_HANDLE_ACTIVE(pDef->eaMissionDisplayOverride[i]->detailStringMsg.msg.hMessage)) {
			REMOVE_HANDLE(pDef->eaMissionDisplayOverride[i]->detailStringMsg.msg.hMessage);
		}
	}
#ifndef NO_EDITORS
	...
	if (pDef->detailStringMsg.msg.pEditorCopy && pDef->detailStringMsg.msg.pEditorCopy->pcDefaultString) {
		StructFreeString(pDef->detailStringMsg.msg.pEditorCopy->pcDefaultString);
		pDef->detailStringMsg.msg.pEditorCopy->pcDefaultString = NULL;
	}
	...
	for (i=0; i<n; i++){
		if (pDef->eaMissionDisplayOverride[i]->detailStringMsg.msg.pEditorCopy && pDef->eaMissionDisplayOverride[i]->detailStringMsg.msg.pEditorCopy->pcDefaultString) {
			StructFreeString(pDef->eaMissionDisplayOverride[i]->detailStringMsg.msg.pEditorCopy->pcDefaultString);
			pDef->eaMissionDisplayOverride[i]->detailStringMsg.msg.pEditorCopy->pcDefaultString = NULL;
		}
	}
#endif
	pDef->missionType = 0;
...
ExprFuncTable* missiondef_CreateExprFuncTable(void)
{
	...
	if (!s_missionFuncTable) {
		s_missionFuncTable = exprContextCreateFunctionTable("MissionDef");
...
ExprFuncTable* missiondef_CreateRequiresExprFuncTable(void)
{
	...
	if (!s_missionRequiresFuncTable) {
		s_missionRequiresFuncTable = exprContextCreateFunctionTable("MissionDef_Requires");
...
ExprFuncTable* missiondef_CreateOpenMissionExprFuncTable(void)
{
	...
	if (!s_openMissionFuncTable) {
		s_openMissionFuncTable = exprContextCreateFunctionTable("OpenMission");
...
ExprFuncTable* missiondef_CreateOpenMissionRequiresExprFuncTable(void)
{
	...
	if (!s_openMissionRequiresFuncTable) {
		s_openMissionRequiresFuncTable = exprContextCreateFunctionTable("OpenMissionRequires");
...
bool missiondef_HasObjectiveOnMap(MissionDef *pDef, const char *pcMapName, const char *pcMapVars) 
{
	...
	if (pDef && pcMapName && pDef->eaObjectiveMaps) {
		...
		if (pDef->bObjectiveMapsIncludesAllStaticMaps) {
			ZoneMapInfo* zmapInfo = zmapInfoGetByPublicName( pcMapName );
			if (zmapInfo && zmapInfoGetMapType(zmapInfo) == ZMTYPE_STATIC) {
				return true;
			}
		}
	}
...
static U32 mission_GetExplicitCooldownBlockStart(U32 uTimeToCheck, U32 uCoolDownHours, bool bUseBlockTime)
{
	if (uCoolDownHours > 0 && bUseBlockTime) {	
		...
		if (uCoolDownHours >= 12 && uCoolDownHours % 12 == 0) {
			...
			if (uBlockSeconds > uTimeToCheck) {
				// use previous block (unless we're backed up against zero)
				if (uBlockSeconds > uCoolDownHours * SECONDS_PER_HOUR)
				{
					uBlockSeconds -= uCoolDownHours * SECONDS_PER_HOUR;
				}
				else
				{
					// Not quite right, but will at least return that the TimeToCheck is within the block.
					uBlockSeconds=0;
				}
			}
		}
...
bool mission_HasCooldownProperties(MissionDef *pMissionDef)
{
	...
	if (pMissionDef->fRepeatCooldownHours || pMissionDef->fRepeatCooldownHoursFromStart)
	{
		// Old-style explicit cooldowns
		return(true);
	}

	if (pMissionDef->eMissionRepeatIntervalMode!=kShardIntervalTimingMode_Off)
	{
		return(true);
	}
...
// How long until the cooldown is gone
U32 mission_CooldownSecondsLeft(MissionDef *pMissionDef, U32 uLastStartedTime, U32 uLastCompletedTime)
{
	U32 uTimeRemaining = 0;
    U32 curTime = ShardClock_SecondsSince2000();

	if (pMissionDef->eMissionRepeatIntervalMode==kShardIntervalTimingMode_Off)
	{
		// Old explicit timing blocks
		U32 uSecondsSinceBlockStart = 0;
	
		if (pMissionDef->fRepeatCooldownHours > 0)
		{
			uSecondsSinceBlockStart = curTime - mission_GetExplicitCooldownBlockStart(uLastCompletedTime,
																						pMissionDef->fRepeatCooldownHours,
																						pMissionDef->bRepeatCooldownBlockTime);

			uTimeRemaining = max(uTimeRemaining, ((U32)(pMissionDef->fRepeatCooldownHours * 3600.0f) - uSecondsSinceBlockStart));
		}

		if (pMissionDef->fRepeatCooldownHoursFromStart > 0)
		{
			uSecondsSinceBlockStart = curTime - mission_GetExplicitCooldownBlockStart(uLastStartedTime,
																						pMissionDef->fRepeatCooldownHours,
																						pMissionDef->bRepeatCooldownBlockTime);
	...
	{
		U32 uNextIntervalStart = ShardIntervalTiming_TimeOfNextInterval(pMissionDef->eMissionRepeatIntervalMode);
		if (uNextIntervalStart > curTime)
		{
			uTimeRemaining = uNextIntervalStart - curTime;
		}
		else
		{
			// Safety? Shouldn't really be a possibility since the TimeOfNextInterval uses ShardClock.
			uTimeRemaining = 0;
		}
	}
...
// This now wraps the old GetCooldownBlockStart and also adds in the newer ShardInterval stuff (which is always based on completion time)
// This only returns that the current time is within the most recent cooldown block. (Which is to say that the mission has been completed
// or started within the same interval as the current time).  This function does not deal with repeat counts.
bool mission_IsInCooldownBlock(MissionDef *pMissionDef, U32 uLastStartedTime, U32 uLastCompletedTime)
{
	bool bInCooldown = false;

	// Note: We should deal properly with 0 as the last started time or last completed time
	...
	if (pMissionDef->eMissionRepeatIntervalMode==kShardIntervalTimingMode_Off)
	{
	    ...
	    U32 curTime = ShardClock_SecondsSince2000();
		bool bCooldownBlockTime = pMissionDef->bRepeatCooldownBlockTime;
		U32 uStartedBlockTime = mission_GetExplicitCooldownBlockStart(uLastStartedTime, pMissionDef->fRepeatCooldownHoursFromStart, bCooldownBlockTime);
		U32 uCompletedBlockTime = mission_GetExplicitCooldownBlockStart(uLastCompletedTime, pMissionDef->fRepeatCooldownHours, bCooldownBlockTime);

		F32 fStartedHoursAgo = (F32)(curTime - uStartedBlockTime)/3600.f;
		F32 fCompletedHoursAgo = (F32)(curTime - uCompletedBlockTime)/3600.f;

		if (pMissionDef->fRepeatCooldownHours > 0.0f && fCompletedHoursAgo < pMissionDef->fRepeatCooldownHours)
		{
			// still in cooldown window
			bInCooldown = true;
		}

		if (pMissionDef->fRepeatCooldownHoursFromStart > 0.0f && fStartedHoursAgo < pMissionDef->fRepeatCooldownHoursFromStart)
		{
			// still in cooldown window
			bInCooldown = true;
		}
	...
	{
		// New shard interval stuff. Always use completion time

		bInCooldown = ShardIntervalTiming_TimeIsInCurrentInterval(uLastCompletedTime, pMissionDef->eMissionRepeatIntervalMode);
	}
...
const MissionCooldownInfo *mission_GetCooldownInfoInternal(MissionDef *pMissionDef, MissionCooldown *pCooldown, CompletedMission *pCompletedMission)
{
	...
	if (mission_HasCooldownProperties(pMissionDef) && (pCompletedMission!=NULL || pCooldown!=NULL))
	{
		...
		if (mission_IsInCooldownBlock(pMissionDef, uLastStartedTime, uLastCompletedTime))
		{
			missionInfo.bIsInCooldownBlock = true;
			if (iRepeatCooldownCount >= pMissionDef->uRepeatCooldownCount)
			{
				missionInfo.bIsInCooldown = true;				
			}
			missionInfo.uCooldownSecondsLeft = mission_CooldownSecondsLeft(pMissionDef, uLastStartedTime, uLastCompletedTime);
...
ExprFuncTable* missiontemplate_CreateTemplateVarExprFuncTable()
{
	...
	if (!s_missionTemplateFuncTable) {
		s_missionTemplateFuncTable = exprContextCreateFunctionTable("MissionTemplate");
...
bool missiondef_DropMissionShouldUpdateCooldown(MissionDef* pDef)
{
	...
	{
		return(mission_HasCooldownProperties(pDef));
	}
	return false;
}
...
bool missiondef_CheckPlayerAllegiance(MissionDef* pMissionDef, Entity* pEnt)
{
...
int missiondef_EvalExpression(int iPartitionIdx, Entity *pEnt, MissionDef *pDef, Expression *pExpr)
{
...
MissionDisplayOverride* missiondef_GetApplicableMissionOverride(MissionDef* pMissionDef, Entity* pEnt)
{
	int i, n;

	n = eaSize(&pMissionDef->eaMissionDisplayOverride);
	for (i = 0; i < n; i++) {
		if (missiondef_EvalExpression(entGetPartitionIdx(pEnt), pEnt, pMissionDef, pMissionDef->eaMissionDisplayOverride[i]->condition)) {
			return pMissionDef->eaMissionDisplayOverride[i];
		}
	}
...
void missiondef_GetSummaryAndDetails(MissionDef* pMissionDef, Entity* pEnt, DisplayMessage *pSummaryMsg, DisplayMessage *pDetailStringMsg)
{
	MissionDisplayOverride* pOverride = missiondef_GetApplicableMissionOverride(pMissionDef,pEnt);

	if (pOverride) {
		if (pSummaryMsg) {
			*pSummaryMsg = pOverride->summaryMsg;
		}
		if (pDetailStringMsg) {
			*pDetailStringMsg = pOverride->detailStringMsg.msg;
		}
	} else {
		if (pSummaryMsg) {
			*pSummaryMsg = pMissionDef->summaryMsg;
		}
		if (pDetailStringMsg) {
			*pDetailStringMsg = pMissionDef->detailStringMsg.msg;
		}
	}
}
