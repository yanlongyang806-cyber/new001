...
PetContactListAltClassNames g_PetContactListAltClassNames;
...
static bool contact_ValidateDialogBlock(ContactDef *def, DialogBlock ***peaBlock)
{
	...
	for(i=eaSize(peaBlock)-1; i>=0; --i)
	{
		...
		if (!GET_REF(block->displayTextMesg.msg.hMessage) &&
			REF_STRING_FROM_HANDLE(block->displayTextMesg.msg.hMessage))
		{
			ErrorFilenamef( def->filename, "Contact references non-existent message: '%s'", REF_STRING_FROM_HANDLE(block->displayTextMesg.msg.hMessage) );
			bResult = false;
		}
	}
...
bool contact_ValidateSpecialDialogBlock(ContactDef* def, SpecialDialogBlock *block, const char* pchOwningMission, SpecialDialogOverride** eaOverridesToInclude, MissionOfferOverride **eaOfferOverridesToInclude)
{
	...
	for(j=eaSize(&block->dialogActions)-1; j>=0; --j) 
	{
		...
		// Validate extra costume prefs
		for (k = eaSize(&block->extraCostumes) - 1; k >= 0; k--)
		{
			if (REF_STRING_FROM_HANDLE(block->extraCostumes[k]->costumeOverride))
			{
				if(IsServer() && !GET_REF(block->extraCostumes[k]->costumeOverride)) {
					ErrorFilenamef(pchFileName, "Contact's special dialog block references non-existent costume: '%s'", REF_STRING_FROM_HANDLE(block->extraCostumes[k]->costumeOverride));
					bResult = false;
				} 
				if(REF_STRING_FROM_HANDLE(block->extraCostumes[k]->hPetOverride)) {
					ErrorFilenamef(pchFileName, "Contact's special dialog block specifies both a costume override and a pet contact list costume");
					bResult = false;
				}
				if(REF_STRING_FROM_HANDLE(block->extraCostumes[k]->hCostumeCritterGroup) || EMPTY_TO_NULL(block->extraCostumes[k]->pchCostumeMapVar)) {
					ErrorFilenamef(pchFileName, "Contact's special dialog block specifies both a costume override and a critter group costume");
					bResult = false;
				}
			} else if(REF_STRING_FROM_HANDLE(block->extraCostumes[k]->hPetOverride)) {
				if(IsServer() && !GET_REF(block->extraCostumes[k]->hPetOverride)) {
					ErrorFilenamef(pchFileName, "Contact's special dialog block references non-existent pet contact list: '%s'", REF_STRING_FROM_HANDLE(block->extraCostumes[k]->hPetOverride));
					bResult = false;
				}
				if(REF_STRING_FROM_HANDLE(block->extraCostumes[k]->hCostumeCritterGroup) || EMPTY_TO_NULL(block->extraCostumes[k]->pchCostumeMapVar)) {
					ErrorFilenamef(pchFileName, "Contact's special dialog block specifies both a pet contact list costume and a critter group costume");
...
/*	if (!GET_REF(def->infoStringMsg.hMessage) &&
	...
	*/
	...
	{
		StoreDef* currentStore = GET_REF(def->stores[i]->ref);
		if (!currentStore &&
		...
		if(gConf.bDisableRemoteContactSelling && 
			currentStore && 
			def->canAccessRemotely &&
			currentStore->bSellEnabled)
		{
			ErrorFilenamef( def->filename, "Contact is remote and has store with selling enabled: '%s'", REF_STRING_FROM_HANDLE(def->stores[i]->ref) );
			bResult = false;
		}
	}
	...
	// Check for explicit mission offers
	if( gConf.bOnlyOneContactPerMission ) {
		if( eaSize( &def->offerList ) != 0 ) {
			ErrorFilenamef( def->filename, "Contact: %s -- Contact has explicit mission offers.  Validation can not guarantee that there is only one contact that offers a mission in this case.",
							def->name );
		}
	} 
...
//	}
	...
	if (contact_ShowInSearchResults(def)){
		...
		if (IsServer()){
			...
			if (!pInfoDialog || !GET_REF(pInfoDialog->displayTextMesg.msg.hMessage)){
	...
	if (contact_IsZoneRewards(def))
	{
		if(!GET_REF(def->hStickerBookCollection))
		{
			ErrorFilenamef(def->filename, "Contact %s is flagged as having a Zone Rewards option, but but the Sticker Book Collection is not specified.", def->name);
			bResult = false;
		}
	}
...
static bool contact_CheckPlayerAllegiance_RequireAll(ContactMissionOffer* pMissionOffer, AllegianceDef* pEntAllegiance, AllegianceDef* pEntSubAllegiance)
{
	...
	{
		pRequiredAllegiance = GET_REF(pMissionOffer->eaRequiredAllegiances[i]->hDef);
...
bool contact_CheckPlayerAllegiance(ContactMissionOffer* pMissionOffer, Entity* pEnt)
{
	...
	if (!eaSize(&pMissionOffer->eaRequiredAllegiances) || !pEnt)
	{
		return true;
	}

	if (pMissionOffer->bRequireAllAllegiances)
	{
		return contact_CheckPlayerAllegiance_RequireAll(pMissionOffer, pEntAllegiance, pEntSubAllegiance);
	}
	else
	{
		AllegianceDef* pRequiredAllegiance = NULL;
...
void contact_GetMissionOfferList(ContactDef* pContact, Entity* pEnt, ContactMissionOffer*** peaReturnList)
{
	...
	if(pContact && peaReturnList)
	{
		...
		for (i = eaSize(&pContact->offerList)-1; i >= 0; i--) {
			if (contact_CheckPlayerAllegiance(pContact->offerList[i], pEnt)) {
				eaPush(peaReturnList, pContact->offerList[i]);
			}
		}
		// Add override offers
		for (i = eaSize(&pContact->eaOfferOverrides)-1; i >= 0; i--) {
			if (contact_CheckPlayerAllegiance(pContact->eaOfferOverrides[i], pEnt)) {
...
#ifdef GAMESERVER
		{
			...
			for (i=0; i < eaSize(&eaNamespacedOverrides); i++) {
				if (contact_CheckPlayerAllegiance(eaNamespacedOverrides[i]->pMissionOffer, pEnt)) {
...
bool contact_IsNearRespec(Entity *pEnt)
{
	...
	return (pContactDef && contact_IsRespec(pContactDef)) || (pContactDialog && pContactDialog->screenType == ContactScreenType_Respec) ||
		(entGetAccessLevel(pEnt) >= ACCESS_DEBUG) || g_PowerTreeRespecConfig.bNoContactRequiredForRespec;
}
...
// Whether this contact is a ZoneRewards
bool contact_IsZoneRewards(ContactDef *pContactDef)
{
	return (( pContactDef->eContactFlags & ContactFlag_ZoneRewards )!=0);
}
...
void PetContactList_LoadDefs(void)
{
	if (IsGameServerBasedType()) {
		...
		loadstart_printf("Loading PetContactList alt names...");

		ParserLoadFiles(NULL,"defs/config/PetContactListAltNames.def", "PetContactListAltNames.bin", PARSER_OPTIONALFLAG, parse_PetContactListAltClassNames, &g_PetContactListAltClassNames);

		loadend_printf(" done (%d PetContactList).",eaSize(&g_PetContactListAltClassNames.eaNames));
...
static void PetContactList_BuildExprContext(void)
{
	...
	{
		ExprFuncTable* contactFuncTable = exprContextCreateFunctionTable("PetContactList");
...
// Compare the class, if there isn't a match compare the alt class
static bool PetContactList_CompareClass(CharacterClass* pPetClass, PetContact* pNode)
{
	S32 i;

	// try name
	if(stricmp(pPetClass->pchName, pNode->pchName) == 0)
	{
		return true;
	}

	// alt name
	for(i = 0; i < eaSize(&g_PetContactListAltClassNames.eaNames); ++i)
	{
		if(g_PetContactListAltClassNames.eaNames[i]->pchName && stricmp(pPetClass->pchName, g_PetContactListAltClassNames.eaNames[i]->pchName) == 0)
		{
			if(g_PetContactListAltClassNames.eaNames[i]->pchAltName && stricmp(g_PetContactListAltClassNames.eaNames[i]->pchAltName, pNode->pchName) == 0)
			{
				return true;
			}
		}
...
static Entity* petContactList_GetEnt(Entity* pEntPlayer, PetContactList* pList, const char*** peaExcludeList, bool *bPlayAudio)
{
	int i, j;
	...
	for(i=0; i < eaSize(&pList->ppNodes); i++) {
		...
		// If we did not find the first choice in our PetContactList, then we do not want to play audio
		if (gConf.bNoAudioOnPetContactsWithoutPet && bPlayAudio && i > 0)
		{
			*bPlayAudio = false;
		}
		...
		if(pNode->eType == PetContactType_AlwaysPropSlot && pEntPlayer->pSaved->ppAlwaysPropSlots) {
			...
			for(j=0;j<eaSize(&pEntPlayer->pSaved->ppOwnedContainers); j++) {
				...
				if(pPetClass && PetContactList_CompareClass(pPetClass, pNode) && pPet->pSaved && (!peaExcludeList || eaFindCmp(peaExcludeList, pPet->pSaved->savedName, petContactListNameEq) == -1)) {
					//Check the condition
	...
	if (gConf.bNoAudioOnPetContactsWithoutPet && bPlayAudio)
	{
		*bPlayAudio = false;
	}
...
static PetContactList* petContactList_FixupList(Entity* pEntPlayer, PetContactList* pList)
{
	...
	PetContactList *pOverrideList = NULL;
	...
	if (pAllegiance && pAllegiance->pcName && stricmp(pAllegiance->pcName, "Allegiance_Klingon") == 0)
	{
		...
		if (strnicmp(pList->pchName, "Klingon_", 8)!=0)
		{
			...
			pOverrideList = RefSystem_ReferentFromString(g_PetContactListDictionary, pNewListName);
	...
	// if there is a an override list use it, otherwise keep old pointer
	if(pOverrideList)
	{
		return pOverrideList;
	}
...
void PetContactList_GetPetOrCostume(Entity* pEntPlayer, PetContactList* pList, const char*** peaExcludeList, Entity** ppPetEntity, CritterDef** ppCritterDef,
									CritterCostume** ppCostume, bool *bPlayAudio)
{
	...
	pFoundPet=petContactList_GetEnt(pEntPlayer, pListActuallyUsed, peaExcludeList, bPlayAudio);
	if (pFoundPet)
	{
...
AUTO_EXPR_FUNC(PetContact) ACMD_NAME(IsPetDefNamed);
int PetContactList_IsPetDefNamed(ExprContext *pContext, const char *pName)
{
	Entity* pEntPet = pContext->selfPtr;
	if(Entity_IsPetDefNamed(pEntPet, pName))
	{
		return true;
	}
	return 0;
...
ExprFuncTable* contact_CreateExprFuncTable(void)
{
	...
	{
		s_contactFuncTable = exprContextCreateFunctionTable("Contact");
...
TextParserResult fixupDialogBlock( DialogBlock* pDialog, enumTextParserFixupType eType, void* pExtraData )
{
	switch( eType ) {
		xcase FIXUPTYPE_POST_TEXT_READ:
			if( pDialog->audioName_Deprecated ) {
				pDialog->displayTextMesg.bHasVO = true;
				pDialog->displayTextMesg.astrLegacyAudioEvent = allocAddString( pDialog->audioName_Deprecated );
				pDialog->audioName_Deprecated = NULL;
			}
	}
