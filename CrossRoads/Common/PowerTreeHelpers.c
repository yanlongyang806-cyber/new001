...
AUTO_TRANS_HELPER;
bool entity_CanBuyPowerTreeNodeHelper(	ATR_ARGS, int iPartitionIdx, ATH_ARG NOCONST(Entity) *pBuyer, 
										ATH_ARG NOCONST(Entity) *pEnt, PTGroupDef *pGroup, 
										PTNodeDef *pNodeDef, int iRank, S32 bRequireNextRank, 
										S32 bCheckGroupMax, S32 bSlave, S32 bIsTraining)
{
	int rtn=0;
	bool bCanBuy = true;
									 ...
									 S32 bStepRespec)
{
	...
	if(pEnt && pEnt->pChar && pStepsRequestedRespec)
	{
		...
		for(i=0; i<s; i++)
		{
			...
			{
#ifdef GAMESERVER
				const char *pcRespecTokenNumeric = PowerTree_GetRespecTokensPurchaseItem(eRespecType);
				const char *pcRespecTokenGAD = PowerTree_GetRespecTokensPurchaseGAD(eRespecType);
				...
				if (trhCharacter_UseForceRespecFromSchedule(pFakeEnt))
				{
					...
					&& eRespecType == kPTRespecGroup_ALL
					...
					|| inv_ent_trh_AddNumeric(ATR_EMPTY_ARGS,pFakeEnt,true,pcRespecTokenNumeric,-1,NULL)
					|| slGAD_trh_ChangeAttribClamped_Force(ATR_EMPTY_ARGS,pFakeData, pcRespecTokenGAD, -1, 0, 100000)))
				{
					//Yay, its free for you!
				}
				else if(g_PowerTreeRespecConfig.bForceUseFreeRespec
					&& eRespecType != kPTRespecGroup_ALL
					&& ((pcRespecTokenNumeric && inv_ent_trh_AddNumeric(ATR_EMPTY_ARGS,pFakeEnt,true,pcRespecTokenNumeric,-1,NULL))
					|| (pcRespecTokenGAD && slGAD_trh_ChangeAttribClamped_Force(ATR_EMPTY_ARGS,pFakeData, pcRespecTokenGAD, -1, 0, 100000))))
				{
					//Yay, its free for you!
				}
										   ...
										   PowerTreeSteps *pStepsResultBuy)
{
	...
	if(bSuccess)
	{
		bSuccess = entity_PowerTreeStepsRespecSteps(iPartitionIdx,pEnt,pPayer,pStepsRespec,pStepsBuy,pStepsResultRespec,pStepsResultBuy,kPTRespecGroup_ALL,false,false);
	}
		...
		if(pStep->iCostRespec > 0)
		{
			...
			if (trhCharacter_UseForceRespecFromSchedule(pEnt))
			{
				...
				&& (trhCharacter_UseFreeRespecFromSchedule(pEnt) 
					...
					|| slGAD_trh_ChangeAttribClamped(ATR_PASS_ARGS, pData, PowerTree_GetRespecTokensPurchaseGAD(eRespecType), -1, 0, 100000))
			{
				//Yay, its free for you!
			}
...
const char *PowerTree_GetRespecTokensPurchaseGAD(PTRespecGroupType eRespecType)
{
	S32 i;
	if(eRespecType == kPTRespecGroup_ALL)
	{
	...
	for(i = 0; i < eaSize(&g_PowerTreeRespecConfig.eaPTGadRespecNames); ++i)
	{
		if(g_PowerTreeRespecConfig.eaPTGadRespecNames[i]->eRespecGroup == eRespecType)
		{
			return g_PowerTreeRespecConfig.eaPTGadRespecNames[i]->pchName;
		}
	}

	return allocAddString("BadRespecGADToken");
}
