...
typedef struct GroupProjectUnlockDef GroupProjectUnlockDef;
...
#define DONATION_TASK_DISCOUNT_EXT "donationtaskdiscount"
#define DONATION_TASK_DISCOUNT_BASE_DIR "defs/GroupProjects"

#define GROUP_PROJECT_BONUS_EXT "groupprojectbonus"
#define GROUP_PROJECT_BONUS_BASE_DIR "defs/GroupProjects"
...
#define GROUP_PROJECT_INVALID_DISCOUNTED_BUCKET_REQUIREMENT (0xffffffff)
...
extern DictionaryHandle g_GroupProjectBonusDict;
extern DictionaryHandle g_DonationTaskDict;
extern DictionaryHandle g_DonationTaskDiscountDict;

#define PROJECT_MESSAGE_MAX_LEN 200
#define PROJECT_PLAYER_NAME_MAX_LEN 64

#define PROJECT_FINALIZATION_REQUEST_LIMITER 30
...
typedef enum DonationTaskRewardType
{
    ...
    DonationTaskRewardType_GuildProjectDiscount, // applies a discount to all group projects with type GroupProjectType_Guild (must be on a Guild project DonationTask)
    DonationTaskRewardType_PlayerProjectDiscount, // applies a discount to all group projects with type GroupProjectType_Player (must be on a Player project DonationTask)
    DonationTaskRewardType_Bonus,
} DonationTaskRewardType;

AUTO_ENUM;
typedef enum GroupProjectDonationDiscountType
{
    DonationDiscountType_None,
    DonationDiscountType_Percentage,
    DonationDiscountType_Absolute,
} GroupProjectDonationDiscountType;
...
AUTO_ENUM;
typedef enum GroupProjectBonusType
{
    BonusType_None,
    BonusType_Percentage,
    BonusType_Absolute,
} GroupProjectBonusType;

AUTO_STRUCT;
typedef struct DonationTaskDiscountDef
{
    // The logical name for this donation discount.
    const char *name;                   AST(STRUCTPARAM POOL_STRING KEY)
    ...
    // The name of the discount that is displayed to the user.
    DisplayMessage displayNameMsg;      AST(STRUCT(parse_DisplayMessage))

    // The detailed description of the discount that is displayed to the user.
    DisplayMessage descriptionMsg;      AST(STRUCT(parse_DisplayMessage))

    // The name of the icon to display for this discount (e.g. reward list).
    const char* iconName;               AST(NAME(Icon) POOL_STRING)

    // How does the discount apply to the required count
    GroupProjectDonationDiscountType discountType;

    // A discount percentage (the actual discount will be the specified percentage of count rounded up to the next donation increment)
    F32 discountPercent;

    // A discount quantity (this value must be an even multiple of the requirement's donationIncrement)
    U32 discountAmount;
} DonationTaskDiscountDef;

AUTO_STRUCT;
typedef struct DonationTaskDiscountDefRef
{
    REF_TO(DonationTaskDiscountDef) discountDef; AST(STRUCTPARAM KEY)
} DonationTaskDiscountDefRef;

AUTO_STRUCT AST_CONTAINER;
typedef struct DonationTaskDiscountDefRefContainer
{
	CONST_REF_TO(DonationTaskDiscountDef) discountDef; AST(PERSIST SUBSCRIBE STRUCTPARAM KEY)
} DonationTaskDiscountDefRefContainer;
...
AUTO_STRUCT;
typedef struct GroupProjectBonusDef
{
    // The logical name for this bonus.
    const char *name;                   AST(STRUCTPARAM POOL_STRING KEY)
    ...
    // The name of the bonus that is displayed to the user.
    DisplayMessage displayNameMsg;      AST(STRUCT(parse_DisplayMessage))

    // The detailed description of the bonus that is displayed to the user.
    DisplayMessage descriptionMsg;      AST(STRUCT(parse_DisplayMessage))

    // The name of the icon to display for this bonus (e.g. reward list).
    const char* iconName;               AST(NAME(Icon) POOL_STRING)

    // The numeric this bonus applies to
    REF_TO(GroupProjectNumericDef) numericDef;          AST(NAME(Numeric))

    // How does the bonus apply to the reward
    GroupProjectBonusType bonusType;

    // The value of this bonus
    F32 value;
} GroupProjectBonusDef;

AUTO_STRUCT;
typedef struct GroupProjectBonusDefRef
{
    REF_TO(GroupProjectBonusDef) bonusDef;  AST(STRUCTPARAM KEY)
} GroupProjectBonusDefRef;

AUTO_STRUCT AST_CONTAINER;
typedef struct GroupProjectBonusDefRefContainer
{
    CONST_REF_TO(GroupProjectBonusDef) bonusDef;    AST(PERSIST SUBSCRIBE STRUCTPARAM KEY)
} GroupProjectBonusDefRefContainer;
...
typedef struct DonationTaskReward
{
    ...
    // The donation task discount
    // Valid for DonationTaskRewardType_GuildProjectDiscount,
    // and DonationTaskRewardType_PlayerProjectDiscount
    REF_TO(DonationTaskDiscountDef) discountDef;        AST(NAME(Discount))

    // The group project bonus to reward
    // Only valid for DonationTaskRewardType_Bonus.
    REF_TO(GroupProjectBonusDef) bonusDef;              AST(NAME(Bonus))
} DonationTaskReward;
...
typedef struct GroupProjectDonationRequirement
{
    ...
    // A list of all discounts this bucket can accept
    const char **discounts; AST(NAME(Discount) POOL_STRING)
...
typedef struct DonationTaskDef
{
	...
	//Is this able to have no cost
	bool noCost;						AST(NAME(NoCost) BOOLFLAG)
    ...
    // If defined, the activity that needs to be active before enabling this project
    const char* activityEventRequirement;      AST(POOL_STRING)
	...
	U32 iUISortOrder;     AST(NAME(UISortOrder))
...
typedef enum DonationTaskState
{
    ...
    DonationTaskState_FinalizedRewardPending, // Task donations have been completed.  Task needs to give rewards
...
typedef struct DonationTaskSlot
{
    ...
    // Non-persisted field used by the GroupProjectServer to keep track of which tasks have pending finalization transactions.
    U32 finalizationRequestedAt;
...
typedef struct GroupProjectContainer
{
    ...
    // Discounts applied across all tasks done on any project in the container.
    CONST_EARRAY_OF(DonationTaskDiscountDefRefContainer) discountList; AST(PERSIST SUBSCRIBE)

    // Bonuses applied across rewards
    CONST_EARRAY_OF(GroupProjectBonusDefRefContainer) bonusList;  AST(PERSIST SUBSCRIBE)
...
typedef struct DonationTaskFilter
{
	...
	DonationTaskCategoryType *peCategoryMask;
	bool bSortByCategories : 1;
	bool bAddCategoryHeaders : 1;
	bool bIncludeUnavailableTasks : 1;
	bool bRequireAvailableForNewProject : 1;
} DonationTaskFilter;

AUTO_STRUCT;
typedef struct GroupProjectRecentUnlock
{
    STRING_POOLED projectName;  AST(POOL_STRING)
    STRING_POOLED unlockName;   AST(POOL_STRING)
    U32 time;
} GroupProjectRecentUnlock;

extern U32 gDebugUnlockNotify;

bool GroupProject_Validate(GroupProjectDef* projectDef);
bool DonationTask_Validate(DonationTaskDef* taskDef);
bool DonationTaskDiscount_Validate(DonationTaskDiscountDef* taskDiscountDef);
...
GroupProjectConstant *GroupProject_FindConstant(const GroupProjectDef *projectDef, const char *key);
...
bool DonationTask_ItemMatchesExpressionRequirement(unsigned int iParitionIdx, Entity *pEnt, const GroupProjectDonationRequirement *taskBucket, Item *item);

bool GuildProject_DonationTaskAllowed(int iPartitionIdx, Entity *playerEnt, GroupProjectState *projectState, DonationTaskDef *taskDef, int taskSlotNum);
bool PlayerProject_DonationTaskAllowed(int iPartitionIdx, Entity *playerEnt, GroupProjectState *projectState, DonationTaskDef *taskDef, int taskSlotNum);
...
void GroupProject_UpdateBucketQuantities(const GroupProjectContainer *pContainer, const GroupProjectState *pState, const GroupProjectDef *pGroupProject, const DonationTaskDef *pDef, const DonationTaskSlot *pSlot, S32 *piCurrentBucketQuantityOut, S32 *piTotalBucketQuantityOut);

bool GroupProject_trh_CheckForDiscount(ATR_ARGS, ATH_ARG NOCONST(GroupProjectContainer) *projectContainer, ATH_ARG NOCONST(GroupProjectState) *projectState, const char *discountName);
#define GroupProject_CheckForDiscount(projectContainer, projectState, discountName) GroupProject_trh_CheckForDiscount(ATR_EMPTY_ARGS, CONTAINER_NOCONST(GroupProjectContainer, projectContainer), CONTAINER_NOCONST(GroupProjectState, projectState), discountName)
U32 GroupProject_trh_GetDiscountedTaskBucketRequirement(ATR_ARGS, ATH_ARG NOCONST(GroupProjectContainer) *projectContainer, ATH_ARG NOCONST(GroupProjectState) *projectState, DonationTaskDef *taskDef, const char *bucketName);
#define GroupProject_GetDiscountedTaskBucketRequirement(projectContainer, projectState, taskDef, bucketName) GroupProject_trh_GetDiscountedTaskBucketRequirement(ATR_EMPTY_ARGS, CONTAINER_NOCONST(GroupProjectContainer, projectContainer), CONTAINER_NOCONST(GroupProjectState, projectState), taskDef, bucketName)
U32 GroupProject_trh_GetDiscountedBucketRequirement(ATR_ARGS, ATH_ARG NOCONST(GroupProjectContainer) *projectContainer, ATH_ARG NOCONST(GroupProjectState) *projectState, ATH_ARG NOCONST(DonationTaskSlot) *slot, const char *bucketName);
#define GroupProject_GetDiscountedBucketRequirement(projectContainer, projectState, slot, bucketName) GroupProject_trh_GetDiscountedBucketRequirement(ATR_EMPTY_ARGS, CONTAINER_NOCONST(GroupProjectContainer, projectContainer), CONTAINER_NOCONST(GroupProjectState, projectState), CONTAINER_NOCONST(DonationTaskSlot, slot), bucketName)
bool GroupProject_trh_CheckBucketRequirementsComplete(ATR_ARGS, ATH_ARG NOCONST(GroupProjectContainer) *projectContainer, ATH_ARG NOCONST(GroupProjectState) *projectState, ATH_ARG NOCONST(DonationTaskSlot) *slot, const char *bucketName);
#define GroupProject_CheckBucketRequirementsComplete(projectContainer, projectState, slot, bucketName) GroupProject_trh_CheckBucketRequirementsComplete(ATR_EMPTY_ARGS, CONTAINER_NOCONST(GroupProjectContainer, projectContainer), CONTAINER_NOCONST(GroupProjectState, projectState), CONTAINER_NOCONST(DonationTaskSlot, slot), bucketName)

UnlocksForNumeric *GroupProject_GetUnlocksForNumeric(GroupProjectNumericDef *numericDef);

bool GroupProject_TaskFilterCheck(int iPartitionIdx, DonationTaskDef *pTask, Entity *pEnt, GroupProjectState *pState, const DonationTaskFilter *pFilter, int slotNum, bool *pbAvailableOut, DonationTaskCategoryType **ppeCategoriesOut);

ItemDef* GroupProject_FindItemDef(const GroupProjectDef *pGroupProject, const char *pchNumericPattern);