...
typedef struct PurchaseHolder
{
	...
	//Determines whether to use the item immediately
	bool bUseItemAfterPurchase;
...
void GenerateMicrotransactionList(const AccountProxyProductList *pList, MicroTransactionProductList *pMTList, MicroTransactionCategory *pMTCategory)
{
	...
	for (i = eaSize(&pList->ppList) - 1; i >= 0; i--)
	{
		...
		//If there's a micro trans def and not marked as deprecated
		if (bFoundCat && pProduct && pDef && !pDef->bDeprecated)
		{
...
static void UseItem( ItemDef * pItemDef, Entity * pEnt ) 
{
	if (pEnt && pItemDef)
	{
		BagIterator *pIter = bagiterator_Create();
		inv_bag_FindItem(pEnt, InvBagIDs_Inventory, pIter, kFindItem_ByName, pItemDef->pchName, true, false);
		...
		if (pIter)
		{
			...
			if (pItem && pBag && item_IsDeviceUsableByPlayer(pEnt, pItem, pBag))
			{
				S32 iNumPowers = item_GetNumItemPowerDefs(pItem, true);
				S32 iPower;
				...
				for (iPower = 0; iPower < iNumPowers; ++iPower)
				{
					...
					{
						Power *pItemPow = item_GetPower(pItem, iPower);
						U32 uID = pItemPow->pParentPower ? pItemPow->pParentPower->uiID : pItemPow->uiID;
						Character *pChar = entGetChar(pEnt);
						Power *pCharPow = character_FindPowerByID(pChar, uID);

						character_ActivatePowerServerBasic(entGetPartitionIdx(pEnt), pChar, pCharPow, pEnt, NULL, true, false, entity_GetCachedGameAccountDataExtract(pEnt));
						bFinished = true;
						break;
					}
				}
			...
			if (!bFinished)
			{
				S32 iBagID = bagiterator_GetCurrentBagID(pIter);
				S32 iSlot = bagiterator_GetSlotID(pIter);
				ItemEquip(pEnt, iBagID, iSlot);
			}
		}
	...
	case TRANSACTION_OUTCOME_SUCCESS:
		{
			...
			char *pCategoryStr = NULL;

			EARRAY_FOREACH_BEGIN(pCBStruct->pProduct->ppCategories, iCategory);
			{
				if (pCategoryStr)
				{
					estrConcatf(&pCategoryStr, ",%s", pCBStruct->pProduct->ppCategories[iCategory]);
				}
				else
				{
					estrPrintf(&pCategoryStr, "%s", pCBStruct->pProduct->ppCategories[iCategory]);
				}
			}
			EARRAY_FOREACH_END;
			
			APLog(pEnt, pCBStruct->uAccountID, "MicroTransactionSuccess", "AccountID %d, ProductID %d, Product %s, MTDef %s, Categories \"%s\", Currency %s, Price %"FORM_LL"d",
				pCBStruct->uAccountID,
				pCBStruct->pProduct->uID,
				pCBStruct->pProduct->pName,
				pDef->pchName,
				pCategoryStr,
				moneyCurrency(pCBStruct->pExpectedPrice),
				moneyCountPoints(pCBStruct->pExpectedPrice));
			estrDestroy(&pCategoryStr);
			...
			if(pEnt && pDef)
			{
				...
				if (pCBStruct->bUseItemAfterPurchase && entGetChar(pEnt))
				{
					S32 iPart;

					for (iPart = 0; iPart < eaSize(&pDef->eaParts); ++iPart)
					{
						if (pDef->eaParts[iPart]->ePartType == kMicroPart_Item)
						{
							ItemDef *pItemDef = GET_REF(pDef->eaParts[iPart]->hItemDef);

							UseItem(pItemDef, pEnt);
...
AUTO_TRANSACTION
ATR_LOCKS(pContainer, ".Plock.Uaccountid, .Plock.Result, .Plock.Fdestroytime, .Plock.Pkey, .Plock.Etransactiontype")
ATR_LOCKS(pEnt, ".Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pplayer.Pmicrotransinfo.Eaonetimepurchases, .Psaved.Ppbuilds, .Pplayer.Pplayeraccountdata.Iversion, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Psaved.Ppuppetmaster.Pppuppets, .pInventoryV2.Ppinventorybags, .pInventoryV2.Pplitebags, .Pplayer.Playertype, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
		...
		case AKV_SUCCESS:
			{
				...
				MicroPurchaseErrorType eError = microtrans_GetCanPurchaseError(
					GetAppGlobalType() == GLOBALTYPE_WEBREQUESTSERVER || GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER
						? WEBREQUEST_PARTITION_INDEX : entGetPartitionIdx(pEnt), 
...
static void GetProductCB(SA_PARAM_OP_VALID const AccountProxyProduct *product, void *userData)
{
	...
	else if(pDef)
	{
		...
		else
		{
			...
			pCBStruct->bUseItemAfterPurchase = holder->bUseItemAfterPurchase;
...
void APPurchaseProduct( Entity *pEntity, U32 iAccountID, int iLangID, SA_PARAM_NN_STR const char *pCategory, U32 uProductID, SA_PARAM_OP_VALID const Money *pExpectedPrice, U32 uOverrideDiscount, SA_PARAM_OP_STR const char *pPaymentMethodVID, SA_PARAM_OP_STR const char *pClientIP, U64 uItemId, bool bUseItemAfterPurchase )
{
	...
	holder->bUseItemAfterPurchase = bUseItemAfterPurchase;
