	...
	case kMicroPart_Species:
		{
			...
			if(!pSpecies->pcUnlockCode || !pSpecies->pcUnlockCode[0])
			{
				ErrorFilenamef(pDef->pchFile, "MicroTransaction %s type %s: Species with no unlock code specified %s.",
					pDef->pchName,
					StaticDefineIntRevLookup(MicroPartTypeEnum, pPart->ePartType),
					REF_STRING_FROM_HANDLE(pPart->hSpeciesDef));
				iValid = 0;
...
static ExprFuncTable * microtrans_CanBuyExprFuncTable(bool bNoEnt)
{
	...
	if (!stFuncs) {
		stFuncs = exprContextCreateFunctionTable("MicroTransactionsCanBuy");
	...
	if (!stFuncsNoEnt) {
		stFuncsNoEnt = exprContextCreateFunctionTable("MicroTransactionsCanBuy_NoEnt");
		for (i = 0; i < ARRAY_SIZE(sachCommonTags); i++) {
...
MicroTransactionDef *microtrans_findDefFromAPProd(const AccountProxyProduct *pProduct)
{
	...
	if (!pDef)
	{
		...
		{
			if (!sMicrotransLookupTable)
				sMicrotransLookupTable = stashTableCreateWithStringKeys(256, StashDeepCopyKeys_NeverRelease);
		...
		for(iIdx = eaSize(&pDef->eaParts)-1; iIdx >= 0; iIdx--)
		{
			...
			switch(pPart->ePartType)
			{
				xcase kMicroPart_Item:
				{
					...
					eBestBagID = GetBestBagForItemDef(pEnt, pItemDef, NULL, pPart->iCount, true, pExtract);
					
					if(pItemDef->flags & kItemDefFlag_EquipOnPickup)
					{
						eBagID = GetBestBagForItemDef(pEnt, pItemDef, NULL, pPart->iCount, false, pExtract);
					}
		...
		for(idx = eaSize(&pDef->eaParts)-1; bCannotPurchase && idx >= 0; idx--)
		{
			...
			case kMicroPart_Species:
				PERFINFO_AUTO_START("Species", 1);
				{
					...
					{
						// Species_IsUnlocked and the functions it calls will not modify gad
						if(!pData || !Species_IsUnlocked(pSpecies, (GameAccountData *)pData))
							bCannotPurchase = false;
					}
				}
				PERFINFO_AUTO_STOP();
												      ...
												      char** pestrError)
{
	...
	else if (pDef->bOnePerCharacter && (!pEnt || !pEnt->pPlayer))
	{
		eError = kMicroPurchaseErrorType_RequiresCharacter;
	}
...
{
	// TODO: this should be completely replaced with a translated string instead of having an expression to retrieve the value, derived from code.
	LocaleID locID = getCurrentLocale();

	if (locID == LOCALE_ID_FRENCH)
	{
		return "https://" ORGANIZATION_DOMAIN "/charge"; // french
	}
	else if (locID == LOCALE_ID_GERMAN)
	{
		return "https://" ORGANIZATION_DOMAIN "/charge"; // german
	}
	else if (locID == LOCALE_ID_TURKISH)
	{
		return "https://" ORGANIZATION_DOMAIN "/charge"; // turkish
	}
	else if (locID == LOCALE_ID_ITALIAN)
	{
		return "https://" ORGANIZATION_DOMAIN "/charge"; // italian
	}
	else if (locID == LOCALE_ID_POLISH)
	{
		return "https://" ORGANIZATION_DOMAIN "/charge"; // polish
	}
	else // default to the US billing site
//	if (locID == LOCALE_ID_ENGLISH) 
	{
		return "https://" ORGANIZATION_DOMAIN "/charge"; // us english
	}
}
