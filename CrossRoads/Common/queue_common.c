...
ExprContext *queue_GetContext(Entity *pEnt)
{
	...
	if(!s_pQueueContext)
	{
		...
		stTable = exprContextCreateFunctionTable("Queue");
...
// Returns a member, whether Preset or not
...
QueueMatchMember* queue_Match_AddMemberToGroup(SA_PARAM_NN_VALID QueueMatch* pMatch, SA_PARAM_OP_VALID QueueGroup* pGroup, U32 uEntID, U32 uTeamID,
											      S32 iGroupRole, S32 iGroupClass, bool bPresetMember)
{
	if (pGroup && uEntID > 0)
	{
		...
		pMatchMember->bPresetMember = bPresetMember;
		eaPush(&pGroup->eaMatchMembers, pMatchMember);
		ea32PushUnique(&pGroup->puiInMapTeamIDs, uTeamID);	// Possibly only for auto-balancing
...
QueueMatchMember* queue_Match_AddMember(SA_PARAM_NN_VALID QueueMatch* pMatch, S32 iGroupIndex, U32 uEntID, U32 uTeamID)
{
	...
	return queue_Match_AddMemberToGroup(pMatch, pGroup, uEntID, uTeamID, 0, 0, false);  // Manual adding, not tracking role/class. No Presets. What a mess.
}
...
// Remove all members from a match, but leave the structures intact
void queue_Match_ClearMembers(SA_PARAM_NN_VALID QueueMatch *pMatch)
{
	...
	for (i = eaSize(&pMatch->eaGroups)-1; i >= 0; i--)
	{
		...
		{
			StructDestroy(parse_QueueMatchMember, eaRemove(&pGroup->eaMatchMembers, j));
			pGroup->iGroupSize--;
			pMatch->iMatchSize--;
		}
	}
...
// Fill the pMatch with the groups/members in the instance that already have a valid group index
// There are no presetMembers for Private queues
AUTO_TRANS_HELPER
ATR_LOCKS(pInstance, ".Eaunorderedmembers");
void queue_trh_PrivateGroupCache(ATH_ARG NOCONST(QueueInstance) *pInstance, 
...
// Count the number of members in this instance who are already in groups, regardless of state
int queue_MatchMemberCountWithMap(QueueInstance *pInstance, QueueDef *pQueueDef, QueueMap *pMap)
{
	...
	if (pMap!=NULL)
	{
		...
		for (iMemberIdx = 0; iMemberIdx < iMemberSize; iMemberIdx++)
		{
			...
			if (pMember!=NULL && pMember->iGroupIndex >= 0 && pMember->iMapKey == pMap->iMapKey)
			{
				iCount++;
			}
		}
	...
	if (pMatch->iMatchSize < queue_QueueGetMaxPlayers(pQueueDef, bOvertime))
	{
		...
		for(iGroupIndex = eaSize(&pMatch->eaGroups)-1; iGroupIndex >= 0; iGroupIndex--)
		{
			...
			if (uTeamID > 0)
			{
				for (iMemberIdx = eaSize(&pGroup->eaMatchMembers)-1; iMemberIdx >= 0; iMemberIdx--)
				{
					if (uTeamID == pGroup->eaMatchMembers[iMemberIdx]->uTeamID)
					{
						break;
					}
				}
...
void queue_GetMinMaxLevel(QueueDef* pDef, int iLevelBand, int* piMinLevel, int* piMaxLevel)
{
	...
	if (iLevelBand >= 0)
	{
		...
		if (pLevelBand)
		{
			...
			iMin = pLevelBand->iMinLevel;
			iMax = pLevelBand->iMaxLevel;
		}
	}
	else
	{
		...
		{
			iMin = pMinLevelBand->iMinLevel;
			iMax = pMaxLevelBand->iMaxLevel;
		}
	}
	if (piMinLevel!=NULL)
	{
											 ...
											 bool bIgnoreRequiresExpr)
{
	if (pDef)
	{
		...
		if (!bIgnoreLevelRestrictions)
		{
			...
			queue_GetMinMaxLevel(pDef, iLevelBand, &iMin, &iMax);

			if (iMin!=0 && iMin > iEntLevel)
			{
				return QueueCannotUseReason_LevelTooLow;
			}
			if (iMax!=0 && iMax < iEntLevel)
			{
...
// Count the number of members in this instance who are in a particular group and also "InMap"
static int queue_StateInMapGroupMemberCount(QueueInstance *pInstance, QueueDef *pQueueDef, QueueMap *pMap, int iGroupIndex)
{
	int iCount=0;
	...
	if (pMap!=NULL)
	{
		...
		for (iMemberIdx = 0; iMemberIdx < iMemberSize; iMemberIdx++)
		{
			...
			if (pMember!=NULL && pMember->iMapKey == pMap->iMapKey && pMember->eState == PlayerQueueState_InMap && pMember->iGroupIndex == iGroupIndex)
			{
				iCount++;
			}
		}
...
{
	bool bMapFull = false;
	if (pMap && pInstance && pQueueDef)
	{
		if (pQueueDef->Settings.bSplitTeams)
		{
			S32 iFreeSlots = 0;
			S32 iGroupIndex, iGroupSize = eaSize(&pQueueDef->eaGroupDefs);
			for (iGroupIndex = 0; iGroupIndex < iGroupSize; iGroupIndex++)
			{
				QueueGroupDef *pGroupDef = pQueueDef->eaGroupDefs[iGroupIndex];				
				...
				if (pGroupDef)
				{
					...
					iCurGroupSize = queue_StateInMapGroupMemberCount(pInstance, pQueueDef, pMap, iGroupIndex);
		...
		else
		{
			...
			for (iGroupIndex = 0; iGroupIndex < iGroupSize; iGroupIndex++)
			{
				...
				if (pGroupDef)
				{
					...
					{
						// We found a group that has space for this team
						bMapFull = false;
						break;
					}
				}
...
{
	S32 i, iEntLevel = pEnt ? entity_GetSavedExpLevel(pEnt) : 0;

	if (!pQueueDef)
		return -1;

	for (i = eaSize(&pQueueDef->eaLevelBands)-1; i >= 0; i--)
	{
		QueueLevelBand* pLevelBand = pQueueDef->eaLevelBands[i];
