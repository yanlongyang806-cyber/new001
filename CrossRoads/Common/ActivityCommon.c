...
void Activity_RegisterEventDictionary(void)
{
	...
	{
		resDictRequestMissingResources(g_hEventDictionary, RES_DICT_KEEP_ALL, false, resClientRequestSendReferentCommand);
	}
}
...
bool Activity_EventIsActive(const char *pchEventName)
{
#ifdef GAMESERVER
...
{
	return(Activity_EventIsActive(pchEventName));
}
