...
typedef enum RewardBonusType
{
	...
	RewardBonusType_GlobalModifier = 8,
} RewardBonusType;

#define RewardBonusType_Default RewardBonusType_Recruit|RewardBonusType_Lifetime|RewardBonusType_Modifier|RewardBonusType_GlobalModifier
...
AUTO_STRUCT AST_CONTAINER;
typedef struct RewardGatedRewardTable
{
	U32 eRewardGateType;				AST(KEY PERSIST SOMETIMES_TRANSACT SUBTABLE(RewardGatedTypeEnum))
	// The reward gate type to use for this reward

	REF_TO(RewardTable) hRewardTable;	AST(PERSIST NAME(RewardTable))
	// The reward table paired with the gate
} RewardGatedRewardTable;
...
typedef struct RewardEntry
{
	...
	U32					bForceBind : 1; //force item to bind-on-pickup to character, overriding ItemDef.
	U32 				bBaseItemsOnly : 1;
	U32					bScaleNumeric : 1;
	U32					bHideInUI : 1;
	// If true this will cause a gate reward table to reset
	U32					bResetGatedTable : 1;

	// this is basically an open field that the UI can use to do special treatment for this reward.  No set meaning.  (For reward packs, mainly)
	int					iSpecialReward;
...
typedef struct RewardBagInfo
{
	...
	int					iSpecialRewardStatus; // from the reward entry
...
typedef struct RewardGatedInfo
{
	...
	// To figure out the gate blocks, first look at the IntervalMode.
	////  If it is not "Off" use the appropriate ShardInterval timing. 

		// Shard-clock-based interval timing. Uses uTimeSet on the PlayerGated structure to determine availability.
		ShardIntervalTimingMode eRewardGatedIntervalMode;		AST(NAME(IntervalMode))
	...
	////  If it IS "off" fall back to the old-style cooldowns
	    ...
	    // How many seconds in a block, 0 == an infinate block size that is only reset by commands or the reward entry bResetGatedTable or by uResetAt
	    // IMPORTANT NOTE - uSecondsPerBlock is ignored if uHoursPerBlock is non-zero.  If uHoursPerBlock is zero, then uSecondsPerBlock is used.
	    U32 uSecondsPerBlock;
...
AUTO_STRUCT;
typedef struct GlobalRewardModifier
{
	U32 iKey;							AST(KEY)		
	REF_TO(ItemDef) hNumeric;			AST(NAME(Numeric))//Numeric to scale
	F32 fBonusValue;					AST(NAME(BonusValue))
	const char *pchRequiredActivity;	AST(POOL_STRING NAME(RequiredActivity))
}GlobalRewardModifier;
...
typedef struct RewardConfig
{
	...
	EARRAY_OF(GlobalRewardModifier) eaRewardModifiers;		AST(NAME(RewardModifier))

	REF_TO(ItemDef) hGatewaySuperCritterPetXP;				AST(NAME(GatewaySuperCritterPetXP))
...
// This struct is essentially a mirror of ItemRewardPackTransferData, except insofar as it doesn't contain any of the
// same data in the same organization.  InventoryBags have their full RewardBagInfo struct in them.  The reward packs
// selectively collect data and put it in their own structure.  I added this struct so I could get at iSpecialRewardStatus.
// Someday, a great warrior will come forth and unite the clans of the reward UI into a great nation to slaughter our enemies.
AUTO_STRUCT;
typedef struct LootToClient
{
...
// Gated Time Block queries
bool Reward_StillInPreviousGatedTimeBlock(U32 uGateTime, RewardGatedInfo *pRewardGatedInfo);
U32 Reward_GatedTimeBlockSecondsLeft(U32 uGateTime, RewardGatedInfo *pRewardGatedInfo);
