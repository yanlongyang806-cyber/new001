	...
	AllegianceDef *pSubAllegiance, bool bUseOverrideLevel, U32 *pSeed)
{
	...
	if(pItemDef)
	{
		...
		ItemProgressionDef* pProgDef = GET_REF(pItemDef->hProgressionDef);
#ifdef GAMECLIENT
		...
		if(pItemDef->eType == kItemType_SuperCritterPet)
		{
			...
			{
				devassertmsgf(0, "%s has a SCP def that failed to load!", pItemDef->pchFileName);
			}
		...
		if (pProgDef)
		{
			itemProgression_trh_FixupNewItemProps(pItem);
		}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pSlot, ".pItem");
bool inv_trh_MatchingItemInSlot(ATR_ARGS, ATH_ARG NOCONST(InventorySlot)* pSlot, const char* ItemDefName)
{
	NOCONST(Item)* pItem = SAFE_MEMBER(pSlot, pItem);
	const char* tmpName = inv_trh_GetItemDefNameFromItem(ATR_PASS_ARGS, pItem);

	if (tmpName && itemProgression_trh_ItemCanStack(pItem))
	{
...
AUTO_TRANS_HELPER
ATR_LOCKS(pBag, ".Ppindexedinventoryslots");
bool inv_trh_MatchingItemInSlotID(ATR_ARGS, ATH_ARG NOCONST(InventoryBag)* pBag, int SlotIdx, const char* ItemDefName)
{
	NOCONST(InventorySlot)* pSlot = inv_trh_GetSlotPtr(ATR_PASS_ARGS, pBag, SlotIdx);

	return inv_trh_MatchingItemInSlot(ATR_PASS_ARGS, pSlot, ItemDefName);
}
...
bool inv_trh_CanEquipUnique(ATR_ARGS, ItemDef* pItemDef, ATH_ARG NOCONST(InventoryBag)* pBag)
{
	...
	for(i = 0; i < eaSize(&pBag->ppIndexedInventorySlots); ++i)
	{
		...
		{
			if(pSlot->pItem && 
				inv_trh_MatchingItemInSlotID(ATR_PASS_ARGS, pBag, i, pItemDef->pchName))
			{
				return false;			
			}
		}
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pDestEnt, ".Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Conowner.Containertype, .Psaved.Conowner.Containerid, .Psaved.Ppallowedcritterpets, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
						...
						GameAccountDataExtract *pExtract)
{
	...
	if (DestSlotIdx>=0 && SrcSlotIdx>=0)
	{
		...
		bool bSameEntity = entity_trh_IsOwnerSame(ATR_PASS_ARGS, pSrcEnt, pDestEnt);
		
		pItemSrc = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pSrcEnt, true, pSrcBag, SrcSlotIdx, -1, bSameEntity, pSrcReason);
		
		if ( ISNULL(pItemSrc) )
			return false;
		
		pItemDst = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pDestEnt, true, pDestBag, DestSlotIdx, -1, bSameEntity, pDestReason);

		if ( ISNULL(pItemDst) )
		{
			// Move from Dest to Src.
			inv_bag_trh_AddItem(ATR_PASS_ARGS, pSrcEnt, eaSrcPets, pSrcBag, SrcSlotIdx, pItemSrc, pDestBag, DestSlotIdx, eFlags, NULL, pSrcReason, pExtract);
			return false;
		}
		...
		// Move from Dest to Src.
		inv_bag_trh_AddItem(ATR_PASS_ARGS, pSrcEnt, eaSrcPets, pSrcBag, SrcSlotIdx, pItemDst, pDestBag, DestSlotIdx, eFlags, NULL, pSrcReason, pExtract);
		// Move from Src to Dest
		inv_bag_trh_AddItem(ATR_PASS_ARGS, pDestEnt, eaDstPets, pDestBag, DestSlotIdx, pItemSrc, pSrcBag, SrcSlotIdx, eFlags, NULL, pDestReason, pExtract);
...
// Returns the last entry of the table's iMaxSlots value
...
// returns the maxSlot tables potential maximum number of slots that a bag could have
S32 inv_GetBagMaxSlotTableMaxSlots(InventoryBag *pBag)
{
	if (pBag)
	{
		const InvBagDef *pDef = invbag_def(pBag);
		if (pDef && pDef->primaryMaxSlots.pchMaxSlotTable)
		{
			int iMaxSlots = 0;
			InvBagSlotTable *pTable = inv_FindBagSlotTableByName(pDef->primaryMaxSlots.pchMaxSlotTable);

			if (pTable)
			{
				iMaxSlots = InvBagSlotTable_FindMaxSlots(pTable);
			}

			// add in the slots from the eaAdditionalMaxSlots, if any
			FOR_EACH_IN_EARRAY_FORWARDS(pDef->eaAdditionalMaxSlots, InvBagVariableMaxSlotDef, pMaxSlotDef)
			{
				pTable = inv_FindBagSlotTableByName(pDef->primaryMaxSlots.pchMaxSlotTable);
				if (pTable)
				{
					iMaxSlots += InvBagSlotTable_FindMaxSlots(pTable);
					if (iMaxSlots >= pDef->additionalMaxSlotsLimit)
					{
						return pDef->additionalMaxSlotsLimit;
					}
				}
			}
			FOR_EACH_END
...
// Really a sort of hacky way to deal with itemPower creation times when joining or adding to a stack.
// (When we have stackable expirable items). 
// We need to make sure the destination slot has its powers updated to a later creation time if the incoming
// item has a later creation time.  Eventually we want to completely get rid of the power way of expiring items
// at which point this code should go away or be replaced with something more sane. WOLF[7Aug2013]
// See also the inline hack in inv_bag_trh_RemoveItem for splitting stacks.
static void AddItem_Fixup_SlotItemPowersCreationTimes(NOCONST(InventorySlot)* pSlot, NOCONST(Item)* pIncomingItem)
{
	int i;
	U32 uLatestCreationTime = 0;

	// Go through all powers on the incoming item and find the latest creation time
	for (i=0;i<eaSize(&pIncomingItem->ppPowers); i++)
	{
		NOCONST(Power)* pPower = pIncomingItem->ppPowers[i];
				
		if (pPower->pParentPower) pPower = pPower->pParentPower;
		if (pPower->uiTimeCreated > uLatestCreationTime)
		{
			uLatestCreationTime = pPower->uiTimeCreated;
		}
	}

	// If we found a time, go through the destination slot and make sure all the powers are set to this time if they happen to be earlier.
	// We want to use the latest time possible. This 'resets' the destination stack if the source has longer to last.
	// We are Assuming that the slot has consistent internal creation times already. 
	if (uLatestCreationTime > 0)
	{
		for (i=0;i<eaSize(&pSlot->pItem->ppPowers); i++)
		{
			NOCONST(Power)* pPower = pSlot->pItem->ppPowers[i];
			if (pPower->pParentPower) pPower = pPower->pParentPower;
			if (pPower->uiTimeCreated < uLatestCreationTime)
			{
				pPower->uiTimeCreated = uLatestCreationTime;
			}
		}
	}
}
...
int inv_bag_trh_AddItemFast(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG CONST_EARRAY_OF(NOCONST(Entity)) eaPets, ATH_ARG NOCONST(InventoryBag)* pDestBag, int SlotIdx, ATH_ARG NOCONST(Item)* pItem, int count, ItemAddFlags eFlags, U64** peaItemIDs, const ItemChangeReason *pReason)
{
	...
	if ( NONNULL(pItemDef) && itemProgression_trh_ItemCanStack(pItem))
	{
		MatchCount = inv_bag_trh_GetItemList(ATR_PASS_ARGS, pDestBag, pItemDef->pchName, &tmpInventoryList );
		ItemMatchesDestSlot = inv_trh_MatchingItemInSlotID(ATR_PASS_ARGS, pDestBag, SlotIdx, pItemDef->pchName);
	...
	if( NONNULL(pSlot) && ItemMatchesDestSlot )
	{
		if( StackLimit <= 0 )
		{
			...
			AddItem_Fixup_SlotItemPowersCreationTimes(pSlot, pItem); // Annoying cleanup for power lifetime stuff. Remove if at all possible later.
			DestSlotIdx = SlotIdx;
		}
		else
		{
	...
	if ( SlotIdx<0 && MatchCount>0 )
	{
		...
		for(ii=0; ii<MatchCount; ii++)
		{
			...
			if (ISNULL(pMatchSlot)) 
				continue;
			if (!inv_trh_MatchingItemInSlot(ATR_PASS_ARGS, pMatchSlot, pItemDef->pchName))
				continue;
			...
			else
			{
				...
				if ( StackLimit <= 0 )
				{
					...
					AddItem_Fixup_SlotItemPowersCreationTimes(pMatchSlot, pItem); // Annoying cleanup for power lifetime stuff. Remove if at all possible later.
...
int inv_bag_trh_GetMaxDropCountWithDef(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG NOCONST(InventoryBag)* pDestBag, ItemDef* pItemDef)
{
	...
	if ( inv_bag_trh_BagFull(ATR_PASS_ARGS, pEnt, pDestBag, GET_REF(pItemDef->hSlotID)) )
	{
		if ( pItemDef->iStackLimit > 1 )
		{
			...
			{
				if ( inv_trh_MatchingItemInSlotID( ATR_PASS_ARGS, pDestBag, ii, pItemDef->pchName ) )
				{
		...
		if (!bBagFull || pItemDef->iStackLimit > 1)
		{
			for (i = 0; i < invbag_trh_maxslots(pEnt, pDestBag); i++)
			{
				...
				if (ISNULL(pSlot))
				{
					...
					inv_trh_MatchingItemInSlotID(ATR_PASS_ARGS, pDestBag, i, pItemDef->pchName))
				{
...
// This gets called from both move and regular adding.
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Psaved.Ppallowedcritterpets, .Psaved.Ppownedcontainers, .pInventoryV2.peaowneduniqueitems, .pInventoryV2.Ppinventorybags[], .pInventoryV2.Pplitebags[], .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Itemidmax, .Hallegiance, .Hsuballegiance, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.Playertype, pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.peaowneduniqueitems")
ATR_LOCKS(pDestBag, ".*")
ATR_LOCKS(pAddItem, ".*");
bool inv_bag_trh_AddItem(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG CONST_EARRAY_OF(NOCONST(Entity)) eaPets,
						 ATH_ARG NOCONST(InventoryBag)* pDestBag, int iDestSlotIdx, ATH_ARG NOCONST(Item)* pAddItem,
							ATH_ARG NOCONST(InventoryBag)* pSrcBag, int iSrcSlotIdx,					 
							ItemAddFlags eFlags, U64** peaItemIDs, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	...
	if (invbag_trh_bagid(pDestBag) == InvBagIDs_Inventory && iDestSlotIdx == -1 && !(eFlags & ItemAdd_OverrideStackRules))
	{
		...
		if (NONNULL(pPlayerBagBag))
		{
			...
			{
				// Do NOT consider a bag as a destination if it's that particular bag that we are moving. 
				if (pSrcBag==NULL || !(invbag_trh_bagid(pSrcBag) == InvBagIDs_PlayerBags && iSrcSlotIdx == i))
				{
		...
		if (itemProgression_trh_ItemCanStack(pAddItem))
		{
			...
			for (i = 0; i < eaSize(&peaInventoryBags) && iCount > 0; i++)
			{
				...
				{
					if (inv_trh_MatchingItemInSlotID(ATR_PASS_ARGS, pCurrBag, j, pAddItemDef->pchName))
					{
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Hallegiance, .Hsuballegiance, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems");
bool inv_ent_trh_AddItem(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG CONST_EARRAY_OF(NOCONST(Entity)) eaPets, int BagID, int SlotIdx, NOCONST(Item)* pItem, ItemAddFlags eFlags, U64** peaItemIDs, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	...
	if (pBag)
	{
		return inv_bag_trh_AddItem(ATR_PASS_ARGS, pEnt, eaPets, pBag, SlotIdx, pItem, NULL, -1, eFlags, peaItemIDs, pReason, pExtract);
	}
	else if (pLiteBag)
	{
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Psaved.Ppbuilds, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems");
...
NOCONST(Item)* inv_bag_trh_RemoveItem(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, bool bSilent, ATH_ARG NOCONST(InventoryBag)* pBag, int SlotIdx, int count, bool bIncludeUnremoveables, const ItemChangeReason *pReason)
{
	int NumSlots = 0;
	NOCONST(InventorySlot)* pSlot = NULL;
	NOCONST(Item)* pItem;
	...
	if (!bIncludeUnremoveables && !item_trh_CanRemoveItem(pSlot->pItem) )
	{
		TRANSACTION_APPEND_LOG_FAILURE("Failed to remove item from Bag:Slot [%s:%d] on Ent [%s]: item_trh_CanRemoveItem() returned false.", 
			StaticDefineIntRevLookup(InvBagIDsEnum, iBagID),SlotIdx, 
			pchEntDebugName);
#ifdef GAMESERVER
		if (!bSilent)
			QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_FAIL, pEnt->myEntityType, pEnt->myContainerID, "InventoryUI.ItemSlottedOnAssignment", kNotifyType_ItemMoveFailed); 
#endif
		return NULL;
	}
	...
	if ( pSlot->pItem->count > 0 )
	{
		...
		// Sort of a complete hack for splitting items with power lifetimes.
		// We need to find the earliest creation time in any of the powers on the item.
		// Creation times use non-shard time because they are part of the powers system.
		// Need to sort this out at some point.
		// Also a discrepancy here between raw time and shard time. Because we are in the
		// power system we need to use raw time. But display on the timer is handled via
		// shard time. This needs to go away at some point. It is all a misfeature.
		// See also inv_bag_trh_AddItemFast and AddItem_Fixup_SlotItemPowersCreationTime in this file.
		{
			int i;
			U32 uEarliestCreationTime = timeSecondsSince2000(); // If we ever start using this with shard time, we need to allow for things in the future. This won't do it. 
			// Uses pSlot->pItem
			for (i=0;i<eaSize(&pSlot->pItem->ppPowers); i++)
			{
				NOCONST(Power)* pPower = pSlot->pItem->ppPowers[i];
				...
				if (pPower->pParentPower) pPower = pPower->pParentPower;
				if (pPower->uiTimeCreated < uEarliestCreationTime)
				{
					uEarliestCreationTime = pPower->uiTimeCreated;
				}
			}

			// And copy the value for the new item (partial stack)
			// Uses pItem
			for (i=0;i<eaSize(&pItem->ppPowers); i++)
			{
				NOCONST(Power)* pPower = pItem->ppPowers[i];
				if (pPower->pParentPower) pPower = pPower->pParentPower;
				pPower->uiTimeCreated = uEarliestCreationTime;
			}
		}
		//////////////////////////////////////////////////////////////////////////////////////////
	}
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pDstEnt, ".Psaved.Ppuppetmaster.Curtype, .Pplayer.Playertype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaDstPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems")
	ATR_LOCKS(pDstBag, ".*")
									 ...
									 GameAccountDataExtract *pExtract)
{
	...
	NOCONST(Item) *pSrcItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pSrcEnt, true, pSrcBag, iSrcSlot, 1, true, pSrcReason);
	NOCONST(Item) *pDstItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pDstEnt, true, pDstBag, iNumBagSlots-1, 1, true, pDestReason);
	...
	if (!inv_bag_trh_AddItem(ATR_PASS_ARGS, pDstEnt, eaDstPets, pDstBag, 1, pSrcItem, NULL, -1, ItemAdd_Silent, NULL, pDestReason, pExtract)) {
		TRANSACTION_APPEND_LOG_FAILURE(
	...
	if (!inv_bag_trh_AddItem(ATR_PASS_ARGS, pSrcEnt, eaSrcPets, pSrcBag, iSrcSlot, pDstItem, NULL, -1, ItemAdd_Silent, NULL, pSrcReason, pExtract)) {
		TRANSACTION_APPEND_LOG_FAILURE(
			"Could not move secondary upgrade into Bag %s:%d on Ent %s", 
			StaticDefineIntRevLookup(InvBagIDsEnum, iSrcBagID), iSrcSlot,
			pchSrcEntDebugName);
...
AUTO_TRANS_HELPER;
bool inv_trh_CanGemSlot(ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(Item) *pHolder, ATH_ARG NOCONST(Item) *pGem, int iDestGemSlot)
{
	...
	// Check to see if we are trying to replace an item. If the Item is flagged as fused, it cannot be replaced
	if (iDestGemSlot >= 0 && pHolder && pHolder->pSpecialProps && iDestGemSlot < eaSize(&pHolder->pSpecialProps->ppItemGemSlots))
	{
		ItemDef *pItemDef = GET_REF(pHolder->pSpecialProps->ppItemGemSlots[iDestGemSlot]->hSlottedItem);
		if (pItemDef && pItemDef->flags & kItemDefFlag_Fused)
		{
			return false;
		}
	}

	// Check to see if we satisfy the equip limits for this item. Cannot slot more than a gem's equip limit on one item.
	if (pGemDef && pGemDef->pEquipLimit && pHolder && pHolder->pSpecialProps)
	{
		ItemEquipLimitCategory eCategory = pGemDef->pEquipLimit->eCategory;
		int iCount = 0;
		int iCategoryCount = 0;
		int i;

		for (i = 0; i < eaSize(&pHolder->pSpecialProps->ppItemGemSlots); i++)
		{
			ItemDef *pCheckDef = GET_REF(pHolder->pSpecialProps->ppItemGemSlots[i]->hSlottedItem);
			ItemEquipLimitCategory eCheckCategory = SAFE_MEMBER2(pCheckDef, pEquipLimit, eCategory);

			if (!pCheckDef || i == iDestGemSlot)
			{
				continue;
			}

			if (pGemDef == pCheckDef && pGemDef->pEquipLimit->iMaxEquipCount)
			{
				if (++iCount >= pGemDef->pEquipLimit->iMaxEquipCount)
				{
					return false;
				}
			}
			...
			{
				ItemEquipLimitCategoryData* pData = item_GetEquipLimitCategory(eCategory);
	...
	// If this item is currently equipped, check the equip limits for all other equipped items as well
	if (NONNULL(pEnt))
	{
		int i, j, iNumBags = eaSize(&pEnt->pInventoryV2->ppInventoryBags);
		bool bEquipped = false;
		for(i = 0; i < iNumBags; i++)
		{
			InventoryBag* pBag = (InventoryBag*)pEnt->pInventoryV2->ppInventoryBags[i];
			if(!!(invbag_flags(pBag) & (InvBagFlag_EquipBag|InvBagFlag_WeaponBag|InvBagFlag_ActiveWeaponBag|InvBagFlag_DeviceBag)))
			{
				for (j = 0; j < eaSize(&pBag->ppIndexedInventorySlots); j++)
				{
					...
					if(pSlot->pItem && pSlot->pItem->id == pHolder->id)
					{
						bEquipped = true;
						break;
					}
		...
		if (bEquipped)
		{
			if (!inv_ent_trh_EquipLimitCheck(ATR_EMPTY_ARGS, pEnt, NULL, pHolderDef, pHolder, -1, -1, NULL, NULL))
			{
				return false;
			}
		}
	...
	return 
		...
		&& (!pHolderDef->ppItemGemSlots[iDestGemSlot]->iMinLevel ||
			pGemDef->iLevel >= pHolderDef->ppItemGemSlots[iDestGemSlot]->iMinLevel)
		&& (!pHolderDef->ppItemGemSlots[iDestGemSlot]->iMaxLevel ||
			pGemDef->iLevel <= pHolderDef->ppItemGemSlots[iDestGemSlot]->iMaxLevel);
}

AUTO_TRANS_HELPER;
void inv_trh_GemItemToSlot(ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(Item) *pItem, int idx, ATH_ARG NOCONST(Item) *pGem)
{
	NOCONST(SpecialItemProps)* pProps = item_trh_GetOrCreateSpecialProperties(pItem);
	...
	pSlot->uProgressionXP = itemProgression_trh_GetTotalXP(pGem);
	pSlot->uProgressionLevel = itemProgression_trh_GetLevel(pGem);
...
AUTO_TRANS_HELPER;
bool inv_trh_GemItem(ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(Item) *pHolder,ATH_ARG NOCONST(Item) *pGem, int iDestGemSlot)
{
	if(iDestGemSlot != -1)
	{
		if(inv_trh_CanGemSlot(pEnt, pHolder,pGem,iDestGemSlot))
		{
			inv_trh_GemItemToSlot(pEnt, pHolder, iDestGemSlot, pGem);
			return true;
		}
	}
	else
	{
		...
		if (iNumSlots > 0)
		{
			...
			{
				if (inv_trh_CanGemSlot(pEnt, pHolder, pGem, i) && 
				...
				{
					inv_trh_GemItemToSlot(pEnt, pHolder, i, pGem);
					return true;
				}
			}
		}
...
AUTO_TRANS_HELPER;
bool inv_trh_RemoveGemmedItem(ATR_ARGS, ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(Item) *pHolder, int iGemSlot,NOCONST(Item) **ppItemOut)
{
	...
	{
		ItemDef* pGemDef;
		U32 uProgXP = pHolder->pSpecialProps->ppItemGemSlots[iGemSlot]->uProgressionXP;
		U32 uProgLevel = pHolder->pSpecialProps->ppItemGemSlots[iGemSlot]->uProgressionLevel;
		...
		*ppItemOut = iGemSlot < iSlotCount  ? 
		...
		if (uProgXP > itemProgression_trh_GetInitialXPValue(*ppItemOut) || uProgLevel > 1)
		{
			NOCONST(AlgoItemProps)* pProps = item_trh_GetOrCreateAlgoProperties((*ppItemOut));
			pProps->uProgressionLevel = uProgLevel;
			pProps->uProgressionXP = uProgXP;
		}
...
bool inv_bag_trh_UnGemItem(ATR_ARGS, ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(InventoryBag) *pSrcBag, int SrcSlotIdx, U64 uSrcItemID, int iGemSlotIdx, ATH_ARG NOCONST(InventoryBag) *pDestBag, int DestSlotIdx, const char *pchCostNumeric, int iCost, GameAccountDataExtract *pExtract, const ItemChangeReason *pReason)
{
	...
	if (pSrcItemDef->flags & kItemDefFlag_Fused)
	{
		return false;
	}
	...
	//Remove the old item
	inv_trh_RemoveGemmedItem(ATR_PASS_ARGS,pEnt,pSrcItemHolder,iGemSlotIdx,&pSrcItem);

	//Add it to the bag
	if(!pSrcItem || !inv_bag_trh_AddItem(ATR_PASS_ARGS,pEnt,NULL,pDestBag,DestSlotIdx,pSrcItem,pSrcBag, SrcSlotIdx, 0,NULL,NULL,pExtract))
		return false;
							...
							GameAccountDataExtract *pExtract)
{
	...
	{
		inv_trh_RemoveGemmedItem(ATR_PASS_ARGS,pSrcEnt,pSrcItemHolder,iSrcGemmedSlot,&pSrcItem);
	}
	else
	{
		pSrcItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS,pSrcEnt,false,pSrcBag,iSrcSlot,1,false, pSrcReason);
	}
	...
	if(NONNULL(pSrcItem))
	{
		...
		{
			inv_trh_RemoveGemmedItem(ATR_PASS_ARGS,pSrcEnt,pDstItemHolder,iDstGemSlot,&pDstItem);
		}
		
		//Gem the item
		if(!inv_trh_GemItem(pSrcEnt,pDstItemHolder,pSrcItem,iDstGemSlot))
			return false;
		...
		if(pDstItem && eaSize(&s_ItemGemConfig.ppUnslotCosts) == 0)
		{
			if(pSrcItemHolder)
			{
				inv_trh_GemItem(pSrcEnt,pSrcItemHolder,pDstItem,iSrcGemmedSlot);
			}
			else
			{
				if(!inv_bag_trh_AddItem(ATR_PASS_ARGS,pSrcEnt,eaSrcPets,pSrcBag,iSrcSlot,pDstItem, pDstBag, iDstSlot, 0,NULL,NULL,pExtract))
					return false;
			}
		}
		else if(pDstItem)
...
//maybe TODO: restore old return iDstSlot functionality/-1 on fail so callers can know where their item went.
						  ...
						  GameAccountDataExtract *pExtract)
{
	...
	int iDstCount = 0;
	int bagFlags = 0;
	bool bSameEntities = entity_trh_IsOwnerSame(ATR_PASS_ARGS, pSrcEnt, pDstEnt);
//	bool bSameEntities = (pSrcEnt->myContainerID == pDstEnt->myContainerID && pSrcEnt->myEntityType == pDstEnt->myEntityType);
	...
	pSrcItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pSrcEnt, true, pSrcBag, iSrcSlot, iCount, bSameEntities, pSrcReason);
	...
	if (NONNULL(pSrcItem)) {
		...
		// source
	// WOLF[20May13] This functionality dates back to SVN 73022. The idea was to distinguish "Adding" from "Moving" under the assumption
	// that 'moving' was the player rearranging their inventory manually. inv_bag_trh_AddItem has code that treats the main bag (InvBagIDs_Inventory)
	// as representative of the whole player inventory and will look for things in other bags to stack with, or will try a different bag if
	// the actual main inventory bag is full. So the idea WAS to override this behaviour in most cases. In reality now we do NOT want to override
	// this 'stacking' behaviour most of the time. The only case where we want to would be if the player was explicitly moving something to the
	// main inventory bag with no specified slot (this corresponds in NW as dragging an item onto the bag item at the bottom of the inventory). We
	// do not currently have a way of distinguishing this case from the 'move to generic inventory'. Both are Bag ID InvBagIDs_Inventory with no
	// specified slot. In any case, to fix numerous other bugs (such as 'unequip' failing if the main bag is full [NW-17102]) we are going to default
	// to never overriding the stack rules here . This will cause artifacts
	// with dragging onto the main bag icon (for instance, dragging something stackable onto the main will search other bags for things to stack with).
	// Eventually we should consider rearchitecting this whole system so that we have an explicit way of distinguishing 'main bag' from 'general bag inventory'.
...
//			eFlags |= ItemAdd_OverrideStackRules;
		...
		return inv_bag_trh_AddItem(ATR_PASS_ARGS, pDstEnt, eaDstPets, pDstBag, iDstSlot, pSrcItem, pSrcBag, iSrcSlot, eFlags, NULL, pDestReason, pExtract);
	} else {
		return false;
	}
}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Psaved.Ppownedcontainers, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Itemidmax, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags[], .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Playertype, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
AUTO_TRANS_HELPER;
void inv_trh_InitAndFixupItemDeluxe(ATR_ARGS,ATH_ARG NOCONST(Item) *pItem, ATH_ARG NOCONST(InventorySlot) *pSlot, ATH_ARG NOCONST(Entity)* pEnt,  bool bFixItemIDs, const ItemChangeReason *pReason)
{
...
// This function was created for the SavedPets who need to get their item IDs fixed up, but don't need to do weird special case things that we would
// only want to do on login
AUTO_TRANS_HELPER 
ATR_LOCKS(pEnt, ".Pinventoryv2.Ppinventorybags, .Pinventoryv2.Peaowneduniqueitems, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Itemidmax, .Pplayer.Playertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
void inv_trh_FixupAllItems(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, const ItemChangeReason *pReason)
{
	int NumBags,iBag;
	const bool bFixItemIDs = true;
	...
	for(iBag=0;iBag<NumBags;iBag++)
	{
		...
		for(iSlot=0;iSlot<NumSlots;iSlot++)
		{
			...
			if (NONNULL(pSlot->pItem) )
			{
				...
				inv_trh_InitAndFixupItemDeluxe(ATR_PASS_ARGS,pSlot->pItem,pSlot,pEnt,bFixItemIDs,pReason);
...
void inv_trh_FixupInventory(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, bool bFixItemIDs, const ItemChangeReason *pReason)
{
	...
	for(iBag=0;iBag<NumBags;iBag++)
	{
		...
		// We must be very careful here. The slot earray can suddenly be collapsed if
		// we find that all remaining slots in a bag are empty. Theoretically we could also
		// add slots if somebody is doing something dangerous with move code. The two moves below seems safe-ish against adds.
		// They both can only move items OUT of a bag to a different bag. So we should not have to worry about
		// slots being added in our current bag.
		//  Anyway, to deal with collapses, process the array backward and be aware we may suddenly become much shorter.
		//
		//  In general:
		//  BE VERY CAREFUL WITH ADDING CODE HERE. Arbitrarily deleting or moving items can have unexpected effects on the
		//  slots.
		//
		//  We will need to go back and clean up all InventorySlots loops to account for similar issues. Collapse does not set
		//   the entries to NULL, so the earray will still seem to contain semi-valid data.
		//
		//  The case that was crashing here involved a character login where the character had a lockToRestrictBag item that needed
		//  fix-up as the last item in a bag that for unknown reason was uncollapsed (there were several empty slots after the item-to-be-moved)
		...
		while (iSlot >= 0)
		{
			...
			if (NONNULL(pSlot->pItem) )
			{
				...
				//BH 08/31/2010: This used to be in what was called "inv_trh_FixupItem" (now "inv_trh_InitAndFixupItem"), but since that function 
				...
				if(pItemDef)
				{
					...
					if(!(pSlot->pItem->flags & kItemFlag_Bound) && pItemDef->flags & kItemDefFlag_BindOnPickup)
					{
						...
						{
							NOCONST(Item) *pRemItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pEnt, true, pBag, iSlot, -1, true, pReason);
				...
				if(pSlot->pItem->pSpecialProps && pSlot->pItem->pSpecialProps->ppItemGemSlots)
				{
					...
					for(iGemSlot=0;iGemSlot<eaSize(&pSlot->pItem->pSpecialProps->ppItemGemSlots);iGemSlot++)
					{
						if(IS_HANDLE_ACTIVE(pSlot->pItem->pSpecialProps->ppItemGemSlots[iGemSlot]->hSlottedItem))
						{
							...
							//inv_trh_InitAndFixupItemDeluxe(ATR_PASS_ARGS,pSlot->pItem->ppItemGemSlots[iGemSlot]->pSlottedItem,NULL,pEnt,bFixItemIDs,pReason);
						}
					}
				}
				...
				***/
				...
				// This is a much simpler pass at the above.  I expect people to iterate on this and make it more comprehensive.
				// There are regrettably several different functions that do various checks to see what's allowed.  They are not well centralized.
				// See inv_trh_FixupEquipBags, which only works on equippable items, and only on items that are not locked to restrict, and
				// throws all found items into the overflow bag.  [RMARR - 7/24/13]
				if (pItemDef && (pItemDef->flags & kItemDefFlag_LockToRestrictBags) && !item_trh_ItemDefMeetsBagRestriction(pItemDef,pBag))
				{
					const bool bGive = true;
					bool bMoveSucceeded = false;
					InvBagIDs destBagID;
					destBagID = inv_trh_GetBestBagForItemDef(pEnt, pItemDef, pSlot->pItem, pSlot->pItem->count, bGive, NULL);
					if (destBagID != InvBagIDs_None)
					{
						bMoveSucceeded = inv_MoveItemSelf(ATR_PASS_ARGS, pEnt, pBag->BagID, destBagID, iSlot, -1, pSlot->pItem->count, ItemAdd_Silent, pReason, NULL);
					}
					
					if (!bMoveSucceeded)
					{
						Errorf("Item %s is in a slot it's not allowed in, and I can't fix it for entity %s",pItemDef ? pItemDef->pchName : "[Unknown]", pEnt->debugName);
					}
				}

				// don't add code here.  You're wrong.  (The above code can move the item, invalidating your stack variables)
			}

			////////////
			// WHILE loop update (we're going backward)

			iSlot -= 1;
			
			// IMPORTANT!  We need to deal with the fact that the number of slots may have wildly changed.
			// (See note at top of loop. We DO NOT deal with the case of slots got added.)
			// We may have lost several slots though.
	...
	for(iBag = eaSize(&pEnt->pInventoryV2->ppInventoryBags)-1; iBag >= 0; iBag--)
	{
		...
		for (iSlot = eaSize(&pBag->ppIndexedInventorySlots)-1; iSlot >= 0; iSlot--)
		{
			...
			if(pSlot->pItem)
			{
				...
				// Check for item sets on any gems that are slotted on this item
				if (pSlot->pItem->pSpecialProps)
				{
					S32 iItemGemSlot;
					S32 iItemGemSlotCount = eaSize(&pSlot->pItem->pSpecialProps->ppItemGemSlots);
					for (iItemGemSlot = 0; iItemGemSlot < iItemGemSlotCount; iItemGemSlot++)
					{
						ItemDef *pGemItemDef = GET_REF(pSlot->pItem->pSpecialProps->ppItemGemSlots[iItemGemSlot]->hSlottedItem);
						if (pGemItemDef)
						{
							S32 iItemSet;
							S32 iItemSetCount = eaSize(&pGemItemDef->ppItemSets);
							for (iItemSet = 0; iItemSet < iItemSetCount; iItemSet++)
							{
								pItemSet = GET_REF(pGemItemDef->ppItemSets[iItemSet]->hDef);
								if(pItemSet)
								{
									S32 iSet = eaFind(&ppItemSets,pItemSet);
									if(iSet<0)
			...
			for(iSlot = NumSlots -1; iSlot >= 0; --iSlot)
			{
				...
				if ( NONNULL(pSlot->pItem) )
				{
					...
					if(!(eItemDefFlags & kItemDefFlag_LockToRestrictBags) &&
						...
						!inv_ent_trh_EquipLimitCheck(ATR_PASS_ARGS, pEnt, pBag, pItemDef, pSlot->pItem, pBag->BagID, iSlot, NULL, NULL)) ||
						(iMaxSlots > 0 && iSlot >= iMaxSlots))
					{
						...
						{
							NOCONST(Item) *pRemItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pEnt, true, pBag, iSlot, pSlot->pItem->count, true, pReason);
...
// Note if you change this function make sure it still works with STO OVERRIDE_LATELINK_gameSpecific_EntitySharedBankExtraData
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.Pscpdata, .Pchar.Ilevelexp, .Psaved.Ppbuilds, .Pinventoryv2, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Uiindexbuild, .Pplayer.Playertype, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pplayer.Eskillspecialization, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pplayer.Skilltype, .Psaved.Pscpdata, .Pchar.Hclass, .Pchar.Ilevelexp, .Psaved.Ppbuilds, .Pinventoryv2, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Uiindexbuild, .Pplayer.Playertype, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Playertype, .Hallegiance, .Hsuballegiance, .Itemidmax, .Psaved.Ppownedcontainers, .pInventoryV2.Ppinventorybags, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .pInventoryV2.Pplitebags, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
int inv_lite_trh_CountItems( ATR_ARGS, ATH_ARG NOCONST(InventoryBagLite)* pBag, const char *ItemDefName )
{
	...
	else
	{
		...
		bagiterator_SetLiteBag(pBag, pIter);
								 ...
								 S32* piEquipLimit)
{
		...
		(!pDstBag || (invbag_trh_flags(pDstBag) & eFlags)))
	{
		...
		S32 i, j, iNumBags = eaSize(&pEnt->pInventoryV2->ppInventoryBags);
		bool bAtLimit = false;
		...
		for (i = 0; i < iNumBags; i++)
		{
			...
			if (invbag_trh_flags(pBag) & eFlags)
			{
				...
				BagIterator* pIter = bagiterator_Create();
				bagiterator_SetBag(pBag, pIter);
				for (; !bagiterator_Stopped(pIter); bagiterator_Next(pIter))
				{
					Item *pCheckItem = CONTAINER_RECONST(Item, bagiterator_GetItem(pIter));
					ItemDef* pCheckDef = pCheckItem ? GET_REF(pCheckItem->hItem) : NULL;
					ItemEquipLimitCategory eCheckCategory = SAFE_MEMBER2(pCheckDef, pEquipLimit, eCategory);
					...
					// Check GemSlots for EquipLimits
					if (pCheckItem && pCheckItem->pSpecialProps)
					{
						for (j = 0; j < eaSize(&pCheckItem->pSpecialProps->ppItemGemSlots); j++)
						{
							pCheckDef = GET_REF(pCheckItem->pSpecialProps->ppItemGemSlots[j]->hSlottedItem);
							eCheckCategory = SAFE_MEMBER2(pCheckDef, pEquipLimit, eCategory);
	...
	if (pItem && pItem->pSpecialProps)
	{
		...
		for (i = 0; i < eaSize(&pItem->pSpecialProps->ppItemGemSlots); i++)
		{
			...
			if (pGemDef)
			{
				if(!inv_ent_trh_EquipLimitCheck(ATR_PASS_ARGS, pEnt, pDstBag, pGemDef, NULL, iIgnoreBagID, iIgnoreSlot, ppLimitCategory, piEquipLimit))
				{
					return false;
				}
			}
									 ...
									 S32* piEquipLimit)
{
	...
	if (pSrcItemDef && !inv_ent_trh_EquipLimitCheck(ATR_PASS_ARGS, pDstEnt, pDstBag, pSrcItemDef, pSrcItem, -1, -1, ppLimitCategory, piEquipLimit))
	{
		return false;
	}
	if (pDstItemDef && !inv_ent_trh_EquipLimitCheck(ATR_PASS_ARGS, pSrcEnt, pSrcBag, pDstItemDef, pSrcItem, -1, -1, ppLimitCategory, piEquipLimit))
	{
		return false;
	}
	return true;
...
#define WITHDRAW_LIMIT_TIME 86400	// One day

bool inv_guildbank_withdrawals_in_current_interval(U32 uWithdrawalTimestamp)
{
	U32 uTimestamp = ShardClock_SecondsSince2000();
	
	if (gConf.bGuildWithdrawalUsesShardInterval)
	{
		// New-style. Always use daily.
		return(ShardIntervalTiming_TimeIsInCurrentInterval(uWithdrawalTimestamp,kShardIntervalTimingMode_Daily));
	}
	else
	{
		// Old style
		if (uTimestamp >= uWithdrawalTimestamp && (uTimestamp - uWithdrawalTimestamp) < WITHDRAW_LIMIT_TIME)
		{
			return(true);
		}
	}
...
bool inv_guildbank_trh_CanWithdrawFromBankTab(ATR_ARGS, ContainerID iContainerID, ATH_ARG NOCONST(Guild) *pGuild, ATH_ARG NOCONST(Entity) *pGuildBank, InvBagIDs iBagID, U32 iCount, U32 iValue) {
	...
	{
		iWithdrawTotal = iValue;
		if (NONNULL(pWithdrawLimit))
		{
			if (inv_guildbank_withdrawals_in_current_interval(pWithdrawLimit->iTimestamp))
			{
				// Using the value here. Not number of items.
				iWithdrawTotal+=pWithdrawLimit->iWithdrawn;
			}
		}
...
bool inv_guildbank_trh_UpdateBankTabWithdrawLimit( ATR_ARGS, ContainerID iContainerID, ATH_ARG NOCONST(Guild) *pGuild, ATH_ARG NOCONST(Entity) *pGuildBank, InvBagIDs iBagID, U32 iCount, U32 iValue )
{
	...
	{
		iItemWithdrawTotal = iCount;
		if (NONNULL(pWithdrawLimit))
		{
			if (inv_guildbank_withdrawals_in_current_interval(pWithdrawLimit->iTimestamp))
			{
				// Using the count here. 
				iItemWithdrawTotal+=pWithdrawLimit->iItemsWithdrawn;
			}
		}
	...
	if (!inv_guildbank_withdrawals_in_current_interval(pWithdrawLimit->iTimestamp))
	{
		// If this happens outside of the interval we were previously tracking, resave the timestamp
		pWithdrawLimit->iTimestamp = ShardClock_SecondsSince2000();
	}
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pBag, ".ppIndexedLiteSlots");
...
{
	NOCONST(InventorySlotLite)* pSlot = eaIndexedGetUsingString(&pBag->ppIndexedLiteSlots, pchItemDefName);
	ItemDef *pItemDef = pSlot ? GET_REF(pSlot->hItemDef) : NULL;
	S32 iOldValue;
	...
	if(pEnt->myEntityType!=GLOBALTYPE_NONE) // Don't bother with fake entities
	{
		...
		if (pItemDef->bLogForEconomy 
			...
			enumLogCategory eLogCategory = LOG_GAMEECONOMY;
			if (pItemDef->eLogCategory) {
				eLogCategory = pItemDef->eLogCategory;
			}

			TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(eLogCategory, "EconomyNumericChange",
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.ppInventoryBags[], .Pchar.Ilevelexp, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pBag, ".Ppindexedliteslots");
bool inv_lite_trh_OnNumericChanged(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG NOCONST(InventoryBagLite)* pBag, const char* pchItemName, bool bSilent, const ItemChangeReason *pReason)
{
	...
	else if(stricmp(pchItemName, "Level") == 0)
	{
...
/*
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Itemidmax, pInventoryV2.ppInventoryBags[], .Pplayer.Playertype, .Pplayer.Eaplayernumericthresholds")
	ATR_LOCKS(pBag, ".ppIndexedLiteSlots[]");
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pchar.Ilevelexp, .pPlayer.eaPlayerNumericThresholds[]");
void inv_trh_NumericThresholds(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ItemDef *pItemDef, const char *astrItemDefName, S32 iOldValue, S32 iNewValue, const ItemChangeReason *pReason)
{
	if(NONNULL(pEnt) && NONNULL(pEnt->pPlayer) && pItemDef && iNewValue > iOldValue)
	{
		if(eaSize(&pItemDef->eaNumericThresholdDefs) && kItemType_Numeric == pItemDef->eType)
		{
			S32 iValueToAdd = iNewValue - iOldValue;
			U32 uCurrentTime = timeSecondsSince2000();
			NOCONST(PlayerNumericThresholds) *pPlayerNumericThresholds;

			assertmsg(!nullStr(astrItemDefName), "inv_trh_NumericThresholds: astrItemDefName is empty!");
			assertmsg(astrItemDefName == allocAddString(astrItemDefName), "inv_trh_NumericThresholds: astrItemDefName is not pooled!");

			pPlayerNumericThresholds = eaIndexedGetUsingString(&pEnt->pPlayer->eaPlayerNumericThresholds, astrItemDefName);
			if(!pPlayerNumericThresholds)
			{
				pPlayerNumericThresholds = StructCreateNoConst(parse_PlayerNumericThresholds);
				pPlayerNumericThresholds->astrItemDefName = astrItemDefName;
				eaIndexedPushUsingStringIfPossible(&pEnt->pPlayer->eaPlayerNumericThresholds, astrItemDefName, pPlayerNumericThresholds);
			}

			FOR_EACH_IN_EARRAY(pItemDef->eaNumericThresholdDefs, NumericThresholdDef, pNumericThresholdDef)
			{
				NOCONST(PlayerNumericThreshold) *pPlayerNumericThreshold = eaIndexedGetUsingString(&pPlayerNumericThresholds->eaPlayerNumericThreshold, pNumericThresholdDef->astrThresholdName);
				if(!pPlayerNumericThreshold)
				{
					pPlayerNumericThreshold = StructCreateNoConst(parse_PlayerNumericThreshold);
					pPlayerNumericThreshold->astrThresholdName = pNumericThresholdDef->astrThresholdName;
					pPlayerNumericThreshold->iNumericEarned = 0;
					eaIndexedPushUsingStringIfPossible(&pPlayerNumericThresholds->eaPlayerNumericThreshold, pPlayerNumericThreshold->astrThresholdName, pPlayerNumericThreshold);
				}
				else
				{
					U32 uTimeDifference = uCurrentTime - pPlayerNumericThreshold->uTimestamp;
					F32 fCooldownRate = item_GetNumericThresholdCooldownRateForLevel(pNumericThresholdDef, entity_trh_GetSavedExpLevel(pEnt));
					pPlayerNumericThreshold->iNumericEarned -= uTimeDifference * fCooldownRate;
					if(pPlayerNumericThreshold->iNumericEarned < 0)
						pPlayerNumericThreshold->iNumericEarned = 0;
				}
				pPlayerNumericThreshold->uTimestamp = uCurrentTime;

				if(pReason)
				{
					if((!pNumericThresholdDef->bOnlyUGC || pReason->bUGC) && (!pNumericThresholdDef->bOnlyKillCredit || pReason->bKillCredit))
					{
						S32 iNumericEarnedPrev = pPlayerNumericThreshold->iNumericEarned;
						pPlayerNumericThreshold->iNumericEarned += iValueToAdd;

#ifdef GAMESERVER
						if(pNumericThresholdDef->bAlert)
						{
							// Since this is also triggered on Gateway servers, we cannot lookup the Entity by entContainerID in the remote command. Pass all the relevant data to it.
							S32 iThreshold = item_GetNumericThresholdForLevel(pNumericThresholdDef, entity_trh_GetSavedExpLevel(pEnt));
							U32 uDuration = item_GetNumericThresholdDurationForLevel(pNumericThresholdDef, entity_trh_GetSavedExpLevel(pEnt));
							if(pPlayerNumericThreshold->iNumericEarned >= iThreshold && iNumericEarnedPrev < iThreshold)
							{
								// Since this is also triggered on Gateway servers, we cannot lookup the Entity by entContainerID, so we will use 2 commands.
								// The verbose one also gives entity position.
								QueueRemoteCommand_gslPlayer_NumericThresholdExceededAlert(ATR_RESULT_SUCCESS, GetAppGlobalType(), GetAppGlobalID(), pEnt->debugName,
									astrItemDefName, pNumericThresholdDef->astrThresholdName, iThreshold, uDuration, iOldValue, iNewValue, pReason);
							}
						}
#endif
					}
...

{
	...
	bool bIsLevellingNumeric;
	const char* pchPooledName = allocFindString(pcItemDefName);
	ItemDef *pItemDef;

	if(ISNULL(pBag) || ISNULL(pchPooledName))
		return false;

	bIsLevellingNumeric = (stricmp(pchPooledName, gConf.pcLevelingNumericItem) == 0);

	pSlot = inv_lite_trh_GetOrCreateLiteSlot(pBag, pchPooledName);
	pItemDef = GET_REF(pSlot->hItemDef);
	...
	if (pItemDef)
	{
		switch(op)
		{
			xcase NumericOp_Add:
			{
				...
				// We want to perform logging, alerting, and governing of numerics using thresholds, but before the bonus XP is applied.
				// This is to prevent false positives for players using a bonus XP item or event.
				if(finalVal > originalValue)
					inv_trh_NumericThresholds(ATR_RECURSE, pEnt, pItemDef, pchPooledName, originalValue, finalVal, pReason);
				...
				// adjust the value
				bSucceeded = inv_lite_trh_SetNumericInternal(ATR_PASS_ARGS, pEnt, bSilent, pBag, pchPooledName, (S32)finalVal, true, pReason);

			}
			xcase NumericOp_RaiseTo: 
				if(pSlot->count < iValue)
				{
					inv_trh_NumericThresholds(ATR_RECURSE, pEnt, pItemDef, pchPooledName, pSlot->count, iValue, pReason);

					bSucceeded = inv_lite_trh_SetNumericInternal(ATR_PASS_ARGS, pEnt, bSilent, pBag, pchPooledName, iValue, false, pReason);
				}
			...
			xcase NumericOp_SetTo:
				...
				bSucceeded = inv_lite_trh_SetNumericInternal(ATR_PASS_ARGS, pEnt, bSilent, pBag, pchPooledName, iValue, true, pReason);
		}
	}
	else
	{
		...
		{
			ErrorDetailsf("Can't grant <%s> with reason %s", pchPooledName, pReason->pcDetail);
		}
		else
		{
			ErrorDetailsf("Can't grant <%s> without a reason", pchPooledName);
		}
	...
	if (!inv_lite_trh_OnNumericChanged(ATR_PASS_ARGS, pEnt, pBag, pchPooledName, bSilent, pReason))
		return false;
...
ATR_LOCKS(pEntDst, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, pInventoryV2.ppInventoryBags[], .pInventoryV2.Pplitebags[], .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, .Pplayer.Eaplayernumericthresholds");
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".pInventoryV2.Ppinventorybags, .pInventoryV2.Pplitebags, .Pplayer.Playertype, pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
InvBagIDs inv_trh_GetBestRestrictBagForItemDef(ATH_ARG NOCONST(Entity)* pEnt, ItemDef* pItemDef, ATH_ARG NOCONST(Item)* pItem, int iCount, bool bGive, InvBagFlag eExcludedFlags, GameAccountDataExtract *pExtract)
{
	InvBagIDs BagID = InvBagIDs_None;
	InvBagIDs eRestrictBagID;

	if (!inv_ent_trh_EquipLimitCheck(ATR_EMPTY_ARGS, pEnt, NULL, pItemDef, pItem, -1, -1, NULL, NULL))
	{
...
InvBagIDs inv_trh_GetBestBagForItemDef(ATH_ARG NOCONST(Entity)* pEnt, ItemDef* pItemDef, ATH_ARG NOCONST(Item)* pItem, int iCount, bool bGive, GameAccountDataExtract *pExtract)
{
	bool found = false;
	InvBagIDs BagID = InvBagIDs_None;
	...
	if ( iCount > 0 )
	{
		...
		if (eaiSize(&pItemDef->peRestrictBagIDs) > 0)
		{
			if (!bGive || !gConf.bDontAutoEquipUpgrades || !(pItemDef->eType == kItemType_Upgrade ||
				...
				BagID = inv_trh_GetBestRestrictBagForItemDef(pEnt, pItemDef, pItem, iCount, bGive, eExcludedFlags, pExtract);
		}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pinventoryv2.Peaowneduniqueitems, .Hallegiance, .Hsuballegiance, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
ATR_LOCKS(pItem, ".Flags, .Id, .Hitem, .Pspecialprops.Hcostumeref, .Pppowers, .Palgoprops.Ppitempowerdefrefs, .Pspecialprops.Pcontainerinfo.Hsavedpet, .Palgoprops.Quality, .Pspecialprops.Ppitemgemslots, .Pspecialprops.Psupercritterpet.Pchname, .bForceBind")
...
void inv_trh_ItemAddedCallbacks(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, bool bSilent, ATH_ARG NOCONST(Item)* pItem, ATH_ARG NOCONST(InventoryBag)* pDestBag, int DestSlotIdx, int count, bool bForceBind, const ItemChangeReason *pReason)
{
	...
	{
		// bForceBind flag is weird because stores do this by passing a parameter through transactions (because the in-store item is not given to the player, it's copied by def)
		// but reward tables set this flag on the item (the item is given to the player, but through one of many code paths).
		if (pItem->bForceBind || bForceBind || (pItemDef->flags & kItemDefFlag_BindOnPickup))
			pItem->flags |= kItemFlag_Bound;
	...
	StickerBook_trh_MaybeRecentlyAcquiredItem(pEnt, pItemDef);
	...
	if(pEnt->myEntityType!=GLOBALTYPE_NONE) // Don't bother with fake entities
	{
		...
		else if (NONNULL(pItem->pSpecialProps) && 
			(invbag_trh_flags(pDestBag) & (InvBagFlag_EquipBag|InvBagFlag_WeaponBag|InvBagFlag_ActiveWeaponBag|InvBagFlag_DeviceBag)))
		{
			int i, s = eaSize(&pItem->pSpecialProps->ppItemGemSlots);
			for (i = 0; i < s; i++)
			{
				pItemDef = GET_REF(pItem->pSpecialProps->ppItemGemSlots[i]->hSlottedItem);
				if (pItemDef && eaSize(&pItemDef->ppItemSets))
				{
					GlobalType eOwnerType = pEnt->myEntityType;
					ContainerID uOwnerID = pEnt->myContainerID;
	...
	if(pEnt->myEntityType!=GLOBALTYPE_NONE) // Don't bother with fake entities
	{
		...
		if (!bSilent)
		{
			...
			if((pEnt->myEntityType != GLOBALTYPE_ENTITYCRITTER) && 
			   (gbEnableItemTrackingLogging || (pItemDef->bLogForTracking)) &&
			   (pItemDef->eType != kItemType_Numeric))
			{
				// Determine the log category
				enumLogCategory eLogCategory = LOG_ITEMTRACKING;
				if (!pItemDef->bLogForTracking) {
					eLogCategory = LOG_ITEMTRACK_AUTO;
				} else if (pItemDef->eLogCategory) {
					eLogCategory = pItemDef->eLogCategory;
				}

				TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(eLogCategory, "ItemRemoved",
...
bool invbag_trh_baseMaxSlotsHelper(ATH_ARG NOCONST(Entity) *pEnt, const InvBagVariableMaxSlotDef* pBagMaxSlot, int *pMaxSlotsOut)
{
	...
	{
		pchTableNumeric = REF_STRING_FROM_HANDLE(pBagMaxSlot->hMaxSlotTableNumericPremium);
	}

	// If player is not premium, or there is no premium numeric specified, then use the standard numeric.
	if ( pchTableNumeric == NULL )
	{
		pchTableNumeric = REF_STRING_FROM_HANDLE(pBagMaxSlot->hMaxSlotTableNumericStandard);
	}

	if(pBagSlotTable = inv_FindBagSlotTableByName(pBagMaxSlot->pchMaxSlotTable))
	{
		*pMaxSlotsOut = invbag_trh_GetMaxSlotsFromTable(pEnt, pBagSlotTable, pchTableNumeric);
		return true;
	}
...
int invbag_trh_basemaxslots(ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(InventoryBag) *pBag)
{
    ...
    if(bag_def->MaxSlots<0 && !bag_def->primaryMaxSlots.pchMaxSlotTable)
        return -1;
    
	if (!invbag_trh_baseMaxSlotsHelper(pEnt, &bag_def->primaryMaxSlots, &iMaxSlots))
	{	// if the primary max slots fails, just return 
		return bag_def->MaxSlots;
	}

	// if we have some additionalMaxSlotsLimit, check it now and if we already exceed it, return
	if (bag_def->additionalMaxSlotsLimit > 0 && 
		iMaxSlots >= bag_def->additionalMaxSlotsLimit)
	{
		return bag_def->additionalMaxSlotsLimit;
	}

	// add in the slots from the eaAdditionalMaxSlots, if any
	FOR_EACH_IN_EARRAY_FORWARDS(bag_def->eaAdditionalMaxSlots, InvBagVariableMaxSlotDef, pMaxSlotDef)
	{
		int iMaxSlotsTmp = 0;
		if (invbag_trh_baseMaxSlotsHelper(pEnt, pMaxSlotDef, &iMaxSlotsTmp))
		{
			iMaxSlots += iMaxSlotsTmp;
			// check if we have a limit, and if we exceed it, just return the limit
			if (bag_def->additionalMaxSlotsLimit > 0 && 
				iMaxSlots >= bag_def->additionalMaxSlotsLimit)
			{
				return bag_def->additionalMaxSlotsLimit;
...
AUTO_TRANSACTION 
ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Hallegiance, .Hsuballegiance, .Itemidmax, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Psaved.Ppownedcontainers, pInventoryV2.ppInventoryBags[], .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.Playertype, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems");
enumTransactionOutcome inv_AddItem(ATR_ARGS, NOCONST(Entity)* pEnt, CONST_EARRAY_OF(NOCONST(Entity)) eaPets, int BagID, int SlotIdx, NON_CONTAINER Item* pItem, const char* pchItemDefName, int /*ItemAddFlags*/ eFlags, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
		...
		(pItemDef->eType != kItemType_Numeric))
	{
		...
		TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(eLogCategory, "ItemAdded",
 				"Item \"%s\" Count %d Reason \"%s\" Detail \"%s\" Map \"%s\" Pos \"%g,%g,%g\"",
				pItemDef->pchName, 
				iCount,
				(pReason && pReason->pcReason ? pReason->pcReason : "Unknown"), 
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags[], .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pchar.Ilevelexp");
Item* invbag_RemoveItem(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, bool bSilent, int BagID, int SlotIdx, int Count, bool bIncludeUnremoveables, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	NOCONST(InventoryBag)* pBag = NULL;
	NOCONST(Item)* pItem;
	...
	if (ISNULL(pBag))
	{
		...
		if (inv_lite_trh_ApplyNumeric(ATR_PASS_ARGS, pEnt, bSilent, pBagLite, pSlot->pchName, -Count, NumericOp_Add, pReason))
		{
		...
		{
			TRANSACTION_APPEND_LOG_FAILURE( "Failed to remove item in Bag:Slot [%s:%d] on Ent [%s]: Failed to remove requested number of itemlites.", 
				StaticDefineIntRevLookup(InvBagIDsEnum,BagID), SlotIdx, pEnt->debugName );
			return NULL;
		}
	...
	pItem = inv_bag_trh_RemoveItem(ATR_PASS_ARGS, pEnt, bSilent, pBag, SlotIdx, Count, bIncludeUnremoveables, pReason);
	return (Item*)pItem;
}
...
AUTO_TRANSACTION
ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Ppinventorybags, .Psaved.Conowner.Containertype, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Pchar.Ilevelexp, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags");
enumTransactionOutcome inventory_RemoveItemByDefName(ATR_ARGS, NOCONST(Entity)* pEnt, const char *def_name, int count, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	...
	iCount += inv_bag_trh_FindItemCountByDefName(ATR_PASS_ARGS,pEnt,InvBagIDs_Overflow,def_name,0,count,true,false,pReason,pExtract,NULL);
	if(iCount == count)
	{
	...
	iCount += inv_trh_FindItemCountByDefNameEx(ATR_PASS_ARGS,pEnt,InvBagFlag_DefaultInventorySearch,0,def_name,0,count,true,false,pReason,pExtract,NULL);
	if(iCount == count)
	{
...
*/
AUTO_TRANS_HELPER;
bool inv_bag_trh_MultiBagFindItem(ATR_ARGS, ATH_ARG NOCONST(InventoryBag)*** peaBags, BagIterator* pIter, ItemFindCriteria eCriteria, const void* pSearchParam, S32 excludeItemFlags)
{
	if(!pIter || (pIter->i_bag == -1 && eaSize(peaBags) <= 0))
		return false;

	//if this is the start of a new iteration, set up the iterator.
	if (pIter->i_bag == -1)
	{
		bagiterator_SetBagEarray(peaBags, pIter);
	}
	...
	while (!bagiterator_Stopped(pIter))
	{
		...
		if (!pItem || (pItem->flags & excludeItemFlags))
		{
			bagiterator_Next(pIter);
			continue;
		}
...
AUTO_TRANS_HELPER;
bool inv_bag_trh_FindItem(ATR_ARGS, ATH_ARG NOCONST(InventoryBag)* pBag, BagIterator* pIter, ItemFindCriteria eCriteria, const void* pData, S32 excludeItemFlags)
{
	...
	return inv_bag_trh_MultiBagFindItem(ATR_PASS_ARGS, &eaBags, pIter, eCriteria, pData, excludeItemFlags);
}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, "pInventoryV2.ppInventoryBags[]");
bool inv_bag_trh_FindItemByDefName(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int BagID, const char* def_name, S32 excludeItemFlags, BagIterator* res)
{
	NOCONST(InventoryBag) *bag;
	...
	return inv_bag_trh_FindItem(ATR_PASS_ARGS, bag, res, kFindItem_ByName, def_name, excludeItemFlags);
}

bool inv_bag_FindItem(Entity* pEnt, int BagID, BagIterator* res, ItemFindCriteria eCriteria, const void* pSearchParam, bool bIncludePlayerBags, bool bIncludeOverflow)
{
	GameAccountDataExtract* pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
	InvBagIDs i;
	NOCONST(InventoryBag)* pBag = NULL;
	...
	return inv_bag_trh_MultiBagFindItem(ATR_EMPTY_ARGS, &eaBags, res, eCriteria, pSearchParam, 0);
}

AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, "pInventoryV2.ppInventoryBags[]");
bool inv_bag_trh_FindItemByType(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int BagID, int eType, BagIterator* res)
{
	NOCONST(InventoryBag) *bag;
	...
	return inv_bag_trh_FindItem(ATR_PASS_ARGS, bag, res, kFindItem_ByType, (void*)eType, 0);//don't judge me!
}
...
bool inv_trh_FindItemByIDEx(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, BagIterator* pIter, U64 item_id, bool bIncludeOverflowBag, bool bSearchAllBags)
{
	int i;
	...
	bRet = inv_bag_trh_MultiBagFindItem(ATR_PASS_ARGS, &eaBags, pIter, kFindItem_ByID, &item_id, 0);
...
AUTO_TRANS_HELPER ATR_LOCKS(pEnt, ".pInventoryV2.Ppinventorybags");
bool inv_trh_FindItemByID(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, U64 item_id, BagIterator* pIter)
{
	return inv_trh_FindItemByIDEx(ATR_PASS_ARGS, pEnt, pIter, item_id, false, false);
}
...
// NOTE: this stops at the first bag that has the ID and assumes global uniqueness for item ids.
AUTO_TRANS_HELPER ATR_LOCKS(pEnt, ".pInventoryV2.Ppinventorybags");
bool inv_trh_FindItemByDefName(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, BagIterator* pIter, const char* pchDefName, bool bIncludeOverflowBag, bool bSearchAllBags)
{
	int i;
	InvBagFlag inventory_bag_flags = InvBagFlag_DefaultInventorySearch;
	bool bRet;
	...
	bRet = inv_bag_trh_MultiBagFindItem(ATR_PASS_ARGS, &eaBags, pIter, kFindItem_ByName, pchDefName, 0);
...
AUTO_TRANS_HELPER
ATR_LOCKS(pBag, ".*");
bool inv_bag_trh_FindItemByID(ATR_ARGS, ATH_ARG NOCONST(InventoryBag)* pBag, U64 item_id, BagIterator* res)
{
	if (!res)
		return false;

	if ((bagiterator_GetCurrentBag(res) != pBag))
	{
		bagiterator_SetBag(pBag, res);
	}
...
// Removes a number of items from a bag.  
// The item is destroyed unless a valid Item list is supplied, peaItemsRemovedOut, 
//		in which case the item will be put onto the list instead of destroyed
// - Returns the number of items removed
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Psaved.Conowner.Containerid, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[]");
S32 inv_bag_trh_FindItemCountByDefName(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, S32 eBagID, const char* pchDefName, 
										S32 excludeItemFlags, int iCount, U32 bRemoveFound, U32 includeUnremoveables,
										const ItemChangeReason *pReason, GameAccountDataExtract* pExtract,
										Item ***peaItemsRemovedOut)
{
	BagIterator* pIter = bagiterator_Create();
	S32 iFound = 0;
	bool bFound = false;
	do {
		if (bFound = inv_bag_trh_FindItemByDefName(ATR_PASS_ARGS, pEnt, eBagID, pchDefName, excludeItemFlags, pIter))
		{
			...
			// excludeItemFlags should probably go up to inv_bag_trh_FindItemByDefName
			if (NONNULL(pSlot) && NONNULL(pSlot->pItem))
			{
				if (bRemoveFound)
				{
					...
					if (pItem = invbag_RemoveItem(ATR_PASS_ARGS, pEnt, false, eBagID, pIter->i_cur, iRemove, includeUnremoveables, pReason, pExtract))
					{
						iFound += iRemove;
						if (!peaItemsRemovedOut)
						{
							StructDestroySafe(parse_Item, &pItem);
						}
						else
						{
							eaPush(peaItemsRemovedOut, pItem);
						}
					}
				}
				else
	...
	} while (bFound && pIter->i_cur >= 0 && iFound < iCount);
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Psaved.Conowner.Containerid, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[]");
S32 inv_bag_trh_RemoveAllItemByDefName(	ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, S32 eBagID, const char* pchDefName, 
										const ItemChangeReason *pReason, GameAccountDataExtract* pExtract)
{
	int iFound = 0;
	BagIterator* pIter = bagiterator_Create();
	bool bFound = false;
	do {
		if (bFound = inv_bag_trh_FindItemByDefName(ATR_PASS_ARGS, pEnt, eBagID, pchDefName, 0, pIter))
		{
			NOCONST(InventoryBag)* pBag = bagiterator_GetCurrentBag(pIter);
			NOCONST(InventorySlot)* pSlot = inv_trh_GetSlotFromBag(ATR_PASS_ARGS, pBag, pIter->i_cur);
			...
			if (NONNULL(pSlot) && NONNULL(pSlot->pItem))
			{
				...
				if (pItem = invbag_RemoveItem(ATR_PASS_ARGS, pEnt, false, eBagID, pIter->i_cur, iRemove, false, pReason, pExtract))
				{
					iFound += iRemove;
					StructDestroySafe(parse_Item, &pItem);
				}
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Ppinventorybags, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags, pInventoryV2.peaowneduniqueitems");
S32 inv_trh_FindItemCountByDefNameEx(	ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, 
										InvBagFlag searchBagFlags, InvBagFlag excludeBagFlags,
										const char* pchDefName, S32 excludeItemFlags, int iCount, U32 bRemoveFound, U32 includeUnremoveables,
										const ItemChangeReason *pReason, GameAccountDataExtract* pExtract, 
										Item ***peaItemsRemovedOut)
{
	int i, iFound = 0;
	...
	for (i = eaSize(&pEnt->pInventoryV2->ppInventoryBags)-1; i >= 0; i--)
	{
		...
		if (!(invbag_trh_flags(pEnt->pInventoryV2->ppInventoryBags[i]) & searchBagFlags))
			continue;
		if (excludeBagFlags && invbag_trh_flags(pEnt->pInventoryV2->ppInventoryBags[i]) & excludeBagFlags)
			continue;
		...
		iFound += inv_bag_trh_FindItemCountByDefName(ATR_PASS_ARGS, pEnt, eBagID, pchDefName, excludeItemFlags, (iCount - iFound), 
														bRemoveFound, includeUnremoveables, pReason, pExtract, peaItemsRemovedOut);
...
{
	InvBagFlag inventory_bag_flags = InvBagFlag_DefaultInventorySearch;
	return inv_trh_FindItemCountByDefNameEx(ATR_PASS_ARGS, pEnt, inventory_bag_flags, 0, pchDefName, 0, iCount, bRemoveFound, true, pReason, pExtract, NULL);
}

AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Ppinventorybags, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags, pInventoryV2.peaowneduniqueitems");
S32 inv_trh_RemoveAllItemByDefName(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, const char* pchDefName, 
									const ItemChangeReason *pReason, GameAccountDataExtract* pExtract)
{
	InvBagFlag inventory_bag_flags = InvBagFlag_DefaultInventorySearch;
	int i, iFound = 0;
...
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Ppinventorybags, .pInventoryV2.PpLitebags, pInventoryV2.peaowneduniqueitems,   .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Ilevelexp");
Item* inv_RemoveItemByID(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, U64 item_id, int count, ItemAddFlags eFlags, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	...
	Item *item = NULL;
	if (inv_trh_FindItemByIDEx(ATR_PASS_ARGS,pEnt,iter, item_id, bUseOverflow, false))
	{
		item = invbag_RemoveItem(ATR_PASS_ARGS, pEnt, false, bagiterator_GetCurrentBagID(iter), iter->i_cur, count, false, pReason, pExtract);
	}
...
{
	S32 iRemoved = inv_bag_trh_FindItemCountByDefName(ATR_PASS_ARGS, pEnt, BagID, def_name, 0, count, true, false, pReason, pExtract, NULL);
	return (iRemoved == count);
}

AUTO_TRANS_HELPER ATR_LOCKS(bag, ".*");
void bagiterator_SetBag(ATH_ARG NOCONST(InventoryBag) *bag, BagIterator* iter)
{
	if(ISNULL(bag) || ISNULL(iter))
		return;

	bagiterator_Reset(iter);
...
void bagiterator_SetBagEarray(ATH_ARG NOCONST(InventoryBag)*** peaBags, BagIterator* iter)
{
	InventoryBag* pBag = NULL;
	if(ISNULL(peaBags) || ISNULL(iter))
		return;
...
AUTO_TRANS_HELPER ATR_LOCKS(bag, ".*");
void bagiterator_SetLiteBag(ATH_ARG NOCONST(InventoryBagLite) *bag, BagIterator* iter)
{
	if(ISNULL(bag) || ISNULL(iter))
		return;
...
void bagiterator_trh_SetBagByID(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int BagID, BagIterator* pIter, GameAccountDataExtract *pExtract)
{
	NOCONST(InventoryBag) *bag = inv_trh_GetBag(ATR_PASS_ARGS, pEnt, BagID, pExtract);
	if (bag)
		bagiterator_SetBag(bag, pIter);
}
...
void bagiterator_trh_SetLiteBagByID(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int BagID, BagIterator* pIter, GameAccountDataExtract *pExtract)
{
	NOCONST(InventoryBagLite) *litebag = inv_trh_GetLiteBag(ATR_PASS_ARGS, pEnt, BagID, pExtract);
	if (litebag)
		bagiterator_SetLiteBag(litebag, pIter);
}
...
{
	if(!iter || (!iter->eaBags && !iter->eaLitebags) || iter->i_bag < 0)
		return InvBagIDs_None;
...
// a transaction.
AUTO_TRANS_HELPER
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Ppinventorybags, .Pchar.Ilevelexp, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppLiteBags");
Item *bagiterator_RemoveItem(ATR_ARGS, BagIterator *iter, ATH_ARG NOCONST(Entity)* ent,  int count, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	if(!iter)
		return NULL;
	return invbag_RemoveItem(ATR_PASS_ARGS,ent,false,bagiterator_GetCurrentBagID(iter),iter->i_cur, count, false, pReason, pExtract);
}
...
// locks all bags
AUTO_TRANS_HELPER 
ATR_LOCKS(pEnt, ".Psaved.Ppuppetmaster.Curtype, .Pplayer.Playertype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
ItemQuality inv_FillRewardRequest(InventoryBag** eaRewardBags, InvRewardRequest* pRequest)
{
	...
	for (i = 0; i < eaSize(&eaRewardBags); i++)
	{
		...
		for (j = 0; j < eaSize(&pRewardBag->ppIndexedInventorySlots); j++)
		{
			...
			ItemDef* pItemDef = pSlot->pItem ? GET_REF(pSlot->pItem->hItem) : NULL;
			InvRewardGrantData * pGrantData = NULL;

			if (!pItemDef || SAFE_MEMBER2(pSlot->pItem, pRewardData, bHideInUI))
				continue;

			pGrantData = StructCreate(parse_InvRewardGrantData);
			pGrantData->iSpecialRewardStatus = pRewardBag->pRewardBagInfo->iSpecialRewardStatus;
			...
			if (pItemDef->eType == kItemType_Numeric)
			{
				...
				eaInsert(&pRequest->eaGrantData, pGrantData, eaSize(&pRequest->eaNumericRewards)); // insert before the items
			...
			else if (pSlot->pItem)
			{
				...
				eaPush(&pRequest->eaGrantData, pGrantData); // put at the end
			}
		}
	}
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Ppallowedcritterpets, .pInventoryV2.Ppinventorybags, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .pInventoryV2.Pplitebags, .Pplayer.Playertype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems");
							...
							else
							{
								...
								if (pItemDef)
								{
									...
									else
									{
										...
										{
											destBag = inv_trh_GetBestBagForItemDef(pEnt, pItemDef, CONTAINER_NOCONST(Item, (pItem)), pItem->count, true, pExtract);
										...
										else
										{
											...
											// We used to check if the bag was full at this point and redirect to the overflow bag.
											// Don't do this anymore as AddItem will take care of it itself. (We hope)
										}
						...
						for (iItem = 0; iItem < ItemCount; iItem++)
						{
							...
							else
							{
								...
								else
								{
									...
									{
										destBag = inv_trh_GetBestBagForItemDef(pEnt, pItemDef, CONTAINER_NOCONST(Item, (pItem)), -1, true, pExtract);
										pLiteBag = inv_trh_GetLiteBag(ATR_PASS_ARGS, pEnt, destBag, pExtract);
										if ( destBag == InvBagIDs_None )
										{
											destBag = InvBagIDs_Inventory;
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Psaved.Ppallowedcritterpets, .pInventoryV2.Ppinventorybags, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .pInventoryV2.Pplitebags, .Pplayer.Playertype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems");
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Psaved.Ppuppetmaster.Curtype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Pplayer.Playertype, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(pSrcBag, ".*")
	ATR_LOCKS(pDestBag, ".*");
