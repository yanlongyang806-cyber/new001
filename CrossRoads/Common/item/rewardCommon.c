...
AUTO_FIXUPFUNC;
TextParserResult RewardConfig_Fixup(RewardConfig *pRewardConfig, enumTextParserFixupType eFixupType, void *pExtraData)
{
	int i;
	...
	switch(eFixupType)
	{
		...
		case FIXUPTYPE_POST_RELOAD:
			...
			{
				g_RewardConfig.eaRewardModifiers[i]->iKey = i;
			}
	}
...
AUTO_STARTUP(RewardsCommon) ASTRT_DEPS(RewardTagsLoad, RewardGatedLoad, ItemTags, EntityCostumes, Items);
void rewardCommon_Load(void)
{
	...
	eaIndexedEnable(&g_RewardConfig.eaRewardModifiers,parse_GlobalRewardModifier);
...
U32 Reward_GetSecondsPerGatedBlock(RewardGatedInfo *pRewardGatedInfo)
{
    if ( pRewardGatedInfo->uHoursPerBlock )
    {
        return pRewardGatedInfo->uHoursPerBlock * SECONDS_PER_HOUR;
    }
    else
    {
        return pRewardGatedInfo->uSecondsPerBlock;
    }
}

bool Reward_StillInPreviousGatedTimeBlock(U32 uGateTime, RewardGatedInfo *pRewardGatedInfo)
{
	if (pRewardGatedInfo->eRewardGatedIntervalMode==kShardIntervalTimingMode_Off)
	{
		// Old style computation
		U32 uBlock = 0;
		U32 uBlockChar = 0;
		U32 uSecondsPerBlock = Reward_GetSecondsPerGatedBlock(pRewardGatedInfo);
		U32 uNowSeconds = ShardClock_SecondsSince2000();
		
		if (uSecondsPerBlock > 0)
		{
			if (pRewardGatedInfo->bGateFromPlayerGrantTime)
			{
				// Blocks that start at the time of granting
				uBlockChar = 0; // assume that whenever the character started was 0, since we don't really care
				uBlock = (uNowSeconds-uGateTime) / uSecondsPerBlock;
			}
			else
			{
				// Absolute blocks that start at Time Zero.
				uBlock = uNowSeconds / uSecondsPerBlock;
				uBlockChar = uGateTime / uSecondsPerBlock;
			}
		}
	...
	{
		// New Daily/Weekly shard timed mode
		return(ShardIntervalTiming_TimeIsInCurrentInterval(uGateTime, pRewardGatedInfo->eRewardGatedIntervalMode));
		
	}
	return(false);
}

// Return the number of seconds left before this gated reward resets
U32 Reward_GatedTimeBlockSecondsLeft(U32 uGateTime, RewardGatedInfo *pRewardGatedInfo)
{
	U32 uTimeRemaining = 0;
	U32 uNowSeconds = timeServerSecondsSince2000();
	...
	if (pRewardGatedInfo->eRewardGatedIntervalMode==kShardIntervalTimingMode_Off)
	{
		...
		if (uBlockChar == uBlock)
		{
			...
			if (pRewardGatedInfo->bGateFromPlayerGrantTime)
			{
				devassert(uSecondsPerBlock > (uNowSeconds-uGateTime));
				uTimeRemaining = uSecondsPerBlock-(uNowSeconds-uGateTime);
			}
			else
			{
				U32 uNextBlockTm = (uBlock + 1) * uSecondsPerBlock;
				U32 uTimeNext = uNextBlockTm - uNowSeconds;
				uTimeRemaining = uTimeNext;
			}
		}
	}
	else
	{
		if (ShardIntervalTiming_TimeIsInCurrentInterval(uGateTime, pRewardGatedInfo->eRewardGatedIntervalMode))
		{
			U32 uNextIntervalStart = ShardIntervalTiming_TimeOfNextInterval(pRewardGatedInfo->eRewardGatedIntervalMode);
			if (uNextIntervalStart > uNowSeconds)
			{
				uTimeRemaining = uNextIntervalStart - uNowSeconds;
			}
			else
			{
...
U32 Reward_GetGatedCooldown(Entity *pEntPlayer, S32 gatedType)
{
	if(pEntPlayer && pEntPlayer->pPlayer)
	{
		...
		if (pRewardGatedInfo)
		{
			// fall through and return zero if there isn't data or we aren't in the same block
			if(pPlayerGated && pPlayerGated->uNumTimes > 0)
			{
				return(Reward_GatedTimeBlockSecondsLeft(pPlayerGated->uTimeSet, pRewardGatedInfo));
			}
		}
		else if(!bSentError)
		{
