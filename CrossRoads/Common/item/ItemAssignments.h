...
typedef struct ItemAssignmentCategorySettings
{
	...
	S32 iNumRanks; 
		// the number of ranks for this category. If 0, uses the number of ranks determined by the leveling schedule
		// this is zero inclusive, and for ranking schedule purposes, this will be iNumRanks
	...
	const char* pchEventName;
		// The name of a calendar event that must be active for this category to be shown.
...
typedef struct ItemAssignmentSettings
{
	...
	S32 eIncludeBagFlagsForItemCosts;	AST(FLAGS SUBTABLE(InvBagFlagEnum))
		// if set, will override the default inventory search flags which are defined to use InvBagFlag_DefaultInventorySearch 
...
typedef struct ItemAssignment
{
	...
	CONST_EARRAY_OF(Item) eaSavedItems;	AST(SERVER_ONLY PERSIST)
		// items that were consumed for this assignment, saving them so if task get canceled we can refund the actual item.
...
typedef struct ItemAssignmentPlayerData
{
	...
	EARRAY_OF(ItemAssignmentDefRef) eaAdditionalPersonalAssignments; AST(SELF_ONLY)
		// ItemAssignments that has been added to the player
...
ExprContext* ItemAssignments_GetContextEx(	Entity* pEnt, 
											...
											const ItemAssignmentCategorySettings *pCategory);
#define ItemAssignments_GetContext(pEnt) ItemAssignments_GetContextEx(pEnt, NULL, NULL, NULL, NULL, NULL, 0, 0, NULL)
...
void ItemAssignments_GetSearchInvBagFlags(SA_PARAM_NN_VALID S32 *piSearchFlags, SA_PARAM_OP_VALID S32 *piExcludeFlags);
...
// A check to see if this assignment can be started based on the number of ones already running. Returns true if its ok to start more of these
bool ItemAssignments_trh_MaxAssignmentCheck(ATH_ARG NOCONST(Entity)* pEnt, ItemAssignmentDef* pDef);
#define gslItemAssignments_MaxAssignmentCheck(pEnt, pDef) ItemAssignments_trh_MaxAssignmentCheck(CONTAINER_NOCONST(Entity, pEnt), pDef)
