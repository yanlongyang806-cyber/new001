...
void ItemAssignment_GetUIOutcomes(	Language lang, 
									...
									ItemAssignmentOutcomeUI*** peaOutcomeUI)
{
	...
	if (pEnt && pDef)
	{
		...
		for (i = 0; i < eaSize(&pDef->eaOutcomes); i++)
		{
			...
			pOutcomeUI->pchDisplayName = langTranslateDisplayMessage(lang, pOutcome->msgDisplayName);
			pOutcomeUI->pchDescription = langTranslateDisplayMessage(lang, pOutcome->msgDescription);
...
void ItemAssignment_FillInFromCompleteDetails(Language lang,
	...
	SA_PARAM_NN_VALID ItemAssignmentCompletedDetails *pCompleteDetails)
{
	...
	for (i = 0; i < eaSize(&pCompleteDetails->eaSlottedItemRefs); i++)
	{
		...
		pSlotUI->pchDestroyDescription = pOutcome && pOutcome->pResults ? langTranslateDisplayMessage(lang, pOutcome->pResults->msgDestroyDescription) : NULL;
		pSlotUI->pchNewAssignmentDescription = pOutcome && pOutcome->pResults ? langTranslateDisplayMessage(lang, pOutcome->pResults->msgNewAssignmentDescription) : NULL;
		...
		if (!pSlotUI->bUpdated)
		{
			...
			if (pSlotUI->uItemID && (!pInvSlot || !pInvSlot->pItem || pInvSlot->pItem->id != pSlotUI->uItemID))
			{
				...
				inv_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), pSlotUI->uItemID, pIter);
				if (bagiterator_GetCurrentBag(pIter))
				{
					pSlotUI->eBagID = bagiterator_GetCurrentBagID(pIter);
					pSlotUI->iBagSlot = pIter->i_cur;
				}
				else
				{
...
ItemAssignmentUI* ItemAssignment_CreateActiveHeader(Language lang, ItemAssignmentUI*** peaData, S32* piCount, const char* pchDisplayNameKey)
{
	const char *pchHeader = langTranslateMessageKey(lang, pchDisplayNameKey);
...
void FillActiveItemAssignment(	Language lang,
								ItemAssignmentUI* pDataOut, 
								const ItemAssignment* pAssignment, 
								Entity* pEnt, 
								...
								U32 uTimeCurrent)
{
	...
	pDataOut->pchDisplayName = pDef ? langTranslateDisplayMessage(lang, pDef->msgDisplayName) : NULL;
	pDataOut->pchDescription = pDef ? langTranslateDisplayMessage(lang, pDef->msgDescription) : NULL;
	pDataOut->pchChainDisplayName = pDef ? langTranslateDisplayMessage(lang, pDef->msgAssignmentChainDisplayName) : NULL;
	pDataOut->pchChainDescription = pDef ? langTranslateDisplayMessage(lang, pDef->msgAssignmentChainDescription) : NULL;
	...
	pDataOut->pchOutcomeDisplayName = pOutcome ? langTranslateDisplayMessage(lang,pOutcome->msgDisplayName) : NULL;
	pDataOut->pchOutcomeDescription = pOutcome ? langTranslateDisplayMessage(lang,pOutcome->msgDescription) : NULL;
	pDataOut->pchOutcomeDestroyedDescription = pOutcome && pOutcome->pResults ? langTranslateDisplayMessage(lang,pOutcome->pResults->msgDestroyDescription) : NULL;
	pDataOut->pchOutcomeNewAssignmentDescription = pOutcome && pOutcome->pResults ? langTranslateDisplayMessage(lang,pOutcome->pResults->msgNewAssignmentDescription) : NULL;
	pDataOut->uOutcomeDestroyed = 0;
	pDataOut->uOutcomeNewAssignment = 0;
#ifdef GAMECLIENT
	if (!resExtractNameSpace_s(pAssignment->pchMapMsgKey, NULL, 0, NULL, 0))
		pDataOut->pchMapDisplayName = gclRequestMapDisplayName(pAssignment->pchMapMsgKey);
	else
		pDataOut->pchMapDisplayName = NULL;
#endif
...
void FillCompletedItemAssignment(Language lang, Entity *pEnt, ItemAssignmentUI* pData, ItemAssignmentCompletedDetails* pAssignment, ItemAssignment* pActiveAssignment, U32 uTimeCurrent)
{
	ItemAssignmentDef* pDef = GET_REF(pAssignment->hDef);
	ItemAssignmentCategorySettings* pCategorySettings = pDef ? ItemAssignmentCategory_GetSettings(pDef->eCategory) : NULL;
	...
	pData->pchDisplayName = pDef ? langTranslateDisplayMessage(lang,pDef->msgDisplayName) : NULL;
	pData->pchDescription = pDef ? langTranslateDisplayMessage(lang,pDef->msgDescription) : NULL;
	pData->pchChainDisplayName = pDef ? langTranslateDisplayMessage(lang,pDef->msgAssignmentChainDisplayName) : NULL;
	pData->pchChainDescription = pDef ? langTranslateDisplayMessage(lang,pDef->msgAssignmentChainDescription) : NULL;
	...
	pData->pchOutcomeDisplayName = pOutcome ? langTranslateDisplayMessage(lang,pOutcome->msgDisplayName) : NULL;
	pData->pchOutcomeDescription = pOutcome ? langTranslateDisplayMessage(lang,pOutcome->msgDescription) : NULL;
	pData->pchOutcomeDestroyedDescription = pOutcome && pOutcome->pResults ? langTranslateDisplayMessage(lang,pOutcome->pResults->msgDestroyDescription) : NULL;
	pData->pchOutcomeNewAssignmentDescription = pOutcome && pOutcome->pResults ? langTranslateDisplayMessage(lang,pOutcome->pResults->msgNewAssignmentDescription) : NULL;
...
#ifdef GAMECLIENT //GAMESERVER does NOT do caching
...
#define ADD_SPACE(pestr) (estrLength(pestr) > 0 ? estrConcatChar(pestr, ' ') : (void)0)
...
bool ItemAssignment_GetFailsRequirementsReason(		Language lang,
													Entity* pEnt, 
													...
													bool bTestSlottedAssets)
{
	...
	if (eaSize(&eaEntries))
	{
		...
		for (i = 0; i < eaSize(&eaEntries); i++)
		{
			...
			const char* pchReasonString = langTranslateMessageKey(lang, pEntry->pchReasonKey);

			sprintf(pchReasonKey, "ItemAssignment_Fails%s", pchReason);

			ADD_SPACE(pestrFailsRequires);
			langFormatGameMessageKey(lang, pestrFailsRequires, pchReasonKey, 
	...
	if (!bTestSlottedAssets && !gclItemAssignment_CheckInventory(pDef, &eFailCategory, &eReason, pbfInvalidSlots, iMaxSlots))
	{
		if (eReason & kItemAssignmentFailsRequiresReason_CantFillSlots)
		{
			...
			{
				ADD_SPACE(pestrFailsRequires);
				langFormatGameMessageKey(lang, pestrFailsRequires, "ItemAssignment_FailsSlotRequirements",
			...
			else
			{
				...
				ADD_SPACE(pestrFailsRequires);
				langFormatGameMessageKey(lang, pestrFailsRequires, "ItemAssignment_FailsSlotRestrictions", STRFMT_END);
			}
		}
		...
		{
			ADD_SPACE(pestrFailsRequires);
			langFormatGameMessageKey(lang, pestrFailsRequires, "ItemAssignment_FailsItemInUnslottableBag", STRFMT_END);
		}
		...
		{
			ADD_SPACE(pestrFailsRequires);
			langFormatGameMessageKey(lang, pestrFailsRequires, "ItemAssignment_FailsItemNonRemovable", STRFMT_END);
		}
...
ItemAssignmentFailsRequiresReason ItemAssignment_UpdateRequirementsInfo(Language lang,
																		...
																		bool bTestSlottedAssets)
{
	S32 i;
	S32 iCount = 0;
	bool bFailsMinLevelReq = false;
	bool bFailsMissionReq = false;
	bool bFailsAssignmentReq = false;

	static U32 s_uLastFlush = 0;
...
//We only care about caching on the client. Everything else should just clear the cached objects every time
#ifdef GAMESERVER 
	s_uLastFlush = 0;
	eaClearStruct(&s_eaDefCache,parse_ItemAssignmentUI);
#endif

//Client is the only thing that needs to care about anything cached.
#ifdef GAMECLIENT 
	...
	if (uTime >= pCache->uNextRequirementsUpdateTime)
	{
		...
		if (ItemAssignment_GetFailsRequirementsReason(lang,
														pEnt, 
														pDef,
...
void SetItemAssignmentData(	Language lang,
							Entity* pEnt, 
							ItemAssignmentUI* pData, 
							ItemAssignmentDef* pDef, 
							...
							bool bRequirementsTestSlottedAssets)
{
	...
	pData->pchWeight = pDef ? StaticDefineIntRevLookup(ItemAssignmentWeightTypeEnum, pDef->eWeight) : NULL;
	pData->fWeightValue = pDef ? ItemAssignmentWeightType_GetWeightValue(pDef->eWeight) : 0.f;
	ItemAssignment_UpdateRequirementsInfo(lang, pData, pEnt, pDef, bRequirementsTestSlottedAssets);
	pData->bIsHeader = false;
...
static void gclAddItemAssignmentToList(	Language lang,
										Entity* pEnt,
										ItemAssignmentDef* pDef, 
										...
										const char *pchStringSearch)
{
	...
	{
		ItemAssignmentFailsRequiresReason eReason = ItemAssignment_UpdateRequirementsInfo(lang, NULL, pEnt, pDef, false);
	...
	if (pchStringSearch)
	{
		const char* pchDisplayName = langTranslateDisplayMessage(lang,pDef->msgDisplayName);
	...
	{
		if (GET_REF((*peaData)[i]->hDef) == pDef)
		{
			eaMove(peaData, (*piCount), i);
			break;
		}
	}
	pData = eaGetStruct(peaData, parse_ItemAssignmentUI, (*piCount)++);

	SetItemAssignmentData(lang, pEnt, pData, pDef, bIsPersonalAssignment, bIsFeatured, false);
	...
	if (pDef && !pDef->bDisabled)
	{
		...
		if (bAddCategoryHeaders && pCategorySettings && eaiFind(peaCategories, pDef->eCategory) < 0)
		{
			...
			pData->pchDisplayName = langTranslateDisplayMessage(lang,pCategorySettings->msgDisplayName);
...
void GetAvailableItemAssignmentsByCategoryInternal(	Language lang,
													Entity *pEnt, 
													ItemAssignmentUI *** peaData,
	...
	if (pPlayerData)
	{
		if (!(uFlags & kGetActiveAssignmentFlags_ExcludeNonPersonal))
		{
			for (i = eaSize(&pPlayerData->eaVolumeAvailableAssignments)-1; i >= 0; i--)
			{
				...
				if (pDef)
				{
					gclAddItemAssignmentToList(lang, pEnt, pDef, s_peFilterCategories, s_eafWeightFilters, peaData, &iCount, 
			...
			if (pItemAssignment->fWeightValue != fLastWeightValue)
			{
				...
				if (pWeight)
				{
					...
					if (pHeader)
					{
						...
						pHeader->pchDisplayName = langTranslateDisplayMessage(lang,pWeight->msgDisplayName);
...
static void gclItemAssignmentGetCategoriesString(	Language lang,
													ItemCategory* peCategories, 
													F32 fModifier, 
													ItemCategory** peaAddedCategories, 
													char** pestrCategories,
													char** pestrCategoriesRaw)
{
	const char* pchSeparator = langTranslateMessageKey(lang, "ItemAssignment_CategorySeparator");
	...
	for (i = 0; i < eaiSize(&peCategories); i++)
	{
		...
		if (pchCategoryName && pchCategoryName[0])
		{
			...
			pchTranslatedName = langTranslateMessageKey(lang, pchMessageKey);
			if (pchTranslatedName)
			{
				...
				{
					langFormatGameMessageKey(lang, pestrCategories, "ItemAssignment_CategoryBonus", 
						STRFMT_STRING("Name", pchTranslatedName), STRFMT_END);
				}
				else if (fModifier < -FLT_EPSILON)
				{
					langFormatGameMessageKey(lang, pestrCategories, "ItemAssignment_CategoryPenalty", 
						STRFMT_STRING("Name", pchTranslatedName), STRFMT_END);
				}
				else
				{
...
static void gclItemAssignmentSetSlotUIData( Language lang,
											Entity* pEnt,
											ItemAssignmentDef* pDef, 
											ItemAssignmentSlotUI* pSlotUI, 
											...
											Item* pItem)
{
	...
	if (g_ItemAssignmentSettings.pOutcomeWeightWindowConfig == NULL)
	{
		for (i = 0; i < eaSize(&ppchOutcomeModifiers); i++)
		{
			...
			if (pModType)
			{
				...
				gclItemAssignmentGetCategoriesString(	lang,
														pModifier->peItemCategories, 
														fModifier, 
	...
	gclItemAssignmentGetCategoriesString(lang, pSlot->peRequiredItemCategories, 0.0f, NULL, &pSlotUI->estrRequiredCategories, &pSlotUI->estrRequiredCategoriesRaw);
}

void ItemAssignmentsSlotItemCheckSwap(Language lang, Entity* pEnt, ItemAssignmentSlotUI* pNewSlot, S32 eBagID, S32 iBagSlot, Item* pItem)
{
	...
	gclItemAssignmentSetSlotUIData(lang, pEnt, pDef, pNewSlot, pSlot, pItem);
}

// compiles the static ItemAssignmentSlotUI slot list pIACache->eaSlots for the given def
bool buildSlotsForItemAssignment(Language lang, Entity *pEnt, ItemAssignmentDef *pDef, bool bIgnoreUpdateThrottle, ItemAssignmentSlotUI ***peaItemAssignmentSlotUICache, S32 *piCacheCount)
{
	...
	if (pEnt)
	{
		...
		for (i = 0; i < eaSize(&pDef->eaSlots); i++)
		{
			...
			if (GET_REF(pSlotUI->hDef) != pDef || pSlotUI->iAssignmentSlot != i)
			{
				...
				gclItemAssignmentSetSlotUIData(lang, pEnt, pDef, pSlotUI, pSlot, NULL);
...
bool SetGenSlotsForItemAssignment(	Language lang, Entity *pEnt, ItemAssignmentSlotUI ***peaItemAssignmentSlotUIs, S32 *piCachedCount, 
									bool bIgnoreUpdateThrottle, ItemAssignmentSlotUI*** peaDataOut, const char* pchAssignmentDef, U32 uOptionFlags)
{
	...
	if (pDef)
	{
		...
		bFull = buildSlotsForItemAssignment(lang, pEnt, pDef, bIgnoreUpdateThrottle, peaItemAssignmentSlotUIs, piCachedCount);
...
void ItemAssignment_AddChainToList(Language lang, Entity *pEnt, ItemAssignmentUI ***peaData, S32 *piCount, ItemAssignmentDef *pDef, U32 uFlags, bool bIsPersonalAssignment)
{
	ItemAssignmentUI *pData = NULL;
	S32 i;
	...
	if (!(uFlags & kItemAssignmentChainFlag_ExcludeAssignments))
	{
		...
		SetItemAssignmentData(lang, pEnt, pData, pDef, bIsPersonalAssignment, false, false);
		pData->bIsHeader = false;
	}
	...
	for (i = eaSize(&pDef->eaDependencies) - 1; i >= 0; i--)
	{
		...
		if (pDep)
			ItemAssignment_AddChainToList(lang, pEnt, peaData, piCount, pDep, uFlags, bIsPersonalAssignment);
	}
}
...
static bool _getMissionLockedMessageForPerkUnlock(	Language lang,
	Entity* pEnt,
	...
	S32 *piNumFoundInOut)
{
	if (!pMission->bHidden && pMission->bDiscovered)
	{
		...
		{
			const char *pchMsg = langTranslateDisplayMessage(lang, pDef->uiStringMsg);
...
// 
static void gclItemAssignments_GetLockedAssignmentSlotMessages(	Language lang,
	Entity* pEnt, 
	...
	S32 iCurLockedSlot)
{
	...
	if (pInfo && iNumMessages > 0)
	{
		...
		{
			if (_getMissionLockedMessageForPerkUnlock(lang, pEnt, pStrictSlotSettings, pMission, ppchReasons, iNumPerks, iCurLockedSlot, &iNumFound))
			{
				bDone = true;
				break;
			}
		...
		for(i=0;i<eaSize(&pStrictSlotSettings->ppUnlockExpression);i++)
		{
			...
			{
				const char *pchMsg = langTranslateDisplayMessage(lang, pStrictSlotSettings->ppUnlockExpression[i]->displayReason);
...
void ItemAssignment_FillItemAssignmentSlots(Language lang, Entity *pEnt, ItemAssignmentUI ***peaItemAssignmentUIData)
{
	ItemAssignmentPersistedData* pPlayerData = SAFE_MEMBER2(pEnt, pPlayer, pItemAssignmentPersistedData);
	...
	if (pPlayerData && g_ItemAssignmentSettings.pStrictAssignmentSlots)
	{
		...
		for (i = 0; i < iNumAvailableAssignmentSlots; ++i)
		{
			...
			if (pFoundAssignment)
			{
				...
				if (pData)
					FillActiveItemAssignment(lang, pData, pFoundAssignment, pEnt, pPlayerData, uTimeCurrent);
			}
			else
			{
		...
		gclItemAssignments_GetLockedAssignmentSlotMessages(lang, pEnt, g_ItemAssignmentSettings.pStrictAssignmentSlots, 
...
void ItemAssignment_FillAssignmentCategories(Language lang, Entity *pEnt, ItemAssignmentCategoryUI ***pppCategories, U32 uFlags)
{
	...
	if (pEnt)
	{
		...
		FOR_EACH_IN_EARRAY(eaCateogries, const ItemAssignmentCategorySettings, pCategory)
		{
			...
			if (pCatUI)
			{
				...
				pCatUI->pchDisplayName = langTranslateDisplayMessage(lang,pCategory->msgDisplayName);
				pCatUI->pchIcon = pCategory->pchIconName;
				pCatUI->pchEventName = pCategory->pchEventName;
				...
				if (g_ItemAssignmentSettings.pCategoryRankingSchedule)
				{
					...
					S32 iNumRanks = ItemAssignments_GetNumRanks(pCategory, g_ItemAssignmentSettings.pCategoryRankingSchedule);

					pCatUI->iCurrentRank = CLAMP(pCatUI->iCurrentRank, 0, iNumRanks);
					pCatUI->iNextRank = CLAMP(pCatUI->iNextRank, 0, iNumRanks);

					xpForCurrentLevel = ItemAssignments_GetExperienceThresholdForRank(pCategory, g_ItemAssignmentSettings.pCategoryRankingSchedule, pCatUI->iCurrentRank);
					pCatUI->iNextXP = ItemAssignments_GetExperienceThresholdForRank(pCategory, g_ItemAssignmentSettings.pCategoryRankingSchedule, pCatUI->iNextRank);
					pCatUI->bIsAtMaxRank = (pCatUI->iCurrentRank >= (iNumRanks - 1));
							...
							if (pReward->pItem)
							{
								FOR_EACH_IN_EARRAY(pOtherOutcome->pData->eaRewards, InventorySlot, pRewardOther)
								{
									if (pRewardOther->pItem && 
										...
										pRewardOther->pItem->count == pReward->pItem->count)
									{
										bFound = true;
										break;
									}
...
// Goes through all the slots for an assignment and tries to find the best item to slot in each
void ItemAssignments_AutoSlotBestItems(Language lang, Entity *pEnt)
{
	InvBagIDs BagID = StaticDefineIntGetInt(InvBagIDsEnum, "CraftingInventory");
	Item **eaItemsOut = NULL;
	Item *pBestItem = NULL;
	ItemDef *pBestItemDef = NULL;

	NOCONST(InventoryBag) *pEntBag = NULL;

	if (BagID == -1)
		return;

	pEntBag = inv_GetBag(CONTAINER_NOCONST(Entity, pEnt), BagID, NULL);
	if (!pEntBag)
		return;
	
	// go through all the slots and find the best item to fit in that slot
		// get the list of items that are valid for this slot
	// find the "best" one and then put it in the slot

	FOR_EACH_IN_EARRAY_FORWARDS(pIACache->eaSlots, ItemAssignmentSlotUI, pSlot)
	{
		ItemAssignments_GetPotentialItemsForSlot(pEnt, BagID, pIACache->eaSlots, pSlot, &eaItemsOut);
		
		// something already slotted here
		if (pSlot->uItemID != 0 || pSlot->bOptionalSlot)
			continue;

		pBestItem = NULL;
		pBestItemDef = NULL;
		FOR_EACH_IN_EARRAY(eaItemsOut, Item, pItem)
		{
			ItemDef *pItemDef = GET_REF(pItem->hItem);
			if (pItemDef)
			{
				if (!pBestItem || pItemDef->Quality > pBestItemDef->Quality)
				{
					pBestItemDef = pItemDef;
					pBestItem = pItem;
				}
			}
		}
		...
		if (pBestItem)
		{
			...
			// we need to find the bagSlot index so we can slot it 
			for(i=0; i < eaSize(&pEntBag->ppIndexedInventorySlots); i++)
			{
				Item *pItem =  CONTAINER_RECONST(Item, pEntBag->ppIndexedInventorySlots[i]->pItem);
			...
			if (iBagSlot != -1)
			{
				ItemAssignmentsSlotItemCheckSwap(lang, pEnt, pSlot, BagID, iBagSlot, pBestItem);
			}
		}
	}
	FOR_EACH_END
	
	eaDestroy(&eaItemsOut);

}


							   
// creates a list of slot-able items in a given bag based on the ItemAssignmentSlotUI
void ItemAssignments_GetPotentialItemsForSlot(Entity *pEnt, InvBagIDs BagID, 
												ItemAssignmentSlotUI **ppSlots, 
												ItemAssignmentSlotUI *pSlot, 
												Item ***peaItemsOut)
{
	static ItemCategory *s_eaiIncludeCategories = NULL;
	int i;
	NOCONST(InventoryBag) *pEntBag;

	PERFINFO_AUTO_START_FUNC();

	pEntBag = (BagID != -1) ? inv_GetBag(CONTAINER_NOCONST(Entity, pEnt), BagID, NULL) : NULL;
	...
	if(pEntBag)
	{
		...
		eaiClear(&s_eaiIncludeCategories);
		if (pchToken = strtok_nondestructive(pSlot->estrRequiredCategoriesRaw, " \r\n\t,|%", &pchContext,&context_char))
		{
			do
			{
			...
			} while (pchToken = strtok_nondestructive(NULL, " \r\n\t,%", &pchContext,&context_char));
		}

		for(i=0;i<eaSize(&pEntBag->ppIndexedInventorySlots);i++)
		{
			if(pEntBag->ppIndexedInventorySlots[i]->pItem)
			{
				Item *pItem =  CONTAINER_RECONST(Item, pEntBag->ppIndexedInventorySlots[i]->pItem);
				ItemDef *pItemDef = GET_REF(pItem->hItem);
				int j;
				...
				for(j=eaSize(&ppSlots)-1;j>=0;j--)
				{
					if(ppSlots[j]->uItemID == pItem->id)
						break;
				}
				...
				for(j=eaiSize(&s_eaiIncludeCategories)-1;j>=0;j--)
				{
					if(eaiFind(&pItemDef->peCategories,s_eaiIncludeCategories[j]) == -1)
						break;
				}
