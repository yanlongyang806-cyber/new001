...
typedef enum InvBagFlag
{
	...
	InvBagFlag_DisallowClickPowers		= (1<<22),  //Items in this bag will not allow their click/toggle/maintain powers to be used.
} InvBagFlag;
...
AUTO_STRUCT;
typedef struct InventoryBagGroup
{
...
typedef struct Inventory
{
	...
	EARRAY_OF(NewItemID) eaiNewItemIDs;						AST(PERSIST NO_TRANSACT SUBSCRIBE)
...
AUTO_STRUCT;
typedef struct InvBagVariableMaxSlotDef
{
...
typedef struct InvBagDef
{
	...
	InvBagVariableMaxSlotDef primaryMaxSlots;			AST(EMBEDDED_FLAT)
		
	InvBagVariableMaxSlotDef **eaAdditionalMaxSlots;	AST(NAME(AdditionalMaxSlots))
		// after primaryMaxSlots is computed, these slots are *added* in to the total maxSlots 

	S32 additionalMaxSlotsLimit;					
		// If using AdditionalMaxSlots, this is the total maximum number of slots we're allowed to give out
...
// Information about the grant of this reward that could be of interest to the UI
// This is perhaps somewhat redundant with RewardBagInfo, but the way the code is structured, it uses
// different auto-structs.
AUTO_STRUCT;
typedef struct InvRewardGrantData
{
	U32 iSpecialRewardStatus; // from the reward entry
} InvRewardGrantData;
...
typedef struct InvRewardRequest
{
	...
	// there will be one of these per reward in each of the previous 2 arrays.  The items will start after the numerics
	EARRAY_OF(InvRewardGrantData) eaGrantData;

	// This is not a good way to do things.  I'm leaving this here because several systems were built on it.
	// The original system (reward packs) no longer uses it.  We should consider cleaning up the others.  We will have to
	// if they want to use eaGrantData
...
{
	InventoryBag** eaBags;			AST(NO_INDEX)
	InventoryBagLite** eaLitebags;	AST(NO_INDEX)
	int i_cur;
	int i_bag;
} BagIterator;
...
bool inv_ent_trh_EquipLimitCheck(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG NOCONST(InventoryBag)* pDstBag, ItemDef* pItemDef, ATH_ARG NOCONST(Item)* pItem, S32 iIgnoreBagID, S32 iIgnoreSlot, ItemEquipLimitCategoryData** ppLimitCategory, S32* piEquipLimit);
#define inv_ent_EquipLimitCheckEx(pEnt, pBag, pItemDef, pItem, iIgnoreBagID, iIgnoreSlot, ppLimitCategory, piEquipLimit) inv_ent_trh_EquipLimitCheck(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, (pEnt)), CONTAINER_NOCONST(InventoryBag, (pBag)), pItemDef, CONTAINER_NOCONST(Item, (pItem)), iIgnoreBagID, iIgnoreSlot, ppLimitCategory, piEquipLimit)
#define inv_ent_EquipLimitCheck(pEnt, pBag, pItemDef, pItem) inv_ent_trh_EquipLimitCheck(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, (pEnt)), CONTAINER_NOCONST(InventoryBag, (pBag)), pItemDef, CONTAINER_NOCONST(Item, (pItem)), -1, -1, NULL, NULL)

bool inv_ent_trh_EquipLimitCheckSwap(ATR_ARGS, ATH_ARG NOCONST(Entity)* pSrcEnt, ATH_ARG NOCONST(Entity)* pDstEnt, ATH_ARG NOCONST(InventoryBag)* pSrcBag, ATH_ARG NOCONST(InventoryBag)* pDstBag, ItemDef* pSrcItemDef, ATH_ARG NOCONST(Item)* pSrcItem, ItemDef* pDstItemDef, ItemEquipLimitCategoryData** ppLimitCategory, S32* piEquipLimit);
#define inv_ent_EquipLimitCheckSwap(pSrcEnt, pDstEnt, pSrcBag, pDstBag, pSrcItemDef, pSrcItem, pDstItemDef, ppLimitCategory, piEquipLimit) inv_ent_trh_EquipLimitCheckSwap(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, (pSrcEnt)), CONTAINER_NOCONST(Entity, (pDstEnt)), CONTAINER_NOCONST(InventoryBag, (pSrcBag)), CONTAINER_NOCONST(InventoryBag, (pDstBag)), pSrcItemDef, CONTAINER_NOCONST(Item, (pSrcItem)), pDstItemDef, ppLimitCategory, piEquipLimit)
...
// does special-case things for crufty data
void inv_trh_FixupInventory(ATR_ARGS, ATH_ARG NOCONST(Entity) *pEnt, bool bFixupItemIDs, const ItemChangeReason *pReason);
// does not do special-case things for crufty data, but still fixes up all kinds of IDs and stuff
...
bool inv_trh_MatchingItemInSlotID(ATR_ARGS, ATH_ARG NOCONST(InventoryBag)* pBag, int SlotIdx, SA_PARAM_NN_STR const char* ItemDefName);
#define inv_MatchingItemInSlot(pBag, SlotIdx, ItemDefName) inv_trh_MatchingItemInSlotID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(InventoryBag, (pBag)), SlotIdx, ItemDefName)
...
#define GetBestBagForItemDef( pEnt, pItemDef, pItem, iCount, bGive, pExtract) inv_trh_GetBestBagForItemDef(CONTAINER_NOCONST(Entity, (pEnt)), pItemDef, CONTAINER_NOCONST(Item, (pItem)), iCount, bGive, pExtract)
...
bool inv_bag_trh_AddItem(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, ATH_ARG CONST_EARRAY_OF(NOCONST(Entity)) eaPets,
							...
							ATH_ARG NOCONST(InventoryBag)* pSrcBag, int iSrcSlotIdx,		// Track the srcBag/Idx so we can prevent bag items from being placed into the inventory slot associated with themselves
...
void bagiterator_trh_SetBagByID(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int BagID, BagIterator* iter, GameAccountDataExtract *pExtract);
#define bagiterator_SetBagByID(pEnt, BagID, pIter, pExtract) bagiterator_trh_SetBagByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), BagID, pIter, pExtract)
void bagiterator_trh_SetLiteBagByID(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int BagID, BagIterator* iter, GameAccountDataExtract *pExtract);
#define bagiterator_SetLiteBagByID(pEnt, BagID, pIter, pExtract) bagiterator_trh_SetLiteBagByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), BagID, pIter, pExtract)
...
__forceinline BagIterator* bagiterator_Create()
{
	BagIterator* iter;
	iter = StructCreate(parse_BagIterator);
	iter->i_bag = -1;
	return iter;
}

__forceinline void bagiterator_Destroy(BagIterator *iter)
{
	if (iter)
	{
		...
		iter->eaBags = NULL;
		iter->eaLitebags = NULL;
		StructDestroy(parse_BagIterator, iter);
	}
}

__forceinline void bagiterator_Reset(BagIterator* pIter)
{
	if (pIter)
	{
		eaClear(&pIter->eaBags);
		eaClear(&pIter->eaLitebags);
		pIter->i_cur = 0;
		pIter->i_bag = -1;
	}
}
...
S32 inv_bag_trh_FindItemCountByDefName(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, S32 eBagID, const char* pchDefName, S32 excludeItemFlags, int iCount, U32 bRemoveFound, U32 includeUnremoveables, const ItemChangeReason *pReason, GameAccountDataExtract* pExtract, Item ***peaItemsRemovedOut);
...
#define inv_bag_FindItemCountByDefName(pEnt, eBagID, pchDefName, iCount) inv_bag_trh_FindItemCountByDefName(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), eBagID, pchDefName, 0, iCount, false, true, NULL, NULL)
...
S32 inv_trh_FindItemCountByDefNameEx(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, InvBagFlag searchBagFlags, InvBagFlag excludeBagFlags, const char* pchDefName, S32 excludeItemFlags, int iCount, U32 bRemoveFound, U32 countUnremoveables, const ItemChangeReason *pReason, GameAccountDataExtract* pExtract, Item ***peaItemsRemovedOut);
#define inv_FindItemCountByDefName(pEnt, pchDefName, iCount) inv_trh_FindItemCountByDefNameEx(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), InvBagFlag_DefaultInventorySearch, 0, pchDefName, 0, iCount, false, true, NULL, NULL, NULL)
#define inv_FindItemCountByDefNameEx(pEnt, bagFlags, excludeBagFlags, pchDefName, excludeTags, iCount, includeUnremoveables) inv_trh_FindItemCountByDefNameEx(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), (bagFlags), (excludeBagFlags), pchDefName, (excludeTags), iCount, false, (includeUnremoveables), NULL, NULL, NULL)
...
#define inv_FindItemByDefName(pEnt, pIter, pchName, bIncludeOverflow, bSearchAllBags) inv_trh_FindItemByDefName(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), pIter, pchName, bIncludeOverflow, bSearchAllBags)
...
#define inv_CanGemSlot(pEnt, pHolder, pGem, iDestGemSlot) inv_trh_CanGemSlot(CONTAINER_NOCONST(Entity, pEnt), CONTAINER_NOCONST(Item, pHolder), CONTAINER_NOCONST(Item, pGem), iDestGemSlot)
...
// These 2 functions are kind of bad, and they should not be in this file, and nobody should use them.
