...
static bool ItemAssignmentSettings_Validate(void)
{
	...
	if(g_ItemAssignmentSettings.pStrictAssignmentSlots)
	{
		for(i = eaSize(&g_ItemAssignmentSettings.pStrictAssignmentSlots->ppUnlockExpression)-1;i>=0;i--)
		{
			...
			{
				Errorf("ItemAssignmentSettings Error in unlock expression %d",i);
				bSuccess = false;
			}
			...
			{
				Errorf("ItemAssignmentSettings Error in complete expression %d",i);
				bSuccess = false;
			}
		}
	}
	...
	if (eaSize(&pDef->eaSlots))
	{
		...
		for (i = numSlots -1; i >= 0; --i)
		{
			...
			if (pSlot->bIsOptional)
			{
				...
				{
					ErrorFilenamef(pDef->pchFileName,"%s Has is optional flag turned on for slot %d, but item assignment settings are not flaged to use optional slots!", pDef->pchName, i);
					bSuccess = false;
				}
				iNumOptionalSlots++;
			}
...
static bool ItemAssignment_ValidateRefs(ItemAssignmentDef* pDef)
{
	...
	if (pDef->pRequirements)
	{
		...
		for (i = 0; i < eaSize(&pDef->pRequirements->eaItemCosts); i++)
		{
			...
			if (!pItemDef && REF_STRING_FROM_HANDLE(pItemCost->hItem))
			{
...
S32 ItemAssignments_EvaluateCategoryIsHidden(Entity *pEnt, const ItemAssignmentCategorySettings *pCategory)
{
	...
	{
		MultiVal mVal;
		ExprContext* pContext = ItemAssignments_GetContextEx(pEnt, NULL, NULL, NULL, NULL, NULL, 0, 0, pCategory);
											 ...
											 ItemCategory eItemCategoryUI) 
{
	if (pEnt && pMod && pData && (pSlottedItem || eItemCategoryUI != kItemCategory_None))
	{
		...
		{
			MultiVal mVal;
			ExprContext* pContext = ItemAssignments_GetContextEx(pEnt, NULL, pOutcome, pMod, pSlottedItem, pSlotDef, eItemCategoryUI, 0, NULL);
										...
										ItemCategory eItemCategoryUI)
{
	if (pEnt && (pSlottedItem || eItemCategoryUI != kItemCategory_None))
	{
		...
		{
			MultiVal mVal;
			ExprContext* pContext = ItemAssignments_GetContextEx(pEnt, NULL, NULL, pMod, pSlottedItem, pSlotDef, eItemCategoryUI, 0, NULL);
																	  ...
																	  GameAccountDataExtract* pExtract)
{
	...
	if (ISNULL(pSlot) || ISNULL(pSlot->pItem) || pSlot->pItem->id != pSlottedItem->uItemID)
	{
		...
		inv_trh_FindItemByIDEx(ATR_PASS_ARGS, pEnt, pIter, pSlottedItem->uItemID, false, true);
		pBag = bagiterator_GetCurrentBag(pIter);
																			 ...
																			 GameAccountDataExtract* pExtract)
{
	...
	if(!ItemAssignments_trh_MaxAssignmentCheck(pEnt, pDef))
	{
...
bool ItemAssignments_CheckItemSlotExpression(ItemAssignmentSlotUnlockExpression *pUnlock, Entity *pEnt, ItemAssignmentCompletedDetails *pCompletedAssignment)
{
	...
	if(pData && pUnlock && pUnlock->pCompletedExpr)
	{
		...
		{
			MultiVal mVal;
			ExprContext* pContext = ItemAssignments_GetContextEx(pEnt,pCompletedAssignment,NULL,NULL,NULL,NULL,0,0, NULL);
...
void ItemAssignments_GetSearchInvBagFlags(S32 *piSearchFlags, S32 *piExcludeFlags)
{
	*piSearchFlags = InvBagFlag_DefaultInventorySearch;
	
	if (g_ItemAssignmentSettings.eIncludeBagFlagsForItemCosts)
	{
		*piSearchFlags = g_ItemAssignmentSettings.eIncludeBagFlagsForItemCosts;
	}

	if (piExcludeFlags)
	{
		*piExcludeFlags = g_ItemAssignmentSettings.eExcludeBagFlagsForItemCosts;
	}
}
			...
			if (eaSize(&pDef->pRequirements->eaItemCosts))
			{
				...
				for (i = 0; i < eaSize(&pDef->pRequirements->eaItemCosts); i++)
				{
					...
					if ((pRequiredDef->eType == kItemType_Numeric &&
						...
						inv_FindItemCountByDefNameEx(pEnt, eSearchBags, eExcludeSearchBags, pRequiredDef->pchName, kItemFlag_SlottedOnAssignment, iCount, false) < iCount))
					{
						if (peaErrors)
						{
...
S32 ItemAssignments_GetExperienceThresholdForRank(SA_PARAM_OP_VALID const ItemAssignmentCategorySettings *pCategory,
													SA_PARAM_NN_VALID ItemAssignmentRankingSchedule *pSchedule, S32 iRank)
{
	S32 iNumRanks = ItemAssignments_GetNumRanks(pCategory, pSchedule);
...
S32 ItemAssignments_GetNumRanks(SA_PARAM_OP_VALID const ItemAssignmentCategorySettings *pCategory, 
								SA_PARAM_NN_VALID ItemAssignmentRankingSchedule *pSchedule)
{
	if (!pCategory || pCategory->iNumRanks <= 0)
		return eaiSize(&pSchedule->eaiExperience);

	return pCategory->iNumRanks;
}

S32 ItemAssignments_GetMaxExperience(SA_PARAM_NN_VALID const ItemAssignmentCategorySettings *pCategory, 
										SA_PARAM_NN_VALID ItemAssignmentRankingSchedule *pSchedule)
{
	S32 iNumRanks = ItemAssignments_GetNumRanks(pCategory, pSchedule);

	return ItemAssignments_GetExperienceThresholdForRank(pCategory, pSchedule, iNumRanks - 1);
}
...
bool ItemAssignments_HasAssignment(Entity* pEnt, ItemAssignmentDef* pDef, ItemAssignmentPlayerData *pItemAssignmentData)
{
	...
	if (pItemAssignmentData)
	{
		...
		i = eaIndexedFindUsingString(&pItemAssignmentData->eaAdditionalPersonalAssignments, pDef->pchName);
		if (i >= 0)
			return true;
	}
																		...
																		ItemAssignmentPersonalIterator *it)
{
	...
	else if (it->iSubList == iNumBuckets + 2)
	{
		eaAvailableAssignmentsList = pPlayerData->eaAdditionalPersonalAssignments;
	}
	else
		return NULL;
...
bool ItemAssignments_GetForceCompleteNumericCost(Entity *pEnt, ItemAssignment* pAssignment, S32 *piCostToComplete)
{
	if (g_ItemAssignmentSettings.pExprForceCompleteNumericCost)
	{
		...
		pContext = ItemAssignments_GetContextEx(pEnt, NULL, NULL, NULL, NULL, NULL, 0, iTimeToComplete, NULL);
...
// Check to see if this assignment can be started based on the number it allows to be run at the same time. Returns true if its ok to start another on
// Only in progress assignments are counted
AUTO_TRANS_HELPER;
bool ItemAssignments_trh_MaxAssignmentCheck(ATH_ARG NOCONST(Entity)* pEnt, ItemAssignmentDef* pDef)
{
	...
	if (iMaxUniqueAssignments > 0)
	{
		NOCONST(ItemAssignmentPersistedData) *pPersistedData;
		S32 iActiveCount = 0, i;
		NOCONST(ItemAssignment) *pActiveAssignment;

		if(ISNULL(pEnt) || ISNULL(pEnt->pPlayer))
		{
			// bad ent or not a player
			return false;
		}

		pPersistedData = pEnt->pPlayer->pItemAssignmentPersistedData;
		if(ISNULL(pPersistedData))
		{
			// No item assignment data yet so there can't be any running
			return true;
		}

		for(i = 0; i < eaSize(&pPersistedData->eaActiveAssignments); ++i)
		{
			ItemAssignmentDef* pActiveDef = NULL;
			pActiveAssignment = pPersistedData->eaActiveAssignments[i];
		
			pActiveDef = GET_REF(pActiveAssignment->hDef);
			if (pDef == pActiveDef)
			{	
				iActiveCount++;
