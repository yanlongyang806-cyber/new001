	...
	for (i = 0; i < eaSize(&pEnt1->pPlayer->pTradeBag->ppTradeSlots); i++) {
		...
		if (pItemDef->eType == kItemType_Numeric) {
			...
			if(pBagItem)
			{
				...
				{
					eCountBag = GetBestBagForItemDef(pEnt2, pItemDef, pSlot->pItem, pSlot->count, true, pExtract2);
				}
			} else {
...
void Trade_GetTradeableItems(TradeSlotLite ***peaSlots, Entity* pEnt, S32 **ppiBagIDs, bool bIncludePets, bool bIncludeUnidentified)
{
	S32 iCount = 0;
	...
	if (pEnt && pEnt->pInventoryV2)
	{
		...
		for (j = 0; j < eaSize(&pEnt->pInventoryV2->ppInventoryBags); j++)
		{
			...
			if (((ea32Find(ppiBagIDs,-1) != -1 && bInANormalBag) || ea32Find(ppiBagIDs,invbag_bagid(pBag)) != -1) && !(invbag_flags(pBag) & InvBagFlag_BankBag))
			{
