...
static int s_hVarCategoryEventName = 0;
static const char *s_pchVarCategoryEventName;
...
void ItemAssignmentsEval_Init(void)
{
	...
	s_pchVarCategoryEventName = allocAddString("ItemCategoryEventName");
}
										  ...
										  const ItemAssignmentCategorySettings *pCategory)
{
	...
	if (!s_pItemAssignmentsContext)
	{
		...
		stTable = exprContextCreateFunctionTable("ItemAssignments");
	...
	exprContextSetStringVarPooledCached(s_pItemAssignmentsContext,s_pchVarCategoryEventName, (pCategory?pCategory->pchEventName:""), &s_hVarCategoryEventName);
