...
NOCONST(PlayerCostume) *algoPetDef_GenerateRandomCostume(AlgoPetDef *pDef, CritterDef *pCritterDef, AllegianceDef *pAllegiance, AllegianceDef *pSubAllegiance, SpeciesDef **ppSpeciesOut, const char *filename, U32 *pSeed)
{
	...
	pSlotType = costumeLoad_GetSlotType(pDef->pchSlotType);
	...
	if (pCritterDef)
	{
		...
		if (eaSize(&pDef->eaSpecies))
		{
			...
			for (i = eaSize(&pDef->eaSpecies)-1; i >= 0; --i)
			{
				...
				eaPush(&eaValidSpecies, pSpecies);
			}

			pSpecies = eaGet(&eaValidSpecies, randomIntRangeSeeded(pSeed,RandType_LCG,0,eaSize(&eaValidSpecies)-1));
			eaDestroy(&eaValidSpecies);
			...
			if (pSpecies)
			{
				...
				costumeRandom_FillRandom(pCostume, pSpecies, NULL, NULL, NULL, NULL, pSlotType, true, true, false, false, true, true, true);
				...
				{
					CostumeRefForAlgoPet **eaSpeciesCostumes = NULL;
					CostumeRefForAlgoPet **eaSkeletonCostumes = NULL;
					CostumeRefForAlgoPet **eaCostumes = NULL;
					F32 fWeight = 0, fValue = 0;
					...
					{
						PlayerCostume *pUniform = GET_REF(pDef->eaUniforms[i]->hPlayerCostume);
						if (!pUniform || pDef->eaUniforms[i]->fWeight < 0)
							continue;
						if (GET_REF(pUniform->hSpecies) == pSpecies)
							eaPush(&eaSpeciesCostumes, pDef->eaUniforms[i]);
						if (REF_COMPARE_HANDLES(pUniform->hSkeleton, pSpecies->hSkeleton))
							eaPush(&eaSkeletonCostumes, pDef->eaUniforms[i]);
						eaPush(&eaCostumes, pDef->eaUniforms[i]);
					}

					if (eaSize(&eaSpeciesCostumes) > 0)
					{
						eaDestroy(&eaCostumes);
						eaCostumes = eaSpeciesCostumes;
						eaSpeciesCostumes = NULL;
					}
					else if (eaSize(&eaSkeletonCostumes) > 0)
					{
						eaDestroy(&eaCostumes);
						eaCostumes = eaSkeletonCostumes;
						eaSkeletonCostumes = NULL;
					}

					for (i = eaSize(&eaCostumes)-1; i >= 0; --i)
					{
						fWeight += eaCostumes[i]->fWeight;
					}

					fValue = (randomF32Seeded(pSeed,RandType_LCG) + 1) * 0.5f * fWeight;

					for (i = eaSize(&eaCostumes)-1; i >= 0; --i)
					{
						fValue -= eaCostumes[i]->fWeight;

						if (fValue <= 0)
						{
							PlayerCostume *pc = GET_REF(eaCostumes[i]->hPlayerCostume);
							PlayerCostume **eaUnlockedCostumes = NULL;
							if (eaCostumes[i]->bUnlockAll)
								eaPush(&eaUnlockedCostumes, pc);
							SET_HANDLE_FROM_REFERENT("Species",pSpecies,pCostume->hSpecies);
							costumeTailor_ApplyCostumeOverlay((PlayerCostume*)pCostume, NULL, pc, eaUnlockedCostumes, "Uniforms", pSlotType, true, false, true, false);
					...
					eaDestroy(&eaCostumes);
					eaDestroy(&eaSkeletonCostumes);
					eaDestroy(&eaSpeciesCostumes);
				}
			}
		}
	}
...
bool algoPetDef_ValidateUniformPart(AlgoPetDef *pDef, int i, CostumeRefForAlgoPet *pUniform, PCPart *pPart)
{
	int k;
	PCGeometryDef *pGeo = GET_REF(pPart->hGeoDef);
	PCMaterialDef *pMat = GET_REF(pPart->hMatDef);
	PCTextureDef *apTexs[5] = {
	...
	if (pGeo && (pGeo->eRestriction & kPCRestriction_Player_Initial) == 0 && !pUniform->bUnlockAll)
	{
		ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def costume %s[%d] uses non-player initial geo %s",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i,pGeo->pcName);
		return false;
	}
	else if (pGeo && (pGeo->eRestriction & (kPCRestriction_Player_Initial | kPCRestriction_Player)) == 0)
	{
		ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def costume %s[%d] uses non-player geo %s",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i,pGeo->pcName);
		return false;
	}

	if (pMat && (pMat->eRestriction & kPCRestriction_Player_Initial) == 0 && !pUniform->bUnlockAll)
	{
		ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def costume %s[%d] uses non-player initial mat %s",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i,pMat->pcName);
		return false;
	}
	else if (pMat && (pMat->eRestriction & (kPCRestriction_Player_Initial | kPCRestriction_Player)) == 0)
	{
		ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def costume %s[%d] uses non-player mat %s",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i,pMat->pcName);
		return false;
	}

	for (k=0;k<ARRAY_SIZE(apTexs);k++)
	{
		if (apTexs[k] && (apTexs[k]->eRestriction & kPCRestriction_Player_Initial) == 0 && !pUniform->bUnlockAll)
		{
			ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def costume %s[%d] uses non-player initial tex %s",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i,apTexs[k]->pcName);
			return false;
		}
		else if (apTexs[k] && (apTexs[k]->eRestriction & (kPCRestriction_Player_Initial | kPCRestriction_Player)) == 0)
		{
			ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def costume %s[%d] uses non-player tex %s",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i,apTexs[k]->pcName);
			return false;
		}
	}

	if (pPart->pClothLayer)
		return algoPetDef_ValidateUniformPart(pDef, i, pUniform, pPart->pClothLayer);
	return true;
}
...
bool algoPetDef_Validate(AlgoPetDef *pDef)
{
	...
	for(i=0;i<eaSize(&pDef->eaUniforms);i++)
	{
		PlayerCostume *pCostume = GET_REF(pDef->eaUniforms[i]->hPlayerCostume);

		if(pDef->eaUniforms[i]->fWeight <= 0)
		{
			ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def has a uniform with a weight of 0! %s[%d]",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i);
			return false;
		}
		...
		if (pCostume)
		{
			...
			{
				if (!algoPetDef_ValidateUniformPart(pDef, i, pDef->eaUniforms[i], pCostume->eaParts[j]))
					return false;
			}
		}
		else
		{
			ErrorFilenamef(pDef->pchFileName,"%s Algo Pet Def references unknown costume %s[%d]",pDef->pchName,REF_STRING_FROM_HANDLE(pDef->eaUniforms[i]->hPlayerCostume),i);
			return false;
		}
	}
...
void AlgoPetExprInit(void)
{
	...
	stTable = exprContextCreateFunctionTable("AlgoPet");
