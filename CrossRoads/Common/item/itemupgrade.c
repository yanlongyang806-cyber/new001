...
void itemUpgrade_SetupContext(void)
{
	...
	stTable = exprContextCreateFunctionTable("ItemUpgrade");
		...
		for(i=0;i<eaSize(&g_sItemUpgradeConfig.ppItemNames);i++)
		{
			...
			for(iStep = 0;iStep<eaSize(&g_sItemUpgradeConfig.ppTiers);iStep++)
			{
				...
				if(pDef || IsClient())
				{
					...
					pContainer->pchItemDef = allocAddString(estrName);
...
int itemUpgrade_FindCurrentRank(ItemDef *pBaseItem, ItemUpgradeLadder **ppLadderOut)
{
	...
	for(i=0;i<eaSize(&g_sItemUpgradeConfig.ppLadders);i++)
	{
		if(strstr(g_sItemUpgradeConfig.ppItemNames[i]->pchName,"%") || strnicmp(g_sItemUpgradeConfig.ppLadders[i]->pchItemPrefix, pBaseItem->pchName, strlen(g_sItemUpgradeConfig.ppLadders[i]->pchItemPrefix))==0)
		{
			...
			{
				if(RefSystem_ReferentFromString(g_hItemDict,pLadder->ppItems[iReturn]->pchItemDef) == pBaseItem)
					return iReturn;
			}
		}	
	}
...
ItemUpgradeModifiers *itemUpgrade_FindModifier(ItemDef *pModifier)
{
	...
	{
		if(g_sItemUpgradeConfig.ppModifiers[i]->pchItemDef == pModifier->pchName)
...
AUTO_TRANS_HELPER;
const char *itemUpgrade_GetUpgradeString(ATH_ARG NOCONST(Entity) *pEnt, ItemDef *pBaseItem, int *iCount, ItemDef *pModifier)
{
	ItemUpgradeLadder *pLadder = NULL;
	ItemUpgradeModifiers *pModifierDef = NULL;
	int iCurrentRank = 0;
	...
	{
		ItemDefRefContainer *pDefCon = NULL;
		const char *pchDefName = NULL;
...
{
	const char *pchItemDef = itemUpgrade_GetUpgradeString(pEnt,pBaseItem,iCount,pModifier);

	if(pchItemDef)
	{
		ItemDef *pDef = RefSystem_ReferentFromString(g_hItemDict,pchItemDef);
