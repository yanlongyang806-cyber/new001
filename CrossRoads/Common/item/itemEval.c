...
static const char *s_pcVarOwnerEnt;
static int s_hVarOwnerEnt = 0;
...
void itemeval_EvalGem(int iPartitionIdx, Expression *pExpr, ItemDef *pDef, ItemPowerDef *pItemPowerDef, Item *pItem, ItemDef *pGemDef, ItemDef *pAppearanceDef, Entity *pEnt, S32 iLevel, ItemQuality eQuality, S32 iEP, const char *pcFilename, S32 iDestinationSlot, MultiVal * pResult)
{
	if (pExpr) {
		...
		if (pEnt)
		{
			...
			else if (pEnt->erOwner)
			{
				pPlayerEnt = entFromEntityRef(entGetPartitionIdx(pEnt),pEnt->erOwner);
			}
			else if (!pEnt->myRef && pEnt->pSaved && pEnt->pSaved->conOwner.containerID)
			{
				pPlayerEnt = entFromContainerID(iPartitionIdx, pEnt->pSaved->conOwner.containerType, pEnt->pSaved->conOwner.containerID);
			}
		}
		...
		if (g_pItemContext && !bContextInUse) 
		{
			...
			exprContextSetPointerVarPooledCached(g_pItemContext, s_pcVarPlayer, (Entity*)pPlayerEnt, parse_Entity, true, true, &s_hVarPlayer);
			exprContextSetPointerVarPooledCached(g_pItemContext, s_pcVarOwnerEnt, (Entity*)pEnt, parse_Entity, true, true, &s_hVarOwnerEnt);
		...
		else if (g_pChildItemContext && !bChildContextInUse) 
		{
			...
			exprContextSetPointerVarPooledCached(g_pItemContext, s_pcVarPlayer, (Entity*)pPlayerEnt, parse_Entity, true, true, &s_hVarPlayer);
			exprContextSetPointerVarPooledCached(g_pChildItemContext, s_pcVarOwnerEnt, (Entity*)pEnt, parse_Entity, true, true, &s_hVarOwnerEnt);
...
S32 itemeval_GetTransmutationCost(int iPartitionIdx, Entity *pEnt, Item* pStats, Item* pAppearance)
{
	if (g_ItemConfig.pItemTransmuteCost)
	{
		...
		ItemDef* pAppearanceBaseDef = GET_REF(pAppearance->hItem);	// The base of the appearance item.
		ItemDef* pStatsDef = GET_REF(pStats->hItem);
		MultiVal mv = {0};

		// If we don't have a transmuted appearance, use the base. 
		if (!pAppearanceDef)
		{
			pAppearanceDef = pAppearanceBaseDef;
		}

		// Free-ness is determined by the base of the appearance item. Certain items-we-want-to-use-the-appearance-of are being given for CTAs now. We want it to
		//   be no cost to transmute this appearance onto items-we-want-to-keep-the-stats-of. Regardless of the level of the items-we-want-to-keep-the-stats-of.
		// Note: We don't want to use the appearance of the appearance item to determine this since it may already be a transmuted item.
		if (pAppearanceBaseDef!=NULL && pAppearanceBaseDef->bTransmutationHasNoCost)
		{
			return(0);
		}
...
int itemeval_Startup(void)
{
	...
	s_pcVarOwnerEnt = allocAddStaticString("OwnerEnt");
	...
	stFuncs = exprContextCreateFunctionTable("ItemEval");
	...
	exprContextSetPointerVarPooledCached(g_pItemContext, s_pcVarOwnerEnt, NULL, parse_Entity, true, true, &s_hVarOwnerEnt);
	...
	exprContextSetPointerVarPooledCached(g_pChildItemContext, s_pcVarOwnerEnt, NULL, parse_Entity, true, true, &s_hVarOwnerEnt);
...
{
	Entity *pEnt = exprContextGetVarPointerUnsafePooled(pContext, s_pcVarOwnerEnt);
	S32 i;
	S32 iResult = 0;
	if (pItem->pAlgoProps)
	{
