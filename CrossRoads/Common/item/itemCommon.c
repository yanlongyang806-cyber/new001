...
void item_trh_FillInPreSlottedGems(ATH_ARG NOCONST(Item) *pItem, int iCharacterLevel, const char *pcRank, AllegianceDef *pAllegiance, U32 *pSeed)
{
	...
	for(i=0;i<eaSize(&pItemDef->ppItemGemSlots);i++)
	{
		...
		if(pGemItem)
		{
			inv_trh_GemItem(NULL,pItem,pGemItem,i);
		}
	}
}
...
void item_trh_FixupAlgoProps(ATH_ARG NOCONST(Item)* pItem)
{
	...
	if (NONNULL(pItem) && NONNULL(pItem->pAlgoProps) && pItemDef)
	{
		if (ISNULL(pItem->pAlgoProps->pDyeData) &&
			...
			pItem->pAlgoProps->uProgressionLevel <= 1 &&
			pItem->pAlgoProps->uProgressionXP == 0 &&
	...
	if (NONNULL(pItem) && NONNULL(pItem->pSpecialProps) && pItemDef)
	{
		if (item_trh_FixupItemGemSlots(pItem) <= 0)
		{
			if (!GET_REF(pItem->pSpecialProps->hCostumeRef) &&
				...
				pItem->pSpecialProps->fRewardPackScale == 1.0f)
			{
...
// helper function to item_trh_FixupPowers
// iPowerIndex - the current index of pppCurrentPowersList we'll be putting this power in 
// iNumItemPowerDefs - the number of ItemPowerDefs in the current item/gem list we're acting on
// iItemPowerDefOffset - using iPowerIndex, the offset to get the right power from item_trh_GetItemPowerDef
AUTO_TRANS_HELPER
ATR_LOCKS(pItem, ".Palgoprops.Ppitempowerdefrefs, .Pspecialprops.Ppitemgemslots, .Hitem, .Palgoprops.Level_Useaccessor");
bool item_trh_FixupPowers_AddPowerHelper(	ATH_ARG NOCONST(Item)* pItem, 
											ItemPowerDef *pItemPowerDef,
											PowerDef *pPowerDef, 
											NOCONST(Power) ***pppCurrentPowersList, 
											S32 iPowerIndex,
											S32 iNumItemPowerDefs,
											S32 iItemPowerDefOffset)
{
	if(pPowerDef->eType != kPowerType_Innate)
	{
		...
		{
			bMatchCheck = false;
			pPower = eaGet(pppCurrentPowersList, iPowerIndex);
			...
			else if(pPower)
			{
				...
				for(iItemPowerDefLater = iPowerIndex + 1; iItemPowerDefLater < iNumItemPowerDefs; iItemPowerDefLater++)
				{
					ItemPowerDef *pItemPowerDefLater = item_trh_GetItemPowerDef(pItem, iItemPowerDefOffset + iItemPowerDefLater);
				...
				if(iItemPowerDefLater >= iNumItemPowerDefs)
				{
					...
					eaRemove(pppCurrentPowersList, iPowerIndex);
		...
		if(!bMatchFound)
		{
			...
			eaInsert(pppCurrentPowersList, pPower, iPowerIndex);
...
// Creates Power instances for a given item and its gems
AUTO_TRANS_HELPER
...
{
	ItemDef *pItemDef;
	S32 iNumItemPowerDefs = 0;
	S32 iNonGemPowerCount = 0;
	S32 i;
	...
	iNumItemPowerDefs = item_trh_GetNumItemPowerDefs(pItem, false);

	// go through all the ItemPowerDefs on the item and try to add the power to the item
	for(i = 0; i < iNumItemPowerDefs; i++)
	{
		ItemPowerDefRef * pItemPowerDefRef = item_trh_GetItemPowerDefRef(pItem, i);
		...
		// try and add the power
		if (item_trh_FixupPowers_AddPowerHelper(pItem, pItemPowerDef, pPowerDef, &pItem->ppPowers, 
												iNonGemPowerCount, iNumItemPowerDefs, 0))
		{
			iNonGemPowerCount++;
		}
	}

	// check if we have gems, and if we do loop through each gem and add each of its powers
	if (pItem->pSpecialProps)
	{
		S32 iCurrentItemGemSlot = 0;
		S32 iCurrentGemPowerOffset = 0;
		S32 iGemPowerCount = 0;
		S32 iNumGemSlots;

		iNumGemSlots = eaSize(&pItem->pSpecialProps->ppItemGemSlots);

		// get the offset to the first gem power on the item
		iCurrentGemPowerOffset = item_trh_GetNumItemPowerDefs(pItem, true) - item_trh_GetNumGemsPowerDefs(pItem);

		for (iCurrentItemGemSlot = 0; iCurrentItemGemSlot < iNumGemSlots; ++iCurrentItemGemSlot)
		{
			ItemDef *pGemDef = GET_REF(pItem->pSpecialProps->ppItemGemSlots[iCurrentItemGemSlot]->hSlottedItem);
			S32 iNumGemPowerDefs;

			if (!pGemDef)
				continue;

			iNumGemPowerDefs = eaSize(&pGemDef->ppItemPowerDefRefs);
			iGemPowerCount = 0;

			// process all the ItemPowerDef on the current gem
			for(i = 0; i < iNumGemPowerDefs; i++)
			{
				ItemPowerDefRef * pItemPowerDefRef = item_trh_GetItemPowerDefRef(pItem, iCurrentGemPowerOffset + i);
				ItemPowerDef *pItemPowerDef = pItemPowerDefRef ? GET_REF(pItemPowerDefRef->hItemPowerDef) : NULL;
				PowerDef *pPowerDef = pItemPowerDef ? GET_REF(pItemPowerDef->hPower) : NULL;
								
				S32 iGemPowerOffset = 0;
				...
				// if the gem has a chance to not apply, check that now
				if (pItemPowerDefRef->fGemSlottingApplyChance < 1.f)
				{
					S32 iRollIndex;
				...
				// try and add the power to the gem
				if (item_trh_FixupPowers_AddPowerHelper(pItem, pItemPowerDef, pPowerDef, 
														&pItem->pSpecialProps->ppItemGemSlots[iCurrentItemGemSlot]->ppPowers, 
														iGemPowerCount, iNumGemPowerDefs, iCurrentGemPowerOffset))
				{
					iGemPowerCount++;
				}
			}

			// increment our offset to the next gem power when using item_trh_GetItemPowerDefRef
			iCurrentGemPowerOffset += iNumGemPowerDefs;
		}
	}
	
	// We've made sure the first iNonGemPowerCount Powers on the Item match the entirety of the Item's ItemPowerDefs.
	//  If we have any Powers left over, we need to delete them.
	while(eaSize(&pItem->ppPowers) > iNonGemPowerCount)
	{
...
NOCONST(Power)* item_trh_FindPowerByID(ATH_ARG NOCONST(Entity)* pEnt, U32 uiID, InvBagIDs *pBagID, int *pSlotIdx, Item **ppItem, S32 *piItemPowerIdx)
{
	...
	for(iBag = eaSize(&pEnt->pInventoryV2->ppInventoryBags)-1; iBag >=0; iBag--)
	{
		...
		bagiterator_trh_SetBagByID(ATR_EMPTY_ARGS, pEnt,bag->BagID, iter, NULL);
...
bool item_idCheck(ATH_ARG NOCONST(Entity) *pEnt, ATH_ARG NOCONST(Item) *pItem, GameAccountDataExtract *pExtract)
{
	...
	for (ii=0; ii<NumBags; ii++)
	{
		...
		iter = bagiterator_Create();
		bagiterator_trh_SetBagByID(ATR_EMPTY_ARGS, pEnt,bag->BagID, iter, pExtract);
		for(; !bagiterator_Stopped(iter); bagiterator_Next(iter))
		{
...
int item_AddPowersFromItems(int iPartitionIdx, Entity *pEnt, GameAccountDataExtract *pExtract)
{
	...
	for(iBag = eaSize(&pEnt->pInventoryV2->ppInventoryBags)-1; iBag>=0; iBag--)
	{
		...
		iter = bagiterator_Create();
		bagiterator_SetBagByID(pEnt,bag->BagID,iter,pExtract);
		for(; !bagiterator_Stopped(iter); bagiterator_Next(iter))
		{
			...
			for(iPower = 0; iPower < NumPowers; iPower++)
			{
				...
				//Check to make sure this type of power isn't disallowed in this bag.
				if ( (invbag_flags(bag) & (InvBagFlag_DisallowClickPowers)) != 0)
				{
					if (pPowerDef->eType != kPowerType_Innate && pPowerDef->eType != kPowerType_Passive)
						continue;
				}
...
void item_UpdateLimitedUsePowers(Entity *pEnt, GameAccountDataExtract *pExtract)
{
	...
	for(iBag = eaSize(&pEnt->pInventoryV2->ppInventoryBags)-1; iBag>=0; iBag--)
	{
		...
		if (iBagID != InvBagIDs_Loot)
		{
			...
			bagiterator_SetBagByID(pEnt, iBagID, iter, pExtract);
...
bool item_ValidateAllButRefs(ItemDef *pDef)
{
	...
	if(pDef->iStackLimit < 1)
	{
		ErrorFilenamef( pDef->pchFileName, "Item stack limit must be at least 1.");
		retcode = false;
	}
	...
	if (pDef->pRewardPackInfo)
	{
		...
		if(pDef->pRewardPackInfo->bCanScaleRewards)
		{
			if(pDef->iStackLimit > 1)
			{
				ErrorFilenamef(pDef->pchFileName,"Reward pack set cannot be set to scale rewards cannot and be stackable. Please set stack limit to 1");
				retcode = false;
			}
		}
	}
	...
	// validate numeric alert thresholds
	if(pDef->eType == kItemType_Numeric)
	{
		FOR_EACH_IN_EARRAY(pDef->eaNumericThresholdDefs, NumericThresholdDef, pNumericThresholdDef)
		{
			S32 iThresholdSize = eaiSize(&pNumericThresholdDef->piThresholdPerLevel);
			S32 iDurationSize = eaiSize(&pNumericThresholdDef->puDurationPerLevel);
			S32 iCooldownSize = MAX(iThresholdSize, iDurationSize);
			S32 iter;

			eafClear(&pNumericThresholdDef->pfCooldownRatePerLevel);
			for(iter = 0; iter < iCooldownSize; iter++)
			{
				S32 iThreshold = (iter < iThresholdSize) ? pNumericThresholdDef->piThresholdPerLevel[iter] : pNumericThresholdDef->piThresholdPerLevel[iThresholdSize - 1];
				S32 iDuration = (iter < iDurationSize) ? pNumericThresholdDef->puDurationPerLevel[iter] : pNumericThresholdDef->puDurationPerLevel[iDurationSize - 1];
				eafPush(&pNumericThresholdDef->pfCooldownRatePerLevel, (F32)iThreshold / (F32)iDuration);
			}
		}
		FOR_EACH_END;
	}
	else if(eaSize(&pDef->eaNumericThresholdDefs))
	{
		ErrorFilenamef(pDef->pchFileName, "Numeric Thresholds are only supported by the Numeric Item Type.");
		retcode = false;
	}
...
bool item_ValidateRefs(ItemDef *pDef)
{
	...
	if (!GET_REF(pDef->eventTooltipMsg.hMessage) && REF_STRING_FROM_HANDLE(pDef->eventTooltipMsg.hMessage))
	{
		ErrorFilenamef(pDef->pchFileName, "Item referencesnon-existent message '%s'", REF_STRING_FROM_HANDLE(pDef->eventTooltipMsg.hMessage));
		retcode = false;
	}
	...
	else if(pDef->pRewardPackInfo && pDef->pRewardPackInfo->bPromptRewardChoices)
	{
		RewardTable *pTable = GET_REF(pDef->pRewardPackInfo->hRewardTable);

		if(pTable && pTable->Algorithm == kRewardAlgorithm_Weighted)
		{
			ErrorFilenamef(pDef->pchFileName, "Item pack %s prompts reward choices %s, but reward table algorithm is Weighted, this will not work",pDef->pchName,pTable->pchName);
		}
	}
#endif
...
static bool itemdef_VerifyUsageRestrictionsAllegiance_RequireAll(UsageRestriction *pRestrict, AllegianceDef *pEntAllegiance, AllegianceDef *pEntSubAllegiance)
{
	...
	{
		pRequiredAllegiance = GET_REF(pRestrict->eaRequiredAllegiances[i]->hDef);
...
{
	AllegianceDef *pEntAllegiance = NONNULL(pOwner) ? GET_REF(pOwner->hAllegiance) : GET_REF(pEnt->hAllegiance);
	AllegianceDef *pEntSubAllegiance = NONNULL(pOwner) ? GET_REF(pOwner->hSubAllegiance) : GET_REF(pEnt->hSubAllegiance);
	int i;
	if (pRestrict && eaSize(&pRestrict->eaRequiredAllegiances))
	{
		if (pRestrict->bRequireAllAllegiances)
		{
			return itemdef_VerifyUsageRestrictionsAllegiance_RequireAll(pRestrict, pEntAllegiance, pEntSubAllegiance);
		}
		...
		for (i = eaSize(&pRestrict->eaRequiredAllegiances)-1; i >= 0; i--)
		{
			...
			if (pDef)
			{
				if (pDef == pEntAllegiance || pDef == pEntSubAllegiance)
				{
					break;
				}
			}
...
bool item_HasRewardPackChoices(ItemDef *pItemDef)
{
	if(item_IsRewardPack(pItemDef) && SAFE_MEMBER2(pItemDef,pRewardPackInfo,bPromptRewardChoices))
	{
		return true;
	}
	return false;
...
bool item_ItemPowerActiveEx(Entity* pEnt, InventoryBag* pBag, Item* pItem, int PowerIdx, int iPartitionIdx)
{
	ItemDef *pItemDef;
	...
	{
		MultiVal mvExprResult = {0};
		int iEntPartitionIdx = entGetPartitionIdx_NoAssert(pEnt);

		if (!iEntPartitionIdx)
			iEntPartitionIdx = iPartitionIdx;

		itemeval_Eval(iEntPartitionIdx, pItemPowerDef->pRestriction->pRequires, pItemDef, pItemPowerDef, pItem, pEnt, pItemDef->iLevel, pItemDef->Quality, 0, pItemDef->pchFileName, -1, &mvExprResult);
...
{
	const char **eaItemAddedCostumeFX = NULL;
	const char **eaItemAddedCostumeBone = NULL;
	...
	if(pItemDef->pcAddedCostumeFX && pItemDef->pcAddedCostumeBone)
	{
		eaPush(&eaItemAddedCostumeFX, pItemDef->pcAddedCostumeFX);
		eaPush(&eaItemAddedCostumeBone, pItemDef->pcAddedCostumeBone);
	}
	if(pItem && pItem->pSpecialProps)
	{
		...
		{
			ItemDef *pGemDefWithFX = SAFE_GET_REF(pItem->pSpecialProps->ppItemGemSlots[i], hSlottedItem);
			if(pGemDefWithFX && pGemDefWithFX->pcGemAddedCostumeFX && pGemDefWithFX->pcGemAddedCostumeBone)
			{
				eaPush(&eaItemAddedCostumeFX, pGemDefWithFX->pcGemAddedCostumeFX);
				eaPush(&eaItemAddedCostumeBone, pGemDefWithFX->pcGemAddedCostumeBone);
			}
		}
	}
				...
				if (g_CostumeConfig.bEnableItemCategoryAddedBones)
				{
					...
					for (iCat = 0; iCat < eaiSize(&pItemDef->peCategories); iCat++)
					{
						if (pItemDef->peCategories[iCat] >= kItemCategory_FIRST_DATA_DEFINED)
						{
							...
							if (pCatInfo)
							{
								ItemCategoryAdditionalCostumeBone*** peaBoneModifications = NULL;
								if (eEquippedSlot != kSlotType_Secondary && eaSize(&pCatInfo->eaPrimarySlotAdditionalBones) > 0)
								{
									peaBoneModifications = &pCatInfo->eaPrimarySlotAdditionalBones;
								...
								{
									peaBoneModifications = &pCatInfo->eaSecondarySlotAdditionalBones;
									estrPrintf(&estrCloneName, "%s_CLONE_%s_Secondary", pCostume->pcName, pCatInfo->pchName);
									pCostumeClone = RefSystem_ReferentFromString(g_hPlayerCostumeClonesForItemsDict, estrCloneName);
								}

								if(peaBoneModifications)
								{
									FOR_EACH_IN_EARRAY_FORWARDS(eaItemAddedCostumeFX, const char, pcItemAddedCostumeFX)
									{
										const char *pcItemAddedCostumeBone = eaItemAddedCostumeBone[FOR_EACH_IDX(eaItemAddedCostumeFX, pcItemAddedCostumeFX)];
										bool bOverrideAtBone = false;
										bool bFXToAddNeedsParams = true;

										NOCONST(PCFX) *pFXToAdd = StructCreateNoConst(parse_PCFX);
										const char* pchBaseFXAtBone = NULL;
										const char* pchFXAtBone = NULL;

										pFXToAdd->pcName = pcItemAddedCostumeFX;
										pchBaseFXAtBone = pcItemAddedCostumeBone;
										...
										if (pCostumeClone)
										{
											for (iBone = 0; iBone < eaSize(peaBoneModifications); iBone++)
											{
												PCBoneDef* pOldBone = GET_REF((*peaBoneModifications)[iBone]->hOldBone);
												PCBoneDef* pNewBone = GET_REF((*peaBoneModifications)[iBone]->hNewBone);

												pchFXAtBone = NULL;
												if (pCatInfo->pchOverrideCostumeFXBone)
													pchFXAtBone = pCatInfo->pchOverrideCostumeFXBone;
												else
													pchFXAtBone = pcItemAddedCostumeBone;

												if ((*peaBoneModifications)[iBone]->eType == kAdditionalCostumeBoneType_Move)
												{
													//may need to adjust FX too
													if (pFXToAdd && pchFXAtBone == pOldBone->pcBoneName)
													{
														bFXToAddNeedsParams = false;
														item_SetFXParamBlockString(pFXToAdd, pNewBone->pcBoneName, pNewBone->pcBoneName, fFXScale);
													}
												}
												else if ((*peaBoneModifications)[iBone]->eType == kAdditionalCostumeBoneType_Clone)
												{
													if (pFXToAdd && pchFXAtBone == pOldBone->pcBoneName)
													{
														NOCONST(PCFX)* pCloneFX = StructCreateNoConst(parse_PCFX);
														pCloneFX->pcName = pFXToAdd->pcName;
										...
										if(pFXToAdd)
										{
											if (bOverrideAtBone || bFXToAddNeedsParams)
												item_SetFXParamBlockString(pFXToAdd, pchBaseFXAtBone, pcItemAddedCostumeBone, fFXScale);
					...
					if (pCostumeClone)
						eaPush(&pData->eaCostumes, pCostumeClone->pCostumeData);
					else if (pCostume)
	...
	eaDestroy(&eaItemAddedCostumeFX);
	eaDestroy(&eaItemAddedCostumeBone);
...
bool item_GetItemCostumeDataToShow(int iPartitionIdx, const Entity *pEnt, CostumeDisplayData ***peaData, GameAccountDataExtract *pExtract)
{
	...
	for(iBag = eaSize(&pEnt->pInventoryV2->ppInventoryBags)-1; iBag>=0; iBag--)
	{
		...
		bagiterator_SetBagByID(pEnt,SAFE_MEMBER(pEnt->pInventoryV2->ppInventoryBags[iBag],BagID),iter,pExtract);
...
U32 item_GetSlottedGemProgressionXP(SA_PARAM_NN_VALID Item *pHolderItem, S32 iSlot)
{
	if (pHolderItem->pSpecialProps && iSlot < eaSize(&pHolderItem->pSpecialProps->ppItemGemSlots) && IS_HANDLE_ACTIVE(pHolderItem->pSpecialProps->ppItemGemSlots[iSlot]->hSlottedItem))
	{
		return pHolderItem->pSpecialProps->ppItemGemSlots[iSlot]->uProgressionXP;
	}
	return 0;
}

U32 item_GetSlottedGemProgressionLevel(SA_PARAM_NN_VALID Item *pHolderItem, S32 iSlot)
{
	if (pHolderItem->pSpecialProps && iSlot < eaSize(&pHolderItem->pSpecialProps->ppItemGemSlots) && IS_HANDLE_ACTIVE(pHolderItem->pSpecialProps->ppItemGemSlots[iSlot]->hSlottedItem))
	{
		return pHolderItem->pSpecialProps->ppItemGemSlots[iSlot]->uProgressionLevel;
	}
	return 0;
}
...
static CostumeCloneForItemCat* item_CreateCostumeCloneInternal(PlayerCostume* pCostume, ItemCategoryInfo* pCatInfo, ItemCategoryAdditionalCostumeBone** eaAddBones)
{
	...
	if (pCostumeClone)
	{
		CostumeCloneForItemCat* pWrapper = StructCreate(parse_CostumeCloneForItemCat);
		pCostumeClone->pcFileName = NULL;
		pWrapper->pCostumeData = CONTAINER_RECONST(PlayerCostume, pCostumeClone);
		return pWrapper;
	}
	return NULL;
}

void item_CreateItemCategoryCostumeClones_Internal(ResourceLoaderStruct* pLoadStruct)
{
					...
					for(iCostume = 0; iCostume < eaSize(&pDef->ppCostumes); iCostume++)
					{
						...
						if (eaSize(&pCatInfo->eaPrimarySlotAdditionalBones) > 0)
						{
							...
							if (!RefSystem_ReferentFromString(g_hPlayerCostumeClonesForItemsDict, estrCloneName))
							{
								...
								{
									CONTAINER_NOCONST(PlayerCostume, pClone->pCostumeData)->pcName = allocAddString(estrCloneName);
									pClone->pcName = pClone->pCostumeData->pcName;
									eaPush(&pLoadStruct->earrayOfStructs, pClone);
								}
							}
						}
...
void item_CreateItemCategoryCostumeClones(void)
{
	if (g_CostumeConfig.bEnableItemCategoryAddedBones)
	{
		...
		int flags = RESOURCELOAD_SHAREDMEMORY;
		int i;
		int childSize = ParserGetTableSize(parse_CostumeCloneForItemCat);

		eaIndexedEnableVoid(&loadStruct.earrayOfStructs,parse_CostumeCloneForItemCat);

		SetUpResourceLoaderParse(resDictGetParseName(g_hPlayerCostumeClonesForItemsDict),childSize,resDictGetParseTable(g_hPlayerCostumeClonesForItemsDict),resDictGetDeprecatedName(g_hPlayerCostumeClonesForItemsDict));

		MakeSharedMemoryName("ItemCategoryCostumeClones", &pSharedMemoryName);

		resEditStartDictionaryModification(g_hPlayerCostumeClonesForItemsDict);

		if (ParserLoadFromShared(pSharedMemoryName,flags,parse_ResourceLoaderStruct,&loadStruct,&shared_memory, &pFixupString, resDictGetName(g_hPlayerCostumeClonesForItemsDict),NULL, false))
		{
			for(i=eaSizeSlow(&loadStruct.earrayOfStructs)-1; i>=0; i--)
			{
				estrClear(&keyString);
				if (!objGetKeyEString(parse_CostumeCloneForItemCat,loadStruct.earrayOfStructs[i],&keyString))
				{
					continue;
				}
				resEditSetWorkingCopy(resGetDictionary(g_hPlayerCostumeClonesForItemsDict), keyString, loadStruct.earrayOfStructs[i]);
			}
			...
			resWaitForFinishLoading(g_hPlayerCostumeClonesForItemsDict, shared_memory, true);
			estrDestroy(&keyString);
		}
		else if (shared_memory)
		{
			// Load data and copy to shared memory
			item_CreateItemCategoryCostumeClones_Internal(&loadStruct);

			FixupStructLeafFirst(parse_ResourceLoaderStruct, &loadStruct, FIXUPTYPE_POST_BINNING_DURING_LOADFILES, NULL);

			ParserMoveToShared(shared_memory,flags,parse_ResourceLoaderStruct,&loadStruct,resEditGetPendingInfo(g_hPlayerCostumeClonesForItemsDict));
			...
			{
				CheckSharedMemory(parse_ResourceLoaderStruct, &loadStruct);
			}
		...
		{
			//Shared memory is disabled, do a normal load.
...
void item_CreateCostumeCloneCachedRefs(ItemDef* pDef)
{
	if (g_CostumeConfig.bEnableItemCategoryAddedBones)
	{
		char* estrCloneName = NULL;
			...
			if (pDef->peCategories[iCat] >= kItemCategory_FIRST_DATA_DEFINED)
			{
				...
				if (pCatInfo && ((eaSize(&pCatInfo->eaPrimarySlotAdditionalBones) > 0) || (eaSize(&pCatInfo->eaSecondarySlotAdditionalBones) > 0)))
				{
					for(iCostume = 0; iCostume < eaSize(&pDef->ppCostumes); iCostume++)
					{
						if (eaSize(&pCatInfo->eaPrimarySlotAdditionalBones) > 0)
						{
							ItemCostumeClone* pItemCostume = StructCreate(parse_ItemCostumeClone);
							estrPrintf(&estrCloneName, "%s_CLONE_%s_Primary", REF_STRING_FROM_HANDLE(pDef->ppCostumes[iCostume]->hCostumeRef), pCatInfo->pchName);
							SET_HANDLE_FROM_STRING(g_hPlayerCostumeClonesForItemsDict, allocAddString(estrCloneName), pItemCostume->hCostumeRef);
							eaPush(&pDef->ppClonedCostumeRefsForClient, pItemCostume);
						}
						if (eaSize(&pCatInfo->eaSecondarySlotAdditionalBones) > 0)
						{
							ItemCostumeClone* pItemCostume = StructCreate(parse_ItemCostumeClone);
							estrPrintf(&estrCloneName, "%s_CLONE_%s_Secondary", REF_STRING_FROM_HANDLE(pDef->ppCostumes[iCostume]->hCostumeRef), pCatInfo->pchName);
							SET_HANDLE_FROM_STRING(g_hPlayerCostumeClonesForItemsDict, allocAddString(estrCloneName), pItemCostume->hCostumeRef);
							eaPush(&pDef->ppClonedCostumeRefsForClient, pItemCostume);
						}
					}
...
static int itemResValidateCB(enumResourceValidateType eType, const char *pDictName, const char *pResourceName, ItemDef *pItem, U32 userID)
{
	...
	xcase RESVALIDATE_POST_BINNING: // Called after binning or text load/reload
		item_CreateCostumeCloneCachedRefs(pItem);
		return VALIDATE_HANDLED;
...
int item_Startup(void)
{
	...
	if (IsServer())
	{
		...
		// Items are implicitly sent from ResourceDB when a UGC Mission requires them.
...
void item_FixMessages(ItemDef *pItemDef)
{
	...
	// Fix up event-based tooltip
	{
		if (!pItemDef->eventTooltipMsg.pEditorCopy) {
			pItemDef->eventTooltipMsg.pEditorCopy = StructCreate(parse_Message);
		}

		// Set up key if not exactly
		estrPrintf(&tmpS, "%s%s.eventTooltip", tmpKeyPrefix, itemNameSansNamespace);
		if (!pItemDef->eventTooltipMsg.pEditorCopy->pcMessageKey || 
			(stricmp(tmpS, pItemDef->eventTooltipMsg.pEditorCopy->pcMessageKey) != 0)) {
				langFixupMessageWithTerseKey(pItemDef->eventTooltipMsg.pEditorCopy, MKP_ITEMEVENTTOOLTIP, tmpS, "Item Event-Based Tooltip", pchScope);
		}
	}
...
static bool item_VerifyBankContacts(Entity *pEnt, InventoryBag *pSrcBag, InventoryBag *pDstBag, bool bSilent)
{
	...
	if ((invbag_flags(pSrcBag) & (InvBagFlag_BankBag | InvBagFlag_GuildBankBag)) || (invbag_flags(pDstBag) & (InvBagFlag_BankBag | InvBagFlag_GuildBankBag)))
	{
...
static bool item_ItemMoveValidInternalCommon(Entity *pEnt, ItemDef *pSrcItemDef, int iCount, Entity *pSrcEnt, InventoryBag *pSrcBag, S32 iSrcSlot, Entity *pDstEnt, InventoryBag *pDstBag, S32 iDstSlot)
{
	...
	if (pSrcBag != pDstBag && !inv_ent_EquipLimitCheckSwap(pSrcEnt, pDstEnt, pSrcBag, pDstBag, pSrcItemDef, pSrcItem, pDstItemDef, &pLimitCategory, &iEquipLimit))
	{
...
TradeErrorType item_GetTradeError(Item *pItem)
{
	...
	if (pItem && (pDef = GET_REF(pItem->hItem)))
	{
		...
		else
		{
			...
			if(pItem->flags & (kItemFlag_SlottedOnAssignment))
			{
				return kTradeErrorType_CraftingItemInUse;
			}
		}
...
bool item_CanDiscard(Entity * pPlayerEnt,ItemDef * pItemDef,bool bCheatForAL9)
{
	if ((pItemDef->flags & kItemDefFlag_CantDiscard) && (!bCheatForAL9 || entGetAccessLevel(pPlayerEnt) < 9))
	{
		// special case - mission items for missions you don't have are always discardable
		if (pItemDef->eType == kItemType_Mission)
		{
			MissionDef * pMissionDef = GET_REF(pItemDef->hMission);
			MissionInfo* pMissionInfo = mission_GetInfoFromPlayer(pPlayerEnt);
			if (pMissionInfo)
			{
				if (!mission_GetMissionFromDef(pMissionInfo, pMissionDef))
				{
					return true;
				}
			}
...
{
	Message *pItemNameMsg = GET_REF(pDef->displayNameMsg.hMessage);
	estrConcatf(pestrDevText,"<br>----Dev Info----<br>EP Value: %i<br>Item Level: %i<br>Def: %s<br>DispNameMsgKey: %s",item_GetEPValue(entGetPartitionIdx(pEnt), pEnt, pItem),pDef->iLevel,pDef->pchName, pItemNameMsg ? pItemNameMsg->pcMessageKey : "NULL KEY");
}
...
void item_GetNormalizedDescCustom(char** pestrResult, Language lang, Entity* pEnt, 
	...
	if (pEnt)
	{
		...
		if(pItem) {
			...
			if(ppdefsInnate)
			{
				...
				powerdefs_AutoDescInnateMods(entGetPartitionIdx(pEnt), lang, pItem, ppdefsInnate, pfScales, &estrInnateMods, pchInnateDescKey,NULL,NULL,iLevel,true, 0, entGetPowerAutoDescDetail(pEnt,false));
		...
		item_GetFormattedResourceString(lang, &estrItemValue, iItemValue, pchValueKey, iExchangeRate, false);
...
void item_GetFormattedResourceString(Language lang, char** pestrResult, int iResources, const char* pchValueFormatKey, S32 iExchangeRate, bool bShowZeros)
{
	...
	const char* pchValueFormat = langTranslateMessageKey(lang, pchValueFormatKey);
...
NOCONST(AlgoItemProps)* item_trh_GetOrCreateAlgoProperties(ATH_ARG NOCONST(Item)* pItem)
{
	...
	{
		ItemDef* pDef = GET_REF(pItem->hItem);
		NOCONST(AlgoItemProps)* pProps = StructCreateNoConst(parse_AlgoItemProps);

		pProps->iPowerFactor = pDef->iPowerFactor;
		pProps->Quality = pDef->Quality;
		pProps->Level_UseAccessor = pDef->iLevel;
		pProps->MinLevel_UseAccessor = pDef->pRestriction ? pDef->pRestriction->iMinLevel : 0;
		pProps->uProgressionXP = itemProgression_trh_GetInitialXPValue(pItem);
		pProps->uProgressionLevel = itemProgression_trh_GetLevel(pItem);

		pItem->pAlgoProps = pProps;
	}
...
{
	if (pOtherItemDef->pRestriction)
	{
		if (pMainItemDef->pRestriction)
		{
			...
			// The restrictions match
			return true;
		}

		// The donor item has restrictions, but the other one doesn't.  Can't do it.
		return false;
	}

	// The donor item has no restrictions
	return true;
}
...
AUTO_TRANS_HELPER;
bool item_trh_CanTransMutateTo(ATR_ARGS, ATH_ARG NOCONST(Entity) * pEnt, ATH_ARG NOCONST(Item) * pMainItem, SA_PARAM_NN_VALID ItemDef *pMainItemDef, ATH_ARG NOCONST(Item) * pTransmutateToItem, SA_PARAM_NN_VALID ItemDef *pTransmutateToItemDef)
{
	bool bDoClassAndPathChecks = true;

	ItemDef * pMainItemCurrentVisualDef = pMainItemDef;


	if (pTransmutateToItemDef == NULL || pMainItemDef==NULL)
		return false; // This shouldn't happen
	
	// If either base item (donor or recipient) is not enabled for transmutation, we can't do it.
	if (!pMainItemDef->bTransmutationEnabled || !pTransmutateToItemDef->bTransmutationEnabled)
	{
		return false;
	}

	// If the donor item has been transmuted already, we really only care about what it LOOKS like
	if (NONNULL(pTransmutateToItem) && pTransmutateToItem->pSpecialProps && pTransmutateToItem->pSpecialProps->pTransmutationProps)
		pTransmutateToItemDef = GET_REF(pTransmutateToItem->pSpecialProps->pTransmutationProps->hTransmutatedItemDef);

	if (pTransmutateToItemDef == NULL)
		return false; // This shouldn't happen

	if (NONNULL(pMainItem) && pMainItem->pSpecialProps && pMainItem->pSpecialProps->pTransmutationProps)
	{
		ItemDef * pNewDef = GET_REF(pMainItem->pSpecialProps->pTransmutationProps->hTransmutatedItemDef);
		if (pNewDef)
			pMainItemCurrentVisualDef = pNewDef;
	}

	if (pMainItemCurrentVisualDef == pTransmutateToItemDef)
	{
		return false;
	}
	...
	if (NONNULL(pMainItem))
	{
		if (pMainItem->flags & kItemFlag_Bound)
		{
			if (itemdef_trh_VerifyUsageRestrictions(ATR_PASS_ARGS, pEnt, pEnt, pMainItemDef, 0, NULL) && itemdef_trh_VerifyUsageRestrictions(ATR_PASS_ARGS, pEnt, pEnt, pTransmutateToItemDef, 0, NULL))
			{
				// Special case, the item I'm trying to transmute is already bound to me, and both items are usable by me, so it doesn't really matter if they don't
				// have strictly the same usage restrictions
				bDoClassAndPathChecks = false;
			}
		}
	}

	if (bDoClassAndPathChecks)
	{
...
{
	return pItemDef &&
		pItemDef->bTransmutationEnabled && 
...
S32 item_GetNumericThresholdForLevel(const NumericThresholdDef *pNumericThresholdDef, S32 iExpLevel)
{
	S32 iSize;

	devassert(pNumericThresholdDef);

	iSize = eaiSize(&pNumericThresholdDef->piThresholdPerLevel);

	if(iExpLevel > iSize)
		return pNumericThresholdDef->piThresholdPerLevel[iSize - 1];
	else
		return pNumericThresholdDef->piThresholdPerLevel[iExpLevel - 1];
}

U32 item_GetNumericThresholdDurationForLevel(const NumericThresholdDef *pNumericThresholdDef, S32 iExpLevel)
{
	S32 iSize;

	devassert(pNumericThresholdDef);

	iSize = eaiSize(&pNumericThresholdDef->puDurationPerLevel);

	if(iExpLevel > iSize)
		return pNumericThresholdDef->puDurationPerLevel[iSize - 1];
	else
		return pNumericThresholdDef->puDurationPerLevel[iExpLevel - 1];
}

F32 item_GetNumericThresholdCooldownRateForLevel(const NumericThresholdDef *pNumericThresholdDef, S32 iExpLevel)
{
	S32 iSize;

	devassert(pNumericThresholdDef);

	iSize = eafSize(&pNumericThresholdDef->pfCooldownRatePerLevel);

	if(iExpLevel > iSize)
		return pNumericThresholdDef->pfCooldownRatePerLevel[iSize - 1];
	else
		return pNumericThresholdDef->pfCooldownRatePerLevel[iExpLevel - 1];
}
