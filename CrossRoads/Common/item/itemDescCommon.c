...
void Item_InnatePowerAutoDesc(Entity *pEnt, Language lang, Item *pItem, char **ppchDesc, S32 eActiveGemType)
{
	...
	if(ppDefsInnate)
	{
		if(RefSystem_ReferentFromString(gMessageDict,"Item.InnateMod"))
			powerdefs_AutoDescInnateMods(entGetPartitionIdx(pEnt), lang, pItem, ppDefsInnate, pfScales, ppchDesc,"Item.InnateMod",pChar, eaSlotRequired,iLevel,true, eActiveGemType,entGetPowerAutoDescDetail(pEnt,false));
		else
			powerdefs_AutoDescInnateMods(entGetPartitionIdx(pEnt), lang, pItem, ppDefsInnate, pfScales, ppchDesc,NULL,pChar, eaSlotRequired, iLevel,true, eActiveGemType, entGetPowerAutoDescDetail(pEnt,false));
...
void Item_GetInnatePowerDifferencesAutoDesc(Entity *pEnt, Language lang, Item *pItem, Item *pOtherItem, char **ppchDesc)
{
	Character *pChar = pEnt ? pEnt->pChar : NULL;
...
void Item_PowerAutoDescCustom(Entity *pEnt, Language lang, Item *pItem, char **ppchDesc, const char *pchPowerMessageKey, const char *pchAttribMessageKey, S32 eActiveGemSlotType)
{
	Character *pChar = pEnt ? pEnt->pChar : NULL;
	int iLevel, iPower, iNumPowers = item_GetNumItemPowerDefs(pItem, true);
	F32 *pfScales = NULL;
	...
	{
		estrCopy2(ppchDesc, langTranslateMessageRef(lang, pItemDef->msgAutoDesc.hMessage));
	}
	else
	{
		...
		for(iPower=0; iPower< iNumPowers; iPower++)
		{
			...
			if (item_ItemPowerActive(pEnt, NULL, pItem, iPower))
			{
				if(pPower)
				{
					...
					power_AutoDesc(iPartitionIdx,lang,pPower,pChar,NULL,pAutoDesc,NULL,NULL,NULL,true,0,entGetPowerAutoDescDetail(pEnt,false),pExtract,NULL);
					...
					if (pPowerDef)
					{
						powerdef_AutoDescCustom(pEnt, lang, ppchDesc, pPowerDef, pAutoDesc, pchPowerMessageKey, pchAttribMessageKey);
					}
				}
				else
				{
					if(pPowerDef && pPowerDef->eType!=kPowerType_Innate)
					{
						powerdef_AutoDesc(iPartitionIdx,lang,pPowerDef,NULL, pAutoDesc,NULL,NULL,NULL,pChar,NULL,NULL,iLevel,true,entGetPowerAutoDescDetail(pEnt,false),pExtract,NULL);
						powerdef_AutoDescCustom(pEnt, lang, ppchDesc, pPowerDef, pAutoDesc, pchPowerMessageKey, pchAttribMessageKey);
					}
				}
			}
...
static void Item_GetItemCategoriesStringNNO(Language lang, SA_PARAM_NN_VALID ItemDef * pItemDef, SA_PARAM_NN_VALID char ** estrItemCategories)
{
	devassert(pItemDef);
	...
	if (pItemDef && estrItemCategories)
	{
		...
		for (i = 0; i < eaiSize(&pItemDef->peCategories); i++)
		{
				...
				(stricmp(pchCategoryName, "Light") == 0 || stricmp(pchCategoryName, "Medium") == 0 || stricmp(pchCategoryName, "Heavy") == 0 || stricmp(pchCategoryName, "Ranged") == 0))
			{
				...
				pchTranslatedName = langTranslateMessageKey(lang, estrMessageKey);
...
void Item_GetItemCategoriesString(Language lang,
	...
	SA_PARAM_OP_VALID char ** estrItemCategories)
{
	if (pItemDef && estrItemCategories)
	{
		...
		{
			Item_GetItemCategoriesStringNNO(lang, pItemDef, estrItemCategories);
		}
		else
		{
...
static void GetItemDescriptionItemSetBonus(char **pestrItemSetBonusDesc, Language lang, SA_PARAM_OP_VALID Item *pItem, SA_PARAM_NN_VALID ItemDef *pItemDef, SA_PARAM_OP_VALID Entity *pEntity, U8 uiCount, const char* pchPowerMessageKey, const char* pchAttribModsMessageKey, GameAccountDataExtract *pExtract)
{
	int i,j,s;
	...
	for(i=0; i<s; i++)
	{
		...
		langFormatGameMessageKey(lang, &estrCount, "Inventory_ItemInfo_ItemSet_Count",STRFMT_INT("Count",pItemPowerDefRef->uiSetMin),STRFMT_END);
		...
		if(ppDefsInnate)
		{
			...
			if(RefSystem_ReferentFromString(gMessageDict,"Item.InnateMod"))
				powerdefs_AutoDescInnateMods(PARTITION_CLIENT, lang, pItem, ppDefsInnate, pfScales, &estrInnates,"Item.InnateMod",pChar,NULL, iLevel,0, true,eDetail);
			else
				powerdefs_AutoDescInnateMods(PARTITION_CLIENT, lang, pItem, ppDefsInnate, pfScales, &estrInnates,NULL,pChar,NULL, iLevel,true,0,eDetail);
		...
		if(ppDefsPowers)
		{
			...
			for(j=0; j<eaSize(&ppDefsPowers); j++)
			{
				...
				if(pPower)
				{
					power_AutoDesc(PARTITION_CLIENT,lang,pPower,pChar,NULL,pAutoDescPower,NULL,NULL,NULL,true,0,eDetail,pExtract,NULL);
				}
				else
				{
					powerdef_AutoDesc(PARTITION_CLIENT,lang,pPowerDef,NULL,pAutoDescPower,NULL,NULL,NULL,pChar,NULL,NULL,iLevel,true,eDetail,pExtract, NULL);
				}
				powerdef_AutoDescCustom(pEntity, lang, &estrPowers, pPowerDef, pAutoDescPower, pchPowerMessageKey, pchAttribModsMessageKey);
		...
		langFormatGameMessageKey(lang, pestrItemSetBonusDesc, bActive ? "Inventory_ItemInfo_ItemSet_BonusActive" : "Inventory_ItemInfo_ItemSet_BonusInactive",
	...
	GameAccountDataExtract *pExtract)
{
	if(pItemDefSet)
	{
		...
		{
			ItemDef **peItemSetItemDefs = NULL;
			S32 *pActiveList = NULL;
			...
			for(i=0; i<s; i++)
			{
				...
				if(pItemDefMember)
				{
							...
							if(invbag_flags(pBag) & (InvBagFlag_EquipBag|InvBagFlag_WeaponBag|InvBagFlag_ActiveWeaponBag|InvBagFlag_DeviceBag))
							{
								for(k=eaSize(&pBag->ppIndexedInventorySlots)-1; k>=0; k--)
								{
									...
									if(pSlot && pSlot->pItem && GET_REF(pSlot->pItem->hItem)==pItemDefMember)
										break;
									else if(pSlot && pSlot->pItem && pSlot->pItem->pSpecialProps)
									{
										for(l=eaSize(&pSlot->pItem->pSpecialProps->ppItemGemSlots)-1; l>=0; l--)
										{
											ItemDef *pGemItemDef = GET_REF(pSlot->pItem->pSpecialProps->ppItemGemSlots[l]->hSlottedItem);
											if(pGemItemDef==pItemDefMember)
												break;
										}
					...
					if (pItemDefSet->bItemSetMembersUseCategories)
					{
						bool bFound = false;
						for(j = 0; j < eaSize(&peItemSetItemDefs); j++)
						{
							for(k = 0; k < eaiSize(&pItemDefMember->peCategories); k++)
							{
								if(eaiFind(&peItemSetItemDefs[j]->peCategories, pItemDefMember->peCategories[k]) >= 0)
								{
									bFound = true;
									break;
								}
						...
						if(!bFound)
						{
							eaPush(&peItemSetItemDefs, pItemDefMember);
							eaiPush(&pActiveList, bActive);
							iSlotCount++;
						}
						else if(bActive)
						{
							peItemSetItemDefs[j] = pItemDefMember;
							pActiveList[j] = true;
						}
					}
					else
					{
						langFormatGameMessageKey(lang, &estrItems, bActive ? "Inventory_ItemInfo_ItemSet_ItemActive" : "Inventory_ItemInfo_ItemSet_ItemInactive",
							STRFMT_STRING("Item",NULL_TO_EMPTY(langTranslateDisplayMessage(lang,pItemDefMember->displayNameMsg))),
							STRFMT_END);
					}
				}
			}

			if (pItemDefSet->bItemSetMembersUseCategories && eaSize(&peItemSetItemDefs) > 0)
			{
				for(i = 0; i < eaSize(&peItemSetItemDefs); i++)
				{
					langFormatGameMessageKey(lang, &estrItems, pActiveList[i] ? "Inventory_ItemInfo_ItemSet_ItemActive" : "Inventory_ItemInfo_ItemSet_ItemInactive",
						STRFMT_STRING("Item",NULL_TO_EMPTY(langTranslateDisplayMessage(lang,peItemSetItemDefs[i]->displayNameMsg))),
						STRFMT_END);
				}

				for(i = eaSize(&peItemSetItemDefs) - 1; i >= 0; i--)
				{
					eaRemove(&peItemSetItemDefs, i);
					eaiRemove(&pActiveList, i);
				}

				eaDestroy(&peItemSetItemDefs);
				eaiDestroy(&pActiveList);
			}
		}
		...
		GetItemDescriptionItemSetBonus(&estrBonuses,lang,pItemSet,pItemDefSet,pEntity,iCount,pchPowerMessageKey,pchAttribModsMessageKey,pExtract);

		langFormatGameMessageKey(lang, pestrItemSetDesc, pchItemSetFormatKey,
			STRFMT_STRING("Name",NULL_TO_EMPTY(langTranslateDisplayMessage(lang,pItemDefSet->displayNameMsg))),
			STRFMT_STRING("Description",NULL_TO_EMPTY(langTranslateDisplayMessage(lang,pItemDefSet->descriptionMsg))),
			STRFMT_STRING("Description",NULL_TO_EMPTY(langTranslateDisplayMessage(lang,pItemDefSet->descShortMsg))),
			STRFMT_INT("Current",iCount),
			STRFMT_INT("Max",pItemDefSet->bItemSetMembersUseCategories ? iSlotCount : eaSize(&pItemDefSet->ppItemSetMembers)),
...
void GetItemDescriptionItemSet(char **pestrItemSetDesc, 
	...
	GameAccountDataExtract *pExtract)
{
	S32 i, j, iItemSetCount = eaSize(&pItemDef->ppItemSets);
	ItemDef **ppItemSetsChecked = NULL;
	S32 iCount = 0;
	...
	for (i = 0; i < iItemSetCount; i++)
	{
		...
		if (pItemSetDef && eaSize(&pItemSetDef->ppItemSetMembers) > 0)
		{
			...
			GetItemDescriptionFromItemSetDef(pestrItemSetDesc,lang,pItemSetDef,pEntity,pchPowerKey,pchModKey,pchItemSetFormatKey,pExtract);
			eaPush(&ppItemSetsChecked, pItemSetDef);
		}
	}

	if (pItem && pItem->pSpecialProps)
	{
		S32 iItemGemSlotCount = eaSize(&pItem->pSpecialProps->ppItemGemSlots);
		for (i = 0; i < iItemGemSlotCount; i++)
		{
			ItemDef *pGemItemDef = GET_REF(pItem->pSpecialProps->ppItemGemSlots[i]->hSlottedItem);
			if (pGemItemDef && eaSize(&pGemItemDef->ppItemSets) > 0)
			{
				iItemSetCount = eaSize(&pGemItemDef->ppItemSets);
				for (j = 0; j < iItemSetCount; j++)
				{
					ItemDef* pItemSetDef = GET_REF(pGemItemDef->ppItemSets[j]->hDef);
					if (pItemSetDef && eaFind(&ppItemSetsChecked, pItemSetDef) == -1 && eaSize(&pItemSetDef->ppItemSetMembers) > 0)
					{
						if (++iCount > 1)
							estrAppend2(pestrItemSetDesc,"\n");
						GetItemDescriptionFromItemSetDef(pestrItemSetDesc,lang,pItemSetDef,pEntity,pchPowerKey,pchModKey,pchItemSetFormatKey,pExtract);
	...
	for(i = eaSize(&ppItemSetsChecked) - 1; i >= 0; i--)
	{
		eaRemove(&ppItemSetsChecked, i);
	}
	eaDestroy(&ppItemSetsChecked);
}
