...
typedef struct ItemProgressionDef ItemProgressionDef;
...
typedef enum ItemType
{
	...
	kItemType_ProgressionFood,		// Items that store bonus XP pools for the Progression system.
	kItemType_Generic,					// It is truly amazing that it took us this long to add a generic item type.
	kItemType_None,
}ItemType;
...
typedef enum TradeErrorType
{
	...
	kTradeErrorType_CraftingItemInUse = 4,
} TradeErrorType;
...
AUTO_STRUCT;
typedef struct ItemIDContainer
{
	U64 uiID;			
}ItemIDContainer;
...
typedef struct SuperCritterPet
{
	...
	// iCurrentSkin was refactored to use -1 for "default" instead of 0.  There exists bad data, so don't read it directly
	// without being super duper careful. Otherwise use scp_GetPetCostumeDef().  Writing to it is less dangerous.
	const U8 iCurrentSkin;					AST(PERSIST SUBSCRIBE DEFAULT(-1))
...
{
	PlayerCostume* pCostumeData;
	const char* pcName;						AST(POOL_STRING KEY)
} CostumeCloneForItemCat;

AUTO_STRUCT;
typedef struct ItemCostumeClone
{
	REF_TO(CostumeCloneForItemCat) hCostumeRef;		AST(REFDICT(CostumeCloneForItemCat) NAME(Costume) STRUCTPARAM)
} ItemCostumeClone;
...
typedef struct RewardPackInfo
{
	...
	U32 bPromptRewardChoices : 1;		AST(NAME(PromptRewardChoices))
		// If the base reward table has "choice" options, and requires input from players

	U32 bCanScaleRewards : 1;			AST(NAME(CanScaleRewards))
		// If the reward pack can store it's scaled value, and apply that to the reward table it grants
} RewardPackInfo;
...
AUTO_STRUCT;
typedef struct NumericThresholdDef
{
	char *astrThresholdName;		AST(KEY STRUCTPARAM POOL_STRING)
		// Designer-specified threshold name

	bool bAlert : 1;				AST(DEFAULT(1))
		// Generate a warning alert when exceeded. Default is ON.
	bool bOnlyUGC : 1;
		// Only raise counters on UGC maps. Cooldown still applies everywhere, even when logged out.
	bool bOnlyKillCredit : 1;
		// Only raise counters on when resulting from a kill credit.
	bool bKillCreditLimiter : 1;
		// Exceeding this threshold stops kill credits from being given until the counter cools down below the threshold

	S32 *piThresholdPerLevel;		AST(NAME(ThresholdPerLevel))
		// The max amount of the numeric that the player can get in the specified time interval before we alert (per level)

	U32 *puDurationPerLevel;		AST(NAME(DurationPerLevel))
		// The time interval over which to apply this limit (in seconds) (per level)

	F32* pfCooldownRatePerLevel;	AST(NO_TEXT_SAVE)
		// The cooldown rate of the numeric counter used to track toward the limit (per level)
} NumericThresholdDef;
...
typedef struct ItemDef
{
	...
	U32 bItemSetMembersUseCategories : 1;
		// If this is true and this item is an item set,
		// then all items in the set are matched by item category rather than item def
	...
	U32 bTransmutationHasNoCost : 1;
		// If set this item costs nothing to use in transmutation. See itemeval_GetTransmutationCost

	U32 bTransmutationEnabled : 1; AST(DEFAULT(1))
		// If this is not set, the item cannot be used in transmutation, either as an appearance, or a stats base.

	U32 bProgressionEvoInsurance : 1;
		// If set on an UpgradeModifier item, will prevent catalysts from being lost on evo failure.

	enumLogCategory eLogCategory;	AST(NAME(LogCategory))
		// Only used if bLogForEconomy or bLogForTracking is true
	...
	const char* pcAddedCostumeFX;	AST(NAME(AddedCostumeFX) POOL_STRING)
		// An additional FX added to the item's costume that honors the Move/Clone bones as defined in ItemCategories, just like GemAddedCostumeFX.
	const char* pcAddedCostumeBone;	AST(NAME(AddedCostumeBone) POOL_STRING)
	...
	ItemCostumeClone **ppClonedCostumeRefsForClient;	AST(NO_TEXT_SAVE)
		//Populated at runtime by item_CreateItemCategoryCostumeClones(). Never looked up directly; just used to make sure the client has the appropriate costumes transmitted along with this def.

	NumericThresholdDef **eaNumericThresholdDefs;		AST(NAME("NumericThreshold") EDIT_ONLY)
		// Numeric alert thresholds

	const char* pchEventTooltipEvent;		AST(NAME(TooltipEvent) POOL_STRING)
	DisplayMessage eventTooltipMsg;			AST(STRUCT(parse_DisplayMessage) USERFLAG(TOK_USEROPTIONBIT_1))

	REF_TO(ItemProgressionDef) hProgressionDef; AST(REFDICT(ItemProgressionDef) NAME(ProgressionDef))

	U32 uProgressionFoodXP;

	U32 uProgressionEvoSuccessBonus;
...
typedef struct AlgoItemProps //Properties that are commonly algo-ized on items.
{
	...
	const U32 uProgressionXP;					AST(PERSIST SUBSCRIBE LOGIN_SUBSCRIBE)
	const U32 uProgressionLevel;				AST(PERSIST SUBSCRIBE LOGIN_SUBSCRIBE)
//	const U32 uBonusSuccessPercent;
} AlgoItemProps;
...
//	CONST_EARRAY_OF(InfuseSlot) ppInfuseSlots;	AST(PERSIST)
	...
	const F32 fRewardPackScale;										AST(PERSIST SUBSCRIBE DEFAULT(1.0f))
...
typedef struct Item
{
	...
	U32 bForceBind : 1;
	// not persisted; used by stores or reward tables to make any item bind-on-pickup to character, overriding the ItemDef behavior.
	// This flag also needs to be on the item so the UI can show that the item you're about to buy will bind on pickup
...
AUTO_STRUCT;
typedef struct ItemRewardPackRequest
{
	InventoryBag** eaRewardBags;	AST(NO_INDEX)
}ItemRewardPackRequest;
...
// Adding the ItemRewardPackType due to the ItemSalvage system using this 
// as a means to notify the client of what items were received from salvaging. 
// todo: this should be extended to a data defined enum; NW invocation uses reward packs and relies that hRewardPackItem is NULL
AUTO_ENUM;
typedef enum ItemRewardPackType
{
	kItemRewardPackType_RewardPack,	
	kItemRewardPackType_SalvagedItem,
	kItemRewardPackType_Undefined,
} ItemRewardPackType;

AUTO_STRUCT;
typedef struct ItemRewardPackTransferData
{
	REF_TO(ItemDef) hRewardPackItem;
	ItemQuality ePackResultQuality;
	InvRewardRequest* pRewards;
	ItemRewardPackType eRewardPackType;
} ItemRewardPackTransferData;
...
#define item_ItemPowerActive(pEnt, pBag, pItem, PowerIdx) item_ItemPowerActiveEx(pEnt, pBag, pItem, PowerIdx, 0)
...
void item_GetNormalizedDescCustom(char** pestrResult, Language lang, Entity* pEnt, Item *pItem, ItemDef* pItemDef,
