...
//where it's stored
AUTO_ENUM;
typedef enum EMailV3SenderType
{
	kEmailV3Type_Player,
	kEmailV3Type_NPC,
...
//what's in it
AUTO_ENUM;
typedef enum EMailV3ContentType
{
	kEmailV3Contents_Default = 0,
	kEmailV3Contents_AuctionSale,
	kEmailV3Contents_AuctionOutbid,
	kEmailV3Contents_AuctionExpired,
	kEmailV3Contents_ReturnToSender,
} EMailV3ContentType;
...
typedef struct EmailV3Message
{
	...
	const EMailV3SenderType	eSenderType;	AST(PERSIST SUBSCRIBE NAME(TypeOfEmail))
	const EMailV3ContentType eContentType;	AST(PERSIST SUBSCRIBE)
...
typedef struct EmailV3
{
	...
	const int iMessageCount;						AST(PERSIST SUBSCRIBE DEFAULT(-1))
	const int iAttachmentsCount;					AST(PERSIST SUBSCRIBE DEFAULT(-1))
} EmailV3;
...
typedef struct EmailV3UIMessage
{
	...
	U32 uID;
	U32 uLotID;
	EMailV3SenderType eSenderType;
	EMailV3ContentType eContentType;
