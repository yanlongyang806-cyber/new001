...
void initPowerAnimFX(void)
{
	...
	ParserSetTableInfo(parse_PowerFXParam_StructParam, sizeof(PowerFXParam), "PowerFXParam_StructParam", NULL, __FILE__, SETTABLEINFO_ALLOW_CRC_CACHING);
...
// Starts any override to the air friction multiplier 
void character_AirFrictionOverrideStart(Character *pchar, PowerActivation *pact)
{
#if !(GAMESERVER || GAMECLIENT)
	assert(0);
#else
	...
	PowerAnimFX * pFX = pdef ? GET_REF(pdef->hFX) : NULL;
 	...
 	if (pFX && pFX->fAirFrictionMultOverride > 0)
	{
		U32 uiTimeStart = (!pFX || !pFX->pLunge) ? pact->uiTimestampActivate : pact->uiTimestampCurrented;

		mrSurfaceAirFrictionMultOverrideStart(	pchar->pEntParent->mm.mrSurface, 
												pact->uchID,
												pFX->fAirFrictionMultOverride,
												uiTimeStart);
	}
#endif
}
...
void character_AirFrictionOverrideStop(Character *pchar, PowerActivation *pact, U32 uiTimeStop)
{
#if !(GAMESERVER || GAMECLIENT)
	assert(0);
#else
	...
	if (pFX && pFX->fAirFrictionMultOverride > 0)
	{
		...
		mrSurfaceAirFrictionMultOverrideStop(	pchar->pEntParent->mm.mrSurface, pact->uchID, uiTimeStart);
	}
#endif
}
...
static ExprContext *ParamBlockContext(void)
{
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("PowerAnim_ParamBlock");
		...
		for (iItemIdx = 0; iItemIdx < eaSize(&pBag->ppIndexedInventorySlots); iItemIdx++)
		{
			...
			if(pSlot->pItem && pSlot->pItem->id == (U64)iItemID)
			{
				...
				if(pBagDef->pFXNodeName)
				{
					...
					if (bTargetNode)
					{
						estrAppend2(pestrBuffer, "T_");
					}

					estrAppend2(pestrBuffer,pBagDef->pFXNodeName);
					(*piItemIdx) = iItemIdx;
					return true;
				}
			}
										...
										char** pestrBuffer)
{
	if(pestrBuffer && *pestrBuffer)
	{
		...
		if((pchWildcardSuffix && strcmp(pchWildcardSuffix,"")!=0)
			...
			|| (bAddZSuffix && pTarget)
			|| (bAddXSuffix && pTarget)
			|| (pchLeftRightSuffix && strcmp(pchLeftRightSuffix, "")!=0))
		{
			...
			if(pchLeftRightSuffix)
			{
				estrAppend2(pestrBuffer,pchLeftRightSuffix);
			}
		}
	}
}

static const char *PowerFXNodeInternal(ExprContext* pContext, 
									   const char* pchWildcardSuffix, 
									   const char* pchLeftRightSuffix,
									   bool bIncludeSlotNumber, 
									   bool bAddZSuffix, 
									   ...
									   bool bTargetNode)
{
	...
	if (iItemID)
	{
		character_GetPowerFXNodeNameFromItemID(pchar, iItemID, &estrBuffer, &iItemIdx, bTargetNode);
...
AUTO_EXPR_FUNC(powerart);
const char *PowerFXNode(ExprContext* pContext, const char* pchWildcardSuffix, bool bIncludeSlotNumber, bool bAddZSuffix, bool bAddXSuffix)
{
	return PowerFXNodeInternal(pContext, pchWildcardSuffix, NULL, bIncludeSlotNumber, bAddZSuffix, bAddXSuffix, false);
}

// Get a power FX node from an item power
AUTO_EXPR_FUNC(powerart);
const char *PowerFXNodeTarget(ExprContext* pContext, const char* pchWildcardSuffix, bool bIncludeSlotNumber, bool bAddZSuffix, bool bAddXSuffix)
{
	return PowerFXNodeInternal(pContext, pchWildcardSuffix, NULL, bIncludeSlotNumber, bAddZSuffix, bAddXSuffix, true);
}

// Get a power FX node from an item power
AUTO_EXPR_FUNC(powerart);
const char *PowerFXNodePaired(ExprContext* pContext, const char* pchWildcardSuffix, bool bIncludeSlotNumber, bool bAddZSuffix, bool bRightPair)
{
	return PowerFXNodeInternal(pContext, pchWildcardSuffix, bRightPair ? "R" : "L", bIncludeSlotNumber, bAddZSuffix, false, false);
}

// Get a power FX node from an item power
AUTO_EXPR_FUNC(powerart);
const char *PowerFXNodeTargetPaired(ExprContext* pContext, const char* pchWildcardSuffix, bool bIncludeSlotNumber, bool bAddZSuffix, bool bRightPair)
{
	return PowerFXNodeInternal(pContext, pchWildcardSuffix, bRightPair ? "R" : "L", bIncludeSlotNumber, bAddZSuffix, false, true);
}
...
bool SourceItemEquippedInSlot(ExprContext* pContext, S32 iSlot)
{
	...
	if (pchar && pchar->pEntParent)
	{
		...
		if (pDef)
		{
			...
			character_WeaponPick(pchar, pDef, &ppItems, pExtract);
			FOR_EACH_IN_EARRAY(ppItems, Item, pItem)
			{
				bagiterator_Reset(pIter);

				inv_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity,pchar->pEntParent), pItem->id, pIter);
...
S32 SourceAllegiance(ExprContext *pContext,const char *pchAllegiance)
{
	...
	// If they have a sub allegiance, that sub allegiance is their only source allegiance
	if (psubdef)
		return (!stricmp_safe(psubdef->pcName,pchAllegiance));
	return (pdef && !stricmp_safe(pdef->pcName,pchAllegiance));
}
...
S32 SourceFaction(ExprContext *pContext, const char *pchFaction)
{
	...
	// If they have a sub faction, that sub faction is their only source faction
	if (pSubFaction)
		return (!stricmp_safe(pSubFaction->pchName,pchFaction));
	return (pFaction && !stricmp_safe(pFaction->pchName,pchFaction));
}
...
void character_AnimFXChargeCancel(int iPartitionIdx, Character *pchar, PowerActivation *pact, bool bInterruped)
{
	...
	character_AirFrictionOverrideStop(pchar, pact, 0);
#endif
}
	...
	if(pafx)
	{
		...
		if(pact && !bPeriodic)
		{
			...
			if(g_CombatConfig.tactical.bTacticalDisableDuringPowers)
			{
				...
				if(pdef && pdef->eType != kPowerType_Instant)
				{
		...
		for(i=-1; i<s; i++)
		{
			...
			if(pafx)
			{
				...
				if(!bKeepAnimsAndPenalties && i<0)
				{
					...
					character_AirFrictionOverrideStop(pchar, pact, uiTime);
						 ...
						 U32 *bAppliedHitPause)
{
	...
	if(papp->pafx && verify(papp->uiActSubID<(1<<PAFX_HITFX_SHIFT)))
	{
		...
		for(i=-1; i<s; i++)
		{
			switch (pafx->hitFXSourceRedirect)
			{
				default:
				case kPowerFXEntityRedirect_None:
				{
					pFXSourceChar = papp->pcharSource;
					copyVec3(papp->vecSourcePos, vFXSource);
				} break;

				case kPowerFXEntityRedirect_ApplicationTarget:
				{
					Entity *pTargetEnt = entFromEntityRef(iPartitionIdx, papp->erTarget);
					if (!pTargetEnt || !pTargetEnt->pChar)
						return;
					
					pFXSourceChar = pTargetEnt->pChar;
					copyVec3(papp->vecTarget, vFXSource);
				} break;
			}
			...
			if(papp->bPrimaryTarget || !pafx->bMainTargetOnly)
			{
				...
				for(j=eaiSize(&piFramesBeforeHit)-1; j>=0; j--)
				{
					...
					if(bBits)
					{
						...
						if (gConf.bNewAnimationSystem && !bTrigger)
						{
									...
									pFXSourceChar->pEntParent)
							{
								...
								entGetFaceSpaceQuat(pFXSourceChar->pEntParent, qSourceFS);
					...
					character_FlashFXEx(iPartitionIdx,pcharTarget,vecTargetHit,papp->uiActID,papp->uiActSubID|(j<<PAFX_HITFX_SHIFT),
							kPowerAnimFXType_HitFlash,pFXSourceChar,vFXSource,papp,papp->pact,papp->ppow,
					...
					if(0)
					{
						...
						character_FlashHitReact(iPartitionIdx,pcharTarget,papp->uiActID,papp->uiActSubID|(j<<PAFX_HITFX_SHIFT),
												papp->pcharSource,papp->pact,(bBits?pafx->ppchHitBits:NULL),
												pafx->ppchHitFX,pafx->ppHitFXParams,fHue,uiTimeDead,bGlobal);
					}
				}
				...
				if(!papp->uiActSubID && papp->pact)
				{
					...
					character_StickyFXOn(iPartitionIdx,pcharTarget,papp->uiActID,papp->uiActSubID,kPowerAnimFXType_HitSticky,
									pFXSourceChar,NULL,papp,papp->pact,papp->ppow,pafx->ppchHitStickyFX,
...
static int ValidateAndGenerateAnimFX(PowerAnimFX *pafx)
{
	...
	if(pafx->fAirFrictionMultOverride < 0)
	{
		ErrorFilenamef(pafx->cpchFile,"If you specify a AirFrictionMultOverride, it must be greater than 0");
		bRet = false;
	}
