...
static void EvalCritChance(int iPartitionIdx, Character *pchar, Power *ppow, PowerDef *pPowerDef, PowerApplication *papp, Character *pcharTarget, F32 fHitChanceOverflow)
{
	...
	if (!pPowerDef && ppow)
	{
		pPowerDef = ppow ? GET_REF(ppow->hDef) : NULL;
	}
	...
	if(pPowerDef
		&& (pPowerDef->eType==kPowerType_Passive 
			|| (-1!=s_iNoDefaultCrit
				&& -1!=eaiFind(&pPowerDef->tags.piTags,s_iNoDefaultCrit))))
	{
		papp->critical.fThreshold = -1;
	}
	else
	{
		// if the character has a pSavedCritical to use, just copy that and early out
		if (pchar->pSavedCritical != NULL) 
		{
			StructCopy(parse_PACritical, pchar->pSavedCritical, &papp->critical, 0,0,0);
			return;
		}

		if (pPowerDef->eCriticalChanceAttrib != -1 && pPowerDef->eCriticalSeverityAttrib != -1)
		{
			eCritChanceAttrib = pPowerDef->eCriticalChanceAttrib;
			eCritSeverityAttrib = pPowerDef->eCriticalSeverityAttrib;
		}
...
static void UpdateCombatTimer(int iPartitionIdx, Character *pcharSource, Character *pcharTarget, PowerDef *pdef)
{
	...
	if(pdef->eType!=kPowerType_Passive && pcharSource!=pcharTarget)
	{
		if(pcharTarget==NULL)
		{
			...
			{
				if (pcharSource->pEntParent->pPlayer == NULL || !g_CombatConfig.pTimer->bPlayersIgnoreTimerUse)
				{
		...
		else
		{
			...
			if(character_TargetIsFoe(iPartitionIdx,pcharSource,pcharTarget))
			{
				...
				if(pcharSource && pcharTarget)
				{
					...
					// TODO - There are things other than projectiles that should put their creators into combat. (Everything?) [RMARR - 6/3/13]
...
// Returns the XZ-plane 2D angle from pent's facing to the source position
F32 EntityAngleToSourcePosUtil(Entity *pent, const Vec3 vSourcePos)
{
	F32 fAngle = 0;

	if(!ISZEROVEC3(vSourcePos))
	{
		Vec3 vecCurPos, vecCurDir, vecToSource, vecNormal, vecNormalScaled, vecToSourceProjection, vecCross;
		Mat4 mEntMat;
		F32 fLength;
		F32 fCosAngle;
		...
		copyVec3(mEntMat[3], vecCurPos);

		if (sameVec3(vSourcePos, vecCurPos))
		{
			return fAngle;
		}

		// Calculate the vector from the entity to the source of the mod
		subVec3(vSourcePos, vecCurPos, vecToSource);

		// Find the normal of the plane described by the ship's orientation
		copyVec3(mEntMat[1], vecNormal);
		normalVec3(vecNormal);

		// Project the vector to the source onto this plane
		scaleVec3(vecNormal, dotVec3(vecToSource, vecNormal), vecNormalScaled);
		subVec3(vecToSource, vecNormalScaled, vecToSourceProjection);

		// Find the angle between the projected vector and the entity's facing
		copyVec3(mEntMat[2], vecCurDir);
		fLength = lengthVec3(vecCurDir) * lengthVec3(vecToSourceProjection);
		if (fLength <= FLT_EPSILON)
		{
			return fAngle;
		}
        fCosAngle = CLAMP(dotVec3(vecCurDir, vecToSourceProjection) / fLength, -1.f, 1.f);
		fAngle = acos(fCosAngle);

		// Now use the cross product to find which half of the circle we are using
		crossVec3(vecCurDir, vecToSourceProjection, vecCross);
		if (dotVec3(vecCross, vecNormal) < 0)
		{
			fAngle *= -1;
		}
	}
	...
	switch(pdef->eEffectArea)
	{
		...
		case kEffectArea_Cylinder:
			PERFINFO_AUTO_START("Cylinder",1);
			{
				...
				if (pdef->bEffectAreaIgnoreSourceCapsule || papp->bOffsetEffectArea)
				{
			...
			PERFINFO_AUTO_START("Cone",1);
			{
				...
				for(i=eaSize(&s_ppEnts)-1; i>=0; i--)
				{
					...
					if(pEnt->pChar && CharacterIsLoaded(pEnt->pChar) && entity_CanAffect(iPartitionIdx, pentSource, pEnt))
					{
						...
						if (pdef->bEffectAreaCentered)
							bInRange = (fDistPow >= entGetDistance(pentSourceDistance, papp->vecSourcePos, pEnt, vEntPos, NULL));
						else
		...
		case kEffectArea_Sphere:
			...
			{
				WorldInteractionNode **ppObjects=NULL;
				F32 fRadius = 0, fMinRadius = 0, fDistPow = 0, fDistPowMin = 0, fSourceCollRadius = 0;
				Entity *pentSourceDistance = pentTarget; 	// This is different from cones and cylinders. Cones and cylinders are positioned 
															// from source to target, but sphere positions are based only on the target
				F32 fQueryRadius = 0.f;
				...
				// TODO(JW): if(fDistPow+fSourceCollRadius>500) Errorf("Area of %s is too large, with not hit all potential targets.  Please see Jered.",papp->pdef->pchName);
				fQueryRadius = fDistPow + fSourceCollRadius + s_fAdditionalQueryDist;
				GetApplyTargetsEntEArray(	papp->iPartitionIdx,
											vecTarget,
											&s_ppEnts,
											fQueryRadius,
				...
				for(i=eaSize(&s_ppEnts)-1; i>=0; i--)
				{
					...
					if(pEnt->pChar && CharacterIsLoaded(pEnt->pChar) && entity_CanAffect(papp->iPartitionIdx, pentSource, pEnt))
					{
						...
						if (pdef->bEffectAreaCentered)
							fDistTarget = entGetDistance(pentSourceDistance, vecTarget, pEnt, vEntPos, NULL);
						else
...
#ifdef GAMESERVER
				//Evaluate the critical chance numbers
				if(papp->ppow || (papp->pcharSource && papp->pcharSource->pSavedCritical))
				{
					EvalCritChance(iPartitionIdx, papp->pcharSource, papp->ppow, papp->pdef, papp, pcharTarget, fHitChanceOverflow);
				}
										  ...
										  GameAccountDataExtract *pExtract)
{
	...
	for(i=0; i<iNumPowers; i++)
	{
		...
		if (pPowerDef)
		{
			...
			// if the level of the item is 0, use the character's combat level
			if (applyParams.iLevel <= 0)
			{
...
{
	if(pchar->bModsOwnedByOwner && pchar->pEntParent->erOwner && entFromEntityRef(iPartitionIdx, pchar->pEntParent->erOwner))
	{
		return pchar->pEntParent->erOwner;
	}
...
#define SetQueueDebugFlag(i) iQueueDebugFlags |= (1<<(i))
	...
	if (pafx)
	{
		...
		if (fTimeRemainInQueue > 0)
		{
			uiTimeActivate = pmTimestampFrom(uiTimeEnterStance, fTimeRemainInQueue);
		}
...
// helper function to find if the given toggle power is currently being activated
// assumes given power is a toggle type
static PowerActivation* character_FindActivatedTogglePower(	SA_PARAM_NN_VALID Character *pChar, 
															SA_PARAM_NN_VALID Power *pTogglePower, S32 *bIsActive)
{
	int i;
	PowerRef ref = {0};
	...
	*bIsActive = false;
	power_GetIDAndSubIdx(pTogglePower, &ref.uiID, &ref.iIdxSub, &ref.iLinkedSub);

	// Find it in list of active toggles
	for(i=eaSize(&pChar->ppPowerActToggle)-1; i>=0; i--)
	{
		// we only want to test the main ID and linkedID, and not the ref's iIdxSub
		// this is because we just want to know if it is the main parent being toggled
		if(ref.uiID == pChar->ppPowerActToggle[i]->ref.uiID && ref.iLinkedSub == pChar->ppPowerActToggle[i]->ref.iLinkedSub)
		{
			*bIsActive = true;
			return pChar->ppPowerActToggle[i];
		}
	}

	// Didn't find it in active list, check for current/queued activations
	if(pChar->pPowActCurrent
		&& (ref.uiID == pChar->pPowActCurrent->ref.uiID && ref.iLinkedSub == pChar->pPowActCurrent->ref.iLinkedSub)
		&& poweract_IsToggle(pChar->pPowActCurrent))
	{
		return pChar->pPowActCurrent;
	}
	else if(pChar->pPowActQueued
		&& (ref.uiID == pChar->pPowActQueued->ref.uiID && ref.iLinkedSub == pChar->pPowActQueued->ref.iLinkedSub)
		&& poweract_IsToggle(pChar->pPowActQueued))
	{
		return pChar->pPowActQueued;
	}
	else if(pChar->pPowActOverflow
		&& (ref.uiID == pChar->pPowActOverflow->ref.uiID && ref.iLinkedSub == pChar->pPowActCurrent->ref.iLinkedSub)
		&& poweract_IsToggle(pChar->pPowActOverflow))
	{
		return pChar->pPowActOverflow;
	}
	...
	if(pActReq->bActivate)
	{
		...
		if(ppow)
		{
			...
			if(pdef->eType==kPowerType_Toggle || (pdef->bComboToggle && !pdef->bCheckComboBeforeToggle))
			{
				...
				pactToggle = character_FindActivatedTogglePower(pchar, ppow, &bToggleActive );
			...
			if(ppowQueue && GET_REF(ppowQueue->hDef))
			{
				...
				if (!ppow || !pdef)
				{
					PowersDebugPrintEnt(EPowerDebugFlags_ACTIVATE, pchar->pEntParent,"Activate %d: FAIL: Power not found\n",pActReq->uchActID);
					if(!eFail)
					{
				...
				else
				{
					...
					character_AirFrictionOverrideStart(pchar, pact);
														...
														SA_PARAM_OP_VALID Entity **ppProxAssistTargetOut)
{
	...
	WorldRegionType eRegionType = entGetWorldRegionTypeOfEnt(ent);
	bool bIgnorePitch = false;
	F32 fRange;

	if (!ent->pPlayer ||
		(!ent->pPlayer->bUseFacingPitch && eRegionType != WRT_Space && eRegionType != WRT_SectorSpace) ||
		powerdef_ignorePitch(pdef))
	{
		bIgnorePitch = true;
	}
...
#ifdef GAMECLIENT
	//Predict a dismount
	gclCharacter_ForceDismount(pchar, pdef);
#endif
		...
		if(ppow)
		{
			...
			if(ppowQueue && GET_REF(ppowQueue->hDef))
			{
				...
				else if (vecTargetPrimary && !character_PowerRequiresValidTarget(pchar, pdef))
				{
					...
					(g_CombatConfig.autoAttack.bDeactivateSelfTargetedMaintainsCancelOnTargetSwitch || pdef->eType != kPowerType_Maintained || !ppowtarget->bRequireSelf))
				{
...
S32 character_ActivatePowerServerBasic(int iPartitionIdx, Character *pchar, Power *ppow, Entity *eTarget, 
										const Vec3 vecTarget, S32 bStart, S32 bCancelExisting, GameAccountDataExtract *pExtract)
{
#if !GAMESERVER && !GAMECLIENT
	assert(0);
#else
