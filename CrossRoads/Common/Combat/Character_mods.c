...
S32 entity_AccrueModsInnate(int iPartitionIdx, Entity *pent)
{
	...
	if(s_pSet->CharacterAttribs.attrBasicAbs.fFlight > 0)
	{
		...
		pmSetFlightBankRate(pent, MAX(0,s_pSet->CharacterAttribs.attrBasicAbs.fBankRateFlying));
		pmSetFlightMaxBankAngle(pent, MAX(0,RAD(s_pSet->CharacterAttribs.attrBasicAbs.fBankMaxAngleFlying)));
	}
	else
	{
...
void character_AccrueModsEx(int iPartitionIdx, Character *pchar, F32 fRate, GameAccountDataExtract *pExtract, bool bBootstrapping, U32 uiTimeLoggedOut)
{
	...
	if(!eaSize(&pchar->modArray.ppMods)
		...
		// todo RP: this is getting a little big, some of these we probably don't care about for every game
...
*/
		...
		// process PowerMode attribs first since the other attribs can depend on powerModes that come from attribs
		for(i=s-1; i>=0; i--)
		{
			AttribMod *pmod = pchar->modArray.ppMods[i];
			AttribType eAttrib = pmod->pDef->offAttrib;
			if(eAttrib==kAttribType_PowerMode)
			{
		...
		for(i=s-1; i>=0; i--)
		{
			...
			AttribType eAttrib = pmod->pDef->offAttrib;
			if(eAttrib!=kAttribType_PowerMode && 
				(eAttrib==kAttribType_DamageTrigger
