...
AUTO_STRUCT;
typedef struct PowerConditionalTags
{
	S32 *piTags;	AST(STRUCTPARAM, SUBTABLE(PowerTagsEnum))
		// The EArray of Tags.
		// this may not have all the actual tags of the attribMod due to other 
		//  attribMods containing the tag without requirements
	
	S32 iAttribIndex;
		// the attribIndex this corresponds to

} PowerConditionalTags;
...
typedef struct PowerDef
{
	...
	PowerConditionalTags **eaConditionalTags;	AST(NO_TEXT_SAVE)
		// conditional tags the power has, attrib mods that have tags with pExprRequires
	...
	Expression *pExprHideAutoDesc;			AST(NAME(ExprHideAutoDesc), LATEBIND)
		// If this expression evaluates to true, do not display the auto desc of this power
	...
	EnhancementAttachUnownedType eEnhancementAttachUnowned : EnhancementAttachUnownedType_NUMBITS; AST(SUBTABLE(EnhancementAttachUnownedTypeEnum) DEFAULT(2))
	...
	U32 bDeactivationLeavesMods : 1;		AST(SERVER_ONLY)
		...
		//  Mutually exclusive with bDeactivationDisablesMods.
	...
	U32 bDeactivationDisablesMods : 1;		AST(SERVER_ONLY)
		...
		//  Valid only on periodic powers.  Mutually exclusive with bDeactivationLeavesMods.
	...
	U32 bEffectAreaIgnoreSourceCapsule : 1;
		// If set, Area of Effect powers will not use the source capsule when computing distances to check intersections
		// valid for Cylinders, Cones and Spheres. 
	...
	U32 bNoDeactivateOnDeath : 1;
		// if set, this toggle cannot be deactivated while dead
	...
	U32 bDeactivatesPowerOnDisable : 1;	AST(DEFAULT(1))
		// if set, the disable mod will deactivate the power. Currently only valid for toggle powers 
...
void character_PowerSetRechargeByID(int iPartitionIdx, SA_PARAM_NN_VALID Character *pchar, U32 uiID, F32 fTime);

// Utility function for setting a specific recharge on a specific Power with proper notification to the client.
void character_PowerSetRecharge(int iPartitionIdx, SA_PARAM_NN_VALID Character *pchar, SA_PARAM_NN_VALID Power *pPower, F32 fTime);
...
void power_GetIDAndSubIdx(	SA_PARAM_OP_VALID Power *ppow, 
							SA_PARAM_NN_VALID U32 *puiIDOut, 
							SA_PARAM_NN_VALID int *piSubIdxOut, 
							SA_PARAM_NN_VALID S16 *piLinkedSubIdxOut);
