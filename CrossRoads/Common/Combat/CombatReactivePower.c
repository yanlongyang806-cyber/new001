...
// --------------------------------------------------------------------------------------------------------------------
bool CombatReactivePower_HasReactivePower(Character *pChar)
{
	return pChar->pCombatReactivePowerInfo != NULL;
}
...
static S32 _CanCancelPowerActivation(Character *pChar, CombatReactivePowerDef *pDef, PowerActivation *pAct)
{
	...
	{	// allow canceling at any point in the charge
		if (pPowerDef->bDoNotAllowCancelBeforeHitFrame)
			return false;
	...
	if (pAct->eActivationStage == kPowerActivationStage_Activate)
	{
		...
		F32 fHitTime = 0.f;
		F32 fHitTimeAdjust = 0.2f;
		F32 fTimeOverride = 0.f;
		if (!pPowerDef)
			return false;
		...
		if (!pAnimFX->pLunge && !pPowerDef->bHasPredictedMods &&
			...
			!pPowerDef->bDoNotAllowCancelBeforeHitFrame)
			return true;

		// checking if we've passed the hitframe, on the client wait a little longer, and on the server allow some room for error
		// 
#if GAMECLIENT  
		fHitTimeAdjust = 0.25f;
#else  
		fHitTimeAdjust = 0.1f;
#endif
				
		if (!pPowerDef->fTimeOverrideReactivePower && pAct->fTimeActivating >= fHitTime + fHitTimeAdjust)
			return true;
		
		fTimeOverride = pPowerDef->fTimeOverrideReactivePower;
#if GAMESERVER
		fTimeOverride += 0.15f;
#endif 

		if (pAct->fTimeActivating > fHitTime && fTimeOverride && 
			pChar->pPowActCurrent->fTimerActivate < fTimeOverride)
			return true;
	}
...
{
	F32 fCooldownPredictionOffset = 0.f; 
...
#if GAMESERVER 
	fCooldownPredictionOffset = 0.1f;
#endif

	if (pInfo->fCooldown > fCooldownPredictionOffset)
	{
		return false;
	}
...
void CombatReactivePower_InitCharacter(Character *pChar, const char *pszDef)
{
	if (pszDef)
	{
		...
		if (pDef)
		{
			...
			pChar->pCombatReactivePowerInfo->bIsRoll = (pDef->pRoll != NULL);
												...
												bool bCancelSpeeds)
{
	...
	{
		mrSurfaceSetStrafingOverride(pChar->pEntParent->mm.mrSurface, false, false, uiDeactivateTime);
	}
	...
	{	// we should already have the roll direction 
		...
		if (pDef->pRoll->fRollSpeed)
		{
			fRollSeconds =	pDef->pRoll->fRollDistance/pDef->pRoll->fRollSpeed + 
							(F32)pDef->pRoll->iRollAccelNumberOfFrames / MM_STEPS_PER_SECOND + 
							(F32)pDef->pRoll->iRollDecelNumberOfFrames / MM_STEPS_PER_SECOND +
							(F32)pDef->pRoll->iRollFrameStart/MM_STEPS_PER_SECOND;
							// don't factor in the pRoll->fRollPostHoldSeconds, 
							// that should be handled by the CombatReactivePowerDef's DeactivateTime

			// on the client, add in a small amount of time 
#if GAMECLIENT
			fRollSeconds += 0.15f;
#endif
		}
...
#if GAMESERVER
					if (!pDef->pRoll && pInfo->uiQueuedDeactivateTime && uiTime >= pInfo->uiQueuedDeactivateTime)
					{
								...
								F32 fTimeOffset)
{
...
#if GAMESERVER
	gslCharacter_CancelMountPower(pChar);
#endif
	...
	if (pDef->bEnableStrafing)
	{
		mrSurfaceSetStrafingOverride(pChar->pEntParent->mm.mrSurface, true, true, uiStartTime);
	}
	...
	if (bImmediate || pInfo->eState == ECombatReactivePowerState_ACTIVATED)
	{
		switch (pInfo->eState)
		{
			...
			case ECombatReactivePowerState_DEACTIVATING:
				...
				pInfo->eState == ECombatReactivePowerState_PREACTIVATE)
	{
		pInfo->uiQueuedDeactivateTime = pmTimestampFrom(uiStartTime, 0.1f);
		character_Wake(pChar);
	}
...
{
	// this expression is getting pretty complex, if anything else needs to be added this should really be refactored
	if (pChar->pCombatReactivePowerInfo && 
		( (pChar->pCombatReactivePowerInfo->uiQueuedActivateTime != 0) ||
		  ( (pChar->pCombatReactivePowerInfo->uiQueuedDeactivateTime == 0 || pChar->pCombatReactivePowerInfo->bIsRoll) && 
