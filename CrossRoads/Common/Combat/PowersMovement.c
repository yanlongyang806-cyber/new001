												...
												PMMove *pMove)
{
	...
	else
	{
		...
		else if(pMove->type == kPowerMoveType_Lurch)
		{
			...
			if (pMove->pchAnimGraphName)
			{
					...
					pGraph->pPowerMovementInfo->pDefaultMovement)
				{
					...
					if (iFrameCount > 0)
					{
						...
						fTestForGroundLength = len = normalizeCopyVec3(vLurchDir, pMove->vMoveDir);
					}
				}
			}
			else
			{
				F32 fDeltaMove = pMove->fSpeed * MM_SECONDS_PER_STEP;
				fTestForGroundLength = fDeltaMove;
				len = pMove->fSpeed; // I think this might be a bug and should be set to fDeltaMove
				...
				(g_CombatConfig.lurch.bStopOnLedges || g_CombatConfig.lurch.bDisableInAir))
			{
				...
				if (g_CombatConfig.lurch.bStopOnLedges && 
					onGround &&
					!mrmCheckGroundAheadBG(msg, pos, pMove->vMoveDir, fTestForGroundLength)) {
					pMove->stopLurching = true;
				}
													...
													PowersMovementLocalBG* localBG)
{
	...
	if(bg->moving)
	{
		...
		else if (SAFE_MEMBER(pMove,stopLurching))
		{
			mrmTargetSetSpeedAsConstantBG(msg, 0.f);
		}
	}
...
void pmUpdateFlightMaxPitch(Entity *e)
{
	if (e->pPlayer && e->pPlayer->pUI)
	{
		ControlSchemes *pControlSchemes = e->pPlayer->pUI->pSchemes;
		ControlScheme *pCurrentControlScheme = schemes_FindScheme(pControlSchemes, pControlSchemes->pchCurrent);
		pmSetFlightMaxPitchAngle(e, pCurrentControlScheme->fFlightMaxPitchAngle);
	}
}
...
void pmInitializeFlightParams(Entity *e)
{
	...
	PlayerCostume *pCostume = costumeEntity_GetEffectiveCostume(e);
	PCSkeletonDef *pSkelDef = pCostume ? GET_REF(pCostume->hSkeleton) : NULL;
	PCFlightControls *pFlightControls = SAFE_MEMBER2(pCostume, pArtistData, pFlightControls);
	AIConfig* config;
	...
	if(e->pPlayer)
	{
		...
		if(e->pChar)
		{
			if((pFlightControls && pFlightControls->bDisableBanking) ||
				(pSkelDef && pSkelDef->bDisableBanking))
			{
				bFakeRoll = false;
			}
		}
		mrFlightSetFakeRoll(e->mm.mrFlight,bFakeRoll);
	}
	...
	if(pFlightControls)
	{
		pmSetFlightBankRate(e, MAX(0, pFlightControls->fBankRateFlying <= 0 && pSkelDef ? pSkelDef->fBankRateFlying : pFlightControls->fBankRateFlying));
		pmSetFlightMaxBankAngle(e, MAX(0, pFlightControls->fBankMaxAngleFlying <= 0 && pSkelDef ? pSkelDef->fBankMaxAngleFlying : pFlightControls->fBankMaxAngleFlying));
	}
...
// Sets flight bank rate
void pmSetFlightBankRate(Entity *e, F32 f)
{
	mrFlightSetBankRate(e->mm.mrFlight, f);
}

// Sets flight max bank
void pmSetFlightMaxBankAngle(Entity *e, F32 f)
{
	mrFlightSetMaxBankAngle(e->mm.mrFlight, f);
}

// Sets flight max pitch
void pmSetFlightMaxPitchAngle(Entity *e, F32 f)
{
	mrFlightSetMaxPitchAngle(e->mm.mrFlight, f);
}
...
void character_UpdateMovement(Character *pchar, CharacterAttribs *pAttribsOld)
{
	...
	if(pchar->pattrBasic->fFlight>0)
	{
		...
		else if(pchar->bUpdateFlightParams)
		{
			...
			pmUpdateFlightMaxPitch(pchar->pEntParent);
		}
		...
		if(bFlightChanged || pAttribsOld->fBankRateFlying != pchar->pattrBasic->fBankRateFlying)
		{
			F32 f = MAX(0, pchar->pattrBasic->fBankRateFlying);

			if (!f)
			{
				PlayerCostume *pCostume = costumeEntity_GetEffectiveCostume(pchar->pEntParent);
				if(pCostume)
				{
					...
					if(pFlightControls)
					{
						f = MAX(0, pFlightControls->fBankRateFlying);
					}

					if(pSkelDef && !f)
					{
						f = MAX(0, pSkelDef->fBankRateFlying);
					}
				}
			}

			pmSetFlightBankRate(pchar->pEntParent, MAX(0, f));
		}
		if(bFlightChanged || pAttribsOld->fBankMaxAngleFlying != pchar->pattrBasic->fBankMaxAngleFlying)
		{
			F32 f = MAX(0, pchar->pattrBasic->fBankMaxAngleFlying);
			...
			if (!f)
			{
				...
				if(pCostume)
				{
					...
					if(pFlightControls)
					{
						f = MAX(0, pFlightControls->fBankMaxAngleFlying);
					}
					
					if(pSkelDef && !f)
					{
						f = MAX(0, pSkelDef->fBankMaxAngleFlying);
					}
				}
			}

			pmSetFlightMaxBankAngle(pchar->pEntParent, MAX(0, RAD(f)));
		}
		if(bFlightChanged
			|| pAttribsOld->fGravityJumpingUp != pchar->pattrBasic->fGravityJumpingUp 
			|| pAttribsOld->fGravityJumpingDown != pchar->pattrBasic->fGravityJumpingDown)
