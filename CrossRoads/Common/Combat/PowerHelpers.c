...
static S32 PowerIDsResetCheck(U32 uiSourceMask, PowerIDList *pResetIDs, U32 uiID)
{
	if(pResetIDs)
		return eaiFind(&pResetIDs->eaiPowerIDs, uiID) != -1;
...
AUTO_TRANS_HELPER;
S32 entity_ResetPowerIDsHelper(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, U32 uiSourceMask, 
								GameAccountDataExtract *pExtract, bool bJustCount, PowerIDList *pResetIDs)
{
	int i,j,k;
	S32 iReset = 0;
	...
	if (pResetIDs && eaiSize(&pResetIDs->eaiPowerIDs) == 0)
	{
		pResetIDs = NULL;
	}

	if(uiSourceMask & ((1<<POWERID_BASE_BITS)-1) && !pResetIDs)
	{
	...
	{
		if(bIgnoreSourceMask || PowerIDsResetCheck(uiSourceMask, pResetIDs, pEnt->pChar->ppPowersPersonal[i]->uiID))
		{
			if(!bJustCount)
	...
	{
		if(bIgnoreSourceMask || PowerIDsResetCheck(uiSourceMask, pResetIDs, pEnt->pChar->ppPowersClass[i]->uiID))
		{
			if(!bJustCount)
	...
	{
		if(bIgnoreSourceMask || PowerIDsResetCheck(uiSourceMask, pResetIDs, pEnt->pChar->ppPowersSpecies[i]->uiID))
		{
			if(!bJustCount)
	...
	for(i=0; i<eaSize(&pEnt->pChar->ppPowerTrees); i++)
	{
		for(j=0; j<eaSize(&pEnt->pChar->ppPowerTrees[i]->ppNodes); j++)
		{
			...
			{
				if(bIgnoreSourceMask || PowerIDsResetCheck(uiSourceMask, pResetIDs, pEnt->pChar->ppPowerTrees[i]->ppNodes[j]->ppPowers[k]->uiID))
				{
					if(!bJustCount)
	...
	if(NONNULL(pEnt->pInventoryV2))
	{
		for(i=0; i<eaSize(&pEnt->pInventoryV2->ppInventoryBags); i++)
		{
			...
			bagiterator_trh_SetBagByID(ATR_PASS_ARGS, pEnt,pEnt->pInventoryV2->ppInventoryBags[i]->BagID, iter, pExtract);
			for(; !bagiterator_Stopped(iter); bagiterator_Next(iter))
			{
				...
				if(pItem)	// check any item for powers as they might need fixup
				{
					...
					{
						if(bIgnoreSourceMask || PowerIDsResetCheck(uiSourceMask, pResetIDs, pItem->ppPowers[k]->uiID))
						{
							if(!bJustCount)
					...
					if (pItem->pSpecialProps)
					{
						...
						{
							NOCONST(ItemGemSlot) *pGemSlot = pItem->pSpecialProps->ppItemGemSlots[j];

							for (k = 0; k < eaSize(&pGemSlot->ppPowers); ++k)
							{
								if(bIgnoreSourceMask || PowerIDsResetCheck(uiSourceMask, pResetIDs, pGemSlot->ppPowers[k]->uiID))
								{
									if(!bJustCount)
										pGemSlot->ppPowers[k]->uiID = entity_GetNewPowerIDHelper(pEnt);
									iReset++;
								}
							}
						}
...
S32 entity_ResetPowerIDsAllHelper(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, GameAccountDataExtract *pExtract)
{
	...
	iReset = entity_ResetPowerIDsHelper(ATR_PASS_ARGS, pEnt, -1, pExtract, false, NULL);

	return iReset;
}

// Resets the given powerIDs and returns the number of powers actually reset
AUTO_TRANS_HELPER;
S32 entity_ResetListedPowerIDsHelper(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, PowerIDList *pResetIDs, GameAccountDataExtract *pExtract)
{
	S32 iReset = 0;

	if(ISNULL(pEnt->pChar) || !pResetIDs || eaiSize(&pResetIDs->eaiPowerIDs) == 0)
		return iReset;

	iReset = entity_ResetPowerIDsHelper(ATR_PASS_ARGS, pEnt, 0, pExtract, false, pResetIDs);
