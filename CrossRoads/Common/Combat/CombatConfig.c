...
// returns a multiplier to scale damage dealt by a level iPlayerLevel player to damage dealt by an iTargetLevel player.
//		Relies on a hand-made fudge table in combatConfig.def.  Used for scary monster tech.
float combatConfig_getPlayerDamageOutNormalizer(int iSourceLevel, int iTargetLevel)
{
	if( iSourceLevel > 0 && iSourceLevel < eafSize(&g_CombatConfig.eafPlayerDamageOutNormalizers) && iTargetLevel > 0 && iTargetLevel < eafSize(&g_CombatConfig.eafPlayerDamageOutNormalizers))
	{
		return g_CombatConfig.eafPlayerDamageOutNormalizers[iTargetLevel] / g_CombatConfig.eafPlayerDamageOutNormalizers[iSourceLevel];
	}
	return 1;
}

// returns a multiplier to scale damage dealt to a level iSourceLevel player to damage dealt to an iPlayerLevel player.
//		Relies on a hand-made fudge table in combatConfig.def.  Used for scary monster tech.
float combatConfig_getPlayerDamageInNormalizer(int iSourceLevel, int iTargetLevel)
{
	if( iSourceLevel > 0 && iSourceLevel < eafSize(&g_CombatConfig.eafPlayerDamageOutNormalizers) && iSourceLevel > 0 && iSourceLevel < eafSize(&g_CombatConfig.eafPlayerDamageOutNormalizers))
	{
		return g_CombatConfig.eafPlayerDamageInNormalizers[iTargetLevel] / g_CombatConfig.eafPlayerDamageInNormalizers[iSourceLevel];
	}
	return 1;
}
