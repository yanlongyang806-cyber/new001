...
typedef struct Character
{
	...
	Power **ppPowersSCP;									AST(SELF_ONLY, LATEBIND, FORCE_CONTAINER)
		// NON-INDEXED earray of powers gotten from active SCP
	...
	PACritical *pSavedCritical;								NO_AST
		// If the Character has its critical state locked (generally entCreates & projectiles)
	...
	U32 bDoNotDraw : 1;
		// Character cannot be drawn by the client
		// Changes rarely
	...
	U32 bSendAllAttribModNet : 1;							NO_AST
		// Whether or not the Character should send all AttribModNets on the next character tick
		// Changes rarely
	...
	U32 bUseBecomeCritterTray : 1;
		// Character uses the Become Critter tray when a Become Critter power is active
		// This is set when a character uses a Become Critter power with no available
		// power slots, and must instead use the Tray to populate with its list of
		// become critter powers
...
// Returns a Power owned generally by the Character, otherwise returns NULL
SA_RET_OP_VALID Power *character_FindPowerByRefParams(SA_PARAM_NN_VALID Character *pchar, U32 uiID, S32 iIdxSub, S16 iLinkedSub);
...
void gclCharacter_ForceDismount(SA_PARAM_NN_VALID Character* pChar, SA_PARAM_OP_VALID PowerDef *pActivatingPower);
...
bool trhCharacter_ModifyStatPoint(NOCONST(Entity)* ent, SA_PARAM_NN_STR const char *pchStatPointPoolName, S32 eType, S32 iPoints, S32 *piNewValue);
