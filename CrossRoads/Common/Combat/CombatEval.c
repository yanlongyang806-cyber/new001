...
static void ContextInitSimple(S32 bInit)
{
	...
	if(bInit)
	{
		...
		stTable = exprContextCreateFunctionTable("Combat_Simple");
...
static void ContextInitEnhance(S32 bInit)
{
	...
	if(bInit)
	{
		...
		// Functions
		//  Generic, PowerDef
		stTable = exprContextCreateFunctionTable("Combat_Enhance");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsGeneric");
...
static void ContextInitActivate(S32 bInit)
{
	...
	if(bInit)
	{
		...
		stTable = exprContextCreateFunctionTable("Combat_Activate");
		exprContextAddFuncsToTableByTag(stTable, "util");
...
static void ContextInitTarget(S32 bInit)
{
	...
	if(bInit)
	{
		...
		// Functions
		//  Generic, Self, Character
		stTable = exprContextCreateFunctionTable("Combat_Target");
		exprContextAddFuncsToTableByTag(stTable, "util");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsGeneric");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsSelf");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsCharacter");
...
static void ContextInitApply(S32 bInit)
{
	...
	if(bInit)
	{
		...
		stTable = exprContextCreateFunctionTable("Combat_Apply");
		exprContextAddFuncsToTableByTag(stTable, "util");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsGeneric");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsSelf");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsCharacter");
...
static void ContextInitAffects(S32 bInit)
{
	...
	if(bInit)
	{
		...
		stTable = exprContextCreateFunctionTable("Combat_Affects");
		exprContextAddFuncsToTableByTag(stTable, "util");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsGeneric");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsPowerDef");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsAttribMod");
...
static void ContextInitExpiration(S32 bInit)
{
	...
	if(bInit)
	{
		...
		stTable = exprContextCreateFunctionTable("Combat_Expiration");
		exprContextAddFuncsToTableByTag(stTable, "util");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsGeneric");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsCharacter");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsPowerDef");
...
static void ContextInitEntCreateEnhancements(S32 bInit)
{
	...
	if(bInit)
	{
		...
		// Functions
		//  Generic, Character, PowerDef, AttribMod
		stTable = exprContextCreateFunctionTable("Combat_CreateEnhancements");
		exprContextAddFuncsToTableByTag(stTable, "util");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsGeneric");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsCharacter");
		exprContextAddFuncsToTableByTag(stTable, "CEFuncsPowerDef");
...
static void ContextInitTeleport(S32 bInit)
{
	...
	if(bInit)
	{
		...
		// Functions
		//  Generic, Character, PowerDef, AttribMod
		stTable = exprContextCreateFunctionTable("Combat_Teleport");
		exprContextAddFuncsToTableByTag(stTable, "util");
...
AUTO_EXPR_FUNC(CEFuncsCharacter) ACMD_NOTESTCLIENT;
int HasConditionalPowerTag(	ACMD_EXPR_PARTITION iPartitionIdx, 
							SA_PARAM_OP_VALID PowerDef_ForExpr *powerDef,
							ACMD_EXPR_ENUM(PowerTag) const char *tagName)
{
	int eTag = StaticDefineIntGetInt(PowerTagsEnum,tagName);
	...
	if (eTag >= 0)
	{
		...
		FOR_EACH_IN_EARRAY_FORWARDS(powerDef->eaConditionalTags, PowerConditionalTags, pConditionalTags)
		{
			if (eaiFind(&pConditionalTags->piTags, eTag) >= 0)
			{
				// tag is found, lets check the requirements
				AttribModDef *pModDef = eaGet(&powerDef->ppOrderedMods, pConditionalTags->iAttribIndex);
				
				bFoundTag = true;

				if (pModDef && pModDef->pExprRequires)
				{
					if(combateval_EvalNew(iPartitionIdx, pModDef->pExprRequires, kCombatEvalContext_Apply, NULL))
					{
						return true;
					}
				}
...
F32 ItemWeaponDamageFromItemDef(int iPartitionIdx, SA_PARAM_OP_VALID PowerApplication *papp, SA_PARAM_OP_VALID ItemDef *pItemDef, CombatEvalMagnitudeCalculationMethod eCalcMethod)
{
	...
	if(pItemDef)
	{
		if (pItemDef->pItemDamageDef)
		{
			...
			{
				int iWeaponLevelToUse = pItemDef->iLevel;
				
				if (papp && papp->bTargetIsLevelAdjusting)
				{
					// if a character is getting their damage boosted by scary monster, cap their weapon level at their character level.
					//	this is to make sure characters below level cap with OP weapons don't do lots more damage than characters at level cap.
					iWeaponLevelToUse = MIN(iWeaponLevelToUse, papp->iLevel);
				}

				fDamage *= powertable_Lookup(pItemDef->pItemDamageDef->pchTableName, iWeaponLevelToUse - 1);
			}
...
AUTO_EXPR_FUNC(CEFuncsSelf);
ExprFuncReturnVal SourceItemGetSCPQualityModifier(ExprContext *pContext, ACMD_EXPR_FLOAT_OUT fOut, ACMD_EXPR_ERRSTRING errString)
{
	Item *pSourceItem = exprContextGetVarPointerUnsafePooled(pContext,s_pchSourceItem);

	if (scp_itemIsSCP(pSourceItem))
	{
		ItemQuality iQuality = item_GetQuality(pSourceItem);
		*fOut = scp_QualityModifer(iQuality);
		return ExprFuncReturnFinished;
	}

	estrPrintf(errString, "Invalid use of SourceItemGetSCPQualityModifier inside power that doesn't use an SCP item");
...
AUTO_EXPR_FUNC(CEFuncsSelf);
ExprFuncReturnVal SourceItemGetProgressionLevel(ExprContext *pContext, ACMD_EXPR_INT_OUT intOut, ACMD_EXPR_ERRSTRING errString)
{
	Item *pSourceItem = exprContextGetVarPointerUnsafePooled(pContext,s_pchSourceItem);

	if (pSourceItem)
	{
		*intOut = itemProgression_GetLevel(pSourceItem);
		return ExprFuncReturnFinished;
	}

	estrPrintf(errString, "Invalid use of SourceItemGetProgressionLevel inside power that doesn't use an item");
	return ExprFuncReturnError;
...
AUTO_EXPR_FUNC(CEFuncsSelf);
ExprFuncReturnVal SourceItemGetZeroBasedProgressionLevel(ExprContext *pContext, ACMD_EXPR_INT_OUT intOut, ACMD_EXPR_ERRSTRING errString)
{
	Item *pSourceItem = exprContextGetVarPointerUnsafePooled(pContext,s_pchSourceItem);

	if (pSourceItem)
	{
		*intOut = itemProgression_GetLevel(pSourceItem)-1;
		return ExprFuncReturnFinished;
	}
...
void STONormalizeAttribs(Entity *pEnt)
{
	...
	{
		memcpy(&g_CombatEvalOverrides.attribs,pEnt->pChar->pattrBasic,g_iCharacterAttribSizeUsed);
