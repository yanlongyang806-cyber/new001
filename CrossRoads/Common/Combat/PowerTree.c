...
AUTO_TRANS_HELPER;
void powertree_FinalizeNodes(ATH_ARG NOCONST(Entity) *e, ATH_ARG NOCONST(PowerTree) *pTree)
{
	int i,j,k;	
	...
	for(i=eaSize(&pTree->ppNodes)-1; i>=0; i--)
	{
		...
		for(j=0; j<=pNode->iRank; j++)
		{
			...
			if(!devassert(pRankDef) || pRankDef->bEmpty)
				continue;

			pRankPowerDef = GET_REF(pRankDef->hPowerDef);

			if(!devassert(pRankPowerDef))
				continue;

			// We have a node rank with powers but no powers on the powertree node. This means
			// something has gone terribly wrong. We're going to attempt to repair the character
			// by creating and adding the missing powers to the powertree node
			if (!ppPowersNodeCopy)
			{
				Alertf("%s PowerTreeNode is missing powers it is supposed to have. Attempting to fix.", REF_STRING_FROM_HANDLE(pNode->hDef));
				for (k=j; k<=pNode->iRank; k++)
				{
					PTNodeRankDef *pFixupRankDef = pNodeDef->ppRanks[k];
					PowerDef *pFixupRankPowerDef;
					NOCONST(Power)* pPower;

					if (!pFixupRankDef || pFixupRankDef->bEmpty)
						continue;

					pFixupRankPowerDef = GET_REF(pFixupRankDef->hPowerDef);
					if (!pFixupRankPowerDef)
						continue;

					pPower = entity_CreatePowerHelper(e, pFixupRankPowerDef, 0);
					eaPush(&pNode->ppPowers,pPower);
				}

				// Now that we've finished adding the powers, we need to recopy the list of powers on the node
				eaCopy(&ppPowersNodeCopy,&pNode->ppPowers);

				// A final safety check to ensure we don't crash the server if this somehow failed
				if (!devassert(ppPowersNodeCopy))
					continue;
			}

			if (!devassert(ppPowersNodeCopy[0]))
				continue;
...
bool powertrees_Load_GroupValidate(PTGroupDef *pGroup, int iGroup, PowerTreeDef *pTree)
{
	...
	for(i=eaSize(&pGroup->ppNodes)-1; i>=0; i--)
	{
		PTNodeDef *pNodeDef = pGroup->ppNodes[i];
		if (pTree->bIsTalentTree)
		{
			// see if any other nodes occupy the same row/column
			FOR_EACH_IN_EARRAY(pGroup->ppNodes, PTNodeDef, pOtherNodeDef)
			{
				if (pNodeDef != pOtherNodeDef && pNodeDef->iUIGridColumn == pOtherNodeDef->iUIGridColumn &&
					pNodeDef->iUIGridRow == pOtherNodeDef->iUIGridRow)
				{
					if (!pGroup->ppNodes[i]->pExprAccessCondition)
					{
						ErrorFilenamef(pTree->pchFile,"%s: Group %d: Row,Col (%d,%d) talent tree sibling nodes do not all have a requirement expression.",
										pTree->pchName, iGroup, pNodeDef->iUIGridRow, pNodeDef->iUIGridColumn);
						bRet = false;
					}
				}
			}
...
ExprContext* powertree_GetContext(void)
{
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("PowerTree");
...
static void powertrees_Load_Post(PowerTreeDef *pTree)
{
	...
	for(i=eaSize(&pTree->ppGroups)-1; i>=0; i--)
	{
		for(c=eaSize(&pTree->ppGroups[i]->ppNodes)-1; c>=0; c--)
		{
			...
			if (pNode->pExprAccessCondition)
			{
				ExprContext* pContext = ptpurchaserequirements_GetContext();
				if (pContext)
					exprGenerate(pNode->pExprAccessCondition, pContext);
			}
...
ExprContext* powertreetypes_GetContext(void)
{
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("PowerTreeTypes");
		exprContextAddFuncsToTableByTag(stTable,"PTECharacter");
		exprContextAddFuncsToTableByTag(stTable,"entityutil");
		exprContextAddFuncsToTableByTag(stTable,"util");
		exprContextSetFuncTable(s_pContext, stTable);
...
ExprContext* powertreeenhtypes_GetContext(void)
{
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("PowerrTreeEnhanceTypes");
		exprContextAddFuncsToTableByTag(stTable,"PTECharacter");
...
ExprContext *ptpurchaserequirements_GetContext()
{
	static ExprContext* s_pContext = NULL;
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("PowerTreePurchaseRequirements");
		exprContextAddFuncsToTableByTag(stTable,"PTECharacter");
...
bool powertree_IsConditionalNode(PTNodeDef *pNodeDef)
{
	return pNodeDef->pExprAccessCondition != NULL;
}

// returns true if the entity can buy the given PTNodeDef. 
// if no pExprAccessCondition is defined, returns true
bool powertree_CanAccessConditionalNode(Entity *pEnt, PTNodeDef *pNodeDef)
{
	if (pNodeDef->pExprAccessCondition)
	{
		...
		exprContextSetPointerVar(pContext,"GameAccount", entity_GetGameAccount(pEnt), parse_GameAccountData, false, true);
		exprContextSetSelfPtr(pContext, (Entity*)pEnt);
		exprContextSetPartition(pContext, entGetPartitionIdx(pEnt));
		exprEvaluateTolerateInvalidUsage(pNodeDef->pExprAccessCondition, pContext, &mVal);
...
bool DEFAULT_LATELINK_ent_NeedsProjectSpecificPowersFixup(Entity *pEnt)
{
	return false;
}

void DEFAULT_LATELINK_ent_FixupProjectSpecificPowers(Entity *pEnt)
{
}
