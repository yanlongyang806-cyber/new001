...
typedef enum StackType
{
	...
	kStackType_RefreshDuration,
		// Update the parameters and extend the duration but no more than the base duration
...
typedef struct AttribModDef
{
	...
	Expression *pExprIgnores;		AST(NAME(ExprBlockIgnores,ExprIgnoresBlock), REDUNDANT_STRUCT(ExprIgnores, parse_Expression_StructParam), LATEBIND)
		// Expression that limits what other AttribMods can affect this one while it is active.
		// Can be checked against both a target AttribModDef and/or a target PowerDef.
	...
	S32 iAutoDescOrder;						AST(NAME(AutoDescOrder))
		// The order to place this AttribModDef in the autodesc
	...
	F32 fAutoDescMagnitudeScale;			AST(NAME(AutoDescMagnitudeScale))
		// The value to multiply this attrib mod's magnitude by for the UI autodesc. A 0 value will be ignored
	...
	U32 bEntCreateSaveApplyStrengths : 1;	AST(NO_TEXT_SAVE, SERVER_ONLY)
		// Derived. With respect to EntCreates. Set to true if the AttribMod can cause the application of additional Powers
		//  and we need to fix the strength of the AttribMods of those Powers ahead of time.
	...
	U32 bDeactivationLeavesMod : 1;			AST(SERVER_ONLY)
		// True if deactivating the power allows the mods it has created to remain.  
		// Valid only on periodic powers and some special cases, like reactive powers
...
typedef struct PowerApplyStrength
{
	...
	F32 *pfModCritThreshold;		AST(PERSIST, NO_TRANSACT)
		// The crit threshold of this mod. Used for rerolling crits if the power
		// is flagged to do so. Useful if you don't want to use the crit flag from
		// piModFlags.
...
typedef struct AttribModParams
{
	...
	float fFacing;
		// Use this as the facing of the entity
...
#define moddef_AffectsModOrPowerChk(iPartitionIdx,pmoddefAffects,pchar,pmodAffects,pmoddefTarget,pmodTarget,ppowdefTarget)	\
	((!pmoddefAffects->pExprAffects && (!pmoddefTarget || !pmoddefTarget->pExprIgnores)) || moddef_AffectsModOrPower(iPartitionIdx,pmoddefAffects,pchar,pmodAffects,pmoddefTarget,pmodTarget,ppowdefTarget))

#define moddef_AffectsPowerChk(iPartitionIdx,pmoddefAffects,pchar,pmodAffects,ppowdefTarget)	\
	(!pmoddefAffects->pExprAffects || moddef_AffectsModOrPower(iPartitionIdx,pmoddefAffects,pchar,pmodAffects,NULL,NULL,ppowdefTarget))
...
// returns true if the attribModDef should save apply strengths
bool moddef_ShouldSaveApplyStrengths(SA_PARAM_NN_VALID AttribModDef *pmoddef);

// returns true if the attribModDef should save apply strengths when the power is coming from an entcreate
