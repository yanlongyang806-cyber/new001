...
static S32 character_GetPowerSlotCount(SA_PARAM_NN_VALID Character *pchar)
{
	PowerSlot **eaSlots = CharacterGetPowerSlotEArray(pchar);
...
// checks if a slot is cooling down right now and this should prevent slotting.
static bool character_PowerSlotIsLockedByCooldown(Character* pChar, int iTray, int iSlot, bool bNotifyErrors)
{
	...
	Power *pOldSwitchedStatePower = CombatPowerStateSwitching_GetSwitchedStatePower(pChar, pOldPower);
	...
	if(pOldPower && (pOldPower->fTimeRecharge || power_GetChargeRefillTime(pOldPower) && power_GetChargesLeft(pOldPower) == 0) 
	|| pOldSwitchedStatePower && (pOldSwitchedStatePower->fTimeRecharge || power_GetChargeRefillTime(pOldSwitchedStatePower) && power_GetChargesLeft(pOldSwitchedStatePower) == 0))
	{
#if defined(GAMESERVER) || defined(GAMECLIENT)
		if (bNotifyErrors)
		{
								...
								bool bNotifyErrors)
{
			...
			|| !character_HasMode(pChar, g_PowerSlotsConfig.eModeToDisableCooldown)))
	{
		...
		if(ppow && entGetAccessLevel(pChar->pEntParent) < 9)
		{
			...
			ClientCmd_PowerSetRechargeClient(pChar->pEntParent, uiIDPower, -1, g_PowerSlotsConfig.fSecondsCooldownAfterSlotting);
#endif
		}
	}
								...
								GameAccountDataExtract *pExtract)
{
		...
		character_PowerSlotIsUnlockedBySlotInTray(pchar, iTrayB, iSlotB))
	{
		...
		if(uiIDA || uiIDB)
		{
			...
			bool bCooldownPreventsSwapping = pSlotA->bPreventSwappingDuringCooldown || pSlotB->bPreventSwappingDuringCooldown;

			// Unslot both slots
			character_PowerTraySlotSet(iPartitionIdx, pchar, iTrayA, iSlotA, 0, false, pExtract, !bCooldownPreventsSwapping, false);
			character_PowerTraySlotSet(iPartitionIdx, pchar, iTrayB, iSlotB, 0, false, pExtract, !bCooldownPreventsSwapping, false);

			// Slot both slots with swapped IDs.
			bSuccess = character_PowerTraySlotSet(iPartitionIdx, pchar, iTrayA, iSlotA, uiIDB, false, pExtract, !bCooldownPreventsSwapping, false) && 
				character_PowerTraySlotSet(iPartitionIdx, pchar, iTrayB, iSlotB, uiIDA, false, pExtract, !bCooldownPreventsSwapping, false);
...
static void CharacterTrayBecomeCritterUpdate(SA_PARAM_NN_VALID Character *pchar)
{
	if (pchar->bBecomeCritter)
	{
		int i, s, iCount = 0;
		...
		if (!pchar->bUseBecomeCritterTray)
		{
			pchar->bUseBecomeCritterTray = true;
			bFlagChanged = true;
		}
		...
		pchar->pSlotSetBecomeCritter = StructCreate(parse_PowerSlotSet);
		s = eaSize(&pchar->ppPowers);
		ea32SetSize(&pchar->pSlotSetBecomeCritter->puiPowerIDs, s);

		for (i = 0; i < s; i++)
		{
			Power *pPow = pchar->ppPowers[i];
			PowerDef *pDef = GET_REF(pPow->hDef);
			if (pDef && pPow->eSource == kPowerSource_AttribMod && POWERTYPE_ACTIVATABLE(pDef->eType))
			{
				pchar->pSlotSetBecomeCritter->puiPowerIDs[iCount] = pPow->uiID;
				iCount++;
			}
		}

		ea32SetSize(&pchar->pSlotSetBecomeCritter->puiPowerIDs, iCount);

		// If the slot data changed, set the dirty bit
		if (eaiCompare(&puiOld, &pchar->pSlotSetBecomeCritter->puiPowerIDs) || bFlagChanged)
			entity_SetDirtyBit(pchar->pEntParent, parse_Character, pchar, false);
		ea32Destroy(&puiOld);
	}
}
								 ...
								 GameAccountDataExtract *pExtract)
{
	...
	if(entIsServer())
	{
		if (character_GetPowerSlotCount(pchar))
		{
			CharacterPowerSlotsBecomeCritterUpdate(pchar);
		}
		else
		{
			CharacterTrayBecomeCritterUpdate(pchar);
		}
	}
							 ...
							 U32 uiID)
{
	...
	{	// critters don't really have the concept of slotting. 
		// so for now we will consider everything always slotted in the critter's case.
		return true;
	}
	else
	{
