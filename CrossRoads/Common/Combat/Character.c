...
{
	if (eaSize(&ppow->ppSubCombatStatePowers) && ppowref->iLinkedSub >= 0)
	{
...
// Returns a Power owned generally by the Character, otherwise returns NULL
Power *character_FindPowerByRefParams(SA_PARAM_NN_VALID Character *pchar, U32 uiID, S32 iIdxSub, S16 iLinkedSub)
{
	if(uiID)
	{
		...
		{	// first see if we are referencing a linked power within, and get that one.
			...
			return character_findPowerByRefInternal(ppow, &ref);
		}
	}
	return NULL;
}
...
void character_LimitedUseSend(Character* pchar, Packet* pak)
{
	...
	for(i = 0; i < iSize; i++)
	{
		...
		if (pDef && pDef->fLifetimeGame  || pDef->fLifetimeUsage)
...
void character_ResetPowersArray(int iPartitionIdx, Character *pchar, GameAccountDataExtract *pExtract)
{
	...
	if(pchar->pEntParent)
	{
		...
		// Add all powers from SuperCritterPets
		scp_AddPowersFromActivePets(iPartitionIdx, pchar->pEntParent);
	}
									...
									ActivationFailureReason *peFailOut)
{
	...
	if(pdef && pdef->eType==kPowerType_Combo)
	{
		...
		{
#if GAMECLIENT
			// If we have no subpowers and we have a pending inventory bag slot switch request,
			// then we're just waiting for a server response before populating the subpowers,
			// so there's no need to error here. This is expected behavior for the client.
			if(!iPowers && pchar->pEntParent && pchar->pEntParent->pInventoryV2 && eaSize(&pchar->pEntParent->pInventoryV2->ppSlotSwitchRequest))
			{
				InvBagSlotSwitchRequest* pRequest = pchar->pEntParent->pInventoryV2->ppSlotSwitchRequest[0];
				if(pRequest && (!pRequest->bHasHandledMoveEvents || !pRequest->bHasChangedSlot))
				{
					return ppow;
				}
			}
...
// if bAllowLeavingIndividualMods is set, allows the individual mods to leave themselves if they are set to on the attribModDef
									 ...
									 S32 bAllowLeavingIndividualMods)
{
	...
	for(i=eaSize(&pchar->modArray.ppMods)-1; i>=0; i--)
	{
		...
		if(pmod && pmod->uiPowerID == uiPowerID && erSource == pmod->erSource
			...
			&& (!bAllowLeavingIndividualMods || !pmod->pDef || !pmod->pDef->bDeactivationLeavesMod))
		{
			if(bDisable)
			{
				pmod->bDisabled = true;
...
void character_RegenRewardsIfChanged(Character *pchar, AttribMod ***pppRewardModifies)
{
	...
	if(pchar && pchar->pEntParent && pchar->pEntParent->pPlayer && entGetType(pchar->pEntParent) == GLOBALTYPE_ENTITYPLAYER)
	{
		...
		{
			// Check if old reward attribs match current attribs

			if (eaSize(&pPlayer->eaRewardMods) != eaSize(&pchar->ppRewardModifies))
			{	
				bRegenRewards = true;
			}
			else if (eaSize(&pchar->ppRewardModifies) == eaSize(pppRewardModifies))
			{	
		...
		else if (eaSize(&pPlayer->eaRewardMods) > 0)
		{	// we have some rewards on the player, but no current attribs
			bRegenRewards = true;
		}
...
void character_CleanupPartial(int iPartitionIdx, Character *pchar, GameAccountDataExtract *pExtract)
{
	...
	for (i = 0; i < eaSize(&pchar->ppPowersSCP); i++)
	{
		power_Destroy(pchar->ppPowersSCP[i], pchar);
	}	
	eaDestroy(&pchar->ppPowersSCP);
...
void character_Cleanup(int iPartitionIdx, Character *pchar, bool isReloading, GameAccountDataExtract *pExtract, bool bDoCleanup)
{
	...
	StructDestroySafe(parse_PACritical, &pchar->pSavedCritical);
...
// returns the current (tick) number times the given event has triggered
U16 character_GetCombatEventCount(SA_PARAM_NN_VALID Character *pchar, CombatEvent eEvent)
{
	if(pchar->pCombatEventState && eEvent < kCombatEvent_Count)
	{
		return pchar->pCombatEventState->auiCombatEventCountCurrent[eEvent];
	}
...
F32 character_NearDeathEnter(int iPartitionIdx, Character *pchar, F32 fTimer)
{
	...
	if(entIsAlive(pchar->pEntParent) && !pchar->pNearDeath)
	{
		...
		if(pClass && pClass->pNearDeathConfig)
		{
			...
			character_DeactivateToggles(iPartitionIdx, pchar, pmTimestamp(0), true, true);
...
void character_LoadTransact(ATH_ARG NOCONST(Entity) *e)
{
	...
	for(i=eaSize(&e->pChar->ppPowerTrees)-1; i>=0; i--)
	{
		...
		powertree_FinalizeNodes(e, e->pChar->ppPowerTrees[i]);
	}
	...
	while(pStatPointPoolDef = (StatPointPoolDef*)RefSystem_GetNextReferentFromIterator(&iter))
	{
		...
		if(i < 0)
		{
			...
			//Repurchase class defined auto spend stat points
			if(pClass)
			{	
				int count, iNewValue = 0;
				for (count=eaSize(&pClass->ppAutoSpendStatPoints)-1; count>=0; --count)
				{
					StatPointPoolDef *pDef = StatPointPool_DefFromAttrib(pClass->ppAutoSpendStatPoints[count]->eType);
					trhCharacter_ModifyStatPoint(e, pDef->pchName, pClass->ppAutoSpendStatPoints[count]->eType, pClass->ppAutoSpendStatPoints[count]->iPoints, &iNewValue);
				}
			}
			break;
		}
...
void character_LoadNonTransact(int iPartitionIdx, Entity *e, S32 bOffline)
{
	...
	//If this is a PvP map and we don't want to respawn the player on match start
	//we should refill the players health and do a little cleanup
	if (gConf.bDisableRespawnOnPvPMatchStart && zmapInfoGetMapType(NULL) == ZMTYPE_PVP)
	{
		pchar->pattrBasic->fHitPoints = pchar->pattrBasic->fHitPointsMax;

		character_DirtyAttribs(pchar);
		character_AttribPoolRespawn(pchar);
		if (pchar->pNearDeath)
...
void gclCharacter_ForceDismount(Character *pChar, PowerDef *pActivatingPower)
{
	if (g_CombatConfig.iMountPowerCategory)
	{
		if (pActivatingPower)
		{ 
			if (eaiFind(&pActivatingPower->piCategories, g_CombatConfig.iMountPowerCategory) >= 0)
				return;
		}
		
		if (pChar->pEntParent->costumeRef.pMountCostume && 
...
#ifdef GAMESERVER
// only works if the g_CombatConfig.iMountPowerCategory is set, assumes the power is an Toggle
// cancels the toggle if a power is active with the right powercategory
void gslCharacter_CancelMountPower(Character *pChar)
{
	if (pChar && g_CombatConfig.iMountPowerCategory)
	{
		if (pChar->ppPowerActToggle)
		{
			FOR_EACH_IN_EARRAY(pChar->ppPowerActToggle, PowerActivation, pAct)
			{
				PowerDef *pDef = GET_REF(pAct->hdef);
				if (pDef && eaiFind(&pDef->piCategories, g_CombatConfig.iMountPowerCategory) >= 0)
				{
					character_DeactivateToggle(entGetPartitionIdx(pChar->pEntParent), pChar, pAct, NULL, pmTimestamp(0), true);
				}
			}
			FOR_EACH_END
		}
