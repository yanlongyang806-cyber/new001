...
static int s_bPowersResPostBinningValidateThrowWarnings = false;
AUTO_CMD_INT(s_bPowersResPostBinningValidateThrowWarnings, PowersPostBinningValidateThrowWarnings);
...
{
	ParserSetTableInfo(parse_PowerDef_ForExpr, sizeof(PowerDef), "PowerDef_ForExpr", NULL, __FILE__, SETTABLEINFO_ALLOW_CRC_CACHING);
	{
...
static void AttribModDefValidate(PowerDef *ppowdef, AttribModDef *pdef, S32 bWarnings)
{
	...
	if(pdef->bPersonal)
	{
		...
		if(IS_BASIC_ASPECT(pdef->offAspect))
		{
			if(IS_SPECIAL_ATTRIB(pdef->offAttrib) && 
				(pdef->offAttrib != kAttribType_PowerMode && pdef->offAttrib != kAttribType_CombatAdvantage && pdef->offAttrib != kAttribType_Null))
			{
	...
	if(pdef->pExprIgnores)
	{
		if(exprIsZero(pdef->pExprIgnores))
		{
			ModWarningf(pdef,"%s: %d: Ignores expression of 0, should be deleted if possible",powerdef_NameFull(ppowdef),pdef->uiDefIdx);
		}
		if(IS_NORMAL_ATTRIB(pdef->offAttrib) && IS_BASIC_ASPECT(pdef->offAspect) && !POWER_AFFECTOR(pdef->offAttrib))
		{
			ModErrorf(pdef,"%s: %d: Basic %s mods do not support an Ignores expression",powerdef_NameFull(ppowdef),pdef->uiDefIdx,StaticDefineIntRevLookup(AttribTypeEnum,pdef->offAttrib));
		}
		if(ppowdef->eType==kPowerType_Innate)
		{
			ModWarningf(pdef,"%s: %d: AttribMods on Innate Powers do not support an Ignores expression",powerdef_NameFull(ppowdef),pdef->uiDefIdx);
		}
	}
	...
	if(pdef->eStack==kStackType_RefreshDuration)
	{
		if(!pdef->pExprDuration)
		{
			ModErrorf(pdef,"%s: %d: Stack type RefreshDuration only valid on AttribMods with a duration",powerdef_NameFull(ppowdef),pdef->uiDefIdx);
		}
	}
	...
	{
		if(!(pdef->eStack==kStackType_Discard || pdef->eStack==kStackType_Replace || pdef->eStack==kStackType_KeepBest || pdef->eStack==kStackType_RefreshDuration))
		{
			ModErrorf(pdef,"%s: %d: Stack Limit cannot be used with the currently selected stack type",powerdef_NameFull(ppowdef),pdef->uiDefIdx);
		}
	...
	if(pdef->pParams)
	{
		...
		{
			if (pdef->offAspect != kAttribAspect_Immunity)
			{
...
#define PowerRetroErrorf(pdef,fmt,...) { pdef->eError |= kPowerError_Error; ErrorFilenameGroupRetroactivef((pdef)->pchFile,"Powers",7,8,25,2009,fmt,##__VA_ARGS__); }
	...
	if(IsServer()) 
	{
		...
		if (pdef->eAITags & kPowerAITag_UseTargetPos && 
			(pdef->eEffectArea == kEffectArea_Cylinder || pdef->eEffectArea == kEffectArea_Cone))
		{
			PowerAnimFX *pafx = GET_REF(pdef->hFX);
			PowerTarget *ptarget = GET_REF(pdef->hTargetMain);
			if (ptarget && ptarget->bRequireSelf && 
				pafx && (pafx->bLockFacingDuringActivate || pafx->bLockFacingDuringPreactivate))
			{
				PowerWarningf(pdef,"%s: PowerAnimFX & AI Tags contains incompatible options: UseTargetPos and LockFacing", powerdef_NameFull(pdef));
			}
		}
	}
	...
	// Check for infinite stacking powers
	if (pdef->eType == kPowerType_Passive && !pdef->bAutoReapply)
	{
		for(i=0;i< eaSize(&pdef->ppMods);i++)
		{
			AttribModDef * pMod = pdef->ppMods[i];
			if (!exprCompare(pMod->pExprDuration,s_pExprForever) && pMod->eStack==kStackType_Stack)
			{
				PowerErrorf(pdef,"%s: This power will stack forever!",powerdef_NameFull(pdef));
				break;
			}
		}
	}
...
S32 powerdef_ValidateReferences(int iPartitionIdx, PowerDef *pdef)
{
	...
	// Client can't do this correctly (e.g. doesn't get reward tables), so don't do it at all (this function is actually only called on GameServers, anyway)
	if(!IsServer())
		return true;
	...
	if(bCustom)
	{
		...
		powerdef_AutoDesc(iPartitionIdx,langGetCurrent(),pdef,NULL,pAutoDescPower,NULL,NULL,NULL,NULL,NULL,NULL,1,true,kAutoDescDetail_Normal,NULL,NULL);
...
static S32 PowerDefGenerate(PowerDef *ppow)
{
	...
	if(ppow->pExprHideAutoDesc)
	{
		ExprContext *pContext = powers_GetAutoDescExprContext(NULL, NULL);
		exprGenerate(ppow->pExprHideAutoDesc, pContext);
	}
	...
	for(i=eaSize(&ppow->ppOrderedMods)-1; i>=0; i--)
	{
		...
		pmod->bSaveApplyStrengths = !!moddef_ShouldSaveApplyStrengths(pmod);
		pmod->bEntCreateSaveApplyStrengths = !!moddef_ShouldSaveApplyStrengthsForEntCreate(pmod);
		...
		combateval_Generate(pmod->pExprIgnores,kCombatEvalContext_Affects);
	...
	for(i=0; i<eaSize(&ppow->ppOrderedMods); i++)
	{
		...
		if(!pmod->bDerivedInternally)
		{
			AttribModDef *pDefClient = StructAlloc(parse_AttribModDef);
			StructCopyFields(parse_AttribModDef, pmod, pDefClient, 0, TOK_SERVER_ONLY);
			eaPush(&ppow->ppOrderedModsClient, pDefClient);
		}

		// Build the ConditionalTags list if this mod has tags and has requirements
		if (eaiSize(&pmod->tags.piTags) > 0 && pmod->pExprRequires && !pmod->bDerivedInternally)
		{
			S32 *piTags = NULL;

			// put all the tags in the list, then remove all the tags that are on mods with no requirements
			eaiPushEArray(&piTags, &pmod->tags.piTags);
						
			FOR_EACH_IN_EARRAY_FORWARDS(ppow->ppOrderedMods, AttribModDef, pModDefOther)
			{
				if (!pModDefOther->pExprRequires && !pModDefOther->bDerivedInternally)
				{
					FOR_EACH_IN_EARRAY_INT(pModDefOther->tags.piTags, S32, iCat)
					{
						S32 iIdx = eaiFind(&piTags, iCat);
						if (iIdx >= 0)
						{
							eaiRemoveFast(&piTags, iIdx);
							if (eaiSize(&piTags) == 0)
								break;
						}
					}
			...
			// if we have any tags left after removing them, make a PowerConditionalTags entry 
			if (eaiSize(&piTags))
			{
				PowerConditionalTags *pTags = StructCreate(parse_PowerConditionalTags);

				eaPush(&ppow->eaConditionalTags, pTags);

				pTags->iAttribIndex = i;
				pTags->piTags = piTags;
				piTags = NULL;
			}
			else
...
static int PowerDefResValidateCB(enumResourceValidateType eType, const char *pDictName, const char *pResourceName, void *pResource, U32 userID)
{
	...
	case RESVALIDATE_POST_BINNING:
		powerdef_Validate(pdef, s_bPowersResPostBinningValidateThrowWarnings);
		return VALIDATE_HANDLED;
...
// Utility function for setting a specific recharge on a specific Power with proper notification to the client.
void character_PowerSetRecharge(int iPartitionIdx, Character *pchar, Power *pPower, F32 fTime)
{
	if (pPower)
	{
		if (pPower->pParentPower)
		...
		power_SetRecharge(iPartitionIdx, pchar, pPower, fTime);
...
#ifdef GAMESERVER
		{
			...
			S32 iIdxSub = 0;
			S16 iLinkedSub = 0;
			power_GetIDAndSubIdx(pPower, &uiID, &iIdxSub, &iLinkedSub);

			ClientCmd_PowerSetRechargeClient(pchar->pEntParent, uiID, iLinkedSub, fTime);
		}
#endif
	}
...
void character_PowerSetRechargeByID(int iPartitionIdx, Character *pchar, U32 uiID, F32 fTime)
{
	if(uiID)
	{
		...
		if(ppow)
		{
			character_PowerSetRecharge(iPartitionIdx,pchar,ppow,fTime);
		}
	}
	else
	{	// Go through all your powers and set the recharge to fTime!
		int i;
...
#ifdef GAMESERVER
		ClientCmd_PowerSetRechargeClient(pchar->pEntParent, 0, -1, fTime);
#endif
...
void power_SetRechargeDefault(int iPartitionIdx, SA_PARAM_NN_VALID Character *pchar, SA_PARAM_NN_VALID Power *ppow, S32 bMissedAllTargets)
{
	...
	if(pdef)
	{
		...
		if(pdef->bRechargeRequiresHit || pdef->bRechargeRequiresCombat || pdef->bChargesSetCooldownWhenEmpty)
		{
			...
			ClientCmd_PowerSetRechargeClient(pchar->pEntParent, uiID, iLinkedSub, ppow->fTimeRecharge);
		}
#endif
	}
}
...
void power_SetRecharge(int iPartitionIdx, Character *pchar, Power *ppow, F32 fTime)
{
	...
	// for linked powers, we're going to apply the recharge to all the linked powers
	if (eaSize(&ppow->ppSubCombatStatePowers) && CombatPowerStateSwitching_ShouldShareRecharge(pchar))
	{
		...
		FOR_EACH_END
			 ...
			 CombatPowerStateSwitching_ShouldShareRecharge(pchar))
	{
...
{
	PowerDef *pDef = GET_REF(ppow->hDef);
	if (ppow->eSource != kPowerSource_Item || (pDef && pDef->fChargeRefillInterval > 0.f))
	{
...
void power_SetChargesUsed(Character *pchar, Power *ppow, PowerDef *pDef, int iChargesUsed)
{
	...
	if (iChargesUsed != power_GetChargesUsed(ppow))
	{
		...
		if (ppow->eSource == kPowerSource_Item && pDef->fChargeRefillInterval <= 0.f)
				...
				if (!pParams->bMount)
				{
					...
					else if (pParams->pcBoneGroup && *pParams->pcBoneGroup && pEnt->pSaved)
					{
						...
						*/
						...
						// if this is a ground puppet use the player. If this is a space puppet find space puppet and use. Other ents just use themselves
						NOCONST(PlayerCostume) *pBaseCostume = NULL;
						NOCONST(PlayerCostume) *pCostume = NULL;

						if(pEnt && pEnt->pSaved && pEnt->pSaved->pPuppetMaster)
						{
							// is this a space costume? space puppets have costume, ground pets won't
							S32 iPupIdx;
							ContainerID iPuppetID = pEnt->pSaved->pPuppetMaster->curID;

							if(iPuppetID == 0)
							{
								iPuppetID = pEnt->pSaved->pPuppetMaster->curTempID;
							}

							for(iPupIdx = 0; iPupIdx < eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets); ++iPupIdx)
							{
								if(pEnt->pSaved->pPuppetMaster->ppPuppets[iPupIdx]->curID == iPuppetID)
								{
									Entity *pPuppet = GET_REF(pEnt->pSaved->pPuppetMaster->ppPuppets[iPupIdx]->hEntityRef);
									if(pPuppet && pPuppet->pSaved && eaSize(&pPuppet->pSaved->costumeData.eaCostumeSlots) > 0 && 
										pPuppet->pSaved->costumeData.eaCostumeSlots[0]->pCostume &&
										pPuppet->pSaved->costumeData.iActiveCostume < eaSize(&pPuppet->pSaved->costumeData.eaCostumeSlots))
									{
										pBaseCostume = CONTAINER_NOCONST(PlayerCostume, pPuppet->pSaved->costumeData.eaCostumeSlots[pPuppet->pSaved->costumeData.iActiveCostume]->pCostume);
									}
									break;
								}
							}
						...
						if(!pBaseCostume)
						{
							...
							{
								// use the active costume for the ent
								pBaseCostume = CONTAINER_NOCONST(PlayerCostume, pEnt->pSaved->costumeData.eaCostumeSlots[pEnt->pSaved->costumeData.iActiveCostume]->pCostume);
							}
						}

						if(pBaseCostume)
						{
							pCostume = CONTAINER_NOCONST(PlayerCostume, costumeTailor_MakeCostumeOverlayEx(CONTAINER_RECONST(PlayerCostume, pBaseCostume), GET_REF(pParams->hSkeleton), pParams->pcBoneGroup, true, true));
						}
...
ExprContext *powers_GetAutoDescExprContext(Character *pChar, PowerDef *pPowerDef)
{
	static ExprContext *s_pContext = NULL;
	static int s_hSelf = 0;
	static int s_hPowerDef = 0;
	...
	if(!s_pContext)
	{
		...
		s_FuncTable = exprContextCreateFunctionTable("Powers");
		...
		exprContextAddFuncsToTableByTag(s_FuncTable, "CEFuncsCharacter");
	...
	exprContextSetPointerVarPooledCached(s_pContext, "Self", pChar, parse_Character, true, true,  &s_hSelf);
	exprContextSetPointerVarPooledCached(s_pContext, "PowerDef", pPowerDef, parse_PowerDef_ForExpr, true, true, &s_hPowerDef);
