		...
		for (i=1; ChatLogEntryTypeEnum[i].key && i < kChatLogEntryType_Count + 1; i++) {
			...
			switch (pInfo->kType) {
				...
				case kChatLogEntryType_Trade:
					pInfo->pchChannelPrefix = TRADE_CHANNEL_NAME;
					break;
				case kChatLogEntryType_LookingForGroup:
					pInfo->pchChannelPrefix = LFG_CHANNEL_NAME;
					break;
...
char *zone_GetZoneLikeChannelNameFromMapName(char *Buffer, int Buffer_size, const char *pchPrefix, const char *mapShortName, 
	int mapInstanceIndex, ZoneMapType eMapType, int iVirtualShardID)
{
	if (Buffer_size > 0)
	{
		...
		case ZMTYPE_STATIC:
			if (iVirtualShardID)
				sprintf_s(SAFESTR2(Buffer), "%s%s_%s%d", pchPrefix, mapShortName, VSHARD_PREFIX, iVirtualShardID);
			else
				sprintf_s(SAFESTR2(Buffer), "%s%s", pchPrefix, mapShortName);
		xdefault:
			if (iVirtualShardID)
				sprintf_s(SAFESTR2(Buffer), "%s%s-%d_%s%d", pchPrefix, mapShortName, mapInstanceIndex, VSHARD_PREFIX, iVirtualShardID);
			else
				sprintf_s(SAFESTR2(Buffer), "%s%s-%d", pchPrefix, mapShortName, mapInstanceIndex);
		}
		return Buffer;
	}
	return NULL;
...
char *zone_GetZoneChannelNameFromMapName(char *Buffer, int Buffer_size, const char *mapShortName, 
										 int mapInstanceIndex, ZoneMapType eMapType, int iVirtualShardID)
{
	return zone_GetZoneLikeChannelNameFromMapName(Buffer, Buffer_size, ZONE_CHANNEL_PREFIX, mapShortName, mapInstanceIndex, eMapType, iVirtualShardID);
}

char *zone_GetTradeChannelNameFromMapName(char *Buffer, int Buffer_size, const char *mapShortName, 
	int mapInstanceIndex, ZoneMapType eMapType, int iVirtualShardID)
{
	return zone_GetZoneLikeChannelNameFromMapName(Buffer, Buffer_size, TRADE_CHANNEL_PREFIX, mapShortName, mapInstanceIndex, eMapType, iVirtualShardID);
}

char *zone_GetLFGChannelNameFromMapName(char *Buffer, int Buffer_size, const char *mapShortName, 
	int mapInstanceIndex, ZoneMapType eMapType, int iVirtualShardID)
{
	return zone_GetZoneLikeChannelNameFromMapName(Buffer, Buffer_size, LFG_CHANNEL_PREFIX, mapShortName, mapInstanceIndex, eMapType, iVirtualShardID);
}
...
extern bool ChatCommon_EncodeMessage(SA_PARAM_OP_VALID char **ppchEncodedMsg, SA_PARAM_OP_VALID const ChatMessage *pMsg)
{
	...
	if (ppchEncodedMsg)
	{
		...
		if (pMsg)
			success = (bool) ParserWriteTextEscaped(ppchEncodedMsg, parse_ChatMessage, (void*) pMsg, 0, 0, 0);
	}
	PERFINFO_AUTO_STOP_FUNC();
	return success;
}
...
void ChatConfigCommon_InitToDefault(SA_PARAM_NN_VALID NOCONST(ChatConfig) *pChatConfig, DefaultChatConfigSource eSource, Language eLanguage)
{
	...
	if (pConfigDefaults)
	{
		...
		pChatConfig->bDisableStickyChannels = pConfigDefaults->bDisableStickyChannels;
	...
	else // no defaults found
	{
		...
		pChatConfig->bDisableStickyChannels = false;
...
bool ChatConfigCommon_SetCurrentInputChannel(ChatConfig *pConfig, const char *pchTabGroup, S32 iTab, const char *pchChannel)
{
	...
	if (stricmp_safe(pConfig->pchCurrentInputChannel, pchChannel))
	{
		...
		else if (strStartsWith(pchChannel, TRADE_CHANNEL_PREFIX))
			pConfig->pchCurrentInputChannel = StructAllocString(CHAT_TRADE_SHORTCUT);
		else if (strStartsWith(pchChannel, LFG_CHANNEL_PREFIX))
			pConfig->pchCurrentInputChannel = StructAllocString(CHAT_LFG_SHORTCUT);
