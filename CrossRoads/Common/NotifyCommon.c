	...
	S32 iValue)
{
	...
	if (pEnt)
	{
		ClientCmd_NotifySendWithTagAndValue(pEnt, eType, pchDisplayString, pchLogicalString, pchTag, iValue);
		notfiy_LogActivity(pEnt, eType, pchDisplayString, pchLogicalString, NULL);
	}
#else 

	assertmsg(!pEnt || pEnt == entActivePlayerPtr(), "Only the local entity can be notified from the client.");
	gclNotifyReceiveWithTagAndValue(eType, pchDisplayString, pchLogicalString, pchTag, iValue);
