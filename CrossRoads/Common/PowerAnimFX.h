...
AUTO_ENUM;
typedef enum PowerFXEntityRedirect
{
	kPowerFXEntityRedirect_None = 0,
	
	kPowerFXEntityRedirect_ApplicationTarget,

	kPowerFXEntityRedirect_Max, EIGNORE
	
} PowerFXEntityRedirect;
...
typedef struct PowerAnimFX
{
	...
	PowerFXEntityRedirect hitFXSourceRedirect; AST(SUBTABLE(PowerFXEntityRedirectEnum))
		// For hitFX, changes what entity is used for the 'source' for the FX
		// This is to solve power setups that require the power to be coming from the caster, but the explosion triggers from the target
		// Note: if the redirected source is the same as the target, the FX is not triggered. (This might not be necessary to do)
	...
	F32 fAirFrictionMultOverride;				AST(NAME("airFrictionMultOverride"))
		// overrides the air friction multiplier while power is active
...
// 
void character_AirFrictionOverrideStart(SA_PARAM_NN_VALID Character *pchar, 
										SA_PARAM_NN_VALID PowerActivation *pact);

//
void character_AirFrictionOverrideStop(	SA_PARAM_NN_VALID Character *pchar, 
										SA_PARAM_NN_VALID PowerActivation *pact,
										U32 uiTimeStop);
