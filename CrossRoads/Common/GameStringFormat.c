...
static bool PetContactFormatField(unsigned char **ppchResult, StrFmtContainer *pContainer, Entity *pEnt, const unsigned char *pchField, StrFmtContext *pContext)
{
	...
	PetContactList_GetPetOrCostume(pEnt, pList, NULL, &pPet, &pDef, &pCostume, NULL);
...
static bool MissionDefFormatField(unsigned char **ppchResult, StrFmtContainer *pContainer, MissionDef *pDef, const unsigned char *pchField, StrFmtContext *pContext)
{
	...
	if (!pDef)
		...
		estrAppend2(ppchResult, langTranslateMessageRef(pContext->langID, pDef->detailStringMsg.msg.hMessage));
...
static bool ContactDialogBlockFormatField(unsigned char **ppchResult, DialogBlock **eaBlock, const unsigned char *pchField, StrFmtContext *pContext)
{
	...
	{
		const char *pch = langTranslateMessageRef(pContext->langID, pBlock->displayTextMesg.msg.hMessage);
		if (pch)
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MessageFormatPlayer, MessageFormatEntity);
const char *MessageExprFormatPlayer(ExprContext *pContext, ACMD_EXPR_DICT(Message) const char *pchMessageKey, SA_PARAM_OP_VALID Entity *pEntity)
{
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(StringFormatPlayer, StringFormatEntity);
const char *MessageExprStringFormatPlayer(ExprContext *pContext, const char *pchString, SA_PARAM_OP_VALID Entity *pEntity)
{
	Language eLang = locGetLanguage(getCurrentLocale());
	static unsigned char *s_pch = NULL;
	s_pchFilename = exprContextGetBlameFile(pContext);
