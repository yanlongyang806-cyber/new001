...
typedef enum MicroPurchaseErrorType
{
	...
	kMicroPurchaseErrorType_RequiresCharacter,
...
typedef enum MicroTrans_ShardCategory
{
	...
	// If it turns out that the "Russia" shard supports more than Russia,
	// then I apologize in advance for my poor naming choice. - VAS 20130903
	// (It's either this or "EU" which is equally likely to be wrong)
	kMTShardCategory_RussiaBeta = 5,
	kMTShardCategory_RussiaPTS = 6,
	kMTShardCategory_RussiaLive = 7,
...
typedef struct MicroTransactionCategory
{
	...
	U32					bHasNewItems : 1;	AST(NO_WRITE)
		// Has any items that are also in the "New" category. Calculated in MicroTrans_GetCategories(). 
		// Don't put this in the .def file, please!
