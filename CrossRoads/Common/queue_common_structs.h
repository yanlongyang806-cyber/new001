...
typedef struct QueueMatchMember
{
	...
	U32 bPresetMember : 1;	// This member was cached into the group before matching was done. It is already assigned to the group and cannot be auto-balanced out of it.
							// Smart grouping WILL take this member into consideration.
							// Invitations/join confirmations will NOT be sent to these members.
							// (We used to track this by changing iGroupSize and iMatchSize without adding things to eaMembers in QueueGroup)
...
{
	QueueMatchMember** eaMatchMembers;
