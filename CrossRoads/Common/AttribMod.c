...
{
	ParserSetTableInfo(parse_AttribModDef_ForExpr, sizeof(AttribModDef), "AttribModDef_ForExpr", NULL, __FILE__, SETTABLEINFO_ALLOW_CRC_CACHING);
	ParserSetTableInfo(parse_AttribMod_ForExpr, sizeof(AttribMod), "AttribMod_ForExpr", NULL, __FILE__, SETTABLEINFO_ALLOW_CRC_CACHING);
...
{
	return EntityAngleToSourcePosUtil(pent, pmod->vecSource);
}
							 ...
							 PowerDef *ppowdefTarget)
{
	...
	if (bRet && pmoddefTarget && pmoddefTarget->pExprIgnores)
	{
		PowerDef *ppowdefAffects = pmoddefAffects->pPowerDef;
		PERFINFO_AUTO_START_FUNC();
		combateval_ContextSetupAffects(pchar,pmodTarget,ppowdefAffects,pmoddefAffects,pmodAffects,NULL);
		bRet = (0.f==combateval_EvalNew(iPartitionIdx, pmoddefTarget->pExprIgnores,kCombatEvalContext_Affects,NULL));
		PERFINFO_AUTO_STOP();
	}
					...
					SA_PARAM_OP_VALID ExprContext **ppOutContext)
{
	...
	if(pdef->eType!=kModType_None)
	{
		...
		if(papp->ppStrengths)
		{
			...
			for(i=0; i<s; i++)
			{
				if(pdef->pPowerDef == GET_REF(papp->ppStrengths[i]->hdef))
				{
					...
					if(iIdx < eafSize(&pStrength->pfModCritThreshold))
						fCritThreshold = pStrength->pfModCritThreshold[pdef->uiDefIdx];
					break;
				}
			}
		}

		if(!fStr || eaSize(&ppEnhancements))
		{
			...
			fStr = moddef_GetStrength(	iPartitionIdx, pdef, papp->pcharSource, papp->pdef, 
										...
										erTargetMod, NULL, false, false, &eFlags, &fStrAdd);

			if (fSavedStr)
				fStr = fStr - 1.0f + fSavedStr;
		}
#if GAMESERVER 
		...
		if(!papp->bLevelAdjusting && !pcharTargetApp->bLevelAdjusting)
		{
			...
			Entity *eTarget = pcharTargetApp->pEntParent;
			
			// get the root owners of these entities
			eOwner = entGetRootOwner(eOwner);
			eTarget = entGetRootOwner(eTarget);
			
			if(eOwner && eOwner->pChar && eTarget && eTarget->pChar &&
				!CombatMod_ignore(iPartitionIdx, eOwner->pChar, eTarget->pChar))
			{
				int iSourceLevel = entity_GetCombatLevel(eOwner);
				int iTargetLevel = entity_GetCombatLevel(eTarget);
		...
		// If this power is flagged to reroll crits from the strength's crit threshold, do so here
		if(papp->ppStrengths && fCritThreshold > -1)
		{
			papp->critical.fThreshold = fCritThreshold;
			papp->critical.fRoll = randomPositiveF32();
			papp->critical.bSuccess = papp->critical.fRoll < papp->critical.fThreshold;

			if(papp->critical.bSuccess)
...
// Returns a list of all PowerDef that are associated with the given AttribModDef
										 ...
										 SA_PARAM_NN_VALID PowerDef ***pppPowerDefsToSave)
{
	...
	switch (pdef->offAttrib)
	{
		...
		{
			Projetile_GetPowerDefs(pdef, pppPowerDefsToSave);
		}
	}
#endif
}
									  ...
									  EntityRef erTargetMod)
{
	...
	if(i<0)
	{
		...
		S32 bStrengths = false, bStrAdds = false, bFlags = false, bCrits = false;
		...
		bIsEntCreate = (pmod->pDef->offAttrib==kAttribType_EntCreate);
		...
		else
		{
			...
			eafSetSize(&pStrength->pfModCritThreshold,s);
			for(i=0; i<s; i++)
			{
				...
				F32 fStrength = moddef_GetStrength(	iPartitionIdx, pmoddef, papp->pcharSource, pdef, 
													iLevelMain, iLevelInline, papp->fTableScale, 
													NULL, erTargetMod, NULL, false, false, &eFlags, &fStrAdd);

				pStrength->pfModStrengths[i] = fStrength;
				pStrength->pfModStrAdd[i] = fStrAdd;
				pStrength->piModFlags[i] = eFlags;
				pStrength->pfModCritThreshold[i] = papp->critical.fThreshold;

				bStrengths |= (fStrength != 1);
				bStrAdds |= (fStrAdd != 0);
				bFlags |= (eFlags != 0);

				if(pmod->pDef && bIsEntCreate)
				{
					EntCreateParams *pParams = (EntCreateParams*)(pmod->pDef->pParams);
					if (pParams->bAlwaysRerollCrits)
					{
						bCrits |= (papp->critical.fThreshold != -1);
					}
				}

				if(pmoddef->bSaveApplyStrengths || (bIsEntCreate && pmoddef->bEntCreateSaveApplyStrengths) )
					ModDefCheckSaveApplyStrength(pmoddef,&ppPowerDefRecurse);
			}
			...
			if(!bCrits)
				eafDestroy(&pStrength->pfModCritThreshold);
		}
		...
		for(i=0; i<s; i++)
		{
			PowerDefSaveApplyStrength(iPartitionIdx,ppPowerDefRecurse[i],pmod,papp,iLevelMain,iLevelInline,erTargetMod);
		}
...
//builds an attrib mod. calls ModFill() for most of the stats calculations; 
// the code here is mostly conditionally copying fields into the AttribMod data structure.
						   ...
						   SA_PARAM_OP_VALID AttribMod*** peaLinkedModsOut)
{
	...
	// new (neverwinter) scary monster level adjusting:
	if (papp->bLevelAdjusting || pcharTarget->bLevelAdjusting)
	{
		// TODO: add a way for this to also effect healing, maybe map-wide?

		//only scale damage AttribMods:
		if (pdef->offAttrib < kAttribType_HitPointsMax && pdef->offAspect == kAttribAspect_BasicAbs)	
		{
			Entity* sourceEnt = entFromEntityRef(iPartitionIdx, papp->erModOwner);
			Entity* targetEnt = pcharTarget->pEntParent;
			int sourceLevel = 0, targetLevel = 0;

			if(sourceEnt && sourceEnt != targetEnt && ! scp_EntIsSuperCritterPet(sourceEnt))
			{
				// this is kinda weird: we will not scale SCP attacks but we will scale damage to SCP's. 
				if (targetEnt && scp_EntIsSuperCritterPet(targetEnt))
				{
					targetLevel = scp_GetPlayerLevelEquivalentOfPetLevel(entity_GetCombatLevel(targetEnt));
				}
				else
				{
					targetLevel = entity_GetCombatLevel(targetEnt);
				}

				sourceLevel = entity_GetCombatLevel(sourceEnt);

				if (papp->bLevelAdjusting)
				{
					// incoming damage from scary monster to player
					pmod->fMagnitude *= combatConfig_getPlayerDamageInNormalizer(sourceLevel, targetLevel);
				}
				else
				{
					// outgoing damage from player to scary monster
					pmod->fMagnitude *= combatConfig_getPlayerDamageOutNormalizer(sourceLevel, targetLevel);
				}
			}
		}
	}
	...
	if(pdef->bSaveApplyStrengths || 
		(pdef->bEntCreateSaveApplyStrengths && papp->pcharSource && papp->pcharSource->uiPowersCreatedEntityTime))
	{
		int i,s;
		// We use the level of the Application's root Power here, because we're calculating Strengths.
		//  The level is only used for the tables on inline enhancements, which are
		...
		for(i=0; i<s; i++)
		{
			PowerDefSaveApplyStrength(	iPartitionIdx, s_ppPowerDefsToSave[i], pmod, papp, 
										iLevelTable, iLevelInline, erTargetMod);
		}
	...
	if(pdef->bSaveSourceDetails)
	{
		PERFINFO_AUTO_START("SaveSourceDetails",1);
		{
			...
			if (papp->pcharSource && papp->pcharSource->pSavedCritical && 
				papp->pdef->eType == kPowerType_Passive)
			{
				StructCopyFields(parse_PACritical, papp->pcharSource->pSavedCritical, pmod->pSourceDetails->pCritical, 0, 0);
			}			
			else
			{
									 ...
									 S64 *plTimerExcludeAccum)
{
	...
	PERFINFO_AUTO_START_FUNC();
	{
		...
		if( pcharTarget->bLevelAdjusting )
		{
			papp->bTargetIsLevelAdjusting = true;
		}
												...
												SA_PARAM_NN_VALID AttribMod *pmod, SA_PARAM_NN_VALID AttribMod *pmodPrior, S32 iModIndex)
{
	...
	else
	{
		...
		if(pmod->pDef && pmod->pDef->pExpiration && !pmod->pDef->pExpiration->bPeriodic)
			character_ApplyModExpiration(iPartitionIdx, pchar, pmodPrior, pExtract);
...
#define MOD_IS_WORSE(pmodA,pmodB) ((pmodA)->fDuration<(pmodB)->fDuration || ((pmodA)->fDuration==(pmodB)->fDuration && (pmodA)->uiApplyID<(pmodB)->uiApplyID))
						...
						if(uiStackLimit)
						{
							...
							// the only reason I keep track of this here is in case this is the ONLY mod, and it's expired, for the extend/refresh case below
							iPriorModIdx = j;
							pmodPrior = pmodCurrent;
							eaPush(&ppAllMatchingMods,pmodCurrent);
						}
						else
						{
				...
				// Special rule for RefreshDuration: ADD a stack, AND refresh the old stacks
				if(!bStacksFull && (pdef->eStack != kStackType_RefreshDuration || !ppAllMatchingMods))
				{
					// We're not at the stack limit (or stacking is not allowed, and there are none currently), so just push it
					eaPush(&pchar->modArray.ppMods,pmod);
					entity_SetDirtyBit(pchar->pEntParent, parse_Character, pchar, false);
				}
				else
				{
					switch(pdef->eStack)
					{
						...
						{
							character_ModsProcessPendingDiscard(iPartitionIdx, pchar, pExtract, pmod, pmodPrior, iPriorModIdx);
							break;
						}
						case kStackType_Extend:
						case kStackType_RefreshDuration:
						{
							// at the time I am writing this, kStackType_Extend is not allowed with stacking, but I've made the code below
							// not care.  [RMARR - 7/3/13]

							if (eaSize(&ppAllMatchingMods) == 1 && pmodPrior->fDuration < 0 && pmodPrior->fDuration!=kModExpirationReason_Duration)
							{
								// we only have one prior mod and it's expired, so just replace it
								mod_Replace(iPartitionIdx, pchar, pmod, pmodPrior, iPriorModIdx, pExtract);
							}
							else
							{
								// Special rule for RefreshDuration: ADD a stack, AND refresh the old stacks
								for (j=0;j<eaSize(&ppAllMatchingMods);j++)
								{
									pmodPrior = ppAllMatchingMods[j];
									...
									{
										if(pdef->eStack == kStackType_RefreshDuration)
										{
											MAX1(pmodPrior->fDuration,0);

											// only refresh to the mod duration. If its greater ignore it
											if(pmodPrior->fDuration < pmod->fDuration)
											{
												pmodPrior->fDuration = pmod->fDuration;
											}
										}	
										else	// kStackType_Extend
										{
						...
						{
							character_ModsProcessPendingReplace(iPartitionIdx, pchar, pExtract, pmod, pmodPrior, iPriorModIdx);
							break;
						}
						case kStackType_KeepBest:
						{
							S32 iCompare = mods_CompareMagnitude(pmod, pmodPrior);
							if (iCompare > 0)
							{
								character_ModsProcessPendingReplace(iPartitionIdx, pchar, pExtract, pmod, pmodPrior, iPriorModIdx);
							}
							...
							{
								if (iCompare == 0 && pmod->fDuration > pmodPrior->fDuration)
								{
									// magically act like a "refresh duration" in this case.
									pmodPrior->fDuration = pmod->fDuration;
								}
								// Discard the new mod
								character_ModsProcessPendingDiscard(iPartitionIdx, pchar, pExtract, pmod, pmodPrior, iPriorModIdx);
...
void character_ModsUpdateNet(Character *pchar)
{
	...
	bool bSend = pchar->bSendAllAttribModNet;
	...
	for(j=t-1; j>=0; j--)
	{
		...
		ModUpdateNet(pchar,s_ppMods[i],pnet,bSend);
	...
	pchar->bSendAllAttribModNet = false;
...
static int Character_ModPostLoad(SA_PARAM_NN_VALID Character *pchar, SA_PARAM_OP_VALID AttribMod *pmod, S32 bLevelDecreased)
{
	if(pmod && pmod->uiVersion && !pmod->bDisabled)
	{
		...
		if(pdef && pmod->uiVersion==pdef->uiVersion)
		{
			...
			if(pmod->fDuration > pmod->fDurationOriginal && (pmoddef->eStack!=kStackType_Extend && pmoddef->eStack!=kStackType_RefreshDuration))
			{
				devassertmsg(0,"Loading AttribMod with more duration than original duration that doesn't extend or RefreshDuration");
				return false;
			}
...
// --------------------------------------------------------------------------------------------------------------------
bool moddef_ShouldSaveApplyStrengths(AttribModDef *pmoddef)
{
	return ((pmoddef->offAttrib==kAttribType_EntCreate && pmoddef->pParams && ((EntCreateParams*)pmoddef->pParams)->eStrength==kEntCreateStrength_Locked)
			|| pmoddef->offAttrib==kAttribType_ProjectileCreate);

}

// --------------------------------------------------------------------------------------------------------------------
bool moddef_ShouldSaveApplyStrengthsForEntCreate(AttribModDef *pmoddef)
{
	return  (moddef_ShouldSaveApplyStrengths(pmoddef)
				|| pmoddef->pExpiration
				|| pmoddef->offAttrib==kAttribType_ApplyPower
				|| pmoddef->offAttrib==kAttribType_DamageTrigger
				|| pmoddef->offAttrib==kAttribType_KillTrigger
				|| pmoddef->offAttrib==kAttribType_TeleThrow
				|| pmoddef->offAttrib==kAttribType_TriggerComplex
				|| pmoddef->offAttrib==kAttribType_TriggerSimple);
}
