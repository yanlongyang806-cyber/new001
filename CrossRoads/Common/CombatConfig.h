...
typedef struct TimerConfig
{
	...
	U32 bPlayersIgnoreTimerUse : 1;
		// Using a power will not put players into combat.  Presumably, TimerUseVisual will be non-zero
} TimerConfig;
...
typedef struct CombatConfig
{
	...
	F32 fFlightPitchClampMin;
		// Used to clamp the minimum value for the flight pitch clamp user setting
	...
	// player level damage and health normalizing values for scary monster tech:
	FLOAT_EARRAY eafPlayerDamageOutNormalizers;	AST(NAME(PlayerDamageOutNormalizers))
	FLOAT_EARRAY eafPlayerDamageInNormalizers;	AST(NAME(PlayerDamageInNormalizers))
