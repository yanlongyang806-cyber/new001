	...
	if (pEntity!=NULL && pMemberJoinCriteria!=NULL)
	{
		if (pEntity && pEntity->pChar)
		{
			...
			if (pClass && pClass->pchName)
			{
				...
				else if (stricmp(pClass->pchName, "Player_Archer")==0)
				{
					pMemberJoinCriteria->iGroupRole = 3;   // dps
					pMemberJoinCriteria->iGroupClass = 6;
				}
				else if (stricmp(pClass->pchName, "Player_Scourge")==0)
				{
					pMemberJoinCriteria->iGroupRole = 3;   // dps
					pMemberJoinCriteria->iGroupClass = 7;
				}
				else
				{
					pMemberJoinCriteria->iGroupRole = 3;   // dps
					pMemberJoinCriteria->iGroupClass = 8;  // essentially 'other'
				}
			}
		}
	}
...
// Allow no more than two of any particular class. (PvP default generally)
bool NNO_aslQueue_IsSmartGroupToAddTo_TwoOfClass(QueueDef* pQueueDef, QueueGroup* pGroup, int iGroupMaxSize, QueueMember* pMember, int iTeamSize,
														QueueMember ***peaReadyMembers)
{
	...
	{
		aiExistingRoles[iExistingMembers] = pGroup->eaMatchMembers[i]->iGroupRole;
		aiExistingClasses[iExistingMembers] = pGroup->eaMatchMembers[i]->iGroupClass;
		iExistingMembers++;
	}
...
// Look for one tank, one healer, and allow no more than two of any particular class. (PvP default generally)
bool NNO_aslQueue_IsSmartGroupToAddTo_TankHealer(QueueDef* pQueueDef, QueueGroup* pGroup, int iGroupMaxSize, QueueMember* pMember, int iTeamSize,
														QueueMember ***peaReadyMembers)
{
														...
														QueueMember ***peaReadyMembers)
{
	...
	if (pQueueDef!=NULL)
	{
		if (pQueueDef->MapRules.eSmartGroupingMode==kQueueSmartGroupingMode_TankHealer)
		{
			return(NNO_aslQueue_IsSmartGroupToAddTo_TankHealer(pQueueDef, pGroup, iGroupMaxSize, pMember, iTeamSize, peaReadyMembers));
		}
		else if (pQueueDef->MapRules.eSmartGroupingMode==kQueueSmartGroupingMode_TwoOfClass)
		{
			return(NNO_aslQueue_IsSmartGroupToAddTo_TwoOfClass(pQueueDef, pGroup, iGroupMaxSize, pMember, iTeamSize, peaReadyMembers));
		}
	}
	return(true);
}
