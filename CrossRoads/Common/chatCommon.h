...
3. Update the color in data/defs/config/ChatDefaults.def
4. If the type should appear in the Client UI Filter list, it must be added to data/defs/config/ChatTypeList.def 
   for each game project
5. Update the kChatLogEntryType_Count value immediately after the AUTO_ENUM
6. Make a new GlobalChatServer build (probably ask Theo to do it)
******/
AUTO_ENUM;
typedef enum ChatLogEntryType
{
	...
	kChatLogEntryType_Trade,		// Zone trade chat.
	kChatLogEntryType_LookingForGroup,	// Zone LFG chat.
	kChatLogEntryType_AntiAddiction, //Anti-addiction chat channel (needs to be red)
} ChatLogEntryType;
extern StaticDefineInt ChatLogEntryTypeEnum[];
// This MUST be changed whenever you append a value to ChatLogEntryType!
#define kChatLogEntryType_Count (kChatLogEntryType_AntiAddiction+1)
...
// Please make sure these channel name prefixes start with the command that is most likely used to send messages in them.
// Like, "/officer" should talk in a channel that starts with "OFFICER".
#define LOCAL_CHANNEL_NAME "Local"
#define ZONE_CHANNEL_NAME "Zone"
#define TRADE_CHANNEL_NAME "Trade"
#define LFG_CHANNEL_NAME "LookingForGroup"
#define PRIVATE_CHANNEL_NAME "Private"
#define PRIVATE_CHANNEL_SENT_NAME "PrivateSent"
#define ZONE_CHANNEL_PREFIX "ZONE_"
#define TRADE_CHANNEL_PREFIX "TRADE_"
#define LFG_CHANNEL_PREFIX "LookingForGroup_"
#define TEAM_CHANNEL_PREFIX "TEAMID_"
#define GUILD_CHANNEL_PREFIX "GUILDID_"
#define OFFICER_CHANNEL_PREFIX "OFFICERID_"
...
#define UGCEDIT_CHANNEL_SHORTCUT "Foundry"
...
#define CHAT_TRADE_SHORTCUT "Trade"
#define CHAT_LFG_SHORTCUT "LookingForGroup"
...
#define CHANNEL_SPECIAL_TRADE			0x100
#define CHANNEL_SPECIAL_LFG				0x200
...
typedef struct ChatConfig
{
	...
	const bool bDisableStickyChannels : 1;		AST(PERSIST)
...
AUTO_STRUCT;
typedef struct EmailWrapper {
	U32 uSent;
	U32 uExpireTime;
	STRING_MODIFIABLE pchSubject;
	STRING_MODIFIABLE pchBody;
	STRING_MODIFIABLE pchSenderName;
	U32 uLotID;
	U32 eTypeOfEmail;
	bool bRead : 1;
} EmailWrapper;
...
AUTO_STRUCT;
typedef struct EmailWrappers {
	U32 accountID;
	U32 containerID;
	EmailWrapper** eaEmailWrappers;
} EmailWrappers;
...
char *zone_GetTradeChannelNameFromMapName(char *pcBuffer, int iBufferSize, SA_PARAM_NN_STR const char *mapShortName, int mapInstanceIndex, ZoneMapType eMapType, int iVirtualShardID);
char *zone_GetLFGChannelNameFromMapName(char *pcBuffer, int iBufferSize, SA_PARAM_NN_STR const char *mapShortName, int mapInstanceIndex, ZoneMapType eMapType, int iVirtualShardID);
