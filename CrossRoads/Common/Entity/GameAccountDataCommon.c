...
void GameAccountDictChanged(enumResourceEventType eType, const char *pDictName, const char *pRefData, Referent pReferent, void *pUserData)
{
	...
	else if (eType == RESEVENT_RESOURCE_MODIFIED)
	{
		if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
		{
			gslGatewayServer_GameAccountDataModified(pData);
		}
	}
#endif
...
bool GAD_Login2CanMakeNumericPurchase(S32 numericValue, ContainerID iVirtualShardID, GameAccountData* pData, GameAccountDataNumericPurchaseDef* pPurchaseDef, bool bCheckKeyValues)
{
	if (!iVirtualShardID && pPurchaseDef)
	{
		if (numericValue < pPurchaseDef->iNumericCost)
		{
			return false;
		}
		else if (bCheckKeyValues)
