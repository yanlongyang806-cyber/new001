...
int team_GetTeamListSelfFirst(Entity* pEnt, Entity*** peaTeamEnts, TeamMember*** peaTeamMembers, bool bIncludeSelf, bool bIncludePets)
{
	...
	if (bIncludePets)
	{
		iSelfPetCount = team_GetOnMapPetList(iPartitionIdx, pEnt, peaTeamEnts, peaTeamMembers);
		iPetCount += iSelfPetCount;
	}
		...
		for (i = 0; i < iTeamSize; i++)
		{
			...
			if (pTeamMember->iEntID != entGetContainerID(pEnt))
			{
				...
				if (pTeamEnt)
				{
					...
					if (bIncludePets)
					{
						iPetCount += team_GetOnMapPetList(iPartitionIdx, pTeamEnt, peaTeamEnts, peaTeamMembers);
					}
				}
			}
		}
	...
	if (bIncludePets == 2 && iCount + iPetCount + (!bIncludeSelf) > TEAM_MAX_SIZE)
	{
		for (i = eaSize(peaTeamEnts) - 1; i >= 0; i--)
		{
			if (entGetType((*peaTeamEnts)[i]) != GLOBALTYPE_ENTITYPLAYER && entGetRef(pEnt) != (*peaTeamEnts)[i]->erOwner)
			{
				eaRemove(peaTeamEnts, i);
				if (peaTeamMembers)
					eaRemove(peaTeamMembers, i);
				--iPetCount;
			}
		}
	}

	return iCount + iPetCount;
}
