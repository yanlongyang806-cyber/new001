...
typedef struct ActiveSuperCritterPet
{
	...
	const U32	uiTimeFinishTraining;		AST(PERSIST SUBSCRIBE SELF_ONLY)
...
typedef struct PetRelationship
{
	...
	bool bNeedsFixup;									NO_AST
	CONST_INT_EARRAY eaPurposes;						AST(PERSIST)

	// The following is to allow the client to see some data and do less pet subscriptions
	char *pcName;										AST(PERSIST NO_TRANSACT SUBSCRIBE SELF_ONLY)
	REF_TO(CharacterClass) hCharacterClassRef;			AST(PERSIST NO_TRANSACT SUBSCRIBE SELF_ONLY REFDICT(CharacterClass))	

	// The pet def. Used for some expressions directly and for checking other information about the pet (like always subscribe)
	REF_TO(PetDef) hPetDefRef;							AST(PERSIST NO_TRANSACT SUBSCRIBE SELF_ONLY REFDICT(PetDef))	
...
typedef struct PuppetMaster
{
	...
	U32 bPuppetSubscribeAll : 1;						AST(SELF_ONLY)			// all puppets should be subscribed
	U32 bPuppetClientWantsAll : 1;						NO_AST					// Set by the client when it wants all, will clear to zero when bPuppetSubscribeAll is set
...
typedef struct PlayerCostumeData
{
	...
	const U8 iActiveCostume;									AST(PERSIST SUBSCRIBE SELF_AND_TEAM_ONLY NAME("ActiveCostume") USERFLAG(TOK_PUPPET_NO_COPY))
	...
	CONST_EARRAY_OF(PlayerCostumeSlot) eaCostumeSlots;			AST(PERSIST SUBSCRIBE SELF_AND_TEAM_ONLY NAME("CostumeSlot") FORCE_CONTAINER NO_INDEX USERFLAG(TOK_PUPPET_NO_COPY))
...
typedef struct SavedEntityData
{
	...
	bool								bSubscribeAllPets : 1;		AST(SELF_ONLY)	// we need to subscribe all pets (non-puppet), set on game server when all are being subscribed
	bool								bClientWantsAllPets : 1;	NO_AST			// client side only flag
	bool								bSavedPetFixupInProgress : 1;		NO_AST // true while fixing up a pet. Cleared when a fix-up finishes
