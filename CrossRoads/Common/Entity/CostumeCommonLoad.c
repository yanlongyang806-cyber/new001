...
bool costumeLoad_ValidatePlayerCostume(PlayerCostume *pCostume, SpeciesDef *pSpecies, bool bInEditor, bool bLoading, bool bCheckFX)
{
	...
	// check that colors are valid player colors
	if (pCostume->eCostumeType == kPCCostumeType_Player)
	{
		bool bColorOk = true;
		UIColorSet * pColorSetSkel = GET_REF(pSkeleton->hSkinColorSet);
		UIColorSet * pColorSetSpecies = pSpecies ? GET_REF(pSpecies->hSkinColorSet) : NULL;
		if (pColorSetSkel || pColorSetSpecies)
		{
			bColorOk = false;
			if (pColorSetSkel && costumeLoad_ColorInSet(pCostume->skinColor, pColorSetSkel))
				bColorOk = true;

			// So if pSpecies is NULL, but hSpecies is non-NULL, we just give up on validation, cause we have to
			if (IS_HANDLE_ACTIVE(pCostume->hSpecies))
			{
				if (pColorSetSpecies == NULL || costumeLoad_ColorInSet(pCostume->skinColor, pColorSetSpecies))
					bColorOk = true;
			}
		}
		...
		if(!bColorOk)
		{
			ErrorFilenamef(pCostume->pcFileName, "Costume '%s' has an invalid player skin color\n", pCostume->pcName);
		}
	}
...
void costumeLoad_LoadData(void)
{
	...
	if (IsGameServerBasedType() || IsLoginServer() || IsAuctionServer() || bLoadCostumes) {
		// Also load from maps/ so that UGC-style bundles work.
		resLoadResourcesFromDisk(g_hPlayerCostumeDict, COSTUMES_BASE_DIR ";maps", ".costume", NULL, RESOURCELOAD_SHAREDMEMORY | PARSER_OPTIONALFLAG);
