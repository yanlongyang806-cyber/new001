...
ATR_LOCKS(pPetEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, pInventoryV2.ppInventoryBags[], .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Hallegiance, .Hsuballegiance, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds")
...
bool Entity_CanRenamePet(SA_PARAM_NN_VALID Entity* pPlayerEnt, SA_PARAM_NN_VALID Entity* pPetEnt)
{
	...
	PetDef* pDef = pPetEnt && pPetEnt->pCritter ? GET_REF(pPetEnt->pCritter->petDef) : NULL;

	if(!pDef || pDef->bPermanentName)
	{
		// can't rename pets with permanent names
		return false;
	}
...
AUTO_TRANS_HELPER;
NOCONST(AlwaysPropSlot)* AlwaysPropSlot_trh_FindByPetID(ATH_ARG NOCONST(Entity)* pEntity, U32 uPetID, U32 uPuppetID, AlwaysPropSlotCategory eCategory)
{
	S32 i;
...
AUTO_TRANS_HELPER;
void AlwaysPropSlot_trh_FindAllByPetID(ATH_ARG NOCONST(Entity)* pEntity, U32 uPetID, U32 uPuppetID, NOCONST(AlwaysPropSlot)*** peaSlots)
{
	S32 i;
	for (i = eaSize(&pEntity->pSaved->ppAlwaysPropSlots)-1; i >= 0; i--)
	{
		...
		{
			eaPush(peaSlots, pEntity->pSaved->ppAlwaysPropSlots[i]);
		}
	}
}

AUTO_TRANS_HELPER;
NOCONST(AlwaysPropSlot)* AlwaysPropSlot_trh_FindBySlotID(ATH_ARG NOCONST(Entity)* pEntity, U32 uSlotID)
{
	S32 i;
	for (i = eaSize(&pEntity->pSaved->ppAlwaysPropSlots)-1; i >= 0; i--)
	{
...
Entity* SavedPet_GetEntityFromPetID(Entity* pOwner, U32 uiPetID)
{
	...
	for (i = 0; i < iPetArraySize; i++)
	{
		...
		else if (!IS_HANDLE_ACTIVE(pPet->hPetRef) && pPet->uiPetID == uiPetID)
		{
#if GAMECLIENT
			if (!pOwner->pSaved->bClientWantsAllPets && !pOwner->pSaved->bSubscribeAllPets)
			{
				pOwner->pSaved->bClientWantsAllPets = true;
...
// Get the ent from the PuppetEnt. This will cause the server to look for a subscription or the client to ask for one if the ref is NULL
Entity *Entity_GetEntFromPuppetOrPet(Entity* pEnt, PuppetEntity* pNewPuppet, PetRelationship *pPetRelationShip)
{
	Entity* pNewPuppetEnt;

	// Check the pet relationship first
	if(pPetRelationShip)
	{
		pNewPuppetEnt = GET_REF(pPetRelationShip->hPetRef);	
		if(pNewPuppetEnt)
		{
			return pNewPuppetEnt;
		}
	}

	if(pNewPuppet)
	{
		pNewPuppetEnt = GET_REF(pNewPuppet->hEntityRef);
		if(pNewPuppetEnt)
		{
			return pNewPuppetEnt;
		}

		if(IS_HANDLE_ACTIVE(pNewPuppet->hEntityRef))
		{
...
#ifdef GAMECLIENT
	if(pEnt && pEnt->pSaved && pEnt->pSaved->pPuppetMaster && pNewPuppet && !pEnt->pSaved->pPuppetMaster->bPuppetClientWantsAll &&
		!pEnt->pSaved->pPuppetMaster->bPuppetSubscribeAll)
	{
		// mark the fact we asked for subscriptions
		pEnt->pSaved->pPuppetMaster->bPuppetClientWantsAll = true;
		ServerCmd_SubscribeAllPuppets();
	}
	else if (pEnt && pEnt->pSaved && pEnt->pSaved->pPuppetMaster && !pNewPuppet && !pEnt->pSaved->bClientWantsAllPets && !pEnt->pSaved->bSubscribeAllPets)
	{
		ServerCmd_SubscribeAllPets();
	}
#endif
	...
	if(pNewPuppet)
	{
		// ask for all puppets to be subscribed, does nothing if subscriptions have already been asked for
...
void SavedPet_SubscribeThisPet(Entity *pOwner, GlobalType petType, ContainerID petId)
{
	if(pOwner && pOwner->pSaved && pOwner->pSaved->pPuppetMaster && petType == GLOBALTYPE_ENTITYSAVEDPET)
	{
		S32 i;
		...
		{
			PetRelationship *pPet = pOwner->pSaved->ppOwnedContainers[i];
			if(pPet && pPet->conID == petId)
			{
				if(!IS_HANDLE_ACTIVE(pPet->hPetRef))
				{
					if(SavedPet_IsPetAPuppet(pOwner, pPet))
					{
#ifdef GAMESERVER
...
bool Entity_CanSetAsActivePuppet(Entity* pEnt, PuppetEntity* pNewPuppet)
{
	...
	if (pNewPuppet)
	{
		Entity* pNewPuppetEnt = Entity_GetEntFromPuppetOrPet(pEnt, pNewPuppet, NULL);
...
bool savedpet_ValidateFormalName(Entity *ent, const char *pcFormalName, char **ppEStringError)
{
	...
	for (i = 0; i < c_iNameParts; i++)
	{
		...
		if (text[0] && (strerr = StringIsInvalidFormalName(text, s_aiNameIndexes[i][iOrder], entGetAccessLevel(ent))))
		{
...
bool Entity_CanAcceptPetTransfer(Entity* pDstEnt, Entity* pPetEnt, GameAccountDataExtract *pExtract, char** pestrError)
{
	int iPetRank = 0;
	PetDef* pPetDef = NULL;
	AllegianceDef *pDstAllegiance = pDstEnt ? GET_REF(pDstEnt->hAllegiance) : NULL;
	AllegianceDef *pDstSubAllegiance = pDstEnt ? GET_REF(pDstEnt->hSubAllegiance) : NULL;
	AllegianceDef *pPetAllegiance = pPetEnt ? GET_REF(pPetEnt->hAllegiance) : NULL;
	...
	// Cannot transfer pets if there is no matching allegiance
	if (pPetAllegiance && pPetAllegiance != pDstAllegiance && pPetAllegiance != pDstSubAllegiance)
	{
		entFormatGameMessageKey(pDstEnt, pestrError, "TradeError_AllegianceMismatch",
			STRFMT_PLAYER(pDstEnt),
			STRFMT_ENTITY_KEY("Pet", pPetEnt),
			STRFMT_END);
		return false;
...
// Does this pet relationship need to be subscribed?
bool SavedPet_DoesPetNeedToBeSubscribed(Entity *pPlayerEnt, PetRelationship *pPetRelationship)
{
	if(pPlayerEnt && pPlayerEnt->myEntityType == GLOBALTYPE_ENTITYPLAYER && pPlayerEnt->pSaved && pPlayerEnt->pSaved->pPuppetMaster && pPetRelationship)
	{
		PuppetEntity *pPuppet;
		PetDef *pPetDef;
		if(pPetRelationship->eState == OWNEDSTATE_ACTIVE)
		{
			return true;
		}

		pPuppet = SavedPet_GetPuppetFromPet(pPlayerEnt, pPetRelationship);
		if(pPuppet && pPuppet->eState == PUPPETSTATE_ACTIVE)
		{
			return true;
		}

		if(!pPetRelationship->pcName)
		{
			// this pet doesn't have all pet info it needs. It needs to be subscribed in order to get that information. SavePet_SetPetNames will do the actual setting of the required data
			return true;
		}

		if(!IS_HANDLE_ACTIVE(pPetRelationship->hPetDefRef))
		{
			// This information is needed to be filled in
			return true;
		}

		pPetDef = GET_REF(pPetRelationship->hPetDefRef);
		if(pPetDef && pPetDef->bAlwaysSubscribe)
		{
			return true;
		}

		// If this is the preferred member of the away team they need to be subscribed
		if(ea32Find(&pPlayerEnt->pSaved->ppPreferredPetIDs, pPetRelationship->uiPetID) >= 0)
		{
			return true;
		}
