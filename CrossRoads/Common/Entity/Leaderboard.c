...
void leaderboard_generateAll(void)
{
	...
	if(!s_pContext)
	{
		...
		s_pFuncTable = exprContextCreateFunctionTable("Leaderboard");
...
LeaderboardStats *entity_newLeaderboardStats(Entity *pEntity, LeaderboardDef *pDef)
{
	if(pEntity && pEntity->pSaved && pDef)
	{
		...
		pNew->iLastTimestamp = ShardClock_SecondsSince2000();
		pNew->fPoints = 0.f;
...
{
	if (pDef->eLeaderboardIntervalMode==kShardIntervalTimingMode_Off)
	{
	...
	{
		return(ShardIntervalTiming_IntervalNumOfTime(uTime, pDef->eLeaderboardIntervalMode));
	}
	return(-1);
}
...
void entity_SetNewLeaderboardPointValue(Entity *pEntity, LeaderboardStats *pStats, F32 fPointValue, bool bNotify)
{
	if(pEntity && pStats)
	{
...
#ifdef GAMESERVER
			if(leaderboard_GetIntervalFromTime(pDef,pStats->iLastTimestamp) < leaderboard_GetIntervalFromTime(pDef,ShardClock_SecondsSince2000()))
			{
		...
		//update timestamps
		pStats->iLastTimestamp = ShardClock_SecondsSince2000();
	}
}
...
void entity_updateLockedLeaderboardStats(Entity *pEntity)
{
	...
	for(i=0;i<eaSize(&g_sLeaderboardDefs.ppLeaderboards);i++)
	{
		...
		for(j=0;j<eaSize(&pEntity->pSaved->ppLeaderboardStats);j++)
		{
			...
			{
				if(leaderboard_GetIntervalFromTime(g_sLeaderboardDefs.ppLeaderboards[i],pEntity->pSaved->ppLeaderboardStats[j]->iLastTimestamp) < leaderboard_GetIntervalFromTime(g_sLeaderboardDefs.ppLeaderboards[i],ShardClock_SecondsSince2000()))
				{
					entity_updateLeaderboardStats(pEntity,g_sLeaderboardDefs.ppLeaderboards[i]);
				}
