...
static void RegionRules_Init()
{
	...
	*ppTempChoiceContext = exprContextCreate();
	...
	stTable = exprContextCreateFunctionTable("RegionRules");
...
RegionRules *getRegionRulesFromRegion(WorldRegion *pRegion)
{
	...
	if(pRegion)
	{
		...
		if(pOverride)
		{
			...
			if(pOverride->iUnteamedPetsPerPlayer != -1)
				pReturn->iUnteamedPetsPerPlayer = pOverride->iUnteamedPetsPerPlayer;
		...
		else
		{
			...
			pReturn->iUnteamedPetsPerPlayer = -1;
...
static bool entity_FindBestPuppetChoice(Entity *pEnt, RegionRules *pRegionRules, PuppetEntity **ppChoiceOut) 
{
	...
	for (i=eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets)-1;i>=0;i--)
	{
		...
		Entity *pPetEntCheck = SavedPuppet_GetEntity(entGetPartitionIdx(pEnt), pPuppetEntity);
		CharacterClass *pPetClass = pPetEntCheck && pPetEntCheck->pChar ? GET_REF(pPetEntCheck->pChar->hClass) : NULL;
		bPassCheck = bPassCheck && (pPetEntCheck || pPuppetEntity->eState != PUPPETSTATE_ACTIVE);
		if(pPetEntCheck && pPuppetEntity->eState == PUPPETSTATE_ACTIVE)
		{
			bool bValid = false;
...
// Returns the first RegionRules which allows the use of a power.
// Currently this is being used to pull measurement info for the power.
// The measurement info is also used for generic distance display for a particular
// region so it would be difficult to abstract the data completely.
// There will be problems if multiple regions use different measurement settings
//  on different regions that all support the same power.
RegionRules* regionRules_GetAnyRegionRulesValidForPower(PowerDef *pDef)
{
	...
	while (pRules = RefSystem_GetNextReferentFromIterator(&Iter))
	{
		if (powerdef_RegionAllowsActivate(pDef, pRules->eRegionType))
		{
			return(pRules);
		}
	}
...
// Returns the first RegionRules which Support the SchemeRegion
// Currently this is being used to pull camera distances in gclControlScheme.c
// There will be problems if multiple regions use different camera settings
//  on different regions that all support the same SchemeRegion
