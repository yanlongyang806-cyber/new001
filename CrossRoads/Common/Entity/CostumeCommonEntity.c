...
void costumeEntity_ResetStoredCostume( Entity *pEnt )
{
	if (IsServer() && pEnt && pEnt->pSaved) {
		PlayerCostume *pCostume = NULL;
		Entity *pCostumeEnt = pEnt;

		// Clean up previous stored costume
		StructDestroySafe(parse_PlayerCostume, &pEnt->costumeRef.pStoredCostume);

		// get puppet and see if it has a costume. If the puppet doesn't have a costume (captain) then use player costume.
		if(pEnt->pSaved->pPuppetMaster && GetCharacterClassEnum(pEnt) == StaticDefineIntGetInt(CharClassTypesEnum, "Space"))
		{
			Entity *pPuppet = NULL;
			S32 i;

			// Are we using the normal puppet?
			if(pEnt->pSaved->pPuppetMaster->curID)
			{
				...
				for(i = 0; i < eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets); ++i)
				{
					...
					{
						pPuppet = GET_REF(pEnt->pSaved->pPuppetMaster->ppPuppets[i]->hEntityRef);
						break;
					}
				}
			}
			else if(pEnt->pSaved->pPuppetMaster->curTempID)
			{
				...
				{
					if(pEnt->pSaved->pPuppetMaster->ppTempPuppets[i]->uiID == (int)pEnt->pSaved->pPuppetMaster->curTempID)
					{
						// temp puppets have a costume, no ent involved
						...
						if(pPetDef)
						{
							...
							{
								S32 j;
								for(j = 0; j < eaSize(&pCritterDef->ppCostume); ++j)
								{
									PlayerCostume *pPetCostume = GET_REF(pCritterDef->ppCostume[j]->hCostumeRef);
									if(pPetCostume)
									{
										pCostume = pPetCostume;
										break;
									}
								}
							}
			...
			// Never use captain puppet costume
			if(pPuppet && pPuppet->pSaved->costumeData.iActiveCostume < eaSize(&pPuppet->pSaved->costumeData.eaCostumeSlots) &&
				pPuppet->pSaved->costumeData.eaCostumeSlots[pPuppet->pSaved->costumeData.iActiveCostume]->pCostume)
			{
				// only use the puppets costume if it has them. Note that newly created characters have a ground puppet with slots but no costumes.
				pCostumeEnt = pPuppet;
			}
		}

		// Find preferred new stored costume if we don't already have one (which should have if we have a temp puppet)
		if(!pCostume && pCostumeEnt->pSaved->costumeData.iActiveCostume < eaSize(&pCostumeEnt->pSaved->costumeData.eaCostumeSlots))
		{
			pCostume = pCostumeEnt->pSaved->costumeData.eaCostumeSlots[pCostumeEnt->pSaved->costumeData.iActiveCostume]->pCostume;
		} 
...
ExprFuncTable* costumeEntity_CreateExprFuncTable()
{
	...
	{
		s_costumeFuncTable = exprContextCreateFunctionTable("CostumeEntity");
	...
	for (i = 0; i < eaSize(&pEnt->pSaved->ppOwnedContainers); i++)
	{
		...
		switch (eStorageType)
		{
			...
			acase kPCCostumeStorageType_Secondary:
				...
				{
					if (pPuppetEnt && pPuppetEnt->eState == PUPPETSTATE_ACTIVE && (CharClassTypes)pPuppetEnt->eType != eSpaceClassType)
						bInclude = true;
				}
			...
			xcase kPCCostumeStorageType_Nemesis:
				...
				if (!pPuppetEnt && pSubEnt && pSubEnt->pNemesis)
					bInclude = true;
		}

		if(bInclude && !pSubEnt)
		{
			// The following call will subscribe pets and puppets 
			pSubEnt = Entity_GetEntFromPuppetOrPet(pEnt, pPuppetEnt, pRelationship);
		}
...
void costume_FreeCostumeDisplayData(CostumeDisplayData* pData)
{
	if (pData)
	{
		eaDestroy(&pData->eaCostumes);
		eaDestroy(&pData->eaAddedFX);
		eaDestroy(&pData->eaStances);
