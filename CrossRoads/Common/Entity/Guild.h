...
typedef struct GuildConfig
{
	...
	bool bUseLeaderSlowDemotion;	AST(NAME("UseLeaderSlowDemotion"))

	// number of hours before demotion takes place for leader
	U32 uDemotionDurationHours;		AST(NAME("DemotionDurationHours"))

	// number of hours before destroying an empty guild
	U32 uDestroyWaitTimeHours;		AST(NAME("DestroyWaitTimeHours"))

	// number of hours for new leader to have reduced powers
	U32 uProvisionalTimeHours;		AST(NAME("ProvisionalTimeHours"))
...
typedef struct GuildMember
{
	...
	// the provisional time for a leader (time when the leader becomes a full leader)
	const U32 uProvisionalTimeSeconds;		AST(PERSIST SUBSCRIBE)
...
typedef struct GuildRecruitMember
{
	...
	S32 iRank;
	char *pcRankName;
	const char *pcDefaultRankName;	AST(POOL_STRING)
} GuildRecruitMember;
...
typedef struct Guild
{
	...
	// The container ID of the leader being demoted
	const ContainerID uDemoteLeaderID;					AST(PERSIST SUBSCRIBE)

	// The time the leader will be demoted (seconds)
	const U32 uDemotionTimeSeconds;						AST(PERSIST SUBSCRIBE)

	// The time the guild will be destroyed if empty, guild server only
	U32 uGuildDestroyTimeSeconds;						AST(PERSIST NO_TRANSACT)
...
bool aslGuild_trh_IsLeader(ATH_ARG NOCONST(Guild) *pGuild, U32 EntID);
