...
typedef enum ESurfaceSpeedPenaltyType
{
	// standard move speed penalty
	ESurfaceSpeedPenaltyType_DEFAULT,

	// when in the air, the multiplier applied to air friction
	ESurfaceSpeedPenaltyType_AIRFRICTION_MULT

} ESurfaceSpeedPenaltyType;
...
typedef struct SurfaceFGToBG {
	...
	// Strafing override flags, 
	// if the override is enabled
	U32								strafingOverrideEnabled		: 1;
	// when the override is enabled, if we are strafing or not 
...
typedef enum SurfaceStanceType {
	...
	MR_SURFACE_STANCE_JUMPRISING,
	MR_SURFACE_STANCE_JUMPFALLING,
...
typedef struct SurfaceLocalBGFlags {
	...
	U32								scheduledIsStrafingOverride			: 1;
	U32								scheduledStrafingOverrideEnabled	: 1;
	U32								scheduledDisableJump				: 1;
} SurfaceLocalBGFlags;
...
#define CASE(x, y) xcase x: *handleOut = mmAnimBitHandles.y
		...
		CASE(MR_SURFACE_STANCE_JUMPRISING, jumpRising);
		CASE(MR_SURFACE_STANCE_JUMPFALLING, jumpFalling);
#undef CASE
xdefault:{
		return 0;
		 }
										...
										S32 bufferLen)
{
	...
	if(eaSize(&localBG->speedPenaltiesQueued)){
		...
		EARRAY_CONST_FOREACH_BEGIN(localBG->speedPenaltiesQueued, i, isize);
		{
			...
			if(sp->flags.isStop){
				...
				pos += snprintf_s(	pos,
					STRBUF_REMAIN(extrasString, pos),
					"  START: id %u, type %u, spc %u, penalty %1.3f [%8.8x]\n",
					sp->id,
					sp->type,
					sp->spc,
					sp->penalty,
	...
	if(eaSize(&localBG->speedPenaltiesActive)){
		...
		EARRAY_CONST_FOREACH_BEGIN(localBG->speedPenaltiesActive, i, isize);
		{
			...
			pos += snprintf_s(	pos,
				STRBUF_REMAIN(extrasString, pos),
				"  id %u, type %u, spc %u, penalty %1.3f [%8.8x]\n",
				sp->id,
				sp->type,
				sp->spc,
				sp->penalty,
	...
	if(bg->airFrictionMultOverride){
		pos += snprintf_s(	pos,
			STRBUF_REMAIN(extrasString, pos),
			"air friction mult override %1.3f [%8.8x]\n",
			bg->airFrictionMultOverride,
			*(S32*)&bg->airFrictionMultOverride);
	}
			...
			(!bg->flags.holdForFlourish || !bg->flags.ownsAnimation))
		{
			...
			else //the hold could be active here, but animation ownership will lag the activate function by a frame
			{
				...
				if (gConf.bUseMovementGraphs)
					mrSurfaceStanceSetBG(msg, bg, localBG, MR_SURFACE_STANCE_JUMPRISING);
				else
	...
	SurfaceLocalBG* localBG)
{
	...
	mrSurfaceStanceClearBG(msg, bg, localBG, MR_SURFACE_STANCE_JUMPRISING);
	mrSurfaceStanceClearBG(msg, bg, localBG, MR_SURFACE_STANCE_JUMPFALLING);
}
										...
										const F32 deltaSeconds)
{
	...
	if(bg->vel[1] <= 0.f)
	{
		...
		if(mrSurfaceShouldPlayFallingAnimBG(msg, bg)){
			if (gConf.bUseMovementGraphs && bg->flags.offGroundByJumping)
				mrSurfaceStanceSetBG(msg, bg, localBG, MR_SURFACE_STANCE_JUMPFALLING);
			else
							...
							const F32 deltaSeconds)
{
	...
	{
		traction *= 0.3f;
		friction *= (bg->airFrictionMultOverride == 0) ? 0.05f : bg->airFrictionMultOverride;
	}
											...
											const SurfaceSyncPublic* syncPublic)
{
		...
		vec3IsZeroXZ(velTarget))
	{
		...
		else if(curSpeedXZ > syncPublic->speeds[MR_SURFACE_SPEED_FAST].speed + 0.5f)
		{
														...
														SurfaceToFG* toFG)
{
	...
	{
		localBG->flagsMutable.isStrafingOverride = localBG->flagsMutable.scheduledIsStrafingOverride;
		localBG->flagsMutable.strafingOverrideEnabled = localBG->flagsMutable.scheduledStrafingOverrideEnabled;
...
#pragma warning(suppress:6201) // /analyze isn't realizing i bigger than 0 even with ANALYSIS_ASSUME
		...
		xcase MPTT_POINT:{
				...
				bg->flags.onGround)
			{
				...
				// If the target is below us, always jump some standard height in case there is a railing we're jumping over or something
				// For some reason, the rules are different for jumping up than jumping down.  I'm going to scale the extra height when jumping up
				// by some factor of the horizontal distance to try to create a more natural feel.  [RMARR - 5/16/13]
				if (jumpTarget[1] < posCur[1])
					apexY = posCur[1]+10.0f;
				else
					apexY = jumpTarget[1]+MIN(distXZ*0.5f,15.0f);

				diffY = apexY-posCur[1];

				// get the velocity to get to the target height (at 0 vertical velocity)
				bg->vel[1]  = sqrt(2 * diffY * g);

				// time to apex is bg->vel[1] / g

				// time to destination position requires the quadratic formula
				// This extra code with temp is to avoid taking the sqrt of a negative value due to floating point rounding error
				temp = bg->vel[1]*bg->vel[1]-2*g*diffY;
				if (temp > 0.0f)
					temp = sqrt(temp);
				else
					temp = 0.0f;
				...
				// calculate the velocity it should take us to get to the target height at the target time
				speedXZ = distXZ / t;
...
static bool mrSurfaceIsStrafing(SurfaceLocalBG* localBG, SurfaceSync* sync)
{
	...
	if (localBG->flags.strafingOverrideEnabled) 
		return localBG->flags.isStrafingOverride;

	return (sync->flags.isStrafing &&
		...
		if(FALSE_THEN_SET(bg->flagsMutable.onGround)){
			if (gConf.bNewAnimationSystem && sync->flags.spawnedOnGround)
			{
				...
				if ((bg->flagsMutable.hasStanceMask & BIT(MR_SURFACE_STANCE_RISING  )) ||
					...
					// due to jumping test we don't need to test jumpApex, jumpRising, & jumpFalling
				{
					mrSurfaceDestroyOffGroundStancesBG(msg, bg, localBG);
					if (!gConf.bUseMovementGraphs)
											...
											const SurfaceSync* sync)
{
	...
	if(eaSize(&localBG->speedPenaltiesQueued))
	{
		const SurfaceSpeedPenalty* spDefaultCurrent = NULL;
		const SurfaceSpeedPenalty* spAirFrictionCurrent = NULL;
		...
		EARRAY_CONST_FOREACH_BEGIN(localBG->speedPenaltiesQueued, i, isize);
		{
			...
			if(spNew->flags.isStop){
				EARRAY_CONST_FOREACH_BEGIN(localBG->speedPenaltiesActive, j, jsize);
				{
					...
					if(sp->id == spNew->id && sp->type == spNew->type){
		...
		if(eaSize(&localBG->speedPenaltiesActive))
		{
			// get the tail of each type of 'speed penalty'
			FOR_EACH_IN_EARRAY(localBG->speedPenaltiesActive, const SurfaceSpeedPenalty, sp)
			{
				if (sp->type == ESurfaceSpeedPenaltyType_DEFAULT)
				{
					if (!spDefaultCurrent)
						spDefaultCurrent = sp;
				}
				else if (sp->type == ESurfaceSpeedPenaltyType_AIRFRICTION_MULT)
				{
					if (!spAirFrictionCurrent)
						spAirFrictionCurrent = sp;
				}

				// if we have one of each type, we know we're done 
				if (spAirFrictionCurrent && spDefaultCurrent)
					break;
			}
			FOR_EACH_END
		}
		...
		if (spDefaultCurrent)
		{
			bg->speedPenalty = spDefaultCurrent->penalty;
			bg->flagsMutable.speedPenaltyIsStrictScale = spDefaultCurrent->flags.isStrictScale;
			mrmLog(	msg,
					NULL,
		...
		if (spAirFrictionCurrent)
		{
			bg->airFrictionMultOverride = spAirFrictionCurrent->penalty;
			mrmLog(	msg,
					NULL,
					"Set airFrictionMultOverride to %1.3f [%8.8x].",
					bg->airFrictionMultOverride,
					*(U32*)&bg->airFrictionMultOverride);
		}
		else if(bg->airFrictionMultOverride > 0.f)
		{
			bg->airFrictionMultOverride = 0.f;

			mrmLog(	msg,
				NULL,
				"Removed airFrictionMultOverride.");
		}
	}
...
void mrSurfaceMsgHandler(const MovementRequesterMsg* msg){
	...
	switch(msg->in.msgType){
		...
		xcase MR_MSG_FG_CREATE_TOBG:{
			...
			{
				toBG->isStrafingOverride = fg->toBG.isStrafingOverride;
				toBG->strafingOverrideEnabled = fg->toBG.strafingOverrideEnabled;
		...
		xcase MR_MSG_BG_UPDATED_TOBG:{
			...
			if(toBG->spcStrafingOverride)
			{
				...
				localBG->flagsMutable.scheduledIsStrafingOverride = toBG->isStrafingOverride;
				localBG->flagsMutable.scheduledStrafingOverrideEnabled = toBG->strafingOverrideEnabled;
...
static bool mrSurfaceSpeedPenaltyStartInternal(	MovementRequester* mr, 
												U32 id, 
												ESurfaceSpeedPenaltyType type, 
												F32 scale, 
												bool bStrictScale, 
												U32 spc)
{
	...
	mrLog(	mr,
		NULL,
		"Speed START: id %u, type %u, spc %u, %s %1.3f [%8.8x].",
		id,
		type,
		spc,
		bStrictScale ? "scale" : "penalty",
		scale,
		*(S32*)&scale);
	...
	sp->penalty = scale;
	sp->spc = spc;
	sp->flags.isStrictScale = bStrictScale;
...
bool mrSurfaceSpeedPenaltyStart(MovementRequester* mr,
								...
								U32 spc)
{
	return mrSurfaceSpeedPenaltyStartInternal(mr, id, ESurfaceSpeedPenaltyType_DEFAULT, speedPenalty, false, spc);
}
...
bool mrSurfaceSpeedScaleStart(	MovementRequester* mr,
								...
								U32 spc)
{
	return mrSurfaceSpeedPenaltyStartInternal(mr, id, ESurfaceSpeedPenaltyType_DEFAULT, speedScale, true, spc);
}

static bool mrSurfaceSpeedPenaltyStopInternal( MovementRequester* mr,
												U32 id,
												ESurfaceSpeedPenaltyType type, 
												U32 spc)
{
...
bool mrSurfaceSpeedPenaltyStop(	MovementRequester* mr,
								...
								U32 spc)
{
	return mrSurfaceSpeedPenaltyStopInternal(mr, id, ESurfaceSpeedPenaltyType_DEFAULT, spc);
}

bool mrSurfaceAirFrictionMultOverrideStart(	MovementRequester* mr,
											U32 id,
											F32 aiFrictionMultOverride,
											U32 spc)
{
	return mrSurfaceSpeedPenaltyStartInternal(mr, id, ESurfaceSpeedPenaltyType_AIRFRICTION_MULT, aiFrictionMultOverride, false, spc);
}


bool	mrSurfaceAirFrictionMultOverrideStop(	MovementRequester* mr,
												U32 id,
												U32 spc)
{
	return mrSurfaceSpeedPenaltyStopInternal(mr, id, ESurfaceSpeedPenaltyType_AIRFRICTION_MULT, spc);
}
									...
									U32 spc)
{
	...
	if ((bool)fg->toBG.isStrafingOverride != isStrafing || 
		(bool)fg->toBG.strafingOverrideEnabled != enableOverride)
	{
		mrLog(	mr,
				NULL,
		...
		fg->toBG.strafingOverrideEnabled = enableOverride;
		fg->toBG.isStrafingOverride = isStrafing;
