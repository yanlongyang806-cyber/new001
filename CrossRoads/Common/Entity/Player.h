...
typedef struct RewardGatedRewardTable RewardGatedRewardTable;
...
AUTO_STRUCT AST_IGNORE(ShowMapChoice);
...
#define KEYBIND_SETTINGS_VERSION 1
...
typedef struct PlayerUI
{
	...
	char *pchUserColorPalette;						AST(PERSIST NO_TRANSACT)

	// Bit field that's used to permit players to disable some color palette states
	U32 ubfColorPaletteStateDisables;				AST(PERSIST NO_TRANSACT)
	...
	U8 uiCompactMode : 2;							AST(PERSIST NO_TRANSACT) // 0 = unset/automatic, 1 = off, 2 = on, ?, 255 = profit
	U8 uiDisableDamageOverlays : 1;					AST(PERSIST NO_TRANSACT)

	S32 iKeybindsVersion;							AST(PERSIST NO_TRANSACT)

	U8 uiStickerBookHasUnviewedChanges;				AST(PERSIST NO_TRANSACT)
...
typedef struct NPCEMailData
{
	...
	CONST_STRING_MODIFIABLE pReason;											AST(ESTRING PERSIST SUBSCRIBE)
	CONST_STRING_MODIFIABLE pDetail;											AST(ESTRING PERSIST SUBSCRIBE)
} NPCEMailData;

AUTO_STRUCT AST_CONTAINER AST_IGNORE(bReadAll) AST_IGNORE(uLastSyncTime);
typedef struct NPCEMail
{
	...
	const U32 uNextFutureDelivery;													AST( PERSIST SUBSCRIBE )
...
AUTO_STRUCT AST_CONTAINER;
typedef struct PlayerNumericThreshold
{
	CONST_STRING_POOLED astrThresholdName;	AST(KEY PERSIST SERVER_ONLY POOL_STRING)
		// The designer-specified name of the threshold
	const S32 iNumericEarned;				AST(PERSIST SERVER_ONLY)
		// The amount of the numeric earned for the time interval. This starts at 0 and increases every time the player raises the numeric and cools down back to 0 based on the cooldown rate.
	const U32 uTimestamp;					AST(PERSIST SERVER_ONLY)
		// The timestamp of the most recent transaction modifying the numeric. This is updated every time a transaction modifies NumericEarned so that readers of
		// the data know how much cooldown to apply when making governing or limiting decisions.
} PlayerNumericThreshold;

AUTO_STRUCT AST_CONTAINER;
typedef struct PlayerNumericThresholds
{
	CONST_STRING_POOLED astrItemDefName;								AST(KEY PERSIST SERVER_ONLY POOL_STRING)
		// The numeric item def name these thresholds are tracking
	CONST_EARRAY_OF(PlayerNumericThreshold) eaPlayerNumericThreshold;	AST(PERSIST SERVER_ONLY)
		// Array of numeric threshold counters for this numeric item
} PlayerNumericThresholds;
...
AUTO_STRUCT AST_CONTAINER;
typedef struct StickerBookItemInfo
{
	CONST_STRING_POOLED astrItemDefName;				AST(KEY PERSIST SUBSCRIBE SELF_ONLY POOL_STRING)
		// The item def name for this completed StickerBook item
} StickerBookItemInfo;
extern ParseTable parse_StickerBookItemInfo[];
#define TYPE_parse_StickerBookItemInfo StickerBookItemInfo

AUTO_STRUCT AST_CONTAINER;
typedef struct StickerBookItemSetInfo
{
	CONST_STRING_POOLED astrStickerBookItemSetRef;		AST(KEY PERSIST SUBSCRIBE SELF_ONLY POOL_STRING)
		// The name for this completed StickerBook item set
		// These are the StickerBookItemSet pchRefString.
} StickerBookItemSetInfo;
extern ParseTable parse_StickerBookItemSetInfo[];
#define TYPE_parse_StickerBookItemSetInfo StickerBookItemSetInfo

AUTO_STRUCT;
typedef struct StickerBookPointCacheData
{
	bool bTotalPointsValid;
	U32 iTotalPoints;

	int iLastItemInfoSize;		AST(DEFAULT(-1))
	U32 iPoints;
} StickerBookPointCacheData;
extern ParseTable parse_StickerBookPointCacheData[];
#define TYPE_parse_StickerBookPointCacheData StickerBookPointCacheData

AUTO_STRUCT;
typedef struct StickerBookPointCacheByCollectionType
{
	int type;				AST(KEY)
	StickerBookPointCacheData pointCacheData;
} StickerBookPointCacheByCollectionType;
...
AUTO_STRUCT;
typedef struct StickerBookPointCacheBySet
{
	const char *astrSetName;					AST(KEY POOL_STRING)
	StickerBookPointCacheData pointCacheData;
} StickerBookPointCacheBySet;
...
AUTO_STRUCT;
typedef struct StickerBookPointCacheByCollection
{
	const char *astrCollectionName;					AST(KEY POOL_STRING)
	StickerBookPointCacheData pointCacheData;

	EARRAY_OF(StickerBookPointCacheBySet) eaPointCacheBySet;
} StickerBookPointCacheByCollection;
...
AUTO_STRUCT;
typedef struct StickerBookPointCache
{
	StickerBookPointCacheData pointCacheData;

	EARRAY_OF(StickerBookPointCacheByCollectionType) eaPointCacheByCollectionType;
	EARRAY_OF(StickerBookPointCacheByCollection) eaPointCacheByCollection;
} StickerBookPointCache;
extern ParseTable parse_StickerBookPointCache[];
#define TYPE_parse_StickerBookPointCache StickerBookPointCache

AUTO_STRUCT AST_CONTAINER AST_IGNORE(eaAwayTeamPetIDs) AST_IGNORE(eaAwayTeamCritterIDs) AST_IGNORE(bNotLookingForGroup) AST_IGNORE(bAnonymous) AST_IGNORE(uPreviousBulletinTime)
AST_IGNORE_STRUCT(pUGCSubscription) AST_IGNORE_STRUCT(pUGCKillCreditLimit) AST_IGNORE_STRUCT(eaPlayerUGCKillCreditLimit2) AST_IGNORE_STRUCT(eaAstrRecentlyBoundStickerBookItems);
typedef struct Player
{
	...
	CONST_EARRAY_OF(PlayerNumericThresholds) eaPlayerNumericThresholds;	AST(PERSIST SERVER_ONLY)
		// numeric alert threshold tracking on the player keyed off of the numeric item name
	...
	const U32 bDoneMailAuctionItemsFixup : 1;			AST(PERSIST)
		// This entity has requested that the Auction server converted any outstanding mail
		// auctions to EmailV3 mails.
	...
	EARRAY_OF(RewardGatedTypeData) eaAccountRewardGatedData;	AST(PERSIST FORCE_CONTAINER SELF_ONLY)
	// The data for account gated rewards

	EARRAY_OF(RewardGatedRewardTable) eaPendingAccountRewards;	AST(PERSIST FORCE_CONTAINER SELF_ONLY NO_TRANSACT)
	// Non-transacted array of pending account gated rewards
    ...
    CONST_EARRAY_OF(NumericConversionState) eaNumericConversionStates; AST(PERSIST FORCE_CONTAINER SELF_ONLY SUBSCRIBE)
    ...
    EARRAY_OF(GroupProjectRecentUnlock) eaRecentUnlocks;    AST(SERVER_ONLY)
    // A non-persisted array that keeps track of recently received group project unlocks.  
    //   This is to avoid race conditions with missions that check unlocks.
	...
	S32 addictionReminderTimeIdx;									AST(SERVER_ONLY DEFAULT(-1))

	CONST_STRING_EARRAY eaAstrRecentlyAcquiredStickerBookItems;			AST(PERSIST SERVER_ONLY POOL_STRING)
		// array of all recently acquired items that may grant sticker book points. These are pooled strings of the ItemDef names.

	CONST_STRING_EARRAY eaAstrRecentlyModifiedStickerBookItemSets;		AST(PERSIST SERVER_ONLY POOL_STRING)
		// array of all recently completed item sets that may grant sticker book points and a reward title.
		// These are the StickerBookItemSet pchRefString.

	CONST_EARRAY_OF(StickerBookItemInfo) eaStickerBookItemInfo;			AST(PERSIST SELF_ONLY)
		// array of all completed StickerBook items.

	CONST_EARRAY_OF(StickerBookItemSetInfo) eaStickerBookItemSetInfo;	AST(PERSIST SELF_ONLY)
		// array of all completed StickerBook items.
		// These are keyed off of the StickerBookItemSet pchRefString.

	StickerBookPointCache *pStickerBookPointCache;				AST(NO_NETSEND)
		// Cache of StickerBook total points and acquired points so we do not keep recomputing the entire tree. Used on both GameServer and GameClient.

	// The list time the player sent mail.  Used to rate limit mail.
    U32 lastMailSendTime;                                   NO_AST
} Player;
