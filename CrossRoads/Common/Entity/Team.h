...
#define TEAM_UPDATE_MIN_INTERVAL 15.f	// Can't send update info more often than once in this time period. Only triggered if something actually changed.
...
typedef struct TeamMember
{
	...
	const U32 uOffMapTimeStamp;				AST(PERSIST SUBSCRIBE)	// If a member has gone off map, this is the timestamp (server) of when they did. 0 otherwise
...
typedef struct StubTeamMember
{
	...
	CONST_STRING_MODIFIABLE pcAccountHandle;AST(PERSIST SUBSCRIBE)
	CONST_STRING_POOLED pchClassName;		AST(PERSIST SUBSCRIBE POOL_STRING NAME(ClassName))
	const S32 iExpLevel;					AST(PERSIST SUBSCRIBE)		// We need this for the lowest level on team expression
} StubTeamMember;
...
AUTO_STRUCT;
typedef struct TeamVoteKickData
{
	U32 iInstigatorID;		// ID of the person starting the vote
	U32 iKickTargetID;		// ID of the target of the vote
	U32 *eaiVoterIDs;		// earray IDs of the members who will be voting.
	U32 *eaiYesVoterIDs;	// earray IDs of the members who voted yes
	U32 *eaiNoVoterIDs;		// earray IDs of the members who voted no

	// Time info.
	U32 uVoteStartTime;		// The timestamp that the vote started at.
...
typedef struct Team
{
	...
	////////////
	// Vote Kick data. Present only if a vote kick is in progress. Only tracked on the TeamServer and not sent anywhere else.
	TeamVoteKickData* pVoteKickData;					NO_AST
	...
	const ContainerID iGameServerOwnerPartition;		AST(PERSIST SUBSCRIBE SERVER_ONLY)
		//The game server ID (and its partition) that has the local team that represents this team.
	...
	const bool bVoteRequiredForKick;					AST(PERSIST SUBSCRIBE)
		// To kick someone from this team requires a vote.
