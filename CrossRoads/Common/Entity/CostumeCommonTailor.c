	...
	if (pBone->bIsChildBone) {
		...
		if (pParentBone) {
			...
			if (pParentPart) {
				...
				if (costumeTailor_GetCategoryForRegion(CONTAINER_RECONST(PlayerCostume, pPCCostume), GET_REF(pBone->hRegion)) != pCategory) {
					PCGeometryDef **eaParentValidGeos = NULL;
					PCGeometryDef **eaOtherParentGeos = NULL;
					// When checking for a valid geo for a child bone in a different category, at least one parent geo must have a valid child geo
					costumeTailor_GetValidGeos(pPCCostume, GET_REF(pPCCostume->hSkeleton), pParentBone, pCategory, pSpecies, eaUnlockedCostumes, &eaParentValidGeos, false, false, bSortDisplay, bUnlockAll);
					for(i=eaSize(&eaParentValidGeos)-1; i>=0; i--) {
						costumeTailor_GetValidChildGeos(pPCCostume, pCategory, eaParentValidGeos[i], pBone, pSpecies, eaUnlockedCostumes, &eaOtherParentGeos, bSortDisplay, bUnlockAll);
						for(j=eaSize(&eaOtherParentGeos)-1; j>=0; j--) {
							eaPushUnique(&eaParentGeos, eaOtherParentGeos[j]);
						}
					}
					eaDestroy(&eaParentValidGeos);
					eaDestroy(&eaOtherParentGeos);
				}
			}
		}
...
void costumeTailor_FillAllBones(NOCONST(PlayerCostume) *pPCCostume, SpeciesDef *pSpecies, const char **eaPowerFXBones, PCSlotType *pSlotType, bool bApplyRequired, bool bAddArtistData, bool bSortDisplay)
{
	...
	for(i=0; i<eaSize(&eaBones); ++i) {
		...
		if (!pNewPart) {
			...
			if (!IS_HANDLE_ACTIVE(pNewPart->hGeoDef)) {
				SET_HANDLE_FROM_STRING(g_hCostumeGeometryDict, "None", pNewPart->hGeoDef);
			}
			if (!IS_HANDLE_ACTIVE(pNewPart->hMatDef)) {
				SET_HANDLE_FROM_STRING(g_hCostumeMaterialDict, "None", pNewPart->hMatDef);
			}
			if (!IS_HANDLE_ACTIVE(pNewPart->hPatternTexture)) {
				SET_HANDLE_FROM_STRING(g_hCostumeTextureDict, "None", pNewPart->hPatternTexture);
			}
			if (!IS_HANDLE_ACTIVE(pNewPart->hDetailTexture)) {
				SET_HANDLE_FROM_STRING(g_hCostumeTextureDict, "None", pNewPart->hDetailTexture);
			}
			if (!IS_HANDLE_ACTIVE(pNewPart->hSpecularTexture)) {
				SET_HANDLE_FROM_STRING(g_hCostumeTextureDict, "None", pNewPart->hSpecularTexture);
			}
			if (!IS_HANDLE_ACTIVE(pNewPart->hDiffuseTexture)) {
				SET_HANDLE_FROM_STRING(g_hCostumeTextureDict, "None", pNewPart->hDiffuseTexture);
			}
		}
		...
		if (!IS_HANDLE_ACTIVE(pNewPart->pMovableTexture->hMovableTexture)) {
			SET_HANDLE_FROM_STRING(g_hCostumeTextureDict, "None", pNewPart->pMovableTexture->hMovableTexture);
		}
	...
	if (bAddArtistData) {
		...
		if (!pPCCostume->pArtistData->pFlightControls) {
			pPCCostume->pArtistData->pFlightControls = StructCreateNoConst(parse_PCFlightControls);
		}
	}
...
void costumeTailor_StripUnnecessary(NOCONST(PlayerCostume) *pPCCostume)
{
	...
	if (pPCCostume->pArtistData) {
		...
		if (!pPCCostume->pArtistData->pcCollisionGeo &&
			...
			!pPCCostume->pArtistData->pFlightControls
			) {
		...
		for(i=0; i<eaSize(&eaData); ++i) {
			...
			if (eaSize(&eaData[i]->eaAddedFX) > 0)
			{
				...
				if (pNewCostume->pArtistData) {
					...
					eaPushStructsNoConst(&pNewCostume->pArtistData->eaFX, &eaFxNC, parse_PCFX);
				}
			}
...
const PCBoneGroup* costumeTailor_FindBoneGroupByName( const PCSkeletonDef* pSkel, const char* name )
{
	int groupIt, groupN = eaSize(&pSkel->eaBoneGroups);

	name = allocFindString( name );
	for ( groupIt = 0; groupIt != groupN; ++groupIt ) {
		PCBoneGroup* group = pSkel->eaBoneGroups[ groupIt ];
		if( name == group->pcName ) {
			return group;
		}
	}
