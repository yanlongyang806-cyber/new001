...
S32 Officer_GetPromoteReturnValue(SA_PARAM_NN_VALID Entity* pEnt, U32 uiOfficerID, AllegianceDef *pAllegiance)
{
	...
	if ( pOfficerRankDef->pchCostNumeric && pOfficerRankDef->pchCostNumeric[0] )
	{
		...
		S32 iOwnerCurrencySpent = inv_GetNumericItemValue(pEnt,pOfficerRankDef->pchPointsSpentNumeric);

		if ( pOfficerRankDef->iCost > (iOwnerCurrency - iOwnerCurrencySpent) )
		{
...
S32 Officer_GetTrainReturnValue(int iPartitionIdx, SA_PARAM_NN_VALID Entity* pBuyer, SA_PARAM_NN_VALID Entity* pTrainer, U32 uiOfficerID, 
								const char* pchOldNode, const char* pchNewNode, bool bCheckCost, bool bCheckNode, AllegianceDef *pAllegiance, PTNodeDef **pTrainNodeDef)
{
	S32 i, j;
	OfficerRankDef* pOfficerRankDef;
	PetRelationship* pPet;
	...
	PowerTreeDef *pOldTreeDef, *pNewTreeDef, *pAltTreeDef;
	PTNode* pOldNode;
	PTNodeDef *pOldNodeDef, *pNewNodeDef, *pAltNodeDef;
	...
	pAltNodeDef = pNewNodeDef ? GET_REF(pNewNodeDef->hNodeAltTrainer) : NULL;
	pOldTreeDef = pOldTree ? GET_REF(pOldTree->hDef) : NULL;
	pNewTreeDef = pNewNodeDef ? powertree_TreeDefFromNodeDef(pNewNodeDef) : NULL;
	pAltTreeDef = pAltNodeDef ? powertree_TreeDefFromNodeDef(pAltNodeDef) : NULL;

	if ( pAltNodeDef && stricmp(pchOldNode,pAltNodeDef->pchNameFull)==0 )
		return OfficerActionReturnValue_SameNode;
	...
	{
		if ( pOldTreeDef && pNewTreeDef && pAltTreeDef && pOldTreeDef == pAltTreeDef )
		{

			// Swap the new and alt nodes. We still need both because the powerstore
			// will be referencing the new node, and not the alt one, but we want to
			// use the alt node for all other behavior
			PTNodeDef *pTempNodeDef = pNewNodeDef;
			pNewNodeDef = pAltNodeDef;
			pAltNodeDef = pTempNodeDef;
		}
		else
		{
			return OfficerActionReturnValue_InvalidAction;
	...
	if (bCheckCost && pOfficerRankDef->pchTrainingNumeric && pOfficerRankDef->pchTrainingNumeric[0])
	{
		...
		if (pInfo && pInfo->pContactDialog)
		{
			...
			for (i = eaSize(&pContactDialog->eaStorePowers)-1; i >= 0; i--)
			{
				...
				if (  ( GET_REF(pPowerInfo->hNode) == pNewNodeDef || GET_REF(pPowerInfo->hNode) == pAltNodeDef )
	...
	{
		if ( stricmp(pNewNodeDef->pchNameFull,REF_STRING_FROM_HANDLE(pOfficer->pChar->ppTraining[i]->hNewNodeDef))==0 )
			break;
	}
	if ( i >= 0 )
	...
	if ( !entity_CanReplacePowerTreeNodeInEscrow(	iPartitionIdx, CONTAINER_NOCONST(Entity, pTrainer), CONTAINER_NOCONST(Entity, pOfficer), 
													pchTree, pchOldNode, pchTree, pNewNodeDef->pchNameFull, true))
	{
		return OfficerActionReturnValue_InvalidAction;
	}
...
bool Officer_CanTrain(int iPartitionIdx, SA_PARAM_NN_VALID Entity* pBuyer, SA_PARAM_NN_VALID Entity* pTrainer, U32 uiOfficerID, const char* pchOldNode,
					  const char* pchNewNode, bool bCheckCost, bool bCheckNode, AllegianceDef *pAllegiance, PTNodeDef **pTrainNodeDef)
{
	return !Officer_GetTrainReturnValue( iPartitionIdx, pBuyer, pTrainer, uiOfficerID, pchOldNode, pchNewNode, bCheckCost, bCheckNode, pAllegiance, pTrainNodeDef);
}
