...
S64 g_NPCMailCurrencyAlertThreshold = 75000000;

AUTO_CMD_INT(g_NPCMailCurrencyAlertThreshold, NPCMailCurrencyAlertThreshold) ACMD_AUTO_SETTING(Auction, GAMESERVER, GATEWAYSERVER, AUCTIONSERVER);
...
void AuctionConfig_Load(void)
{
	...
	if (gAuctionConfig.iNPCMailCurrencyAlertThreshold > 0)
		g_NPCMailCurrencyAlertThreshold = gAuctionConfig.iNPCMailCurrencyAlertThreshold;
...
U32 Auction_GetPostingFee(Entity *pEntity, U32 iBuyoutPrice, U32 iStartingBid, U32 iAuctionDuration, bool bPlayerIsAtAuctionHouse)
{
	...
	uPostingFee = round(iPriceUsedForFee * fPostingFeeMultiplier);
...
AUTO_TRANS_HELPER;
void AuctionLot_trh_GetNewLotLogDetailString(ATH_ARG NOCONST(AuctionLot)* pLot, U32 uiLotID, char** pEstrOut)
{
	if (NONNULL(pLot) && pEstrOut)
	{
		NOCONST(Item)* pItem = pLot->ppItemsV2 && pLot->ppItemsV2[0] ? pLot->ppItemsV2[0]->slot.pItem : NULL;

		if (!pItem)
			return;

		AuctionLot_trh_GetGenericLogDetailString(pLot, uiLotID, pEstrOut);

		estrConcatf(pEstrOut, ", PostingFee %d", pLot->uPostingFee);
	}
}

AUTO_TRANS_HELPER;
void AuctionLot_trh_GetBidSaleLogDetailString(ATH_ARG NOCONST(AuctionLot)* pLot, U32 uiPriceAfterFees, char** pEstrOut)
{
	if (NONNULL(pLot) && pEstrOut)
	{
		NOCONST(Item)* pItem = pLot->ppItemsV2 && pLot->ppItemsV2[0] ? pLot->ppItemsV2[0]->slot.pItem : NULL;
		...
		AuctionLot_trh_GetGenericLogDetailString(pLot, pLot->iContainerID, pEstrOut);

		estrConcatf(pEstrOut, ", CurrentHighBid %d, CurrentHighBidderID %d, CurrentHighBidderAccountID %d, SalePriceAfterFees %d", NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iCurrentBid : 0, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iBiddingPlayerContainerID : 0, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iBiddingPlayerAccountID : 0, uiPriceAfterFees);
	}
}

AUTO_TRANS_HELPER;
void AuctionLot_trh_GetNewBidLogDetailString(ATH_ARG NOCONST(AuctionLot)* pLot, char** pEstrOut)
{
	if (NONNULL(pLot) && pEstrOut)
	{
		NOCONST(Item)* pItem = pLot->ppItemsV2 && pLot->ppItemsV2[0] ? pLot->ppItemsV2[0]->slot.pItem : NULL;
		...
		estrConcatf(pEstrOut, ", CurrentHighBid %d, CurrentHighBidderID %d, CurrentHighBidderAccountID %d", NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iCurrentBid : 0, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iBiddingPlayerContainerID : 0, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iBiddingPlayerAccountID : 0);
	}
}


AUTO_TRANS_HELPER;
void AuctionLot_trh_GetBuyoutSaleLogDetailString(ATH_ARG NOCONST(AuctionLot)* pLot, ATH_ARG NOCONST(Entity)* pBuyer, U32 uiPriceAfterFees, char** pEstrOut)
{
	if (NONNULL(pLot) && NONNULL(pBuyer) && pEstrOut)
	{
		NOCONST(Item)* pItem = pLot->ppItemsV2 && pLot->ppItemsV2[0] ? pLot->ppItemsV2[0]->slot.pItem : NULL;
		...
		estrConcatf(pEstrOut, ", HighBidBeforeBuyout %d, HighBidderIDBeforeBuyout %d, HighBidderAccountIDBeforeBuyout %d, BuyerID %d, BuyerAccountID %d, SalePriceAfterFees %d", NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iCurrentBid : 0, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iBiddingPlayerContainerID : 0, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iBiddingPlayerAccountID : 0, pBuyer->myContainerID, NONNULL(pBuyer->pPlayer) ? pBuyer->pPlayer->accountID : 0, uiPriceAfterFees);
	}
}

AUTO_TRANS_HELPER;
void AuctionLot_trh_GetGenericLogDetailString(ATH_ARG NOCONST(AuctionLot)* pLot, U32 uiLotID, char** pEstrOut)
{
	if (NONNULL(pLot) && pEstrOut)
	{
		NOCONST(Item)* pItem = pLot->ppItemsV2 && pLot->ppItemsV2[0] ? pLot->ppItemsV2[0]->slot.pItem : NULL;
		...
		estrPrintf(pEstrOut, "LotID %d, SellerID %d, SellerAccountID %d, ItemDef %s, ItemQuantity %d, MinBid %d, Buyout %d", uiLotID, pLot->ownerID, pLot->OwnerAccountID, REF_STRING_FROM_HANDLE(pItem->hItem), pItem->count, NONNULL(pLot->pBiddingInfo) ? pLot->pBiddingInfo->iMinimumBid : 0, pLot->price);
	}
}

void AuctionLot_GetGameEconomyDetailString(AuctionLot* pLot, char** pEstrOut)
{
	if (pLot && pEstrOut)
	{
		...
		estrPrintf(pEstrOut, "quantity:%d itemdef:%s", pItem->count, REF_STRING_FROM_HANDLE(pItem->hItem));
	}
}
