...
AUTO_EXPR_FUNC(ai) ACMD_NAME(SetUndrawable);
void exprFuncSetUndrawable(ACMD_EXPR_SELF Entity* e, S32 on)
{
	if(e->pChar)
	{
		e->pChar->bDoNotDraw = !!on;
		entity_SetDirtyBit(e, parse_Character, e->pChar, false);
	}
}
...
AUTO_EXPR_FUNC(entity) ACMD_NAME(GetMyCreator);
void exprFuncGetMyCreator(ACMD_EXPR_SELF Entity* e, ACMD_EXPR_PARTITION iPartitionIdx, ACMD_EXPR_ENTARRAY_OUT entsOut)
{
	Entity* creator = e->erCreator ? entFromEntityRef(iPartitionIdx, e->erCreator) : NULL;
	if(creator && entIsAlive(creator))
		eaPush(entsOut, creator);
}
...
// Returns the entity's creator if the entity is a pet or an empty ent array if the entity is not a pet
// Will return the owner regardless if it is alive or not
AUTO_EXPR_FUNC(entity) ACMD_NAME(GetMyCreatorDeadAll);
void exprFuncGetMyCreatorDeadAll(ACMD_EXPR_SELF Entity* e, ACMD_EXPR_PARTITION iPartitionIdx, ACMD_EXPR_ENTARRAY_OUT entsOut)
{
	Entity* creator = e->erCreator ? entFromEntityRef(iPartitionIdx, e->erCreator) : NULL;
	if(creator)
		eaPush(entsOut, creator);
...
AUTO_EXPR_FUNC(entity, ai, OpenMission, encounter_action) ACMD_NAME(EntCropCurrentlyUsingPower);
ExprFuncReturnVal exprFuncEntCropCurrentlyUsingPower(ACMD_EXPR_ENTARRAY_IN_OUT entsInOut, const char *pszPowerName)
{
	PowerDef *pPowerDef = powerdef_Find(pszPowerName);
	...
	for(iEnt=eaSize(entsInOut)-1; iEnt>=0; iEnt--)
	{
		...
		if (pChar)
		{
			PowerActivation *pCurrent = pChar->pPowActCurrent;
			if (pCurrent)
			{
				PowerDef *pCheckPowerDef = GET_REF(pCurrent->hdef);
				if (pPowerDef != pCheckPowerDef)
				{
...
AUTO_EXPR_FUNC(entity, ai, OpenMission, encounter_action) ACMD_NAME(EntCropCurrentlyUsingPowerWithCategory);
ExprFuncReturnVal exprFuncEntCropCurrentlyUsingPowerWithCategory(ACMD_EXPR_ENTARRAY_IN_OUT entsInOut, const char *pszCategory)
{
	S32 eCategory = StaticDefineIntGetInt(PowerCategoriesEnum, pszCategory);
	...
	for(iEnt=eaSize(entsInOut)-1; iEnt>=0; iEnt--)
	{
		...
		if (pChar)
		{
			...
			if (pCurrent)
			{
				PowerDef *pPowerDef = GET_REF(pCurrent->hdef);
...
AUTO_EXPR_FUNC(entity, ai, OpenMission, encounter_action) ACMD_NAME(EntCropCurrentlyUsingPowerWithTag);
ExprFuncReturnVal exprFuncEntCropCurrentlyUsingPowerWithTag(ACMD_EXPR_ENTARRAY_IN_OUT entsInOut, const char *pszTag)
{
...
AUTO_EXPR_FUNC(player) ACMD_NAME(PlayerHasItemAssignment);
bool exprPlayerHasItemAssignment(ExprContext* pContext, const char* pchDefName)
{
	Entity* pEnt = exprContextGetVarPointerUnsafePooled(pContext, g_PlayerVarName);
	ItemAssignmentPlayerData* pPlayerData = SAFE_MEMBER2(pEnt, pPlayer, pItemAssignmentData);
	ItemAssignmentPersistedData* pPersistedData = SAFE_MEMBER2(pEnt, pPlayer, pItemAssignmentPersistedData);
	...
	if (pDef && pPersistedData)
	{
		if (ItemAssignment_EntityGetActiveAssignmentByDef(pEnt, pDef))
		{
			return true;
		}
	}
...
AUTO_EXPR_FUNC(Player, Entity, UIGen) ACMD_NAME(IsAllegiance) ACMD_EXPR_STATIC_CHECK(allegiance_IsAllegiance_LoadVerify);
...
AUTO_EXPR_FUNC(entityutil, Entity, UIGen) ACMD_NAME(EntGetTargetFromArray);
SA_RET_OP_VALID Entity* EntGetTargetFromArray(ACMD_EXPR_ENTARRAY_IN playersIn)
{
	Entity *pEntity;
	...
	return entity_GetTarget(pEntity);
}
...
AUTO_EXPR_FUNC(entityutil, Entity, UIGen) ACMD_NAME(EntGetTargetArray);
void EntGetTargetArray(ACMD_EXPR_ENTARRAY_IN_OUT entsInOut)
{
	Entity *pEntity, *pTargetEnt;
	Entity **eaEnts = NULL;

	if(eaSize(entsInOut) > 0)
	{
		pEntity = (*entsInOut)[0];

		if(pEntity)
		{
			pTargetEnt = entity_GetTarget(pEntity);

			if(pTargetEnt)
			{
				eaPush(&eaEnts, pTargetEnt);
			}
		}
	}

	eaCopy(entsInOut, &eaEnts);
	eaDestroy(&eaEnts);
}
...
AUTO_EXPR_FUNC(gameutil,UIGen,CEFuncsGeneric, CEFuncsCharacter) ACMD_NAME(IsOnUGCMap);
...
bool exprPlayerOwnsPet(ExprContext* pContext, ACMD_EXPR_PARTITION iPartitionIdx, const char* pchPetDefName)
{
	...
	if (pEnt && pEnt->pSaved && pchPetDefName)
	{
		...
		for (i = eaSize(&pEnt->pSaved->ppOwnedContainers)-1; i >= 0; i--)
		{
			...
			PetDef* pPetDef = GET_REF(pPet->hPetDefRef);	// PetRelationship now stores a ref to the petdef, needed when the pet is not subscribed

			if(!pPetDef)
			{
				// Check the actual ent to see if we have the def ... this should not occur unless you are looking at a pet on a different player on the client
				pPetDef = pPetEnt && pPetEnt->pCritter ? GET_REF(pPetEnt->pCritter->petDef) : NULL;
			}
...
AUTO_EXPR_FUNC(Entity, UIGen);
bool EntPetCanBeRenamed(SA_PARAM_OP_VALID Entity* pEntPet)
{
	return Entity_PetCanBeRenamed(pEntPet);
}

AUTO_EXPR_FUNC(Entity, UIGen);
bool EntPetCanBeDismissed(SA_PARAM_OP_VALID Entity* pEntPet)
{
	PetDef* pDef = SAFE_GET_REF2(pEntPet, pCritter, petDef);
	return SAFE_MEMBER(pDef, bCanRemove);
}
