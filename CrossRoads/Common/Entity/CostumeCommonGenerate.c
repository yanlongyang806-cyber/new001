...
WLCostumePart *costumeGenerate_SetWLPart(WLCostume *pCostume, PlayerCostume *pPCCostume, SpeciesDef *pSpecies, PCPart *pPCPart, bool *pbComplete)
{
	...
	int i = 0, j = 0;
	bool found = false;
	bool geoAnimNodeAliasList = false;
	...
	if (pGeoDef)
	{
		...
		// Set the animation node alias list
		if (pGeoDef->pcAnimNodeAliasList)
		{
			geoAnimNodeAliasList = true;

			if (RefSystem_IsReferentStringValid(hDynAnimNodeAliasListDict,
												pGeoDef->pcAnimNodeAliasList))
			{
				RefSystem_SetHandleFromString(	hDynAnimNodeAliasListDict,
												pGeoDef->pcAnimNodeAliasList,
												REF_HANDLEPTR(pPart->hAnimNodeAliasList));
				pCostume->bHasNodeAliases = true;
			}
			else
			{
				Errorf("Unable to find DynAnimNodeAliasList %s when building CostumeGeo %s\n", pGeoDef->pcAnimNodeAliasList, pGeoDef->pcName);
			}
		}

		// Set the animation node auxiliary transform list
		if (pGeoDef->pcAnimNodeAuxTransformList)
		{
			if (RefSystem_IsReferentStringValid(hDynAnimNodeAuxTransformListDict,
												pGeoDef->pcAnimNodeAuxTransformList))
			{
				RefSystem_SetHandleFromString(	hDynAnimNodeAuxTransformListDict,
												pGeoDef->pcAnimNodeAuxTransformList,
												REF_HANDLEPTR(pPart->hAnimNodeAuxTransformList));
				pCostume->bHasNodeAuxTransforms = true;
			}
			else
			{
				Errorf("Unable to find DynAnimNodeAuxTransformList %s when building CostumeGeo %s\n", pGeoDef->pcAnimNodeAuxTransformList, pGeoDef->pcName);
			}
		}
	}

	if (!geoAnimNodeAliasList)
	{
		// Set the animation node alias list
		if (pPCPart->pchAnimNodeAliasList)
		{
			if (RefSystem_IsReferentStringValid(hDynAnimNodeAliasListDict,
												pPCPart->pchAnimNodeAliasList))
			{
				RefSystem_SetHandleFromString(	hDynAnimNodeAliasListDict,
												pPCPart->pchAnimNodeAliasList,
												REF_HANDLEPTR(pPart->hAnimNodeAliasList));
			...
			{
				Errorf("Unable to find DynAnimNodeAliasList %s when building PlayerCostume %s part on bone %s\n", pPCPart->pchAnimNodeAliasList, pPCCostume->pcName, pPCPart->pchOrigBone);
			}
		}
	}
...
WLCostume* costumeGenerate_CreateWLCostumeEx(PlayerCostume *pPCCostume, SpeciesDef *pSpecies, CharacterClass* pClass, AllegianceDef *pAllegiance, AllegianceDef *pSubAllegiance,
	const char** eaExtraStances, const PCSlotType *pSlotType, PCMood *pMood, PCFXTemp ***eaAdditionalFX, char *pcNamePrefix, GlobalType type, ContainerID id, bool bIsPlayer,
	bool bGenerateSkeletonFX, WLCostume*** peaSubCostumes)
{
	WLCostume* pCostume;
	...
	if (pAllegiance)
	{
		for(i=0; i<eaSize(&pAllegiance->eaFXSwap); ++i) {
			if (pAllegiance->eaFXSwap[i]->pcOldName && strlen(pAllegiance->eaFXSwap[i]->pcOldName) &&
				pAllegiance->eaFXSwap[i]->pcNewName && strlen(pAllegiance->eaFXSwap[i]->pcNewName)) {
					CostumeFXSwap *pSwap = StructCreate(parse_CostumeFXSwap);
					SET_HANDLE_FROM_STRING("DynFXInfo", pAllegiance->eaFXSwap[i]->pcOldName, pSwap->hOldFx);
					SET_HANDLE_FROM_STRING("DynFXInfo", pAllegiance->eaFXSwap[i]->pcNewName, pSwap->hNewFx);
					eaPush(&pCostume->eaFXSwap, pSwap);
			}
		}
	}
	if (pSubAllegiance)
	{
		for(i=0; i<eaSize(&pSubAllegiance->eaFXSwap); ++i) {
			if (pSubAllegiance->eaFXSwap[i]->pcOldName && strlen(pSubAllegiance->eaFXSwap[i]->pcOldName) &&
				pSubAllegiance->eaFXSwap[i]->pcNewName && strlen(pSubAllegiance->eaFXSwap[i]->pcNewName)) {
					CostumeFXSwap *pSwap = StructCreate(parse_CostumeFXSwap);
					SET_HANDLE_FROM_STRING("DynFXInfo", pSubAllegiance->eaFXSwap[i]->pcOldName, pSwap->hOldFx);
					SET_HANDLE_FROM_STRING("DynFXInfo", pSubAllegiance->eaFXSwap[i]->pcNewName, pSwap->hNewFx);
					eaPush(&pCostume->eaFXSwap, pSwap);
			}
		}
	}
...
void costumeGenerate_FixEntityCostume(Entity* e)
{
	...
	DynAnimNodeAliasList** eaOldAliasList = NULL;
	DynAnimNodeAuxTransformList** eaOldAuxXformList = NULL;
	...
	#ifdef GAMECLIENT
	eaCreate(&eaOldAliasList);
	eaCreate(&eaOldAuxXformList);
	#endif
	...
	if (wlOld)
	{
		bWasMounted = wlOld->bMount;

		#ifdef GAMECLIENT
		if (wlOld->bHasNodeAliases ||
			wlOld->bHasNodeAuxTransforms)
		{
			FOR_EACH_IN_EARRAY_FORWARDS(wlOld->eaCostumeParts, WLCostumePart, pCostumePart)
			{
				DynAnimNodeAliasList* pAlias = GET_REF(pCostumePart->hAnimNodeAliasList);
				DynAnimNodeAuxTransformList* pAuxXform = GET_REF(pCostumePart->hAnimNodeAuxTransformList);
				if (pAlias) {
					eaPush(&eaOldAliasList, pAlias);	
				}
				if (pAuxXform) {
					eaPush(&eaOldAuxXformList, pAuxXform);
				}
			}
			FOR_EACH_END;
		}
	...
	else if (!IsServer() && pc)
	{
		...
		wlc = costumeGenerate_CreateWLCostume(	pc,
												...
												NULL,
												GET_REF(e->hAllegiance),
												GET_REF(e->hSubAllegiance),
												NULL,
			...
			(pc && pc->bCostumeChangeAlwaysSaveSequencersIfSkelsMatch)))
	{
		...
		if (pOldSkeleton)
		{
			if (wlc->bHasNodeAliases       == !!eaSize(&eaOldAliasList) &&
				wlc->bHasNodeAuxTransforms == !!eaSize(&eaOldAuxXformList))
			{
				bool bMissingAlias = false;
				bool bMissingAuxXform = false;

				if (wlc->bHasNodeAliases ||
					wlc->bHasNodeAuxTransforms)
				{
					DynAnimNodeAliasList** eaNewAliasList = NULL;
					DynAnimNodeAuxTransformList** eaNewAuxXformList = NULL;

					eaCreate(&eaNewAliasList);
					eaCreate(&eaNewAuxXformList);

					FOR_EACH_IN_EARRAY_FORWARDS(wlc->eaCostumeParts, WLCostumePart, pCostumePart)
					{
						DynAnimNodeAliasList* pAlias = GET_REF(pCostumePart->hAnimNodeAliasList);
						DynAnimNodeAuxTransformList* pAuxXform = GET_REF(pCostumePart->hAnimNodeAuxTransformList);
						if (pAlias) {
							if (eaFind(&eaOldAliasList, pAlias) < 0) {
								bMissingAlias = true;
								break;
							}
							eaPush(&eaNewAliasList, pAlias);
						}
						if (pAuxXform) {
							if (eaFind(&eaOldAuxXformList, pAuxXform) < 0) {
								bMissingAuxXform = true;
								break;
							}
							eaPush(&eaNewAuxXformList, pAuxXform);
						}
					}
					FOR_EACH_END;
					
					if (!bMissingAlias) {
						FOR_EACH_IN_EARRAY_FORWARDS(eaOldAliasList, DynAnimNodeAliasList, pAlias)
						{
							if (eaFind(&eaNewAliasList, pAlias) < 0) {
								bMissingAlias = true;
								break;
							}
						}
					...
					if (!bMissingAuxXform) {
						FOR_EACH_IN_EARRAY_FORWARDS(eaOldAuxXformList, DynAnimNodeAuxTransformList, pAuxXform)
						{
							if (eaFind(&eaNewAuxXformList, pAuxXform) < 0) {
								bMissingAuxXform = true;
								break;
							}
						}
					...
					eaDestroy(&eaNewAliasList);
					eaDestroy(&eaNewAuxXformList);
				}
	...
	eaDestroy(&eaOldAliasList);
	eaDestroy(&eaOldAuxXformList);
	#endif
