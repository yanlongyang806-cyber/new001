...
typedef struct AuctionConfig
{
	...
	// Whether players have to be standing at an auction house contact to use its functions.
	bool bAllowContactlessAuctionHouse;
	...
	//Maximum sum of all currency in a character's inbox before a shard-level alert is triggered.
	S64 iNPCMailCurrencyAlertThreshold;
...
typedef struct AuctionBiddingInfo
{
	...
	// This is used by the ObjectDB to prevent us from offlining a character with bids outstanding. 
	// If this is moved or renamed, we must also change the objPath to it in dbOfflining:PrebuildAuctionLotStatusCB.
...
typedef struct AuctionLot {
	...
	const U32 bWasPostedRemotely : 1;		AST(PERSIST)
...
void AuctionLot_trh_GetNewLotLogDetailString(NOCONST(AuctionLot)* pLot, U32 uiLotID, char** pEstrOut);
void AuctionLot_trh_GetBidSaleLogDetailString(NOCONST(AuctionLot)* pLot, U32 uiPriceAfterFees, char** pEstrOut);
void AuctionLot_trh_GetNewBidLogDetailString(ATH_ARG NOCONST(AuctionLot)* pLot, char** pEstrOut);
void AuctionLot_trh_GetBuyoutSaleLogDetailString(NOCONST(AuctionLot)* pLot, NOCONST(Entity)* pBuyer, U32 uiPriceAfterFees, char** pEstrOut);
void AuctionLot_trh_GetGenericLogDetailString(NOCONST(AuctionLot)* pLot, U32 uiLotID, char** pEstrOut);
