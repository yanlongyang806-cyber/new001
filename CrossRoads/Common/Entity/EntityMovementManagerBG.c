...
#define CHECK_MM_BG_PROCESSINGTHREAD	1

// Processing Thread Checker
// we could add lazy initialization to this, but I'd rather not, right now
// the TlsAlloc should be happening before any sets or gets
#if CHECK_MM_BG_PROCESSINGTHREAD
	#define MM_TlsSetValue(processingThread, t) \
		devassert(processingThread);			\
		TlsSetValue(processingThread, t)
	#define MM_TlsGetValue(processingThread)	\
		(devassert(processingThread),TlsGetValue(processingThread))
#else
	#define MM_TlsSetValue(processingThread, t)	\
		TlsSetValue(processingThread, t)
	#define MM_TlsGetValue(processingThread)	\
		TlsGetValue(processingThread)
#endif
...
static DWORD WINAPI mmProcessingThreadMain(MovementProcessingThread* t)
{
	...
	MM_TlsSetValue(mgState.bg.threads.tls.processingThread, t);
										...
										MovementProcessingThreadCB cb)
{
		...
		!beaconIsBeaconizer())
	{
		...
		MM_TlsSetValue(mgState.bg.threads.tls.processingThread, NULL);
...
void mmSetAfterSimWakesOnceBG(MovementManager* mm){
	MovementThreadData* td = MM_THREADDATA_BG(mm);
	MovementProcessingThread* t = MM_TlsGetValue(mgState.bg.threads.tls.processingThread);
							...
							F32 fDistance )
{
	...
	if (CAPSULE_RADIUS < fDistance)
	{
		F32 fChecked = CAPSULE_RADIUS;
		while (fChecked <= fDistance+CAPSULE_RADIUS)
		{
			scaleAddVec3(dir, fChecked, pos, posStartCast);
