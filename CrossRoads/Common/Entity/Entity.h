...
typedef struct SharedBankData SharedBankData;
...
typedef enum GlobalEntityFlags {
	...
	ENTITYFLAG_PLAYER_DISCONNECTED = 1 << 6,	// This flag is set anytime ENTITYFLAG_PLAYER_LOGGING_IN or ENTITYFLAG_PLAYER_LOGGING_OUT is set.
	ENTITYFLAG_PLAYER_LOGGING_IN = 1 << 7,	// Entity is logging in (has transfered to a new game server)
	...
	ENTITYFLAG_CRITTERPET = 1 << 21, // This is a critter pet, and requires some special handling
...
typedef struct Entity
{
	...
	*****/
	...
	U32					mySendFlags;					AST(SERVER_ONLY)			// Flags concerning the send status of this entity. ENT_SEND_FLAG_XXX

		// The next 2 flags are separate U32s (vs the previous) as they are set within the threaded iterations of gslEntityNet.c. Otherwise
		//  we could get thread contention. They are only ever set within the threading and then cleared in gslEntityUpdateEnd(). (Which
		//  is set up to block on all the threads completing.
	
	U32					ServerClientSentThisFrame;					AST(SERVER_ONLY)
	...
	CONST_OPTIONAL_STRUCT(SharedBankData) pSharedBankData;	AST(PERSIST SUBSCRIBE SELF_ONLY)
	...
	const U32 bNeedsChatMailFixup : 1;					AST(PERSIST)
	U32 bChatMailFixupInProgress : 1;					NO_AST
...
// Needed for fixing up costumes on STO
LATELINK;
void GameSpecific_CaptainCostumeFixup(Entity* pEnt);
