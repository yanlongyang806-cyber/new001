...
Power *entity_TrayGetPower(Entity *e, TrayElem *pelem)
{
	...
	if(e->pChar && pelem)
	{
		if (e->pChar->bBecomeCritter && e->pChar->bUseBecomeCritterTray)
		{
			ppow = entity_FindBecomeCritterPower(e, pelem);
			return ppow;
		}
...
// Returns the power that should be in the TrayElem from the entity's BecomeCritter slot set
// powers list. May return NULL.
Power *entity_FindBecomeCritterPower(Entity *pEnt, TrayElem *pelem)
{
	Power *ppow = NULL;

	if (pEnt->pChar && pEnt->pChar->bBecomeCritter && pEnt->pChar->pSlotSetBecomeCritter &&
		pEnt->pChar->pSlotSetBecomeCritter->puiPowerIDs)
	{
		if (pelem->iTraySlot < eaiSize(&pEnt->pChar->pSlotSetBecomeCritter->puiPowerIDs))
		{
			U32 uPowerIndex = pelem->iTraySlot + (pelem->iTray * TRAY_SIZE_MAX_CONFIG);
			ppow = character_FindPowerByID(pEnt->pChar, pEnt->pChar->pSlotSetBecomeCritter->puiPowerIDs[uPowerIndex]);
		}
	}
