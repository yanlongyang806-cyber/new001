...
{
	if(NONNULL(pEntity) && NONNULL(pEntity->pPlayer) && NONNULL(pEntity->pPlayer->pEmailV2))
	{
		S32 i;
		U32 tm = timeSecondsSince2000();

		pEntity->pPlayer->pEmailV2->bUnreadMail = false;
		pEntity->pPlayer->pEmailV2->uNextFutureDelivery = UINT_MAX;

		for(i = 0; i < eaSize(&pEntity->pPlayer->pEmailV2->mail); ++i)
		{
			if (!pEntity->pPlayer->pEmailV2->mail[i]->bRead)
			{
				if(tm >= pEntity->pPlayer->pEmailV2->mail[i]->sentTime)
				{
					pEntity->pPlayer->pEmailV2->bUnreadMail = true;
					break;
				}
				else
				{
					MIN1(pEntity->pPlayer->pEmailV2->uNextFutureDelivery, pEntity->pPlayer->pEmailV2->mail[i]->sentTime);
				}
			}
		}
	}
...
void EntityMail_trh_CheckCurrencySum(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, const char* pchCurrencyName)
{
	int i, j;
	S64 iSum = 0;
	const char* pchPooledCurrencyName = allocFindString(pchCurrencyName);

	if(ISNULL(pEnt) || ISNULL(pEnt->pPlayer) || ISNULL(pEnt->pPlayer->pEmailV2) || !pchPooledCurrencyName)
	{
		return;
	}
	...
	for (i = 0; i < eaSize(&pEnt->pPlayer->pEmailV2->mail); i++)
	{
		if (!pEnt->pPlayer->pEmailV2->mail[i])
			continue;

		for (j = 0; j < eaSize(&pEnt->pPlayer->pEmailV2->mail[i]->ppItemSlot); j++)
		{
			NOCONST(Item)* pItem;

			if (!pEnt->pPlayer->pEmailV2->mail[i]->ppItemSlot[j])
				continue;

			pItem = pEnt->pPlayer->pEmailV2->mail[i]->ppItemSlot[j]->pItem;
			if (pItem && REF_STRING_FROM_HANDLE(pItem->hItem) == pchPooledCurrencyName)
				iSum += pItem->count;
		}
	}

	if (iSum >= g_NPCMailCurrencyAlertThreshold)
	{
		TriggerAlertf("NPCMAIL_INBOX_BULK_CURRENCY", ALERTLEVEL_WARNING, ALERTCATEGORY_GAMEPLAY, 0, GLOBALTYPE_ENTITYPLAYER, pEnt->myContainerID, GetAppGlobalType(), GetAppGlobalID(), NULL, 0, "Player ID %u has a total of %lld %s currency in his NPCMail inbox at one time.", pEnt->myContainerID, iSum, pchCurrencyName);
	}
}
...
{
	NOCONST(NPCEMailData) *pNewMail = NULL;
	bool bRet = true;
	...
	pNewMail->pReason = pReason && pReason->pcReason ? estrCreateFromStr(pReason->pcReason) : NULL;
	pNewMail->pDetail = pReason && pReason->pcDetail ? estrCreateFromStr(pReason->pcDetail) : NULL;
	...
	if (uFutureTimeSeconds <= 0)
		pEnt->pPlayer->pEmailV2->bUnreadMail = true;
	else
		MIN1(pEnt->pPlayer->pEmailV2->uNextFutureDelivery, timeSecondsSince2000() + uFutureTimeSeconds);
