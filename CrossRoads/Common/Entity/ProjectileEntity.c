...
// -------------------------------------------------------------------------------------------------------------------
void Projetile_GetPowerDefs(SA_PARAM_NN_VALID AttribModDef *pdef, 
							SA_PARAM_NN_VALID PowerDef ***peaPowerDefsToSave)
{
	ProjectileCreateParams *pParams = (ProjectileCreateParams*)pdef->pParams;

	if (pParams)
	{
		ProjectileEntityDef *pProjDef = GET_REF(pParams->hProjectileDef);
		if (pProjDef)
		{
			PowerDef *pDef;	
			pDef = GET_REF(pProjDef->hPowerOnHitEnt);
			if (pDef) eaPushUnique(peaPowerDefsToSave, pDef);
			pDef = GET_REF(pProjDef->hPowerOnCollideWorld);
			if (pDef) eaPushUnique(peaPowerDefsToSave, pDef);
			pDef = GET_REF(pProjDef->hPowerOnExpire);
			if (pDef) eaPushUnique(peaPowerDefsToSave, pDef);
			pDef = GET_REF(pProjDef->hPowerOnHitGotoEnt);
			if (pDef) eaPushUnique(peaPowerDefsToSave, pDef);
			pDef = GET_REF(pProjDef->hPowerOnDeath);
			if (pDef) eaPushUnique(peaPowerDefsToSave, pDef);
		}
	}
