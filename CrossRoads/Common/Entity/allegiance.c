...
bool allegiance_CanPlayerUseWarpAllegiance(Entity *pEnt, AllegianceDef *pAllegiance)
{
	...
	{
		if (pAllegiance->pWarpRestrict->bSkipCheckInUGC && isProductionEditMode())
		{
			return true;
		}
		if (pAllegiance->pWarpRestrict->iRequiredLevel > entity_GetSavedExpLevel(pEnt))
		{
			return false;
		}
		if (pAllegiance->pWarpRestrict->pchRequiredMission && pAllegiance->pWarpRestrict->pchRequiredMission[0])
		{
...
bool allegiance_CanPlayerUseWarp(Entity* pEnt)
{
	...
	if (pSubAllegiance && pSubAllegiance->pWarpRestrict &&
		allegiance_CanPlayerUseWarpAllegiance(pEnt, pSubAllegiance))
		return true;
...
bool allegiance_CanUseNamePrefix(AllegianceNamePrefix* pPrefix, Entity* pEnt, GameAccountData *pGameAccount)
{
	...
	if (eaSize(&pPrefix->eaClassesExcluded) > 0)
	{
		if (!pClass)
			return false;

		for (j = eaSize(&pPrefix->eaClassesExcluded) - 1; j >= 0; j--)
			if (GET_REF(pPrefix->eaClassesExcluded[j]->hClass) == pClass)
				break;
		if (j >= 0)
			return false;
	}
	...
	if (eaSize(&pPrefix->eaSpeciesExcluded) > 0)
	{
		if (!pSpecies)
			return false;

		for (j = eaSize(&pPrefix->eaSpeciesExcluded) - 1; j >= 0; j--)
			if (GET_REF(pPrefix->eaSpeciesExcluded[j]->hSpecies) == pSpecies)
				break;
		if (j >= 0)
			return false;
	}
