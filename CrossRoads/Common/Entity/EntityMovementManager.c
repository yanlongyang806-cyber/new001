...
void mmRegisterBits(void)
{
	...
	ANIM_BIT(jumpRising, "JumpRising");
	ANIM_BIT(jumpFalling, "JumpFalling");
...
// this function can be used in multiple general update threads at once, so we don't want to use a global for the keyName
			...
			STR_COMBINE_BEGIN(keyName);
			{
				EARRAY_INT_CONST_FOREACH_BEGIN(bits, i, isize);
				{
					...
					if (!b)
					{
						...
						{
							if (keyNameSpace < 3) {
								ErrorDetailsf("%u bits",eaiUSize(&bits));
								Errorf("Movement Manager attempted overflow of bit keyname buffer, trimming list to prevent crash!");
								break;
							}
							STR_COMBINE_CAT(".0");
							keyNameSpace -= 2;
					...
					else
					{
						...
						useLength = strlen(buffer+j);

						if (keyNameSpace < useLength+1) {
							ErrorDetailsf("%u bits",eaiUSize(&bits));
							Errorf("Movement Manager attempted overflow of bit keyname buffer, trimming list to prevent crash!");
							break;
						}
						STR_COMBINE_CAT(buffer + j);
						keyNameSpace -= useLength;
					}
				}
				EARRAY_FOREACH_END;
			}	
								...
								const char* prefix)
{
	...
	if (iStackOverflow > 0) {
		...
		ErrorDetailsf("%i extra values, %i capacity", iStackOverflow, eaiCapacity(&values));
		Errorf("Movement Manager: attempted %s stack overflow in %s", pcTypeName, __FUNCTION__);
	}
									...
									const char* name)
{
	...
	if (iStackOverflow) {
		ErrorDetailsf("%i extra values, %i capacity", iStackOverflow, eaiCapacity(&lastAnim.flags));
		Errorf("Movement Manager: attempted flag stack overflow in %s", __FUNCTION__);
	}
								...
								const char *pcFuncName)
{
	d->pc = s->pc;
	d->anim = s->anim;
	mmCopyAnimValueToSizedStack(&d->flags, s->flags, pcFuncName, "flag");
}
										...
										const char *pcFuncName)
{
	...
	if (iStackOverflow) {
		ErrorDetailsf("%i extra values, %i capacity", iStackOverflow, eaiCapacity(&s->flags));
		Errorf("Movement Manager: attempted flag stack overflow in %s", pcFuncName);
	}
		...
		switch(mavType){
			...
			acase MAVT_STANCE_OFF:{
				...
				if(!invert == (mavType == MAVT_STANCE_ON)){
					...
					{
						ErrorDetailsf("at least %i extra value, %i capacity", 1, eaiCapacity(stancesInOut));
						Errorf("Movement Manager: attempted stance stack overflow in %s", pcCallingFunc);
					} else {
						eaiPush(stancesInOut, stance);
					}
				}
											...
											const MovementOutput** oFirstReplayOut)
{
	...
	if(!oFirstReplay->flags.animViewedLocal){
		...
		if (iOffStackOverflow) {
			ErrorDetailsf("%i extra values, %i capacity", iOffStackOverflow, eaiCapacity(&stancesOff));
			Errorf("Movement Manager: attempted stance off stack overflow in %s", __FUNCTION__);
		}

		if (iOnStackOverflow) {
			ErrorDetailsf("%i extra values, %i capacity", iOnStackOverflow, eaiCapacity(&stancesOn));
			Errorf("Movement Manager: attempted stance on stack overflow in %s", __FUNCTION__);
		}
									...
									U32** flagsInOut)
{
	...
	if (iStackOverflow) {
		ErrorDetailsf("%i extra values, %i capacity", iStackOverflow, eaiCapacity(&flagsFromValues));
		Errorf("Movement Manager: attempted flag stack overflow in %s", __FUNCTION__);
	}
...
static void mmManagerDestroyVerifyClean(MovementManager* mm){
	...
	if (TRUE_THEN_RESET(mm->fg.flagsMutable.sentUserThreadDataUpdateToBGbit)) {
		Errorf("MM Flag Bug : Caught sentUserThreadDataUpdateToBGbit == 1 during %s", __FUNCTION__);
	}
	if (TRUE_THEN_RESET(mm->fg.flagsMutable.afterSimOnceFromBGbit)) {
		Errorf("MM Flag Bug : Caught afterSimOnceFromBGbit == 1 during %s", __FUNCTION__);
	}
									...
									const char *pcTypeName)
{
	if (eaiSize(&src) > eaiCapacity(dest))
	{
		...
		ErrorDetailsf("%i extra values, %i capacity", eaiSize(&src)-eaiCapacity(dest), eaiCapacity(dest));
		Errorf("Movement Manager: attempted %s stack overflow in %s", pcTypeName, pcFuncName);
