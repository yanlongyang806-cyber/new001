							...
							Vec3 dirOut)
{
	if(FALSE_THEN_SET(localBG->flags.inputDirIsCurrent)){
		...
		if(g_CombatConfig.fFlightPitchClamp) {
			F32 fFlightPitch = CLAMP(sync->maxPitchAngle, g_CombatConfig.fFlightPitchClampMin, g_CombatConfig.fFlightPitchClamp);
			fFlightPitch = MIN(RAD(fFlightPitch), MAX_FLIGHT_PITCH);
			dirRelative[1] *= tan(fFlightPitch);
		}
		else {
			dirRelative[1] *= tan(MAX_FLIGHT_PITCH);
		}
														...
														FlightSync* sync)
{
	...
	switch(shared->target.pos->targetType){
		xcase MPTT_INPUT:{
			...
			if(!vec3IsZero(dirInput) && dirScale != 0.0f){
				...
				{
					float dirY = dirInput[1];

					// Limited turning means you go in the direction you're facing
					createMat3_2_YP(dirInput, bg->pyrTarget);

					// If we are ignoring pitch, that means that the input direction controls vertical movement
	...
	{
		bg->flags.hasConstantForceVel = 1;
	}
	else
	{
		bg->flags.hasConstantForceVel = 0;
	}
									...
									Vec3 pyrInOut)
{
	...
	
	{
		...
		if(sync->flags.fakeroll){
			...
			F32 forwardSpeed, targetRoll, rollDiff, maxDeltaRoll, maxRoll;

			maxRoll = MR_MAX_ROLL;
			if(sync->maxBankAngle){
				maxRoll = sync->maxBankAngle;
			}

			// Calculate the max roll based on the current pitch of the ship.
			maxRoll *= cosf(pyrCur[0]);

			// Calculate the max roll angle based on the speed of movement.
			forwardSpeed = SQR(bg->vel[0]) + SQR(bg->vel[1]) + SQR(bg->vel[2]);
			targetRoll = CLAMP(yawd/0.003, -1.0, 1.0f) * (forwardSpeed/SQR(14.0f)) * maxRoll;
			targetRoll = CLAMP(targetRoll, -1 * maxRoll, maxRoll);
			...
			if(sync->bankRate){
				rollDiff *= sync->bankRate;
				maxDeltaRoll *= sync->bankRate;
			}
	...
	if(g_CombatConfig.fFlightPitchClamp){
		F32 p = MIN(RAD(g_CombatConfig.fFlightPitchClamp), MAX_FLIGHT_PITCH);
		pyrInOut[0] = CLAMP(pyrInOut[0],-p,p);
	}else{
		pyrInOut[0] = CLAMP(pyrInOut[0],-MAX_FLIGHT_PITCH,MAX_FLIGHT_PITCH);
	}
						...
						F32 bankRate)
{
	...
	if(GET_SYNC(&sync, NULL)){
		IF_DIFF_THEN_SET(sync->bankRate, bankRate);
							...
							F32 maxBankAngle)
{
	...
	if(GET_SYNC(&sync, NULL)){
		IF_DIFF_THEN_SET(sync->maxBankAngle, maxBankAngle);
							...
							F32* maxBankAngleOut)
{
	...
	if(GET_SYNC(&sync, NULL)){
		*maxBankAngleOut = sync->maxBankAngle;
							...
							F32 maxPitchAngle)
{
	...
	if(GET_SYNC(&sync, NULL)){
		IF_DIFF_THEN_SET(sync->maxPitchAngle, maxPitchAngle);
							...
							F32* maxPitchAngleOut)
{
	...
	if(GET_SYNC(&sync, NULL)){
		*maxPitchAngleOut = sync->maxPitchAngle;
