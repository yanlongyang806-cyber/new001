...

{
	...
	if (playerCostume)
	{
		...
		if (ragdollData)
		{
			pCostume = costumeGenerate_CreateWLCostume(playerCostume, e->pChar ? GET_REF(e->pChar->hSpecies) : NULL, e->pChar ? GET_REF(e->pChar->hClass) : NULL, GET_REF(e->hAllegiance),
				GET_REF(e->hSubAllegiance), NULL, costumeEntity_GetActiveSavedSlotType(e), GET_REF(e->costumeRef.hMood), NULL, "Entity.", e->myEntityType, e->myContainerID,
				e->pPlayer != NULL, NULL); // Use container ID
