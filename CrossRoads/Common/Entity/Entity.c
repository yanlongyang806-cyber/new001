...
void entGetPosDir(Entity *e, Vec3 vOutputPos, Vec3 vOutputDir)
{
	...
	{
		Vec2 pyFace;
		WorldRegionType eRegionType = entGetWorldRegionTypeOfEnt(e);
		entGetFacePY(e,pyFace);
		createMat3_2_YP(vOutputDir, pyFace);

		if (!SAFE_MEMBER2(e, pPlayer, bUseFacingPitch) &&
			eRegionType != WRT_Space && eRegionType != WRT_SectorSpace)
		{
			vOutputDir[1] = 0;
		}
...
{	
	...
	if (gbEntSystemInitted) 
	{
		...
		while(e = EntityIteratorGetNext(iter))
		{
			...
			if( partitionIdx != PARTITION_ENT_BEING_DESTROYED ) {
				mmSetWorldColl(	e->mm.movement, worldGetActiveColl(partitionIdx));
			}
		}
...
void entGetLocalBoundingBox(Entity* e, Vec3 vBoundMin, Vec3 vBoundMax, bool bGetTargetingBounds)
{
	...
	if (pSkeleton && (!pPCSkel || !pPCSkel->bUseCapsuleBoundsForTargeting || !bGetTargetingBounds))
	{
		...
		dynSkeletonGetVisibilityExtents(pSkeleton, vBoundMin, vBoundMax, true);
...
void DEFAULT_LATELINK_GameSpecific_CaptainCostumeFixup(Entity* pEnt)
{
...
// walks up the chain of entity's erOwners until it gets to one that doesn't have an owner
// will return given entity if there is no owner
Entity *entGetRootOwner(Entity *e)
{
	if (e && e->erOwner)
	{
...
void entity_UpdateTargetableNodes( Entity* ent )
{
	...
	if (gConf.bUseGlobalExpressionForInteractableGlowDistance)
	{
		...
		if(!context)
		{
			...
			s_FuncTable = exprContextCreateFunctionTable("Entity_UpdateTargetableNodes");
...
SA_RET_OP_VALID Entity* entity_GetSubEntity(int iPartitionIdx, Entity* pOwner, GlobalType iType, ContainerID iContainerID)
{
	...
	// This will subscribe this pet and all other pets (or puppets) as required
	SavedPet_SubscribeThisPet(pOwner, iType, iContainerID);
...
// Can this entity be be renamed?
bool Entity_PetCanBeRenamed(Entity *pEntPet)
{
	if(pEntPet)
	{
		PetDef* pDef = pEntPet->pCritter ? GET_REF(pEntPet->pCritter->petDef) : NULL;

		if(pDef && !pDef->bPermanentName)
		{
			// This type of pet can be re-named
			return true;
		}
	}
...
// Is this is a pet and and does it have this name
bool Entity_IsPetDefNamed(Entity *pPetEnt, const char *pName)
{
	if(pPetEnt && pPetEnt->pCritter && pName)
	{
		PetDef *pPetDef = GET_REF(pPetEnt->pCritter->petDef);
		if(pPetDef && stricmp(pPetDef->pchPetName, pName) == 0)
		{
			return true;
		}
	}
...
const char *OVERRIDE_LATELINK_ParserGetTableName_WithEntityFixup(ParseTable *table, void *pObject)
{
	if (table == parse_Entity && pObject)
	{
		return GlobalTypeToName(((Entity*)pObject)->myEntityType);
	}
