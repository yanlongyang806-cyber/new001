...
//   RegionRules should really have some sort of refactor done to them.
// Currently they are referenced from WorldRegion (WorldGridPrivate.h) via a type field of WorldRegionType (which is an enum in WorldLibEnums.h)
// This necessarily means that we there can be only one RegionRules per type in the enum and if a game wishes to make new ones it has to add
// to the enum. In addition, WorldRegion is part of WorldLib, whereas RegionRules is not. Any hookup between the two has to be indirect (as it currently
// is via the enum).
//   What we really would like would be a way to specify mutliple RegionRules on a project independently of the other projects. And possibly have multiple
// region rules that reference the same type.
// -Various systems in WorldLib currently are hardcoded to use this type directly. Typically to distinguish Space/Ground in STO. These are all essentially
//  hacks and a big part of the cleanup would be to remove these hacks and make the various systems acquire their data in a more direct fashion. Anything that
//  references WRT_Space, etc. directly is suspect.
// -Note that there is also a WorldRegionRules (unrelated to RegionRules) that exists in WorldLib and is also referenced with the same
// type field in WorldRegion. This means we have to have one of these for every RegionRules in the RegionRules.def file. This is unfortunate.
// - RegionRules are also a mishmash of various things used by different systems in different ways. It may be virtuous to remove some things out
// to WorldRegionRules, or other places.
...
// These functions returns the first RegionRules which fit the criteria for each function.
// There will be problems if multiple regions use different settings
//  on different regions that all meet the criteria
RegionRules* regionRules_GetAnyRegionRulesValidForPower(PowerDef *pDef);
RegionRules *regionRules_GetAnyRegionRulesValidForSchemeRegionType(S32 eSchemeRegionType);
