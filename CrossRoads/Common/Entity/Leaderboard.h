...
typedef struct LeaderboardDef {
    ...
    // Time time interval over which we calculate leaders
	...
	////  If it is not "Off" use the appropriate ShardInterval timing. 
		...
		// Shard-clock-based interval timing. Uses pStats->iLastTimestamp to determine availability.
		ShardIntervalTimingMode eLeaderboardIntervalMode;		AST(NAME(IntervalMode))
