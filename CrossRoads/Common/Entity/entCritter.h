...
typedef struct CritterCostume
{
	...
	// If 0 or greater, this costume is only available in a particular
	// AssetSet.
	int		iAssetSetIndex;				AST(DEF(-1))
...
typedef struct PetDef
{
	...
	U32 bPermanentName : 1;					AST(NAME(PermanentName))
		// This pet has a name that can not be changed
	U32 bAlwaysSubscribe : 1;				AST(NAME(AlwaysSubscribe))
		// This pet must always be subscribe
...
// structure used for tracking when a rollover is attempting to be collected by a player
AUTO_STRUCT;
typedef struct RolloverTracker
{
	// the entity that attempted to pickup
	EntityRef erRef;
	
	// the next time it is valid for the entity to attempt pickup
	U32	uiNextPickupAttemptTime;
	
	// the number of successive attempts, used to increase attempt timeouts
	U8 attemptCount;
	
	// if the entity was in proximity the last check
	U32 bWasInProximity : 1;

	// if they were standing on it too long, just ignore them 
	// this will get cleared if they move off
	U32 bLockedAttempts : 1;
...
typedef struct Critter
{
	...
	// The power ID of the EntCreate that created this critter. If not EntCreated, will be 0.
	U32 uiEntCreatePowerID;

	// The index in the PowerDef of the AttribMod in the EntCreate that created this critter. If not EntCreated, will be 0.
	...
	RolloverTracker **eaRolloverTrackers;	AST(SERVER_ONLY)
