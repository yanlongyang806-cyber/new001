										...
										const RecordedEntityPos* recPos)
{
	...
	if (iFlagStackOverflow) {
		ErrorDetailsf("%i extra values, %i capacity", iFlagStackOverflow, eaiCapacity(&flags));
		Errorf("Movement Manager: attempted flag stack overflow in %s", __FUNCTION__);
	}

	if (iStanceStackOverflow) {
		ErrorDetailsf("%i extra values, %i capacity", iStanceStackOverflow, eaiCapacity(&stances));
		Errorf("Movement Manager: attempted stance stack overflow in %s", __FUNCTION__);
	}
