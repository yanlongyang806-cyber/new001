...
extern GatewaySession **g_eaSessions;
#endif
...
void createEntityReport(char **pestrBuffer)
{
	...
	// Iterate entities in the player container subscription dictionary
	{
		...
		while( true ) {
			U32 iNumRefs;
			ReferentInfoStruct *pinfo = RefSystem_GetNextReferentInfoFromIterator(&iter);

			if(!pinfo)
				break;

			iNumRefs = eSetGetCount(&pinfo->setHandles);
			pEntity = pinfo->pReferent;
			pcName = pinfo->pOpaqueRefData;
			...
			if (eaFindAndRemoveFast(&eaEntities, pEntity) < 0) {
				...
				estrConcatf(pestrBuffer, "    Subscribed player '%s' (%d refs)", pEntity->debugName, iNumRefs);
			}
			++iNumSubPlayer;
...
#ifdef GAMESERVER
	// Iterate entities in the offline pet list
	if(IsGatewayServer() && eaSize(&g_eaSessions) > 0)
	{
		estrConcatf(pestrBuffer, "GATEWAY OFFLINE ENTITIES:\n");
		for(i=0; i<eaSize(&g_eaSessions); ++i) {
			GatewaySession *psess = g_eaSessions[i];
			if(eaSize(&psess->ppContainers) > 0) {
				int j;
				for(j=0; j<eaSize(&psess->ppContainers); ++j) {
					pEntity = psess->ppContainers[j]->pOfflineCopy;
					if(pEntity) {
						// Remove the entity from the working list
						if (eaFindAndRemoveFast(&eaEntities, pEntity) < 0) {
							estrConcatf(pestrBuffer, "    **Warning: Entity '%s' is in Gateway offline list but is not being tracked\n", pEntity->debugName);
						} else {
							estrConcatf(pestrBuffer, "    Gateway offline '%s'\n", pEntity->debugName);
						}
						++iNumGatewayOffline;
					}
				}			
	...
	estrConcatf(pestrBuffer, "    %d Gateway offline entities\n", iNumGatewayOffline);
