...
static ExprContext *CharacterPath_GetContext(void)
{
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("CharacterPath");
...
static ExprContext *CharacterClass_GetContext(void)
{
	...
	if(!s_pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("CharacterClass");
...
void entity_trh_GetChosenCharacterPaths(ATH_ARG NOCONST(Entity) *pEnt, CharacterPath*** peaPathsOut)
{
	if (NONNULL(pEnt) && NONNULL(pEnt->pChar) && GET_REF(pEnt->pChar->hPath))
	{
		...
		if (pPath)
			...
			Errorf("Entity had a chosen character path with a non-existent def %s.", REF_HANDLE_GET_STRING(pEnt->pChar->hPath));
		...
		for (i = 0; i < eaSize(&pEnt->pChar->ppSecondaryPaths); i++)
		{
			...
			if (pPath)
				...
				Errorf("Entity had a chosen character path with a non-existent def %s.", REF_HANDLE_GET_STRING(pEnt->pChar->ppSecondaryPaths[i]->hPath));
...
AUTO_EXPR_FUNC(entityutil);
int EntIsClassCategory(SA_PARAM_OP_VALID Entity *entity, const char *classCategoryName)
{
	int r = 0;
	if(entity && entity->pChar && classCategoryName && IS_HANDLE_ACTIVE(entity->pChar->hClass))
	{
		CharacterClass *pClass = GET_REF(entity->pChar->hClass);
...
AUTO_EXPR_FUNC(entityutil);
bool EntDoesHaveCharacterPath(SA_PARAM_OP_VALID Entity *entity, const char *characterPathName)
{
	if (entity)
	{
		return entity_HasCharacterPath(entity, characterPathName);
	}
