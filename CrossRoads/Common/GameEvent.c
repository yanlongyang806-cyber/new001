	...
	for (i = 0; i < n; i++)
	{
		...
		switch (i)
		{
			...
			xcase EventType_UGCAccountChanged:
				...
				info->hasSourcePlayer = true;
			xcase EventType_PowerActivation:
				info->pchDescription = "An entity has activated a power";
				info->hasSourceCritter = true;
				info->hasSourcePlayer = true;
				info->hasSourcePlayerTeam = true;
				info->hasTargetCritter = true;
				...
				info->hasVolumeName = true;
				info->hasMap = true;
			xcase EventType_PowerActivationEnded:
				info->pchDescription = "An entity's power activation has ended";
				info->hasSourceCritter = true;
				info->hasSourcePlayer = true;
				info->hasSourcePlayerTeam = true;
				info->hasTargetCritter = true;
				...
				info->hasVolumeName = true;
				info->hasMap = true;
			xcase EventType_PvPPlayerPointsAwarded:
				info->pchDescription = "Sends when a player is awarded points in a PvP match";
				info->hasSourcePlayer = true;
				info->hasSourcePlayerTeam = true;
				info->hasMap = true;
			xdefault:
...
static GameEvent* gameevent_FastCopyListener(const GameEvent *pSrc)
{
	...
	if (ev->pchPowerCategoryName) ev->pchPowerCategoryName = StructAllocString(ev->pchPowerCategoryName);
	if (ev->pchPowerTagName) ev->pchPowerTagName = StructAllocString(ev->pchPowerTagName);
