...
// Extract the allegiance name used for character slot computations from the extra header fields.
const char *
Login2_GetSubAllegianceFromCharacterChoice(Login2CharacterChoice *characterChoice)
{
	return GetExtraStringFromCharacterChoice(characterChoice, gConf.iCharacterChoiceExtraHeaderForSubAllegiance);
}
...
Login2_UGCGetSeriesMaxSlots(GameAccountData *gameAccountData)
{
	...
	extraMapCountValue = gameAccountData ? AccountProxyFindValueFromKeyContainer(gameAccountData->eaAccountKeyValues, GetAccountUgcProjectSeriesExtraSlotsKey()) : 0;
    extraMapSlots = (extraMapCountValue ? atoi(extraMapCountValue) : 0);
