...
char *UGCExportImport_SearchInit(bool includeSaved, bool includePublished, UGCProjectSearchInfo *pUGCProjectSearchInfo)
{
	...
	pParams = Py_BuildValue("(iis)", includeSaved, includePublished, NULL_TO_EMPTY(estrUGCProjectSearchInfo));
		...
		const char *strPreviousShard, const char *strComment, bool forceDelete)
{
	...
	pParams = Py_BuildValue("(sssssi)", estrUGCProject, NULL_TO_EMPTY(estrUGCProjectDataPublished), NULL_TO_EMPTY(estrUGCProjectDataSaved),
...
void UGCExportImport_ProgressBar(int x, int n, int r, int w)
{
	int c;
	int i;
	if(x == n)
...
AUTO_STARTUP(UGCExportImport) ASTRT_DEPS(UGCTags);
void UGCExportImport_Startup(void)
{
}
