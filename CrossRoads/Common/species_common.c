...
static void validateSpecies(SpeciesDef *pDef)
{
	...
	if (pDef->pcUnlockCode) {
		char *pchGameTitle = NULL;
		MicroItemType eType = kMicroItemType_None;
		char *pchItemName = NULL;
		S32 iCount = 0;
		char *pcUnlockCode = NULL;

		strdup_alloca(pcUnlockCode, pDef->pcUnlockCode);
		iCount = MicroTrans_TokenizeItemID(pcUnlockCode, &pchGameTitle, &eType, &pchItemName);

		if (pchGameTitle && stricmp(GetShortProductName(), pchGameTitle)) {
			ErrorFilenamef(pDef->pcFileName, "Species '%s' uses unlock code with game product prefix '%s' instead of the game product prefix '%s'", pDef->pcName, pchGameTitle, GetShortProductName());
		}
	}
}
...
static void species_GetAvailableSpeciesList(GameAccountData *pGameAccountData, AllegianceDef *pAllegiance, AllegianceDef *pSubAllegiance, CharacterClass *pCharClass, const char *pcSpeciesName, PCSkeletonDef *pSkeleton, SpeciesDef ***peaSpecies)
{
	...
	SpeciesDef *pSpecies;
	int i, j;
	S64 iIgnoreUnlockTimestamps = gad_GetAccountValueInt(pGameAccountData, "Test.IgnoreSpeciesUnlockTimestamps");
	...
	if (pCharClass && eaSize(&pCharClass->eaPermittedSpecies)) {
		...
		for (i=eaSize(&pCharClass->eaPermittedSpecies)-1; i>=0; --i) {
			...
			if ((pSpecies->eRestriction & kPCRestriction_Player) && !(pSpecies->eRestriction & kPCRestriction_Player_Initial)) {
				...
				if (!Species_IsUnlocked(pSpecies, pGameAccountData)) {
					continue;
				}
			}

			// Too soon
			if( (pSpecies->uUnlockTimestamp != 0) && (pSpecies->uUnlockTimestamp > timeServerSecondsSince2000()) && !iIgnoreUnlockTimestamps )
				continue;
...
AUTO_EXPR_FUNC(Player,UIGen) ACMD_NAME(PlayerGetSpeciesName);
...
AUTO_EXPR_FUNC(Player,UIGen) ACMD_NAME(PlayerGetSpeciesSpeciesName);
const char* exprPlayerGetSpeciesSpeciesName(ExprContext *pContext)
{
	Entity* pEnt = exprContextGetVarPointerUnsafe(pContext, "Player");
	Character *pChar = pEnt ? pEnt->pChar : NULL;
	SpeciesDef *pSpeciesDef = pChar ? GET_REF( pChar->hSpecies ) : NULL;
	return pSpeciesDef ? pSpeciesDef->pcSpeciesName : "";
}

bool Species_IsUnlocked(const SpeciesDef*pSpecies, GameAccountData *pGAD)
{
	if(pSpecies && pGAD)
	{
		if(!pSpecies->pcUnlockCode || !(*pSpecies->pcUnlockCode))
		{
			// No unlock code, always available
			return true;
		}

		// Gad attribute
		if(gad_GetAttribInt(pGAD, pSpecies->pcUnlockCode) > 0)
		{
			return true;
		}

		// try game permission
		if(GamePermission_HasTokenType(pGAD, kGameToken_Unlock, pSpecies->pcUnlockCode))
		{
			return true;
		}
	}
