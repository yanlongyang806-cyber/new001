...
AUTO_STRUCT;
typedef struct CostumeEditDyePackStructList
{
	CostumeEditDyePackStruct **eaDyePack;
} CostumeEditDyePackStructList;
...
typedef struct PCGeometryDef {
	...
	// Used for FX node aliasing (so weapons can goto different spots)
	const char *pcAnimNodeAliasList;			AST( POOL_STRING )

	// Used to tweak skeletal node transforms based on the geometry
	const char *pcAnimNodeAuxTransformList;		AST( POOL_STRING )
...
typedef struct CostumeRefForSet {
	...
	const char **eapcSlotTypes;					AST(POOL_STRING NAME(SlotType))
...
// in several places.
AUTO_STRUCT AST_CONTAINER;
typedef struct PCFlightControls {
	const bool bDisableBanking;				AST( PERSIST SUBSCRIBE NAME("DisableBanking"))
	const F32 fBankRateFlying;				AST( PERSIST SUBSCRIBE NAME("BankRateFlying"))
	const F32 fBankMaxAngleFlying;			AST( PERSIST SUBSCRIBE NAME("BankMaxAngleFlying"))
} PCFlightControls;
...
typedef struct PCPart {
	...
	// Used for FX node aliasing (so weapons can goto different spots)
	CONST_STRING_POOLED pchAnimNodeAliasList;
} PCPart;
...
typedef struct PCArtistCostumeData {
	...
	CONST_OPTIONAL_STRUCT(PCFlightControls) pFlightControls; AST( PERSIST SUBSCRIBE NAME("FlightControls") )
